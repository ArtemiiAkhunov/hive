...
public class TestNullTreatment
{
    ...
    {
        ASTNode tree = parseDriver.parse("select last_value(b) respect nulls over(partition by a) from t1", null).getTree();

        Assert.assertEquals(String.format(EXPECTED_RESULT_BASE, "TOK_RESPECT_NULLS"), ((ASTNode) tree.getChild(0)).dump());
    }
    ...
    {
        ASTNode tree = parseDriver.parse("select last_value(b) ignore nulls over(partition by a) from t1", null).getTree();

        Assert.assertEquals(String.format(EXPECTED_RESULT_BASE, "TOK_IGNORE_NULLS"), ((ASTNode) tree.getChild(0)).dump());
    }
