...
public class TestParseTrim
{
    ...
    {
        ASTNode tree = parseDriver.parseSelect("select trim('f' from 'foo  bar')", null).getTree();
    ...
    {
        ASTNode tree = parseDriver.parseSelect("select trim(leading 'fo' from 'foo  bar')", null).getTree();
    ...
    {
        ASTNode tree = parseDriver.parseSelect("select trim(trailing 'fo' from 'foo  bar')", null).getTree();
    ...
    {
        ASTNode tree = parseDriver.parseSelect("select trim(both 'rfo' from 'foo  bar')", null).getTree();
    ...
    {
        ASTNode tree = parseDriver.parseSelect("select trim('  foo  bar ')", null).getTree();

        assertTrimFunction(tree, "trim", "'  foo  bar '");
    }

    private void assertTrimFunction(ASTNode tree, String expectedFunctionName, String expectedStringToTrim)
    {
        assertTrimFunction(tree, expectedFunctionName, expectedStringToTrim, 2);
    }

    private void assertTrimFunction(ASTNode tree, String expectedFunctionName, String expectedStringToTrim, String expectedTrimChars)
    {
    ...
    private ASTNode assertTrimFunction(ASTNode tree, String expectedFunctionName, String expectedStringToTrim, int functionNodeChildCount)
    {
    ...
    public void testParseWhenFunctionHasTrimLikeStructure()
    {
        ...
        {
            Assert.assertEquals("Failure didn't match.", "line 1:13 cannot recognize input near 'both' ''rfo'' 'from' in function specification", ex.getMessage());
        }
    }
}
