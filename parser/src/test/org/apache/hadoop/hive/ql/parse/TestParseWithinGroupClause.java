...
public class TestParseWithinGroupClause
{
    ...
    {
        ASTNode tree = parseDriver.parseSelect("SELECT percentile_cont(0.4) WITHIN GROUP (ORDER BY val) FROM src", null).getTree();
    ...
    {
        ASTNode tree = parseDriver.parseSelect("SELECT rank(3, 4) WITHIN GROUP (ORDER BY val, val2) FROM src", null).getTree();
        ASTNode selExprNode = (ASTNode) tree.getChild(0);
        ASTNode functionNode = (ASTNode) selExprNode.getChild(0);
