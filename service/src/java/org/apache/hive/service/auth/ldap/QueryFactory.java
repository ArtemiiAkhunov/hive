...
final class QueryFactory
{
    ...
    {
        return Query.builder().filter("(&(objectClass=<groupClassAttr>)(<guidAttr>=<groupID>))").map("guidAttr", guidAttr).map("groupClassAttr", groupClassAttr).map("groupID", groupId).limit(2).build();
    }
    ...
    public Query findUserDnBySearch(String user, String userSearchFilter)
    {
        ...
        return Query.builder().filter(userSearchFilter).build();
    }
    ...
    public Query findDnByUserAndGroupSearch(String user, String userDn, String groupSearchFilter)
    {
        ...
        return Query.builder().filter(groupSearchFilter).build();
    }
    ...
    public Query isUserMemberOfGroup(String userId, String groupDn)
    {
        ...
        return Query.builder().filter("(&(|<classes:{ class |(objectClass=<class>)}>)" + "(<userMembershipAttr>=<groupDn>)(<guidAttr>=<userId>))").map("classes", USER_OBJECT_CLASSES).map("guidAttr", guidAttr).map("userMembershipAttr", userMembershipAttr).map("userId", userId).map("groupDn", groupDn).limit(2).build();
    }
