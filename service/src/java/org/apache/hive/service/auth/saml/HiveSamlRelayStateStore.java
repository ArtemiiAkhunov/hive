...
public class HiveSamlRelayStateStore implements ValueGenerator
{
    ...
    private final Cache<String, HiveSamlRelayStateInfo> relayStateCache = CacheBuilder.newBuilder()
    ...
    {
        Optional<String> portNumber = webContext.getRequestHeader(HiveSamlUtils.SSO_TOKEN_RESPONSE_PORT);
        if (!portNumber.isPresent())
        {
            throw new RuntimeException("SAML response port header " + HiveSamlUtils.SSO_TOKEN_RESPONSE_PORT + " is not set ");
        }
        ...
        webContext.setResponseHeader(HiveSamlUtils.SSO_CLIENT_IDENTIFIER, relayStateInfo.getClientIdentifier());
    ...
    public synchronized boolean validateClientIdentifier(String relayStateKey, String clientIdentifier)
    {
        ...
        LOG.debug("Validating client identifier {} with {}", clientIdentifier, relayStateInfo.getClientIdentifier());
