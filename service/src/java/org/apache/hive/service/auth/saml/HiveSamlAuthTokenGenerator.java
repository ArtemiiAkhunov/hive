...
public class HiveSamlAuthTokenGenerator implements ISAMLAuthTokenGenerator
{
    ...
    {
        ttlMs = conf.getTimeVar(HiveConf.ConfVars.HIVE_SERVER2_SAML_CALLBACK_TOKEN_TTL, TimeUnit.MILLISECONDS);
    }
    ...
    public String validate(String encodedToken) throws HttpSamlAuthenticationException
    {
        ...
        String tokenStr = getTokenStr(keyValue.get(USER), keyValue.get(ID), keyValue.get(CREATE_TIME), keyValue.get(RELAY_STATE));
        ...
        if (isExpired(System.currentTimeMillis(), Long.parseLong(keyValue.get(CREATE_TIME))))
        {
    ...
    {
        LOG.debug("Checking if the token is expired or not. " + "CurrentTime = {}, tokenExpiryTime = {} TTL = {}", currentTime, tokenTime, ttlMs);
