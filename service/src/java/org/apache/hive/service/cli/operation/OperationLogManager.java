...
public class OperationLogManager
{
    ...
    private void initHistoricOperationLogRootDir()
    {
        ...
        String logRootDir = new StringBuilder(historicParentLogDir).append("/").append(serverInstance).append("_").append(System.currentTimeMillis()).toString();
    ...
    public void deleteHistoricQueryLogs()
    {
        ...
        Set<String> sessionIds = operationManager.getHistoricalQueryInfos().stream().map(QueryInfo::getSessionId).collect(Collectors.toSet());
        Set<String> queryIds = operationManager.getHistoricalQueryInfos().stream().map(queryInfo -> queryInfo.getQueryDisplay().getQueryId()).collect(Collectors.toSet());
    ...
    private void deleteElderLogRootDirs()
    {
        File[] children = new File(historicParentLogDir).listFiles(new FileFilter()
        {
            ...
            {
                return child.isDirectory() && child.getName().startsWith(serverInstance) && !child.getAbsolutePath().equals(historicLogRootDir);
            }
        });
    ...
    private class OperationLogDirCleaner extends Thread
    {
        ...
        {
            long checkInterval = HiveConf.getTimeVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_HISTORIC_OPERATION_LOG_CHECK_INTERVAL, TimeUnit.MILLISECONDS);
    ...
    public static void closeOperation(Operation operation)
    {
        ...
        if (hiveConf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_HISTORIC_OPERATION_LOG_ENABLED) && operation instanceof SQLOperation)
        {
            ...
            String histOpLogFileLocation = new StringBuilder(historicLogRootDir).append("/").append(sessionHandle).append("/").append(queryId).toString();
            try
            {
