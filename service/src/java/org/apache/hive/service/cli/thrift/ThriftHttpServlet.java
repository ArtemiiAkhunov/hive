...
public class ThriftHttpServlet extends TServlet
{
    ...
    public ThriftHttpServlet(TProcessor processor, TProtocolFactory protocolFactory, UserGroupInformation serviceUGI, UserGroupInformation httpUGI, HiveAuthFactory hiveAuthFactory, HiveConf hiveConf) throws Exception
    {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try
        {
            ...
            if (clientUserName == null)
            {
                ...
                } else
                {
                    ...
                    } else
                    {
                        ...
                        if (!proxyHeader.equals("") && request.getHeader(proxyHeader) != null)
                        { //Trusted header is present, which means the user is already authorized.
            ...
            if (requireNewCookie && !authType.isEnabled(HiveAuthConstants.AuthTypes.NOSASL))
            {
        ...
        } catch (HttpAuthenticationException e)
        {
            ...
            if (e instanceof HttpEmptyAuthenticationException && authType.isEnabled(HiveAuthConstants.AuthTypes.KERBEROS))
            {
            ...
            } else
            {
                try
                {
                    LOG.error("Login attempt is failed for user : " + getUsername(request) + ". Error Messsage :" + e.getMessage());
                } catch (Exception ex)
                {
                    // Ignore Exception
                }
     ...
     */
    private boolean needsRedirect(HttpServletRequest request, HttpServletResponse response)
    {
     ...
     */
    private void doSamlRedirect(HttpServletRequest request, HttpServletResponse response) throws HttpSamlAuthenticationException
    {
     ...
     */
    private String doSamlAuth(HttpServletRequest request, HttpServletResponse response) throws HttpAuthenticationException
    {
        String token = extractBearerToken(request, response);
        if (token == null)
        {
        ...
        if (HiveSamlAuthTokenGenerator.parse(token, keyValues))
        {
            ...
            if (!HiveSamlRelayStateStore.get().validateClientIdentifier(relayStateKey, clientIdentifier))
            {
                throw new HttpSamlAuthenticationException("Client identifier could not be validated");
            }
        }
        return user;
    }
        ...
        public String run() throws HttpAuthenticationException
        {
            ...
            } catch (GSSException e)
            {
                if (gssContext != null)
                {
                    try
                    {
                        LOG.error("Login attempt is failed for user : " + getPrincipalWithoutRealmAndHost(gssContext.getSrcName().toString()) + ". Error Messsage :" + e.getMessage());
                    } catch (Exception ex)
                    {
                        // Ignore Exception
                    }
    ...
    private String getUsername(HttpServletRequest request) throws HttpAuthenticationException
    {
    ...
    private String getPassword(HttpServletRequest request) throws HttpAuthenticationException
    {
        String authHeaderDecodedString = getAuthHeaderDecodedString(request);
        String[] creds = authHeaderDecodedString.split(":", 2);
    ...
    public boolean isAuthTypeEnabled(HttpServletRequest request, HiveAuthConstants.AuthTypes authType)
    {
        ...
        if (hasAuthScheme(request, HttpAuthUtils.BASIC) && this.authType.isPasswordBasedAuth(authType) && this.authType.isEnabled(authType))
        {
            // The "Authorization: Basic" scheme indicates password-based authentication.
            return true;
        }
        if (this.authType.isEnabled(HiveAuthConstants.AuthTypes.KERBEROS) && authType == HiveAuthConstants.AuthTypes.KERBEROS && (StringUtils.isNotEmpty(request.getHeader(HIVE_DELEGATION_TOKEN_HEADER)) || hasAuthScheme(request, HttpAuthUtils.NEGOTIATE)))
        {
            // Hive delegation token or "Authorization: Negotiate" scheme indicates Kerberos
            // authentication.
            return true;
        }
        if (this.authType.isEnabled(HiveAuthConstants.AuthTypes.SAML) && authType == HiveAuthConstants.AuthTypes.SAML && (StringUtils.isNotEmpty(request.getHeader(HiveSamlUtils.SSO_TOKEN_RESPONSE_PORT)) || hasAuthScheme(request, HttpAuthUtils.BEARER) && !hasJWT(request)))
        {
            // X-Hive-Token-Response-Port header or "Authorization: Bearer <SAML TOKEN>" scheme
            // indicates SAML authentication.
            return true;
        }
        if (this.authType.isEnabled(HiveAuthConstants.AuthTypes.JWT) && authType == HiveAuthConstants.AuthTypes.JWT && hasAuthScheme(request, HttpAuthUtils.BEARER) && hasJWT(request))
        {
