...
public class SessionManager extends CompositeService
{
    ...
    public static final String INACTIVE_ERROR_MESSAGE = "Cannot open sessions on an inactive HS2 instance, " + "or the HS2 server leader is not ready; please use service discovery to " + "connect the server leader again";
    ...
    public synchronized void init(HiveConf hiveConf)
    {
        ...
        if (hiveConf.getBoolVar(ConfVars.HIVE_SERVER2_SUPPORT_DYNAMIC_SERVICE_DISCOVERY) && !hiveConf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_ACTIVE_PASSIVE_HA_ENABLE) && hiveConf.getBoolVar(ConfVars.HIVE_ZOOKEEPER_KILLQUERY_ENABLE))
        {
    ...
    {
        return createSession(null, protocol, username, password, ipAddress, sessionConf, withImpersonation, delegationToken).getSessionHandle();
    }
    ...
    private void incrementConnections(final String username, final String ipAddress, final List<String> forwardedAddresses) throws HiveSQLException
    {
    ...
    private String anyViolations(final String username, final String ipAddress)
    {
        ...
        if (trackConnectionsPerUserIpAddress(username, ipAddress) && !withinLimits(username + ":" + ipAddress, userIpAddressLimit))
        {
            return "Connection limit per user:ipaddress reached (user:ipaddress: " + username + ":" + ipAddress + " limit: " + userIpAddressLimit + ")";
        }
    ...
    {
        return userIpAddressLimit > 0 && username != null && !username.isEmpty() && ipAddress != null && !ipAddress.isEmpty();
    }
    ...
    {
        List<HiveSessionHook> sessionHooks = HookUtils.readHooksFromConf(hiveConf, HookContext.HookType.HIVE_SERVER2_SESSION_HOOK);
