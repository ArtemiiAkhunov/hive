...
public abstract class ThriftCLIService extends AbstractService implements TCLIService.Iface, Runnable
{
    ...
    public TExecuteStatementResp ExecuteStatement(TExecuteStatementReq req) throws TException
    {
        ...
        try
        {
            ...
            {
                StringBuilder urlBuilder = new StringBuilder("The url to track the statement: ").append(hiveConf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_USE_SSL) ? "https" : "http").append("://").append(sessionManager.getHiveServer2HostName()).append(":").append(hiveConf.getIntVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_PORT)).append("/query_page.html?operationId=").append(operationHandle.getHandleIdentifier().toString());
    ...
    public TUploadDataResp UploadData(TUploadDataReq req) throws TException
    {
        ...
        {
            SessionHandle sessionHandle = new SessionHandle(req.getSessionHandle());
            OperationHandle operationHandle = cliService.uploadData(sessionHandle, req.bufferForValues(), req.getTableName(), req.getPath());
            resp.setOperationHandle(operationHandle.toTOperationHandle());
            resp.setStatus(OK_STATUS);
        } catch (Exception e)
        {
    ...
    public TDownloadDataResp DownloadData(TDownloadDataReq req) throws TException
    {
        ...
        {
            SessionHandle sessionHandle = new SessionHandle(req.getSessionHandle());
            OperationHandle operationHandle = cliService.downloadData(sessionHandle, req.getTableName(), req.getQuery(), req.getFormat(), req.getDownloadOptions());
            resp.setOperationHandle(operationHandle.toTOperationHandle());
            resp.setStatus(OK_STATUS);
        } catch (Exception e)
        {
    ...
    private String getProxyUser(String realUser, Map<String, String> sessionConf, String ipAddress) throws HiveSQLException
    {
        ...
        if (HiveAuthConstants.AuthTypes.NONE.toString().equalsIgnoreCase(hiveConf.getVar(ConfVars.HIVE_SERVER2_AUTHENTICATION)))
        {
            return proxyUser;
        }
