...
public class RetryingThriftCLIServiceClient implements InvocationHandler
{
    ...
    public static class CLIServiceClientWrapper extends CLIServiceClient
    {
        ...
        {
            return cliService.getCrossReference(sessionHandle, primaryCatalog, primarySchema, primaryTable, foreignCatalog, foreignSchema, foreignTable);
        }
    ...
    protected RetryingThriftCLIServiceClient(HiveConf conf)
    {
        ...
        retryDelaySeconds = (int) conf.getTimeVar(HiveConf.ConfVars.HIVE_SERVER2_THRIFT_CLIENT_RETRY_DELAY_SECONDS, TimeUnit.SECONDS);
    }
    ...
    protected synchronized TTransport connect(HiveConf conf) throws HiveSQLException, TTransportException
    {
        ...
        LOG.info("Connecting to {}:{} using a thrift max message of size: {}", host, port, maxThriftMessageSize);
