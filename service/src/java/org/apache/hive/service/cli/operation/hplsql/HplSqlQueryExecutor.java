...
public class HplSqlQueryExecutor implements QueryExecutor
{
    ...
    private class OperationRowResult implements RowResult
    {
        ...
        private RowSet fetch()
        {
            try
            {
                return hiveSession.fetchResults(handle, FetchOrientation.FETCH_NEXT, fetchSize, FetchType.QUERY_OUTPUT);
            } catch (HiveSQLException e)
            {
                throw new QueryException(e);
            }
        ...
        public <T> T get(int columnIndex, Class<T> type)
        {
            ...
            } else
            {
                ...
                if (type == BigDecimal.class && serdeConstants.DECIMAL_TYPE_NAME.equalsIgnoreCase(metadata(handle).columnTypeName(columnIndex)))
                {
