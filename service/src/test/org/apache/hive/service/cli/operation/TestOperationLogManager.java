...
public class TestOperationLogManager
{
    ...
    public void setUp() throws Exception
    {
        ...
        HiveConf.setVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_LOGGING_OPERATION_LOG_LOCATION, TEST_DATA_DIR + File.separator + "operation_logs");
        HiveConf.setVar(hiveConf, HiveConf.ConfVars.HIVE_AUTHORIZATION_MANAGER, "org.apache.hadoop.hive.ql.security.authorization.plugin.sqlstd.SQLStdHiveAuthorizerFactory");
    }
    ...
    {
        FakeHiveSession session = new FakeHiveSession(new SessionHandle(TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V11), new HiveConf(hiveConf));
        session.setOperationLogSessionDir(new File(HiveConf.getVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_LOGGING_OPERATION_LOG_LOCATION)));
        ...
        int readLenght = (int) HiveConf.getSizeVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_HISTORIC_OPERATION_LOG_FETCH_MAXBYTES);
    ...
    private byte[] writeBytes(File logFile, int maxBytes) throws Exception
    {
        ...
        int written = 0;
        byte[] result;
        try (FileOutputStream fos = new FileOutputStream(logFile, true); ByteArrayOutputStream baos = new ByteArrayOutputStream(maxBytes))
        {
    ...
    private class FakeHiveSession extends HiveSessionImpl
    {
        ...
        {
            super(sessionHandle, TProtocolVersion.HIVE_CLI_SERVICE_PROTOCOL_V11, "dummy", "", serverConf, "0.0.0.0", null);
        }
    }
