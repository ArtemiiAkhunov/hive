...
public class TimestampColumnVector extends ColumnVector
{
    ...
    {
        return asScratchTimestamp(elementNum1).compareTo(timestampColVector2.asScratchTimestamp(elementNum2));
    }
    ...
    {
        return timestampColVector1.asScratchTimestamp(elementNum1).compareTo(asScratchTimestamp(elementNum2));
    }
    ...
    public void stringifyValue(StringBuilder buffer, int row)
    {
        ...
        if (noNulls || !isNull[row])
        {
            ...
            if (isUTC)
            {
                LocalDateTime ts = LocalDateTime.ofInstant(Instant.ofEpochMilli(time[row]), ZoneOffset.UTC).withNano(nanos[row]);
    ...
    private void updateDataAccordingProlepticSetting() throws Exception
    {
        ...
        {
            if (time[i] >= CalendarUtils.SWITCHOVER_MILLIS)
            ...
            long millis = usingProlepticCalendar ? CalendarUtils.convertTimeToProleptic(scratchTimestamp.getTime()) : CalendarUtils.convertTimeToHybrid(scratchTimestamp.getTime());
