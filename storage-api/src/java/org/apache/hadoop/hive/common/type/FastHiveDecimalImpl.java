...
public class FastHiveDecimalImpl extends FastHiveDecimal
{
    ...
    private static final long[] powerOfTenTable = {1L,                   // 0
            10L, 100L, 1000L, 10000L, 100000L, 1000000L, 10000000L, 100000000L,           // 8
            1000000000L, 10000000000L, 100000000000L, 1000000000000L, 10000000000000L, 100000000000000L, 1000000000000000L, 10000000000000000L,   // 16
            100000000000000000L, 1000000000000000000L, // 18
    };
    ...
    private static final long MAX_HIGHWORD_DECIMAL = powerOfTenTable[HIGHWORD_DECIMAL_DIGITS] - 1;

    // 38 * 2 or 76 full decimal maximum - (64 + 8) digits in 4 lower longs (4 digits here).
    private static final long FULL_MAX_HIGHWORD_DECIMAL = powerOfTenTable[MAX_DECIMAL_DIGITS * 2 - (FOUR_X_LONGWORD_DECIMAL_DIGITS + INTWORD_DECIMAL_DIGITS)] - 1;
    ...
    public static final BigInteger BIG_INTEGER_MAX_DECIMAL = BIG_INTEGER_TEN.pow(MAX_DECIMAL_DIGITS).subtract(BigInteger.ONE);

    private static final BigInteger BIG_INTEGER_MAX_LONGWORD_DECIMAL = BigInteger.valueOf(MAX_LONGWORD_DECIMAL);

    private static final BigInteger BIG_INTEGER_LONGWORD_MULTIPLIER = BigInteger.ONE.add(BIG_INTEGER_MAX_LONGWORD_DECIMAL);
    private static final BigInteger BIG_INTEGER_LONGWORD_MULTIPLIER_2X = BIG_INTEGER_LONGWORD_MULTIPLIER.multiply(BIG_INTEGER_LONGWORD_MULTIPLIER);
    private static final BigInteger BIG_INTEGER_MAX_HIGHWORD_DECIMAL = BigInteger.valueOf(MAX_HIGHWORD_DECIMAL);
    private static final BigInteger BIG_INTEGER_HIGHWORD_MULTIPLIER = BigInteger.ONE.add(BIG_INTEGER_MAX_HIGHWORD_DECIMAL);
    ...
    public static boolean fastSetFromBytes(byte[] bytes, int offset, int length, boolean trimBlanks, FastHiveDecimal fastResult)
    {
        ...
        if (index < end && (bytes[index] == BYTE_EXPONENT_UPPER || bytes[index] == BYTE_EXPONENT_LOWER))
        {
            haveExponent = true;
            index++;
            if (index >= end)
        ...
        if (roundingNecessary)
        {
            ...
            {
                if (!fastAdd(fastResult.fastSignum, fastResult.fast0, fastResult.fast1, fastResult.fast2, fastResult.fastIntegerDigitCount, fastResult.fastScale, fastResult.fastSignum, 1, 0, 0, 0, trailingZeroesScale, fastResult))
                {
                    return false;
                }
            }
    ...
    public static boolean fastSetFromBigDecimal(BigDecimal bigDecimal, boolean allowRounding, FastHiveDecimal fastResult)
    {
        ...
        if (!fastSetFromBigInteger(bigDecimal.unscaledValue(), bigDecimal.scale(), bigDecimal.precision(), fastResult))
        {
            return false;
        }
        return true;
    ...
    public static void fastSetFromLong(long longValue, FastHiveDecimal fastResult)
    {
        ...
        } else
        {
            ...
            {
                fastResult.fastIntegerDigitCount = LONGWORD_DECIMAL_DIGITS + fastLongWordPrecision(fastResult.fast1);
            } else
            {
                fastResult.fastIntegerDigitCount = fastLongWordPrecision(fastResult.fast0);
            }
    ...
    public static boolean fastSetFromLongAndScale(long longValue, int scale, FastHiveDecimal fastResult)
    {
        ...
        if (!fastScaleByPowerOfTen(fastResult, -scale, fastResult))
        {
            return false;
        }
        return true;
    ...
    public static boolean fastSetFromBigInteger(BigInteger bigInteger, FastHiveDecimal fastResult)
    {
        ...
        {
            fastResult.fastIntegerDigitCount = TWO_X_LONGWORD_DECIMAL_DIGITS + fastHighWordPrecision(fastResult.fast2);
        }
        return true;
    }
    ...
    public static boolean doBinaryToDecimalConversion(long lowerWord, long middleWord, long highWord, FastHiveDecimal middleWordMultiplier, FastHiveDecimal highWordMultiplier, FastHiveDecimal fastResult)
    {
        ...
        if (middleWord != 0 || highWord != 0)
        {
            ...
            if (highWord == 0)
            {
                ...
                if (!fastMultiply5x5HalfWords(middleWord % MULTIPLER_LONGWORD_DECIMAL, middleWord / MULTIPLER_LONGWORD_DECIMAL, 0, middleWordMultiplier.fast0, middleWordMultiplier.fast1, middleWordMultiplier.fast2, fastResult))
                {
                    return false;
                }
            ...
            } else if (middleWord == 0)
            {
                ...
                if (!fastMultiply5x5HalfWords(highWord % MULTIPLER_LONGWORD_DECIMAL, highWord / MULTIPLER_LONGWORD_DECIMAL, 0, highWordMultiplier.fast0, highWordMultiplier.fast1, highWordMultiplier.fast2, fastResult))
                {
                    return false;
                }
     ...
     */
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_62 = new FastHiveDecimal(1, 1686018427387904L, 461L, 0, 19, 0);
     ...
     */
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_63 = new FastHiveDecimal(1, 3372036854775808L, 922L, 0, 19, 0);
     ...
     */
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_125 = new FastHiveDecimal(1, 1825928971026432L, 9586511730793292L, 425352L, 38, 0);
     ...
     */
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_63_INVERSE = new FastHiveDecimal(1, 6994171142578125L, 5044340074528008L, 1084202172485L, 45, 0);
    ...
    public static boolean fastSerializationUtilsRead(InputStream inputStream, int scale, byte[] scratchBytes, FastHiveDecimal fastResult) throws IOException
    {
         ...
         */
        if (!doBinaryToDecimalConversion(lowerWord63, middleWord63, highWord63, FAST_HIVE_DECIMAL_TWO_POWER_62, FAST_HIVE_DECIMAL_TWO_POWER_125,    // 2^(62 + 63)
                fastResult))
        {
            return false;
        }
        ...
        if (isNegative)
        {
            ...
            if (!doAddSameScaleSameSign(
                    /* resultSignum */ 1, fastResult.fast0, fastResult.fast1, fastResult.fast2, 1, 0, 0, fastResult))
            {
                return false;
            }
        }
    ...
    public static boolean fastSerializationUtilsWrite(OutputStream outputStream, int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, long[] scratchLongs) throws IOException
    {
         ...
         */
        if (!doDecimalToBinaryConversion(adjust0, adjust1, adjust2, FAST_HIVE_DECIMAL_TWO_POWER_63_INVERSE, SERIALIZATION_UTILS_WRITE_QUOTIENT_INTEGER_WORD_NUM, SERIALIZATION_UTILS_WRITE_QUOTIENT_INTEGER_DIGIT_NUM, FAST_HIVE_DECIMAL_TWO_POWER_63, scratchLongs))
        {
            // Overflow.
            return false;
        }
     ...
     * performance.
     */
    public static void fastDeserialize64(final long inputDecimal64Long, final int inputScale, FastHiveDecimal fastResult)
    {
     ...
     */
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_56 = new FastHiveDecimal(1, 2057594037927936L, 7L, 0, 17, 0);
     ...
     */
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_112 = new FastHiveDecimal(1, 8530496329220096L, 9229685853482762L, 51L, 34, 0);
    ...
    //
    private static final FastHiveDecimal FAST_HIVE_DECIMAL_TWO_POWER_56_INVERSE = new FastHiveDecimal(1, 9585113525390625L, 8078144567552953L, 13877787L, 40, 0);
    ...
    public static boolean fastSetFromBigIntegerBytesAndScale(byte[] bytes, int offset, int length, int scale, FastHiveDecimal fastResult)
    {
        ...
        if (!doBinaryToDecimalConversion(lowerWord56, middleWord56, highWord56, FAST_HIVE_DECIMAL_TWO_POWER_56, FAST_HIVE_DECIMAL_TWO_POWER_112,    // 2^(56 + 56)
                fastResult))
        {
     ...
     * conversion failed.
     */
    public static int fastBigIntegerBytesUnscaled(final int fastSignum, long fast0, long fast1, long fast2, long[] scratchLongs, byte[] buffer)
    {
         ...
         */
        if (!doDecimalToBinaryConversion(fast0, fast1, fast2, FAST_HIVE_DECIMAL_TWO_POWER_56_INVERSE, BIG_INTEGER_BYTES_QUOTIENT_INTEGER_WORD_NUM, BIG_INTEGER_BYTES_QUOTIENT_INTEGER_DIGIT_NUM, FAST_HIVE_DECIMAL_TWO_POWER_56, scratchLongs))
        {
     ...
     */
    public static int fastBigIntegerBytesScaled(final int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int serializeScale, long[] scratchLongs, byte[] buffer)
    {
        ...
        } else
        {
            ...
            FastHiveDecimal fastTemp = new FastHiveDecimal();
            if (!fastRound(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, serializeScale, BigDecimal.ROUND_HALF_UP, fastTemp))
            {
                return 0;
            }
            return fastBigIntegerBytesUnscaled(fastSignum, fastTemp.fast0, fastTemp.fast1, fastTemp.fast2, scratchLongs, buffer);
    ...
    private static final FastHiveDecimal FASTHIVEDECIMAL_MIN_BYTE_VALUE_MINUS_ONE = new FastHiveDecimal((long) Byte.MIN_VALUE - 1L);
    private static final FastHiveDecimal FASTHIVEDECIMAL_MAX_BYTE_VALUE_PLUS_ONE = new FastHiveDecimal((long) Byte.MAX_VALUE + 1L);

    private static final int MAX_SHORT_DIGITS = 5;
    private static final FastHiveDecimal FASTHIVEDECIMAL_MIN_SHORT_VALUE_MINUS_ONE = new FastHiveDecimal((long) Short.MIN_VALUE - 1L);
    private static final FastHiveDecimal FASTHIVEDECIMAL_MAX_SHORT_VALUE_PLUS_ONE = new FastHiveDecimal((long) Short.MAX_VALUE + 1L);

    private static final int MAX_INT_DIGITS = 10;
    private static final FastHiveDecimal FASTHIVEDECIMAL_MIN_INT_VALUE_MINUS_ONE = new FastHiveDecimal((long) Integer.MIN_VALUE - 1L);
    private static final FastHiveDecimal FASTHIVEDECIMAL_MAX_INT_VALUE_PLUS_ONE = new FastHiveDecimal((long) Integer.MAX_VALUE + 1L);

    private static final FastHiveDecimal FASTHIVEDECIMAL_MIN_LONG_VALUE = new FastHiveDecimal(Long.MIN_VALUE);
    private static final FastHiveDecimal FASTHIVEDECIMAL_MAX_LONG_VALUE = new FastHiveDecimal(Long.MAX_VALUE);
    private static final int MAX_LONG_DIGITS = FASTHIVEDECIMAL_MAX_LONG_VALUE.fastIntegerDigitCount;
    private static final FastHiveDecimal FASTHIVEDECIMAL_MIN_LONG_VALUE_MINUS_ONE = new FastHiveDecimal("-9223372036854775809");
    private static final FastHiveDecimal FASTHIVEDECIMAL_MAX_LONG_VALUE_PLUS_ONE = new FastHiveDecimal("9223372036854775808");
    ...
    public static long fastLongValueClip(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale)
    {
        ...
        } else
        {
            ...
            // Do first comparison as UNSIGNED.
            if (fastCompareTo(1, result0, result1, result2, /* fastScale */ 0, FASTHIVEDECIMAL_MAX_LONG_VALUE) <= 0)
            {
                if (fastSignum == 1)
                {
            ...
            if (fastEquals(fastSignum, result0, result1, result2, /* fastScale */ 0, FASTHIVEDECIMAL_MIN_LONG_VALUE))
            {
    ...
    public static BigInteger fastBigIntegerValue(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int fastSerializationScale)
    {
        ...
        {
            return fastBigIntegerValueScaled(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, fastSerializationScale);
        } else
        {
            return fastBigIntegerValueUnscaled(fastSignum, fast0, fast1, fast2);
        }
    }
    ...
    public static boolean fastScaleByPowerOfTen(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int power, FastHiveDecimal fastResult)
    {
      ...
      */
        ...
        } else if (fastScale + absPower <= HiveDecimal.MAX_SCALE)
        {
            ...
            final int trailingZeroCount = fastTrailingDecimalZeroCount(fast0, fast1, fast2, newIntegerDigitCount, newScale);
            if (trailingZeroCount > 0)
            {
                fastResult.fastSignum = fastSignum;
    ...
    */
    ...
    private static void doClearRoundIntegerPortionAndAddOne(long fast0, long fast1, long fast2, int absRoundPower, FastHiveDecimal fastResult)
    {
        ...
        if (absRoundPower < LONGWORD_DECIMAL_DIGITS)
        {
            ...
            final long r0 = ((fast0 / roundFactor) * roundFactor) + roundFactor;
            result0 = r0 % MULTIPLER_LONGWORD_DECIMAL;
            final long r1 = fast1 + r0 / MULTIPLER_LONGWORD_DECIMAL;
        ...
        } else if (absRoundPower < TWO_X_LONGWORD_DECIMAL_DIGITS)
        {
            ...
            result0 = 0;
            final long r1 = ((fast1 / roundFactor) * roundFactor) + roundFactor;
            result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
            result2 = fast2 + r1 / MULTIPLER_LONGWORD_DECIMAL;
    ...
    public static boolean fastRoundIntegerUp(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int roundPower, FastHiveDecimal fastResult)
    {
        ...
        {
            doFastScaleDown(fast0, fast1, fast2,
                    /* scaleDown */ fastScale, fastResult);
        }
    ...
    public static boolean fastRoundFractionalUp(int fastSignum, long fast0, long fast1, long fast2, int scaleDown, FastHiveDecimal fastResult)
    {
        ...
        if (!isRoundPortionAllZeroes)
        {
            ...
            final long r1 = fastResult.fast1 + r0 / MULTIPLER_LONGWORD_DECIMAL;
            fastResult.fast1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
            fastResult.fast2 = fastResult.fast2 + r1 / MULTIPLER_LONGWORD_DECIMAL;
        }
    ...
    public static boolean fastRoundFractionalHalfUp5Words(int fastSignum, long fast0, long fast1, long fast2, long fast3, long fast4, int scaleDown, FastHiveDecimal fastResult)
    {
        ...
        if (scaleDown < LONGWORD_DECIMAL_DIGITS)
        {
            ...
            } else
            {
                ...
                result0 = r0 % MULTIPLER_LONGWORD_DECIMAL;
                final long r1 = fast1 / divideFactor + ((fast2 % divideFactor) * multiplyFactor) + r0 / MULTIPLER_LONGWORD_DECIMAL;
                result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
                final long r2 = fast2 / divideFactor + +((fast3 % divideFactor) * multiplyFactor) + r1 / MULTIPLER_LONGWORD_DECIMAL;
        ...
        } else if (scaleDown < TWO_X_LONGWORD_DECIMAL_DIGITS)
        {
            ...
            } else
            {
                ...
                result0 = r0 % MULTIPLER_LONGWORD_DECIMAL;
                final long r1 = fast2 / divideFactor + ((fast3 % divideFactor) * multiplyFactor) + r0 / MULTIPLER_LONGWORD_DECIMAL;
                result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
                final long r2 = fast3 / divideFactor + ((fast4 % divideFactor) * multiplyFactor) + r1 / MULTIPLER_LONGWORD_DECIMAL;
                result2 = r2 % MULTIPLER_LONGWORD_DECIMAL;
        ...
        } else
        {
            ...
            } else
            {
                ...
                result0 = r0 % MULTIPLER_LONGWORD_DECIMAL;
                final long r1 = fast3 / divideFactor + ((fast4 % divideFactor) * multiplyFactor) + r0 / MULTIPLER_LONGWORD_DECIMAL;
                result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
    ...
    public static FastCheckPrecisionScaleStatus fastCheckPrecisionScale(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int maxPrecision, int maxScale)
    {
    ...
    public static boolean fastUpdatePrecisionScale(final int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int maxPrecision, int maxScale, FastCheckPrecisionScaleStatus status, FastHiveDecimal fastResult)
    {
    ...
    {
        return doAddSameScaleSameSign(
                /* resultSignum */ fastLeft.fastSignum, fastLeft.fast0, fastLeft.fast1, fastLeft.fast2, fastRight.fast0, fastRight.fast1, fastRight.fast2, fastResult);
    }
    ...
    public static boolean doAddSameScaleSameSign(int resultSignum, long left0, long left1, long left2, long right0, long right1, long right2, FastHiveDecimal fastResult)
    {
        ...
        result0 = r0 % MULTIPLER_LONGWORD_DECIMAL;
        final long r1 = left1 + right1 + r0 / MULTIPLER_LONGWORD_DECIMAL;
        result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
        result2 = left2 + right2 + r1 / MULTIPLER_LONGWORD_DECIMAL;
    ...
    private static boolean doAddSameScale(int leftSignum, long leftFast0, long leftFast1, long leftFast2, int rightSignum, long rightFast0, long rightFast1, long rightFast2, int scale, FastHiveDecimal fastResult)
    {
        ...
        {
            if (!doAddSameScaleSameSign(
                    /* resultSignum */ leftSignum, leftFast0, leftFast1, leftFast2, rightFast0, rightFast1, rightFast2, fastResult))
            {
        ...
        } else
        {
            ...
            {
                if (!doSubtractSameScaleNoUnderflow(
                        /* resultSignum */ rightSignum, rightFast0, rightFast1, rightFast2, leftFast0, leftFast1, leftFast2, fastResult))
                {
                    throw new RuntimeException("Unexpected underflow");
                }
            }
    ...
    private static boolean fastAddDifferentScale(long leftFast0, long leftFast1, long leftFast2, int leftIntegerDigitCount, int leftScale, long rightFast0, long rightFast1, long rightFast2, int rightIntegerDigitCount, int rightScale, FastHiveDecimal fastResult)
    {
        ...
        if (diffScale < LONGWORD_DECIMAL_DIGITS)
        {
            ...
            result0 = r0 % MULTIPLER_LONGWORD_DECIMAL;
            final long r1 = result1 + shift0 / divideFactor + (shift1 % divideFactor) * multiplyFactor + r0 / MULTIPLER_LONGWORD_DECIMAL;
            result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
            final long r2 = result2 + shift1 / divideFactor + (shift2 % divideFactor) * multiplyFactor + r1 / MULTIPLER_LONGWORD_DECIMAL;
            result2 = r2 % MULTIPLER_LONGWORD_DECIMAL;
        ...
        } else if (diffScale < TWO_X_LONGWORD_DECIMAL_DIGITS)
        {
            ...
            result1 = r1 % MULTIPLER_LONGWORD_DECIMAL;
            final long r2 = result2 + shift0 / divideFactor + (shift1 % divideFactor) * multiplyFactor + r1 / MULTIPLER_LONGWORD_DECIMAL;
            result2 = r2 % MULTIPLER_LONGWORD_DECIMAL;
            final long r3 = shift1 / divideFactor + (shift2 % divideFactor) * multiplyFactor + r2 / MULTIPLER_LONGWORD_DECIMAL;
            result3 = r3 % MULTIPLER_LONGWORD_DECIMAL;
        ...
        } else
        {
            ...
            result2 = r2 % MULTIPLER_LONGWORD_DECIMAL;
            final long r3 = shift0 / divideFactor + (shift1 % divideFactor) * multiplyFactor + r2 / MULTIPLER_LONGWORD_DECIMAL;
            result3 = r3 % MULTIPLER_LONGWORD_DECIMAL;
            final long r4 = shift1 / divideFactor + (shift2 % divideFactor) * multiplyFactor + r3 / MULTIPLER_LONGWORD_DECIMAL;
            result4 = r4 % MULTIPLER_LONGWORD_DECIMAL;
    ...
    public static boolean fastAdd(int leftSignum, long leftFast0, long leftFast1, long leftFast2, int leftIntegerDigitCount, int leftScale, int rightSignum, long rightFast0, long rightFast1, long rightFast2, int rightIntegerDigitCount, int rightScale, FastHiveDecimal fastResult)
    {
        ...
        if (leftScale == rightScale)
        {
            return doAddSameScale(leftSignum, leftFast0, leftFast1, leftFast2, rightSignum, rightFast0, rightFast1, rightFast2, leftScale, fastResult);
        } else
        {
            return doAddDifferentScale(leftSignum, leftFast0, leftFast1, leftFast2, leftIntegerDigitCount, leftScale, rightSignum, rightFast0, rightFast1, rightFast2, rightIntegerDigitCount, rightScale, fastResult);
        }
    }
    ...
    public static boolean fastSubtract(int leftSignum, long leftFast0, long leftFast1, long leftFast2, int leftIntegerDigitCount, int leftScale, int rightSignum, long rightFast0, long rightFast1, long rightFast2, int rightIntegerDigitCount, int rightScale, FastHiveDecimal fastResult)
    {
        ...
        if (leftScale == rightScale)
        {
            return doAddSameScale(leftSignum, leftFast0, leftFast1, leftFast2, flippedDecSignum, rightFast0, rightFast1, rightFast2, leftScale, fastResult);
        } else
        {
            return doAddDifferentScale(leftSignum, leftFast0, leftFast1, leftFast2, leftIntegerDigitCount, leftScale, flippedDecSignum, rightFast0, rightFast1, rightFast2, rightIntegerDigitCount, rightScale, fastResult);
        }
    }
    ...
    public static boolean fastMultiply5x5HalfWords(long left0, long left1, long left2, long right0, long right1, long right2, FastHiveDecimal fastResult)
    {
        ...
        product = halfRight0 * halfLeft1 + halfRight1 * halfLeft0 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z1 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[2]
        product = halfRight0 * halfLeft2 + halfRight1 * halfLeft1 + halfRight2 * halfLeft0 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z2 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[3]
        product = halfRight0 * halfLeft3 + halfRight1 * halfLeft2 + halfRight2 * halfLeft1 + halfRight3 * halfLeft0 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z3 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[4]
        product = halfRight0 * halfLeft4 + halfRight1 * halfLeft3 + halfRight2 * halfLeft2 + halfRight3 * halfLeft1 + halfRight4 * halfLeft0 + (product / MULTIPLER_INTWORD_DECIMAL);
        ...
        if ((halfRight4 != 0 && (halfLeft4 != 0 || halfLeft3 != 0 || halfLeft2 != 0 || halfLeft1 != 0)) || (halfRight3 != 0 && (halfLeft4 != 0 || halfLeft3 != 0 || halfLeft2 != 0)) || (halfRight2 != 0 && (halfLeft4 != 0 || halfLeft3 != 0)) || (halfRight1 != 0 && halfLeft4 != 0))
        {
            return false;
        }
    ...
    public static boolean fastMultiplyFullInternal(long left0, long left1, long left2, long right0, long right1, long right2, long[] result)
    {
        ...
        product = halfRight1 * halfLeft4 + halfRight2 * halfLeft3 + halfRight3 * halfLeft2 + halfRight4 * halfLeft1 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z5 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[6] -- since integer #5 is always 0, some products here are not included.
        product = halfRight2 * halfLeft4 + halfRight3 * halfLeft3 + halfRight4 * halfLeft2 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z6 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[7] -- since integer #5 is always 0, some products here are not included.
        product = halfRight3 * halfLeft4 + halfRight4 * halfLeft3 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z7 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[8] -- since integer #5 is always 0, some products here are not included.
        product = halfRight4 * halfLeft4 + (product / MULTIPLER_INTWORD_DECIMAL);
    ...
    public static boolean fastMultiply5x6HalfWords(long left0, long left1, long left2, long right0, long right1, long right2, long[] result)
    {
        ...
        product = halfRight1 * halfLeft4 + halfRight2 * halfLeft3 + halfRight3 * halfLeft2 + halfRight4 * halfLeft1 + halfRight5 * halfLeft0 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z5 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[6] -- since left integer #5 is always 0, some products here are not included.
        product = halfRight2 * halfLeft4 + halfRight3 * halfLeft3 + halfRight4 * halfLeft2 + halfRight5 * halfLeft1 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z6 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[7] -- since left integer #5 is always 0, some products here are not included.
        product = halfRight3 * halfLeft4 + halfRight4 * halfLeft3 + halfRight5 * halfLeft2 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z7 = (int) (product % MULTIPLER_INTWORD_DECIMAL);

        // v[8] -- since left integer #5 is always 0, some products here are not included.
        product = halfRight4 * halfLeft4 + halfRight5 * halfLeft3 + (product / MULTIPLER_INTWORD_DECIMAL);
        final int z8 = (int) (product % MULTIPLER_INTWORD_DECIMAL);
    ...
    private static long doSingleWordQuotient(long leftFast0, long leftFast1, long leftFast2, long rightFast0, FastHiveDecimal fastResult)
    {
        ...
        if (leftFast2 == 0 && leftFast1 == 0)
        {
            ...
            final long k0 = leftFast0 - quotient0 * rightFast0;
        ...
        } else if (leftFast2 == 0)
        {
            ...
            final long k1 = leftFast1 - quotient1 * rightFast0;
            final long quotientSubexpr0 = k1 * MULTIPLER_LONGWORD_DECIMAL + leftFast0;
            quotient0 = quotientSubexpr0 / rightFast0;
            final long k0 = quotientSubexpr0 - quotient0 * rightFast0;
            remainderSubexpr2 = k0 * MULTIPLER_LONGWORD_DECIMAL;
        } else if (leftFast1 == 0)
        {
        ...
        {
            quotient2 = leftFast2 / rightFast0;
            final long k2 = leftFast2 - quotient2 * rightFast0;
            final long quotientSubexpr1 = k2 * MULTIPLER_LONGWORD_DECIMAL + leftFast1;
            quotient1 = quotientSubexpr1 / rightFast0;
            final long k1 = quotientSubexpr1 - quotient1 * rightFast0;
            final long quotientSubexpr0 = k1 * MULTIPLER_LONGWORD_DECIMAL;
            quotient0 = quotientSubexpr0 / rightFast0;
            final long k0 = quotientSubexpr0 - quotient0 * rightFast0;
            remainderSubexpr2 = k0 * MULTIPLER_LONGWORD_DECIMAL;
        }
    ...
    private static int doSingleWordRemainder(long leftFast0, long leftFast1, long leftFast2, long rightFast0, long remainderSubexpr2, FastHiveDecimal fastResult)
    {
        ...
        } else
        {
            ...
            final long k2 = remainderSubexpr2 - remainder2 * rightFast0;
            if (k2 == 0)
            {
                remainder1 = 0;
                remainder0 = 0;
                remainderDigitCount = LONGWORD_DECIMAL_DIGITS - fastLongWordTrailingZeroCount(remainder2);
            } else
            {
                final long remainderSubexpr1 = k2 * MULTIPLER_LONGWORD_DECIMAL;
                long remainderSubexpr0;
                remainder1 = remainderSubexpr1 / rightFast0;
                final long k1 = remainderSubexpr1 - remainder1 * rightFast0;
                if (k1 == 0)
                {
                    remainder0 = 0;
                    remainderDigitCount = LONGWORD_DECIMAL_DIGITS + LONGWORD_DECIMAL_DIGITS - fastLongWordTrailingZeroCount(remainder1);
                } else
                {
                    remainderSubexpr0 = k2 * MULTIPLER_LONGWORD_DECIMAL;

                    remainder0 = remainderSubexpr0 / rightFast0;
                    remainderDigitCount = TWO_X_LONGWORD_DECIMAL_DIGITS + LONGWORD_DECIMAL_DIGITS - fastLongWordTrailingZeroCount(remainder0);
                }
            }
        }
    ...
    */
        ...
        BigDecimal quotient = denominator.divide(divisor, HiveDecimal.MAX_SCALE, BigDecimal.ROUND_HALF_UP);

        if (!fastSetFromBigDecimal(quotient, true, fastResult))
        {
            return false;
        }
    ...
    public static String fastToFormatString(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int formatScale)
    {
        ...
        final int index = doFastToFormatBytes(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, formatScale, scratchBuffer);
        return new String(scratchBuffer, index, FAST_SCRATCH_BUFFER_LEN_TO_BYTES - index, StandardCharsets.UTF_8);
    }
    ...
    {
        final int index = doFastToBytes(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, formatScale, scratchBuffer);
    ...
    public static int doFastToFormatBytes(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale, int formatScale, byte[] scratchBuffer)
    {
        ...
        {
            FastHiveDecimal fastTemp = new FastHiveDecimal();
            if (!fastRound(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, formatScale, BigDecimal.ROUND_HALF_UP, fastTemp))
            {
                return 0;
            }
    ...
    {
        byte[] scratchBuffer = new byte[FAST_SCRATCH_BUFFER_LEN_TO_BYTES];
        final int index = doFastToDigitsOnlyBytes(fast0, fast1, fast2, fastIntegerDigitCount, scratchBuffer);
        return new String(scratchBuffer, index, FAST_SCRATCH_BUFFER_LEN_TO_BYTES - index, StandardCharsets.UTF_8);
    }
    ...
    {
        byte[] scratchBuffer = new byte[FAST_SCRATCH_BUFFER_LEN_TO_BYTES];
        final int index = doFastToBytes(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, fastTrailingZeroesScale, scratchBuffer);
        return new String(scratchBuffer, index, FAST_SCRATCH_BUFFER_LEN_TO_BYTES - index, StandardCharsets.UTF_8);
    }
    ...
    public static boolean fastIsValid(int fastSignum, long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale)
    {
        ...
        {
            isValid = ((fast0 >= 0 && fast0 <= MAX_LONGWORD_DECIMAL) && (fast1 >= 0 && fast1 <= MAX_LONGWORD_DECIMAL) && (fast2 >= 0 && fast2 <= MAX_HIGHWORD_DECIMAL));
            if (!isValid)
            {
            ...
            } else
            {
                ...
                } else
                {
                    ...
                    if (isValid)
                    {
                        final int trailingZeroCount = fastTrailingDecimalZeroCount(fast0, fast1, fast2, fastIntegerDigitCount, fastScale);
                        if (trailingZeroCount != 0)
                        {
    ...
    private enum SpecialCharacters
    {
        ...
        HORIZONTAL_TABULATION('\u0009'), VERTICAL_TABULATION('\u000B'), FORM_FEED('\u000C'), SPACE_SEPARATOR('\u0020');
