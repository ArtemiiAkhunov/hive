...
public class DiskRangeList extends DiskRange
{
    ...
    private final static void assertInvalidOrder(DiskRangeList ref, DiskRangeList prev, DiskRangeList next)
    {
    ...
    public static class CreateHelper
    {
        ...
        {
            if (doMerge && tail != null && tail.merge(offset, end))
            {
                return;
            }
            if (doLogNew)
            {
                LOG.debug("Creating new range; last range (which can include some previous adds) was " + tail);
            }
