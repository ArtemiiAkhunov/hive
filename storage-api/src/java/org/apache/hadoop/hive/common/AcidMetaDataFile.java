...
public class AcidMetaDataFile
{
    ...
    public enum DataFormat
    {
        ...
        COMPACTED, // written by truncate
    TRUNCATED, // written by drop partition
    DROPPED;
    ...
    public static void writeToFile(FileSystem fs, Path basePath, DataFormat format) throws IOException
    {
        ...
        try (FSDataOutputStream out = fs.create(new Path(basePath, METADATA_FILE)); OutputStreamWriter writer = new OutputStreamWriter(out, "UTF-8"))
        {
            writer.write(data);
            writer.flush();
        }
