...
public class AssertHelpers
{
    ...
    {
        AbstractThrowableAssert<?, ? extends Throwable> check = Assertions.assertThatThrownBy(callable::call).withFailMessage(message).isInstanceOf(expected);
     ...
     */
    public static void assertThrows(String message, Class<? extends Exception> expected, String containedInMessage, Runnable runnable)
    {
        AbstractThrowableAssert<?, ? extends Throwable> check = Assertions.assertThatThrownBy(runnable::run).withFailMessage(message).isInstanceOf(expected);
        if (null != containedInMessage)
        {
            check.hasMessageContaining(containedInMessage);
        }
     ...
     * @param runnable           A Runnable that is expected to throw the runtime exception
     */
    public static void assertThrowsCause(String message, Class<? extends Exception> expected, String containedInMessage, Runnable runnable)
    {
        Assertions.assertThatThrownBy(runnable::run).withFailMessage(message).getCause().isInstanceOf(expected).hasMessageContaining(containedInMessage);
    }
}
