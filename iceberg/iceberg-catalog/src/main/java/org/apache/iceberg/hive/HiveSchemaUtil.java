...
public final class HiveSchemaUtil
{
    ...
    {
        return schema.columns().stream().map(col -> new FieldSchema(col.name(), convertToTypeString(col.type()), col.doc())).collect(Collectors.toList());
    }
    ...
    public static class SchemaDifference
    {
        ...
        {
            return missingFromFirst.isEmpty() && missingFromSecond.isEmpty() && typeChanged.isEmpty() && commentChanged.isEmpty();
        }
    ...
    private static String convertToTypeString(Type type)
    {
        switch (type.typeId())
        {
            ...
            case STRUCT:
                ...
                final String nameToType = structType.fields().stream().map(f -> String.format("%s:%s", f.name(), convert(f.type()))).collect(Collectors.joining(","));
