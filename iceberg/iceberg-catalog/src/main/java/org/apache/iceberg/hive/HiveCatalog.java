...
public class HiveCatalog extends BaseMetastoreCatalog implements SupportsNamespaces, Configurable
{
    ...
    public List<TableIdentifier> listTables(Namespace namespace)
    {
        ...
        try
        {
            ...
            } else
            {
                ...
                tableIdentifiers = tableObjects.stream().filter(table -> table.getParameters() != null && BaseMetastoreTableOperations.ICEBERG_TABLE_TYPE_VALUE.equalsIgnoreCase(table.getParameters().get(BaseMetastoreTableOperations.TABLE_TYPE_PROP))).map(table -> TableIdentifier.of(namespace, table.getTableName())).collect(Collectors.toList());
            }
    ...
    public boolean dropTable(TableIdentifier identifier, boolean purge)
    {
        ...
        try
        {
            clients.run(client ->
            {
                client.dropTable(database, identifier.name(), false /* do not delete data */, false /* throw NoSuchObjectException if the table doesn't exist */);
                return null;
            });
    ...
    public void createNamespace(Namespace namespace, Map<String, String> meta)
    {
        ...
        Preconditions.checkArgument(meta.get(HMS_DB_OWNER_TYPE) == null || meta.get(HMS_DB_OWNER) != null, "Create namespace setting %s without setting %s is not allowed", HMS_DB_OWNER_TYPE, HMS_DB_OWNER);
        try
        {
            clients.run(client ->
            {
    ...
    public List<Namespace> listNamespaces(Namespace namespace)
    {
        ...
        try
        {
            List<Namespace> namespaces = clients.run(IMetaStoreClient::getAllDatabases).stream().map(Namespace::of).collect(Collectors.toList());
    ...
    public boolean dropNamespace(Namespace namespace)
    {
        ...
        try
        {
            clients.run(client ->
            {
                client.dropDatabase(namespace.level(0), false /* deleteData */, false /* ignoreUnknownDb */, false /* cascade */);
                return null;
            });
    ...
    {
        Preconditions.checkArgument((properties.get(HMS_DB_OWNER_TYPE) == null) == (properties.get(HMS_DB_OWNER) == null), "Setting %s and %s has to be performed together or not at all", HMS_DB_OWNER_TYPE, HMS_DB_OWNER);
    ...
    {
        Preconditions.checkArgument(properties.contains(HMS_DB_OWNER_TYPE) == properties.contains(HMS_DB_OWNER), "Removing %s and %s has to be performed together or not at all", HMS_DB_OWNER_TYPE, HMS_DB_OWNER);
        Map<String, String> parameter = Maps.newHashMap();
    ...
    {
        return MoreObjects.toStringHelper(this).add("name", name).add("uri", this.conf == null ? "" : this.conf.get(HiveConf.ConfVars.METASTORE_URIS.varname)).toString();
    }
