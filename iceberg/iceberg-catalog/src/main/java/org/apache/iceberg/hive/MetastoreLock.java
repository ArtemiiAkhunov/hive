...
public class MetastoreLock implements HiveLock
{
    ...
    public MetastoreLock(Configuration conf, ClientPool<IMetaStoreClient, TException> metaClients, String catalogName, String databaseName, String tableName)
    {
        ...
        this.lockCreationTimeout = conf.getLong(HIVE_LOCK_CREATION_TIMEOUT_MS, HIVE_LOCK_CREATION_TIMEOUT_MS_DEFAULT);
        this.lockCreationMinWaitTime = conf.getLong(HIVE_LOCK_CREATION_MIN_WAIT_MS, HIVE_LOCK_CREATION_MIN_WAIT_MS_DEFAULT);
        this.lockCreationMaxWaitTime = conf.getLong(HIVE_LOCK_CREATION_MAX_WAIT_MS, HIVE_LOCK_CREATION_MAX_WAIT_MS_DEFAULT);
        this.lockHeartbeatIntervalTime = conf.getLong(HIVE_LOCK_HEARTBEAT_INTERVAL_MS, HIVE_LOCK_HEARTBEAT_INTERVAL_MS_DEFAULT);
        ...
        this.exitingScheduledExecutorService = Executors.newSingleThreadScheduledExecutor(new ThreadFactoryBuilder().setDaemon(true).setNameFormat("iceberg-hive-lock-heartbeat-" + fullName.replace("%", "%%") + "-%d").build());
    ...
    public void ensureActive() throws LockException
    {
        ...
        {
            throw new LockException(heartbeat.encounteredException, "Failed to heartbeat for hive lock. %s", heartbeat.encounteredException.getMessage());
        }
        if (!heartbeat.active())
        {
    ...
    private long acquireLock() throws LockException
    {
        ...
        {
            throw new IllegalArgumentException(String.format("HMS lock ID=%s already acquired for table %s.%s", hmsLockId.get(), databaseName, tableName));
        }
        ...
        try
        {
            if (lockInfo.lockState.equals(LockState.WAITING))
            {
                ...
                Tasks.foreach(lockInfo.lockId).retry(Integer.MAX_VALUE - 100).exponentialBackoff(lockCheckMinWaitTime, lockCheckMaxWaitTime, lockAcquireTimeout, 1.5).throwFailureWhenFinished().onlyRetryOn(WaitingForLockException.class).run(id ->
                {
                    try
                    {
                        ...
                        {
                            throw new WaitingForLockException(String.format("Waiting for lock on table %s.%s", databaseName, tableName));
                        }
                    } catch (InterruptedException e)
                    {
                        Thread.interrupted(); // Clear the interrupt status flag
                        LOG.warn("Interrupted while waiting for lock on table {}.{}", databaseName, tableName, e);
                    }
        ...
        if (!lockInfo.lockState.equals(LockState.ACQUIRED))
        {
            ...
            if (timeout)
            {
                throw new LockException("Timed out after %s ms waiting for lock on %s.%s", duration, databaseName, tableName);
            }

            if (thriftError != null)
            {
                throw new LockException(thriftError, "Metastore operation failed for %s.%s", databaseName, tableName);
            }

            // Just for safety. We should not get here.
            throw new LockException("Could not acquire the lock on %s.%s, lock request ended in state %s", databaseName, tableName, lockInfo.lockState);
        } else
        {
    ...
    private LockInfo createLock() throws LockException
    {
        ...
        LockComponent lockComponent = new LockComponent(LockType.EXCL_WRITE, LockLevel.TABLE, databaseName);
        ...
        Tasks.foreach(lockRequest).retry(Integer.MAX_VALUE - 100).exponentialBackoff(lockCreationMinWaitTime, lockCreationMaxWaitTime, lockCreationTimeout, 2.0).shouldRetryTest(e -> !interrupted.get() && e instanceof LockException && HiveVersion.min(HiveVersion.HIVE_2)).throwFailureWhenFinished().run(request ->
        {
            try
            {
            ...
            } catch (TException te)
            {
                ...
                } catch (InterruptedException e)
                {
                    ...
                    LOG.warn("Interrupted while trying to find lock for table {}.{}", databaseName, tableName, e);
                    throw new LockException(e, "Interrupted while trying to find lock for table %s.%s", databaseName, tableName);
                }
            } catch (InterruptedException e)
            {
                Thread.currentThread().interrupt();
                ...
                throw new LockException(e, "Interrupted while creating lock on table %s.%s", databaseName, tableName);
            }
        }, LockException.class);
    ...
    private static class LockInfo
    {
        ...
        {
            return MoreObjects.toStringHelper(this).add("lockId", lockId).add("lockState", lockState).toString();
        }
    }
