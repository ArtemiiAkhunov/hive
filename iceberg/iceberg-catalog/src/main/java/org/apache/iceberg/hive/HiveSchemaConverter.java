...
class HiveSchemaConverter
{
    ...
    List<Types.NestedField> convertInternal(List<String> names, List<TypeInfo> typeInfos, List<String> comments)
    {
        ...
        {
            result.add(Types.NestedField.optional(outerId - names.size() + i, names.get(i), convertType(typeInfos.get(i)), comments.isEmpty() || i >= comments.size() ? null : comments.get(i)));
        }
        ...
        switch (typeInfo.getCategory())
        {
            case PRIMITIVE:
                switch (((PrimitiveTypeInfo) typeInfo).getPrimitiveCategory())
                {
                    ...
                    case BYTE:
                    case SHORT:
                        Preconditions.checkArgument(autoConvert, "Unsupported Hive type %s, use integer " + "instead or enable automatic type conversion, set 'iceberg.mr.schema.auto.conversion' to true", ((PrimitiveTypeInfo) typeInfo).getPrimitiveCategory());
                    ...
                    case CHAR:
                    case VARCHAR:
                        Preconditions.checkArgument(autoConvert, "Unsupported Hive type %s, use string " + "instead or enable automatic type conversion, set 'iceberg.mr.schema.auto.conversion' to true", ((PrimitiveTypeInfo) typeInfo).getPrimitiveCategory());
