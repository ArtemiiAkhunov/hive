...
public class TestHiveIcebergOutputCommitter
{
    ...
    private static final Schema CUSTOMER_SCHEMA = new Schema(required(1, "customer_id", Types.LongType.get()), required(2, "first_name", Types.StringType.get()));

    private static final PartitionSpec PARTITIONED_SPEC = PartitionSpec.builderFor(CUSTOMER_SCHEMA).identity("customer_id").build();
    ...
    private Table table(String location, boolean partitioned)
    {
        ...
        return tables.create(CUSTOMER_SCHEMA, partitioned ? PARTITIONED_SPEC : PartitionSpec.unpartitioned(), ImmutableMap.of(InputFormatConfig.CATALOG_NAME, Catalogs.ICEBERG_HADOOP_TABLE_NAME), location);
    }
    ...
    private JobConf jobConf(Table table, int taskNum)
    {
        ...
        conf.set(InputFormatConfig.TABLE_CATALOG_PREFIX + table.name(), table.properties().get(InputFormatConfig.CATALOG_NAME));
        ...
        tableDesc.getProperties().setProperty(InputFormatConfig.CATALOG_NAME, table.properties().get(InputFormatConfig.CATALOG_NAME));
    ...
    private List<Record> writeRecords(String name, int taskNum, int attemptNum, boolean commitTasks, boolean abortTasks, JobConf conf, OutputCommitter committer) throws IOException
    {
        ...
        for (int i = 0; i < taskNum; ++i)
        {
            ...
            HiveIcebergWriter testWriter = WriterBuilder.builderFor(table).attemptID(TezUtil.taskAttemptWrapper(taskId)).queryId("Q_ID").tableName(conf.get(Catalogs.NAME)).operation(Context.Operation.OTHER).build();
