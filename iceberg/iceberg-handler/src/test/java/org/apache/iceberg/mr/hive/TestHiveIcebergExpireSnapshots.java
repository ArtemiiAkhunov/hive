...
public class TestHiveIcebergExpireSnapshots extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    public void testExpireSnapshotsWithTimestamp() throws IOException, InterruptedException
    {
        ...
        Table table = testTables.createTableWithVersions(shell, identifier.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 5);
    ...
    {
        TableIdentifier identifier = TableIdentifier.of("default", "source");
        Table table = testTables.createTableWithVersions(shell, identifier.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 10);
        Assert.assertEquals(10, IterableUtils.size(table.snapshots()));

        // Expire one snapshot
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE EXPIRE_SNAPSHOTS" + "('" + table.history().get(2).snapshotId() + "')");
        table.refresh();
        Assert.assertEquals(9, IterableUtils.size(table.snapshots()));
        // Expire multiple snapshots
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE EXPIRE_SNAPSHOTS('" + table.history().get(3).snapshotId() + "," + table.history().get(4).snapshotId() + "')");
        table.refresh();
    ...
    public void testExpireSnapshotsWithRetainLast() throws IOException, InterruptedException
    {
        ...
        shell.executeStatement("ALTER TABLE " + identifier.name() + " SET TBLPROPERTIES('" + MAX_SNAPSHOT_AGE_MS + "'='1')");
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE EXPIRE_SNAPSHOTS RETAIN LAST 5");
        table.refresh();
    ...
    public void testExpireSnapshotsWithDefaultParams() throws IOException, InterruptedException
    {
        ...
        shell.executeStatement("ALTER TABLE " + identifier.name() + " SET TBLPROPERTIES('" + MAX_SNAPSHOT_AGE_MS + "'='1'" + ",'" + MIN_SNAPSHOTS_TO_KEEP + "'='3')");
        ...
        shell.executeStatement("ALTER TABLE " + identifier.name() + " SET TBLPROPERTIES('" + MIN_SNAPSHOTS_TO_KEEP + "'='2')");
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE EXPIRE_SNAPSHOTS");
        table.refresh();
    ...
    public void testDeleteOrphanFiles() throws IOException, InterruptedException
    {
        ...
        Assert.assertEquals(numDataFiles, RemoteIterators.toList(fs.listFiles(new Path(table.location(), "data"), true)).size());

        Assert.assertEquals(numMetadataFiles, RemoteIterators.toList(fs.listFiles(new Path(table.location(), "metadata"), true)).size());
        ...
        Assert.assertEquals(numDataFiles - 1, RemoteIterators.toList(fs.listFiles(new Path(table.location(), "data"), true)).size());

        Assert.assertEquals(numMetadataFiles - 1, RemoteIterators.toList(fs.listFiles(new Path(table.location(), "metadata"), true)).size());
