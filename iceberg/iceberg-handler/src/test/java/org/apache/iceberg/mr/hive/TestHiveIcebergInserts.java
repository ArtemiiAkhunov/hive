...
public class TestHiveIcebergInserts extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    public void testSortedInsert() throws IOException
    {
        ...
        Schema schema = new Schema(optional(1, "id", Types.IntegerType.get(), "unique ID"), optional(2, "data", Types.StringType.get()));
        SortOrder order = SortOrder.builderFor(schema).asc("id", NULLS_FIRST).desc("data", NULLS_LAST).build();

        testTables.createTable(shell, identifier.name(), schema, order, PartitionSpec.unpartitioned(), fileFormat, ImmutableList.of(), formatVersion, ImmutableMap.of());
    ...
    public void testSortedAndTransformedInsert() throws IOException
    {
        ...
        SortOrder order = SortOrder.builderFor(HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA).asc(bucket("customer_id", 2), NULLS_FIRST).desc(truncate("first_name", 4), NULLS_LAST).asc("last_name", NULLS_LAST).build();

        testTables.createTable(shell, identifier.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, order, PartitionSpec.unpartitioned(), fileFormat, ImmutableList.of(), formatVersion, ImmutableMap.of());
    ...
    public void testSortedAndTransformedInsertIntoPartitionedTable() throws IOException
    {
        ...
        Schema schema = new Schema(optional(1, "a", Types.IntegerType.get()), optional(2, "b", Types.StringType.get()), optional(3, "c", Types.IntegerType.get()));
        SortOrder order = SortOrder.builderFor(schema).desc("c", NULLS_FIRST).asc(truncate("b", 1)).build();
        PartitionSpec partitionSpec = PartitionSpec.builderFor(schema).bucket("b", 2).build();
        testTables.createTable(shell, identifier.name(), schema, order, partitionSpec, fileFormat, ImmutableList.of(), formatVersion, ImmutableMap.of());
    ...
    {
        Assume.assumeTrue("Testing the create table ... stored as ORCFILE syntax is enough for a single scenario.", testTableType == TestTables.TestTableType.HIVE_CATALOG && fileFormat == FileFormat.ORC);
    ...
    {
        Assume.assumeTrue("Testing the create table ... stored as TEXTFILE syntax is enough for a single scenario.", testTableType == TestTables.TestTableType.HIVE_CATALOG && fileFormat == FileFormat.ORC);
        AssertHelpers.assertThrows("Create table should not work with textfile", IllegalArgumentException.class, "Unsupported fileformat", () -> shell.executeStatement("CREATE TABLE IF NOT EXISTS t2(c0 DOUBLE , c1 DOUBLE , c2 DECIMAL) STORED BY " + "ICEBERG STORED AS TEXTFILE"));
    }
    ...
    public void testInsertOverwritePartitionedTable() throws IOException
    {
        ...
        expected.add(TestHelper.RecordsBuilder.newInstance(HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA).add(8L, "Sue", "Green").build().get(0)); // add one more to 'Green' so we have a partition w/ multiple records
        ...
        List<Record> newRecords = TestHelper.RecordsBuilder.newInstance(HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA).add(0L, "Mike", "Brown") // overwritten
    ...
    {
        TableIdentifier target = TableIdentifier.of("default", "target");
        Table table = testTables.createTable(shell, target.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS);
        shell.executeStatement("ALTER TABLE target SET PARTITION SPEC(TRUNCATE(2, last_name))");
        List<Record> newRecords = TestHelper.RecordsBuilder.newInstance(HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA).add(0L, "Mike", "Taylor").add(1L, "Christy", "Hubert").build();
        AssertHelpers.assertThrows("IOW should not work on tables with partition evolution", IllegalArgumentException.class, "Cannot perform insert overwrite query on Iceberg table where partition evolution happened.", () -> shell.executeStatement(testTables.getInsertQuery(newRecords, target, true)));
    ...
    {
        Table source = testTables.createTable(shell, "source", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, ImmutableList.of());
        Table target = testTables.createTable(shell, "target", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, ImmutableList.of());

        shell.executeStatement("INSERT INTO target SELECT * FROM source");
        HiveIcebergTestUtils.validateData(target, ImmutableList.of(), 0);

        testTables.appendIcebergTable(shell.getHiveConf(), source, fileFormat, null, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS);
    ...
    public void testInsertOverwriteOnEmptyV2Table() throws IOException
    {
        ...
        TableIdentifier target = TableIdentifier.of("default", "target");
        Map<String, String> opTypes = ImmutableMap.of(TableProperties.DELETE_MODE, HiveIcebergStorageHandler.MERGE_ON_READ, TableProperties.MERGE_MODE, HiveIcebergStorageHandler.MERGE_ON_READ, TableProperties.UPDATE_MODE, HiveIcebergStorageHandler.MERGE_ON_READ);

        Table table = testTables.createTable(shell, target.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 2, opTypes);
