...
public class TestHiveIcebergCRUD extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    {
        Assume.assumeTrue("Reading V2 tables with eq delete files are only supported currently in " + "non-vectorized mode", !isVectorized && formatVersion == 2);

        Table tbl = testTables.createTable(shell, "customers", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, PartitionSpec.unpartitioned(), fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, formatVersion);
    ...
    public void testReadAndWriteFormatV2Partitioned_EqDelete_AllColumnsSupplied() throws IOException
    {
        ...
        Table tbl = testTables.createTable(shell, "customers", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, spec, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, formatVersion);
    ...
    public void testReadAndWriteFormatV2Unpartitioned_PosDelete() throws IOException
    {
        ...
        // delete one of the rows
        DataFile dataFile = StreamSupport.stream(tbl.currentSnapshot().addedDataFiles(tbl.io()).spliterator(), false).findFirst().orElseThrow(() -> new RuntimeException("Did not find any data files for test table"));
    ...
    public void testReadAndWriteFormatV2Partitioned_PosDelete_RowNotSupplied() throws IOException
    {
        ...
        DataFile dataFile = StreamSupport.stream(tbl.currentSnapshot().addedDataFiles(tbl.io()).spliterator(), false).filter(file -> file.partition().get(0, Long.class) == 0L).filter(file -> file.recordCount() == 3).findAny().orElseThrow(() -> new RuntimeException("Did not find the desired data file in the test table"));
    ...
    public void testDeleteStatementUnpartitioned() throws TException, InterruptedException
    {
        ...
        testTables.createTable(shell, "customers", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, PartitionSpec.unpartitioned(), fileFormat, HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_2, formatVersion);

        // verify delete mode set to merge-on-read
        if (formatVersion == 2)
        {
            Assert.assertEquals(HiveIcebergStorageHandler.MERGE_ON_READ, shell.metastore().getTable("default", "customers").getParameters().get(TableProperties.DELETE_MODE));
        }

        // insert one more batch so that we have multiple data files within the same partition
        shell.executeStatement(testTables.getInsertQuery(HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_1, TableIdentifier.of("default", "customers"), false));
    ...
    {
        PartitionSpec spec = PartitionSpec.builderFor(HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA).identity("last_name").bucket("customer_id", 16).build();

        // create and insert an initial batch of records
        testTables.createTable(shell, "customers", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, spec, fileFormat, HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_2, formatVersion);
        // insert one more batch so that we have multiple data files within the same partition
        shell.executeStatement(testTables.getInsertQuery(HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_1, TableIdentifier.of("default", "customers"), false));
    ...
    public void testDeleteStatementWithOtherTable()
    {
        ...
        testTables.createTable(shell, "customers", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, spec, fileFormat, HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_2, formatVersion);
        testTables.createTable(shell, "other", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, spec, fileFormat, HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_1, formatVersion);
    ...
    public void testDeleteStatementWithPartitionAndSchemaEvolution()
    {
        ...
        Schema newSchema = new Schema(optional(2, "given_name", Types.StringType.get()), optional(1, "customer_id", Types.LongType.get()), optional(3, "last_name", Types.StringType.get(), "This is last name"), optional(4, "department", Types.StringType.get()));
    ...
    public void testDeleteForSupportedTypes() throws IOException
    {
        ...
        for (int i = 0; i < SUPPORTED_TYPES.size(); i++)
        {
            ...
            if (type == Types.UUIDType.get() && (fileFormat == FileFormat.PARQUET || fileFormat == FileFormat.ORC && isVectorized) || type == Types.TimeType.get() && fileFormat == FileFormat.PARQUET && isVectorized)
            {
                continue;
            }
            ...
            Table table = testTables.createTable(shell, tableName, schema, PartitionSpec.unpartitioned(), fileFormat, records, formatVersion);
    ...
    public void testUpdateStatementUnpartitioned() throws TException, InterruptedException
    {
        ...
        if (formatVersion == 2)
        {
            Assert.assertEquals(HiveIcebergStorageHandler.MERGE_ON_READ, shell.metastore().getTable("default", "customers").getParameters().get(TableProperties.UPDATE_MODE));
        }
    ...
    public void testUpdateForSupportedTypes() throws IOException
    {
        ...
        for (int i = 0; i < SUPPORTED_TYPES.size(); i++)
        {
            ...
            Table table = testTables.createTable(shell, tableName, schema, PartitionSpec.unpartitioned(), fileFormat, originalRecords, formatVersion);
    ...
    {
        Assume.assumeTrue(fileFormat == FileFormat.PARQUET && isVectorized && testTableType == TestTables.TestTableType.HIVE_CATALOG);
        ...
        try
        {
            Tasks.range(2).executeWith(Executors.newFixedThreadPool(2)).run(i ->
            {
                ...
                HiveConf.setVar(shell.getHiveConf(), HiveConf.ConfVars.HIVE_QUERY_REEXECUTION_STRATEGIES, RETRY_STRATEGIES_WITHOUT_WRITE_CONFLICT);
    ...
    {
        Assume.assumeTrue(fileFormat == FileFormat.PARQUET && isVectorized && testTableType == TestTables.TestTableType.HIVE_CATALOG && formatVersion == 2);
    ...
    public void testConcurrent2MergeInserts()
    {
        ...
        testTables.createTable(shell, "source", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, PartitionSpec.unpartitioned(), fileFormat, HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_1);
        testTables.createTable(shell, "target", HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, PartitionSpec.unpartitioned(), fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, formatVersion);
