...
public class TestHiveShell
{
    ...
    public void start()
    {
        ...
        hs2Conf.setVar(HiveConf.ConfVars.METASTORE_WAREHOUSE, metastore.hiveConf().getVar(HiveConf.ConfVars.METASTORE_WAREHOUSE));
        hs2Conf.setVar(HiveConf.ConfVars.HIVE_METASTORE_WAREHOUSE_EXTERNAL, metastore.hiveConf().getVar(HiveConf.ConfVars.HIVE_METASTORE_WAREHOUSE_EXTERNAL));
        ...
        hs2.init(hs2Conf);
        hs2.start();
        client = hs2.getServices().stream().filter(CLIService.class::isInstance).findFirst().map(CLIService.class::cast).get();
        started = true;
    }
    ...
    private HiveSession openSession()
    {
        ...
        try
        {
            SessionHandle sessionHandle = client.getSessionManager().openSession(CLIService.SERVER_VERSION, "", "", "127.0.0.1", Collections.emptyMap());
    ...
    {
        Preconditions.checkState(session.get() != null, "You have to start TestHiveShell and open a session first, before running a query.");
        try
        {
            OperationHandle handle = client.executeStatement(session.get().getSessionHandle(), statement, Collections.emptyMap());
    ...
    public String executeAndStringify(String statement)
    {
        ...
        return objects.stream().map(o -> Joiner.on("\t").useForNull("NULL").join(o)).collect(Collectors.joining("\n"));
    }
