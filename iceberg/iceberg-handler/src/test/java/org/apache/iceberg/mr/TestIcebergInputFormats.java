...
public class TestIcebergInputFormats
{
    ...
    private static final Schema SCHEMA = new Schema(required(1, "data", Types.StringType.get()), required(2, "id", Types.LongType.get()), required(3, "date", Types.StringType.get()));
    ...
    public void testFailedResidualFiltering() throws Exception
    {
        ...
        Assertions.assertThatThrownBy(() -> testInputFormat.create(builder.conf())).isInstanceOf(UnsupportedOperationException.class).hasMessage("Filter expression ref(name=\"id\") == 0 is not completely satisfied. Additional rows can be returned " + "not satisfied by the filter expression");
    ...
    private static final Schema LOG_SCHEMA = new Schema(Types.NestedField.optional(1, "id", Types.IntegerType.get()), Types.NestedField.optional(2, "date", Types.StringType.get()), Types.NestedField.optional(3, "level", Types.StringType.get()), Types.NestedField.optional(4, "message", Types.StringType.get()));
        ...
        private static <T> TestMapredIcebergInputFormat<T> create(Configuration conf)
        {
            ...
            try
            {
                ...
                for (org.apache.hadoop.mapred.InputSplit split : splits)
                {
                    ...
                    org.apache.hadoop.mapred.RecordReader<Void, Container<T>> reader = inputFormat.getRecordReader(split, job, Reporter.NULL);
