...
public class TestHiveIcebergRollback extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    public void testRollbackToTimestamp() throws IOException, InterruptedException
    {
        ...
        Table table = testTables.createTableWithVersions(shell, identifier.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 3);
    ...
    */
    ...
    Assert.assertEquals(3, table.history().size());
     */
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE ROLLBACK(" + table.history().get(1).snapshotId() + ")");
        Assert.assertEquals(4, shell.executeStatement("SELECT * FROM " + identifier.name()).size());
        table.refresh();
        Assert.assertEquals(4, table.history().size());
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE ROLLBACK(" + table.history().get(0).snapshotId() + ")");
        Assert.assertEquals(3, shell.executeStatement("SELECT * FROM " + identifier.name()).size());
        table.refresh();
        Assert.assertEquals(5, table.history().size());
    }
    ...
    {
        Assume.assumeTrue("Rollback revert is only supported for tables from Hive Catalog", testTableType.equals(TestTables.TestTableType.HIVE_CATALOG));
        TableIdentifier identifier = TableIdentifier.of("default", "source");
        Table table = testTables.createTableWithVersions(shell, identifier.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 2);
    ...
    {
        TableIdentifier identifier = TableIdentifier.of("default", "source");
        testTables.createTableWithVersions(shell, identifier.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 2);
