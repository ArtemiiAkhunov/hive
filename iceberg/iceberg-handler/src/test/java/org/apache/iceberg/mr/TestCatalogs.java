...
public class TestCatalogs
{
    ...
    public void testLoadTableFromLocation() throws IOException
    {
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.loadTable(conf)).isInstanceOf(IllegalArgumentException.class).hasMessageContaining("Table location not set");
    ...
    public void testLoadTableFromCatalog() throws IOException
    {
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.loadTable(conf)).isInstanceOf(IllegalArgumentException.class).hasMessage("Table identifier not set");
    ...
    public void testCreateDropTableToLocation() throws IOException
    {
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.createTable(conf, missingSchema)).isInstanceOf(NullPointerException.class).hasMessage("Table schema not set");

        conf.set(CatalogUtil.ICEBERG_CATALOG_TYPE, Catalogs.LOCATION);
        Properties missingLocation = new Properties();
        missingLocation.put(InputFormatConfig.TABLE_SCHEMA, SchemaParser.toJson(SCHEMA));

        Assertions.assertThatThrownBy(() -> Catalogs.createTable(conf, missingLocation)).isInstanceOf(NullPointerException.class).hasMessage("Table location not set");
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.dropTable(conf, new Properties())).isInstanceOf(NullPointerException.class).hasMessage("Table location not set");
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.loadTable(conf, dropProperties)).isInstanceOf(NoSuchTableException.class).hasMessage("Table does not exist at location: " + properties.getProperty("location"));
    }
    ...
    public void testCreateDropTableToCatalog() throws IOException
    {
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.createTable(conf, missingIdentifier)).isInstanceOf(NullPointerException.class).hasMessage("Table identifier not set");
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.dropTable(conf, new Properties())).isInstanceOf(NullPointerException.class).hasMessage("Table identifier not set");
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.loadTable(conf, dropProperties)).isInstanceOf(NoSuchTableException.class).hasMessage("Table does not exist: test.table");
    }
    ...
    public void testLoadCatalogUnknown()
    {
        ...
        Assertions.assertThatThrownBy(() -> Catalogs.loadCatalog(conf, catalogName)).isInstanceOf(UnsupportedOperationException.class).hasMessage("Unknown catalog type: fooType");
    }
    ...
    public void testDefaultCatalogProperties()
    {
        ...
        Assert.assertEquals("true", defaultCatalog.newTableOps(TableIdentifier.of("default", "iceberg")).io().properties().get(catalogProperty));

        // set property at catalog level, and that should take precedence over the global property.
        conf.setBoolean(String.format("%s%s.%s", InputFormatConfig.CATALOG_CONFIG_PREFIX, Catalogs.ICEBERG_DEFAULT_CATALOG_NAME, catalogProperty), false);
        ...
        Assert.assertEquals("false", defaultCatalog.newTableOps(TableIdentifier.of("default", "iceberg")).io().properties().get(catalogProperty));
    }
    ...
    {
        conf.set(InputFormatConfig.catalogPropertyConfigKey(catalogName, CatalogProperties.WAREHOUSE_LOCATION), warehouseLocation);
        conf.set(InputFormatConfig.catalogPropertyConfigKey(catalogName, CatalogProperties.CATALOG_IMPL), CustomHadoopCatalog.class.getName());
