...
public class HiveIcebergWriterTestBase
{
    ...
    public static final Schema SCHEMA = new Schema(Types.NestedField.required(1, "id", Types.IntegerType.get()), Types.NestedField.required(2, "data", Types.StringType.get()));
    ...
    {
        return Lists.newArrayList(new Object[][]{{FileFormat.PARQUET, true, true}, {FileFormat.ORC, true, true}, {FileFormat.AVRO, true, true}, {FileFormat.PARQUET, false, true}, {FileFormat.PARQUET, true, false}, {FileFormat.ORC, true, false}, {FileFormat.AVRO, true, false}, {FileFormat.PARQUET, false, false},
                // Skip this until the ORC reader is fixed - test only issue
                //        { FileFormat.ORC, false },
                {FileFormat.AVRO, false, true}, {FileFormat.AVRO, false, false}});
    }
    ...
    public void init() throws IOException
    {
        ...
        PartitionSpec spec = !partitioned ? PartitionSpec.unpartitioned() : PartitionSpec.builderFor(SCHEMA).bucket("data", 3).build();
        this.helper = new TestHelper(new HiveConf(), tables, location.toString(), SCHEMA, spec, fileFormat, Collections.singletonMap(WriterBuilder.ICEBERG_DELETE_SKIPROWDATA, String.valueOf(skipRowData)), temp);
        ...
        writerBuilder = WriterBuilder.builderFor(table).attemptID(taskAttemptID).queryId("Q_ID").tableName("dummy");
    }
    ...
    protected StructLikeSet rowSetWithoutIds(List<Record> records, Set<Integer> idToDelete)
    {
        ...
        records.stream().filter(row -> !idToDelete.contains(row.getField("id"))).forEach(set::add);
        return set;
    }
