...
public abstract class TestTables
{
    ...
    public String createHiveTableSQL(TableIdentifier identifier, Map<String, String> tableProps)
    {
        ...
        return String.format("CREATE EXTERNAL TABLE %s.%s STORED BY '%s' %s %s", identifier.namespace(), identifier.name(), HiveIcebergStorageHandler.class.getName(), locationForCreateTableSQL(identifier), propertiesForCreateTableSQL(tableProps));
    }
    ...
    {
        return createTable(shell, tableName, schema, SortOrder.unsorted(), PartitionSpec.unpartitioned(), fileFormat, records, formatVersion, tblProperties);
    }
    ...
    {
        ImmutableMap<String, String> tblProps = ImmutableMap.<String, String>builder().putAll(tblProperties).put(TableProperties.FORMAT_VERSION, Integer.toString(formatVersion)).build();
        Table table = createIcebergTable(shell.getHiveConf(), tableName, schema, order, partSpec, fileFormat, tblProps, records);
    ...
    {
        return createTable(shell, tableName, schema, spec, fileFormat, records, formatVersion, tblProperties, HiveMetaHook.ICEBERG);
    }
    ...
    public Table createTable(TestHiveShell shell, String tableName, Schema schema, PartitionSpec spec, FileFormat fileFormat, List<Record> records, Integer formatVersion, Map<String, String> tblProperties, String storageHandler)
    {
        ...
        String tblProps = propertiesForCreateTableSQL(ImmutableMap.<String, String>builder().putAll(tblProperties).put(TableProperties.DEFAULT_FILE_FORMAT, fileFormat.toString()).put(InputFormatConfig.TABLE_SCHEMA, SchemaParser.toJson(schema)).put(InputFormatConfig.PARTITION_SPEC, PartitionSpecParser.toJson(spec)).put(TableProperties.FORMAT_VERSION, Integer.toString(formatVersion)).build());
        shell.executeStatement("CREATE EXTERNAL TABLE " + identifier + " STORED BY " + storageHandler + " " + locationForCreateTableSQL(identifier) + tblProps);
    ...
    public String getUpdateQuery(String tableName, Record record)
    {
        ...
        query.append(record.struct().fields().stream().map(field -> field.name() + "=" + getStringValueForInsert(record.getField(field.name()), field.type())).collect(Collectors.joining(",")));
        return query.toString();
    }
    ...
    {
        return createIcebergTable(configuration, tableName, schema, SortOrder.unsorted(), PartitionSpec.unpartitioned(), fileFormat, additionalTableProps, records);
    }
    ...
    public Table createIcebergTable(Configuration configuration, String tableName, Schema schema, SortOrder order, PartitionSpec partSpec, FileFormat fileFormat, Map<String, String> additionalTableProps, List<Record> records) throws IOException
    {
        ...
        TestHelper helper = new TestHelper(new Configuration(configuration), tables(), identifier, schema, partSpec, fileFormat, additionalTableProps, temp);
    ...
    private static class CatalogToTables implements Tables
    {
        ...
        public Table create(Schema schema, PartitionSpec spec, SortOrder sortOrder, Map<String, String> properties, String tableIdentifier)
        {
            ...
            return catalog.buildTable(tableIdent, schema).withPartitionSpec(spec).withSortOrder(sortOrder).withProperties(properties).create();
        }
    ...
    static class CustomCatalogTestTables extends TestTables
    {
        ...
        {
            this(conf, temp, (HiveVersion.min(HiveVersion.HIVE_3) ? "file:" : "") + temp.newFolder("custom", "warehouse").toString(), catalogName);
        }
        ...
        public Map<String, String> properties()
        {
            return ImmutableMap.of(InputFormatConfig.catalogPropertyConfigKey(catalog, CatalogProperties.CATALOG_IMPL), TestCatalogs.CustomHadoopCatalog.class.getName(), InputFormatConfig.catalogPropertyConfigKey(catalog, CatalogProperties.WAREHOUSE_LOCATION), warehouseLocation);
        }
    ...
    static class HadoopCatalogTestTables extends TestTables
    {
        ...
        {
            this(conf, temp, (HiveVersion.min(HiveVersion.HIVE_3) ? "file:" : "") + temp.newFolder("hadoop", "warehouse").toString(), catalogName);
        }
        ...
        public Map<String, String> properties()
        {
            return ImmutableMap.of(InputFormatConfig.catalogPropertyConfigKey(catalog, CatalogUtil.ICEBERG_CATALOG_TYPE), CatalogUtil.ICEBERG_CATALOG_TYPE_HADOOP, InputFormatConfig.catalogPropertyConfigKey(catalog, CatalogProperties.WAREHOUSE_LOCATION), warehouseLocation);
        }
    ...
    static class HiveTestTables extends TestTables
    {
        ...
        {
            super(CatalogUtil.loadCatalog(HiveCatalog.class.getName(), CatalogUtil.ICEBERG_CATALOG_TYPE_HIVE, ImmutableMap.of(), conf), temp, catalogName);
        }

        @Override
        public Map<String, String> properties()
        {
            return ImmutableMap.of(InputFormatConfig.catalogPropertyConfigKey(catalog, CatalogUtil.ICEBERG_CATALOG_TYPE), CatalogUtil.ICEBERG_CATALOG_TYPE_HIVE);
        }
    ...
    public enum TestTableType
    {
        ...
        public abstract TestTables instance(Configuration conf, TemporaryFolder temporaryFolder, String catalogName) throws IOException;
    }
}
