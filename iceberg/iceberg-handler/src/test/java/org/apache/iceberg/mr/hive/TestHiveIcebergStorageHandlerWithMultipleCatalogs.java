...
public class TestHiveIcebergStorageHandlerWithMultipleCatalogs
{
        ...
        for (String engine : EXECUTION_ENGINES)
        {
            ...
            if (javaVersion.equals("1.8"))
            {
                for (TestTables.TestTableType testTableType : TestTables.ALL_TABLE_TYPES)
                {
                    ...
                    {
                        testParams.add(new Object[]{FileFormat.PARQUET, FileFormat.ORC, engine, TestTables.TestTableType.HIVE_CATALOG, HIVECATALOGNAME, testTableType, OTHERCATALOGNAME});
                    }
                }
            }
        }
    ...
    {
        createAndAddRecords(testTables1, fileFormat1, TableIdentifier.of("default", "customers1"), HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS);
        createAndAddRecords(testTables2, fileFormat2, TableIdentifier.of("default", "customers2"), HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS);
    ...
    public void testCTASFromOtherCatalog() throws IOException
    {
        ...
        shell.executeStatement(String.format("CREATE TABLE target STORED BY ICEBERG TBLPROPERTIES ('%s'='%s') AS SELECT * FROM source", InputFormatConfig.CATALOG_NAME, HIVECATALOGNAME));
