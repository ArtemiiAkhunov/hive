...
public class TestHiveIcebergVectorization extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    public void testRowIterator() throws Exception
    {
        ...
        Schema allSchema = new Schema(optional(1, "binary_col", Types.BinaryType.get()), optional(2, "boolean_col", Types.BooleanType.get()), optional(3, "date_col", Types.DateType.get()), optional(4, "decimal_col", Types.DecimalType.of(6, 4)), optional(5, "double_col", Types.DoubleType.get()), optional(6, "fixed_col", Types.FixedType.ofLength(4)), optional(7, "float_col", Types.FloatType.get()), optional(8, "int_col", Types.IntegerType.get()), optional(9, "long_col", Types.LongType.get()), optional(10, "string_col", Types.StringType.get()),
        ...
        Path dataFilePath = new Path(Lists.newArrayList(Lists.newArrayList(table.newScan().planTasks().iterator()).get(0).files().iterator()).get(0).file().path().toString());
        ...
        RecordReader<NullWritable, VectorizedRowBatch> internalVectorizedRecordReader = inputFormat.getRecordReader(new FileSplit(dataFilePath, 0L, Long.MAX_VALUE, new String[]{}), jobConf, new MockReporter());
    ...
    private void testVectorizedReadWithDeleteFilter(Map<String, String> props)
    {
        ...
        Schema schema = new Schema(optional(1, "customer_id", Types.LongType.get()), optional(2, "customer_age", Types.IntegerType.get()));
        ...
        Function<Integer, Void> validation = expectedCount ->
        {
            ...
            for (Object[] row : result)
            {
                ...
                Assert.assertTrue("Found a row with customer_id between 1000 and 3000 (both exclusive)", id <= 1000 || 3000 <= id);
    ...
    {
        Assume.assumeTrue(isVectorized && testTableType == TestTables.TestTableType.HIVE_CATALOG && fileFormat == FileFormat.PARQUET);

        Schema schema = new Schema(optional(1, "customer_id", Types.LongType.get()), optional(2, "customer_age", Types.IntegerType.get()), optional(3, "date_col", Types.DateType.get()));
    ...
    static JobConf prepareMockJob(Schema schema, Path dataFilePath) throws HiveException
    {
        ...
        String hiveColumnNames = String.join(",", oi.getAllStructFieldRefs().stream().map(sf -> sf.getFieldName()).collect(Collectors.toList()));
        String hiveTypeInfoNames = String.join(",", oi.getAllStructFieldRefs().stream().map(sf -> sf.getFieldObjectInspector().getTypeName()).collect(Collectors.toList()));
