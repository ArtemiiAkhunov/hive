...
public class TestHiveIcebergCTAS extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    {
        Assume.assumeTrue(HiveIcebergSerDe.CTAS_EXCEPTION_MSG, testTableType == TestTables.TestTableType.HIVE_CATALOG && isVectorized && formatVersion == 1);
    }
    ...
    public void testCTASFromHiveTable()
    {
        ...
        shell.executeStatement(testTables.getInsertQuery(HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, TableIdentifier.of("default", "source"), false));

        shell.executeStatement(String.format("CREATE TABLE target STORED BY ICEBERG %s %s AS SELECT * FROM source", testTables.locationForCreateTableSQL(TableIdentifier.of("default", "target")), testTables.propertiesForCreateTableSQL(ImmutableMap.of(TableProperties.DEFAULT_FILE_FORMAT, fileFormat.toString()))));
    ...
    public void testCTASPartitionedFromHiveTable() throws TException, InterruptedException
    {
        ...
        shell.executeStatement(String.format("CREATE TABLE target PARTITIONED BY (dept, name) " + "STORED BY ICEBERG %s AS SELECT * FROM source", testTables.propertiesForCreateTableSQL(ImmutableMap.of(TableProperties.DEFAULT_FILE_FORMAT, fileFormat.toString()))));
        ...
        Assert.assertEquals(fileFormat.toString().toLowerCase(), hmsTable.getParameters().get(TableProperties.DEFAULT_FILE_FORMAT));
    ...
    public void testCTASTblPropsAndLocationClause() throws Exception
    {
        ...
        shell.executeStatement(String.format("CREATE TABLE target PARTITIONED BY (dept, name) " + "STORED BY ICEBERG STORED AS %s LOCATION '%s' TBLPROPERTIES ('customKey'='customValue') " + "AS SELECT * FROM source", fileFormat.toString(), location));
    ...
    public void testCTASFollowedByTruncate() throws IOException
    {
        ...
        shell.executeStatement(String.format("CREATE EXTERNAL TABLE target STORED BY ICEBERG STORED AS %s %s AS SELECT * FROM source", fileFormat, testTables.locationForCreateTableSQL(TableIdentifier.of("default", "target"))));
