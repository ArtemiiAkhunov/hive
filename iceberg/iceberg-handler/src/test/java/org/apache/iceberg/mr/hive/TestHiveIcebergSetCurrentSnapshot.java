...
public class TestHiveIcebergSetCurrentSnapshot extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    public void testSetCurrentSnapshot() throws IOException, InterruptedException
    {
        ...
        List<Object[]> result4 = shell.executeStatement("SELECT * from " + identifier.name() + " FOR SYSTEM_VERSION AS OF " + table.history().get(4).snapshotId());
        List<Object[]> result3 = shell.executeStatement("SELECT * from " + identifier.name() + " FOR SYSTEM_VERSION AS OF " + table.history().get(3).snapshotId());
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE SET_CURRENT_SNAPSHOT(" + table.history().get(3).snapshotId() + ")");
        ...
        shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE SET_CURRENT_SNAPSHOT(" + table.history().get(4).snapshotId() + ")");
    ...
    public void testSetCurrentSnapshotBySnapshotRef() throws IOException, InterruptedException
    {
        ...
        AssertHelpers.assertThrows("should throw exception", IllegalArgumentException.class, "SnapshotRef unknown_ref does not exist", () ->
        {
        ...
        AssertHelpers.assertThrows("should throw exception", IllegalArgumentException.class, "SnapshotRef " + table.currentSnapshot().snapshotId() + " does not exist", () ->
        {
            shell.executeStatement("ALTER TABLE " + identifier.name() + " EXECUTE SET_CURRENT_SNAPSHOT" + "('" + table.currentSnapshot().snapshotId() + "')");
        });
    }
