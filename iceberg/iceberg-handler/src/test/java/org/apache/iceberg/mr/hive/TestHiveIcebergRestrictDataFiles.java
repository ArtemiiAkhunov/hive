...
public class TestHiveIcebergRestrictDataFiles extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    {
        shell = HiveIcebergStorageHandlerTestUtils.shell(Collections.singletonMap(HIVE_ICEBERG_ALLOW_DATAFILES_IN_TABLE_LOCATION_ONLY.varname, "true"));
    }
    ...
    public void testRestrictDataFiles() throws IOException, InterruptedException
    {
        ...
        testTables.createTableWithVersions(shell, table1.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, 2);

        AssertHelpers.assertThrows("Should throw exception since there are files outside the table directory", IllegalArgumentException.class, "The table contains paths which are outside the table location", () -> shell.executeStatement("SELECT * FROM " + table1.name()));

        // Create another table with files within the table location
        TableIdentifier table2 = TableIdentifier.of("default", "tab2");
        testTables.createTableWithVersions(shell, table2.name(), HiveIcebergStorageHandlerTestUtils.CUSTOMER_SCHEMA, fileFormat, null, 0);

        shell.executeStatement(testTables.getInsertQuery(HiveIcebergStorageHandlerTestUtils.CUSTOMER_RECORDS, table2, false));
        ...
        shell.executeStatement(testTables.getInsertQuery(HiveIcebergStorageHandlerTestUtils.OTHER_CUSTOMER_RECORDS_1, table2, false));
