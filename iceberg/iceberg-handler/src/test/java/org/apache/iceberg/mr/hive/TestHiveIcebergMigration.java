...
public class TestHiveIcebergMigration extends HiveIcebergStorageHandlerWithEngineBase
{
    ...
    public void testMigrateHiveTableWithPrimitiveTypeColumnsToIceberg() throws TException, InterruptedException
    {
        ...
        shell.executeStatement(String.format("CREATE EXTERNAL TABLE %s (" + "a INT, " + "decimal_col DECIMAL(30, 3), " + "tinyint_col TINYINT, " + "boolean_col BOOLEAN, " + "float_col FLOAT, " + "bigint_col BIGINT, " + "double_col DOUBLE, " + "string_col STRING, " + "int_col INT, " + "smallint_col SMALLINT) " + "STORED AS %s %s %s", identifier.name(), fileFormat.name(), testTables.locationForCreateTableSQL(identifier), testTables.propertiesForCreateTableSQL(ImmutableMap.of())));
    ...
    {
        // enough to test once
        Assume.assumeTrue(fileFormat == FileFormat.ORC && isVectorized && testTableType == TestTables.TestTableType.HIVE_CATALOG);
        TableIdentifier identifier = TableIdentifier.of("default", "tbl_unsupportedtypes");
        shell.executeStatement(String.format("CREATE EXTERNAL TABLE %s (" + "char_col CHAR(10)) STORED AS %s %s %s", identifier.name(), fileFormat.name(), testTables.locationForCreateTableSQL(identifier), testTables.propertiesForCreateTableSQL(ImmutableMap.of())));
        AssertHelpers.assertThrows("should throw exception", IllegalArgumentException.class, "Cannot convert hive table to iceberg that", () ->
        {
    ...
    public void testMigrateHiveTableToIcebergWithTBLPROPERTIES() throws TException, InterruptedException
    {
        ...
        String createQuery = "CREATE EXTERNAL TABLE " + tableName + " (a int) STORED AS " + fileFormat.name() + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(Collections.singletonMap("random.prop", "random"));
        ...
        AssertHelpers.assertThrows("Should throw exception", IllegalArgumentException.class, "Can not convert table to ICEBERG ,Table is already of that format", () ->
        {
    ...
    {
        String tableName = "tbl";
        String createQuery = "CREATE EXTERNAL TABLE " + tableName + " (a int) STORED AS " + fileFormat.name() + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of());
        shell.executeStatement(createQuery);
        shell.executeStatement("INSERT INTO " + tableName + " VALUES (1), (2), (3)");
    ...
    public void testMigratePartitionedHiveTableToIceberg() throws TException, InterruptedException
    {
        ...
        shell.executeStatement("CREATE EXTERNAL TABLE " + tableName + " (a int) PARTITIONED BY (b string) STORED AS " + fileFormat.name() + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of()));
    ...
    public void testMigratePartitionedBucketedHiveTableToIceberg() throws TException, InterruptedException
    {
        ...
        shell.executeStatement("CREATE EXTERNAL TABLE " + tableName + " (a int) PARTITIONED BY (b string) clustered by " + "(a) INTO 2 BUCKETS STORED AS " + fileFormat.name() + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of()));
        shell.executeStatement("INSERT INTO " + tableName + " PARTITION (b='aaa') VALUES (1), (2), (3)");
        shell.executeStatement("INSERT INTO " + tableName + " PARTITION (b='bbb') VALUES (4), (5)");
        shell.executeStatement("INSERT INTO " + tableName + " PARTITION (b='ccc') VALUES (6)");
        shell.executeStatement("INSERT INTO " + tableName + " PARTITION (b='ddd') VALUES (7), (8), (9), (10)");
    ...
    public void testRollbackMigrateHiveTableToIceberg() throws TException, InterruptedException
    {
        ...
        shell.executeStatement("CREATE EXTERNAL TABLE " + tableName + " (a int) STORED AS " + fileFormat.name() + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of()));
        shell.executeStatement("INSERT INTO " + tableName + " VALUES (1), (2), (3)");
    ...
    {
        String tableName = "tbl_rollback";
        shell.executeStatement("CREATE EXTERNAL TABLE " + tableName + " (a int) PARTITIONED BY (b string, c int) " + "STORED AS " + fileFormat.name() + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of()));
    ...
    public void testMigrationFailsForUnsupportedSourceFileFormat()
    {
        ...
        {
            shell.executeStatement("CREATE EXTERNAL TABLE " + tableName + " (a int) STORED AS " + format + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of()));
            shell.executeStatement("INSERT INTO " + tableName + " VALUES (1), (2), (3)");
            AssertHelpers.assertThrows("Migrating a " + format + " table to Iceberg should have thrown an exception.", IllegalArgumentException.class, "Cannot convert hive table to iceberg with input format: ", () -> shell.executeStatement("ALTER TABLE " + tableName + " Convert to iceberg"));
    ...
    public void testMigrationFailsForManagedTable()
    {
        ...
        Assume.assumeTrue(fileFormat == FileFormat.ORC && isVectorized && testTableType == TestTables.TestTableType.HIVE_CATALOG);
        String tableName = "tbl_unsupported";
        shell.executeStatement("CREATE MANAGED TABLE " + tableName + " (a int) STORED AS " + fileFormat + " " + testTables.locationForCreateTableSQL(TableIdentifier.of("default", tableName)) + testTables.propertiesForCreateTableSQL(ImmutableMap.of()));
        shell.executeStatement("INSERT INTO " + tableName + " VALUES (1), (2), (3)");
        AssertHelpers.assertThrows("Migrating a managed table to Iceberg should have thrown an exception.", IllegalArgumentException.class, "Converting non-external, temporary or transactional hive table to iceberg", () -> shell.executeStatement("ALTER TABLE " + tableName + " convert to iceberg"));
    }
    ...
    private Table validateMigration(String tableName, String tblProperties) throws TException, InterruptedException
    {
    ...
    {
        List<String> partitions = shell.metastore().run(client -> client.listPartitionNames("default", tableName, (short) -1));
    ...
    private void validateMigrationRollback(String tableName) throws TException, InterruptedException
    {
        ...
        {
            mockedTableUtil.when(() -> HiveTableUtil.importFiles(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any(PartitionSpecProxy.class), ArgumentMatchers.anyList(), ArgumentMatchers.any(Properties.class), ArgumentMatchers.any(Configuration.class))).thenThrow(new MetaException());
            try
            {
                shell.executeStatement("ALTER TABLE " + tableName + " CONVERT TO ICEBERG");
