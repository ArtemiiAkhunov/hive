...
public class WriterBuilder
{
    ...
    public HiveIcebergWriter build()
    {
        ...
        OutputFileFactory outputFileFactory = OutputFileFactory.builderFor(table, partitionId, taskId).format(dataFileFormat).operationId("data-" + operationId).build();

        OutputFileFactory deleteOutputFileFactory = OutputFileFactory.builderFor(table, partitionId, taskId).format(deleteFileFormat).operationId("delete-" + operationId).build();

        HiveFileWriterFactory writerFactory = new HiveFileWriterFactory(table, dataFileFormat, dataSchema, null, deleteFileFormat, null, null, null, skipRowData ? null : dataSchema);
        ...
        } else
        {
            switch (operation)
            {
                case DELETE:
                    ...
                    break;
                case OTHER:
                    writer = new HiveIcebergRecordWriter(dataSchema, specs, currentSpecId, writerFactory, outputFileFactory, io, targetFileSize, fanoutEnabled);
                    break;
                default:
                    // Update and Merge should be splitted to inserts and deletes
                    throw new IllegalArgumentException("Unsupported operation when creating IcebergRecordWriter: " + operation.name());
            }
        }
