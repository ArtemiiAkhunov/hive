...
public final class Catalogs
{
    ...
    {
        return loadTable(conf, conf.get(InputFormatConfig.TABLE_IDENTIFIER), conf.get(InputFormatConfig.TABLE_LOCATION), conf.get(InputFormatConfig.CATALOG_NAME));
    }
    ...
    {
        return loadTable(conf, props.getProperty(NAME), props.getProperty(LOCATION), props.getProperty(InputFormatConfig.CATALOG_NAME));
    }
    ...
    static Optional<Catalog> loadCatalog(Configuration conf, String catalogName)
    {
        ...
        } else
        {
            ...
            return Optional.of(CatalogUtil.buildIcebergCatalog(name, getCatalogProperties(conf, name, catalogType), conf));
        }
    }
    ...
    private static Map<String, String> getCatalogProperties(Configuration conf, String catalogName, String catalogType)
    {
        ...
        conf.forEach(config ->
        {
            ...
            {
                catalogProperties.putIfAbsent(config.getKey().substring(InputFormatConfig.CATALOG_DEFAULT_CONFIG_PREFIX.length()), config.getValue());
    ...
    private static String getCatalogType(Configuration conf, String catalogName)
    {
        if (catalogName != null)
        {
            String catalogType = conf.get(InputFormatConfig.catalogPropertyConfigKey(catalogName, CatalogUtil.ICEBERG_CATALOG_TYPE));
