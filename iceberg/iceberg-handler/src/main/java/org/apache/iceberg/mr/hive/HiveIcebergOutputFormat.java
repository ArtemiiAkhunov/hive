...
public class HiveIcebergOutputFormat<T> implements OutputFormat<NullWritable, Container<Record>>, HiveOutputFormat<NullWritable, Container<Record>>
{
    ...
    @Override
    public org.apache.hadoop.mapred.RecordWriter<NullWritable, Container<Record>> getRecordWriter(FileSystem ignored, JobConf job, String name, Progressable progress)
    {
        return writer(job);
    }
    ...
    private static void setWriterLevelConfiguration(JobConf jc, Table table)
    {
        ...
        {
            if (table.properties().get(TableProperties.PARQUET_ROW_GROUP_SIZE_BYTES) == null && jc.get(ParquetOutputFormat.BLOCK_SIZE) != null)
            {
                table.properties().put(TableProperties.PARQUET_ROW_GROUP_SIZE_BYTES, jc.get(ParquetOutputFormat.BLOCK_SIZE));
            }
            if (table.properties().get(TableProperties.PARQUET_COMPRESSION) == null && jc.get(ParquetOutputFormat.COMPRESSION) != null)
            {
