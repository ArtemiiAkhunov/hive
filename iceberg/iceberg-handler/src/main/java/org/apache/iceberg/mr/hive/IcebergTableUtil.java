...
public class IcebergTableUtil
{
    ...
    static Table getTable(Configuration configuration, org.apache.hadoop.hive.metastore.api.Table hmsTable, boolean skipCache)
    {
        ...
        hmsTable.getParameters().computeIfPresent(InputFormatConfig.CATALOG_NAME, (k, v) ->
        {
    ...
    {
        List<TransformSpec> partitionTransformSpecList = SessionStateUtil.getResource(configuration, hive_metastoreConstants.PARTITION_TRANSFORM_SPEC).map(o -> (List<TransformSpec>) o).orElseGet(() -> null);
    ...
    public static void setCurrentSnapshot(Table table, String value)
    {
        ...
        try
        {
            ...
            LOG.debug("Rolling the iceberg table {} from snapshot id {} to snapshot ID {}", table.name(), table.currentSnapshot().snapshotId(), snapshotId);
        } catch (NumberFormatException e)
        {
            ...
            LOG.debug("Rolling the iceberg table {} from snapshot id {} to the snapshot ID {} of SnapshotRef {}", table.name(), table.currentSnapshot().snapshotId(), snapshotId, refName);
        }
    ...
    {
        return props != null && "2".equals(props.get(TableProperties.FORMAT_VERSION));
    }
    ...
    public static boolean isCopyOnWriteMode(Context.Operation operation, BinaryOperator<String> props)
    {
        ...
        {
            case DELETE:
                mode = props.apply(TableProperties.DELETE_MODE, TableProperties.DELETE_MODE_DEFAULT);
                break;
            case UPDATE:
                mode = props.apply(TableProperties.UPDATE_MODE, TableProperties.UPDATE_MODE_DEFAULT);
                break;
            case MERGE:
                mode = props.apply(TableProperties.MERGE_MODE, TableProperties.MERGE_MODE_DEFAULT);
                break;
        }
