...
public class HiveIcebergDeleteOrphanFiles implements DeleteOrphanFiles
{
    ...
    public Result execute()
    {
        ...
        Tasks.foreach(result.deletedFiles).executeWith(deleteExecutorService).retry(3).stopRetryOn(FileNotFoundException.class).suppressFailureWhenFinished().onFailure((file, thrown) -> LOG.warn("Delete failed for file: {}", file, thrown)).run(deleteFunc::accept);
        return result;
    }
    ...
    private Set<String> getFilesToBeDeleted(long lastTime, Set<String> validFiles, Path location) throws IOException
    {
        ...
        while (metadataLocations.hasNext())
        {
            ...
            if (metadataFile.getModificationTime() < lastTime && !validFiles.contains(getUriPath(metadataFile.getPath().toString())))
            {
    ...
    {
        return MetadataTableUtils.createMetadataTableInstance(((HasTableOperations) table).operations(), table.name(), table.name() + "#" + ALL_ENTRIES.name(), ALL_ENTRIES);
    }
