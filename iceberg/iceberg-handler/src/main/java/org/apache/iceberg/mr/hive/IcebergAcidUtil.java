...
public class IcebergAcidUtil
{
    ...
    private static final Types.NestedField PARTITION_HASH_META_COL = Types.NestedField.required(MetadataColumns.PARTITION_COLUMN_ID, MetadataColumns.PARTITION_COLUMN_NAME, Types.LongType.get());
    ...
    public static Schema createFileReadSchemaForUpdate(List<Types.NestedField> dataCols, Table table)
    {
        ...
        // Old column values
        cols.addAll(dataCols.stream().map(f -> Types.NestedField.optional(1147483545 + f.fieldId(), "__old_value_for" + f.name(), f.type())).collect(Collectors.toList()));
        // New column values
        cols.addAll(dataCols);
        return new Schema(cols);
    }
    ...
    public static Schema createSerdeSchemaForUpdate(List<Types.NestedField> dataCols)
    {
        ...
        SERDE_META_COLS.forEach((metaCol, index) -> cols.add(metaCol));
        // Old column values
        cols.addAll(dataCols.stream().map(f -> Types.NestedField.optional(1147483545 + f.fieldId(), "__old_value_for_" + f.name(), f.type())).collect(Collectors.toList()));
        // New column values
        cols.addAll(dataCols);
        return new Schema(cols);
    }
    ...
    public static class VirtualColumnAwareIterator<T> implements CloseableIterator<T>
    {
        ...
        public VirtualColumnAwareIterator(CloseableIterator<T> currentIterator, Schema expectedSchema, Configuration conf)
        {
            ...
            this.conf = conf;
            current = GenericRecord.create(new Schema(expectedSchema.columns().subList(4, expectedSchema.columns().size())));
        }
        ...
        public T next()
        {
            ...
            PositionDeleteInfo.setIntoConf(conf, IcebergAcidUtil.parseSpecId(rec), IcebergAcidUtil.computePartitionHash(rec), IcebergAcidUtil.parseFilePath(rec), IcebergAcidUtil.parseFilePosition(rec));
