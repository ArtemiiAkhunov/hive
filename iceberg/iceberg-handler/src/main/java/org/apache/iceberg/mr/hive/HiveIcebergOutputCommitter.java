...
public class HiveIcebergOutputCommitter extends OutputCommitter
{
    ...
    public boolean needsTaskCommit(TaskAttemptContext context)
    {
        ...
        return TaskType.REDUCE.equals(context.getTaskAttemptID().getTaskID().getTaskType()) || context.getJobConf().getNumReduceTasks() == 0;
    }
    ...
    public void commitTask(TaskAttemptContext originalContext) throws IOException
    {
        ...
        Map<String, List<HiveIcebergWriter>> writers = Optional.ofNullable(WriterRegistry.writers(attemptID)).orElseGet(() ->
        {
            ...
            Tasks.foreach(outputs).retry(3).stopOnFailure().throwFailureWhenFinished().executeWith(tableExecutor).run(output ->
            {
                ...
                if (table != null)
                {
                    ...
                    if (writers.get(output) != null)
                    {
                        ...
                        createFileForCommit(new FilesForCommit(dataFiles, deleteFiles, replacedDataFiles, referencedDataFiles), fileForCommitLocation, table.io());
                    } else
                    {
    ...
    public void commitJobs(List<JobContext> originalContextList, Operation operation) throws IOException
    {
        ...
        try
        {
            ...
            Tasks.foreach(outputs).throwFailureWhenFinished().stopOnFailure().executeWith(tableExecutor).run(output ->
            {
                ...
                jobLocations.addAll(output.jobContexts.stream().map(jobContext -> generateJobLocation(table.location(), jobContext.getJobConf(), jobContext.getJobID())).collect(Collectors.toList()));
    ...
    {
        return jobContextList.stream().flatMap(jobContext -> HiveIcebergStorageHandler.outputTables(jobContext.getJobConf()).stream().map(output -> new OutputTable(HiveIcebergStorageHandler.catalogName(jobContext.getJobConf(), output), output, SessionStateUtil.getResource(jobContext.getJobConf(), output).filter(o -> o instanceof Table).map(o -> (Table) o)
                // fall back to getting the serialized table from the config
                .orElseGet(() -> HiveIcebergStorageHandler.table(jobContext.getJobConf(), output)))).filter(output -> Objects.nonNull(output.table)).map(output -> new SimpleImmutableEntry<>(output, jobContext))).collect(Collectors.groupingBy(Map.Entry::getKey, Collectors.mapping(Map.Entry::getValue, Collectors.toList()))).entrySet().stream().map(kv ->
        {
    ...
    public void abortJobs(List<JobContext> originalContextList) throws IOException
    {
        ...
        try
        {
            ...
            Tasks.foreach(outputs.stream().flatMap(kv -> kv.jobContexts.stream().map(jobContext -> new SimpleImmutableEntry<>(kv.table, jobContext)))).suppressFailureWhenFinished().executeWith(tableExecutor).onFailure((output, exc) -> LOG.warn("Failed cleanup table {} on abort job", output, exc)).run(output ->
            {
                ...
                Tasks.foreach(results.allFiles()).retry(3).suppressFailureWhenFinished().executeWith(fileExecutor).onFailure((file, exc) -> LOG.warn("Failed to remove data file {} on abort job", file.path(), exc)).run(file -> table.io().deleteFile(file.path().toString()));
            }, IOException.class);
        } finally
        {
            fileExecutor.shutdown();
    ...
    private void commitTable(FileIO io, ExecutorService executor, OutputTable outputTable, Operation operation)
    {
        ...
        for (JobContext jobContext : outputTable.jobContexts)
        {
            ...
            Expression jobContextFilterExpr = (Expression) SessionStateUtil.getResource(conf, InputFormatConfig.QUERY_FILTERS).orElse(Expressions.alwaysTrue());
            ...
            LOG.info("Committing job has started for table: {}, using location: {}", table, generateJobLocation(outputTable.table.location(), conf, jobContext.getJobID()));

            int numTasks = SessionStateUtil.getCommitInfo(conf, name).map(info -> info.get(jobContext.getJobID().toString())).map(SessionStateUtil.CommitInfo::getTaskNum).orElseGet(() ->
            {
            ...
            FilesForCommit writeResults = collectResults(numTasks, executor, outputTable.table.location(), jobContext, io, true);
        ...
        if (Operation.IOW != operation)
        {
            ...
            {
                LOG.info("Not creating a new commit for table: {}, jobIDs: {}, since there were no new files to add", table, outputTable.jobContexts.stream().map(JobContext::getJobID).map(String::valueOf).collect(Collectors.joining(",")));
            } else
            {
        ...
        } else
        {
            ...
            RewritePolicy rewritePolicy = RewritePolicy.fromString(outputTable.jobContexts.stream().findAny().map(x -> x.getJobConf().get(ConfVars.REWRITE_POLICY.varname)).orElse(RewritePolicy.DEFAULT.name()));
     ...
     */
    private void commitWrite(Table table, String branchName, Long snapshotId, long startTime, FilesForCommit results, Operation operation, Expression filterExpr)
    {
     ...
     */
    private void commitOverwrite(Table table, String branchName, long startTime, FilesForCommit results, RewritePolicy rewritePolicy)
    {
        ...
        if (!results.dataFiles().isEmpty())
        {
            ...
            LOG.info("Overwrite commit took {} ms for table: {} with {} file(s)", System.currentTimeMillis() - startTime, table, results.dataFiles().size());
    ...
    private void cleanup(JobContext jobContext, Collection<String> jobLocations) throws IOException
    {
        ...
        Tasks.foreach(jobLocations).retry(3).suppressFailureWhenFinished().onFailure((jobLocation, exc) -> LOG.debug("Failed to remove directory {} on job cleanup", jobLocation, exc)).run(jobLocation ->
        {
    ...
    {
        int size = conf.getInt(InputFormatConfig.COMMIT_FILE_THREAD_POOL_SIZE, InputFormatConfig.COMMIT_FILE_THREAD_POOL_SIZE_DEFAULT);
        return Executors.newFixedThreadPool(size, new ThreadFactoryBuilder().setDaemon(true).setPriority(Thread.NORM_PRIORITY).setNameFormat("iceberg-commit-file-pool-%d").build());
    }
