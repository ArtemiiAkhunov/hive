...
class HiveIcebergCopyOnWriteRecordWriter extends HiveIcebergWriterBase
{
    ...
    {
        super(schema, specs, io, new ClusteredDataWriter<>(fileWriterFactory, fileFactory, io, targetFileSize));
    ...
    public void write(Writable row) throws IOException
    {
        ...
        if (positionDelete.pos() < 0)
        {
            ...
            DataFile dataFile = DataFiles.builder(specs.get(specId)).withPath(positionDelete.path().toString()).withPartition(partition(rowData, specId)).withFileSizeInBytes(0).withRecordCount(0).build();
