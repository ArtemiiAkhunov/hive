...
public class VectorizedReadUtils
{
    ...
    public static ByteBuffer getSerializedOrcTail(Path path, SyntheticFileId fileId, JobConf job) throws IOException
    {
        ...
        if (result == null)
        {
            org.apache.orc.OrcFile.ReaderOptions readerOptions = org.apache.orc.OrcFile.readerOptions(job).useUTCTimestamp(true).filesystem(path.getFileSystem(job));

            try (org.apache.orc.impl.ReaderImpl orcFileReader = (org.apache.orc.impl.ReaderImpl) OrcFile.createReader(path, readerOptions))
            {
