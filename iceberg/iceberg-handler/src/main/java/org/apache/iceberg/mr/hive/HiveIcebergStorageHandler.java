...
public class HiveIcebergStorageHandler implements HiveStoragePredicateHandler, HiveStorageHandler
{
            ...
            if (table.currentSnapshot() != null)
            {
                ...
                if (summary != null)
                {
                    ...
                    if (summary.containsKey(SnapshotSummary.TOTAL_RECORDS_PROP))
                    {
                        ...
                        if (summary.containsKey(SnapshotSummary.TOTAL_EQ_DELETES_PROP) && summary.containsKey(SnapshotSummary.TOTAL_POS_DELETES_PROP))
                        {
    ...
    private boolean writeColStats(ColumnStatistics tableColStats, Table tbl)
    {
        try
        {
            ...
            try (PuffinWriter puffinWriter = Puffin.write(tbl.io().newOutputFile(statsPath)).createdBy(Constants.HIVE_ENGINE).build())
            {
                long snapshotId = tbl.currentSnapshot().snapshotId();
                long snapshotSequenceNumber = tbl.currentSnapshot().sequenceNumber();
                puffinWriter.add(new Blob(ColumnStatisticsObj.class.getSimpleName(), ImmutableList.of(1), snapshotId, snapshotSequenceNumber, ByteBuffer.wrap(serializeColStats), PuffinCompressionCodec.NONE, ImmutableMap.of()));
                puffinWriter.finish();
    ...
    {
        Table table = IcebergTableUtil.getTable(conf, hmsTable.getTTable());
        return IcebergTableUtil.getColStatsPath(table).map(statsPath -> readColStats(table, statsPath)).orElse(new ColumnStatistics()).getStatsObj();
    }
    ...
    public boolean canComputeQueryUsingStats(org.apache.hadoop.hive.ql.metadata.Table hmsTable)
    {
        if (getStatsSource().equals(HiveMetaHook.ICEBERG) && hmsTable.getMetaTable() == null)
        {
            ...
            {
                Map<String, String> summary = table.currentSnapshot().summary();
                if (summary != null && summary.containsKey(SnapshotSummary.TOTAL_EQ_DELETES_PROP) && summary.containsKey(SnapshotSummary.TOTAL_POS_DELETES_PROP))
                {
    ...
    {
        return HiveConf.getVar(conf, HiveConf.ConfVars.HIVE_ICEBERG_STATS_SOURCE, HiveMetaHook.ICEBERG).toUpperCase();
    }

    private boolean shouldRewriteColStats(Table tbl)
    {
        return SessionStateUtil.getQueryState(conf).map(QueryState::getHiveOperation).filter(opType -> HiveOperation.ANALYZE_TABLE == opType).isPresent() || IcebergTableUtil.getColStatsPath(tbl).isPresent();
    }
    ...
    private void checkAndMergeColStats(ColumnStatistics statsObjNew, Table tbl) throws InvalidObjectException
    {
        ...
        {
            ColumnStatistics statsObjOld = IcebergTableUtil.getColStatsPath(tbl, previousSnapshotId).map(statsPath -> readColStats(tbl, statsPath)).orElse(null);
    ...
    public List<TransformSpec> getPartitionTransformSpec(org.apache.hadoop.hive.ql.metadata.Table hmsTable)
    {
        ...
        return table.spec().fields().stream().map(f -> getTransformSpec(table, f.transform().toString().toUpperCase(), f.sourceId())).collect(Collectors.toList());
    }

    private List<TransformSpec> getSortTransformSpec(Table table)
    {
        return table.sortOrder().fields().stream().map(s -> getTransformSpec(table, s.transform().toString().toUpperCase(), s.sourceId())).collect(Collectors.toList());
    }
    ...
    private TransformSpec getTransformSpec(Table table, String transformName, int sourceId)
    {
        ...
        {
            spec.setTransformType(TransformSpec.TransformType.valueOf(transformName.substring(0, transformName.indexOf("["))));
    ...
    @Override
    public DynamicPartitionCtx createDPContext(HiveConf hiveConf, org.apache.hadoop.hive.ql.metadata.Table hmsTable, Operation writeOperation) throws SemanticException
    {
    ...
    private void addCustomSortExpr(Table table, org.apache.hadoop.hive.ql.metadata.Table hmsTable, Operation writeOperation, List<Function<List<ExprNodeDesc>, ExprNodeDesc>> customSortExprs, List<TransformSpec> transformSpecs)
    {
        ...
        int offset = (shouldOverwrite(hmsTable, writeOperation) ? ACID_VIRTUAL_COLS_AS_FIELD_SCHEMA : acidSelectColumns(hmsTable, writeOperation)).size();

        customSortExprs.addAll(transformSpecs.stream().map(spec -> IcebergTransformSortFunctionUtil.getCustomSortExprs(spec, fieldOrderMap.get(spec.getColumnName()) + offset)).collect(Collectors.toList()));
    }
    ...
    @Override
    public void storageHandlerCommit(Properties commitProperties, Operation operation) throws HiveException
    {
        ...
        } catch (Throwable e)
        {
            ...
            LOG.error("Error while trying to commit job: {}, starting rollback changes for table: {}", ids, tableName, e);
            try
            {
    ...
    public void executeOperation(org.apache.hadoop.hive.ql.metadata.Table hmsTable, AlterTableExecuteSpec executeSpec)
    {
        ...
        switch (executeSpec.getOperationType())
        {
            ...
            case EXPIRE_SNAPSHOT:
                LOG.info("Executing expire snapshots operation on iceberg table {}.{}", hmsTable.getDbName(), hmsTable.getTableName());
                ...
                int numThreads = conf.getInt(HiveConf.ConfVars.HIVE_ICEBERG_EXPIRE_SNAPSHOT_NUMTHREADS.varname, HiveConf.ConfVars.HIVE_ICEBERG_EXPIRE_SNAPSHOT_NUMTHREADS.defaultIntVal);
            ...
            case SET_CURRENT_SNAPSHOT:
                ...
                LOG.debug("Executing set current snapshot operation on iceberg table {}.{} to version {}", hmsTable.getDbName(), hmsTable.getTableName(), setSnapshotVersionSpec.getSnapshotIdOrRefName());
            ...
            case DELETE_ORPHAN_FILES:
                int numDeleteThreads = conf.getInt(HiveConf.ConfVars.HIVE_ICEBERG_EXPIRE_SNAPSHOT_NUMTHREADS.varname, HiveConf.ConfVars.HIVE_ICEBERG_EXPIRE_SNAPSHOT_NUMTHREADS.defaultIntVal);
    ...
    private void expireSnapshot(Table icebergTable, AlterTableExecuteSpec.ExpireSnapshotsSpec expireSnapshotsSpec, int numThreads)
    {
        ...
        try
        {
            ...
            {
                expireSnapshotByTimestampRange(icebergTable, expireSnapshotsSpec.getFromTimestampMillis(), expireSnapshotsSpec.getTimestampMillis(), deleteExecutorService);
    ...
    private void expireSnapshotByTimestampRange(Table icebergTable, Long fromTimestamp, Long toTimestamp, ExecutorService deleteExecutorService)
    {
        ...
        for (Snapshot snapshot : icebergTable.snapshots())
        {
            if (snapshot.timestampMillis() >= fromTimestamp && snapshot.timestampMillis() <= toTimestamp)
            {
                ...
                LOG.debug("Expiring snapshot on {} with id: {} and timestamp: {}", icebergTable.name(), snapshot.snapshotId(), snapshot.timestampMillis());
            }
        }
    ...
    public URI getURIForAuth(org.apache.hadoop.hive.metastore.api.Table hmsTable) throws URISyntaxException
    {
        ...
        StringBuilder authURI = new StringBuilder(ICEBERG_URI_PREFIX).append(encodeString(dbName)).append("/").append(encodeString(tableName)).append("?snapshot=");
        ...
        Optional<String> metadataLocation = SessionStateUtil.getProperty(conf, BaseMetastoreTableOperations.METADATA_LOCATION_PROP);
        ...
        } else
        {
            ...
            if (locationProperty.isPresent())
            {
                ...
                authURI.append(getPathForAuth(locationProperty.get())).append(encodeString("/metadata/dummy.metadata.json"));
            } else
            {
                Table table = IcebergTableUtil.getTable(conf, hmsTable);
                authURI.append(getPathForAuth(((BaseTable) table).operations().current().metadataFileLocation(), hmsTable.getSd().getLocation()));
            }
        }
    ...
    {
        return getPathForAuth(locationProperty, SessionStateUtil.getProperty(conf, SessionStateUtil.DEFAULT_TABLE_LOCATION).orElse(null));
    }

    String getPathForAuth(String locationProperty, String defaultTableLocation)
    {
        boolean maskDefaultLocation = conf.getBoolean(HiveConf.ConfVars.HIVE_ICEBERG_MASK_DEFAULT_LOCATION.varname, HiveConf.ConfVars.HIVE_ICEBERG_MASK_DEFAULT_LOCATION.defaultBoolVal);
        String location = URI.create(locationProperty).getPath();
        if (!maskDefaultLocation || defaultTableLocation == null || !arePathsInSameFs(locationProperty, defaultTableLocation))
        {
        ...
        try
        {
            ...
            Path defaultLocationPath = locationPath.toUri().getScheme() != null ? FileUtils.makeQualified(new Path(defaultTableLocation), conf) : Path.getPathWithoutSchemeAndAuthority(new Path(defaultTableLocation));
    ...
    private boolean arePathsInSameFs(String locationProperty, String defaultTableLocation)
    {
        try
        {
            return FileUtils.equalsFileSystem(new Path(locationProperty).getFileSystem(conf), new Path(defaultTableLocation).getFileSystem(conf));
        } catch (IOException e)
        {
    ...
    public void validateSinkDesc(FileSinkDesc sinkDesc) throws SemanticException
    {
        ...
        if (sinkDesc.getInsertOverwrite())
        {
            ...
            if (table.currentSnapshot() != null && Long.parseLong(table.currentSnapshot().summary().get(SnapshotSummary.TOTAL_RECORDS_PROP)) == 0)
            {
                // If the table is empty we don't have any danger that some data can get lost.
                return;
            }
            if (RewritePolicy.fromString(conf.get(ConfVars.REWRITE_POLICY.varname, RewritePolicy.DEFAULT.name())) == RewritePolicy.ALL_PARTITIONS)
            {
    ...
    public List<FieldSchema> acidSelectColumns(org.apache.hadoop.hive.ql.metadata.Table table, Operation operation)
    {
        switch (operation)
        {
            ...
            case UPDATE:
                return shouldOverwrite(table, operation) ? ACID_VIRTUAL_COLS_AS_FIELD_SCHEMA : ListUtils.union(ACID_VIRTUAL_COLS_AS_FIELD_SCHEMA, table.getCols());
            case MERGE:
    ...
    public List<FieldSchema> sortColumns(org.apache.hadoop.hive.ql.metadata.Table hmsTable)
    {
        ...
        return table.sortOrder().fields().stream().map(s -> new FieldSchema(schema.findColumnName(s.sourceId()), colNameToColType.get(schema.findColumnName(s.sourceId())), String.format("Transform: %s, Sort direction: %s, Null sort order: %s", s.transform().toString(), s.direction().name(), s.nullOrder().name()))).collect(Collectors.toList());
    }
    ...
    public static Table table(Configuration config, String name)
    {
        ...
        if (table == null && config.getBoolean(hive_metastoreConstants.TABLE_IS_CTAS, false) && StringUtils.isNotBlank(config.get(InputFormatConfig.TABLE_LOCATION)))
        {
    ...
    {
        if (table != null && config.getBoolean(InputFormatConfig.CONFIG_SERIALIZATION_DISABLED, InputFormatConfig.CONFIG_SERIALIZATION_DISABLED_DEFAULT) && table.io() instanceof HadoopConfigurable)
        {
    ...
    static void overlayTableProperties(Configuration configuration, TableDesc tableDesc, Map<String, String> map)
    {
        ...
        Maps.fromProperties(props).entrySet().stream().filter(entry -> !map.containsKey(entry.getKey())) // map overrides tableDesc properties
    ...
    private void fallbackToNonVectorizedModeBasedOnProperties(Properties tableProps)
    {
        ...
        if (FileFormat.AVRO.name().equalsIgnoreCase(tableProps.getProperty(TableProperties.DEFAULT_FILE_FORMAT)) || isValidMetadataTable(tableProps.getProperty(IcebergAcidUtil.META_TABLE_PROPERTY)) || hasOrcTimeInSchema(tableProps, tableSchema) || !hasParquetNestedTypeWithinListOrMap(tableProps, tableSchema))
        {
    ...
    private List<JobContext> generateJobContext(Configuration configuration, String tableName, String branchName)
    {
        ...
        Optional<Map<String, SessionStateUtil.CommitInfo>> commitInfoMap = SessionStateUtil.getCommitInfo(jobConf, tableName);
    ...
    {
        return SessionStateUtil.getProperty(conf, Operation.class.getSimpleName()).orElse(Operation.OTHER.name());
    }
    ...
    {
        if (alterTableDesc instanceof AlterTableSetPropertiesDesc && alterTableDesc.getProps().containsKey(BaseMetastoreTableOperations.METADATA_LOCATION_PROP))
        {
    ...
    public void setTableParametersForCTLT(org.apache.hadoop.hive.ql.metadata.Table tbl, CreateTableLikeDesc desc, Map<String, String> origParams)
    {
        ...
        {
            tbl.getParameters().put(InputFormatConfig.TABLE_SCHEMA, origParams.get(InputFormatConfig.TABLE_SCHEMA));
            tbl.getParameters().put(InputFormatConfig.PARTITION_SPEC, origParams.get(InputFormatConfig.PARTITION_SPEC));
        } else
        {
    ...
    public void addResourcesForCreateTable(Map<String, String> tblProps, HiveConf hiveConf)
    {
        ...
        {
            SessionStateUtil.addResourceOrThrow(hiveConf, BaseMetastoreTableOperations.METADATA_LOCATION_PROP, metadataLocation);
        }
    }
    ...
    public List<String> showPartitions(DDLOperationContext context, org.apache.hadoop.hive.ql.metadata.Table hmstbl) throws HiveException
    {
        ...
        try
        {
            ...
            try (RecordReader<WritableComparable, Writable> reader = inputFormat.getRecordReader(splits[0], job, Reporter.NULL))
            {
                return getPartitions(context, job, reader, hmstbl);
            }
        } catch (Exception e)
        {
            throw new HiveException(e, ErrorMsg.GENERIC_ERROR, "show partitions for table " + hmstbl.getTableName() + ". " + ErrorMsg.TABLE_NOT_PARTITIONED + " or the table is empty ");
        }
    }
    ...
    {
        Map<String, PartitionField> partitionFieldMap = table.spec().fields().stream().collect(Collectors.toMap(PartitionField::name, Function.identity()));
    ...
    public boolean canPerformMetadataDelete(org.apache.hadoop.hive.ql.metadata.Table hmsTable, String branchName, SearchArgument sarg)
    {
        ...
        {
            LOG.warn("Unable to create Iceberg filter," + " continuing without metadata delete: ", e);
            return false;
        }
        Table table = IcebergTableUtil.getTable(conf, hmsTable.getTTable());
    ...
    public List<FieldSchema> getPartitionKeys(org.apache.hadoop.hive.ql.metadata.Table hmsTable)
    {
        ...
        List<FieldSchema> hiveSchema = HiveSchemaUtil.convert(schema);
        Map<String, String> colNameToColType = hiveSchema.stream().collect(Collectors.toMap(FieldSchema::getName, FieldSchema::getType));
        return icebergTable.spec().fields().stream().map(partField -> new FieldSchema(schema.findColumnName(partField.sourceId()), colNameToColType.get(schema.findColumnName(partField.sourceId())), String.format("Transform: %s", partField.transform().toString()))).collect(Collectors.toList());
    }
    ...
    public List<Partition> getPartitionsByExpr(org.apache.hadoop.hive.ql.metadata.Table hmsTable, ExprNodeDesc desc) throws SemanticException
    {
        ...
        try (CloseableIterable<FileScanTask> fileScanTasks = partitionsTable.newScan().planFiles())
        {
            ...
            })).stream().filter(partitionData -> resEval.residualFor(partitionData).isEquivalentTo(Expressions.alwaysTrue())).collect(Collectors.toSet())));


            return partitionList.stream().map(partitionData -> new DummyPartition(hmsTable, pSpec.partitionToPath(partitionData))).collect(Collectors.toList());
        } catch (IOException e)
        {
            throw new SemanticException(String.format("Error while fetching the partitions due to: %s", e));
        }
