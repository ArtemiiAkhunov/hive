...
public class HiveIcebergMetaHook implements HiveMetaHook
{
    ...
    private static final Set<String> PROPERTIES_TO_REMOVE = ImmutableSet
            ...
            .of(BaseMetastoreTableOperations.METADATA_LOCATION_PROP, BaseMetastoreTableOperations.PREVIOUS_METADATA_LOCATION_PROP);
    static final EnumSet<AlterTableType> SUPPORTED_ALTER_OPS = EnumSet.of(AlterTableType.ADDCOLS, AlterTableType.REPLACE_COLUMNS, AlterTableType.RENAME_COLUMN, AlterTableType.ADDPROPS, AlterTableType.DROPPROPS, AlterTableType.SETPARTITIONSPEC, AlterTableType.UPDATE_COLUMNS, AlterTableType.RENAME, AlterTableType.EXECUTE, AlterTableType.CREATE_BRANCH, AlterTableType.CREATE_TAG, AlterTableType.DROP_BRANCH, AlterTableType.DROPPARTITION, AlterTableType.DROP_TAG, AlterTableType.COMPACT);
    ...
    private static final List<org.apache.commons.lang3.tuple.Pair<Integer, byte[]>> EMPTY_FILTER = Lists.newArrayList(org.apache.commons.lang3.tuple.Pair.of(1, new byte[0]));
    ...
    {
        this.catalogProperties = getCatalogProperties(hmsTable);
        this.deleteIcebergTable = hmsTable.getParameters() != null && "TRUE".equalsIgnoreCase(hmsTable.getParameters().get(InputFormatConfig.EXTERNAL_TABLE_PURGE));
    ...
    public void commitDropTable(org.apache.hadoop.hive.metastore.api.Table hmsTable, boolean deleteData)
    {
        if (deleteData && deleteIcebergTable)
        {
            ...
            } catch (Exception e)
            {
                ...
                LOG.warn("Exception during commitDropTable operation for table {}.{}.", hmsTable.getDbName(), hmsTable.getTableName(), e);
            }
        }
    }
    ...
    public void preAlterTable(org.apache.hadoop.hive.metastore.api.Table hmsTable, EnvironmentContext context) throws MetaException
    {
        ...
        {
            catalogProperties.put(Catalogs.NAME, TableIdentifier.of(context.getProperties().get(OLD_DB_NAME), context.getProperties().get(OLD_TABLE_NAME)).toString());
        }
        if (commitLock == null)
        {
            commitLock = new MetastoreLock(conf, new CachedClientPool(conf, Maps.fromProperties(catalogProperties)), catalogProperties.getProperty(Catalogs.NAME), hmsTable.getDbName(), hmsTable.getTableName());
        }
    ...
    private void doPreAlterTable(org.apache.hadoop.hive.metastore.api.Table hmsTable, EnvironmentContext context) throws MetaException
    {
        ...
        } catch (NoSuchTableException nte)
        {
            ...
            try
            {
                ...
                preAlterTableProperties.partitionSpecProxy = db.getMSC().listPartitionSpecs(hmsTable.getCatName(), hmsTable.getDbName(), hmsTable.getTableName(), Integer.MAX_VALUE);
    ...
    private void assertNotCrossTableMetadataLocationChange(Map<String, String> tblParams, EnvironmentContext context)
    {
        ...
        {
            Preconditions.checkArgument(icebergTable != null, "Cannot perform table migration to Iceberg and setting the snapshot location in one step. " + "Please migrate the table first");
            ...
            {
                throw new UnsupportedOperationException(String.format("Cannot change iceberg table %s metadata location pointing to another table's metadata %s", icebergTable.name(), newMetadataLocation));
            }
    ...
    private void assertTableCanBeMigrated(org.apache.hadoop.hive.metastore.api.Table hmsTable) throws MetaException
    {
        ...
        List<TypeInfo> typeInfos = hmsTable.getSd().getCols().stream().map(f -> TypeInfoUtils.getTypeInfoFromTypeString(f.getType())).collect(Collectors.toList());
    ...
    public void commitAlterTable(org.apache.hadoop.hive.metastore.api.Table hmsTable, EnvironmentContext context) throws MetaException
    {
        ...
        if (isTableMigration)
        {
            ...
            HiveTableUtil.importFiles(preAlterTableProperties.tableLocation, preAlterTableProperties.format, preAlterTableProperties.partitionSpecProxy, preAlterTableProperties.partitionKeys, catalogProperties, conf);
        } else if (currentAlterTableOp != null)
        {
            switch (currentAlterTableOp)
            {
                ...
                case SETPARTITIONSPEC:
                    ...
                    break;
                case RENAME:
                    Catalogs.renameTable(conf, catalogProperties, TableIdentifier.of(hmsTable.getDbName(), hmsTable.getTableName()));
                    break;
            }
        }
    }
    ...
    public void preTruncateTable(org.apache.hadoop.hive.metastore.api.Table table, EnvironmentContext context, List<String> partNames) throws MetaException
    {
        ...
        this.icebergTable = Catalogs.loadTable(conf, catalogProperties);
        Map<String, PartitionField> partitionFieldMap = icebergTable.spec().fields().stream().collect(Collectors.toMap(PartitionField::name, Function.identity()));
            ...
            for (String partName : partNames)
            {
                ...
                for (Map.Entry<String, String> entry : specMap.entrySet())
                {
                    ...
                    if (partitionFieldMap.containsKey(entry.getKey()))
                    {
                        ...
                        Type resultType = partitionField.transform().getResultType(icebergTable.schema().findField(partitionField.sourceId()).type());
    ...
    private Schema schema(Properties properties, org.apache.hadoop.hive.metastore.api.Table hmsTable, Set<String> identifierFields)
    {
    ...
    private Schema getSchemaWithIdentifierFields(Schema schema, Set<String> identifierFields)
    {
        ...
        Set<Integer> identifierFieldIds = identifierFields.stream().map(column ->
        {
            ...
            Preconditions.checkNotNull(field, "Cannot find identifier field ID for the column %s in schema %s", column, schema);
            return field.fieldId();
        }).collect(Collectors.toSet());

        List<Types.NestedField> cols = schema.columns().stream().map(column -> identifierFieldIds.contains(column.fieldId()) ? column.asRequired() : column).collect(Collectors.toList());
    ...
    {
        Collection<FieldSchema> addedCols = HiveSchemaUtil.getSchemaDiff(hmsTable.getSd().getCols(), HiveSchemaUtil.convert(icebergTable.schema()), false).getMissingFromSecond();
        ...
        {
            updateSchema.addColumn(addedCol.getName(), HiveSchemaUtil.convert(TypeInfoUtils.getTypeInfoFromTypeString(addedCol.getType())), addedCol.getComment());
        }
        updateSchema.commit();
    }
    ...
    private void handleReplaceColumns(org.apache.hadoop.hive.metastore.api.Table hmsTable) throws MetaException
    {
        ...
        if (!schemaDifference.getMissingFromSecond().isEmpty() || !schemaDifference.getTypeChanged().isEmpty() || !schemaDifference.getCommentChanged().isEmpty() || outOfOrder != null)
        {
        ...
        transaction = icebergTable.newTransaction();
        updateSchema = transaction.updateSchema();
        LOG.info("handleReplaceColumns: Dropping the following columns for Iceberg table {}, cols: {}", hmsTable.getTableName(), schemaDifference.getMissingFromFirst());
    ...
    private void handleChangeColumn(org.apache.hadoop.hive.metastore.api.Table hmsTable) throws MetaException
    {
        ...
        {
            renameMapping = ImmutableMap.of(schemaDifference.getMissingFromSecond().get(0).getName(), schemaDifference.getMissingFromFirst().get(0).getName());
        }
        ...
        } else
        {
            ...
            LOG.info("Found no difference between new and old schema for ALTER TABLE CHANGE COLUMN for" + " table: {}. There will be no Iceberg commit.", hmsTable.getTableName());
            return;
        }
        ...
        } else if (!schemaDifference.getTypeChanged().isEmpty())
        {
            ...
            updateSchema.updateColumn(updatedField.getName(), getPrimitiveTypeOrThrow(updatedField), updatedField.getComment());
    ...
    {
        return !"FALSE".equalsIgnoreCase(hmsTable.getParameters().get(ORC_FILES_ONLY)) && ((hmsTable.getSd().getInputFormat() != null && hmsTable.getSd().getInputFormat().toUpperCase().contains(org.apache.iceberg.FileFormat.ORC.name())) || org.apache.iceberg.FileFormat.ORC.name().equalsIgnoreCase(hmsTable.getSd().getSerdeInfo().getParameters().get("write.format.default")) || org.apache.iceberg.FileFormat.ORC.name().equalsIgnoreCase(hmsTable.getParameters().get("write.format.default")));
    }

    // TODO: remove this if copy-on-write mode gets implemented in Hive
    private void setDeleteModeOnTableProperties(Table icebergTbl, Map<String, String> newProps, EnvironmentContext context)
    {
        ...
        if ((icebergTbl == null || ((BaseTable) icebergTbl).operations().current().formatVersion() == 1) && IcebergTableUtil.isV2Table(newProps))
        {
            List<String> writeModeList = Arrays.asList(DELETE_MODE, UPDATE_MODE, MERGE_MODE);
            writeModeList.stream().filter(writeMode -> catalogProperties.get(writeMode) == null).forEach(writeMode ->
            {
    ...
    public void preDropPartitions(org.apache.hadoop.hive.metastore.api.Table hmsTable, EnvironmentContext context, List<org.apache.commons.lang3.tuple.Pair<Integer, byte[]>> partExprs) throws MetaException
    {
        ...
        {
            ExprNodeDesc exprNodeDesc = SerializationUtilities.deserializeObjectWithTypeInformation(partExpr.getRight(), true);
        ...
        try (CloseableIterable<FileScanTask> fileScanTasks = partitionsTable.newScan().planFiles())
        {
            ...
            for (PartitionData partitionData : partitionList)
            {
                ...
                for (int index = 0; index < pSpec.fields().size(); index++)
                {
                    ...
                    partFilter = Expressions.and(partFilter, Expressions.equal(field.name(), partitionData.get(index, Object.class)));
                }
