...
public class IcebergInputFormat<T> extends InputFormat<Void, T>
{
    ...
    private static <T extends Scan<T, FileScanTask, CombinedScanTask>> Scan<T, FileScanTask, CombinedScanTask> applyConfig(Configuration conf, Scan<T, FileScanTask, CombinedScanTask> scanToConfigure)
    {
        ...
        Scan<T, FileScanTask, CombinedScanTask> scan = scanToConfigure.caseSensitive(conf.getBoolean(InputFormatConfig.CASE_SENSITIVE, InputFormatConfig.CASE_SENSITIVE_DEFAULT));
    ...
    public List<InputSplit> getSplits(JobContext context)
    {
        ...
        Table table = Optional.ofNullable(HiveIcebergStorageHandler.table(conf, conf.get(InputFormatConfig.TABLE_IDENTIFIER))).orElseGet(() ->
        {
    ...
    private static final class IcebergRecordReader<T> extends RecordReader<Void, T>
    {
        ...
        public void initialize(InputSplit split, TaskAttemptContext newContext)
        {
            ...
            this.table = SerializationUtil.deserializeFromBase64(conf.get(InputFormatConfig.SERIALIZED_TABLE_PREFIX + conf.get(InputFormatConfig.TABLE_IDENTIFIER)));
            ...
            this.inMemoryDataModel = conf.getEnum(InputFormatConfig.IN_MEMORY_DATA_MODEL, InputFormatConfig.InMemoryDataModel.GENERIC);
        ...
        {
            Preconditions.checkArgument(!task.file().format().equals(FileFormat.AVRO), "Vectorized execution is not yet supported for Iceberg avro tables. " + "Please turn off vectorization and retry the query.");
            ...
            CloseableIterable<T> iterator = HIVE_VECTORIZED_READER_BUILDER.invoke(table, path, task, idToConstant, context, residual, readSchema);
        ...
        private CloseableIterable<T> openGeneric(FileScanTask task, Schema readSchema)
        {
            if (task.isDataTask())
            {
                ...
                IcebergInternalRecordWrapper wrapper = new IcebergInternalRecordWrapper(table.schema().asStruct(), readSchema.asStruct());
                return (CloseableIterable) CloseableIterable.transform(((DataTask) task).rows(), row -> wrapper.wrap(row));
            }

            DataFile file = task.file();
            InputFile inputFile = table.encryption().decrypt(EncryptedFiles.encryptedInput(table.io().newInputFile(file.path().toString()), file.keyMetadata()));
        ...
        private CloseableIterable<T> newAvroIterable(InputFile inputFile, FileScanTask task, Schema readSchema)
        {
            ...
            avroReadBuilder.createReaderFunc((expIcebergSchema, expAvroSchema) -> DataReader.create(expIcebergSchema, expAvroSchema, constantsMap(task, IdentityPartitionConverters::convertConstant)));
        ...
        private CloseableIterable<T> newParquetIterable(InputFile inputFile, FileScanTask task, Schema readSchema)
        {
            ...
            Parquet.ReadBuilder parquetReadBuilder = Parquet.read(inputFile).project(readSchema).filter(residual).caseSensitive(caseSensitive).split(task.start(), task.length());
            ...
            parquetReadBuilder.createReaderFunc(fileSchema -> GenericParquetReaders.buildReader(readSchema, fileSchema, constantsMap(task, IdentityPartitionConverters::convertConstant)));
        ...
        private CloseableIterable<T> newOrcIterable(InputFile inputFile, FileScanTask task, Schema readSchema)
        {
            ...
            ORC.ReadBuilder orcReadBuilder = ORC.read(inputFile).project(readSchemaWithoutConstantAndMetadataFields).filter(residual).caseSensitive(caseSensitive).split(task.start(), task.length());
        ...
        private static Schema readSchema(Configuration conf, Table table, boolean caseSensitive)
        {
            ...
            if (selectedColumns != null)
            {
                readSchema = caseSensitive ? readSchema.select(selectedColumns) : readSchema.caseInsensitiveSelect(selectedColumns);
            }
        ...
        private static Schema schemaWithoutConstantsAndMeta(Schema readSchema, Map<Integer, ?> idToConstant)
        {
            ...
            Set<Integer> partitionFields = Optional.ofNullable(readSchema.findField(MetadataColumns.PARTITION_COLUMN_ID)).map(Types.NestedField::type).map(Type::asStructType).map(Types.StructType::fields).map(fields -> fields.stream().map(Types.NestedField::fieldId).collect(Collectors.toSet())).orElseGet(Collections::emptySet);

            // remove constants and meta columns too
            Set<Integer> collect = Stream.of(idToConstant.keySet(), MetadataColumns.metadataFieldIds(), partitionFields).flatMap(Set::stream).collect(Collectors.toSet());
