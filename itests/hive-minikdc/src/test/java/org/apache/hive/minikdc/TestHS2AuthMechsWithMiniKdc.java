...
@CreateLdapServer(transports = {@CreateTransport(protocol = "LDAP", port = 10390), @CreateTransport(protocol = "LDAPS", port = 10640)})
...
public class TestHS2AuthMechsWithMiniKdc extends AbstractLdapTestUnit
{
    ...
    public void setUpBefore() throws Exception
    {
        if (miniHS2 == null)
        {
            ...
            hiveConf.setVar(HiveConf.ConfVars.HIVE_SERVER2_PLAIN_LDAP_URL, "ldap://localhost:" + ldapServer.getPort());
            hiveConf.setBoolVar(HiveConf.ConfVars.HIVE_SUPPORT_CONCURRENCY, false);
            hiveConf.setVar(HiveConf.ConfVars.HIVE_SERVER2_PLAIN_LDAP_USERDNPATTERN, "uid=%s,ou=People,dc=example,dc=com");

            AuthenticationProviderFactory.AuthMethods.LDAP.getConf().setVar(HiveConf.ConfVars.HIVE_SERVER2_PLAIN_LDAP_URL, "ldap://localhost:" + ldapServer.getPort());
            AuthenticationProviderFactory.AuthMethods.LDAP.getConf().setVar(HiveConf.ConfVars.HIVE_SERVER2_PLAIN_LDAP_USERDNPATTERN, "uid=%s,ou=People,dc=example,dc=com");
            miniHS2 = MiniHiveKdc.getMiniHS2WithKerb(miniHiveKdc, hiveConf, HiveAuthConstants.AuthTypes.KERBEROS.getAuthName() + "," + HiveAuthConstants.AuthTypes.LDAP.getAuthName());
