...
public class TestHS2JWTWithMiniKdc
{
    ...
    public static void setUpBeforeClass() throws Exception
    {
        ...
        miniHS2 = MiniHiveKdc.getMiniHS2WithKerb(miniHiveKdc, hiveConf, HiveAuthConstants.AuthTypes.KERBEROS.getAuthName() + "," + HiveAuthConstants.AuthTypes.JWT.getAuthName());
    ...
    {
        String krb5Url = new StringBuilder(miniHS2.getBaseHttpJdbcURL()).append("default;").append("transportMode=http;httpPath=cliservice;").append("principal=" + miniHiveKdc.getFullHiveServicePrincipal()).toString();
        ...
        StringBuilder jwtUrl = new StringBuilder(miniHS2.getBaseHttpJdbcURL()).append("default;").append("transportMode=http;httpPath=cliservice;").append("auth=jwt;").append("jwt=");

        try (Connection hs2Conn = DriverManager.getConnection(jwtUrl.append(generateJWT("user1", true, TimeUnit.MINUTES.toMillis(5))).toString()))
        {
            try (Statement statement = hs2Conn.createStatement())
            {
                statement.execute("create table if not exists test_hs2_with_jwt_kerberos(a string)");
        ...
        try
        {
            DriverManager.getConnection(jwtUrl.append(generateJWT("user1", false, TimeUnit.MINUTES.toMillis(5))).toString());
    ...
    {
        final byte[] content = authorized ? Files.readAllBytes(jwtAuthorizedKeyFile.toPath()) : Files.readAllBytes(jwtUnauthorizedKeyFile.toPath());
        ...
        JWTClaimsSet claimsSet = new JWTClaimsSet.Builder().jwtID(UUID.randomUUID().toString()).issueTime(now).issuer("auth-server").subject(user).expirationTime(expirationTime).claim("custom-claim-or-payload", "custom-claim-or-payload").build();
