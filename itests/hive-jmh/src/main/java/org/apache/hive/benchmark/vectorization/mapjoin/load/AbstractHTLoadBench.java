...
public abstract class AbstractHTLoadBench
{
    ...
    public void hashTableLoadBench() throws Exception
    {
        ...
        when(mockTezContext.getTezProcessorContext().getCounters().findCounter(anyString(), anyString())).thenReturn(mockTezCounter);
        // Replace streaming Tez-Input with our custom Iterator
        when(mockTezContext.getInput(any())).thenReturn(mockLogicalInput);
        when(mockLogicalInput.getReader()).thenReturn(customKeyValueReader);
        /* Mocking Done*/
        HashTableLoader ht = LOAD_THREADS_NUM == 0 ? new LegacyVectorMapJoinFastHashTableLoader(mockTezContext, testDesc.hiveConf, createMapJoinResult.mapJoinOperator) : new VectorMapJoinFastHashTableLoader(mockTezContext, testDesc.hiveConf, createMapJoinResult.mapJoinOperator);
        ht.load(new MapJoinTableContainer[createMapJoinResult.mapJoinOperator.getConf().getTagLength()], null);
    }

    // Common Setup
    protected void setupMapJoinHT(HiveConf hiveConf, long seed, int rowCount, VectorMapJoinVariation vectorMapJoinVariation, MapJoinTestImplementation mapJoinImplementation, String[] bigTableColumnNames, TypeInfo[] bigTableTypeInfos, int[] bigTableKeyColumnNums, TypeInfo[] smallTableValueTypeInfos, int[] smallTableRetainKeyColumnNums, SmallTableGenerationParameters smallTableGenerationParameters) throws Exception
    {
        ...
        this.testDesc = new MapJoinTestDescription(hiveConf, vectorMapJoinVariation, bigTableColumnNames, bigTableTypeInfos, bigTableKeyColumnNums, smallTableValueTypeInfos, smallTableRetainKeyColumnNums, smallTableGenerationParameters, MapJoinPlanVariation.DYNAMIC_PARTITION_HASH_JOIN);
        this.testData = new MapJoinTestData(1, testDesc, seed);

        MapJoinDesc mapJoinDesc = MapJoinTestConfig.createMapJoinDesc(testDesc);
        this.createMapJoinResult = MapJoinTestConfig.createMapJoinImplementation(mapJoinImplementation, testDesc, testData, mapJoinDesc,/* shareMapJoinTableContainer */ null);
    }
