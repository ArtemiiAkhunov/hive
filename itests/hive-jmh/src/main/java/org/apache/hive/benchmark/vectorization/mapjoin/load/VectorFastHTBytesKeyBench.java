...
public class VectorFastHTBytesKeyBench
{
    ...
    {
        Options opt = new OptionsBuilder().include(".*" + VectorFastHTBytesKeyBench.class.getSimpleName() + ".*").warmupIterations(4) // number of times the warmup iteration should take place
                .measurementIterations(4) //number of times the actual iteration should take place
                .forks(1).jvmArgs("-Xmx8g", "-Xms8g", "-XX:MaxDirectMemorySize=512M").build();
        new Runner(opt).run();
    }
}
