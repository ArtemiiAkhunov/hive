...
public class QOutProcessor
{
    ...
    private static final PatternReplacementPair MASK_STATS = new PatternReplacementPair(Pattern.compile(" Num rows: [1-9][0-9]* Data size: [1-9][0-9]*"), " Num rows: ###Masked### Data size: ###Masked###");
    private static final PatternReplacementPair MASK_DATA_SIZE = new PatternReplacementPair(Pattern.compile(" Data size: [1-9][0-9]*"), " Data size: ###Masked###");
    private static final PatternReplacementPair MASK_TIMESTAMP = new PatternReplacementPair(Pattern.compile("[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1]) (2[0-3]|[01][0-9]):[0-5][0-9]:[0-5][0-9].[0-9]{1,3} [a-zA-Z/]*"), "  ###MaskedTimeStamp### ");
    private static final PatternReplacementPair MASK_LINEAGE = new PatternReplacementPair(Pattern.compile("POSTHOOK: Lineage: .*"), "POSTHOOK: Lineage: ###Masked###");
    ...
    private final Pattern[] planMask = toPattern(new String[]{".*[.][.][.] [0-9]* more.*", "pk_-?[0-9]*_[0-9]*_[0-9]*", "fk_-?[0-9]*_[0-9]*_[0-9]*", "uk_-?[0-9]*_[0-9]*_[0-9]*", "nn_-?[0-9]*_[0-9]*_[0-9]*", // not null constraint name
    ...
    private final String[] maskIfStartsWith = new String[]{"Deleted", "Repair: Added partition to metastore", "latestOffsets", "minimumLag"};

    // Using String.contains instead of pattern, as it is much faster
    private final String[] maskIfContains = new String[]{"file:", "pfile:", "/tmp/", "invalidscheme:", "lastUpdateTime", "lastAccessTime", "lastModifiedTim", "Owner", "owner", "CreateTime", "LastAccessTime", "Location", "LOCATION '", "transient_lastDdlTime", "last_modified_", "at org", "at sun", "at java", "at junit", "LOCK_QUERYID:", "LOCK_TIME:", "grantTime", "job_", "USING 'java -cp", "DagName:", "DagId:", "total number of created files now is", "hive-staging", "at com.sun.proxy", "at com.jolbox", "at com.zaxxer", "at com.google"};

    // Using String.contains instead of pattern, as it is much faster
    private final String[][] maskIfContainsMultiple = new String[][]{{"Input:", "/data/files/"}, {"Output:", "/data/files/"}};
    ...
    private final Pattern[] partialReservedPlanMask = toPattern(new String[]{"data/warehouse/(.*?/)+\\.hive-staging"  // the directory might be db/table/partition
...

    {
        ...
        ppm.add(new PatternReplacementPair(Pattern.compile("Error: Node: (.*) : Error while running task"), "Error: Node: #NODE# : Error while running task"));

        ppm.add(new PatternReplacementPair(Pattern.compile("rowcount = [0-9]+(\\.[0-9]+(E[0-9]+)?)?, cumulative cost = \\{.*\\}, id = [0-9]*"), "rowcount = ###Masked###, cumulative cost = ###Masked###, id = ###Masked###"));
        ...
        ppm.add(new PatternReplacementPair(Pattern.compile("Vertex killed, vertexName=(.*?),.*\\[\\1\\] killed\\/failed due to:OTHER_VERTEX_FAILURE\\]"), "[Masked Vertex killed due to OTHER_VERTEX_FAILURE]"));
        ...
        {
            {
                ...
                add(toPatternPair(PATH_HDFS_WITH_DATE_USER_GROUP_REGEX, String.format("%s %s$3$4 %s $6%s", HDFS_USER_MASK, HDFS_GROUP_MASK, HDFS_DATE_MASK, HDFS_MASK)));
