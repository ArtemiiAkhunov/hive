...
public class HBaseTestSetup extends QTestSetup
{
    ...
    public void preTest(HiveConf conf) throws Exception
    {
        ...
        auxJars = (StringUtils.isBlank(auxJars) ? "" : (auxJars + ",")) + "file://" + new JobConf(conf, HBaseConfiguration.class).getJar();
    ...
    private static byte[] createAvroRecordWithNestedTimestamp() throws IOException
    {
        ...
        {
            try (DataFileWriter<GenericRecord> dataFileWriter = new DataFileWriter<GenericRecord>(new GenericDatumWriter<>(schema)))
            {
    ...
    private void createAvroTable() throws IOException
    {
        ...
        try (Admin hbaseAdmin = hbaseConn.getAdmin())
        {
            ...
            try (Table table = hbaseConn.getTable(hbaseTable))
            {
                ...
                p.add(new KeyValue("1".getBytes(), "data".getBytes(), "frV4".getBytes(), createAvroRecordWithNestedTimestamp()));
                table.put(p);
            }
        }
    }
