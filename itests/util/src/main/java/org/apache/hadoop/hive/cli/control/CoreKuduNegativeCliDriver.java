...
public class CoreKuduNegativeCliDriver extends CliAdapter
{
    ...
    {
        qt = new QTestUtil(QTestArguments.QTestArgumentsBuilder.instance().withOutDir(cliConfig.getResultsDir()).withLogDir(cliConfig.getLogDir()).withClusterType(cliConfig.getClusterType()).withConfDir(cliConfig.getHiveConfDir()).withInitScript(cliConfig.getInitScript()).withCleanupScript(cliConfig.getCleanupScript()).withLlapIo(true).withQTestSetup(new KuduTestSetup()).build());
    }
