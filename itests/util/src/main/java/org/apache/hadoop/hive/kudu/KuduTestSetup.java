...
public class KuduTestSetup extends QTestMiniClusters.QTestSetup
{
    ...
    public static final Schema KV_SCHEMA = new Schema(Arrays.asList(new ColumnSchema.ColumnSchemaBuilder("key", Type.INT32).key(true).build(), new ColumnSchema.ColumnSchemaBuilder("value", Type.STRING).build()));
    ...
    private void setupWithHiveConf(HiveConf conf) throws Exception
    {
        if (null == miniCluster)
        {
            ...
            miniCluster = new MiniKuduCluster.MiniKuduClusterBuilder().numMasterServers(3).numTabletServers(3).build();
        }
    ...
    private void updateConf(HiveConf conf)
    {
        if (miniCluster != null)
        {
            HiveConf.setVar(conf, HiveConf.ConfVars.HIVE_KUDU_MASTER_ADDRESSES_DEFAULT, miniCluster.getMasterAddressesAsString());
        }
    }
