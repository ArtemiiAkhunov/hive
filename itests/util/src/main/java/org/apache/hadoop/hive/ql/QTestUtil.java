...
public class QTestUtil
{
    ...
    {
        LOG.info("Setting up QTestUtil with outDir={}, logDir={}, clusterType={}, confDir={}," + " initScript={}, cleanupScript={}, withLlapIo={}, fsType={}", testArgs.getOutDir(), testArgs.getLogDir(), testArgs.getClusterType(), testArgs.getConfDir(), testArgs.getInitScript(), testArgs.getCleanupScript(), testArgs.isWithLlapIo(), testArgs.getFsType());
        ...
        {
            HiveConf.setHiveSiteLocation(new URL("file://" + new File(testArgs.getConfDir()).toURI().getPath() + "/hive-site.xml"));
    ...
    private CommandProcessorResponse executeTestCommand(String command) throws CommandProcessorException
    {
        ...
        String wareHouseDir = SessionState.get().getConf().getVar(ConfVars.METASTORE_WAREHOUSE).replaceAll("^[a-zA-Z]+://.*?:\\d+", "");
    ...
    public QTestProcessExecResult checkNegativeResults(String tname, Error e) throws Exception
    {
        ...
        outfd.write("FAILED: " + e.getClass().getSimpleName() + " " + e.getClass().getName() + ": " + e.getMessage() + "\n");
        outfd.close();
    ...
    {
        Assert.fail("Client Execution was expected to fail, but succeeded with error code 0 for fname=" + fname + (debugHint != null ? (" " + debugHint) : ""));
    }
    ...
    public void failedQuery(Throwable e, int ecode, String fname, String debugHint)
    {
        ...
        String message = String.format("Client execution failed with error code = %d %nrunning %s %nfname=%s%n%s%n %s", ecode, command != null ? command : "", fname, debugHint != null ? debugHint : "", e == null ? "" : org.apache.hadoop.util.StringUtils.stringifyException(e));
        LOG.error(message);
        Assert.fail(message);
    }
