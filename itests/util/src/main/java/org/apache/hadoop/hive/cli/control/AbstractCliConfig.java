...
public abstract class AbstractCliConfig
{
    ...
    public AbstractCliConfig(Class<? extends CliAdapter> adapter)
    {
        ...
        metastoreType = QTestSystemProperties.getMetaStoreDb() == null ? "derby" : QTestSystemProperties.getMetaStoreDb();
    }
    ...
    public Set<File> getQueryFiles() throws Exception
    {
        ...
        if (isQFileSpecified())
        {
            ...
            for (String qFile : TEST_SPLITTER.split(queryFile))
            {
                ...
                {
                    LOG.warn(qF.getAbsolutePath() + " is among the excluded query files for this driver." + " Please update CliConfigs.java or testconfiguration.properties file to" + " include the qfile or specify qfile through command line explicitly: -Dqfile=test.q");
                }
                testFiles.add(qF);
            }
        ...
        for (String qFileName : excludedQueryFileNames)
        {
            ...
            if ("hiveptest".equals(System.getProperty("user.name")) || !isQFileSpecified() || QTestSystemProperties.shouldForceExclusions())
            {
