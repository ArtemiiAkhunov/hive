...
public class QFileBeeLineClient implements AutoCloseable
{
    ...
    private String[] TEST_FIRST_COMMANDS = new String[]{"!set outputformat tsv2", "!set verbose false", "!set silent true", "!set showheader false", "!set escapeCRLF false", "USE default;", "SHOW TABLES;",};
    private String[] TEST_SET_LOG_COMMANDS = new String[]{"set hive.testing.short.logs=true;", "set hive.testing.remove.logs=false;",};
    private String[] TEST_RESET_COMMANDS = new String[]{"set hive.testing.short.logs=false;", "!set verbose true", "!set silent false", "!set showheader true", "!set escapeCRLF false", "!set outputformat table", "USE default;"};

    protected QFileBeeLineClient(String jdbcUrl, String jdbcDriver, String username, String password, File log) throws IOException
    {
        logFile = log;
        beeLine = new BeeLine();
        ...
        beeLine.runCommands(new String[]{"!set verbose true", "!set shownestederrs true", "!set showwarnings true", "!set showelapsedtime false", "!set trimscripts false", "!set maxwidth -1", "!connect " + jdbcUrl + " " + username + " " + password + " " + jdbcDriver});
    }
    ...
    private void beforeExecute(QFile qFile) throws Exception
    {
        ...
        if (qFile.isUseSharedDatabase())
        {
            ...
            Set<String> dropCommands = getDatabases().stream().filter(database -> !database.equals("default")).map(database -> "DROP DATABASE `" + database + "` CASCADE;").collect(Collectors.toSet());

            Set<String> srcTables = QTestDatasetHandler.getSrcTables();
            dropCommands.addAll(getTables().stream().filter(table -> !srcTables.contains(table)).map(table -> "DROP TABLE `" + table + "` PURGE;").collect(Collectors.toSet()));

            dropCommands.addAll(getViews().stream().map(view -> "DROP VIEW `" + view + "`;").collect(Collectors.toSet()));
        ...
        } else
        {
            ...
            extraCommands = new String[]{"DROP DATABASE IF EXISTS `" + qFile.getDatabaseName() + "` CASCADE;", "CREATE DATABASE `" + qFile.getDatabaseName() + "`;", "USE `" + qFile.getDatabaseName() + "`;"};
        }
    ...
    private void afterExecute(QFile qFile) throws Exception
    {
        ...
        if (!qFile.isUseSharedDatabase())
        {
            ...
            String[] extraCommands = new String[]{"DROP DATABASE IF EXISTS `" + qFile.getDatabaseName() + "` CASCADE;"};
            commands = ArrayUtils.addAll(commands, extraCommands);
        }
