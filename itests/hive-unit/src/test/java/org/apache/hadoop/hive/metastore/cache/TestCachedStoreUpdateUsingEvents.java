...
public class TestCachedStoreUpdateUsingEvents
{
    ...
    private Table createTestTblParam(String dbName, String tblName, String tblOwner, List<FieldSchema> cols, List<FieldSchema> ptnCols, Map<String, String> tblParams) throws IOException
    {
        ...
        StorageDescriptor sd = new StorageDescriptor(cols, serdeLocation, null, null, false, 3, serdeInfo, null, null, serdeParams);
    ...
    private Table createTestTbl(String dbName, String tblName, String tblOwner, List<FieldSchema> cols, List<FieldSchema> ptnCols) throws IOException
    {
    ...
    public void testConstraintsForUpdateUsingEvents() throws Exception
    {
        ...
        List<SQLPrimaryKey> parentPkBase = Arrays.asList(new SQLPrimaryKey(dbName, parentTableName, col1.getName(), 1, "parentpk1", false, false, false));

        // Constraints for table
        List<SQLPrimaryKey> pkBase = Arrays.asList(new SQLPrimaryKey(dbName, tblName, col1.getName(), 1, "pK1", false, false, false));
        List<SQLUniqueConstraint> ucBase = Arrays.asList(new SQLUniqueConstraint(DEFAULT_CATALOG_NAME, dbName, tblName, col1.getName(), 2, "uC1", false, false, false));
        List<SQLNotNullConstraint> nnBase = Arrays.asList(new SQLNotNullConstraint(DEFAULT_CATALOG_NAME, dbName, tblName, col1.getName(), "nN1", false, false, false));
        List<SQLDefaultConstraint> dcBase = Arrays.asList(new SQLDefaultConstraint(DEFAULT_CATALOG_NAME, tbl.getDbName(), tbl.getTableName(), col2.getName(), "1", "dC1", false, false, false));
        List<SQLCheckConstraint> ccBase = Arrays.asList(new SQLCheckConstraint(DEFAULT_CATALOG_NAME, tbl.getDbName(), tbl.getTableName(), col2.getName(), "1", "cC1", false, false, false));
        List<SQLForeignKey> fkBase = Arrays.asList(new SQLForeignKey(parentPkBase.get(0).getTable_db(), parentPkBase.get(0).getTable_name(), parentPkBase.get(0).getColumn_name(), dbName, tblName, col3.getName(), 2, 1, 2, "fK1", parentPkBase.get(0).getPk_name(), false, false, false));
        ...
        DropConstraintRequest dropConstraintRequest = new DropConstraintRequest(dbName, tblName, fkBase.get(0).getFk_name());
    ...
    public void assertRawStoreAndCachedStoreConstraint(AllTableConstraintsRequest request) throws TException
    {
        ...
        AllTableConstraintsResponse constraints = hmsHandler.get_all_table_constraints(request);
    ...
    private void updateTableColStats(String dbName, String tblName, String[] colName, double highValue, double avgColLen, boolean isTxnTable) throws Throwable
    {
        long writeId = -1;
        ...
        ColumnStatistics colStatsCache = sharedCache.getTableColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Lists.newArrayList(colName[0]), validWriteIds, true);
        ...
        colStatsCache = sharedCache.getTableColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Lists.newArrayList(colName[1]), validWriteIds, true);
    ...
    private void updatePartColStats(String dbName, String tblName, boolean isTxnTable, String[] colName, String partName, double highValue, double avgColLen) throws Throwable
    {
        long writeId = -1;
        String validWriteIds = null;
        ...
        List<ColumnStatistics> statRowStore = rawStore.getPartitionColumnStatistics(DEFAULT_CATALOG_NAME, dbName, tblName, Collections.singletonList(partName), Collections.singletonList(colName[1]), Constants.HIVE_ENGINE, validWriteIds);
        Deadline.stopTimer();
        ...
        List<ColumnStatistics> statSharedCache = sharedCache.getPartitionColStatsListFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Collections.singletonList(partName), Collections.singletonList(colName[1]), validWriteIds, true);
        ...
        SharedCache.ColumStatsWithWriteId statPartCache = sharedCache.getPartitionColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, CachedStore.partNameToVals(partName), colName[0], validWriteIds);
        verifyStatDouble(statPartCache.getColumnStatisticsObj(), colName[0], highValue);

        statPartCache = sharedCache.getPartitionColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, CachedStore.partNameToVals(partName), colName[1], validWriteIds);
        verifyStatString(statPartCache.getColumnStatisticsObj(), colName[1], avgColLen);
    }

    private List<ColumnStatisticsObj> getStatsObjects(String dbName, String tblName, String[] colName, double highValue, double avgColLen)
    {
        ...
        ColumnStatisticsData statsData = new ColStatsBuilder<>(double.class).numNulls(NUM_NULLS).numDVs(NUM_DVS).low(LOW_VALUE).high(highValue).hll(1.1, 2.2, 3.3).kll(1.1, 2.2, 3.3).build();
        ...
        statsData = new ColStatsBuilder<>(String.class).numNulls(NUM_NULLS).numDVs(NUM_DVS).avgColLen(avgColLen).maxColLen(MAX_COL_LEN).hll("1.1", "2.2", "3.3").build();
    ...
    private void verifyStatDouble(ColumnStatisticsObj colStats, String colName, double highValue)
    {
        ...
        ColumnStatisticsData expectedColStats = new ColStatsBuilder<>(double.class).numNulls(NUM_NULLS).numDVs(NUM_DVS).low(LOW_VALUE).high(highValue).hll(1.1, 2.2, 3.3).kll(1.1, 2.2, 3.3).build();
    ...
    private void verifyStatString(ColumnStatisticsObj colStats, String colName, double avgColLen)
    {
        ...
        ColumnStatisticsData expectedColStats = new ColStatsBuilder<>(String.class).numNulls(NUM_NULLS).numDVs(NUM_DVS).avgColLen(avgColLen).maxColLen(MAX_COL_LEN).hll("1.1", "2.2", "3.3").build();
        Assert.assertEquals(expectedColStats, colStats.getStatsData());
    }
    ...
    private void createTableWithPart(String dbName, String tblName, String[] colName, boolean isTxnTbl) throws Throwable
    {
        ...
        StorageDescriptor sd = new StorageDescriptor(cols, null, "orc", "orc", false, 0, new SerDeInfo("serde", "seriallib", new HashMap<>()), null, null, tableParams);
        sd.setInputFormat(OrcInputFormat.class.getName());
        sd.setOutputFormat(OrcOutputFormat.class.getName());

        Table tbl = new Table(tblName, dbName, null, 0, 0, 0, sd, partCols, tableParams, null, null, TableType.MANAGED_TABLE.toString());
        tbl.setCatName(DEFAULT_CATALOG_NAME);
        ...
        Partition ptn1 = new Partition(partVals1, dbName, tblName, 0, 0, sd, partParams);
        ptn1.setCatName(DEFAULT_CATALOG_NAME);
        hmsHandler.add_partition_with_environment_context(ptn1, environmentContext);
        Partition ptn2 = new Partition(partVals2, dbName, tblName, 0, 0, sd, partParams);
        ptn2.setCatName(DEFAULT_CATALOG_NAME);
    ...
    {
        GetValidWriteIdsRequest validWriteIdsRequest = new GetValidWriteIdsRequest(Collections.singletonList(TableName.getDbTable(dbName, tblName)));
        GetValidWriteIdsResponse validWriteIdsResponse = hmsHandler.get_valid_write_ids(validWriteIdsRequest);
        return TxnCommonUtils.createValidReaderWriteIdList(validWriteIdsResponse.getTblValidWriteIds().get(0)).writeToString();
    }
    ...
    private void deleteColStats(String dbName, String tblName, String[] colName) throws Throwable
    {
        ...
        Assert.assertEquals(sharedCache.getTableColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Lists.newArrayList(colName[0]), null, true).getStatsObj().isEmpty(), true);
        Assert.assertEquals(sharedCache.getTableColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Lists.newArrayList(colName[1]), null, true).getStatsObj().isEmpty(), true);
        validateTablePara(dbName, tblName);
    }

    private void deletePartColStats(String dbName, String tblName, String[] colName, String partName) throws Throwable
    {
        boolean status = hmsHandler.delete_partition_column_statistics(dbName, tblName, partName, colName[1], Constants.HIVE_ENGINE);
        Assert.assertEquals(status, true);

        SharedCache.ColumStatsWithWriteId colStats = sharedCache.getPartitionColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, CachedStore.partNameToVals(partName), colName[1], null);
    ...
    public void testTableColumnStatisticsTxnTableMultiAbort() throws Throwable
    {
        ...
        Deadline.startTimer("getPartitionColumnStatistics");
        List<ColumnStatistics> statRawStore = rawStore.getPartitionColumnStatistics(DEFAULT_CATALOG_NAME, dbName, tblName, Collections.singletonList(partName), Collections.singletonList(colName[1]), Constants.HIVE_ENGINE, validWriteIds);
        Deadline.stopTimer();
        ...
        List<ColumnStatistics> statsListFromCache = sharedCache.getPartitionColStatsListFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Collections.singletonList(partName), Collections.singletonList(colName[1]), validWriteIds, true);
        ...
        SharedCache.ColumStatsWithWriteId columStatsWithWriteId = sharedCache.getPartitionColStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, CachedStore.partNameToVals(partName), colName[1], validWriteIds);
    ...
    private void verifyAggrStat(String dbName, String tblName, String[] colName, List<String> partitions, boolean isTxnTbl, double highValue) throws Throwable
    {
        List<Long> txnIds = allocateTxns(1);
        ...
        AggrStats aggrStats = rawStore.get_aggr_stats_for(DEFAULT_CATALOG_NAME, dbName, tblName, partitions, Collections.singletonList(colName[0]), Constants.HIVE_ENGINE, validWriteIds);
        Deadline.stopTimer();
        ...
        List<ColumnStatisticsObj> stats = sharedCache.getAggrStatsFromCache(DEFAULT_CATALOG_NAME, dbName, tblName, Collections.singletonList(colName[0]), SharedCache.StatsType.ALL);
