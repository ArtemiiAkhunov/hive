...
public class TestReplChangeManager
{
    ...
    private static void internalSetUpProvidePerm() throws Exception
    {
        ...
        permhiveConf.set(HiveConf.ConfVars.METASTORE_WAREHOUSE.varname, "hdfs://" + permDdfs.getNameNode().getHostAndPort() + HiveConf.ConfVars.METASTORE_WAREHOUSE.defaultStrVal);
    ...
    private static void internalSetUp() throws Exception
    {
        ...
        hiveConf.set(HiveConf.ConfVars.METASTORE_WAREHOUSE.varname, "hdfs://" + m_dfs.getNameNode().getHostAndPort() + HiveConf.ConfVars.METASTORE_WAREHOUSE.defaultStrVal);
    ...
    private Partition createPartition(String dbName, String tblName, List<FieldSchema> columns, List<String> partVals, SerDeInfo serdeInfo)
    {
    ...
    public void testClearer() throws Exception
    {
        ...
        fs.setTimes(ReplChangeManager.getCMPath(hiveConf, part11.getName(), fileChksum11, cmroot.toString()), now - 7 * 86400 * 1000 * 2, now - 7 * 86400 * 1000 * 2);
        fs.setTimes(ReplChangeManager.getCMPath(hiveConf, part21.getName(), fileChksum21, cmroot.toString()), now - 7 * 86400 * 1000 * 2, now - 7 * 86400 * 1000 * 2);
        fs.setTimes(ReplChangeManager.getCMPath(hiveConf, part31.getName(), fileChksum31, cmroot.toString()), now - 7 * 86400 * 1000 * 2, now - 7 * 86400 * 1000 * 2);
        fs.setTimes(ReplChangeManager.getCMPath(hiveConf, part32.getName(), fileChksum32, cmroot.toString()), now - 7 * 86400 * 1000 * 2, now - 7 * 86400 * 1000 * 2);
    ...
    public void tesRecyleImpersionationWithGroupPermissions() throws Exception
    {
        ...
        createFile(part22, "testClearer22");
        String fileChksum22 = ReplChangeManager.checksumFor(part22, fs);
        final UserGroupInformation proxyUserUgi = UserGroupInformation.createUserForTesting("impala2", new String[]{"supergroup"});
        ...
        fs.setTimes(ReplChangeManager.getCMPath(hiveConf, part21.getName(), fileChksum21, cmroot), now - 7 * 86400 * 1000 * 2, now - 7 * 86400 * 1000 * 2);
        ReplChangeManager.scheduleCMClearer(hiveConf);
        ...
        do
        {
            ...
            if (!fs.exists(ReplChangeManager.getCMPath(hiveConf, part21.getName(), fileChksum21, cmroot)) && fs.exists(ReplChangeManager.getCMPath(hiveConf, part22.getName(), fileChksum22, cmroot)))
            {
                cleared = true;
            }
        } while (!cleared);
    ...
    public void testRecycleUsingImpersonationWithAccess() throws Exception
    {
        try
        {
            ...
            String fileChksum12 = ReplChangeManager.checksumFor(part12, fs);
            final UserGroupInformation proxyUserUgi = UserGroupInformation.createRemoteUser("impala");
            setGroupsInConf(UserGroupInformation.getCurrentUser().getGroupNames(), "impala", permhiveConf);
            //set owner of data path to impala
            fs.setOwner(dirTbl1, "impala", "default");
            proxyUserUgi.doAs((PrivilegedExceptionAction<Void>) () ->
            {
            ...
            fs.setTimes(ReplChangeManager.getCMPath(permhiveConf, part11.getName(), fileChksum11, permCmroot), now - 7 * 86400 * 1000 * 2, now - 7 * 86400 * 1000 * 2);
            ...
            do
            {
                ...
                if (!fs.exists(ReplChangeManager.getCMPath(permhiveConf, part11.getName(), fileChksum11, permCmroot)) && fs.exists(ReplChangeManager.getCMPath(permhiveConf, part12.getName(), fileChksum12, permCmroot)))
                {
                    cleared = true;
                }
            } while (!cleared);
    ...
    private void setGroupsInConf(String[] groupNames, String proxyUserName, Configuration conf) throws IOException
    {
    ...
    {
        assertTrue(ReplChangeManager.isCMFileUri(new Path("hdfs://localhost:90000/somepath/adir/", "ab.jar#e239s2233")));
        assertFalse(ReplChangeManager.isCMFileUri(new Path("/somepath/adir/", "ab.jar")));
    }
}
