...
public class TestMaterializedViewRebuild extends CompactorOnTezTest
{
    ...
    private static final List<String> FULL_REBUILD_PLAN = Arrays.asList("CBO PLAN:", "HiveProject(a=[$0], b=[$1], c=[$2])", "  HiveFilter(condition=[OR(IS NULL($0), >($0, 0))])", "    HiveTableScan(table=[[default, t1]], table:alias=[t1])", "");

    private static final List<String> INCREMENTAL_REBUILD_PLAN = Arrays.asList("CBO PLAN:", "HiveProject(a=[$0], b=[$1], c=[$2])", "  HiveFilter(condition=[AND(<(2, $5.writeid), OR(>($0, 0), IS NULL($0)))])", "    HiveTableScan(table=[[default, t1]], table:alias=[t1])", "");

    private static final List<String> EXPECTED_RESULT = Arrays.asList("1\tone\t1.1", "2\ttwo\t2.2", "3\tthree\t3.3", "NULL\tNULL\tNULL");
    ...
    public void setup() throws Exception
    {
        ...
        executeStatementOnDriver("create materialized view " + MV1 + " stored as orc TBLPROPERTIES ('transactional'='true') as " + "select a,b,c from " + TABLE1 + " where a > 0 or a is null", driver);
    }
    ...
    private static final List<String> EXPECTED_RESULT_AFTER_UPDATE = Arrays.asList("1\tChanged\t1.1", "2\ttwo\t2.2", "NULL\tNULL\tNULL");
