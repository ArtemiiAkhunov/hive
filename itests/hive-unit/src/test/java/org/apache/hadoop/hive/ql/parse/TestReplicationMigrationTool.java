...
public class TestReplicationMigrationTool extends BaseReplicationAcrossInstances
{
    ...
    public void testSuccessfulSync() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table table1 (id int)").run("insert into table table1 values (100)").run("insert into table table1 values (200),(300),(400)").run("insert into table table1 values (500),(600)").run("create external table table2 (place string) partitioned by (country string)").run("insert into table table2 partition(country='india') values ('chennai')").run("insert into table table2 partition(country='india') values ('jaipur'),('udaipur'),('kota')").run("insert into table table2 partition(country='us') values ('new york')").run("insert into table table2 partition(country='france') values ('paris')").run("create external table table3 (id int) partitioned by (department string)").dump(primaryDbName, withClause);

        verifySuccessfulResult(tuple);

        replica.load(replicatedDbName, primaryDbName, withClause).run("repl status " + replicatedDbName).verifyResult(tuple.lastReplicationId).verifyReplTargetProperty(replicatedDbName);

        verifySuccessfulResult(tuple);

        // Insert some more data and new tables, and check the verification is success after incremental as well.

        tuple = primary.run("use " + primaryDbName).run("create external table table4 (id int) partitioned by (name string)").run("insert into table table1 values (2000)").run("insert into table table1 values (880),(660),(440)").run("insert into table table2 partition(country='nepal') values ('kathmandu'),('pokhra')").run("insert into table table2 partition(country='india') values ('lucknow'),('kanpur'),('agra'),('varanasi')," + "('delhi')").run("insert into table table3 partition(department='engg') values (10),(40),(50),(60)").run("insert into table table3 partition(department='support') values (20),(350)").run("insert into table table3 partition(department='support') values (20),(350)").run("insert into table table4 partition(name='abc') values (120),(3150)").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).verifyReplTargetProperty(replicatedDbName);
    ...
    public void testExtraAndMissingFilesAtSource() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table tablea (i int, j int) row format delimited fields terminated by ',' " + "location '" + externalTableLocation.toUri() + "'").run("create external table tableb (id int)").run("insert into table tableb values (25)").run("insert into table tableb values (28),(36),(42)").dump(primaryDbName, withClause);
        ...
        verifyFailures(tuple, replTool, "Directory Size mismatch", "Source Or Target has an extra/less files", "Source Or Target has an extra/less files.");
    ...
    public void testModifiedContentAndOpenFiles() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table tablea (i int, j int) row format delimited fields terminated by ',' " + "location '" + externalTableLocationa.toUri() + "'").run("create external table tableb (i int, j int) row format delimited fields terminated by ',' " + "location '" + externalTableLocationb.toUri() + "'").run("insert into table tableb values (25,26)").run("insert into table tableb values (28,29),(36,37),(42,43)").dump(primaryDbName, withClause);
        ...
        ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation, "-fileLevelCheck", "-verifyOpenFiles"});
    ...
    public void testFilters() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table tablea (i int, j int) row format delimited fields terminated by ',' " + "location '" + externalTableLocation.toUri() + "'").run("insert into table tablea values (215,226)").run("insert into table tablea values (281,229),(336,347),(542,453)").run("create external table tableb (id int)").run("insert into table tableb values (15)").run("insert into table tableb values (24),(26),(82)").dump(primaryDbName, withClause);
        ...
        assertEquals(0, ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation, "-fileLevelCheck", "-filters", ".*randomStuff.*"}));
        ...
        assertEquals(0, ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation, "-fileLevelCheck", "-verifyChecksum", "-filters", ".*randomStuff.*"}));
        assertTrue(out.toString(), out.toString().contains("Completed verification. Source & Target are in Sync."));
        out.reset();
    }
    ...
    private void validateChecksumValidationFails(ReplicationMigrationTool replTool, WarehouseInstance.Tuple tuple, String message) throws Exception
    {
        ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation, "-fileLevelCheck", "-verifyChecksum"});
    ...
    private void verifyFailures(WarehouseInstance.Tuple tuple, ReplicationMigrationTool replTool, String dirLevelFailureMsg, String fileLevelFailureMsg, String checksumLevelFailureMsg) throws Exception
    {
    ...
    private void validateFileLevelValidationFails(ReplicationMigrationTool replTool, WarehouseInstance.Tuple tuple, String fileLevelFailureMsg) throws Exception
    {
    ...
    private void verifySuccessfulResult(WarehouseInstance.Tuple tuple) throws Exception
    {
        ...
        ReplicationMigrationTool replTool = new ReplicationMigrationTool();
        assertEquals(0, ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation + "/hive/_file_list_external", "-dirLevelCheck"}));
        ...
        // Verify at file level.
        assertEquals(0, ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation + "/hive", "-fileLevelCheck"}));
        assertTrue(out.toString().contains("Completed verification."));
        assertFalse(err.toString(), err.toString().isEmpty());
        out.reset();
        err.reset();
        ...
        // Verify at file level, with checksum.
        assertEquals(0, ToolRunner.run(conf, replTool, new String[]{"-dumpFilePath", tuple.dumpLocation, "-fileLevelCheck", "-verifyChecksum"}));
