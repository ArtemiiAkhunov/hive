...
class MetastoreHousekeepingLeaderTestBase
{
    ...
    void internalSetup(final String leaderHostName, boolean configuredLeader) throws Exception
    {
        ...
        MetastoreConf.setVar(conf, ConfVars.METASTORE_HOUSEKEEPING_LEADER_ELECTION, configuredLeader ? "host" : "lock");
        ...
        port = MetaStoreTestUtils.startMetaStoreWithRetry(HadoopThriftAuthBridge.getBridge(), conf, false, false, true, true);
    ...
    {
        conf.setTimeDuration(METASTORE_THREAD_TASK_FREQ_CONF, REMOTE_TASKS_INTERVAL, TimeUnit.MILLISECONDS);
    ...
    {
        String remoteTaskClassPaths = RemoteMetastoreTaskThreadTestImpl1.class.getCanonicalName() + "," + RemoteMetastoreTaskThreadTestImpl2.class.getCanonicalName();
    ...
    private static String getAllThreadsAsString()
    {
        ...
        for (Map.Entry<Thread, StackTraceElement[]> entry : threadStacks.entrySet())
        {
            ...
            sb.append("Name: ").append(t.getName()).append(" State: ").append(t.getState()).append(" Class name: ").append(t.getClass().getCanonicalName());
        }
        return sb.toString();
    }
