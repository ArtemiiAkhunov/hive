...
public class TestConstraintsMerge
{
    ...
    private enum Table
    {
        TBL_SOURCE("table_source"), TBL_CHECK_MERGE("table_check_merge");
    ...
    public void setUp() throws Exception
    {
        ...
        hiveConf.setVar(ConfVars.HIVE_AUTHORIZATION_MANAGER, "org.apache.hadoop.hive.ql.security.authorization.plugin.sqlstd.SQLStdHiveAuthorizerFactory");
        ...
        dropTables();
        runStatementOnDriver("create table " + Table.TBL_SOURCE + "(name string, age int, gpa double) " + "clustered by (name) into " + BUCKET_COUNT + " buckets stored as orc " + getTblProperties(), hiveConf);
        runStatementOnDriver("create table " + Table.TBL_CHECK_MERGE + "(name string, age int, gpa double  CHECK (gpa BETWEEN 0.0 AND 4.0)) " + "clustered by (name) into " + BUCKET_COUNT + " buckets stored as orc " + getTblProperties(), hiveConf);
        runStatementOnDriver("insert into " + Table.TBL_SOURCE + "(name, age, gpa) values " + "('student1', 16, null)," + "(null, 20, 4.0)", hiveConf);
    }
