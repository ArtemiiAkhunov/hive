...
public class TestSchemaToolCatalogOps
{
    ...
    public static void initDb() throws HiveMetaException, IOException
    {
        ...
        testMetastoreDB = System.getProperty("java.io.tmpdir") + File.separator + "testschematoolcatopsdb";
        ...
        schemaTool.init(System.getProperty("test.tmp.dir", "target/tmp"), new String[]{"-dbType", "derby", "--info"}, null, conf);
    ...
    public void moveDatabase() throws HiveMetaException, TException
    {
        ...
        withCatalog(toCatName, catName ->
        {
            ...
            String argsMoveDB = String.format("-moveDatabase %s -fromCatalog %s -toCatalog %s", dbName, DEFAULT_CATALOG_NAME, catName);
            ...
            Partition fetchedPart = client.getPartition(catName, dbName, tableName, Collections.singletonList(partVal));
    ...
    public void moveNonExistentDatabase() throws TException, HiveMetaException
    {
        ...
        withCatalog(catName, name ->
        {
            String argsMoveDB = String.format("-moveDatabase nosuch -fromCatalog %s -toCatalog %s", name, DEFAULT_CATALOG_NAME);
            execute(new SchemaToolTaskMoveDatabase(), argsMoveDB);
        });
    }
    ...
    public void moveDbToNonExistentCatalog() throws TException, HiveMetaException
    {
        ...
        try
        {
            String argsMoveDB = String.format("-moveDatabase %s -fromCatalog %s -toCatalog nosuch", dbName, DEFAULT_CATALOG_NAME);
            execute(new SchemaToolTaskMoveDatabase(), argsMoveDB);
    ...
    public void moveNonExistentTable() throws TException, HiveMetaException
    {
        ...
        new DatabaseBuilder().setName(toDbName).create(client, conf);
        String argsMoveTable = String.format("-moveTable nosuch -fromCatalog %s -toCatalog %s -fromDatabase %s -toDatabase %s", DEFAULT_CATALOG_NAME, DEFAULT_CATALOG_NAME, DEFAULT_DATABASE_NAME, toDbName);
        execute(new SchemaToolTaskMoveTable(), argsMoveTable);
    }
    ...
    private void withCatalog(String catName, CheckFunc<String> f) throws HiveMetaException, TException
    {
        try
        {
            new CatalogBuilder().setName(catName).setLocation(catalogLocation.toString()).create(client);
            f.apply(catName);
        } finally
        {
