...
public class TestScheduledReplicationScenarios extends BaseReplicationScenariosAcidTables
{
    ...
    public static void classLevelSetup() throws Exception
    {
        ...
        overrides.put(HiveConf.ConfVars.HIVE_SCHEDULED_QUERIES_EXECUTOR_PROGRESS_REPORT_INTERVAL.varname, "1s");
    ...
    public void testAcidTablesReplLoadBootstrapIncr() throws Throwable
    {
        ...
        try (ScheduledQueryExecutionService schqS = ScheduledQueryExecutionService.startScheduledQueryExecutorService(primary.hiveConf))
        {
            ...
            Path dumpRoot = new Path(primary.hiveConf.getVar(HiveConf.ConfVars.REPL_DIR), Base64.getEncoder().encodeToString(primaryDbName.toLowerCase().getBytes(StandardCharsets.UTF_8.name())));
    ...
    public void testCompleteFailoverWithReverseBootstrap() throws Throwable
    {
        ...
        try (ScheduledQueryExecutionService schqS = ScheduledQueryExecutionService.startScheduledQueryExecutorService(primary.hiveConf))
        {
            ...
            assertTrue(MetaStoreUtils.isDbBeingPlannedFailedOverAtEndpoint(primary.getDatabase(sourceDbName), MetaStoreUtils.FailoverEndpoint.SOURCE));
            assertTrue(MetaStoreUtils.isDbBeingPlannedFailedOverAtEndpoint(replica.getDatabase(replicaDbName), MetaStoreUtils.FailoverEndpoint.TARGET));
            ...
            assertTrue(fs.exists(failoverReadyMarker));
            assertTrue(MetaStoreUtils.isDbBeingPlannedFailedOverAtEndpoint(replica.getDatabase(replicaDbName), MetaStoreUtils.FailoverEndpoint.SOURCE));
            assertTrue(MetaStoreUtils.isDbBeingPlannedFailedOverAtEndpoint(primary.getDatabase(sourceDbName), MetaStoreUtils.FailoverEndpoint.TARGET));
