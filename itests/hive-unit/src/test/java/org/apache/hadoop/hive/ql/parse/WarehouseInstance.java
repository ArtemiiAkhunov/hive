...
public class WarehouseInstance implements Closeable
{
    ...
    WarehouseInstance(Logger logger, MiniDFSCluster cluster, Map<String, String> overridesForHiveConf, String keyNameForEncryptedZone) throws Exception
    {
        this.logger = logger;
        ...
        String tmpDir = "/tmp/" + TestReplicationScenarios.class.getCanonicalName().replace('.', '_') + "_" + System.nanoTime();
    ...
    WarehouseInstance(Logger logger, MiniDFSCluster cluster, Map<String, String> overridesForHiveConf) throws Exception
    {
        this(logger, cluster, overridesForHiveConf, null);
    }

    private void initialize(String cmRoot, String externalTableWarehouseRoot, String warehouseRoot, Map<String, String> overridesForHiveConf) throws Exception
    {
        ...
        hiveConf.setVar(HiveConf.ConfVars.METASTORE_CONNECT_URL_KEY, "jdbc:derby:memory:${test.tmp.dir}/APP;create=true");
        ...
        hiveConf.set(HiveConf.ConfVars.METASTORE_RAW_STORE_IMPL.varname, "org.apache.hadoop.hive.metastore.InjectableBehaviourObjectStore");
    ...
    */
    ...
    private Path mkDir(DistributedFileSystem fs, String pathString) throws IOException, SemanticException
    {
    ...
    WarehouseInstance runFailure(String command, int errorCode) throws Throwable
    {
        ...
        } catch (CommandProcessorException e)
        {
            ...
            {
                throw new RuntimeException("Command: " + command + " returned incorrect error code: " + e.getResponseCode() + " instead of " + errorCode);
            }
            return this;
        }
    }

    Tuple dump(String dbName) throws Throwable
    {
        return dump(dbName, Collections.emptyList());
    }

    Tuple dump(String dumpExpression, List<String> withClauseOptions) throws Throwable
    {
        String dumpCommand = "REPL DUMP " + dumpExpression;
    ...
    {
        String dumpCommand = "REPL DUMP " + dbName;
        advanceDumpDir();
    ...
    WarehouseInstance load(String replicatedDbName, String primaryDbName, List<String> withClauseOptions) throws Throwable
    {
    ...
    WarehouseInstance loadFailure(String replicatedDbName, String primaryDbName, List<String> withClauseOptions) throws Throwable
    {
        String replLoadCmd = "REPL LOAD " + primaryDbName + " INTO " + replicatedDbName;
        if ((withClauseOptions != null) && !withClauseOptions.isEmpty())
        {
    ...
    WarehouseInstance loadFailure(String replicatedDbName, String primaryDbName, List<String> withClauseOptions, int errorCode) throws Throwable
    {
        String replLoadCmd = "REPL LOAD " + primaryDbName + " INTO " + replicatedDbName;
        if ((withClauseOptions != null) && !withClauseOptions.isEmpty())
        {
    ...
    WarehouseInstance verifyResults(String[] data) throws IOException
    {
        ...
        List<String> filteredResults = results.stream().filter(x -> !x.toLowerCase().contains(SemanticAnalyzer.VALUES_TMP_TABLE_NAME_PREFIX.toLowerCase())).map(String::toLowerCase).collect(Collectors.toList());
        List<String> lowerCaseData = Arrays.stream(data).map(String::toLowerCase).collect(Collectors.toList());
        assertEquals(data.length, filteredResults.size());
        assertTrue(StringUtils.join(filteredResults, ",") + " does not contain all expected " + StringUtils.join(lowerCaseData, ","), filteredResults.containsAll(lowerCaseData));
        return this;
    }
    ...
    {
        IncrementalLoadEventsIterator itr = new IncrementalLoadEventsIterator(dumpLocation + File.separator + ReplUtils.REPL_HIVE_BASE_DIR, conf);
     ...
     * @return - list of ColumnStatisticsObj objects in the order of the specified columns
     */
    public Map<String, List<ColumnStatisticsObj>> getAllPartitionColumnStatistics(String dbName, String tableName) throws Exception
    {
        List<String> colNames = new ArrayList();
        client.getFields(dbName, tableName).forEach(fs -> colNames.add(fs.getName()));
        return client.getPartitionColumnStatistics(dbName, tableName, client.listPartitionNames(dbName, tableName, (short) -1), colNames, Constants.HIVE_ENGINE);
    }
     ...
     * @throws Exception
     */
    List<ColumnStatisticsObj> getPartitionColumnStatistics(String dbName, String tableName, String partName, List<String> colNames) throws Exception
    {
        return client.getPartitionColumnStatistics(dbName, tableName, Collections.singletonList(partName), colNames, Constants.HIVE_ENGINE).get(0);
    }
    ...
    {
        return client.getNotNullConstraints(new NotNullConstraintsRequest(Warehouse.DEFAULT_CATALOG_NAME, dbName, tblName));
    }
    ...
    public void testEventCounts(String dbName, long fromEventId, Long toEventId, Integer limit, long expectedCount) throws Exception
    {
    ...
    {
        if (hiveConf.getBoolVar(HiveConf.ConfVars.HIVE_SUPPORT_CONCURRENCY) && hiveConf.getVar(HiveConf.ConfVars.HIVE_TXN_MANAGER).equals("org.apache.hadoop.hive.ql.lockmgr.DbTxnManager"))
        {
            return true;
        }
        return false;
    ...
    List<Path> copyToHDFS(List<URI> localUris) throws IOException, SemanticException
    {
        ...
        localUris.forEach(uri ->
        {
            ...
            try
            {
                ...
                boolean success = FileUtils.copy(localFs, localPath, fs, destinationBasePath, false, false, hiveConf, copyStatistics);
                if (!success)
                {
        ...
        List<FileStatus> fileStatuses = Arrays.asList(fs.globStatus(new Path(destinationBasePath, "*")));
