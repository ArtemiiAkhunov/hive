...
public class TestTableLevelReplicationScenarios extends BaseReplicationScenariosAcidTables
{
    ...
    private void createTables(List<CreateTableInfo> createTblsInfo) throws Throwable
    {
        for (CreateTableInfo tblInfo : createTblsInfo)
        {
            ...
            {
                strBuilder.append(" clustered by (a) into 2 buckets stored as orc " + "tblproperties (\"transactional\"=\"true\")");
            } else if (tblInfo.type == CreateTableType.MM_ACID)
            {
                strBuilder.append(" tblproperties(\"transactional\"=\"true\", " + "\"transactional_properties\"=\"insert_only\")");
            }

            String createTableCmd = strBuilder.toString();
            primary.run("use " + primaryDbName).run(createTableCmd).run("insert into " + tblInfo.tableName + " values(1, 10)");
        }
    }
    ...
    private String replicateAndVerify(String replPolicy, String lastReplId, List<String> dumpWithClause, List<String> loadWithClause, String[] expectedTables) throws Throwable
    {
        return replicateAndVerify(replPolicy, null, lastReplId, dumpWithClause, loadWithClause, null, expectedTables);
    }

    private String replicateAndVerify(String replPolicy, String oldReplPolicy, String lastReplId, List<String> dumpWithClause, List<String> loadWithClause, String[] bootstrappedTables, String[] expectedTables) throws Throwable
    {
        return replicateAndVerify(replPolicy, oldReplPolicy, lastReplId, dumpWithClause, loadWithClause, bootstrappedTables, expectedTables, null);
    }
    ...
    private String replicateAndVerifyClearDump(String replPolicy, String oldReplPolicy, String lastReplId, List<String> dumpWithClause, List<String> loadWithClause, String[] bootstrappedTables, String[] expectedTables, String[] records) throws Throwable
    {
        if (dumpWithClause == null)
        {
            dumpWithClause = new ArrayList<>();
    ...
    private void verifyBootstrapDirInIncrementalDump(String dumpLocation, String[] bootstrappedTables) throws Throwable
    {
    ...
    {
        WarehouseInstance.Tuple tupleBootstrap = primary.run("use " + primaryDbName).dump(primaryDbName);
        ...
        replicateAndVerify(replPolicy, primaryDbName, tupleBootstrap.lastReplicationId, null, null, null, replicatedTables);
    }
    ...
    public void testBasicIncrementalWithIncludeAndExcludeList() throws Throwable
    {
        ...
        replicateAndVerify(primaryDbName, replPolicy, tupleBootstrap.lastReplicationId, null, null, bootstrappedTables, replicatedTables);
    }
    ...
    public void testIncorrectTablePolicyInReplDump() throws Throwable
    {
        ...
        boolean failed;
        String[] invalidReplPolicies = new String[]{primaryDbName + ".t1.t2", // Didn't enclose table pattern within single quotes.
        ...
        primary.run("use " + primaryDbName).dump(primaryDbName);
        replica.load(replicatedDbName, primaryDbName);

        // Invalid pattern, include/exclude table list is empty.
        invalidReplPolicies = new String[]{primaryDbName + ".''.'t2'", // Include list is empty.
        ...
        for (String invalidReplPolicy : invalidReplPolicies)
        {
            ...
            } catch (Exception ex)
            {
                ...
                Assert.assertEquals(ErrorMsg.REPL_INVALID_DB_OR_TABLE_PATTERN.getErrorCode(), ErrorMsg.getErrorMsg(ex.getMessage()).getErrorCode());
                failed = true;
            }
            Assert.assertTrue(failed);
        }
    ...
    public void testFullDbBootstrapReplicationWithDifferentReplPolicyFormats() throws Throwable
    {
        ...
        String[] fullDbReplPolicies = new String[]{primaryDbName + ".'.*?'", primaryDbName};

        // Replicate and verify if all 3 tables are replicated to target.
        for (String replPolicy : fullDbReplPolicies)
        {
            replicateAndVerifyClearDump(replPolicy, null, null, null, null, null, originalTables, null);
        }
    }
    ...
    public void testBootstrapAcidTablesIncrementalPhaseWithIncludeAndExcludeList() throws Throwable
    {
        ...
        List<String> dumpWithoutAcidClause = Collections.singletonList("'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='false'");
        ...
        List<String> dumpWithAcidBootstrapClause = Arrays.asList("'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES + "'='true'");
    ...
    public void testBootstrapExternalTablesWithIncludeAndExcludeList() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).dump(replPolicy, dumpWithClause);
        ...
        Assert.assertTrue(primary.miniDFSCluster.getFileSystem().exists(new Path(hiveDumpDir, EximUtil.FILE_LIST_EXTERNAL)));
        ...
        replica.load(replicatedDbName, replPolicy, loadWithClause).run("use " + replicatedDbName).run("show tables").verifyResults(replicatedTables).verifyReplTargetProperty(replicatedDbName);
    }
    ...
    public void testBootstrapExternalTablesIncrementalPhaseWithIncludeAndExcludeList() throws Throwable
    {
        ...
        replica.load(replicatedDbName, replPolicy, loadWithClause).run("use " + replicatedDbName).run("show tables").verifyResults(incrementalReplicatedTables).verifyReplTargetProperty(replicatedDbName);
    }
    ...
    public void testBasicReplaceReplPolicy() throws Throwable
    {
        ...
        lastReplId = replicateAndVerify(replPolicy, oldReplPolicy, lastReplId, null, null, bootstrappedTables, replicatedTables);
    ...
    public void testReplacePolicyOnBootstrapAcidTablesIncrementalPhase() throws Throwable
    {
        ...
        replicateAndVerify(replPolicy, oldReplPolicy, lastReplId, dumpWithAcidBootstrapClause, null, bootstrappedTables, incrementalReplicatedTables);
    }
    ...
    public void testReplacePolicyWhenAcidTablesDisabledForRepl() throws Throwable
    {
        ...
        lastReplId = replicateAndVerify(replPolicy, oldReplPolicy, lastReplId, dumpWithoutAcidClause, null, bootstrappedTables, incrementalReplicatedTables);
    }
    ...
    public void testReplacePolicyOnBootstrapExternalTablesIncrementalPhase() throws Throwable
    {
        ...
        lastReplId = replicateAndVerify(replPolicy, oldReplPolicy, lastReplId, dumpWithClause, loadWithClause, bootstrappedTables, incrementalReplicatedTables);
        ...
        dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='true'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_EXTERNAL_TABLES.varname + "'='true'");
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).dump(replPolicy, dumpWithClause);
        loadWithClause = ReplicationTestUtils.includeExternalTableClause(true);
    ...
    public void testRenameTableScenariosBasic() throws Throwable
    {
        ...
        String lastReplId = replicateAndVerify(replPolicy, null, null, null, null, new String[]{}, new String[]{});
        ...
        lastReplId = replicateAndVerify(replPolicy, replPolicy, lastReplId, null, null, bootstrapTables, replicatedTables);

        // Rename tables to make them satisfy the filter.
        primary.run("use " + primaryDbName).run("alter table out3 rename to in3").run("alter table out4 rename to in4").run("alter table out5 rename to in5");

        replicatedTables = new String[]{"in1", "in2", "in3", "in4", "in5"};
        bootstrapTables = new String[]{"in3", "in4", "in5"};
        lastReplId = replicateAndVerify(replPolicy, replPolicy, lastReplId, null, null, bootstrapTables, replicatedTables);

        primary.run("use " + primaryDbName).run("alter table in3 rename to in7").run("alter table in7 rename to in8") // Double rename, both satisfying the filter, so no bootstrap.
        ...
        primary.run("use " + primaryDbName).run("alter table in8 rename to in12") // table is renamed from satisfying to satisfying, no bootstrap
        ...
        replicateAndVerify(replPolicy, replPolicy, lastReplId, null, null, bootstrapTables, replicatedTables);
    }
    ...
    public void testRenameTableScenariosWithDmlOperations() throws Throwable
    {
        ...
        String[] bootstrapTables = new String[]{};
        lastReplId = replicateAndVerify(replPolicy, null, lastReplId, null, null, bootstrapTables, replicatedTables);

        // Rename tables and do some operations.
        primary.run("use " + primaryDbName).run("alter table in1 rename to out1").run("insert into out1 values(2, 100)").run("alter table out1 rename to in4").run("alter table in100 rename to out100").run("insert into out100 values(2, 100)").run("alter table out100 rename to in400");

        replicatedTables = new String[]{"in4", "in400"};
        bootstrapTables = new String[]{"in4", "in400"};
        replicateAndVerify(replPolicy, null, lastReplId, null, null, bootstrapTables, replicatedTables, new String[]{"1", "2"});
    }
    ...
    {
        String replPolicy = primaryDbName + ".'in[0-9]+'.'out[0-9]+'";
        List<String> dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES.varname + "'='false'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='false'");
        String lastReplId = replicateAndVerify(replPolicy, null, null, dumpWithClause, null, new String[]{}, new String[]{});
        ...
        String[] bootstrapTables = new String[]{};
        lastReplId = replicateAndVerify(replPolicy, replPolicy, lastReplId, dumpWithClause, null, bootstrapTables, replicatedTables);

        // Rename tables to make them satisfy the filter and enable acid tables.
        primary.run("use " + primaryDbName).run("alter table out3 rename to in3").run("alter table out4 rename to in4").run("alter table out5 rename to in5");

        dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES.varname + "'='true'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'");
        replicatedTables = new String[]{"in1", "in2", "in3", "in4", "in5"};
        bootstrapTables = new String[]{"in2", "in3", "in4", "in5"};
        replicateAndVerify(replPolicy, replPolicy, lastReplId, dumpWithClause, null, bootstrapTables, replicatedTables);
    }
    ...
    public void testRenameTableScenariosExternalTable() throws Throwable
    {
        String replPolicy = primaryDbName + ".'in[0-9]+'.'out[0-9]+'";
        List<String> loadWithClause = ReplicationTestUtils.includeExternalTableClause(false);
        List<String> dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='false'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_EXTERNAL_TABLES.varname + "'='false'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES.varname + "'='false'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='false'");
        String lastReplId = replicateAndVerify(replPolicy, null, null, dumpWithClause, loadWithClause, new String[]{}, new String[]{});
        ...
        String[] replicatedTables = new String[]{"in1"};
        String[] bootstrapTables = new String[]{};
        lastReplId = replicateAndVerify(replPolicy, null, lastReplId, dumpWithClause, loadWithClause, bootstrapTables, replicatedTables);

        // Rename tables to make them satisfy the filter and enable acid and external tables.
        primary.run("use " + primaryDbName).run("alter table out4 rename to in4").run("alter table out5 rename to in5");

        dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='true'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_EXTERNAL_TABLES.varname + "'='true'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES.varname + "'='true'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'", "'distcp.options.pugpb'=''");
        loadWithClause = ReplicationTestUtils.includeExternalTableClause(true);
        replicatedTables = new String[]{"in1", "in2", "in3", "in4", "in5"};
        bootstrapTables = new String[]{"in2", "in3", "in4", "in5"};
        lastReplId = replicateAndVerify(replPolicy, null, lastReplId, dumpWithClause, loadWithClause, null, replicatedTables);

        dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='true'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'");

        primary.run("use " + primaryDbName).run("alter table out6 rename to in6")  // external table bootstrap.
        ...
        replicateAndVerify(replPolicy, null, lastReplId, dumpWithClause, loadWithClause, bootstrapTables, replicatedTables);
    }
    ...
    public void testRenameTableScenariosWithReplaceExternalTable() throws Throwable
    {
        ...
        primary.run("use " + primaryDbName).run("alter table out4 rename to in5") // Old name matching old, new name matching both
                ...
                .run("drop table in10").run("alter table out11 rename to out12") // Old name not matching old and new name not matching both
        ...
        dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='true'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_EXTERNAL_TABLES.varname + "'='false'", "'distcp.options.pugpb'=''");
        ...
        replicateAndVerify(newPolicy, replPolicy, lastReplId, dumpWithClause, loadWithClause, bootstrapTables, replicatedTables);
    }
    ...
    public void testRenameTableScenariosWithReplacePolicyDMLOperattion() throws Throwable
    {
        ...
        String[] originalFullAcidTables = new String[]{"in1", "in2", "out3", "out4", "out5", "in100", "in200", "in300", "out3000", "out4000", "out4001"};
        ...
        primary.run("use " + primaryDbName).run("alter table in200 rename to in2000") // Old name not matching old, old and new matching new policy.
                .run("alter table in400 rename to out400") // Old name matching new and old policy, new matching none.
                .run("alter table out500 rename to in500").run("alter table out3000 rename to in3000") // Old name not matching old policy and both name matching new
                .run("alter table in1 rename to out7").run("alter table in300 rename to in301") // for rename its all matching to matching.
                .run("alter table in301 rename to in305") // ideally in305 bootstrap should not happen.
                .run("alter table out3 rename to in8").run("alter table out4 rename to in9").run("drop table in9").run("alter table out5 rename to in10").run("alter table in10 rename to out11").run("drop table out11").run("insert into in100 values(2, 100)").run("insert into in8 values(2, 100)").run("insert into in305 values(2, 100)").run("insert into in3000 values (2, 100)").run("insert into in2000 values (2, 100)").run("insert into in500 values(2, 100)").run("alter table out4000 rename to out5000").run("alter table out5000 rename to in5000").run("insert into in5000 values (2, 100)").run("alter table out4001 rename to out5001").run("alter table out5001 rename to out5002").run("insert into out5002 values (2, 100)");
        ...
        lastReplId = replicateAndVerify(newPolicy, replPolicy, lastReplId, null, null, bootstrapTables, replicatedTables, new String[]{"1", "2"});

        // No table filter
        replPolicy = newPolicy;
        newPolicy = primaryDbName;
        primary.run("use " + primaryDbName).run("alter table in100 rename to in12") // should be just rename, but for replace its always bootstrap
                ...
                .run("insert into in2 values(2, 100)").run("insert into in1 values(2, 100)").run("insert into in400 values(2, 100)").run("drop table out3");   // table will be removed from bootstrap list.
        ...
        replicateAndVerify(newPolicy, replPolicy, lastReplId, null, null, bootstrapTables, replicatedTables, new String[]{"1", "2"});
    }
    ...
    public void testRenameTableScenariosUpgrade() throws Throwable
    {
        ...
        List<String> loadWithClause = ReplicationTestUtils.includeExternalTableClause(false);
        List<String> dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='false'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='false'");
        ...
        String lastReplId = replicateAndVerify(replPolicy, null, null, dumpWithClause, null, new String[]{}, replicatedTables);
        ...
        primary.run("use " + primaryDbName).run("alter table out4 rename to in4").run("alter table out5 rename to in5").run("alter table out6 rename to in6");
        ...
        lastReplId = replicateAndVerify(newReplPolicy, replPolicy, lastReplId, dumpWithClause, loadWithClause, bootstrapTables, replicatedTables);

        primary.run("use " + primaryDbName).run("alter table in4 rename to out4").run("alter table in5 rename to out5").run("alter table in6 rename to out6");

        dumpWithClause = Arrays.asList("'" + HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES.varname + "'='true'", "'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'", "'distcp.options.pugpb'=''");

        // Database replication with ACID and EXTERNAL table.
        replicatedTables = new String[]{"in1", "in2", "in3", "out4", "out5", "out6", "in7", "in8", "in9", "out10", "out11", "out12"};
        bootstrapTables = new String[]{"out4", "out5", "out6", "in8", "out10", "out11", "out12"};
        replicateAndVerify(replPolicy, newReplPolicy, lastReplId, dumpWithClause, loadWithClause, bootstrapTables, replicatedTables);
    }
    ...
    public void testCheckPointingDataDumpFailureSamePolicyExpression() throws Throwable
    {
        ...
        WarehouseInstance.Tuple bootstrapDump = primary.run("use " + primaryDbName).run("CREATE TABLE t1(a string) STORED AS TEXTFILE").run("CREATE TABLE t2(a string) STORED AS TEXTFILE").run("insert into t1 values (1)").run("insert into t1 values (2)").run("insert into t1 values (3)").run("insert into t2 values (11)").run("insert into t2 values (21)").dump(replPolicy);
    ...
    public void testIncrementalDumpCheckpointingSameExpression() throws Throwable
    {
        ...
        WarehouseInstance.Tuple bootstrapDump = primary.run("use " + primaryDbName).run("CREATE TABLE t1(a string) STORED AS TEXTFILE").run("CREATE TABLE t2(a string) STORED AS TEXTFILE").dump(replPolicy);

        replica.load(replicatedDbName, replPolicy).run("select * from " + replicatedDbName + ".t1").verifyResults(new String[]{}).run("select * from " + replicatedDbName + ".t2").verifyResults(new String[]{});
        ...
        WarehouseInstance.Tuple incrementalDump1 = primary.run("use " + primaryDbName).run("insert into t1 values (1)").run("insert into t2 values (2)").dump(replPolicy);
        ...
        WarehouseInstance.Tuple incrementalDump2 = primary.run("use " + primaryDbName).dump(replPolicy);
        ...
        replica.load(replicatedDbName, replPolicy).run("select * from " + replicatedDbName + ".t1").verifyResults(new String[]{"1"}).run("select * from " + replicatedDbName + ".t2").verifyResults(new String[]{"2"});


        //Case 2: When the last dump was half way through
        ReplDumpWork.testDeletePreviousDumpMetaPath(true);

        WarehouseInstance.Tuple incrementalDump3 = primary.run("use " + primaryDbName).run("insert into t1 values (3)").run("insert into t2 values (4)").dump(replPolicy);
    ...
    public void testIncrementalDumpCheckpointingDiffExpression() throws Throwable
    {
        ...
        replica.load(replicatedDbName, replPolicy).run("select * from " + replicatedDbName + ".t1").verifyResults(new String[]{"3"}).run("select * from " + replicatedDbName + ".t2").verifyResults(new String[]{"4"});
    }
}
