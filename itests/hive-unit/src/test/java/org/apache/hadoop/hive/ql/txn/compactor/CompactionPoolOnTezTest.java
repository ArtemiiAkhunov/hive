...
public class CompactionPoolOnTezTest extends CompactorOnTezTest
{
    ...
    private void checkCompactionRequest(String state, String pool) throws MetaException
    {
        ...
        Assert.assertEquals("Expecting compaction state '" + state + "' and found:" + compacts.get(0).getState(), state, compacts.get(0).getState());
        Assert.assertEquals("Expecting compaction pool '" + pool + "' and found:" + compacts.get(0).getPoolName(), pool, compacts.get(0).getPoolName());
    }
    ...
    public void testShowCompactionsContainsPoolName() throws Exception
    {
        ...
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time\tDuration(ms)" + "\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\tHighest WriteId", results.get(0));
