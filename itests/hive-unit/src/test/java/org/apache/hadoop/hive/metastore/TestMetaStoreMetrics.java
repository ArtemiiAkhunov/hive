...
public class TestMetaStoreMetrics
{
    ...
    {
        int initDbCount = (Integer) Metrics.getRegistry().getGauges().get(MetricsConstants.TOTAL_DATABASES).getValue();
        int initTblCount = (Integer) Metrics.getRegistry().getGauges().get(MetricsConstants.TOTAL_TABLES).getValue();
        int initPartCount = (Integer) Metrics.getRegistry().getGauges().get(MetricsConstants.TOTAL_PARTITIONS).getValue();
        ...
        Assert.assertEquals(initDbCount + 1, Metrics.getRegistry().getGauges().get(MetricsConstants.TOTAL_DATABASES).getValue());
        Assert.assertEquals(initTblCount + 4, Metrics.getRegistry().getGauges().get(MetricsConstants.TOTAL_TABLES).getValue());
        Assert.assertEquals(initPartCount + 6, Metrics.getRegistry().getGauges().get(MetricsConstants.TOTAL_PARTITIONS).getValue());
    ...
    public void testConnections() throws Exception
    {
        ...
        long initialCount = Metrics.getRegistry().getCounters().get(MetricsConstants.OPEN_CONNECTIONS).getCount();

        //create two connections
        HiveMetaStoreClient msc = new HiveMetaStoreClient(hiveConf);
        HiveMetaStoreClient msc2 = new HiveMetaStoreClient(hiveConf);

        Assert.assertEquals(initialCount + 2, Metrics.getRegistry().getCounters().get(MetricsConstants.OPEN_CONNECTIONS).getCount());

        //close one connection, verify still two left
        msc.close();
        Thread.sleep(2000);  // TODO Evil!  Need to figure out a way to remove this sleep.
        Assert.assertEquals(initialCount + 1, Metrics.getRegistry().getCounters().get(MetricsConstants.OPEN_CONNECTIONS).getCount());

        //close one connection, verify still one left
        msc2.close();
        Thread.sleep(2000);  // TODO Evil!  Need to figure out a way to remove this sleep.
        Assert.assertEquals(initialCount, Metrics.getRegistry().getCounters().get(MetricsConstants.OPEN_CONNECTIONS).getCount());
    }
    ...
    public void testConnectionPoolMetrics()
    {
        ...
        Assert.assertEquals(MetastoreConf.getIntVar(hiveConf, MetastoreConf.ConfVars.CONNECTION_POOLING_MAX_CONNECTIONS), Metrics.getRegistry().getGauges().get(poolMetricName).getValue());
        Assert.assertEquals(2, Metrics.getRegistry().getGauges().get(secondaryPoolMetricName).getValue());
    }
