...
public class TestReplicationFilterTransactions
{
    ...
    private final static String tid = TestReplicationFilterTransactions.class.getCanonicalName().toLowerCase().replace('.', '_') + "_" + System.currentTimeMillis();
    ...
    static public class ReplicaEventListenerTestImpl extends MetaStoreEventListener
    {
        ...
        public static Map<Long, Long> getTxnMapping()
        {
            return new HashMap(txnMapping);
        }
    ...
    private Map<String, String> setupConf(String dfsUri, String listenerClassName)
    {
        ...
        confMap.put(MetastoreConf.ConfVars.EVENT_MESSAGE_FACTORY.getHiveName(), GzipJSONMessageEncoder.class.getCanonicalName());
        return confMap;
    }
    ...
    public void setup() throws Throwable
    {
        ...
        MiniDFSCluster miniDFSCluster = new MiniDFSCluster.Builder(TestReplicationFilterTransactions.dfsConf).numDataNodes(2).format(true).build();

        Map<String, String> conf = setupConf(miniDFSCluster.getFileSystem().getUri().toString(), PrimaryEventListenerTestImpl.class.getName());
        primary = new WarehouseInstance(LOG, miniDFSCluster, conf);

        conf = setupConf(miniDFSCluster.getFileSystem().getUri().toString(), ReplicaEventListenerTestImpl.class.getName());
    ...
    private void prepareBootstrapData() throws Throwable
    {
        ...
        primary.run("use " + primaryDbName).run("create table t999 (id int) clustered by(id) into 3 buckets stored as orc " + "tblproperties (\"transactional\"=\"true\")").run("insert into t999 values (99901)").run("insert into t999 values (99902)").run("insert into t999 values (99903)").run("insert into t999 values (99904)").run("insert into t999 values (99905)").run("insert into t999 values (99906)").run("insert into t999 values (99907)").run("insert into t999 values (99908)").run("insert into t999 values (99909)").run("insert into t999 values (99910)").run("drop table t999");
        txnOffset = 10;
        ...
        primary.run("use " + primaryDbName).run("create table t1 (id int) clustered by(id) into 3 buckets stored as orc " + "tblproperties (\"transactional\"=\"true\")").run("insert into t1 values(1)").run("create table t2 (place string) partitioned by (country string) clustered by(place) " + "into 3 buckets stored as orc tblproperties (\"transactional\"=\"true\")").run("insert into t2 partition(country='india') values ('bangalore')").run("create table t3 (id int) stored as orc tblproperties (\"transactional\"=\"true\")").run("insert into t3 values(111), (222)");

        // otherDbName is not replicated, but contains ACID tables.
        primary.run("use " + otherDbName).run("create table t1 (id int) clustered by(id) into 3 buckets stored as orc " + "tblproperties (\"transactional\"=\"true\")").run("insert into t1 values(100)").run("create table t2 (place string) partitioned by (country string) clustered by(place) " + "into 3 buckets stored as orc tblproperties (\"transactional\"=\"true\")").run("insert into t2 partition(country='usa') values ('san francisco')").run("create table t3 (id int) stored as orc tblproperties (\"transactional\"=\"true\")").run("insert into t3 values(1110), (2220)");
    }
    ...
    private void alterBucketFile(Path warehouseRoot, String dbName, String tableName, boolean toCorrupted) throws IOException
    {
        ...
        PathBuilder pb = new PathBuilder(warehouseRoot.toString()).addDescendant(dbName.toLowerCase() + ".db").addDescendant(tableName).addDescendant("delta_0000001_0000001_0000").addDescendant(bucket);
    ...
    private void prepareAbortTxn(String dbName, int value) throws Throwable
    {
        ...
        try
        {
            primary.run("use " + dbName).run("update t3 set id = 999 where id = " + String.valueOf(value));
    ...
    {
        primary.run("use " + primaryDbName).run("insert into t1 values (2), (3)").run("insert into t2 partition(country='india') values ('chennai')").run("insert into t2 partition(country='india') values ('pune')");
        prepareAbortTxn(primaryDbName, 222);
        primary.run("use " + otherDbName).run("insert into t1 values (200), (300)").run("insert into t2 partition(country='usa') values ('santa clara')").run("insert into t2 partition(country='usa') values ('palo alto')");
        prepareAbortTxn(otherDbName, 2220);
    }

    private List<String> withTxnOptimized(boolean optimizationOn)
    {
        return Collections.singletonList(String.format("'%s'='%s'", HiveConf.ConfVars.REPL_FILTER_TRANSACTIONS.toString(), String.valueOf(optimizationOn)));
    }
    ...
    public void testTxnEventsUnoptimized() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).dump(primaryDbName, withTxnOptimized(false));
        ...
        tuple = primary.run("use " + primaryDbName).dump(primaryDbName, withTxnOptimized(false));
        replica.load(replicatedDbName, primaryDbName, withTxnOptimized(false));
    ...
    public void testTxnEventsOptimized() throws Throwable
    {
        ...
        prepareIncrementalData();
        tuple = primary.run("use " + primaryDbName).dump(primaryDbName, withTxnOptimized(true));
