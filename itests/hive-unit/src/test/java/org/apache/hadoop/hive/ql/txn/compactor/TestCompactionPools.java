...
public class TestCompactionPools extends TestCompactorBase
{
    ...
    public void setup() throws Exception
    {
        ...
        executeStatementOnDriver("CREATE TABLE " + TABLE_NAME + "(a INT, b STRING) " + " CLUSTERED BY(a) INTO 4 BUCKETS" + //currently ACID requires table to be bucketed
                " STORED AS ORC TBLPROPERTIES ('transactional'='true')", driver);

        StrictDelimitedInputWriter writer = StrictDelimitedInputWriter.newBuilder().withFieldDelimiter(',').build();
        HiveStreamingConnection connection = HiveStreamingConnection.newBuilder().withDatabase(DB_NAME).withTable(TABLE_NAME).withAgentInfo("UT_" + Thread.currentThread().getName()).withHiveConf(conf).withRecordWriter(writer).connect();
        ...
        execSelectAndDumpData("select * from " + TABLE_NAME, driver, "Dumping data for " + TABLE_NAME + " after load:");
    }
