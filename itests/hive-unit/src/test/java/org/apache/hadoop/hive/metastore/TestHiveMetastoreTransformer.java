...
public class TestHiveMetastoreTransformer
{
    ...
    public void setUp() throws Exception
    {
        ...
        wh = new File(System.getProperty("java.io.tmpdir") + File.separator + "hive" + File.separator + "warehouse" + File.separator + "hive" + File.separator);
        wh.mkdirs();

        ext_wh = new File(System.getProperty("java.io.tmpdir") + File.separator + "hive" + File.separator + "warehouse" + File.separator + "hive-external" + File.separator);
        ext_wh.mkdirs();
    ...
    public void testTransformerWithOldTables() throws Exception
    {
        try
        {
            ...
            assertEquals("Created and retrieved tables do not match:" + tbl2.getTableName() + ":" + tblName, tbl2.getTableName(), tblName);
    ...
    public void testTransformerExternalTable() throws Exception
    {
        try
        {
            ...
            assertEquals("Created and retrieved tables do not match:" + hiveTbl.getTableName() + ":" + tblNameUpper.toLowerCase(Locale.ROOT), hiveTbl.getTableName(), tblNameUpper.toLowerCase(Locale.ROOT));
            Path expectedTablePath = new Path(db.getLocationUri(), tblNameUpper.toLowerCase(Locale.ROOT));
            assertEquals(String.format("Table location %s is not a subset of the database location %s", actualPath.toString(), db.getLocationUri()), expectedTablePath.toString(), actualPath.toString());
            ...
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType()); // old client, AccessType not set
            ...
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType()); // RW with HIVEBUCKET2 but no EXTWRITE
            resetHMSClient();

            setHMSClient("testTransformerAcceptsUnmodifiedMetadata", (new String[]{"ACCEPTS_UNMODIFIED_METADATA"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Expected buckets does not match:", buckets, tbl2.getSd().getNumBuckets()); // no transformation
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType()); // RO without HIVEBUCKET2 but with ACCEPTS_UNMODIFIED_METADATA
            resetHMSClient();
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType());
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType());
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType());
            ...
            assertTrue("Returned required capabilities list does not contain HIVEBUCKET2", tbl2.getRequiredWriteCapabilities().contains("HIVEBUCKET2"));
            resetHMSClient();
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_NONE, tbl2.getAccessType()); // requires EXTREAD for RO
            ...
            assertNotNull("Required write capabilities is null", tbl2.getRequiredWriteCapabilities());
            assertTrue("Required read capabilities does not contain EXTREAD", tbl2.getRequiredReadCapabilities().contains("EXTREAD"));
            assertTrue("Required write capabilities does not contain EXTWRITE", tbl2.getRequiredWriteCapabilities().contains("EXTWRITE"));

            setHMSClient("testTransformerExternalTableSpark", (new String[]{"EXTREAD", "CONNECTORREAD"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType()); // requires EXTREAD for RO
            assertEquals(buckets, tbl2.getSd().getNumBuckets());
            assertNotNull("Required write capabilities is null", tbl2.getRequiredWriteCapabilities());
            assertTrue("Required write capabilities does not contain EXTWRITE", tbl2.getRequiredWriteCapabilities().contains("EXTWRITE"));
    ...
    public void testTransformerManagedTable() throws Exception
    {
        try
        {
            ...
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType()); // no translation to be done, so accessType not set
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType()); // no transformation

            setHMSClient("testMGDwithConnectorRead", new String[]{"CONNECTORREAD"});
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType());
            assertNotNull("Required write capabilities are null", tbl2.getRequiredWriteCapabilities());
            assertTrue("Required write capabilities does not contain CONNECTORWRITE", tbl2.getRequiredWriteCapabilities().contains("CONNECTORWRITE"));
            resetHMSClient();
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType());
            assertNull("Required read capabilities are not null", tbl2.getRequiredReadCapabilities());
            assertNull("Required write capabilities are not null", tbl2.getRequiredWriteCapabilities());
            resetHMSClient();
            ...
            properties.append("transactional=true");
            properties.append(";");
            properties.append(CAPABILITIES_KEY).append("=").append("HIVEMANAGEDINSERTREAD,HIVEMANAGEDINSERTWRITE,HIVECACHEINVALIDATE,").append("HIVEMANAGEDSTATS,CONNECTORREAD,CONNECTORWRITE");
            properties.append(";");
            properties.append("transactional_properties=insert_only");
            tProps.put("TBLNAME", tblName);
            tProps.put("PROPERTIES", properties.toString());
            ...
            setHMSClient("testMGDwithAllWrites", new String[]{"HIVEMANAGEDINSERTWRITE", "HIVECACHEINVALIDATE", "HIVEMANAGEDSTATS", "CONNECTORWRITE"});
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType());
            assertNull("Required read capabilities are not null", tbl2.getRequiredReadCapabilities());
            assertNull("Required write capabilities are not null", tbl2.getRequiredWriteCapabilities());
            ...
            setHMSClient("testMGDwith1MissingWrite", new String[]{"HIVEMANAGEDINSERTREAD", "HIVEMANAGEDINSERTWRITE", "HIVECACHEINVALIDATE"});
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType());
            assertNull("Required read capabilities are not null", tbl2.getRequiredReadCapabilities());
            assertNotNull("Required write capabilities are null", tbl2.getRequiredWriteCapabilities());
            assertTrue("Required write capabilities contains HIVEMANAGEDSTATS", tbl2.getRequiredWriteCapabilities().contains("HIVEMANAGEDSTATS"));
            resetHMSClient();
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType()); // translation skipped due to no capabilities
            ...
            properties.append("transactional=true");
            properties.append(";");
            properties.append(CAPABILITIES_KEY).append("=").append("HIVEFULLACIDREAD,HIVEFULLACIDWRITE,HIVECACHEINVALIDATE,").append("HIVEMANAGEDSTATS,CONNECTORREAD,CONNECTORWRITE");
            tProps.put("TBLNAME", tblName);
            tProps.put("TBLTYPE", TableType.MANAGED_TABLE);
            tProps.put("PROPERTIES", properties.toString());
            setHMSClient("createTable", new String[]{"HIVEMANAGEDINSERTWRITE", "HIVEFULLACIDWRITE"});
            ...
            setHMSClient("testMGDFULLwithAllWrites", new String[]{"HIVEFULLACIDWRITE", "HIVECACHEINVALIDATE", "HIVEMANAGEDSTATS", "CONNECTORWRITE"});
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType());
            resetHMSClient();

            setHMSClient("testMGDFULLwith1MissingWrite", new String[]{"HIVECACHEINVALIDATE", "HIVEMANAGEDSTATS", "CONNECTORWRITE"});
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType());
            resetHMSClient();
            ...
            setHMSClient("testMGDwith1MissingWrite", new String[]{"HIVEFULLACIDREAD", "HIVEFULLACIDWRITE", "HIVECACHEINVALIDATE"});
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, ACCESSTYPE_READWRITE, tbl2.getAccessType());
            resetHMSClient();
            ...
            setHMSClient("testTranformerExternalTable", (new String[]{"HIVEBUCKET2", "EXTREAD", "EXTWRITE"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_NONE, tbl2.getAccessType());
    ...
    public void testTransformerVirtualView() throws Exception
    {
        try
        {
            ...
            assertEquals("Created and retrieved views do not match:" + tbl2.getTableName() + ":" + tblName, tbl2.getTableName(), tblName);
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType()); // old client, no transformation

            setHMSClient("testTransformerVirtualView", (new String[]{"HIVESQL"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType()); // RO accessonly for all views

            setHMSClient("testTransformerVirtualView", (new String[]{"EXTWRITE", "HIVEFULLACIDWRITE"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_NONE, tbl2.getAccessType()); // missing HIVESQL
    ...
    public void testTransformerMaterializedView() throws Exception
    {
        try
        {
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match expected value:" + tblName, 0, tbl2.getAccessType()); // View has capabilities, processor doesnt, no tranformation

            setHMSClient("testTransformerMQTFullSet", (new String[]{"HIVEFULLACIDREAD", "HIVEONLYMQTWRITE", "HIVEMANAGESTATS", "HIVEMQT", "CONNECTORREAD"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_READONLY, tbl2.getAccessType()); // RO accessonly for all views
            ...
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_NONE, tbl2.getAccessType()); // RO accessonly for all views

            setHMSClient("testTransformerMQTDummySet", (new String[]{"EXTWRITE", "HIVEFULLACIDWRITE"}));
            tbl2 = client.getTable(dbName, tblName);
            assertEquals("Table access type does not match the expected value:" + tblName, ACCESSTYPE_NONE, tbl2.getAccessType()); // missing HIVEMQT + *READ
    ...
    public void testCreateTable() throws Exception
    {
        try
        {
            ...
            assertTrue("external.table.purge is expected to be true", tbl2.getParameters().get("external.table.purge").equalsIgnoreCase("TRUE"));
            ...
            try
            {
                ...
                assertTrue("Table params expected to contain ACID properties", tbl2.getParameters().get("transactional").equals("true"));
                assertTrue("Table params not expected to contain INSERT ACID properties", ((tbl2.getParameters().get("transactional_properties") == null) || !(tbl2.getParameters().get("transactional_properties").equalsIgnoreCase("insert_only"))));
            } catch (MetaException me)
            {
            ...
            table_params.append("transactional=true");
            table_params.append(";");
            table_params.append(CAPABILITIES_KEY).append("=").append("HIVEFULLACIDREAD,HIVEFULLACIDWRITE,HIVECACHEINVALIDATE,").append("HIVEMANAGEDSTATS,CONNECTORREAD,CONNECTORWRITE");
            tProps.put("PROPERTIES", table_params.toString());
            ...
            try
            {
                ...
                assertEquals("TableType expected to be MANAGED_TABLE", TableType.MANAGED_TABLE.name(), tbl2.getTableType());
                assertTrue("Table params expected to contain ACID properties", tbl2.getParameters().get("transactional").equals("true"));
                assertTrue("Table params expected to contain ACID properties", tbl2.getParameters().get("transactional_properties").equals("insert_only"));
            } catch (MetaException me)
            {
                fail("CreateTable expected to succeed, but failed for " + tblName);
            }
    ...
    public void testTransformerAlterTable() throws Exception
    {
        try
        {
            ...
            String expectedPath = ext_wh.getAbsolutePath().concat(File.separator).concat(dbName).concat(DATABASE_WAREHOUSE_SUFFIX).concat(File.separator).concat(tblName);
            assertEquals("Table location", expectedPath, tableLocation);

            String newLocation = wh.getAbsolutePath().concat(File.separator).concat(dbName).concat(File.separator).concat(tblName);
            ...
            newLocation = ext_wh.getAbsolutePath().concat(File.separator).concat(dbName).concat(File.separator).concat(tblName);
    ...
    public void testTransformerDatabase() throws Exception
    {
        try
        {
            ...
            Database db = client.getDatabase(dbName);
            assertTrue("Database location not as expected:actual=" + db.getLocationUri(), db.getLocationUri().contains(conf.get(MetastoreConf.ConfVars.WAREHOUSE_EXTERNAL.getVarname())));
            ...
            assertTrue("Database location expected to be external warehouse:actual=" + db.getLocationUri(), db.getLocationUri().contains(conf.get(MetastoreConf.ConfVars.WAREHOUSE_EXTERNAL.getVarname())));
            resetHMSClient();
            ...
            new DatabaseBuilder().setName(dbWithLocation).setLocation(mgdPath).create(client, conf);
    ...
    public void testTransformerWithNonHiveCatalogs() throws Exception
    {
        try
        {
            ...
            String newLocation = wh.getAbsolutePath().concat(File.separator).concat(sparkDbName).concat(File.separator).concat(tblName);
            tbl2.getSd().setLocation(newLocation);
            ...
            tbl2 = client.getTable(catalog, sparkDbName, tblName);
            newLocation = ext_wh.getAbsolutePath().concat(File.separator).concat(sparkDbName).concat(File.separator).concat(tblName);
            tbl2.getSd().setLocation(newLocation);
