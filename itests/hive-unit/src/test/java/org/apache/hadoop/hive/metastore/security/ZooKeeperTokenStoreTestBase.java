...
public abstract class ZooKeeperTokenStoreTestBase
{
    ...
    private Configuration createConf(String zkPath)
    {
        ...
        conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_CONNECT_STR, "localhost:" + zkPort);
        ...
        if (zkSslEnabled)
        {
            ...
            conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_KEYSTORE_LOCATION, dataFileDir + File.separator + LOCALHOST_KEY_STORE_NAME);
            conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_KEYSTORE_PASSWORD, KEY_STORE_TRUST_STORE_PASSWORD);
            conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_KEYSTORE_TYPE, KEY_STORE_TRUST_STORE_TYPE);
            conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_TRUSTSTORE_LOCATION, dataFileDir + File.separator + TRUST_STORE_NAME);
            conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_TRUSTSTORE_PASSWORD, KEY_STORE_TRUST_STORE_PASSWORD);
            conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_TRUSTSTORE_TYPE, KEY_STORE_TRUST_STORE_TYPE);
    ...
    public void testAclNoAuth() throws Exception
    {
        ...
        Configuration conf = createConf(ZK_PATH);
        conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_ACL, "ip:127.0.0.1:r");
    ...
    public void testAclInvalid() throws Exception
    {
        ...
        Configuration conf = createConf(ZK_PATH);
        conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_ACL, aclString);
    ...
    public void testAclPositive() throws Exception
    {
        ...
        Configuration conf = createConf(ZK_PATH);
        conf.set(MetastoreDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_ACL, "ip:127.0.0.1:cdrwa,world:anyone:cdrwa");
        ts = new ZooKeeperTokenStore();
        ts.setConf(conf);
        ts.init(null, HadoopThriftAuthBridge.Server.ServerMode.METASTORE);
