...
public class TestExportImport
{
    ...
    public static void classLevelSetup() throws Exception
    {
        ...
        conf.set(HiveConf.ConfVars.REPL_RUN_DATA_COPY_TASKS_ON_TARGET.varname, "false");
        MiniDFSCluster miniDFSCluster = new MiniDFSCluster.Builder(conf).numDataNodes(1).format(true).build();
        ...
        srcHiveWarehouse = new WarehouseInstance(LOG, miniDFSCluster, overridesForHiveConf);
        destHiveWarehouse = new WarehouseInstance(LOG, miniDFSCluster, overridesForHiveConf);
        dumpExternalWarehouse = new WarehouseInstance(LOG, miniDFSCluster, overridesForHiveConfDump);
    }
    ...
    public void shouldExportImportATemporaryTable() throws Throwable
    {
        ...
        srcHiveWarehouse.run("use " + dbName).run("create temporary table t1 (i int)").run("insert into table t1 values (1),(2)").run("export table t1 to " + exportPath).run("create temporary table t2 like t1").run("load data inpath '" + importDataPath + "' overwrite into table t2").run("select * from t2").verifyResults(new String[]{"1", "2"});
    }
    ...
    public void dataImportAfterMetadataOnlyImport() throws Throwable
    {
        ...
        srcHiveWarehouse.run("create table " + dbName + ".t1 (i int)").run("insert into table " + dbName + ".t1 values (1),(2)").run("export table " + dbName + ".t1 to " + exportMDPath + " for metadata replication('1')").run("export table " + dbName + ".t1 to " + exportDataPath + " for replication('2')");

        destHiveWarehouse.run("import table " + replDbName + ".t1 from " + exportMDPath).run("import table " + replDbName + ".t1 from " + exportDataPath).run("select * from " + replDbName + ".t1").verifyResults(new String[]{"1", "2"});
    }
    ...
    public void testExportExternalTableSetFalse() throws Throwable
    {
        ...
        srcHiveWarehouse.run("create external table " + dbName + ".t1 (i int)").run("insert into table " + dbName + ".t1 values (1),(2)").run("export table " + dbName + ".t1 to " + exportMDPath + " for metadata replication('1')").run("export table " + dbName + ".t1 to " + exportDataPath).runFailure("export table " + dbName + ".t1 to " + exportDataPathRepl + " for replication('2')");
    ...
    public void testExportExternalTableSetTrue() throws Throwable
    {
        ...
        String exportDataPath = "'" + path + "2/'";
        String exportDataPathRepl = "'" + path + "3/'";
        dumpExternalWarehouse.run("create external table " + dbName + ".t1 (i int)").run("insert into table " + dbName + ".t1 values (1),(2)").run("export table " + dbName + ".t1 to " + exportDataPathRepl + " for replication('2')").run("export table " + dbName + ".t1 to " + exportMDPath + " for metadata replication('1')").run("export table " + dbName + ".t1 to " + exportDataPath);
    ...
    public void databaseTheTableIsImportedIntoShouldBeParsedFromCommandLine() throws Throwable
    {
        ...
        srcHiveWarehouse.run("create table " + dbName + ".t1 (i int)").run("insert into table " + dbName + ".t1 values (1),(2)").run("export table " + dbName + ".t1 to " + exportPath);

        destHiveWarehouse.run("create database test1").run("use default").run("import table test1.t1 from " + exportPath).run("select * from test1.t1").verifyResults(new String[]{"1", "2"});
    ...
    public void testExportNonNativeTable() throws Throwable
    {
        ...
        srcHiveWarehouse.run("use " + dbName).run(createTableQuery).runFailure("export table " + tableName + " to '" + exportPath + "'").run("export table " + tableName + " to '" + exportMetaPath + "'" + " for metadata replication('1')");

        destHiveWarehouse.run("use " + replDbName).runFailure("import table " + tableName + " from '" + exportPath + "'").run("show tables").verifyFailure(new String[]{tableName}).run("import table " + tableName + " from '" + exportMetaPath + "'").run("show tables").verifyResult(tableName);
