...
{
    private static final Logger LOG = LoggerFactory.getLogger(TestMetastoreHousekeepingLeaderEmptyConfig.class);
    ...
    public void testHouseKeepingThreadExistence() throws Exception
    {
        ...
        for (Map.Entry<Class<? extends Thread>, Boolean> entry : threadClasses.entrySet())
        {
            ...
            } else
            {
                ...
                Assert.assertFalse("Thread found for class " + entry.getKey().getSimpleName(), entry.getValue());
            }
        }
    }
}
