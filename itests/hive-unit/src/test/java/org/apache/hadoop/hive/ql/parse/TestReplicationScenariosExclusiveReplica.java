...
public class TestReplicationScenariosExclusiveReplica extends BaseReplicationAcrossInstances
{
    ...
    public void testDistCpCopyWithRemoteStagingAndCopyTaskOnTarget() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table t1 (id int)").run("insert into table t1 values (100)").run("create table t2 (id int)").run("insert into table t2 values (200)").dump(primaryDbName, withClauseOptions);
        ...
        replica.load(replicatedDbName, primaryDbName, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResult("100").run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResult("200");

        tuple = primary.run("use " + primaryDbName).run("create external table t3 (id int)").run("insert into table t3 values (300)").run("create table t4 (id int)").run("insert into table t4 values (400)").dump(primaryDbName, withClauseOptions);
        ...
        replica.load(replicatedDbName, primaryDbName, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("show tables like 't2'").verifyResult("t2").run("show tables like 't3'").verifyResult("t3").run("show tables like 't4'").verifyResult("t4").run("select id from t1").verifyResult("100").run("select id from t2").verifyResult("200").run("select id from t3").verifyResult("300").run("select id from t4").verifyResult("400");
    }
    ...
    public void testTableLevelReplicationWithRemoteStaging() throws Throwable
    {
        List<String> withClauseOptions = getStagingLocationConfig(replica.repldDir, true);
        withClauseOptions.add("'" + HiveConf.ConfVars.HIVE_IN_TEST_REPL.varname + "'='" + false + "'");
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table t1 (id int)").run("insert into table t1 values (100)").run("create table t2 (id int)").run("insert into table t2 values (200)").dump(primaryDbName + ".'t[0-9]+'", withClauseOptions);

        // verify that the external table list is written correctly for bootstrap
        ReplicationTestUtils.assertExternalFileList(Arrays.asList("t1"), tuple.dumpLocation, replica);

        //verify table list
        verifyTableListForPolicy(replica.miniDFSCluster.getFileSystem(), tuple.dumpLocation, new String[]{"t1", "t2"});

        replica.load(replicatedDbName, primaryDbName, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResult("100").run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResult("200");

        tuple = primary.run("use " + primaryDbName).run("create external table t3 (id int)").run("insert into table t3 values (300)").run("create table t4 (id int)").run("insert into table t4 values (400)").run("create table t5 (id int) partitioned by (p int)").run("insert into t5 partition(p=1) values(10)").run("insert into t5 partition(p=2) values(20)").dump(primaryDbName + ".'t[0-9]+'", withClauseOptions);

        // verify that the external table list is written correctly for incremental
        ReplicationTestUtils.assertExternalFileList(Arrays.asList("t1", "t3"), tuple.dumpLocation, replica);

        //verify table list
        verifyTableListForPolicy(replica.miniDFSCluster.getFileSystem(), tuple.dumpLocation, new String[]{"t1", "t2", "t3", "t4", "t5"});

        replica.load(replicatedDbName, primaryDbName, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("show tables like 't2'").verifyResult("t2").run("show tables like 't3'").verifyResult("t3").run("show tables like 't4'").verifyResult("t4").run("show tables like 't5'").verifyResult("t5").run("select id from t1").verifyResult("100").run("select id from t2").verifyResult("200").run("select id from t3").verifyResult("300").run("select id from t4").verifyResult("400").run("select id from t5").verifyResults(new String[]{"10", "20"});
    }
    ...
    public void testDistCpCopyWithLocalStagingAndCopyTaskOnTarget() throws Throwable
    {
        ...
        withClauseOptions.add("'" + HiveConf.ConfVars.HIVE_IN_TEST_REPL.varname + "'='" + false + "'");
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table t1 (id int)").run("insert into table t1 values (500)").run("create table t2 (id int)").run("insert into table t2 values (600)").dump(primaryDbName, withClauseOptions);

        // verify that the external table list is written correctly for bootstrap
        ReplicationTestUtils.assertExternalFileList(Arrays.asList("t1"), tuple.dumpLocation, primary);

        replica.load(replicatedDbName, primaryDbName, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResult("500").run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResult("600");

        tuple = primary.run("use " + primaryDbName).run("create external table t3 (id int)").run("insert into table t3 values (700)").run("create table t4 (id int)").run("insert into table t4 values (800)").dump(primaryDbName, withClauseOptions);

        // verify that the external table list is written correctly for incremental
        ReplicationTestUtils.assertExternalFileList(Arrays.asList("t1", "t3"), tuple.dumpLocation, primary);

        replica.load(replicatedDbName, primaryDbName, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("show tables like 't2'").verifyResult("t2").run("show tables like 't3'").verifyResult("t3").run("show tables like 't4'").verifyResult("t4").run("select id from t1").verifyResult("500").run("select id from t2").verifyResult("600").run("select id from t3").verifyResult("700").run("select id from t4").verifyResult("800");
    }
    ...
    public void externalTableReplicationDropDatabase() throws Throwable
    {
        ...
        String tableName = "t1";
        List<String> withClauseOptions = getStagingLocationConfig(primary.repldDir, false);
        WarehouseInstance.Tuple tuple = primary.run("create database " + primaryDb).run("alter database " + primaryDb + " set dbproperties('repl.source.for'='1,2,3')").run("use " + primaryDb).run("create external table " + tableName + " (id int)").run("insert into table " + tableName + " values (500)").dump(primaryDb, withClauseOptions);

        replica.load(replicaDb, primaryDb, withClauseOptions).run("use " + replicaDb).run("show tables like '" + tableName + "'").verifyResult(tableName).run("select id from " + tableName).verifyResult("500");
        ...
        primary.run("show databases like '" + primaryDb + "'").verifyResult(primaryDb);
        replica.run("show databases like '" + replicaDb + "'").verifyResult(replicaDb);
        primary.run("drop database " + primaryDb + " cascade");
        replica.run("drop database " + replicaDb + " cascade");
        primary.run("show databases like '" + primaryDb + "'").verifyResult(null);
        replica.run("show databases like '" + replicaDb + "'").verifyResult(null);
    ...
    public void testCustomWarehouseLocations() throws Throwable
    {
        ...
        primary.run("use " + srcDb).run("insert into table t1 values (1000)").run("insert into table t2 values (2000)").run("insert into tp1 partition(p=1) values(30)").run("insert into tp1 partition(p=2) values(40)").dump(srcDb, withClauseOptions);
        replica.load(replicatedDbName, srcDb, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResults(new String[]{"500", "1000"}).run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResults(new String[]{"1000", "2000"}).run("show tables like 'tp1'").verifyResult("tp1").run("select id from tp1").verifyResults(new String[]{"10", "20", "30", "40"});
        primary.run("use " + srcDb).run("insert into table t1 values (2000)").run("insert into table t2 values (3000)").run("create table t3 (id int)").run("insert into table t3 values (3000)").run("create external table t4 (id int)").run("insert into table t4 values (4000)").run("insert into tp1 partition(p=1) values(50)").run("insert into tp1 partition(p=2) values(60)").run("create table tp2 (id int) partitioned by (p int)").run("insert into tp2 partition(p=1) values(100)").run("insert into tp2 partition(p=2) values(200)").dump(srcDb, withClauseOptions);
        replica.load(replicatedDbName, srcDb, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResults(new String[]{"500", "1000", "2000"}).run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResults(new String[]{"1000", "2000", "3000"}).run("show tables like 't3'").verifyResult("t3").run("select id from t3").verifyResults(new String[]{"3000"}).run("show tables like 't4'").verifyResult("t4").run("select id from t4").verifyResults(new String[]{"4000"}).run("select id from tp1").verifyResults(new String[]{"10", "20", "30", "40", "50", "60"}).run("show tables like 'tp1'").verifyResult("tp1").run("select id from tp2").verifyResults(new String[]{"100", "200"});
    ...
    public void testCustomWarehouseLocationsConf() throws Throwable
    {
        ...
        String dbWhExternalLoc = new Path(primary.externalTableWarehouseRoot.getParent(), "customExternalLoc1").toUri().getPath();
        String srcDb = "srcDbConf";
        WarehouseInstance.Tuple tuple = primary.run("create database " + srcDb + " LOCATION '" + dbWhExternalLoc + "' MANAGEDLOCATION '" + dbWhManagedLoc + "' WITH DBPROPERTIES ( '" + SOURCE_OF_REPLICATION + "' = '1,2,3')").run("use " + srcDb).run("create table t1 (id int)").run("insert into table t1 values (500)").run("create external table t2 (id int)").run("insert into table t2 values (1000)").dump(srcDb, withClauseOptions);

        withClauseOptions.add("'" + HiveConf.ConfVars.REPL_RETAIN_CUSTOM_LOCATIONS_FOR_DB_ON_TARGET.varname + "'='false'");
        replica.load(replicatedDbName, srcDb, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResult("500").run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResult("1000");
        List<String> listOfTables = new ArrayList<>();
        listOfTables.addAll(Arrays.asList("t1", "t2"));
        verifyCustomDBLocations(srcDb, listOfTables, dbWhManagedLoc, dbWhExternalLoc, false);
        primary.run("use " + srcDb).run("insert into table t1 values (1000)").run("insert into table t2 values (2000)").dump(srcDb, withClauseOptions);
        replica.load(replicatedDbName, srcDb, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResults(new String[]{"500", "1000"}).run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResults(new String[]{"1000", "2000"});
        primary.run("use " + srcDb).run("insert into table t1 values (2000)").run("insert into table t2 values (3000)").run("create table t3 (id int)").run("insert into table t3 values (3000)").run("create external table t4 (id int)").run("insert into table t4 values (4000)").dump(srcDb, withClauseOptions);
        replica.load(replicatedDbName, srcDb, withClauseOptions).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("select id from t1").verifyResults(new String[]{"500", "1000", "2000"}).run("show tables like 't2'").verifyResult("t2").run("select id from t2").verifyResults(new String[]{"1000", "2000", "3000"}).run("show tables like 't3'").verifyResult("t3").run("select id from t3").verifyResults(new String[]{"3000"}).run("show tables like 't4'").verifyResult("t4").run("select id from t4").verifyResults(new String[]{"4000"});
        listOfTables.addAll(Arrays.asList("t3", "t4"));
        verifyCustomDBLocations(srcDb, listOfTables, dbWhManagedLoc, dbWhExternalLoc, false);
    }

    private void verifyCustomDBLocations(String srcDb, List<String> listOfTables, String managedCustLocOnSrc, String externalCustLocOnSrc, boolean replaceCustPath) throws Exception
    {
        ...
        if (replaceCustPath)
        {
            ...
            Assert.assertNotEquals(externalCustLocOnTgt, new Path(replica.externalTableWarehouseRoot, replicatedDbName.toLowerCase() + ".db").toUri().getPath());
        } else
        {
            Assert.assertNotEquals(managedCustLocOnSrc, null);
            Assert.assertEquals(managedCustLocOnTgt, new Path(replica.warehouseRoot, replicatedDbName.toLowerCase() + ".db").toUri().getPath());
            Assert.assertNotEquals(externalCustLocOnSrc, externalCustLocOnTgt);
            Assert.assertEquals(externalCustLocOnTgt, new Path(replica.externalTableWarehouseRoot, replicatedDbName.toLowerCase() + ".db").toUri().getPath());
        }
        verifyTableLocations(srcDb, replDatabase, listOfTables, replaceCustPath);
    }

    private void verifyTableLocations(String srcDb, Database replDb, List<String> tables, boolean customLocOntgt) throws Exception
    {
        ...
        for (String tname : tables)
        {
            ...
            {
                //Managed Table case
                Path tblPathOnTgt = customLocOntgt ? new Path(replDb.getManagedLocationUri(), tname) : new Path(replica.warehouseRoot, replicatedDbName.toLowerCase() + ".db" + "/" + tname);
                Assert.assertEquals(new Path(table.getSd().getLocation()), tblPathOnTgt);
            }
        }
    }

    private void verifyTableDataExists(WarehouseInstance warehouse, Path dbDataPath, String tableName, boolean shouldExists) throws IOException
    {
    ...
    private List<String> getStagingLocationConfig(String stagingLoc, boolean addDistCpConfigs) throws IOException
    {
        ...
        if (addDistCpConfigs)
        {
            ...
            confList.add("'" + HiveConf.ConfVars.HIVE_DISTCP_DOAS_USER.varname + "'='" + UserGroupInformation.getCurrentUser().getUserName() + "'");
        }
        return confList;
    }
