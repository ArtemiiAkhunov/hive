...
public class TestReplScenariosWithStrictManaged extends BaseReplicationAcrossInstances
{
    ...
    public void dynamicallyConvertManagedToExternalTable() throws Throwable
    {
        ...
        primary.run("use " + primaryDbName).run("create table t1 (id int) stored as orc").run("insert into table t1 values (1)").run("create table t2 (id int) partitioned by (key int) stored as orc").run("insert into table t2 partition(key=10) values (1)").runFailure("alter table t1 set tblproperties('EXTERNAL'='true')").runFailure("alter table t1 set tblproperties('EXTERNAL'='true', 'TRANSACTIONAL'='false')").runFailure("alter table t2 set tblproperties('EXTERNAL'='true')");
    }
    ...
    public void dynamicallyConvertExternalToManagedTable() throws Throwable
    {
        ...
        primary.run("use " + primaryDbName).run("create external table t1 (id int) stored as orc").run("insert into table t1 values (1)").run("create external table t2 (place string) partitioned by (country string)").run("insert into table t2 partition(country='india') values ('bangalore')").runFailure("alter table t1 set tblproperties('EXTERNAL'='false')").runFailure("alter table t1 set tblproperties('EXTERNAL'='false', 'TRANSACTIONAL'='true')").runFailure("alter table t2 set tblproperties('EXTERNAL'='false')");
    }
}
