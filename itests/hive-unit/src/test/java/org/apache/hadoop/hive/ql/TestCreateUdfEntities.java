...
public class TestCreateUdfEntities
{
    ...
    {
        int rc = ((ReExecDriver) driver).compile("CREATE FUNCTION " + funcName + " AS " + "'org.apache.hadoop.hive.ql.udf.generic.GenericUDFPrintf'  using file '" + "file:///tmp/udf1.jar'", true);
        assertEquals(0, rc);
    ...
    {
        int rc = ((ReExecDriver) driver).compile("CREATE FUNCTION default." + funcName + " AS " + "'org.apache.hadoop.hive.ql.udf.generic.GenericUDFPrintf'  using file '" + "hdfs:///tmp/udf1.jar'", true);
        assertEquals(0, rc);
        WriteEntity outputEntities[] = driver.getPlan().getOutputs().toArray(new WriteEntity[]{});
        assertEquals(outputEntities.length, 2);
