...
public class TestTimedOutTxnNotificationLogging
{
    ...
    {
        return Arrays.asList(new Object[][]{{3, TxnType.REPL_CREATED, 3}, {3, TxnType.DEFAULT, 3}, {3, TxnType.READ_ONLY, 0}});
    }
    ...
    private void setConf()
    {
        ...
        MetastoreConf.setTimeVar(hiveConf, MetastoreConf.ConfVars.EVENT_DB_LISTENER_CLEAN_INTERVAL, 10, TimeUnit.MILLISECONDS);
        MetastoreConf.setTimeVar(hiveConf, MetastoreConf.ConfVars.EVENT_DB_LISTENER_CLEAN_STARTUP_WAIT_INTERVAL, 0, TimeUnit.SECONDS);
    }
    ...
    private int getNumberOfNotificationsWithEventType(List<Long> txnIds, String eventType) throws TException
    {
        ...
        IMetaStoreClient.NotificationFilter evFilter = new AndFilter(new ReplEventFilter(new ReplScope()), new CatalogFilter(MetaStoreUtils.getDefaultCatalog(hiveConf)), new EventBoundaryFilter(0, 100));
