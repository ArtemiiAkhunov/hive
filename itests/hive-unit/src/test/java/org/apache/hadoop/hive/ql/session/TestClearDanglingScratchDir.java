...
public class TestClearDanglingScratchDir
{
    ...
    static public void oneTimeSetup() throws Exception
    {
        ...
        conf.setVar(HiveConf.ConfVars.METASTORE_WAREHOUSE, new Path(System.getProperty("test.tmp.dir"), "warehouse").toString());
    ...
    public void testLocalDanglingFilesCleaning() throws Exception
    {
        ...
        Path localPipeOutFileRemove = new Path(customLocalTmpDir + l + appId + "-started-with-session-name.pipeout");
        Path localPipeOutFileNotRemove = new Path(customLocalTmpDir + l + "not-started-with-session-name-" + appId + ".pipeout");
        Path localPipeOutFileFailRemove = new Path(customLocalTmpDir + l + appId + "-started-with-session-name-but-fail-delete.pipeout");
        ...
        ClearDanglingScratchDir clearDanglingScratchDirMain = new ClearDanglingScratchDir(false, false, true, hdfsRootDir.toString(), conf);
        ...
        Assert.assertFalse("Local session dir '" + localSessionDir + "' still exists, should have been removed!", fs.exists(localSessionDir));
        Assert.assertFalse("Local .pipeout file '" + localPipeOutFileRemove + "' still exists, should have been removed!", fs.exists(localPipeOutFileRemove));
        Assert.assertTrue("Local .pipeout file '" + localPipeOutFileNotRemove + "' does not exist, should have not been removed!", fs.exists(localPipeOutFileNotRemove));
        Assert.assertTrue("Local .pipeout file '" + localPipeOutFileFailRemove + "' does not exist, should have not been removed!", fs.exists(localPipeOutFileFailRemove));
    }
}
