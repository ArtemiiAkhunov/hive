...
class CompactorTestUtil
{
     ...
     */
    static List<String> getBaseOrDeltaNames(FileSystem fs, PathFilter filter, Table table, String partitionName) throws IOException
    {
        Path path = partitionName == null ? new Path(table.getSd().getLocation()) : new Path(table.getSd().getLocation(), partitionName);
        FileStatus[] fileStatuses = filter != null ? fs.listStatus(path, filter) : fs.listStatus(path);
        return Arrays.stream(fileStatuses).map(FileStatus::getPath).map(Path::getName).sorted().collect(Collectors.toList());
    }
     ...
     */
    static List<String> getBucketFileNames(FileSystem fs, Table table, String partitionName, String deltaName) throws IOException
    {
        Path path = partitionName == null ? new Path(table.getSd().getLocation(), deltaName) : new Path(new Path(table.getSd().getLocation()), new Path(partitionName, deltaName));
        return Arrays.stream(fs.listStatus(path, AcidUtils.bucketFileFilter)).map(FileStatus::getPath).map(Path::getName).sorted().collect(Collectors.toList());
    }

    static List<String> getBucketFileNamesForMMTables(FileSystem fs, Table table, String partitionName, String deltaName) throws IOException
    {
        Path path = partitionName == null ? new Path(table.getSd().getLocation(), deltaName) : new Path(new Path(table.getSd().getLocation()), new Path(partitionName, deltaName));
        return Arrays.stream(fs.listStatus(path, FileUtils.HIDDEN_FILES_PATH_FILTER)).map(FileStatus::getPath).map(Path::getName).sorted().collect(Collectors.toList());
    }

    static List<String> getBucketFileNamesWithoutAttemptId(FileSystem fs, Table table, String partitionName, List<String> deltaDirs) throws IOException
    {
     ...
     */
    static void runCompaction(HiveConf conf, String dbName, String tblName, CompactionType compactionType, boolean isQueryBased, Map<String, String> properties, String... partNames) throws Exception
    {
     ...
     * @throws Exception compaction cannot be started.
     */
    static void runCompaction(HiveConf conf, String dbName, String tblName, CompactionType compactionType, boolean isQueryBased, String... partNames) throws Exception
    {
    ...
    static void runStreamingAPI(HiveConf conf, String dbName, String tblName, List<StreamingConnectionOption> connectionOption) throws StreamingException
    {
