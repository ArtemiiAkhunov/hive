...
public class TestCompactor extends TestCompactorBase
{
    ...
    public void testHeartbeatShutdownOnFailedCompaction() throws Exception
    {
        ...
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) " + " PARTITIONED BY(bkt INT)" + " CLUSTERED BY(a) INTO 4 BUCKETS" + //currently ACID requires table to be bucketed
                " STORED AS ORC  TBLPROPERTIES ('transactional'='true')", driver);

        StrictDelimitedInputWriter writer = StrictDelimitedInputWriter.newBuilder().withFieldDelimiter(',').build();
        HiveStreamingConnection connection = HiveStreamingConnection.newBuilder().withDatabase(dbName).withTable(tblName).withStaticPartitionValues(Collections.singletonList("0")).withAgentInfo("UT_" + Thread.currentThread().getName()).withHiveConf(conf).withRecordWriter(writer).connect();
    ...
    public void schemaEvolutionAddColDynamicPartitioningInsert() throws Exception
    {
        String tblName = "dpct";
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) " + " PARTITIONED BY(ds string)" + " CLUSTERED BY(a) INTO 2 BUCKETS" + //currently ACID requires table to be bucketed
                " STORED AS ORC TBLPROPERTIES ('transactional'='true')", driver);
    ...
    public void testStatsAfterCompactionPartTbl() throws Exception
    {
        ...
        statsUpdater.gatherStats(ci, conf, System.getProperty("user.name"), CompactorUtil.getCompactorJobQueueName(conf, ci, table), msClient);
    ...
    public void minorCompactWhileStreaming() throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) " + " CLUSTERED BY(a) INTO 1 BUCKETS" + //currently ACID requires table to be bucketed
                " STORED AS ORC  TBLPROPERTIES ('transactional'='true')", driver);
    ...
    public void autoCompactOnStreamingIngestWithDynamicPartition() throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a STRING) " + " PARTITIONED BY (b INT)" + //currently ACID requires table to be bucketed
                " STORED AS ORC  TBLPROPERTIES ('transactional'='true')", driver);
        ...
        StreamingConnection connection1 = HiveStreamingConnection.newBuilder().withDatabase(dbName).withTable(tblName).withAgentInfo(agentInfo).withHiveConf(conf).withRecordWriter(writer1).withStreamingOptimizations(true)
                // Transaction size has to be one or exception should happen.
                .withTransactionBatchSize(1).connect();

        StreamingConnection connection2 = HiveStreamingConnection.newBuilder().withDatabase(dbName).withTable(tblName).withAgentInfo(agentInfo).withHiveConf(conf).withRecordWriter(writer2).withStreamingOptimizations(true)
                // Transaction size has to be one or exception should happen.
                .withTransactionBatchSize(1).connect();
    ...
    public void testNoDataLossWhenMaxNumDeltaIsUsed() throws Exception
    {
        ...
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) " + " STORED AS ORC  TBLPROPERTIES ('transactional'='true')", driver);
        ...
        for (int i = 0; i < 3; i++)
        {
            executeStatementOnDriver("MERGE INTO " + tblName + " AS T USING (" + "select * from " + tblName + " union all select a+1, b from " + tblName + ") AS S " + "ON T.a=s.a " + "WHEN MATCHED THEN DELETE " + "WHEN not MATCHED THEN INSERT values (s.a, s.b)", driver);
        }
    ...
    public void majorCompactAfterAbort() throws Exception
    {
        ...
        CompactorTestUtil.checkExpectedTxnsPresent(stat[0].getPath(), null, columnNamesProperty, columnTypesProperty, 0, 1L, 4L, Lists.newArrayList(5, 6), 1);
    }
    ...
    public void testAbortAfterMarkCleaned() throws Exception
    {
        ...
        // Create three folders with two different transactions
        HiveStreamingConnection connection1 = HiveStreamingConnection.newBuilder().withDatabase(dbName).withTable(tableName).withAgentInfo(agentInfo).withHiveConf(conf).withRecordWriter(writer).withStreamingOptimizations(true).withTransactionBatchSize(1).connect();

        HiveStreamingConnection connection2 = HiveStreamingConnection.newBuilder().withDatabase(dbName).withTable(tableName).withAgentInfo(agentInfo).withHiveConf(conf).withRecordWriter(writer).withStreamingOptimizations(true).withTransactionBatchSize(1).connect();
        ...
        List<TaskHandler> cleanupHandlers = TaskHandlerFactory.getInstance().getHandlers(conf, mockedTxnHandler, metadataCache, false, fsRemover);
    ...
    private void assertAndCompactCleanAbort(String dbName, String tblName, boolean partialAbort, boolean singleSession) throws Exception
    {
        ...
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from COMPACTION_QUEUE"), useCleanerForAbortCleanup ? 0 : 1, count);
        runWorker(conf);
        ...
        RemoteIterator<LocatedFileStatus> it = fs.listFiles(new Path(table.getSd().getLocation()), true);
    ...
    public void testCleanAbortCompactSeveralTables() throws Exception
    {
        ...
        // Only one job is added to the queue per table. This job corresponds to all the entries for a particular table
        // with rows in TXN_COMPONENTS
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from COMPACTION_QUEUE"), useCleanerForAbortCleanup ? 0 : 2, count);
        ...
        RemoteIterator<LocatedFileStatus> it = fs.listFiles(new Path(table1.getSd().getLocation()), true);
        if (it.hasNext())
        {
            Assert.fail("Expected cleaner to drop aborted delta & base directories, FileStatus[] stat " + Arrays.toString(stat));
        }
    ...
    public void testCleanDynPartAbortNoDataLoss() throws Exception
    {
        ...
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from COMPACTION_QUEUE"), useCleanerForAbortCleanup ? 3 : 4, count);
    ...
    public void testCleanAbortAndMinorCompact() throws Exception
    {
        ...
        int count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from COMPACTION_QUEUE");
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from COMPACTION_QUEUE"), useCleanerForAbortCleanup ? 1 : 2, count);
    ...
    private HiveStreamingConnection prepareTableAndConnection(String dbName, String tblName, int batchSize) throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(b STRING) " + " PARTITIONED BY (a INT)" + //currently ACID requires table to be bucketed
                " STORED AS ORC  TBLPROPERTIES ('transactional'='true')", driver);

        StrictDelimitedInputWriter writer = StrictDelimitedInputWriter.newBuilder().withFieldDelimiter(',').build();

        // Create three folders with two different transactions
        return HiveStreamingConnection.newBuilder().withDatabase(dbName).withTable(tblName).withAgentInfo(agentInfo).withHiveConf(conf).withRecordWriter(writer).withStreamingOptimizations(true)
                // Transaction size has to be one or exception should happen.
                .withTransactionBatchSize(batchSize).connect();
    }
    ...
    private HiveStreamingConnection prepareTableTwoPartitionsAndConnection(String dbName, String tblName, int batchSize) throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(c STRING) " + " PARTITIONED BY (a INT, b INT)" + //currently ACID requires table to be bucketed
                " STORED AS ORC  TBLPROPERTIES ('transactional'='true')", driver);
    ...
    public void mmTable() throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) STORED AS ORC" + " TBLPROPERTIES ('transactional'='true', 'transactional_properties'='insert_only')", driver);
        IMetaStoreClient msClient = new HiveMetaStoreClient(conf);
        Table table = msClient.getTable(dbName, tblName);
        msClient.close();
    ...
    @Test
    public void mmTableOriginalsMajorOrc() throws Exception
    {
        mmTableOriginalsMajor("orc", true);
    }

    @Test
    public void mmTableOriginalsMajorText() throws Exception
    {
    ...
    private void mmTableOriginalsMajor(String format, boolean allowOriginals) throws Exception
    {
        ...
        String tblName = "mm_nonpart";
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) STORED AS " + format + " TBLPROPERTIES ('transactional'='false')", driver);
        Table table = msClient.getTable(dbName, tblName);

        executeStatementOnDriver("INSERT INTO " + tblName + "(a,b) VALUES(1, 'foo')", driver);
        executeStatementOnDriver("INSERT INTO " + tblName + " (a,b) VALUES(2, 'bar')", driver);
        executeStatementOnDriver("INSERT INTO " + tblName + "(a,b) SELECT a,b FROM " + tblName + " UNION ALL SELECT a,b FROM " + tblName, driver);

        verifyFooBarResult(tblName, 3);

        FileSystem fs = FileSystem.get(conf);
        executeStatementOnDriver("ALTER TABLE " + tblName + " SET TBLPROPERTIES " + "('transactional'='true', 'transactional_properties'='insert_only')", driver);
    ...
    @Test
    public void mmMajorOriginalsDeltasOrc() throws Exception
    {
        mmMajorOriginalsDeltas("orc", true);
    }

    @Test
    public void mmMajorOriginalsDeltasText() throws Exception
    {
    ...
    @Test
    public void mmMajorOriginalsBaseOrc() throws Exception
    {
        mmMajorOriginalsBase("orc", true);
    }

    @Test
    public void mmMajorOriginalsBaseText() throws Exception
    {
    ...
    private void mmMajorOriginalsBase(String format, boolean allowOriginals) throws Exception
    {
        ...
        executeStatementOnDriver("ALTER TABLE " + tblName + " SET TBLPROPERTIES " + "('transactional'='true', 'transactional_properties'='insert_only')", driver);
        executeStatementOnDriver("INSERT OVERWRITE TABLE " + tblName + " SELECT a,b FROM " + tblName + " UNION ALL SELECT a,b FROM " + tblName, driver);
    ...
    public void mmTableBucketed() throws Exception
    {
        ...
        String tblName = "mm_nonpart";
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) CLUSTERED BY (a) " + "INTO 64 BUCKETS STORED AS ORC TBLPROPERTIES ('transactional'='true', " + "'transactional_properties'='insert_only')", driver);
        IMetaStoreClient msClient = new HiveMetaStoreClient(conf);
        Table table = msClient.getTable(dbName, tblName);
        msClient.close();
        ...
        FileStatus[] files = fs.listStatus(new Path(table.getSd().getLocation(), baseDir), FileUtils.HIDDEN_FILES_PATH_FILTER);
    ...
    public void mmTableOpenWriteId() throws Exception
    {
        ...
        String tblName = "mm_nonpart";
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) STORED AS TEXTFILE" + " TBLPROPERTIES ('transactional'='true', 'transactional_properties'='insert_only')", driver);
        IMetaStoreClient msClient = new HiveMetaStoreClient(conf);
        Table table = msClient.getTable(dbName, tblName);
        msClient.close();
    ...
    private void verifyHasBase(StorageDescriptor sd, FileSystem fs, String baseName) throws Exception
    {
        verifyHasDir(sd, fs, baseName, AcidUtils.baseFileFilter);
    }

    private void verifyHasDir(StorageDescriptor sd, FileSystem fs, String name, PathFilter filter) throws Exception
    {
    ...
    private void verifyDeltaCount(StorageDescriptor sd, FileSystem fs, int count) throws Exception
    {
        verifyDirCount(sd, fs, count, AcidUtils.deltaFileFilter);
    }

    private void verifyDirCount(StorageDescriptor sd, FileSystem fs, int count, PathFilter filter) throws Exception
    {
        FileStatus[] stat = fs.listStatus(new Path(sd.getLocation()), filter);
    ...
    public void mmTablePartitioned() throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) " + " PARTITIONED BY(ds int) STORED AS TEXTFILE" + " TBLPROPERTIES ('transactional'='true', 'transactional_properties'='insert_only')", driver);
        ...
        IMetaStoreClient msClient = new HiveMetaStoreClient(conf);
        Partition p1 = msClient.getPartition(dbName, tblName, "ds=1"), p2 = msClient.getPartition(dbName, tblName, "ds=2"), p3 = msClient.getPartition(dbName, tblName, "ds=3");
        msClient.close();
    ...
    public void majorCompactWhileStreamingForSplitUpdate() throws Exception
    {
        ...
        String columnTypesProperty = "int:string";
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("CREATE TABLE " + tblName + "(a INT, b STRING) " + " CLUSTERED BY(a) INTO 2 BUCKETS" + //currently ACID requires table to be bucketed
                " STORED AS ORC  TBLPROPERTIES ('transactional'='true', " + "'transactional_properties'='default') ", driver); // this turns on split-update U=D+I
    ...
    public void testCompactionForFileInSratchDir() throws Exception
    {
        String dbName = "default";
        String tblName = "cfs";
        String createQuery = "CREATE TABLE " + tblName + "(a INT, b STRING) " + "STORED AS ORC  TBLPROPERTIES ('transactional'='true'," + "'transactional_properties'='default')";
        executeStatementOnDriver("drop table if exists " + tblName, driver);
    ...
    public void testTableProperties() throws Exception
    {
        ...
        executeStatementOnDriver("CREATE TABLE " + tblName2 + "(a INT, b STRING) " + " CLUSTERED BY(a) INTO 2 BUCKETS STORED AS ORC TBLPROPERTIES (" + "'transactional'='true'," + "'compactor.mapreduce.map.memory.mb'='2048'," + // 2048 MB memory for compaction map job
    ...
    public void testDisableCompactionDuringReplLoad() throws Exception
    {
        ...
        executeStatementOnDriver("CREATE TABLE " + database + "." + tblName + "(a INT, b STRING) " + " PARTITIONED BY(ds string)" + " CLUSTERED BY(a) INTO 2 BUCKETS" + //currently ACID requires table to be bucketed
                " STORED AS ORC TBLPROPERTIES ('transactional'='true')", driver);
        executeStatementOnDriver("insert into " + database + "." + tblName + " partition (ds) values (1, 'fred', " + "'today'), (2, 'wilma', 'yesterday')", driver);

        executeStatementOnDriver("ALTER TABLE " + database + "." + tblName + " SET TBLPROPERTIES ( 'hive.repl.first.inc.pending' = 'true')", driver);
        List<ShowCompactResponseElement> compacts = getCompactionList();
        Assert.assertEquals(0, compacts.size());

        executeStatementOnDriver("alter database " + database + " set dbproperties ('hive.repl.first.inc.pending' = 'true')", driver);
        executeStatementOnDriver("ALTER TABLE " + database + "." + tblName + " SET TBLPROPERTIES ( 'hive.repl.first.inc.pending' = 'false')", driver);
        compacts = getCompactionList();
        Assert.assertEquals(0, compacts.size());

        executeStatementOnDriver("alter database " + database + " set dbproperties ('hive.repl.first.inc.pending' = 'false')", driver);
        executeStatementOnDriver("ALTER TABLE " + database + "." + tblName + " SET TBLPROPERTIES ( 'hive.repl.first.inc.pending' = 'false')", driver);
        compacts = getCompactionList();
        Assert.assertEquals(2, compacts.size());
    ...
    public void testCompactionDataLoadedWithInsertOverwrite() throws Exception
    {
        ...
        executeStatementOnDriver("DROP TABLE IF EXISTS " + tableName, driver);
        executeStatementOnDriver("CREATE EXTERNAL TABLE " + externalTableName + "(a int, b int, c int) STORED AS TEXTFILE", driver);
        executeStatementOnDriver("CREATE TABLE " + tableName + "(a int, b int, c int) STORED AS ORC TBLPROPERTIES('transactional'='true')", driver);

        executeStatementOnDriver("INSERT INTO " + externalTableName + " values (1, 1, 1), (2, 2, 2), (3, 3, 3), (4, 4, 4)", driver);
    ...
    public void testAcidDirCacheOnDropTable() throws Exception
    {
        ...
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("create table " + tblName + " (a string) stored as orc " + "TBLPROPERTIES ('transactional'='true', 'hive.exec.orc.split.strategy'='BI')", driver);
    ...
    public void testAcidDirCacheOnDropPartitionedTable() throws Exception
    {
        ...
        // First phase, populate the cache
        executeStatementOnDriver("drop table if exists " + tblName, driver);
        executeStatementOnDriver("create table " + tblName + " (a string) PARTITIONED BY (p string) stored as orc " + "TBLPROPERTIES ('transactional'='true', 'hive.exec.orc.split.strategy'='BI')", driver);
    ...
    private void processStreamingAPI(String dbName, String tblName) throws StreamingException
    {
        List<CompactorTestUtil.StreamingConnectionOption> options = Lists.newArrayList(new CompactorTestUtil.StreamingConnectionOption(false, false), new CompactorTestUtil.StreamingConnectionOption(false, false), new CompactorTestUtil.StreamingConnectionOption(true, false));
