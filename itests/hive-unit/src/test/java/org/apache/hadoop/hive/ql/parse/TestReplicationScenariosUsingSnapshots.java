...
public class TestReplicationScenariosUsingSnapshots extends BaseReplicationAcrossInstances
{
    ...
    public void tearDown() throws Throwable
    {
        ...
        SnapshottableDirectoryStatus[] snaps = primary.miniDFSCluster.getFileSystem().getSnapshottableDirListing();
        ...
        if (snaps != null)
        {
            for (SnapshottableDirectoryStatus sn : snaps)
            {
                ...
                SnapshotUtils.deleteSnapshotSafe(primary.miniDFSCluster.getFileSystem(), path, firstSnapshot(primaryDbName.toLowerCase()));
                SnapshotUtils.deleteSnapshotSafe(primary.miniDFSCluster.getFileSystem(), path, secondSnapshot(primaryDbName.toLowerCase()));
            }
        }
    ...
    public void testBasicReplicationWithSnapshots() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table table1 (id int)").run("insert into table table1 values (1)").run("insert into table table1 values (2)").run("create external table table2 (place string) partitioned by " + "(country string)").run("insert into table table2 partition(country='india') values " + "('bangalore')").run("insert into table table2 partition(country='us') values " + "('austin')").run("insert into table table2 partition(country='france') values " + "('paris')").dump(primaryDbName);

        // Call load, For the first time, only snapshots would be created and distCp would run from source snapshot to
        // target.
        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("show tables like 'table1'").verifyResult("table1").run("show tables like 'table2'").verifyResult("table2").run("repl status " + replicatedDbName).verifyResult(tuple.lastReplicationId).run("select country from table2 where country = 'us'").verifyResult("us").run("select country from table2 where country = 'france'").verifyResult("france").run("show partitions table2").verifyResults(new String[]{"country" + "=france", "country=india", "country=us"}).run("select * from table1").verifyResults(new String[]{"1", "2"});
        ...
        tuple = primary.run("use " + primaryDbName).run("create external table table3 (id int)").run("insert into table table3 values (10)").run("create external table table4 as select id from table3").dump(primaryDbName);

        // Verify that the table info is written correctly for incremental

        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("show tables like 'table3'").verifyResult("table3").run("select id from table3").verifyResult("10").run("select id from table4").verifyResult("10");
        ...
        tuple = primary.run("use " + primaryDbName).run("drop table table2").run("insert into table1 values (3),(4) ").dump(primaryDbName);

        // Check if the dropped table isn't there and the new data is available.
        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("select * from table1 order by id").verifyResults(new String[]{"1", "2", "3", "4"}).run("show tables like 'table2'").verifyFailure(new String[]{"table2"});
    }
    ...
    public void testBasicStartFromIncrementalReplication() throws Throwable
    {
        ...
        tuple = primary.run("use " + primaryDbName).run("create external table t1 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' ").run("insert into t1 partition(country='india') values ('delhi')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 't1'").verifyResult("t1").run("show partitions t1").verifyResults(new String[]{"country=india"}).run("select place from t1 order by place").verifyResults(new String[]{"delhi"}).verifyReplTargetProperty(replicatedDbName);
        ...
        primary.run("use " + primaryDbName).run("insert into t1 partition(country='india') values ('mumbai')").dump(primaryDbName);

        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("select place from t1 order by place").verifyResults(new String[]{"delhi", "mumbai"}).verifyReplTargetProperty(replicatedDbName);
        ...
        tuple = primary.run("use " + primaryDbName).run("insert into t1 partition(country='india') values ('pune')").run("create external table t2 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' ").run("insert into t2 partition(country='usa') values ('new york')").dump(primaryDbName);

        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("show tables like 't2'").verifyResult("t2").run("show partitions t2").verifyResults(new String[]{"country=usa"}).run("select place from t2 order by place").verifyResults(new String[]{"new york"}).run("select place from t1 order by place").verifyResults(new String[]{"delhi", "mumbai", "pune"}).verifyReplTargetProperty(replicatedDbName);
    ...
    public void testBasicExternalTableWithPartitions() throws Throwable
    {
        ...
        replica.load(replicatedDbName, primaryDbName, withClause).run("show partitions t2").verifyResults(new String[]{"country=australia", "country=india", "country=france"});

        // Test alter partition location.
        String tmpLocation3 = "/tmp1/" + System.nanoTime() + "_3";
        primary.miniDFSCluster.getFileSystem().mkdirs(new Path(tmpLocation2), new FsPermission("777"));
        primary.run("use " + primaryDbName).run("alter table t2 partition (country='australia') set location '" + tmpLocation3 + "'").run("insert into table t2 partition(country='australia') values " + "('sydney')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("select place from t2 where country='australia'").verifyResult("sydney");
    }
    ...
    public void testSnapshotCleanupsOnDatabaseLocationChange() throws Throwable
    {
        ...
        Path externalDatabaseLocationDest = new Path(REPLICA_EXTERNAL_BASE, testName.getMethodName() + "/externalDatabase/");

        Path externalDatabaseLocationAlterDest = new Path(REPLICA_EXTERNAL_BASE, testName.getMethodName() + "/externalDatabaseAlter/");

        List<String> withClause = ReplicationTestUtils.includeExternalTableClause(true);

        // Create a normal and partitioned table in the database location.

        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("alter database " + primaryDbName + " set location '" + externalDatabaseLocation + "'").run("create external table emp1 (id int)").run("insert into emp1 values(1),(2)").run("create external table t2 (place string) partitioned by (country " + "string) row format delimited fields terminated by ','").run("insert into t2 partition(country='india') values ('bangalore')").dump(primaryDbName, withClause);

        // Do a load, post that one snapshot should have been created in the source for each table.

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'emp1'").verifyResults(new String[]{"emp1"}).run("select id from emp1").verifyResults(new String[]{"1", "2"}).run("select place from t2").verifyResults(new String[]{"bangalore"}).verifyReplTargetProperty(replicatedDbName);
        ...
        assertNotNull(fs.getFileStatus(new Path(externalDatabaseLocation, ".snapshot/" + secondSnapshot(primaryDbName.toLowerCase()))));
        ...
        assertNotNull(fs.getFileStatus(new Path(externalDatabaseLocationDest, ".snapshot/" + firstSnapshot(primaryDbName.toLowerCase()))));

        // Alter database location and create another table inside it.

        tuple = primary.run("use " + primaryDbName).run("alter database " + primaryDbName + " set location '" + externalDatabaseLocationAlter + "'").run("create external table empnew (id int)").run("insert into empnew values (3),(4)").run("drop table emp").run("insert into t2 partition(country='france') " + "values('paris')").dump(primaryDbName, withClause);

        // Do a load and see if everything is correct.

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'emp'").verifyFailure(new String[]{"emp"}).run("show tables like 'empnew'").verifyResults(new String[]{"empnew"}).run("select id from empnew").verifyResults(new String[]{"3", "4"}).run("select place from t2 where country='france'").verifyResults(new String[]{"paris"}).run("select place from t2 where country='india'").verifyResults(new String[]{"bangalore"}).verifyReplTargetProperty(replicatedDbName);
        ...
        // Check if the snapshot exists at source.
        assertNotNull(fs.getFileStatus(new Path(externalDatabaseLocationAlter, ".snapshot/" + secondSnapshot(primaryDbName.toLowerCase()))));

        // Check if the old snapshot got deleted at source.
        LambdaTestUtils.intercept(FileNotFoundException.class, () -> fs.getFileStatus(new Path(externalDatabaseLocation, ".snapshot/" + secondSnapshot(primaryDbName.toLowerCase()))));
        ...
        // Check if snapshot exist at destination.
        assertNotNull(fs.getFileStatus(new Path(externalDatabaseLocationAlterDest, ".snapshot/" + firstSnapshot(primaryDbName.toLowerCase()))));

        //Check if the destination old snapshot is deleted.
        LambdaTestUtils.intercept(FileNotFoundException.class, () -> fs.getFileStatus(new Path(externalDatabaseLocationDest, ".snapshot/" + firstSnapshot(primaryDbName.toLowerCase()))));
    }
    ...
    public void testCustomPathTableSnapshots() throws Throwable
    {
        ...
        List<String> withClause = ReplicationTestUtils.includeExternalTableClause(true);
        withClause.add("'hive.repl.external.warehouse.single.copy.task.paths'='" + externalTableLocation2.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "," + externalTablePart1.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "'");

        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table t1 (place string) partitioned by (country" + " string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation1.toString() + "'").run("insert into t1 partition(country='india') values ('bangalore')").run("create external table t2 (place string) partitioned by (country" + " string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation2.toString() + "'").run("insert into t1 partition(country='australia') values " + "('sydney')").run("insert into t2 partition(country='nepal') values " + "('kathmandu')").run("alter table t1 add partition (country='france') location '" + externalTablePart1.toString() + "'").run("insert into t1 partition(country='france') values ('paris')").run("alter table t2 add partition (country='china') location '" + externalTablePart2.toString() + "'").run("insert into t2 partition(country='china') values ('beejing')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 't1'").verifyResults(new String[]{"t1"}).run("show tables like 't2'").verifyResults(new String[]{"t2"}).run("select place from t1 where country='india'").verifyResults(new String[]{"bangalore"}).run("select place from t1 where country='france'").verifyResults(new String[]{"paris"}).run("select place from t2 where country='nepal'").verifyResults(new String[]{"kathmandu"}).run("select place from t2 where country='china'").verifyResults(new String[]{"beejing"}).verifyReplTargetProperty(replicatedDbName);
        ...
        withClause.add("'hive.repl.external.warehouse.single.copy.task.paths'='" + externalTableLocation2.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "," + externalTableLocation1.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "," + externalTablePart1.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "'");

        primary.run("use " + primaryDbName).run("drop table t2").run("create external table t4 (place string) partitioned by (country string) row format delimited fields" + " terminated by ',' location '" + externalTableLocation2.toString() + "'").run("MSCK REPAIR TABLE t4 ADD PARTITIONS").run("insert into t4 partition(country='china') values ('Shanghai')") // new partition in the recreated table.
        ...
        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 't2'").verifyFailure(new String[]{"t1"}).run("select place from t1 where country='india'").verifyResults(new String[]{"bangalore", "chennai"}).run("select place from t1 where country='france'").verifyResults(new String[]{"paris"}).run("select place from t4 where country='nepal'").verifyResults(new String[]{"kathmandu", "pokhra"}) // Old and new entry both should be there
                .run("select place from t4 where country='china'").verifyResults(new String[]{"Shanghai"}) // the newely added partition should be there.
    ...
    public void testCustomPathTableSnapshotsCleanup() throws Throwable
    {
        ...
        List<String> withClause = ReplicationTestUtils.includeExternalTableClause(true);
        withClause.add("'hive.repl.external.warehouse.single.copy.task.paths'='" + externalTableLocation2.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "," + externalTableLocation1.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "'");

        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table t1 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation1.toString() + "'").run("insert into t1 partition(country='india') values ('bangalore')").run("create external table t2 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation2.toString() + "'").run("insert into t2 partition(country='nepal') values ('kathmandu')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 't1'").verifyResults(new String[]{"t1"}).run("show tables like 't2'").verifyResults(new String[]{"t2"}).run("select place from t1 where country='india'").verifyResults(new String[]{"bangalore"}).run("select place from t2 where country='nepal'").verifyResults(new String[]{"kathmandu"}).verifyReplTargetProperty(replicatedDbName);
        ...
        assertNotNull(fs.getFileStatus(new Path(externalTableLocation1, ".snapshot/" + secondSnapshot(primaryDbName.toLowerCase()))));
        ...
        assertNotNull(fs.getFileStatus(new Path(externalTableLocation2, ".snapshot/" + secondSnapshot(primaryDbName.toLowerCase()))));
        ...
        tuple = primary.run("use " + primaryDbName).run("drop table t1").run("alter table t2 SET LOCATION '" + externalTableLocation2New + "'").run("alter table t2 drop partition(country='nepal')").run("insert into t2 partition(country='china') values ('beejing')").dump(primaryDbName, withClause);

        // Load and check if the data is there and the dropped table isn't there.
        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 't1'").verifyFailure(new String[]{"t1"}).run("show tables like 't2'").verifyResults(new String[]{"t2"}).run("select place from t2 where country='china'").verifyResults(new String[]{"beejing"}).verifyReplTargetProperty(replicatedDbName);
    ...
    public void testTargetModified() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table table1 (id int)").run("insert into table table1 values (1)").run("insert into table table1 values (2)").dump(primaryDbName);

        // Call load, For the first time, only snapshots would be created and distCp would run from source snapshot to
        // target.
        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("show tables like 'table1'").verifyResult("table1").run("select * from table1").verifyResults(new String[]{"1", "2"});
        ...
        tuple = primary.run("use " + primaryDbName).run("create external table table3 (id int)").run("insert into table table3 values (10)").run("insert into table table1 values (3)").dump(primaryDbName);

        // Verify that the table info is written correctly for incremental

        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("show tables like 'table3'").verifyResult("table3").run("select id from table3").verifyResult("10").run("select id from table1").verifyResults(new String[]{"1", "2", "3"});
        ...
        Path targetWhPath = externalTableDataPath(conf, REPLICA_EXTERNAL_BASE, new Path(primary.getDatabase(primaryDbName).getLocationUri()));
        ...
        tuple = primary.run("use " + primaryDbName).run("insert into table table3 values (11)").run("insert into table table1 values (4)").dump(primaryDbName);

        // Load and see the data is there.

        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("select id from table3").verifyResults(new String[]{"10", "11"}).run("select id from table1").verifyResults(new String[]{"1", "2", "3", "4"});
        ...
        tuple = primary.run("use " + primaryDbName).run("insert into table table1 values (5)").dump(primaryDbName);

        // Load and see the new data is there.
        replica.load(replicatedDbName, primaryDbName).run("use " + replicatedDbName).run("select id from table1").verifyResults(new String[]{"1", "2", "3", "4", "5"});
    ...
    public void testPurgeAndReBootstrap() throws Throwable
    {
        ...
        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table table1 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation1.toString() + "'").run("ALTER TABLE table1 SET TBLPROPERTIES ('external.table.purge'='true')").run("create external table table2 (id int)").run("create external table table3 (id int)").run("ALTER TABLE table3 SET TBLPROPERTIES ('external.table.purge'='true')").run("insert into table1 partition(country='nepal') values ('kathmandu')").run("insert into table1 partition(country='china') values ('beejing')").run("insert into table2 values(1)").run("insert into table3 values(5)").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'table1'").verifyResults(new String[]{"table1"}).run("select place from table1 where country='nepal'").verifyResults(new String[]{"kathmandu"}).run("select place from table1 where country='china'").verifyResults(new String[]{"beejing"}).verifyReplTargetProperty(replicatedDbName);

        // Check if the t2 directory is snapshotoble and the snapshot is there.
        validateInitialSnapshotsCreated(externalTableLocation1.toString());

        // Add some more data and do a dump & load
        primary.run("use " + primaryDbName).run("insert into table1 partition(country='china') values ('wuhan')").run("insert into table2 values(2)").run("insert into table3 values(6)").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("select place from table1 where country='china'").verifyResults(new String[]{"beejing", "wuhan"}).verifyReplTargetProperty(replicatedDbName);
        ...
        // Now purge the table
        primary.run("use " + primaryDbName).run("drop table table1 purge").run("insert into table2 values(3)").run("insert into table3 values(7)").dump(primaryDbName, withClause);

        // Check if target dir exists.
        assertFalse(fs.exists(externalTableLocation1));

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'table1'").verifyFailure(new String[]{"table1"}).run("select id from table2").verifyResults(new String[]{"1", "2", "3"}).run("select id from table3").verifyResults(new String[]{"5", "6", "7"}).verifyReplTargetProperty(replicatedDbName);
        ...
        replica.run("use " + replicatedDbName).run("drop table table2").run("drop table table3").run("drop database " + replicatedDbName + " cascade");
        ...
        Path dbLocation = new Path(REPLICA_EXTERNAL_BASE, Path.getPathWithoutSchemeAndAuthority(new Path(primary.getDatabase(primaryDbName).getLocationUri())).toString().replaceFirst("/", ""));
    ...
    public void testSnapshotsWithCustomDbLevelPaths() throws Throwable
    {
        ...
        // single copy task.
        List<String> withClause = ReplicationTestUtils.includeExternalTableClause(true);
        withClause.add("'" + REPL_EXTERNAL_WAREHOUSE_SINGLE_COPY_TASK_PATHS.varname + "'='" + externalCustomDb1Path.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "," + externalTableLocation3.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "'");

        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table table1 (id int)").run("create external table table2 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation2.toString() + "'").run("create external table table3 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation3.toString() + "'").run("create external table table4 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation4.toString() + "'").run("create external table table5 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation5.toString() + "'").run("create external table table6 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation6.toString() + "'").run("create external table table7 (place string) partitioned by (country string) row format " + "delimited fields terminated by ',' location '" + externalTableLocation7.toString() + "'").run("insert into table2 partition(country='india') values ('bangalore')").run("insert into table3 partition(country='china') values ('beejing')").run("insert into table4 partition(country='usa') values ('new york')").run("insert into table5 partition(country='japan') values ('tokyo')").run("insert into table6 partition(country='nepal') values ('kathmandu')").run("insert into table7 partition(country='australia') values ('sydney')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("select place from table2 where country='india'").verifyResults(new String[]{"bangalore"}).run("select place from table3 where country='china'").verifyResults(new String[]{"beejing"}).run("select place from table4 where country='usa'").verifyResults(new String[]{"new york"}).run("select place from table5 where country='japan'").verifyResults(new String[]{"tokyo"}).run("select place from table6 where country='nepal'").verifyResults(new String[]{"kathmandu"}).run("select place from table7 where country='australia'").verifyResults(new String[]{"sydney"}).verifyReplTargetProperty(replicatedDbName);
        ...
        tuple = primary.run("use " + primaryDbName).run("insert into table2 partition(country='india') values ('chennai')").run("insert into table3 partition(country='china') values ('chengdu')").run("insert into table4 partition(country='usa') values ('washington')").run("select place from table4 where country='usa'").verifyResults(new String[]{"new york", "washington"}).run("insert into table5 partition(country='japan') values ('osaka')").run("insert into table6 partition(country='nepal') values ('pokhra')").run("insert into table7 partition(country='australia') values ('perth')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'table%'").verifyResults(new String[]{"table1", "table2", "table3", "table4", "table5", "table6", "table7"}).run("select place from table2 where country='india'").verifyResults(new String[]{"bangalore", "chennai"}).run("select place from table3 where country='china'").verifyResults(new String[]{"beejing", "chengdu"}).run("select place from table4 where country='usa'").verifyResults(new String[]{"new york", "washington"}).run("select place from table5 where country='japan'").verifyResults(new String[]{"tokyo", "osaka"}).run("select place from table6 where country='nepal'").verifyResults(new String[]{"kathmandu", "pokhra"}).run("select place from table7 where country='australia'").verifyResults(new String[]{"sydney", "perth"}).verifyReplTargetProperty(replicatedDbName);
        ...
        tuple = primary.run("use " + primaryDbName).run("insert into table4 partition(country='usa') values ('chicago')").run("drop table table5 purge").run("insert into table6 partition(country='nepal') values ('lalitpur')").run("insert into table7 partition(country='australia') values ('adelaide')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'table%'").verifyResults(new String[]{"table1", "table2", "table3", "table4", "table6", "table7"}).run("select place from table4 where country='usa'").verifyResults(new String[]{"new york", "washington", "chicago"}).run("select place from table6 where country='nepal'").verifyResults(new String[]{"kathmandu", "pokhra", "lalitpur"}).run("select place from table7 where country='australia'").verifyResults(new String[]{"sydney", "perth", "adelaide"}).verifyReplTargetProperty(replicatedDbName);
        ...
        withClause.add("'" + REPL_EXTERNAL_WAREHOUSE_SINGLE_COPY_TASK_PATHS.varname + "'='" + externalCustomDb2Path.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "," + externalTableLocation3.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "'");

        primary.run("use " + primaryDbName).run("insert into table4 partition(country='usa') values ('austin')").run("insert into table6 partition(country='nepal') values ('janakpur')").run("insert into table7 partition(country='australia') values ('darwin')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("show tables like 'table%'").verifyResults(new String[]{"table1", "table2", "table3", "table4", "table6", "table7"}).run("select place from table4 where country='usa'").verifyResults(new String[]{"new york", "washington", "chicago", "austin"}).run("select place from table6 where country='nepal'").verifyResults(new String[]{"kathmandu", "pokhra", "lalitpur", "janakpur"}).run("select place from table7 where country='australia'").verifyResults(new String[]{"sydney", "perth", "adelaide", "darwin"}).verifyReplTargetProperty(replicatedDbName);
    ...
    public void testSnapshotsWithFiltersCustomDbLevelPaths() throws Throwable
    {
        ...
        List<String> withClause = ReplicationTestUtils.includeExternalTableClause(true);
        withClause.add("'" + REPL_EXTERNAL_WAREHOUSE_SINGLE_COPY_TASK_PATHS.varname + "'='" + project.makeQualified(fs.getUri(), fs.getWorkingDirectory()).toString() + "'");

        // Add Filter file
        withClause.add("'distcp.options.filters'='" + filterFilePath + "'");

        WarehouseInstance.Tuple tuple = primary.run("use " + primaryDbName).run("create external table table1 (place string)  row format " + "delimited fields terminated by ',' location '" + table1.toString() + "'").run("create external table table2 (place string)  row format " + "delimited fields terminated by ',' location '" + table2.toString() + "'").run("create external table table3 (place string)  row format " + "delimited fields terminated by ',' location '" + table3.toString() + "'").run("create external table table4 (place string)  row format " + "delimited fields terminated by ',' location '" + table4.toString() + "'").run("insert into table1 values ('bangalore')").run("select place from table1").verifyResults(new String[]{"bangalore"}).run("insert into table2 values ('beejing')").run("insert into table3 values ('new york')").run("insert into table4 values ('tokyo')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("select place from table1").verifyResults(new String[]{"bangalore"}).run("select place from table2").verifyResults(new String[]{"beejing"}).run("select place from table3").verifyResults(new String[]{"new york"}).run("select place from table4").verifyResults(new String[]{"tokyo"}).verifyReplTargetProperty(replicatedDbName);
        ...
        tuple = primary.run("use " + primaryDbName).run("insert into table1 values ('delhi')").run("insert into table2 values ('wuhan')").run("insert into table3 values ('washington')").run("insert into table4 values ('osaka')").dump(primaryDbName, withClause);

        replica.load(replicatedDbName, primaryDbName, withClause).run("use " + replicatedDbName).run("select place from table1").verifyResults(new String[]{"bangalore", "delhi"}).run("select place from table2").verifyResults(new String[]{"beejing", "wuhan"}).run("select place from table3").verifyResults(new String[]{"new york", "washington"}).run("select place from table4").verifyResults(new String[]{"tokyo", "osaka"}).verifyReplTargetProperty(replicatedDbName);
    ...
    private void validateInitialSnapshotsCreated(String location) throws Exception
    {
        ...
        assertTrue("Snapshot not enabled for the target location", dfsTarget.getFileStatus(locationPathTarget).isSnapshotEnabled());

        // Check whether the snapshot got created in the target location.
        assertNotNull(dfsTarget.getFileStatus(new Path(locationPathTarget, ".snapshot" + "/" + firstSnapshot(primaryDbName.toLowerCase()))));
    }
