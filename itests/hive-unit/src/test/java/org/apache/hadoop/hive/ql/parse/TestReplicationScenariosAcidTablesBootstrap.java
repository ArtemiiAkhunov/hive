 ...
 */
public class TestReplicationScenariosAcidTablesBootstrap extends BaseReplicationScenariosAcidTables
{

    private static List<String> dumpWithoutAcidClause = Collections.singletonList("'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='false'");
    private static List<String> dumpWithAcidBootstrapClause = Arrays.asList("'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'", "'" + HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES + "'='true'");
    private static List<String> dumpWithAcidClause = Arrays.asList("'" + ReplUtils.REPL_DUMP_INCLUDE_ACID_TABLES + "'='true'");
    ...
    public void testAcidTablesBootstrapDuringIncremental() throws Throwable
    {
        ...
        WarehouseInstance.Tuple incrementalDump = primary.run("use " + primaryDbName).dump(primaryDbName, dumpWithAcidBootstrapClause);
        replica.load(replicatedDbName, primaryDbName);
        ...
        LOG.info(testName.getMethodName() + ": second incremental dump and load dump after incremental with acid table " + "bootstrap.");
    ...
    public void retryIncBootstrapAcidFromDifferentDumpWithoutCleanTablesConfig() throws Throwable
    {
        ...
        replica.loadFailure(replicatedDbName, primaryDbName, Collections.emptyList(), ErrorMsg.REPL_BOOTSTRAP_LOAD_PATH_NOT_VALID.getErrorCode());
    }
    ...
    public void testBootstrapAcidTablesDuringIncrementalWithConcurrentWrites() throws Throwable
    {
        ...
        BehaviourInjection<CurrentNotificationEventId, CurrentNotificationEventId> callerInjectedBehavior = new BehaviourInjection<CurrentNotificationEventId, CurrentNotificationEventId>()
        {
            @Nullable
            @Override
        ...
        LOG.info(testName.getMethodName() + ": loading incremental dump containing bootstrapped ACID tables.");
        replica.load(replicatedDbName, primaryDbName);
        verifyIncLoad(replicatedDbName, incDump.lastReplicationId);

        // Next Incremental should include the concurrent writes
        LOG.info(testName.getMethodName() + ": dumping second normal incremental dump from event id = " + incDump.lastReplicationId);
        WarehouseInstance.Tuple inc2Dump = primary.dump(primaryDbName);
        LOG.info(testName.getMethodName() + ": loading second normal incremental dump from event id = " + incDump.lastReplicationId);
        replica.load(replicatedDbName, primaryDbName);
        verifyInc2Load(replicatedDbName, inc2Dump.lastReplicationId);
    }
}
