...
public class TestHiveServer2Acid
{
    ...
    public void testCancelOperation() throws Exception
    {
        ...
        OperationHandle opHandle = serviceClient.executeStatementAsync(sessHandle, "select sleepMsUDF(id, 1000), id from " + tableName, confOverlay);
        serviceClient.cancelOperation(opHandle);

        assertOperationWasCancelled(serviceClient, opHandle);
        Assert.assertEquals(0, TestTxnDbUtil.countQueryAgent(miniHS2.getHiveConf(), "select count(*) from HIVE_LOCKS"));
        Assert.assertEquals(1, TestTxnDbUtil.countQueryAgent(miniHS2.getHiveConf(), "select 1 from TXNS where TXN_ID = (select max(TXN_ID) from TXNS) and TXN_STATE='a'"));
    ...
    private void assertOperationFinished(CLIServiceClient serviceClient, OperationHandle opHandle) throws HiveSQLException, InterruptedException
    {
        assertOperationStatus(serviceClient, opHandle, OperationState.FINISHED);
    }

    private void assertOperationWasCancelled(CLIServiceClient serviceClient, OperationHandle opHandle) throws HiveSQLException, InterruptedException
    {
        assertOperationStatus(serviceClient, opHandle, OperationState.CANCELED);
    }

    private void assertOperationStatus(CLIServiceClient serviceClient, OperationHandle opHandle, OperationState opState) throws InterruptedException, HiveSQLException
    {
