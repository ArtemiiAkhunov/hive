...
public class TestJdbcWithMiniLlapVectorArrowBatch extends BaseJdbcWithMiniLlap
{
    ...
    private final MultiSet<List<Object>> convertedLegacyTimestampExpectedOutput = getConvertedLegacyTimestampExpectedOutput();
    ...
    @Test
    public void testOrcHybridMixedDates() throws Exception
    {

        final String tableName = "testOrcHybridMixedDates";
        executeSQL("create table " + tableName + " (d date) stored as orc");
        executeSQL("INSERT INTO " + tableName + " VALUES " + "('2012-02-21'), " + "('2014-02-11'), " + "('1947-02-11'), " + "('8200-02-11'), " + "('1012-02-21'), " + "('1014-02-11'), " + "('0947-02-11'), " + "('0200-02-11')");

        final String query = "select * from " + tableName;

        testDateQueries(query, "orc.proleptic.gregorian.default", hybridMixedDateExpectedOutput, hybridMixedDateExpectedOutput);
    }

    // test with legacy orc files
    // similar to ql/src/test/queries/clientpositive/orc_legacy_mixed_date.q
    @Test
    public void testOrcLegacyMixedDates() throws Exception
    {
    ...
    @Test
    public void testOrcHybridMixedTimestamps() throws Exception
    {

        final String tableName = "testOrcHybridMixedTimestamps";
        executeSQL("create table " + tableName + " (d timestamp) stored as orc");
        executeSQL("INSERT INTO " + tableName + " VALUES ('2012-02-21 07:08:09.123')," + "('2014-02-11 07:08:09.123')," + "('1947-02-11 07:08:09.123')," + "('8200-02-11 07:08:09.123')," + "('1012-02-21 07:08:09.123')," + "('1014-02-11 07:08:09.123')," + "('0947-02-11 07:08:09.123')," + "('0200-02-11 07:08:09.123')");

        final String query = "select * from " + tableName;

        testTimestampQueries(query, "orc.proleptic.gregorian.default", hybridMixedTimestampExpectedOutput, hybridMixedTimestampExpectedOutput);
    }

    // test with legacy parquet files
    // similar to ql/src/test/queries/clientpositive/orc_legacy_mixed_timestamp.q
    @Test
    public void testOrcLegacyMixedTimestamps() throws Exception
    {
        ...
        // ORC properties (here orc.proleptic.gregorian.default) are not propogated to LLAP as of now
        testTimestampQueries(query, "orc.proleptic.gregorian.default", legacyTimestampExpectedOutput, legacyTimestampExpectedOutput);
    }

    // test with new parquet files
    // similar to ql/src/test/queries/clientpositive/parquet_hybrid_mixed_date.q
    @Test
    public void testParquetHybridMixedDates() throws Exception
    {
        ...
        testDateQueries(query, HIVE_PARQUET_DATE_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedDateExpectedOutput, hybridMixedDateExpectedOutput);
    }

    // test with legacy parquet files
    // similar to ql/src/test/queries/clientpositive/parquet_legacy_mixed_date.q
    @Test
    public void testParquetLegacyMixedDates() throws Exception
    {
        ...
        testDateQueries(query, HIVE_PARQUET_DATE_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedDateExpectedOutput, legacyDateExpectedOutput);
    }

    // test newly inserted parquet records.
    // similar to ql/src/test/queries/clientpositive/parquet_hybrid_mixed_timestamp.q
    @Test
    public void testParquetHybridMixedTimestamps() throws Exception
    {
        ...
        testTimestampQueries(query, HIVE_PARQUET_DATE_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedTimestampExpectedOutput, hybridMixedTimestampExpectedOutput);
    }

    // test with legacy parquet files
    // similar to ql/src/test/queries/clientpositive/parquet_legacy_mixed_timestamp.q
    @Test
    public void testParquetLegacyMixedTimestamps() throws Exception
    {

        final String tableName = "testParquetLegacyMixedTimestamps";
        executeSQL("create table " + tableName + " (d timestamp) stored as parquet");
        executeSQL("load data local inpath '../../data/files/parquet_legacy_mixed_timestamps.parq' into table " + tableName);
    ...
    @Test
    public void testAvroHybridMixedDates() throws Exception
    {
        ...
        testDateQueries(query, HIVE_AVRO_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedDateExpectedOutput, hybridMixedDateExpectedOutput);
    }

    // test with legacy parquet files
    // similar to ql/src/test/queries/clientpositive/avro_legacy_mixed_date.q
    @Test
    public void testAvroLegacyMixedDates() throws Exception
    {

        final String tableName = "testAvroLegacyMixedDates";

        executeSQL("create table " + tableName + " (d date) ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' " + "COLLECTION ITEMS TERMINATED BY ',' MAP KEYS TERMINATED BY ':' stored as avro");
        executeSQL("load data local inpath '../../data/files/avro_legacy_mixed_dates.avro' into table " + tableName);

        final String query = "select * from " + tableName;

        testDateQueries(query, HIVE_AVRO_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedDateExpectedOutput, legacyDateExpectedOutput);
    }

    // test newly inserted avro records
    // similar to ql/src/test/queries/clientpositive/avro_hybrid_mixed_timestamp.q
    @Test
    public void testAvroHybridMixedTimestamps() throws Exception
    {

        final String tableName = "testAvroHybridMixedTimestamps";
        executeSQL("create table " + tableName + " (d timestamp) ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' " + "COLLECTION ITEMS TERMINATED BY ',' MAP KEYS TERMINATED BY ':' stored as avro");
        executeSQL("INSERT INTO " + tableName + " VALUES ('2012-02-21 07:08:09.123')," + "('2014-02-11 07:08:09.123')," + "('1947-02-11 07:08:09.123')," + "('8200-02-11 07:08:09.123')," + "('1012-02-21 07:08:09.123')," + "('1014-02-11 07:08:09.123')," + "('0947-02-11 07:08:09.123')," + "('0200-02-11 07:08:09.123')");

        final String query = "select * from " + tableName;

        testTimestampQueries(query, HIVE_AVRO_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedTimestampExpectedOutput, hybridMixedTimestampExpectedOutput);
    }

    // test with legacy avro files
    // similar to ql/src/test/queries/clientpositive/avro_legacy_mixed_timestamp.q
    @Test
    public void testAvroLegacyMixedTimestamps() throws Exception
    {
        ...
        testTimestampQueries(query, HIVE_AVRO_PROLEPTIC_GREGORIAN_DEFAULT.toString(), hybridMixedTimestampExpectedOutput, convertedLegacyTimestampExpectedOutput);
    }

    private void testDateQueries(final String query, final String fileReaderLevelCalendarProp, MultiSet<List<Object>> newHybridExpected, MultiSet<List<Object>> legacyExpected) throws Exception
    {

        MultiSet<List<Object>> llapResultTransformed = transformLlapDateResultSet(runQueryUsingLlapArrowBatchReader(query, ImmutableMap.of(LLAP_EXTERNAL_CLIENT_USE_HYBRID_CALENDAR.toString(), "true")), false);
        assertEquals(newHybridExpected, llapResultTransformed);

        // test hybrid output with fileReaderLevelCalendarProp
        llapResultTransformed = transformLlapDateResultSet(runQueryUsingLlapArrowBatchReader(query, ImmutableMap.of(LLAP_EXTERNAL_CLIENT_USE_HYBRID_CALENDAR.toString(), "true", fileReaderLevelCalendarProp, "true")), false);
        assertEquals(legacyExpected, llapResultTransformed);
    }

    private void testTimestampQueries(final String query, final String fileReaderLevelCalendarProp, MultiSet<List<Object>> newHybridExpected, MultiSet<List<Object>> legacyExpected) throws Exception
    {

        MultiSet<List<Object>> llapResultTransformed = transformLlapTimestampResultSet(runQueryUsingLlapArrowBatchReader(query, ImmutableMap.of(LLAP_EXTERNAL_CLIENT_USE_HYBRID_CALENDAR.toString(), "true")), false);
        assertEquals(newHybridExpected, llapResultTransformed);

        // test hybrid output with fileReaderLevelCalendarProp
        llapResultTransformed = transformLlapTimestampResultSet(runQueryUsingLlapArrowBatchReader(query, ImmutableMap.of(LLAP_EXTERNAL_CLIENT_USE_HYBRID_CALENDAR.toString(), "true", fileReaderLevelCalendarProp, "true")), false);
        assertEquals(legacyExpected, llapResultTransformed);
    }

    private MultiSet<List<Object>> transformLlapDateResultSet(MultiSet<List<Object>> llapResult, final boolean useProleptic)
    {
        MultiSet<List<Object>> llapResultTransformed = new HashMultiSet<>();
        for (List<Object> list : llapResult)
        {
            llapResultTransformed.add(list.stream().map(ele -> CalendarUtils.formatDate((int) ele, useProleptic)).collect(Collectors.toList()));
        }
        return llapResultTransformed;
    }

    private MultiSet<List<Object>> transformLlapTimestampResultSet(MultiSet<List<Object>> llapResult, final boolean useProleptic)
    {
        MultiSet<List<Object>> llapResultTransformed = new HashMultiSet<>();
        for (List<Object> list : llapResult)
        {
            llapResultTransformed.add(list.stream().map(ele -> CalendarUtils.formatTimestamp((long) ele / 1000, useProleptic)).collect(Collectors.toList()));
        }
        return llapResultTransformed;
    }
