...
public class TestWMMetricsWithTrigger
{
    ...
    static HiveConf defaultConf() throws Exception
    {
        ...
        defaultConf.setVar(HiveConf.ConfVars.HIVE_AUTHENTICATOR_MANAGER, "org.apache.hadoop.hive.ql.security.SessionStateUserAuthenticator");
    ...
    public static void beforeTest() throws Exception
    {
        ...
        Connection conDefault = BaseJdbcWithMiniLlap.getConnection(miniHS2.getJdbcURL(), System.getProperty("user.name"), "bar");
    ...
    void runQueryWithTrigger(int queryTimeoutSecs) throws Exception
    {
        ...
        String query = "select sleep(t1.int_col + t2.int_col, 500), t1.value from " + tableName + " t1 join " + tableName + " t2 on t1.int_col>=t2.int_col";
        long start = System.currentTimeMillis();
        Connection conn = BaseJdbcWithMiniLlap.getConnection(miniHS2.getJdbcURL(testDbName), System.getProperty("user.name"), "bar");
        ...
        assertTrue("Query was killed due to " + throwable.getMessage() + " and not because of trigger violation", throwable.getMessage().contains("violated"));
    ...
    private static void verifyMetrics(int numExecutors, int numExecutorsMax, int numParallelQueries, int numRunningQueries) throws Exception
    {
