...
public class TestSchemaTool
{
    ...
    {
        String testScript[] = {"-- this is a comment", "DROP TABLE IF EXISTS fooTab;", "/*!1234 this is comment code like mysql */;", "CREATE TABLE fooTab(id INTEGER);", "DROP TABLE footab;", "-- ending comment"};
        String resultScript[] = {"DROP TABLE IF EXISTS fooTab", "/*!1234 this is comment code like mysql */", "CREATE TABLE fooTab(id INTEGER)", "DROP TABLE footab",};
        ...
        String flattenedSql = HiveSchemaHelper.getDbCommandParser("derby", false).buildCommand(testScriptFile.getParentFile().getPath(), testScriptFile.getName());
    ...
    public void testNestedScriptsForDerby() throws Exception
    {
        ...
        String parentTestScript[] = {" -- this is a comment", "DROP TABLE IF EXISTS " + parentTab + ";", " -- this is another comment ", "CREATE TABLE " + parentTab + "(id INTEGER);", "RUN '" + generateTestScript(childTestScript1).getName() + "';", "DROP TABLE " + parentTab + ";", "RUN '" + generateTestScript(childTestScript2).getName() + "';", "--ending comment ",};
    ...
    public void testNestedScriptsForMySQL() throws Exception
    {
        ...
        File testScriptFile = generateTestScript(parentTestScript);
        String flattenedSql = HiveSchemaHelper.getDbCommandParser("mysql", false).buildCommand(testScriptFile.getParentFile().getPath(), testScriptFile.getName());
        Assert.assertFalse(flattenedSql.contains("RUN"));
        Assert.assertFalse(flattenedSql.contains("comment"));
        Assert.assertTrue(flattenedSql.contains(childTab1));
        Assert.assertTrue(flattenedSql.contains(childTab2));
    ...
    public void testNestedScriptsForOracle() throws Exception
    {
        ...
        File testScriptFile = generateTestScript(parentTestScript);
        String flattenedSql = HiveSchemaHelper.getDbCommandParser("oracle", false).buildCommand(testScriptFile.getParentFile().getPath(), testScriptFile.getName());
