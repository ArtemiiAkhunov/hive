...
public class TestJdbcWithMiniHS2
{
    ...
    public static void setupBeforeClass() throws Exception
    {
        ...
        stmt.execute("create database " + testDbName + " WITH DBPROPERTIES ( '" + SOURCE_OF_REPLICATION + "' = '1,2,3')");
        stmt.close();
    ...
    private static Connection getConnection(String jdbcURL, String user, String pwd) throws SQLException
    {
    ...
    private static void createTestTables(Connection conn, String dbName) throws SQLException
    {
        ...
        // create a table
        stmt.execute("create table " + tableName + " (int_col int comment 'the int column', value string) comment '" + tableComment + "'");
        // load data
    ...
    private static void startMiniHS2(HiveConf conf, boolean httpMode) throws Exception
    {
        ...
        conf.setVar(ConfVars.POST_EXEC_HOOKS, ReadableHook.class.getName() + "," + LineageLogger.class.getName());
    ...
    private static List<Future<Boolean>> startTasks(ExecutorService workers, Connection conn, String tableName, int numTasks)
    {
    ...
    private void verifyConfProperty(Statement stmt, String property, String expectedValue) throws Exception
    {
    ...
    public void testSessionScratchDirs() throws Exception
    {
        ...
        FsPermission expectedFSPermission = new FsPermission(HiveConf.getVar(conf, HiveConf.ConfVars.SCRATCH_DIR_PERMISSION));
    ...
    private void verifyScratchDir(HiveConf conf, FileSystem fs, Path scratchDirPath, FsPermission expectedFSPermission, String userName, boolean isLocal) throws Exception
    {
    ...
    public void testHttpHeaderSize() throws Exception
    {
        ...
        } finally
        {
            ...
            assertTrue("Incorrect HTTP Response:" + headerException.getMessage(), headerException.getMessage().contains("HTTP Response code: 431"));
        }
    ...
    public void testPermFunc() throws Exception
    {
        ...
        String createSameFunctionAgain = "CREATE FUNCTION example_add AS '" + testUdfClassName + "' USING JAR '" + jarFilePath + "'";
        try
        {
    ...
    public void testReplDumpResultSet() throws Exception
    {
        ...
        Path testPath = new Path(testPathName + Path.SEPARATOR + Base64.getEncoder().encodeToString(testDbName.toLowerCase().getBytes(StandardCharsets.UTF_8)));
    ...
    private void checkVertices()
    {
        ...
        {
            assertFalse("Each insert affects a column so should be some vertices", vertices.isEmpty());
            assertEquals("Each insert affects one column so should be one vertex", 1, vertices.size());
            ...
            assertTrue("did not see one of the 2 expected column names", label.equals("testjdbcminihs2.testconcurrentlineage1.col1") || label.equals("testjdbcminihs2.testconcurrentlineage2.col2"));
        }
    }
    ...
    public void testDescribe() throws Exception
    {
        try (Statement stmt = conTestDb.createStatement())
        {
            ...
            assertTrue("description appears truncated: " + extendedDescription, extendedDescription.endsWith(")"));
            assertTrue("bad line delimiter: " + extendedDescription, extendedDescription.contains("line.delim=\\n"));
            assertTrue("bad field delimiter: " + extendedDescription, extendedDescription.contains("field.delim=\\t"));
            ...
            assertTrue("bad view text: " + extendedViewDescription, extendedViewDescription.contains("viewOriginalText:select * from " + table));
            assertTrue("bad expanded view text: " + extendedViewDescription, extendedViewDescription.contains("viewExpandedText:select `testdescribe`.`orderid`, `testdescribe`.`orderdate`, " + "`testdescribe`.`customerid` from `testjdbcminihs2`"));
        }
    }
    ...
    static String getDetailedTableDescription(Statement stmt, String table) throws SQLException
    {
        ...
        try (ResultSet rs = stmt.executeQuery("describe extended " + table))
        {
            while (rs.next())
            {
                ...
                if ("Detailed Table Information".equals(out))
