...
public class TestQueryDisplay
{
    ...
    public void checkWebuiShowGraph() throws Exception
    {
        ...
        session.getSessionConf().setIntVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_MAX_GRAPH_SIZE, 0);
        testGraphDDL(session, true);
        session.getSessionConf().setIntVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_MAX_GRAPH_SIZE, 40);
    ...
    private void testGraphDDL(HiveSession session, boolean exceedMaxGraphSize) throws Exception
    {
        ...
        verifyDDLHtml("Query information not available.", opHandleGraph.getHandleIdentifier().toString(), exceedMaxGraphSize);
        verifyDDLHtml("STAGE DEPENDENCIES", opHandleGraph.getHandleIdentifier().toString(), !exceedMaxGraphSize);
    ...
    private void verifyDDL(QueryInfo queryInfo, String stmt, String handle, boolean finished)
    {
        ...
        Assert.assertFalse(qDisplay1.getPerfLogStarts(QueryDisplay.Phase.COMPILATION).containsKey(PerfLogger.DRIVER_EXECUTE));
