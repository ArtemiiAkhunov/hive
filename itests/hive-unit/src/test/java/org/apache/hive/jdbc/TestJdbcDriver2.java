...
public class TestJdbcDriver2
{
    ...
    private static void createTestTables(Statement stmt, String testDbName) throws SQLException
    {
        ...
        stmt.execute("create table " + tableNameWithPk + " (a STRING, b STRING, primary key (a) disable) ");
        // load data
    ...
    public void testExceucteUpdateCounts() throws Exception
    {
        ...
        stmt.execute("set " + ConfVars.HIVE_TXN_MANAGER.varname + "=org.apache.hadoop.hive.ql.lockmgr.DbTxnManager");
        ...
        stmt.close();
        PreparedStatement pStmt = con.prepareStatement("update transactional_crud set b = ? where a = ? or a = ?");
        pStmt.setInt(1, 15);
        pStmt.setInt(2, 1);
        pStmt.setInt(3, 3);
    ...
    private void testExceucteMergeCounts(boolean splitUpdateEarly) throws Exception
    {
        ...
        stmt.execute("create table source (a int, b int) stored as orc " + "tblproperties('transactional'='true', 'transactional_properties'='default')");
    ...
    public void testDataTypes2() throws Exception
    {
        ...
        ResultSet res = stmt.executeQuery("select c5, c1 from " + dataTypeTableName);
    ...
    public void testErrorMessages() throws SQLException
    {
        ...
        doTestErrorCase("create table " + tableName + " (key int, value string)", "FAILED: Execution Error, return code 40000 from org.apache.hadoop.hive.ql.ddl.DDLTask", "08S01", 40000);
    }
    ...
    public void testShowTablesInDb() throws SQLException
    {
        ...
        stmt.execute("create table " + fullTestTableName + " (under_col int comment 'the under column', value string) comment '" + tableComment + "'");
    ...
    public void testMetaDataGetTablesHive() throws SQLException
    {
        ...
        getTablesTest(ImmutableSet.of(TableType.MANAGED_TABLE.toString(), TableType.EXTERNAL_TABLE.toString()), TableType.VIRTUAL_VIEW.toString());
    }
    ...
    public void testMetaDataGetExternalTables() throws SQLException
    {
        ...
        stmt.execute("set " + HiveConf.ConfVars.HIVE_SERVER2_TABLE_TYPE_MAPPING.varname + " = " + TableTypeMappings.HIVE.toString());
        stmt.close();
        ResultSet rs = con.getMetaData().getTables(testDbName, null, null, new String[]{TableType.EXTERNAL_TABLE.toString()});
    ...
    {
        Statement stmt = con.createStatement();
        ResultSet res = stmt.executeQuery("select * from " + dataTypeTableName + " limit 1");
    ...
    private static final String[][] URL_PROPERTIES = new String[][]{
            ...
            {"jdbc:hive2://", "", "", "default"}, {"jdbc:hive2://localhost:10001/default", "localhost", "10001", "default"}, {"jdbc:hive2://localhost/notdefault", "localhost", "10000", "notdefault"}, {"jdbc:hive2://foo:1243", "foo", "1243", "default"},

            // http mode
            {"jdbc:hive2://server:10002/db;user=foo;password=bar?" + "hive.server2.transport.mode=http;" + "hive.server2.thrift.http.path=hs2", "server", "10002", "db"},};
    ...
    private static final String[][] HTTP_URL_PROPERTIES = new String[][]{{"jdbc:hive2://server:10002/db;user=foo;password=bar;transportMode=http;httpPath=hs2", "server", "10002", "db", "http", "hs2"}, {"jdbc:hive2://server:10000/testdb;user=foo;password=bar;transportMode=binary;httpPath=", "server", "10000", "testdb", "binary", ""},};
    ...
    public void testSetCommand() throws SQLException
    {
        ...
        while (res.next())
        {
            ...
            assertFalse("set output must not contain hidden variables such as the metastore password:" + rline, rline.contains(HiveConf.ConfVars.METASTORE_PWD.varname) && !(rline.contains(HiveConf.ConfVars.HIVE_CONF_HIDDEN_LIST.varname)));
    ...
    public void testGetQueryLogForReplCommands() throws Exception
    {
        ...
        try
        {
            ...
            String[] expectedBootstrapDumpLogs = {"REPL::START", "REPL::TABLE_DUMP", "REPL::END"};

            // Bootstrap dump
            stmt = (HiveStatement) con.createStatement();
            advanceDumpDir();
            ResultSet replDumpRslt = stmt.executeQuery("repl dump " + primaryDb + " with ('hive.repl.rootdir' = '" + replDir + "')");
            ...
            String[] expectedBootstrapLoadLogs = {"REPL::START", "REPL::TABLE_LOAD", "REPL::END"};

            // Bootstrap load
            stmt = (HiveStatement) con.createStatement();
            stmt.execute("repl load " + primaryDb + " into " + replicaDb + " with ('hive.repl.rootdir' = '" + replDir + "')");
            ...
            String[] expectedIncrementalDumpLogs = {"REPL::START", "REPL::EVENT_DUMP", "REPL::END"};

            // Incremental dump
            stmt = (HiveStatement) con.createStatement();
            advanceDumpDir();
            replDumpRslt = stmt.executeQuery("repl dump " + primaryDb + " with ('hive.repl.rootdir' = '" + replDir + "')");
            assertTrue(replDumpRslt.next());
            logs = stmt.getQueryLog(false, 10000);
            stmt.close();
            ...
            String[] expectedIncrementalLoadLogs = {"REPL::START", "REPL::EVENT_LOAD", "REPL::END"};
    ...
    public void testResultNextAcidTable() throws Exception
    {
        ...
        } finally
        {
            ...
            stmt.execute("set " + ConfVars.HIVE_TXN_MANAGER.varname + "=org.apache.hadoop.hive.ql.lockmgr.DummyTxnManager");
            stmt.close();
        }
    }
    ...
    public void testHeaderFooterNonTextFiles() throws Exception
    {
        ...
        try
        {
            ...
            stmt.execute("CREATE EXTERNAL TABLE parquet_emp (id int) STORED AS PARQUET " + "TBLPROPERTIES ('skip.header.line.count'='1')");
            ...
            stmt.execute("CREATE EXTERNAL TABLE parquetf_emp (id int) STORED AS PARQUET " + "TBLPROPERTIES ('skip.footer.line.count'='1')");
    ...
    public void testLoadDataNegativeForDynamicPartition() throws Exception
    {
        ...
        try
        {
            ...
            {
                assertTrue("load data inpath", e.getMessage() != null && e.getMessage().contains("Invalid path"));
            }
        } finally
        {
            stmt.close();
