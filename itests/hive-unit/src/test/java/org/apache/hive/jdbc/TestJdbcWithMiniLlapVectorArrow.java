...
public class TestJdbcWithMiniLlapVectorArrow extends BaseJdbcWithMiniLlap
{
    ...
    public void testTypesNestedInListWithLimitAndFilters() throws Exception
    {
        ...
        {
            statement.execute("CREATE TABLE complex_tbl(c1 array<string>, " + "c2 array<struct<f1:string,f2:string>>, " + "c3 array<array<struct<f1:string,f2:string>>>, " + "c4 int) STORED AS ORC");

            statement.executeUpdate("INSERT INTO complex_tbl VALUES " + "(" + "ARRAY('a1', 'a2', 'a3', null), " + "ARRAY(NAMED_STRUCT('f1','a1', 'f2','a2'), NAMED_STRUCT('f1','a3', 'f2','a4')), " + "ARRAY((ARRAY(NAMED_STRUCT('f1','a1', 'f2','a2'), NAMED_STRUCT('f1','a3', 'f2','a4')))), " + "1),      " + "(" + "ARRAY('b1'), " + "ARRAY(NAMED_STRUCT('f1','b1', 'f2','b2'), NAMED_STRUCT('f1','b3', 'f2','b4')), " + "ARRAY((ARRAY(NAMED_STRUCT('f1','b1', 'f2','b2'), NAMED_STRUCT('f1','b3', 'f2','b4'))), " + "(ARRAY(NAMED_STRUCT('f1','b5', 'f2','b6'), NAMED_STRUCT('f1','b7', 'f2','b8')))), " + "2), " + "(" + "ARRAY('c1', 'c2'), ARRAY(NAMED_STRUCT('f1','c1', 'f2','c2'), NAMED_STRUCT('f1','c3', 'f2','c4'), " + "NAMED_STRUCT('f1','c5', 'f2','c6')), ARRAY((ARRAY(NAMED_STRUCT('f1','c1', 'f2','c2'), " + "NAMED_STRUCT('f1','c3', 'f2','c4'))), (ARRAY(NAMED_STRUCT('f1','c5', 'f2','c6'), " + "NAMED_STRUCT('f1','c7', 'f2','c8'))), (ARRAY(NAMED_STRUCT('f1','c9', 'f2','c10'), " + "NAMED_STRUCT('f1','c11', 'f2','c12')))), " + "3), " + "(" + "ARRAY(null), " + "ARRAY(NAMED_STRUCT('f1','d1', 'f2','d2'), NAMED_STRUCT('f1','d3', 'f2','d4'), " + "NAMED_STRUCT('f1','d5', 'f2','d6'), NAMED_STRUCT('f1','d7', 'f2','d8')), " + "ARRAY((ARRAY(NAMED_STRUCT('f1','d1', 'f2', 'd2')))), " + "4)");

        }

        List<Object[]> expected = new ArrayList<>();
        expected.add(new Object[]{asList("a1", "a2", "a3", null), asList(asList("a1", "a2"), asList("a3", "a4")), asList(asList(asList("a1", "a2"), asList("a3", "a4"))), 1});
        expected.add(new Object[]{asList("b1"), asList(asList("b1", "b2"), asList("b3", "b4")), asList(asList(asList("b1", "b2"), asList("b3", "b4")), asList(asList("b5", "b6"), asList("b7", "b8"))), 2});
        expected.add(new Object[]{asList("c1", "c2"), asList(asList("c1", "c2"), asList("c3", "c4"), asList("c5", "c6")), asList(asList(asList("c1", "c2"), asList("c3", "c4")), asList(asList("c5", "c6"), asList("c7", "c8")), asList(asList("c9", "c10"), asList("c11", "c12"))), 3});
        List<String> nullList = new ArrayList<>();
        nullList.add(null);
        expected.add(new Object[]{nullList, asList(asList("d1", "d2"), asList("d3", "d4"), asList("d5", "d6"), asList("d7", "d8")), asList(asList(asList("d1", "d2"))), 4});
        ...
        verifyResult(rowCollector.rows, expected.get(0), expected.get(1), expected.get(2), expected.get(3));
    ...
    public void testTypesNestedInMapWithLimitAndFilters() throws Exception
    {
        try (Statement statement = hs2Conn.createStatement())
        {
            statement.execute("CREATE TABLE complex_tbl2(c1 map<int, string>," + " c2 map<int, array<string>>, " + " c3 map<int, struct<f1:string,f2:string>>, c4 int) STORED AS ORC");

            statement.executeUpdate("INSERT INTO complex_tbl2 VALUES " + "(MAP(1, 'a1'), MAP(1, ARRAY('a1', 'a2')), MAP(1, NAMED_STRUCT('f1','a1', 'f2','a2')), " + "1), " + "(MAP(1, 'b1',2, 'b2'), MAP(1, ARRAY('b1', 'b2'), 2, ARRAY('b3') ), " + "MAP(1, NAMED_STRUCT('f1','b1', 'f2','b2')), " + "2), " + "(MAP(1, 'c1',2, 'c2'), MAP(1, ARRAY('c1', 'c2'), 2, ARRAY('c3') ), " + "MAP(1, NAMED_STRUCT('f1','c1', 'f2','c2'), 2, NAMED_STRUCT('f1', 'c3', 'f2', 'c4') ), " + "3)");

        }

        List<Object[]> expected = new ArrayList<>();
        expected.add(new Object[]{ImmutableMap.of(1, "a1"), ImmutableMap.of(1, asList("a1", "a2")), ImmutableMap.of(1, asList("a1", "a2")), 1,});
        expected.add(new Object[]{ImmutableMap.of(1, "b1", 2, "b2"), ImmutableMap.of(1, asList("b1", "b2"), 2, asList("b3")), ImmutableMap.of(1, asList("b1", "b2")), 2,});
        expected.add(new Object[]{ImmutableMap.of(1, "c1", 2, "c2"), ImmutableMap.of(1, asList("c1", "c2"), 2, asList("c3")), ImmutableMap.of(1, asList("c1", "c2"), 2, asList("c3", "c4")), 3,});
