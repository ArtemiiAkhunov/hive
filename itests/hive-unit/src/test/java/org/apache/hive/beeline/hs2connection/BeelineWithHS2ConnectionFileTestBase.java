...
public abstract class BeelineWithHS2ConnectionFileTestBase
{
    ...
    protected void assertBeelineOutputContains(String path, String[] beelineArgs, String expectedOutput) throws Exception
    {
        BeelineResult res = getBeelineOutput(path, beelineArgs);
        assertEquals(0, res.exitCode);
        Assert.assertNotNull(res.output);
        Assert.assertTrue("Output " + res.output + " does not contain " + expectedOutput, res.output.toLowerCase().contains(expectedOutput.toLowerCase()));
    }
