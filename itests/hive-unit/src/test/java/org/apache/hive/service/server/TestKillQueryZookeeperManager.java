...
public class TestKillQueryZookeeperManager
{
    ...
    {
        return CuratorFrameworkFactory.builder().connectString(server.getConnectString()).sessionTimeoutMs(TIMEOUT * 100).connectionTimeoutMs(TIMEOUT).retryPolicy(new RetryOneTime(1)).build();
    }
    ...
    public void testBarrierServerCrash() throws Exception
    {
        try (CuratorFramework client = getClient())
        {
            ...
            final KillQueryZookeeperManager.KillQueryZookeeperBarrier barrier = new KillQueryZookeeperManager.KillQueryZookeeperBarrier(client, BARRIER_ROOT_PATH);
