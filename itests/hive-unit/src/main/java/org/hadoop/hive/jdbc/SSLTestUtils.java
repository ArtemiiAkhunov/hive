...
public class SSLTestUtils
{
    ...
    private static final String dataFileDir = !System.getProperty("test.data.files", "").isEmpty() ? System.getProperty("test.data.files") : conf.get("test.data.files").replace('\\', '/').replace("c:", "");

    public static final String SSL_CONN_PARAMS = "ssl=true;sslTrustStore=" + URLEncoder.encode(dataFileDir + File.separator + TRUST_STORE_NAME) + ";trustStorePassword=" + KEY_STORE_TRUST_STORE_PASSWORD;
    ...
    public static void setSslConfOverlay(Map<String, String> confOverlay)
    {
        ...
        confOverlay.put(HiveConf.ConfVars.HIVE_SERVER2_SSL_KEYSTORE_PATH.varname, dataFileDir + File.separator + LOCALHOST_KEY_STORE_NAME);
        confOverlay.put(HiveConf.ConfVars.HIVE_SERVER2_SSL_KEYSTORE_PASSWORD.varname, KEY_STORE_TRUST_STORE_PASSWORD);
    }

    public static void setMetastoreSslConf(HiveConf conf)
    {
        MetastoreConf.setBoolVar(conf, MetastoreConf.ConfVars.USE_SSL, true);
        MetastoreConf.setVar(conf, MetastoreConf.ConfVars.SSL_KEYSTORE_PATH, dataFileDir + File.separator + LOCALHOST_KEY_STORE_NAME);
        MetastoreConf.setVar(conf, MetastoreConf.ConfVars.SSL_KEYSTORE_PASSWORD, KEY_STORE_TRUST_STORE_PASSWORD);
        MetastoreConf.setVar(conf, MetastoreConf.ConfVars.SSL_TRUSTSTORE_PATH, dataFileDir + File.separator + TRUST_STORE_NAME);
        MetastoreConf.setVar(conf, MetastoreConf.ConfVars.SSL_TRUSTSTORE_PASSWORD, KEY_STORE_TRUST_STORE_PASSWORD);
    }
    ...
    public static void setupTestTableWithData(String tableName, Path dataFilePath, Connection hs2Conn) throws Exception
    {
