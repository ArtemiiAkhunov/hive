...
public class AlterPartitionParamsExample extends GenericUDF
{
    ...
    public ObjectInspector initialize(ObjectInspector[] arguments) throws UDFArgumentException
    {
        ...
        {
            throw new UDFArgumentLengthException("Requires 4 argument, got " + arguments.length);
        }
        if (!(arguments[0] instanceof ConstantObjectInspector))
        {
            throw new UDFArgumentException("The first argument should be a string constant, got " + arguments[0].getTypeName());
        }
    ...
    {
        ret.set(false);
        if (arguments[1].get() == null || arguments[2].get() == null || arguments[3].get() == null)
        {
            return ret;
        }
        ...
        try
        {
            List<Partition> partitionList = Hive.get().getPartitionsByNames(table, Arrays.asList(partName));
            ...
            Hive.get().alterPartition(table.getCatName(), table.getDbName(), table.getTableName(), partition, null, true);
            ret.set(true);
        } catch (Exception e)
        {
