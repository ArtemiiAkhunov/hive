...
public class MiniDruidCluster extends AbstractService
{
    ...
    private static final String COMMON_DRUID_JVM_PROPERTIES = "-Duser.timezone=UTC -Dfile.encoding=UTF-8 -Djava.util.logging.manager=org.apache.logging.log4j.jul.LogManager " + "-Ddruid.emitter=logging -Ddruid.emitter.logging.logLevel=info";

    private static final List<String> HISTORICAL_JVM_CONF = Arrays.asList("-server", "-XX:MaxDirectMemorySize=10g", "-Xmx512m", "-Xmx512m", COMMON_DRUID_JVM_PROPERTIES);

    private static final List<String> COORDINATOR_JVM_CONF = Arrays.asList("-server", "-XX:MaxDirectMemorySize=2g", "-Xmx512m", "-Xms512m", COMMON_DRUID_JVM_PROPERTIES);

    private static final Map<String, String> COMMON_DRUID_CONF = ImmutableMap.of("druid.metadata.storage.type", "derby", "druid.storage.type", "hdfs", "druid.processing.buffer.sizeBytes", "10485760", "druid.processing.numThreads", "2", "druid.worker.capacity", "4");

    private static final Map<String, String> COMMON_DRUID_HISTORICAL = ImmutableMap.of("druid.server.maxSize", "130000000000");

    private static final Map<String, String> COMMON_COORDINATOR_INDEXER = ImmutableMap.<String, String>builder().put("druid.indexer.logs.type", "file").put("druid.coordinator.asOverlord.enabled", "true").put("druid.coordinator.asOverlord.overlordService", "druid/overlord").put("druid.coordinator.period", "PT2S").put("druid.manager.segments.pollDuration", "PT2S").put("druid.indexer.runner.javaOpts", "-Xmx512m").build();
    ...
    public MiniDruidCluster(String name, String logDir, String tmpDir, Integer zookeeperPort, String classpath)
    {
        ...
        derbyURI = String.format("jdbc:derby://localhost:%s/%s/druid_derby/metadata.db;create=true", derbyPort, dataDirectory.getAbsolutePath());
        String segmentsCache = String.format("[{\"path\":\"%s/druid/segment-cache\",\"maxSize\":130000000000}]", dataDirectory.getAbsolutePath());
        ...
        Map<String, String> coordinatorProperties = coordinatorMapBuilder.putAll(COMMON_DRUID_CONF).putAll(COMMON_COORDINATOR_INDEXER).put("druid.metadata.storage.connector.connectURI", derbyURI).put("druid.metadata.storage.connector.port", String.valueOf(derbyPort)).put("druid.indexer.logs.directory", indexingLogDir).put("druid.zk.service.host", "localhost:" + zookeeperPort).put("druid.coordinator.startDelay", "PT1S").put("druid.indexer.runner", "local").put("druid.storage.storageDirectory", getDeepStorageDir()).put("druid.port", String.valueOf(coordinatorPort)).build();
        Map<String, String> historicalProperties = historicalMapBuilder.putAll(COMMON_DRUID_CONF).putAll(COMMON_DRUID_HISTORICAL).put("druid.zk.service.host", "localhost:" + zookeeperPort).put("druid.segmentCache.locations", segmentsCache).put("druid.storage.storageDirectory", getDeepStorageDir()).put("druid.port", String.valueOf(historicalPort)).build();
        Map<String, String> brokerProperties = brokerMapBuilder.putAll(COMMON_DRUID_CONF).put("druid.zk.service.host", "localhost:" + zookeeperPort).put("druid.port", String.valueOf(brokerPort)).build();
        coordinator = new ForkingDruidNode("coordinator", classpath, coordinatorProperties, COORDINATOR_JVM_CONF, logDirectory, null);
        historical = new ForkingDruidNode("historical", classpath, historicalProperties, HISTORICAL_JVM_CONF, logDirectory, null);
