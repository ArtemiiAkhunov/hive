...
public class ForkingDruidNode extends DruidNode
{
    ...
    private final List<String> allowedPrefixes = Lists.newArrayList("com.metamx", "druid", "org.apache.druid", "java.io.tmpdir", "hadoop");

    public ForkingDruidNode(String nodeType, String extraClasspath, Map<String, String> properties, List<String> jvmArgs, File logLocation, String javaCmd)
    {
        super(nodeType);

        final List<String> command = Lists.newArrayList();
        this.classpath = Strings.isNullOrEmpty(extraClasspath) ? System.getProperty("java.class.path") : extraClasspath;
        ...
        for (String propName : System.getProperties().stringPropertyNames())
        {
            for (String allowedPrefix : allowedPrefixes)
            {
                ...
                {
                    command.add(String.format("-D%s=%s", propName, System.getProperty(propName)));
                }
            }
        }
        this.properties.forEach((key, value) -> command.add(String.format("-D%s=%s", key, value)));
