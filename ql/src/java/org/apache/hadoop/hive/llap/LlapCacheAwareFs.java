...
public class LlapCacheAwareFs extends FileSystem
{
    ...
    public static Path registerFile(DataCache cache, Path path, Object fileKey, TreeMap<Long, Long> index, Configuration conf, CacheTag tag) throws IOException
    {
    ...
    private static class CacheAwareInputStream extends InputStream implements Seekable, PositionedReadable
    {
        ...
        public CacheAwareInputStream(DataCache cache, Configuration conf, TreeMap<Long, Long> chunkIndex, Path path, Object fileKey, int bufferSize, CacheTag tag)
        {
            this.cache = cache;
        ...
        public int read(byte[] array, final int arrayOffset, final int len) throws IOException
        {
            ...
            try (FSDataInputStream is = fs.open(path, bufferSize))
            {
                ...
                while (current != null)
                {
                    ...
                    {
                        long chunkFrom = Math.max(from, missingChunk.getKey()), chunkTo = Math.min(to, missingChunk.getValue()), chunkLength = chunkTo - chunkFrom;
                        ...
                        try
                        {
                            ...
                            if (maxAlloc < chunkLength)
                            {
                                ...
                                for (int i = 0; i < largeBuffers.length; ++i)
                                {
                                    ...
                                    copyDiskDataToCacheBuffer(array, arrayOffset + offsetFromReadStart + extraDiskDataOffset, remaining, bb, cacheRanges, i, chunkFrom + extraOffsetInChunk);
                            ...
                            if (smallSize > 0)
                            {
                                ...
                                copyDiskDataToCacheBuffer(array, arrayOffset + offsetFromReadStart + extraDiskDataOffset, smallSize, bb, cacheRanges, largeBufCount, chunkFrom + extraOffsetInChunk);
        ...
        private void validateAndUpdatePosition(int len, long sizeRead)
        {
            if (sizeRead != len)
            {
                throw new AssertionError("Reading at " + position + " for " + len + ": " + sizeRead + " bytes copied");
            }
            position += len;
        }
        ...
        private SortedMap<Long, Long> getAndValidateMissingChunks(int maxAlloc, long from, long to)
        {
            ...
            if (firstMissing.getValue() <= from || ((from - firstMissing.getKey()) % maxAlloc) != 0)
            {
                // The data does not belong to a recognized chunk, or is split wrong.
                throw new AssertionError("Lower bound for start offset " + from + " is [" + firstMissing.getKey() + ", " + firstMissing.getValue() + ")");
            }
            ...
            long lastMissingOffset = missingChunks.lastKey(), lastMissingEnd = missingChunks.get(lastMissingOffset);
            if (lastMissingEnd < to || (to != lastMissingEnd && ((to - lastMissingOffset) % maxAlloc) != 0))
            {
                // The data does not belong to a recognized chunk, or is split wrong.
                throw new AssertionError("Lower bound for end offset " + to + " is [" + lastMissingOffset + ", " + lastMissingEnd + ")");
            }
