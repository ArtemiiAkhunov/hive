...
public class HiveRelBuilder extends RelBuilder
{
    ...
    {
        final RexNode x = RexUtil.composeConjunction(cluster.getRexBuilder(), predicates, false);
        if (!x.isAlwaysTrue())
        {
    ...
    public static SqlAggFunction getRollup(SqlAggFunction aggregation)
    {
        ...
        if (aggregation instanceof HiveSqlSumAggFunction || aggregation instanceof HiveSqlMinMaxAggFunction || aggregation instanceof HiveSqlSumEmptyIsZeroAggFunction)
        {
            return aggregation;
        }
        if (aggregation instanceof HiveSqlCountAggFunction)
        {
            ...
            return new HiveSqlSumEmptyIsZeroAggFunction(countAgg.isDistinct(), countAgg.getReturnTypeInference(), countAgg.getOperandTypeInference(), countAgg.getOperandTypeChecker());
        }
        return null;
    }
