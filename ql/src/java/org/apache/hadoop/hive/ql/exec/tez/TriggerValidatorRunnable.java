...
public class TriggerValidatorRunnable implements Runnable
{
    ...
    TriggerValidatorRunnable(final SessionTriggerProvider sessionTriggerProvider, final TriggerActionHandler triggerActionHandler)
    {
                        ...
                        if (currentCounters.containsKey(desiredCounter))
                        {
                            ...
                            if (currentTrigger.apply(currentCounterValue))
                            {
                                ...
                                if (violatedSessions.containsKey(sessionState))
                                {
                                    ...
                                    } else if (existingTrigger.getAction().getType().equals(Action.Type.MOVE_TO_POOL) && currentTrigger.getAction().getType().equals(Action.Type.MOVE_TO_POOL))
                                    {
                                    ...
                                    } else if (existingTrigger.getAction().getType().equals(Action.Type.KILL_QUERY) && currentTrigger.getAction().getType().equals(Action.Type.KILL_QUERY))
                                    {
                                        // if multiple KILL happens, only first kill will be chosen
                                        LOG.warn("Conflicting KILL triggers ({} and {}). Choosing the first KILL trigger: {}", existingTrigger, currentTrigger, existingTrigger.getName());
                                    }
                                } else
                                {
