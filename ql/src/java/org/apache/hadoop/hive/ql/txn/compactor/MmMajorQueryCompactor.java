...
final class MmMajorQueryCompactor extends QueryCompactor
{
    ...
    public boolean run(CompactorContext context) throws IOException
    {
        ...
        List<String> createTableQueries = getCreateQueries(tmpTableName, table, storageDescriptor, resultBaseDir.toString());
        ...
        runCompactionQueries(driverConf, tmpTableName, storageDescriptor, writeIds, context.getCompactionInfo(), Lists.newArrayList(resultBaseDir), createTableQueries, compactionQueries, dropQueries, table.getParameters());
        return true;
    }
    ...
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MAJOR, CompactionQueryBuilder.Operation.CREATE, true, tmpTableName).setSourceTab(table).setStorageDescriptor(storageDescriptor).setLocation(baseLocation).build());
    }

    private List<String> getCompactionQueries(Table t, Partition p, String tmpName)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MAJOR, CompactionQueryBuilder.Operation.INSERT, true, tmpName).setSourceTab(t).setSourcePartition(p).build());
    }

    private List<String> getDropQueries(String tmpTableName)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MAJOR, CompactionQueryBuilder.Operation.DROP, true, tmpTableName).build());
    }
}
