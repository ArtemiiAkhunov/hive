...
public class VectorizedOrcAcidRowBatchReader implements org.apache.hadoop.mapred.RecordReader<NullWritable, VectorizedRowBatch>
{
    ...
    @VisibleForTesting
    VectorizedOrcAcidRowBatchReader(OrcSplit inputSplit, JobConf conf, Reporter reporter, VectorizedRowBatchCtx rbCtx) throws IOException
    {
        this(conf, inputSplit, reporter, rbCtx == null ? Utilities.getVectorizedRowBatchCtx(conf) : rbCtx, false, null);
    ...
    private VectorizedOrcAcidRowBatchReader(JobConf conf, OrcSplit orcSplit, Reporter reporter, VectorizedRowBatchCtx rowBatchCtx, boolean isFlatPayload, MapWork mapWork) throws IOException
    {
        ...
        if (LlapHiveUtils.isLlapMode(conf) && LlapProxy.isDaemon() && HiveConf.getBoolVar(conf, ConfVars.LLAP_TRACK_CACHE_USAGE))
        {
            if (mapWork == null)
            {
        ...
        try
        {
            ...
            der = new ColumnizedDeleteEventRegistry(conf, orcSplit, deleteEventReaderOptions, keyInterval, cacheTag, writeIdLoader);
        ...
        if (isOriginal)
        {
            recordIdColumnVector = new StructColumnVector(VectorizedRowBatch.DEFAULT_SIZE, new LongColumnVector(), new LongColumnVector(), new LongColumnVector());
        } else
        {
            ...
            if (deleteEventRegistry.isEmpty() && !rowIdProjected)
            {
                ...
                while (parent != null && !rootPath.equals(parent))
                {
                    ...
                    } else
                    {
                        ...
                        if (validWriteIdList.isWriteIdRangeValid(pd.getMinWriteId(), pd.getMaxWriteId()) == ValidWriteIdList.RangeResponse.ALL)
                        {
    ...
    private void setSARG(OrcRawRecordMerger.KeyInterval keyInterval, Reader.Options deleteEventReaderOptions, long minBucketProp, long maxBucketProp, long minRowId, long maxRowId)
    {
        ...
        if (keyInterval.getMinKey() != null)
        {
            ...
            b.startAnd()  //not(ot < 7) -> ot >=7
                    .startNot().lessThan(OrcRecordUpdater.ORIGINAL_WRITEID_FIELD_NAME, PredicateLeaf.Type.LONG, k.getWriteId()).end();
    ...
    public void setBaseAndInnerReader(final org.apache.hadoop.mapred.RecordReader<NullWritable, VectorizedRowBatch> baseReader)
    {
    ...
    static Path[] getDeleteDeltaDirsFromSplit(OrcSplit orcSplit, Map<String, AcidInputFormat.DeltaMetaData> pathToDeltaMetaData)
    {
        ...
        if (orcSplit.hasBase())
        {
            ...
            } else
            {
                ...
                assert root.equals(orcSplit.getRootDir()) : "root mismatch: baseDir=" + orcSplit.getRootDir() + " path.p.p=" + root;
            }
        } else
        {
    ...
    public boolean next(NullWritable key, VectorizedRowBatch value) throws IOException
    {
        ...
        this.deleteEventRegistry.findDeletedRecords(innerRecordIdColumnVector, vectorizedRowBatchBase.size, notDeletedBitSet);
    ...
    private void copyFromBase(VectorizedRowBatch value)
    {
        ...
        if (isFlatPayload)
        {
            ...
            System.arraycopy(vectorizedRowBatchBase.cols, payloadCol + 1, value.cols, 0, vectorizedRowBatchBase.cols.length - payloadCol - 1);
        } else
        {
    ...
    private ColumnVector[] handleOriginalFile(BitSet selectedBitSet, ColumnVector[] innerRecordIdColumnVector) throws IOException
    {
         ...
         */
        boolean needSyntheticRowId = needSyntheticRowIds(true, !deleteEventRegistry.isEmpty(), rowIdProjected);
        if (needSyntheticRowId)
        {
            ...
            if (innerReader == null)
            {
                throw new IllegalStateException(getClass().getName() + " requires " + org.apache.orc.RecordReader.class + " to handle original files that require ROW__IDs: " + rootPath);
            }
            /**
        ...
        if (syntheticProps.getSyntheticWriteId() > 0)
        {
            ...
            if (needSyntheticRowId)
            {
                findRecordsWithInvalidWriteIds(innerRecordIdColumnVector, vectorizedRowBatchBase.size, selectedBitSet);
            } else
            {
    ...
    private void findRecordsWithInvalidWriteIds(ColumnVector[] cols, int size, BitSet selectedBitSet)
    {
        if (cols[OrcRecordUpdater.CURRENT_WRITEID].isRepeating)
        {
            ...
            long currentWriteIdForBatch = ((LongColumnVector) cols[OrcRecordUpdater.CURRENT_WRITEID]).vector[0];
        ...
        long[] currentWriteIdVector = ((LongColumnVector) cols[OrcRecordUpdater.CURRENT_WRITEID]).vector;
    ...
    static class SortMergedDeleteEventRegistry implements DeleteEventRegistry
    {
        ...
        SortMergedDeleteEventRegistry(JobConf conf, OrcSplit orcSplit, Reader.Options readerOptions, boolean fetchDeletedRows) throws IOException
        {
            ...
            if (deleteDeltas.length > 0)
            {
                ...
                this.deleteRecords = new OrcRawRecordMerger(conf, true, null, false, bucket, validWriteIdList, readerOptions, deleteDeltas, mergerOptions, deltaToAttemptId);
        ...
        public void findDeletedRecords(ColumnVector[] cols, int size, BitSet selectedBitSet) throws IOException
        {
            ...
            long[] originalWriteId = cols[OrcRecordUpdater.ORIGINAL_WRITEID].isRepeating ? null : ((LongColumnVector) cols[OrcRecordUpdater.ORIGINAL_WRITEID]).vector;
            long[] bucket = cols[OrcRecordUpdater.BUCKET].isRepeating ? null : ((LongColumnVector) cols[OrcRecordUpdater.BUCKET]).vector;
            long[] rowId = cols[OrcRecordUpdater.ROW_ID].isRepeating ? null : ((LongColumnVector) cols[OrcRecordUpdater.ROW_ID]).vector;

            // The following repeatedX values will be set, if any of the columns are repeating.
            long repeatedOriginalWriteId = (originalWriteId != null) ? -1 : ((LongColumnVector) cols[OrcRecordUpdater.ORIGINAL_WRITEID]).vector[0];
            long repeatedBucket = (bucket != null) ? -1 : ((LongColumnVector) cols[OrcRecordUpdater.BUCKET]).vector[0];
            long repeatedRowId = (rowId != null) ? -1 : ((LongColumnVector) cols[OrcRecordUpdater.ROW_ID]).vector[0];
            ...
            RecordIdentifier firstRecordIdInBatch = new RecordIdentifier(originalWriteId != null ? originalWriteId[firstValidIndex] : repeatedOriginalWriteId, bucket != null ? (int) bucket[firstValidIndex] : (int) repeatedBucket, rowId != null ? (int) rowId[firstValidIndex] : repeatedRowId);

            // Get the last valid row in the batch still available.
            int lastValidIndex = selectedBitSet.previousSetBit(size - 1);
            RecordIdentifier lastRecordIdInBatch = new RecordIdentifier(originalWriteId != null ? originalWriteId[lastValidIndex] : repeatedOriginalWriteId, bucket != null ? (int) bucket[lastValidIndex] : (int) repeatedBucket, rowId != null ? (int) rowId[lastValidIndex] : repeatedRowId);
            ...
            while (deleteRecordKey.compareRow(firstRecordIdInBatch) == -1)
            {
                ...
                if (!isDeleteRecordAvailable)
                {
                    return; // exhausted all delete records, return.
                }
            }
    ...
    static class ColumnizedDeleteEventRegistry implements DeleteEventRegistry
    {
        ...
        static class DeleteRecordKey implements Comparable<DeleteRecordKey>
        {
            ...
            {
                return "DeleteRecordKey(" + originalWriteId + "," + RecordIdentifier.bucketToString(bucketProperty) + "," + rowId + ")";
            }
        }
        ...
        static class DeleteReaderValue
        {
            ...
            private static final List<Integer> DELETE_DELTA_INCLUDE_COLUMNS = IntStream.range(0, OrcInputFormat.getRootColumn(false) - 1).boxed().collect(toList());
            ...
            DeleteReaderValue(Reader deleteDeltaReader, Path deleteDeltaFile, Reader.Options readerOptions, int bucket, ValidWriteIdList validWriteIdList, boolean isBucketedTable, final JobConf conf, OrcRawRecordMerger.KeyInterval keyInterval, OrcSplit orcSplit, long numRows, CacheTag cacheTag, Object fileId) throws IOException
            {
                ...
                final boolean useDecimal64ColumnVector = HiveConf.getVar(conf, ConfVars.HIVE_VECTORIZED_INPUT_FORMAT_SUPPORTS_ENABLED).equalsIgnoreCase("decimal_64");
            ...
            static boolean isDeleteEventInRange(OrcRawRecordMerger.KeyInterval keyInterval, DeleteRecordKey deleteRecordKey)
            {
                ...
                if (keyInterval.getMaxKey() != null && deleteRecordKey.compareTo(keyInterval.getMaxKey()) > 0)
                {
            ...
            public void close() throws IOException
            {
                ...
                LOG.debug("Num events stats({},{},{})", numEvents, numEventsFromDisk, numEventsLoaded);
            }

            private long setCurrentDeleteKey(DeleteRecordKey deleteRecordKey)
            {
                int originalWriteIdIndex = batch.cols[OrcRecordUpdater.ORIGINAL_WRITEID].isRepeating ? 0 : indexPtrInBatch;
                long originalWriteId = ((LongColumnVector) batch.cols[OrcRecordUpdater.ORIGINAL_WRITEID]).vector[originalWriteIdIndex];
                int bucketPropertyIndex = batch.cols[OrcRecordUpdater.BUCKET].isRepeating ? 0 : indexPtrInBatch;
                ...
                int currentWriteIdIndex = batch.cols[OrcRecordUpdater.CURRENT_WRITEID].isRepeating ? 0 : indexPtrInBatch;
                long currentWriteId = ((LongColumnVector) batch.cols[OrcRecordUpdater.CURRENT_WRITEID]).vector[currentWriteIdIndex];
            ...
            private void checkBucketId() throws IOException
            {
                ...
                if (isBucketPropertyRepeating)
                {
                    int bucketPropertyFromRecord = (int) ((LongColumnVector) batch.cols[OrcRecordUpdater.BUCKET]).vector[0];
            ...
            private void checkBucketId(int bucketPropertyFromRecord) throws IOException
            {
                ...
                if (bucketIdFromRecord != bucketForSplit)
                {
                    ...
                    throw new IOException("Corrupted records with different bucket ids " + "from the containing bucket file found! Expected bucket id " + bucketForSplit + ", however found " + dummy + ".  (" + orcSplit + "," + deleteDeltaFile + ")");
                }
            }
            ...
            public String toString()
            {
                return "{recordReader=" + recordReader + ", isBucketPropertyRepeating=" + isBucketPropertyRepeating + ", bucketForSplit=" + bucketForSplit + ", isBucketedTable=" + isBucketedTable + "}";
            }

            private static RecordReader getLlapRecordReader(Path deleteDeltaFile, org.apache.orc.Reader.Options readerOptions, JobConf conf, CacheTag tag, Object fileId) throws IOException
            {
                ...
                return wrapLlapVectorizedRecordReader(LlapProxy.getIo().llapVectorizedOrcReaderForPath(fileId, deleteDeltaFile, tag, DELETE_DELTA_INCLUDE_COLUMNS, c, 0L, Long.MAX_VALUE, null));
            }
             ...
             * @return
             */
            private static RecordReader wrapLlapVectorizedRecordReader(org.apache.hadoop.mapred.RecordReader<NullWritable, VectorizedRowBatch> rr)
            {
                if (rr == null)
                {
                    return null;
        ...
        static class OriginalWriteIds
        {
            ...
            protected int indexOfRowId(long owid, int bucketProperty, long rowId, long[] rowIds)
            {
                ...
                if (owid < compressedOwids[0].originalWriteId || owid > compressedOwids[compressedOwids.length - 1].originalWriteId)
                {
                    return -1;
                }
            ...
            public void markAsDeleted(long owid, int bucketProperty, long rowId, long[] rowIds, int setBitIndex, BitSet selectedBitSet, LongColumnVector currentWriteIdVector)
            {
        ...
        ColumnizedDeleteEventRegistry(JobConf conf, OrcSplit orcSplit, Reader.Options readerOptions, OrcRawRecordMerger.KeyInterval keyInterval, CacheTag cacheTag, OriginalWriteIdLoader writeIdLoader) throws IOException, DeleteEventsOverflowMemoryException
        {
            ...
            try
            {
                ...
                {
                    AcidOutputFormat.Options orcSplitMinMaxWriteIds = AcidUtils.parseBaseOrDeltaBucketFilename(orcSplit.getPath(), conf);
                        ...
                        for (Pair<Path, Integer> deleteDeltaDir : deltaMetaData.getPaths(orcSplit.getRootDir()))
                        {
                            ...
                            for (AcidInputFormat.DeltaFileMetaData fileMetaData : deltaMetaData.getDeltaFilesForStmtId(stmtId))
                            {
                                ...
                                try (ReaderData readerData = getOrcReaderData(deleteDeltaFile, conf, cacheTag, fileId))
                                {
                                    ...
                                    deleteReaderValue = new DeleteReaderValue(readerData.reader, deleteDeltaFile, readerOptions, bucket, validWriteIdList, isBucketedTable, conf, keyInterval, orcSplit, numRows, cacheTag, fileId);
                    ...
                    LOG.debug("Number of delete events(limit, actual)=({},{})", totalDeleteEventCount, size());
                }
        ...
        private void checkSize(int index) throws DeleteEventsOverflowMemoryException
        {
            if (index > maxEventsInMemory)
            {
                ...
                LOG.info("Total number of delete events exceeds the maximum number of " + "delete events that can be loaded into memory for " + orcSplit + ". The max limit is currently set at " + maxEventsInMemory + " and can be changed by setting the Hive config variable " + ConfVars.HIVE_TRANSACTIONAL_NUM_EVENTS_IN_MEMORY.varname);
         ...
         */
        private void readAllDeleteEventsFromDeleteDeltas(OriginalWriteIdLoader writeIdLoader) throws IOException, DeleteEventsOverflowMemoryException
        {
        ...
        public void findDeletedRecords(ColumnVector[] cols, int size, BitSet selectedBitSet)
        {
            ...
            long[] originalWriteIdVector = cols[OrcRecordUpdater.ORIGINAL_WRITEID].isRepeating ? null : ((LongColumnVector) cols[OrcRecordUpdater.ORIGINAL_WRITEID]).vector;
            long repeatedOriginalWriteId = (originalWriteIdVector != null) ? -1 : ((LongColumnVector) cols[OrcRecordUpdater.ORIGINAL_WRITEID]).vector[0];

            long[] bucketProperties = cols[OrcRecordUpdater.BUCKET].isRepeating ? null : ((LongColumnVector) cols[OrcRecordUpdater.BUCKET]).vector;
            int repeatedBucketProperty = (bucketProperties != null) ? -1 : (int) ((LongColumnVector) cols[OrcRecordUpdater.BUCKET]).vector[0];

            long[] rowIdVector = ((LongColumnVector) cols[OrcRecordUpdater.ROW_ID]).vector;

            LongColumnVector currentWriteIdVector = (LongColumnVector) cols[OrcRecordUpdater.CURRENT_WRITEID];
            for (int setBitIndex = selectedBitSet.nextSetBit(0); setBitIndex >= 0; setBitIndex = selectedBitSet.nextSetBit(setBitIndex + 1))
            {
                long owid = originalWriteIdVector != null ? originalWriteIdVector[setBitIndex] : repeatedOriginalWriteId;
                int bucketProperty = bucketProperties != null ? (int) bucketProperties[setBitIndex] : repeatedBucketProperty;
    ...
    private static OrcRawRecordMerger.KeyInterval getKeyInterval(List<OrcProto.ColumnStatistics> colStats)
    {
        ...
        columnStatsArray[OrcRecordUpdater.ORIGINAL_WRITEID + 1] = deserializeIntColumnStatistics(colStats, OrcRecordUpdater.ORIGINAL_WRITEID + 1);
        columnStatsArray[OrcRecordUpdater.BUCKET + 1] = deserializeIntColumnStatistics(colStats, OrcRecordUpdater.BUCKET + 1);
        columnStatsArray[OrcRecordUpdater.ROW_ID + 1] = deserializeIntColumnStatistics(colStats, OrcRecordUpdater.ROW_ID + 1);
    ...
    private static class RowIsDeletedColumnVector extends LongColumnVector
    {
        ...
        void set(BitSet notDeletedBitSet)
        {
            ...
            } else
            {
                ...
                isRepeating = false;
                for (int setBitIndex = notDeletedBitSet.nextSetBit(0); setBitIndex >= 0; setBitIndex = notDeletedBitSet.nextSetBit(setBitIndex + 1))
                {
