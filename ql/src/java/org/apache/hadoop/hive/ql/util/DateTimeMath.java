...
public class DateTimeMath
{
    ...
    public boolean add(Timestamp ts, HiveIntervalDayTime interval, Timestamp result)
    {
        ...
        long newMillis = ts.toEpochMilli() + TimeUnit.SECONDS.toMillis(interval.getTotalSeconds() + nanosResult.seconds);
        result.setTimeInMillis(newMillis, nanosResult.nanos);
        return true;
    }

    @Deprecated
    public boolean add(java.sql.Timestamp ts, HiveIntervalDayTime interval, java.sql.Timestamp result)
    {
        if (ts == null || interval == null)
        {
            return false;
        ...
        long newMillis = ts.getTime() + TimeUnit.SECONDS.toMillis(interval.getTotalSeconds() + nanosResult.seconds);
    ...
    @Deprecated
    public boolean add(HiveIntervalDayTime interval, java.sql.Timestamp ts, java.sql.Timestamp result)
    {
        if (ts == null || interval == null)
        {
            return false;
    ...
    public boolean subtract(Timestamp left, Timestamp right, HiveIntervalDayTime result)
    {
        ...
        long totalSeconds = TimeUnit.MILLISECONDS.toSeconds(left.toEpochMilli()) - TimeUnit.MILLISECONDS.toSeconds(right.toEpochMilli()) + nanosResult.seconds;
        result.set(totalSeconds, nanosResult.nanos);
        return true;
    }

    @Deprecated
    public boolean subtract(java.sql.Timestamp left, java.sql.Timestamp right, HiveIntervalDayTime result)
    {
        if (left == null || right == null)
        {
            return false;
