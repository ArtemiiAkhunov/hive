...
public class HiveCollectionConverter extends HiveGroupConverter
{
    ...
    public static HiveGroupConverter forMap(GroupType mapType, ConverterParent parent, int index, TypeInfo hiveTypeInfo)
    {
        return new HiveCollectionConverter(mapType, parent, index, true /* its a map */, hiveTypeInfo);
    }

    public static HiveGroupConverter forList(GroupType listType, ConverterParent parent, int index, TypeInfo hiveTypeInfo)
    {
        return new HiveCollectionConverter(listType, parent, index, false /* nUnknown hive type infoot a map */, hiveTypeInfo);
    }

    private HiveCollectionConverter(GroupType collectionType, ConverterParent parent, int index, boolean isMap, TypeInfo hiveTypeInfo)
    {
        ...
        if (isMap)
        {
            this.innerConverter = new KeyValueConverter(repeatedType.asGroupType(), this, hiveTypeInfo);
        ...
        {
            this.innerConverter = new ElementConverter(repeatedType.asGroupType(), this, extractListCompatibleType(hiveTypeInfo));
        }
    }
    ...
    public void end()
    {
        parent.set(index, new ArrayWritable(Writable.class, list.toArray(new Writable[0])));
    }
    ...
    private static class KeyValueConverter extends HiveGroupConverter
    {
        ...
        public KeyValueConverter(GroupType keyValueType, HiveGroupConverter parent, TypeInfo hiveTypeInfo)
        {
            setMetadata(parent.getMetadata());
            this.parent = parent;
            this.keyConverter = getConverterFromDescription(keyValueType.getType(0), 0, this, hiveTypeInfo == null ? null : ((MapTypeInfo) hiveTypeInfo).getMapKeyTypeInfo());
            this.valueConverter = getConverterFromDescription(keyValueType.getType(1), 1, this, hiveTypeInfo == null ? null : ((MapTypeInfo) hiveTypeInfo).getMapValueTypeInfo());
        }
    ...
    private static class ElementConverter extends HiveGroupConverter
    {
        ...
        public ElementConverter(GroupType repeatedType, HiveGroupConverter parent, TypeInfo hiveTypeInfo)
        {
            setMetadata(parent.getMetadata());
            this.parent = parent;
            this.elementConverter = getConverterFromDescription(repeatedType.getType(0), 0, this, hiveTypeInfo);
        }
