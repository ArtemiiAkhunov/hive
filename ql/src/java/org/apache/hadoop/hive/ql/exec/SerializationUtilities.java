...
public class SerializationUtilities
{
    ...
    private static class KryoWithHooks extends Kryo implements Configurable
    {
        ...
        private static final class SerializerWithHook extends com.esotericsoftware.kryo.Serializer
        {
            ...
            {
                return hook.preRead(type) ? hook.postRead(old.read(kryo, input, type)) : old.read(kryo, input, type);
            }
        ...
        @Override
        public <T> T readObjectOrNull(Input input, Class<T> type, @SuppressWarnings("rawtypes") com.esotericsoftware.kryo.Serializer serializer)
        {
            Hook hook = ponderGlobalPreReadHook(type);
        ...
        @Override
        public <T> T readObject(Input input, Class<T> type, @SuppressWarnings("rawtypes") com.esotericsoftware.kryo.Serializer serializer)
        {
            Hook hook = ponderGlobalPreReadHook(type);
        ...
        public com.esotericsoftware.kryo.Registration getRegistration(Class type)
        {
            ...
            if (isExprNodeFirst && classCounter == 0)
            {
                ...
                {
                    throw new UnsupportedOperationException("The object to be deserialized must be an ExprNodeDesc, but encountered: " + type);
                }
            }
            classCounter++;
     ...
     */
    private static class CopyOnFirstWritePropertiesSerializer extends com.esotericsoftware.kryo.serializers.MapSerializer<Map>
    {
    ...
    private static class MapWorkSerializer extends FieldSerializer<MapWork>
    {
        ...
        private void filterMapworkProperties(Kryo kryo, MapWork mapWork)
        {
            ...
            if (configuration == null || HiveConf.getVar(configuration, HiveConf.ConfVars.HIVE_PLAN_MAPWORK_SERIALIZATION_SKIP_PROPERTIES).isEmpty())
            {
                return;
            }
            String[] filterProps = HiveConf.getVar(configuration, HiveConf.ConfVars.HIVE_PLAN_MAPWORK_SERIALIZATION_SKIP_PROPERTIES).split(",");
            for (String prop : filterProps)
            {
                ...
                for (Entry<Path, PartitionDesc> partDescEntry : mapWork.getPathToPartitionInfo().entrySet())
                {
                    ...
                    if (isRegex)
                    {
                        Iterator<Entry<Object, Object>> itProps = partDescEntry.getValue().getProperties().entrySet().iterator();
    ...
    public static MapredWork clonePlan(MapredWork plan)
    {
        ...
        MapredWork newPlan = deserializePlan(new ByteArrayInputStream(baos.toByteArray()), MapredWork.class);
        // Restore the context.
    ...
    public static List<Operator<?>> cloneOperatorTree(List<Operator<?>> roots)
    {
        ...
        @SuppressWarnings("unchecked") List<Operator<?>> result = deserializePlan(new ByteArrayInputStream(baos.toByteArray()), roots.getClass());
        // Restore the context.
    ...
    public static BaseWork cloneBaseWork(BaseWork plan)
    {
        ...
        ByteArrayOutputStream baos = new ByteArrayOutputStream(4096);
        serializePlan(plan, baos);
        BaseWork newPlan = deserializePlan(new ByteArrayInputStream(baos.toByteArray()), plan.getClass());
        // Restore the context.
        for (Operator<?> newOp : newPlan.getAllOperators())
        {
            newOp.setCompilationOpContext(ctx);
