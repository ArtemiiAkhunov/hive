...
public class GenericUDAFComputeStats extends AbstractGenericUDAFResolver
{
    ...
    public static class GenericUDAFBooleanStatsEvaluator extends GenericUDAFEvaluator
    {
        ...
        public ObjectInspector init(Mode m, ObjectInspector[] parameters) throws HiveException
        {
            ...
            } else
            {
                ...
                countTruesFieldOI = (WritableLongObjectInspector) countTruesField.getFieldObjectInspector();

                countFalsesField = soi.getStructFieldRef("countfalses");
                countFalsesFieldOI = (WritableLongObjectInspector) countFalsesField.getFieldObjectInspector();
    ...
    public static abstract class GenericUDAFNumericStatsEvaluator<V, OI extends PrimitiveObjectInspector> extends GenericUDAFEvaluator
    {
        ...
        public abstract class NumericStatsAgg extends AbstractAggregationBuffer
        {
            ...
            {
                JavaDataModel model = JavaDataModel.get();
                return (int) (model.lengthFor(columnType) + model.primitive1() + model.primitive2() + ((numDV == null) ? lengthFor(model, null) : numDV.lengthFor(model)));
            }

            protected void initNDVEstimator(String func, int numBitVectors)
            {
                numDV = NumDistinctValueEstimatorFactory.getEmptyNumDistinctValueEstimator(func, numBitVectors);
            }
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException
        {
            ...
            if (myagg.numDV == null)
            {
                ...
                func = parameters[1] == null ? "fm" : PrimitiveObjectInspectorUtils.getString(parameters[1], funcOI);
                if (parameters.length == 3)
                {
                    numVectors = parameters[2] == null ? 0 : PrimitiveObjectInspectorUtils.getInt(parameters[2], numVectorsOI);
        ...
        public void merge(AggregationBuffer agg, Object partial) throws HiveException
        {
            if (partial != null)
            {
                ...
                if (buf != null && buf.length != 0)
                {
                    ...
                    {
                        myagg.numDV.mergeEstimators(NumDistinctValueEstimatorFactory.getNumDistinctValueEstimator(buf));
                    }
                }
            }
        }
    ...
    public static class GenericUDAFStringStatsEvaluator extends GenericUDAFEvaluator
    {
        ...
        public static class StringStatsAgg extends AbstractAggregationBuffer
        {
            ...
            {
                JavaDataModel model = JavaDataModel.get();
                return (int) (model.primitive1() * 2 + model.primitive2() * 4 + model.lengthFor(columnType) + ((numDV == null) ? lengthFor(model, null) : numDV.lengthFor(model)));
        ...
        {
            aggBuffer.numDV = NumDistinctValueEstimatorFactory.getEmptyNumDistinctValueEstimator(func, numBitVectors);
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException
        {
            ...
            {
                int numVectors = 0;
                String func = parameters[1] == null ? "fm" : PrimitiveObjectInspectorUtils.getString(parameters[1], funcOI);
                if (parameters.length > 2)
                {
    ...
    public static class GenericUDAFDecimalStatsEvaluator extends GenericUDAFNumericStatsEvaluator<HiveDecimal, HiveDecimalObjectInspector>
    {
        ...
        public class DecimalStatsAgg extends NumericStatsAgg
        {
            ...
            {
                if ((minValue != null) && (min == null || min.compareTo(minFieldOI.getPrimitiveJavaObject(minValue)) > 0))
                {
            ...
            {
                if ((maxValue != null) && (max == null || max.compareTo(maxFieldOI.getPrimitiveJavaObject(maxValue)) < 0))
                {
    ...
    public static class GenericUDAFDateStatsEvaluator extends GenericUDAFNumericStatsEvaluator<DateWritableV2, DateObjectInspector>
    {
        ...
        public class DateStatsAgg extends NumericStatsAgg
        {
            ...
            {
                if ((minValue != null) && (min == null || min.compareTo(minFieldOI.getPrimitiveWritableObject(minValue)) > 0))
                {
                    // DateWritableV2 is mutable, DateStatsAgg needs its own copy
            ...
            {
                if ((maxValue != null) && (max == null || max.compareTo(maxFieldOI.getPrimitiveWritableObject(maxValue)) < 0))
                {
                    // DateWritableV2 is mutable, DateStatsAgg needs its own copy
    ...
    static int lengthFor(JavaDataModel model, Integer numVector)
    {
        ...
        if (numVector > 0)
        {
            ...
            length += (model.object() + model.array() + model.primitive1() + model.primitive2()) * numVector; // bitset array
        }
        return length;
    }
}
