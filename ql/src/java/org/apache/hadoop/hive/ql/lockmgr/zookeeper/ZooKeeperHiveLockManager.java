...
public class ZooKeeperHiveLockManager implements HiveLockManager
{
     ...
     **/
    @Override
    public List<HiveLock> lock(List<HiveLockObj> lockObjects, boolean keepAlive, DriverState driverState) throws LockException
    {
    ...
    /**
     * @param hiveLocks list of hive locks to be released Release all the locks specified. If some of the
    ...
    public void releaseLocks(List<HiveLock> hiveLocks)
    {
        if (hiveLocks != null)
        {
            ...
            for (int pos = len - 1; pos >= 0; pos--)
            {
                ...
                try
                {
                    LOG.debug("About to release lock for {}", hiveLock.getHiveLockObject().getName());
                    unlock(hiveLock);
                } catch (LockException e)
                {
     ...
     * @param keepAlive Whether the lock is to be persisted after the statement Acquire the
    ...
    {
        LOG.debug("Acquiring lock for {} with mode {} {}", key.getName(), mode, key.getData().getLockMode());
        ...
        do
        {
            ...
            } catch (Exception e)
            {
                if (tryNum >= numRetriesForUnLock)
                {
                    ...
                    throw new LockException("Node " + name + " can not be deleted after " + numRetriesForUnLock + " attempts.", e);
                }
            }
     ...
     **/
    private static HiveLockObject getLockObject(HiveConf conf, String path, HiveLockMode mode, HiveLockObjectData data, String parent, boolean verifyTablePartition) throws LockException
    {
        try
        {
