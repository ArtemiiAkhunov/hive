...
final class MajorQueryCompactor extends QueryCompactor
{
    ...
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MAJOR, CompactionQueryBuilder.Operation.CREATE, false, fullName).setSourceTab(t).setLocation(tmpTableLocation).build());
    }

    private List<String> getCompactionQueries(Table t, Partition p, String tmpName)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MAJOR, CompactionQueryBuilder.Operation.INSERT, false, tmpName).setSourceTab(t).setSourcePartition(p).build());
    }

    private List<String> getDropQueries(String tmpTableName)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MAJOR, CompactionQueryBuilder.Operation.DROP, false, tmpTableName).build());
    }
}
