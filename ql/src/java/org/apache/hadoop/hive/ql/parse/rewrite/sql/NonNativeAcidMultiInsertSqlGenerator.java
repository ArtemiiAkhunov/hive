...
public class NonNativeAcidMultiInsertSqlGenerator extends MultiInsertSqlGenerator
{
    ...
    private void appendAcidSelectColumns(Context.Operation operation, boolean markRowIdAsDeleted, boolean skipPrefix)
    {
        ...
        for (FieldSchema fieldSchema : acidSelectColumns)
        {
            ...
            String identifier = deletedRowId ? "-1" : HiveUtils.unparseIdentifier(fieldSchema.getName(), this.conf);
            ...
            if (StringUtils.isNotEmpty(deletePrefix) && (!markRowIdAsDeleted || !skipPrefix))
            {
                ...
                String prefixedIdentifier = deletedRowId ? "-1" : HiveUtils.unparseIdentifier(deletePrefix + fieldSchema.getName(), this.conf);
    ...
    {
        return targetTable.getStorageHandler().acidSortColumns(targetTable, Context.Operation.DELETE).stream().map(fieldSchema -> qualify(HiveUtils.unparseIdentifier(deletePrefix + fieldSchema.getName(), this.conf))).collect(Collectors.toList());
    }
}
