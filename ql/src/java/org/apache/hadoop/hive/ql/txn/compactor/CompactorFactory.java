...
public final class CompactorFactory
{
    ...
    public CompactorPipeline getCompactorPipeline(Table table, HiveConf configuration, CompactionInfo compactionInfo, IMetaStoreClient msc) throws HiveException
    {
        ...
        {
            if (!"tez".equalsIgnoreCase(HiveConf.getVar(configuration, HiveConf.ConfVars.HIVE_EXECUTION_ENGINE)) || !HiveConf.getBoolVar(configuration, HiveConf.ConfVars.COMPACTOR_CRUD_QUERY_BASED))
            {
        ...
        } else if (MetaStoreUtils.isIcebergTable(table.getParameters()))
        {
            switch (compactionInfo.type)
            {
                case MAJOR:
                    ...
                    try
                    {
                        Class<? extends QueryCompactor> icebergMajorQueryCompactor = (Class<? extends QueryCompactor>) Class.forName(ICEBERG_MAJOR_QUERY_COMPACTOR_CLASS, true, Utilities.getSessionSpecifiedClassLoader());
