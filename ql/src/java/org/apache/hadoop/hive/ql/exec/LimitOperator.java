...
public class LimitOperator extends Operator<LimitDesc> implements Serializable
{
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        {
            LOG.info("LimitOperator exits early as query limit already reached: {} >= {}", currentCountForAllTasksInt, limit);
            onLimitReached();
        }
    }
    ...
    public void process(Object row, int tag) throws HiveException
    {
        ...
        if (offset <= currCount && currCount < (offset + limit) && offset <= currentCountForAllTasksInt && currentCountForAllTasksInt < (offset + limit))
        {
    ...
    private static boolean checkLimitReached(JobConf jobConf, String queryId, String limitReachedKey)
    {
        try
        {
            return ObjectCacheFactory.getCache(jobConf, queryId, false, true).retrieve(limitReachedKey, new Callable<AtomicBoolean>()
            {
                @Override
                public AtomicBoolean call()
                {
