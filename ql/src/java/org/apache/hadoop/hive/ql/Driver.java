...
public class Driver implements IDriver
{
    ...
    {
        driverContext = new DriverContext(queryState, queryInfo, new HookRunner(queryState.getConf(), CONSOLE), txnManager);
    ...
    private void runInternal(String command, boolean alreadyCompiled) throws CommandProcessorException
    {
        ...
        try
        {
            HiveDriverRunHookContext hookContext = new HiveDriverRunHookContextImpl(driverContext.getConf(), alreadyCompiled ? context.getCmd() : command);
    ...
    private boolean validateTxnList() throws CommandProcessorException
    {
        ...
        int maxRetrySnapshotCount = HiveConf.getIntVar(driverContext.getConf(), HiveConf.ConfVars.HIVE_TXN_MAX_RETRYSNAPSHOT_COUNT);
        ...
        try
        {
            do
            {
                ...
                if (!driverTxnHandler.isValidTxnListState())
                {
                    ...
                    driverContext.getConf().set(ValidTxnList.VALID_TXNS_KEY, driverContext.getTxnManager().getValidTxns().toString());
    ...
    private boolean shouldSetTxnManager(int retryShapshotCount, int maxRetrySnapshotCount) throws CommandProcessorException
    {
        if (retryShapshotCount > maxRetrySnapshotCount)
        {
            // Throw exception
            HiveException e = new HiveException("Operation could not be executed, " + SNAPSHOT_WAS_OUTDATED_WHEN_LOCKS_WERE_ACQUIRED + ".");
    ...
    private void runPreDriverHooks(HiveDriverRunHookContext hookContext) throws CommandProcessorException
    {
        ...
        } catch (Exception e)
        {
            ...
            throw DriverUtils.createProcessorException(driverContext, 12, errorMessage, ErrorMsg.findSQLState(e.getMessage()), e);
        }
    }
    ...
    public void lockAndRespond() throws CommandProcessorException
    {
        ...
        {
            throw new IllegalStateException("No previously compiled query for driver - queryId=" + driverContext.getQueryState().getQueryId());
        }
    ...
    {
        String queryId = Strings.isNullOrEmpty(driverContext.getQueryState().getQueryId()) ? QueryPlan.makeQueryId() : driverContext.getQueryState().getQueryId();
    ...
    public boolean hasResultSet()
    {
        ...
        return driverContext.getPlan().getFetchTask() != null && driverContext.getPlan().getResultSchema() != null && driverContext.getPlan().getResultSchema().isSetFieldSchemas();
    }
    ...
    private void releaseContext()
    {
        try
        {
            if (context != null)
            {
                ...
                if (driverContext.getCacheUsage() != null && driverContext.getCacheUsage().getStatus() == CacheUsage.CacheStatus.QUERY_USING_CACHE)
                {
    ...
    private boolean hasBadCacheAttempt()
    {
        ...
        return (driverContext.getCacheUsage() != null && driverContext.getCacheUsage().getStatus() == CacheUsage.CacheStatus.CAN_CACHE_QUERY_RESULTS && driverContext.getCacheUsage().getCacheEntry() != null);
    }
