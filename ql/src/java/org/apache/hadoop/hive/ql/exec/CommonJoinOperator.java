...
public abstract class CommonJoinOperator<T extends JoinDesc> extends Operator<T> implements Serializable
{
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(CommonJoinOperator.class.getName());
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        joinEmitInterval = HiveConf.getIntVar(hconf, HiveConf.ConfVars.HIVE_JOIN_EMIT_INTERVAL);
        joinCacheSize = HiveConf.getIntVar(hconf, HiveConf.ConfVars.HIVE_JOIN_CACHE_SIZE);
            ...
            if (aliasNum == numAliases - 1)
            {
                if (!(allLeftNull && rightNull))
                {
                    ...
                    if (needsPostEvaluation)
                    {
                        ...
                        boolean forward = createForwardJoinObject(skipVectors[numAliases - 1], type == JoinDesc.ANTI_JOIN);
                        producedRow |= forward;
    ...
    {
        return !skip[left] && !skip[right] && isLeftValid(left, right) && isRightValid(left, right);
    }
    ...
    protected void checkAndGenObject() throws HiveException
    {
        if (closeOpCalled)
        {
            LOG.warn("checkAndGenObject is called after operator " + id + " " + getName() + " called closeOp");
            return;
        }
