...
public class VectorAssignRow
{
    ...
    public void init(List<String> typeNames) throws HiveException
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            final TypeInfo typeInfo = TypeInfoUtils.getTypeInfoFromTypeString(typeNames.get(i));
     ...
     * @param columnsToIncludeTruncated Flag array indicating which columns are to be included.
    ...
    public void assignConvertRowColumn(VectorizedRowBatch batch, int batchIndex, int logicalColumnIndex, Object object)
    {
        ...
        final int projectionColumnNum = projectionColumnNums[logicalColumnIndex];
        assignConvertRowColumn(batch.cols[projectionColumnNum], batchIndex, targetTypeInfos[logicalColumnIndex], convertSourceOI[logicalColumnIndex], convertTargetWritables[logicalColumnIndex], object);
    }
