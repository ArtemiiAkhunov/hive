...
public class DemuxOperator extends Operator<DemuxDesc> implements Serializable
{
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        try
        {
            ...
            for (int i = 0; i < newTagToOldTag.length; i++)
            {
                ...
                AbstractSerDe inputValueDeserializer = ReflectionUtil.newInstance(valueTableDesc.getSerDeClass(), null);
    ...
    protected void initializeChildren(Configuration hconf) throws HiveException
    {
        ...
        {
            LOG.info("Initializing child " + i + " " + childOperatorsArray[i].getIdentifier() + " " + childOperatorsArray[i].getName() + " " + childInputObjInspectors[i].length);
    ...
    protected void closeOp(boolean abort) throws HiveException
    {
        for (int i = 0; i < newTagToOldTag.length; i++)
        {
            ...
            int oldTag = newTagToOldTag[i];
            int childIndex = newTagToChildIndex[newTag];
            LOG.info(id + " (newTag, childIndex, oldTag)=(" + newTag + ", " + childIndex + ", " + oldTag + "),  forwarded " + cntrs[newTag] + " rows");
        }
    }
