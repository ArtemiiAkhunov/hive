...
public class ScriptOperator extends Operator<ScriptDesc> implements Serializable
{
    ...
    boolean blackListed(Configuration conf, String name)
    {
        if (blackListedConfEntries == null)
        {
            ...
            if (conf != null)
            {
                String bl = conf.get(HiveConf.ConfVars.HIVE_SCRIPT_ENV_BLACKLIST.toString(), HiveConf.ConfVars.HIVE_SCRIPT_ENV_BLACKLIST.getDefaultValue());
    ...
    void addJobConfToEnvironment(Configuration conf, Map<String, String> env)
    {
        ...
        while (it.hasNext())
        {
            ...
            if (!blackListed(conf, name))
            {
                ...
                boolean truncate = conf.getBoolean(HiveConf.ConfVars.HIVE_SCRIPT_TRUNCATE_ENV.toString(), false);
    ...
    public void process(Object row, int tag) throws HiveException
    {
        ...
        if (firstRow)
        {
            ...
            try
            {
                ...
                env.put(safeEnvVarName(HiveConf.ConfVars.HIVE_ALIAS.varname), String.valueOf(alias));

                // Create an environment variable that uniquely identifies this script
                // operator
                String idEnvVarName = HiveConf.getVar(hconf, HiveConf.ConfVars.HIVE_SCRIPT_ID_ENV_VAR);
                ...
                errThread = new StreamThread(scriptErrReader, new ErrorStreamProcessor(HiveConf.getIntVar(hconf, HiveConf.ConfVars.SCRIPT_ERROR_LIMIT)), "ErrorProcessor");

                if (HiveConf.getBoolVar(hconf, HiveConf.ConfVars.HIVE_SCRIPT_AUTO_PROGRESS))
                {
