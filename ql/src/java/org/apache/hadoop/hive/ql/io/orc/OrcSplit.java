...
public class OrcSplit extends FileSplit implements ColumnarSplit, LlapAwareSplit
{
    ...
    public OrcSplit(Path path, Object fileId, long offset, long length, String[] hosts, OrcTail orcTail, boolean isOriginal, boolean hasBase, List<AcidInputFormat.DeltaMetaData> deltas, long projectedDataSize, long fileLen, Path rootDir, OffsetAndBucketProperty syntheticAcidProps)
    {
        ...
        long minWriteId = !deltas.isEmpty() ? AcidUtils.parseBaseOrDeltaBucketFilename(path, null).getMinimumWriteId() : -1;
        this.deltas.addAll(deltas.stream()
                // filtering out delete deltas with transactions happened before the transactions of the split
                .filter(delta -> delta.getMaxWriteId() >= minWriteId).flatMap(delta -> filterDeltasByBucketId(delta, bucketId)).collect(Collectors.toList()));
    ...
    {
        Map<Integer, AcidInputFormat.DeltaFileMetaData> bucketFilesbyStmtId = dmd.getDeltaFiles().stream().filter(deltaFileMetaData -> deltaFileMetaData.getBucketId() == bucketId).collect(Collectors.toMap(AcidInputFormat.DeltaFileMetaData::getStmtId, Function.identity()));
    ...
    public void write(DataOutput out) throws IOException
    {
        ...
        {
            LOG.trace("Writing additional {} bytes to OrcSplit as payload. Required {} bytes.", additional, required);
        }
    }
    ...
    private void writeAdditionalPayload(final DataOutputStream out) throws IOException
    {
        ...
        int flags = (hasBase ? BASE_FLAG : 0) | (isOriginal ? ORIGINAL_FLAG : 0) | (hasFooter ? FOOTER_FLAG : 0) | (isFileIdLong ? HAS_LONG_FILEID_FLAG : 0) | (isFileIdWritable ? HAS_SYNTHETIC_FILEID_FLAG : 0) | (syntheticAcidProps != null ? HAS_SYNTHETIC_ACID_PROPS_FLAG : 0);
        out.writeByte(flags);
    ...
    public boolean canUseLlapIo(Configuration conf)
    {
        ...
        {
            if (HiveConf.getBoolVar(conf, ConfVars.LLAP_IO_ACID_ENABLED) && Utilities.getIsVectorized(conf))
            {
            ...
            {
                LOG.info("Skipping Llap IO based on the following: [vectorized={}, hive.llap.io.acid={}] for {}", Utilities.getIsVectorized(conf), HiveConf.getBoolVar(conf, ConfVars.LLAP_IO_ACID_ENABLED), this);
                return false;
            }
        } else
        {
    ...
    {
        OrcRawRecordMerger.TransactionMetaData tmd = OrcRawRecordMerger.TransactionMetaData.findWriteIDForSynthetcRowIDs(getPath(), rootPath, conf);
    ...
    {
        return "OrcSplit [" + getPath() + ", start=" + getStart() + ", length=" + getLength() + ", isOriginal=" + isOriginal + ", fileLength=" + fileLen + ", hasFooter=" + hasFooter + ", hasBase=" + hasBase + ", deltas=" + (deltas == null ? 0 : deltas.size()) + "]";
    }
}
