...
public class HashTableLoader implements org.apache.hadoop.hive.ql.exec.HashTableLoader
{
    ...
    public void load(MapJoinTableContainer[] mapJoinTables, MapJoinTableContainerSerDe[] mapJoinTableSerdes) throws HiveException
    {
        ...
        boolean useOptimizedTables = HiveConf.getBoolVar(hconf, HiveConf.ConfVars.HIVE_MAPJOIN_USE_OPTIMIZED_TABLE);
        ...
        {
            totalMapJoinMemory = HiveConf.getLongVar(hconf, HiveConf.ConfVars.HIVE_CONVERT_JOIN_NOCONDITIONAL_TASK_THRESHOLD);
        }

        long processMaxMemory = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getMax();
        if (totalMapJoinMemory > processMaxMemory)
        {
            float hashtableMemoryUsage = HiveConf.getFloatVar(hconf, HiveConf.ConfVars.HIVE_HASHTABLE_FOLLOWBY_GBY_MAX_MEMORY_USAGE);
        ...
        if (useHybridGraceHashJoin && mapJoinTables.length > 2)
        {
            ...
            try
            {
                numPartitions = HybridHashTableContainer.calcNumPartitions(memory, maxSize, HiveConf.getIntVar(hconf, HiveConf.ConfVars.HIVE_HYBRIDGRACE_HASHJOIN_MIN_NUM_PARTITIONS), HiveConf.getIntVar(hconf, HiveConf.ConfVars.HIVE_HYBRIDGRACE_HASHJOIN_MIN_WB_SIZE));
            } catch (IOException e)
            {
