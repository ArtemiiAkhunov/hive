...
public class HiveRelMdRowCount extends RelMdRowCount
{
    ...
    public Double getRowCount(HiveJoin join, RelMetadataQuery mq)
    {
        ...
        final Pair<PKFKRelationInfo, RexNode> constraintBasedResult = constraintsBasedAnalyzeJoinForPKFK(join, mq);
        if (constraintBasedResult != null)
        {
            ...
            double joinSelectivity = Math.min(1.0, constraintBasedResult.left.pkInfo.selectivity * constraintBasedResult.left.ndvScalingFactor);
            ...
            {
                LOG.debug("Identified Primary - Foreign Key relation from constraints:\n {} {} Row count for join: {}\n" + " Join selectivity: {}\n Residual selectivity: {}\n", RelOptUtil.toString(join), constraintBasedResult.left, rowCount, joinSelectivity, residualSelectivity);
            }
            return rowCount;
        }
    ...
    public static PKFKRelationInfo analyzeJoinForPKFK(Join joinRel, RelMetadataQuery mq)
    {
         ...
         * null.
         */
        boolean leftIsKey = (joinRel.getJoinType() == JoinRelType.INNER || joinRel.getJoinType() == JoinRelType.RIGHT) && isKey(lBitSet, left, mq);
        ...
        boolean isPKSideSimpleTree = leftIsKey ? SimpleTreeOnJoinKey.check(false, left, lBitSet, mq).left : SimpleTreeOnJoinKey.check(false, right, rBitSet, mq).left;
    ...
    public static Pair<PKFKRelationInfo, RexNode> constraintsBasedAnalyzeJoinForPKFK(Join join, RelMetadataQuery mq)
    {
        ...
        RelOptUtil.classifyFilters(join, joinFilters, join.getJoinType(), false, !join.getJoinType().generatesNullsOnRight(), !join.getJoinType().generatesNullsOnLeft(), joinFilters, leftFilters, rightFilters);

        // 3) Check if we are joining on PK-FK
        final PKFKJoinInfo leftInputResult = HiveRelOptUtil.extractPKFKJoin(join, joinFilters, false, mq);
        final PKFKJoinInfo rightInputResult = HiveRelOptUtil.extractPKFKJoin(join, joinFilters, true, mq);
        ...
        boolean rightIsKey = (join.getJoinType() == JoinRelType.INNER || join.isSemiJoin() || join.getJoinType() == JoinRelType.ANTI || join.getJoinType() == JoinRelType.LEFT) && rightInputResult.isPkFkJoin;
        if (!leftIsKey && !rightIsKey)
        {
            // Nothing to do here, bail out
            return null;
        ...
        RexNode residualCond = residualFilters.isEmpty() ? null : residualFilters.size() == 1 ? residualFilters.get(0) : rexBuilder.makeCall(SqlStdOperatorTable.AND, residualFilters);
        ...
        if (pkSide == 0)
        {
            ...
            return Pair.of(new PKFKRelationInfo(1, fkInfo, pkInfo, ndvScalingFactor, isNoFilteringPKSideTree), residualCond);
        } else
        { // pkSide == 1
            FKSideInfo fkInfo = new FKSideInfo(leftRowCount, leftNDV);
            ...
            return Pair.of(new PKFKRelationInfo(0, fkInfo, pkInfo, ndvScalingFactor, isNoFilteringPKSideTree), residualCond);
        }
    }
