...
class VectorSpillBlockContainer
{
    ...
    class VectorSpillBlock
    {
        ...
        {
            bufferedBatchVectorDeserializeRow = new VectorDeserializeRow<LazyBinaryDeserializeRead>(new LazyBinaryDeserializeRead(bufferedTypeInfos,
                    /* useExternalBuffer */ true));

            // Deserialize the fields into a batch using the buffered batch column map.
            bufferedBatchVectorDeserializeRow.init(doFinalRead ? bufferedColumnMap : IntStream.range(0, bufferedColumnMap.length).toArray());
        }
        ...
        {
            return String.format("[%s: blockIndex: %d, startBatchIndex: %d, startRowIndex: %s, spilled: %s, spillBatchCount: %d, spillRowCount: %d]", getClass().getSimpleName(), blockIndex, startBatchIndex, Arrays.toString(startRowIndex), didSpillToDisk, spillBatchCount, spillRowCount);
        }
    ...
    VectorSpillBlock getCurrentBlock()
    {
        ...
        if (spillBlockIndex > blocks.size() - 1)
        {
            ...
            {
                block.startRowIndex[0] = (int) (blocks.get(spillBlockIndex - 1).startRowIndex[0] + blocks.get(spillBlockIndex - 1).spillRowCount);
            }
            blocks.add(block);
        } else
        {
