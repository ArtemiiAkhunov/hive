...
public class AlterMaterializedViewRebuildAnalyzer extends CalcitePlanner
{
    ...
    protected class MVRebuildCalcitePlannerAction extends CalcitePlannerAction
    {
        ...
        protected RelNode applyMaterializedViewRewriting(RelOptPlanner planner, RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            try
            {
                ...
                materialization = db.getMaterializedViewForRebuild(mvTable.getDbName(), mvTable.getTableName(), tablesUsedQuery, getTxnMgr());
            ...
            generatePartialProgram(program, true, HepMatchOrder.TOP_DOWN, HiveMaterializedViewRule.MATERIALIZED_VIEW_REWRITING_RULES);
            ...
            if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_MATERIALIZED_VIEW_REBUILD_INCREMENTAL))
            {
                ...
                RelNode incrementalRebuildPlan = applyRecordIncrementalRebuildPlan(basePlan, mdProvider, executorProvider, optCluster, calcitePreMVRewritingPlan, materialization);
                ...
                return applyPartitionIncrementalRebuildPlan(basePlan, mdProvider, executorProvider, materialization, optCluster, calcitePreMVRewritingPlan);
            }
        ...
        private RelNode applyRecordIncrementalRebuildPlan(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider, RelOptCluster optCluster, RelNode calcitePreMVRewritingPlan, HiveRelOptMaterialization materialization)
        {
            ...
            boolean acidView = AcidUtils.isFullAcidTable(mvTable.getTTable()) || AcidUtils.isNonNativeAcidTable(mvTable);
        ...
        private RelNode applyAggregateInsertDeleteIncremental(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            return applyIncrementalRebuild(basePlan, mdProvider, executorProvider, HiveAggregateInsertDeleteIncrementalRewritingRule.INSTANCE);
        }

        private RelNode applyAggregateInsertIncremental(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider, RelOptCluster optCluster, RelNode calcitePreMVRewritingPlan)
        {
            mvRebuildMode = MaterializationRebuildMode.AGGREGATE_INSERT_REBUILD;
            basePlan = applyIncrementalRebuild(basePlan, mdProvider, executorProvider, HiveInsertOnlyScanWriteIdRule.INSTANCE, HiveAggregateInsertIncrementalRewritingRule.INSTANCE);
            ...
            try
            {
                ...
                final double factorSelectivity = HiveConf.getFloatVar(conf, HiveConf.ConfVars.HIVE_MATERIALIZED_VIEW_REBUILD_INCREMENTAL_FACTOR);
        ...
        private RelNode applyJoinInsertIncremental(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            return applyIncrementalRebuild(basePlan, mdProvider, executorProvider, HiveInsertOnlyScanWriteIdRule.INSTANCE, HiveJoinInsertIncrementalRewritingRule.INSTANCE);
        }
        ...
        private RelNode applyPartitionIncrementalRebuildPlan(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider, HiveRelOptMaterialization materialization, RelOptCluster optCluster, RelNode calcitePreMVRewritingPlan)
        {
            ...
            RelNode incrementalRebuildPlan = applyIncrementalRebuild(basePlan, mdProvider, executorProvider, HiveInsertOnlyScanWriteIdRule.INSTANCE, HiveAggregatePartitionIncrementalRewritingRule.INSTANCE);

            // Make a cost-based decision factoring the configuration property
            RelOptCost costOriginalPlan = calculateCost(optCluster, mdProvider, HiveTezModelRelMetadataProvider.DEFAULT, calcitePreMVRewritingPlan);

            RelOptCost costIncrementalRebuildPlan = calculateCost(optCluster, mdProvider, HiveIncrementalRelMdRowCount.createMetadataProvider(materialization), incrementalRebuildPlan);
    ...
    {
        ASTNode updateNode = new CalcitePlanner.ASTSearcher().simpleBreadthFirstSearch(newAST, HiveParser.TOK_QUERY, HiveParser.TOK_INSERT);
        ASTNode subqueryNodeInputROJ = new CalcitePlanner.ASTSearcher().simpleBreadthFirstSearch(newAST, HiveParser.TOK_QUERY, HiveParser.TOK_FROM, HiveParser.TOK_RIGHTOUTERJOIN, HiveParser.TOK_SUBQUERY);
    ...
    private void fixUpASTAggregateIncrementalRebuild(ASTNode subqueryNodeInputROJ, ASTNode updateInsertNode, Map<Context.DestClausePrefix, ASTNode> disjuncts, MaterializedViewASTBuilder astBuilder) throws SemanticException
    {
        ...
        astBuilder.createAcidSortNodes(TableName.getDbTable(materializationNode.getChild(0).getText(), materializationNode.getChild(1).getText())).forEach(astNode -> ParseDriver.adaptor.addChild(selectNodeInputROJ, astNode));
    ...
    private void fixUpASTAggregateInsertDeleteIncrementalRebuild(ASTNode newAST, MaterializedViewASTBuilder astBuilder) throws SemanticException
    {
        ...
        {
            if (disjunct.getChild(0).getType() == HiveParser.TOK_FUNCTION && disjunct.getChild(0).getChild(0).getType() == HiveParser.Identifier && "isnull".equals(disjunct.getChild(0).getChild(0).getText()))
            {
                disjunctMap.put(Context.DestClausePrefix.INSERT, disjunct);
            } else if (disjunct.getChild(0).getType() != HiveParser.TOK_FUNCTION && new ASTSearcher().simpleBreadthFirstSearch(disjunct, HiveParser.KW_AND, HiveParser.KW_OR, HiveParser.KW_AND, HiveParser.EQUAL) != null)
            {
                disjunctMap.put(Context.DestClausePrefix.DELETE, disjunct);
            } else if (disjunct.getChild(0).getType() != HiveParser.TOK_FUNCTION && new ASTSearcher().simpleBreadthFirstSearch(disjunct, HiveParser.KW_AND, HiveParser.KW_OR, HiveParser.KW_AND, HiveParser.GREATERTHAN) != null)
            {
    ...
    private void addDeleteBranch(ASTNode updateNode, ASTNode subqueryNodeInputROJ, ASTNode predicate, MaterializedViewASTBuilder astBuilder) throws SemanticException
    {
        ...
        astBuilder.createDeleteSelectNodes(subqueryNodeInputROJ.getChild(1).getText()).forEach(astNode -> ParseDriver.adaptor.addChild(selectNodeInDelete, astNode));
    ...
    private void fixUpASTJoinInsertIncrementalRebuild(ASTNode newAST) throws SemanticException
    {
        ...
        //      TOK_SELECT
        //         ...
        ASTNode dest = new ASTSearcher().simpleBreadthFirstSearch(newAST, HiveParser.TOK_QUERY, HiveParser.TOK_INSERT, HiveParser.TOK_DESTINATION);
        ASTNode newChild = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_INSERT_INTO, "TOK_INSERT_INTO");
