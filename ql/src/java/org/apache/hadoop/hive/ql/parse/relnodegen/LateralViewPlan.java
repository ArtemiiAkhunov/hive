...
public class LateralViewPlan
{
    ...
    public static final ImmutableSet<Integer> TABLE_ALIAS_TOKEN_TYPES = ImmutableSet.of(HiveParser.TOK_SUBQUERY, HiveParser.TOK_TABREF, HiveParser.TOK_PTBLFUNCTION);
    ...
    public LateralViewPlan(ASTNode lateralView, RelOptCluster cluster, RelNode inputRel, RowResolver inputRR, UnparseTranslator unparseTranslator, HiveConf conf, FunctionHelper functionHelper) throws SemanticException
    {
        ...
        this.lateralViewRel = HiveTableFunctionScan.create(cluster, TraitsUtil.getDefaultTraitSet(cluster), ImmutableList.of(inputRel), udtfCall, null, retType, createColumnMappings(inputRel));
    }
    ...
    public static void validateLateralView(ASTNode lateralView) throws SemanticException
    {
        ...
        if (!TABLE_ALIAS_TOKEN_TYPES.contains(next.getToken().getType()) && HiveParser.TOK_LATERAL_VIEW != next.getToken().getType())
        {
    ...
    private List<String> getColumnAliasesFromASTNode(ASTNode selExprClause, RexCall udtfCall) throws SemanticException
    {
        ...
        for (Node obj : selExprClause.getChildren())
        {
            ...
            if (child.getToken().getType() == HiveParser.TOK_TABALIAS || child.getToken().getType() == HiveParser.TOK_FUNCTION)
            {
                continue;
            }
        ...
        {
            colAliases.addAll(Lists.transform(udtfCall.getType().getFieldList(), RelDataTypeField::getName));
        }
    ...
    private RowResolver getOutputRR(RowResolver inputRR, RexCall udtfCall, List<String> columnAliases, String lateralTableAlias) throws SemanticException
    {
        ...
        for (int i = 0, j = 0; i < columnAliases.size(); i++)
        {
            ...
            localOutputRR.put(lateralTableAlias, columnAliases.get(i), new ColumnInfo(internalColName, typeInfos.getAllStructFieldTypeInfos().get(i), lateralTableAlias, false));
        }
    ...
    private RelDataType getRetType(RelOptCluster cluster, RelNode inputRel, RexNode udtfCall, List<String> columnAliases)
    {
        ...
        List<String> allDataTypeNames = new ArrayList<>(Lists.transform(inputRel.getRowType().getFieldList(), RelDataTypeField::getName));
