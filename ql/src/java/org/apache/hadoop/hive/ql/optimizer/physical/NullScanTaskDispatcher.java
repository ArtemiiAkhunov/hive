...
public class NullScanTaskDispatcher implements SemanticDispatcher
{
    ...
    public NullScanTaskDispatcher(PhysicalContext context, Map<SemanticRule, SemanticNodeProcessor> rules)
    {
        ...
        this.rules = rules;
        this.maxAsyncLookupCount = physicalContext.getConf().getIntVar(HiveConf.ConfVars.HIVE_COMPUTE_SPLITS_NUM_THREADS);
    }
    ...
    private void lookupAndProcessPath(MapWork work, Path path, Collection<String> aliasesToOptimize)
    {
        ...
        {
            LOG.warn("Could not determine if path {} was empty." + "Cannot use null scan optimization for this path.", path, e);
        }
    }
    ...
    private void processTableScans(MapWork work, Set<TableScanOperator> tableScans)
    {
        ...
        { // don't create executor service for one partition
    ...
    private Map<Path, Partition> getPathToPartitionMap(String alias, TableScanOperator tso)
    {
        ...
        try
        {
            Set<Partition> partitions = physicalContext.getParseContext().getPrunedPartitions(alias, tso).getPartitions();
        ...
        {
            LOG.warn("Error while determining partitions of {}." + "We cannot determine its empty partitions from stats.", alias, e);
        }
