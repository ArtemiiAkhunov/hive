...
public class LoadDatabase
{
    ...
    {
        if (context.hiveConf.getBoolVar(HiveConf.ConfVars.REPL_RETAIN_CUSTOM_LOCATIONS_FOR_DB_ON_TARGET) && Boolean.parseBoolean(dbInMetadata.getParameters().get(ReplConst.REPL_IS_CUSTOM_DB_LOC)))
        {
    ...
    {
        if (context.hiveConf.getBoolVar(HiveConf.ConfVars.REPL_RETAIN_CUSTOM_LOCATIONS_FOR_DB_ON_TARGET) && Boolean.parseBoolean(dbInMetadata.getParameters().get(ReplConst.REPL_IS_CUSTOM_DB_MANAGEDLOC)))
        {
    ...
    private boolean isDbAlreadyBootstrapped(Database db)
    {
        ...
        return ((props != null) && props.containsKey(ReplicationSpec.KEY.CURR_STATE_ID_SOURCE.toString()) && !props.get(ReplicationSpec.KEY.CURR_STATE_ID_SOURCE.toString()).isEmpty());
    }
    ...
    {
        return alterDbTask(dbObj.getName(), updateDbProps(dbObj, context.dumpDirectory), context.hiveConf, context.dumpDirectory, this.metricCollector);
    }
    ...
    public static class AlterDatabase extends LoadDatabase
    {
        ...
        public TaskTracker tasks() throws SemanticException
        {
            ...
            tracker.addTask(alterDbTask(dbObj.getName(), dbObj.getParameters(), context.hiveConf, context.dumpDirectory, this.metricCollector));
            return tracker;
        }
    }
}
