...
public class CastTimestampToVarCharWithFormat extends CastTimestampToVarChar
{
    ...
    public CastTimestampToVarCharWithFormat(int inputColumn, byte[] patternBytes, int len, int outputColumnNum)
    {
        ...
        {
            throw new IllegalStateException("Tried to cast (<timestamp> to varchar with format" + "<pattern>), but <pattern> not found");
        }
        sqlFormatter = new HiveSqlDateTimeFormatter(new String(patternBytes, StandardCharsets.UTF_8), false);
    }
