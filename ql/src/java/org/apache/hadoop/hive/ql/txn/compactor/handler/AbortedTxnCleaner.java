...
class AbortedTxnCleaner extends TaskHandler
{
    ...
    public List<Runnable> getTasks() throws MetaException
    {
        ...
        {
            return readyToCleanAborts.stream().map(info -> ThrowingRunnable.unchecked(() -> clean(info, info.minOpenWriteTxnId, metricsEnabled))).collect(Collectors.toList());
        }
    ...
    private void clean(CompactionInfo info, long minOpenWriteTxn, boolean metricsEnabled) throws MetaException, InterruptedException
    {
        ...
        {
            LOG.error("Caught an interrupted exception when cleaning, unable to complete cleaning of {} due to {}", info, e.getMessage());
            handleCleanerAttemptFailure(info, e.getMessage());
            throw e;
        } catch (Exception e)
        {
            LOG.error("Caught exception when cleaning, unable to complete cleaning of {} due to {}", info, e.getMessage());
            handleCleanerAttemptFailure(info, e.getMessage());
    ...
    {
        ValidTxnList validTxnList = TxnUtils.createValidTxnListForCleaner(txnHandler.getOpenTxns(), minOpenWriteTxn, true);
        ...
        info.highestWriteId = Math.min(isNull(validWriteIdList.getMinOpenWriteId()) ? Long.MAX_VALUE : validWriteIdList.getMinOpenWriteId() - 1, validWriteIdList.getHighWatermark());
