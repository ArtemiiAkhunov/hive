...
public class OrcOutputFormat extends FileOutputFormat<NullWritable, OrcSerdeRow> implements AcidOutputFormat<NullWritable, OrcSerdeRow>
{
    ...
    private static class OrcRecordWriter implements RecordWriter<NullWritable, OrcSerdeRow>, StatsProvidingRecordWriter
    {
        ...
        public void close(boolean b) throws IOException
        {
            if (writer == null)
            {
                ...
                FileSystem fs = options.getFileSystem() == null ? path.getFileSystem(options.getConfiguration()) : options.getFileSystem();
    ...
    {
        return new OrcRecordWriter(new Path(name), getOptions(conf, null));
    }
    ...
    private class DummyOrcRecordUpdater implements RecordUpdater
    {
        ...
        {
            out.println("insert " + path + " currWriteId: " + currentWriteId + " obj: " + stringifyObject(row, inspector));
        }

        @Override
        public void update(long currentWriteId, Object row) throws IOException
        {
            out.println("update " + path + " currWriteId: " + currentWriteId + " obj: " + stringifyObject(row, inspector));
        }
        ...
        private void stringifyObject(StringBuilder buffer, Object obj, ObjectInspector inspector) throws IOException
        {
            if (inspector instanceof StructObjectInspector)
            {
                ...
                for (StructField field : soi.getAllStructFieldRefs())
                {
                    ...
                    buffer.append(": ");
                    stringifyObject(buffer, soi.getStructFieldData(obj, field), field.getFieldObjectInspector());
                }
                buffer.append(" }");
    ...
    public org.apache.hadoop.hive.ql.exec.FileSinkOperator.RecordWriter getRawRecordWriter(Path path, Options options) throws IOException
    {
        ...
        {
            opts.bufferSize(OrcRecordUpdater.DELTA_BUFFER_SIZE).stripeSize(OrcRecordUpdater.DELTA_STRIPE_SIZE).blockPadding(false);
            if (!MetastoreConf.getBoolVar(options.getConfiguration(), MetastoreConf.ConfVars.COMPACTOR_MINOR_STATS_COMPRESSION))
            {
        ...
        return new org.apache.hadoop.hive.ql.exec.FileSinkOperator.RecordWriter()
        {
            ...
            public void write(Writable w) throws IOException
            {
                ...
                watcher.addKey(((IntWritable) orc.getFieldValue(OrcRecordUpdater.OPERATION)).get(), ((LongWritable) orc.getFieldValue(OrcRecordUpdater.ORIGINAL_WRITEID)).get(), ((IntWritable) orc.getFieldValue(OrcRecordUpdater.BUCKET)).get(), ((LongWritable) orc.getFieldValue(OrcRecordUpdater.ROW_ID)).get());
                writer.addRow(w);
            }
