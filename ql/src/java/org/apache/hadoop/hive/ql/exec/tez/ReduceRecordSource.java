...
public class ReduceRecordSource implements RecordSource
{
    ...
    private Object deserializeValue(BytesWritable valueWritable, byte tag) throws HiveException
    {
        ...
        {
            throw new HiveException("Hive Runtime Error: Unable to deserialize reduce input value (tag=" + tag + ") from " + Utilities.formatBinaryString(valueWritable.getBytes(), 0, valueWritable.getLength()) + " with properties " + valueTableDesc.getProperties(), e);
        }
    }
    ...
    private class GroupIterator
    {
        ...
        public void next() throws HiveException
        {
            ...
            } catch (Exception e)
            {
                ...
                // client application.
                l4j.trace("Hive Runtime Error while processing row (tag=" + tag + ") " + rowString);
    ...
    private void processVectorGroup(BytesWritable keyWritable, Iterable<Object> values, byte tag) throws HiveException, IOException
    {
        ...
        final int maxSize = (vectorizedTestingReducerBatchSize > 0 ? Math.min(vectorizedTestingReducerBatchSize, batch.getMaxSize()) : batch.getMaxSize());
        ...
        } catch (Exception e)
        {
            ...
            l4j.error("Hive Runtime Error while processing vector batch (tag=" + tag + ") (vectorizedVertexNum " + vectorizedVertexNum + ") " + rowString, e);
            throw new HiveException("Hive Runtime Error while processing vector batch (tag=" + tag + ") (vectorizedVertexNum " + vectorizedVertexNum + ")", e);
        }
    }
