...
public class MapJoinOperator extends AbstractMapJoinOperator<MapJoinDesc> implements Serializable
{
    ...
    private Pair<MapJoinTableContainer[], MapJoinTableContainerSerDe[]> loadHashTableInternal(ExecMapperContext mapContext, MapredContext mrContext) throws HiveException
    {
        ...
        Pair<MapJoinTableContainer[], MapJoinTableContainerSerDe[]> pair = new ImmutablePair<>(mapJoinTables, mapJoinTableSerdes);
    ...
    private Pair<MapJoinTableContainer[], MapJoinTableContainerSerDe[]> loadHashTableBMJ(ExecMapperContext mapContext, MapredContext mrContext) throws HiveException
    {
        ...
        LlapObjectSubCache<Pair<MapJoinTableContainer[], MapJoinTableContainerSerDe[]>> subCache = new LlapObjectSubCache<>(cache, cacheKey + "_BMJ", numBuckets);

        subCache.lock(bucketId);
        try
        {
            Pair<MapJoinTableContainer[], MapJoinTableContainerSerDe[]> pair = subCache.get(bucketId);
            if (pair != null)
            {
                // match found! use it
                // update the tables.
    ...
    protected Pair<MapJoinTableContainer[], MapJoinTableContainerSerDe[]> loadHashTable(ExecMapperContext mapContext, MapredContext mrContext) throws HiveException
    {
        ...
        if (conf.isBucketMapJoin() && cache instanceof LlapObjectCache && numBuckets > 0 && HiveConf.getBoolVar(hconf, ConfVars.HIVE_TEZ_BMJ_USE_SUBCACHE))
        {
    ...
    protected JoinUtil.JoinResult setMapJoinKey(ReusableGetAdaptor dest, Object row, byte alias) throws HiveException
    {
     ...
     * no NULLs.
     */
    protected JoinUtil.JoinResult setMapJoinKeyNoNulls(ReusableGetAdaptor dest, Object row, byte alias, MatchTracker matchTracker) throws HiveException
    {
            ...
            if (hashMapRowGetters == null)
            {
                ...
                for (byte pos = 0; pos < order.length; pos++)
                {
                    if (pos != alias)
                    {
                        ...
                        if (isFullOuterMapJoin)
                        {
                            matchTracker = ((ReusableGetAdaptorDirectAccess) hashMapRowGetters[pos]).createMatchTracker();
                        }
                    }
                }
            }
    ...
    private void createMatchTracker(MapJoinTableContainer smallTable)
    {
        ...
        matchTracker = ((ReusableGetAdaptorDirectAccess) hashMapRowGetter).createMatchTracker();
    ...
    protected void generateFullOuterSmallTableNoMatches(byte smallTablePos, MapJoinTableContainer substituteSmallTable) throws HiveException
    {
        ...
        if (smallTableValuesIndex == null)
        {
            ...
            smallTableValuesIndex = ArrayUtils.toPrimitive(valuesList.toArray(new Integer[0]));
         ...
         */
        NonMatchedSmallTableIterator nonMatchedIterator = substituteSmallTable.createNonMatchedSmallTableIterator(matchTracker);
        while (nonMatchedIterator.isNext())
        {
            ...
            for (List<Object> valueObjList = iter.first(); valueObjList != null; valueObjList = iter.next())
            {
                ...
                Object standardCopyRow = ObjectInspectorUtils.copyToStandardObject(row, outputObjInspector, ObjectInspectorCopyOption.WRITABLE);
    ...
    public void closeOp(boolean abort) throws HiveException
    {
        ...
        if (isFullOuterMapJoin)
        {
            ...
            generateFullOuterSmallTableNoMatches(smallTablePos, (MapJoinTableContainer) mapJoinTables[smallTablePos]);
        }
    ...
    {
        return !(mapContext == null || mapContext.getLocalWork() == null || mapContext.getLocalWork().getInputFileChangeSensitive() == false);
    }
