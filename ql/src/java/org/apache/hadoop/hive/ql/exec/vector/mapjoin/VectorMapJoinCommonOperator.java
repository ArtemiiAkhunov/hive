...
public abstract class VectorMapJoinCommonOperator extends MapJoinOperator implements VectorizationOperator, VectorizationContextRegion
{
    ...
    protected void determineCommonInfo(boolean isOuter) throws HiveException
    {
        ...
        bigTableByteColumnVectorColumns = getByteColumnVectorColumns(bigTableRetainColumnMap, bigTableRetainTypeInfos);

        nonOuterSmallTableKeyByteColumnVectorColumns = getByteColumnVectorColumns(nonOuterSmallTableKeyColumnMap, nonOuterSmallTableKeyTypeInfos);

        outerSmallTableKeyByteColumnVectorColumns = getByteColumnVectorColumns(outerSmallTableKeyMapping);

        smallTableByteColumnVectorColumns = getByteColumnVectorColumns(smallTableValueMapping);
        ...
        if (LOG.isInfoEnabled())
        {
            ...
            LOG.info(getLoggingPrefix() + " order " + Arrays.toString(orderDisplayable));
            LOG.info(getLoggingPrefix() + " posBigTable " + (int) posBigTable);
            LOG.info(getLoggingPrefix() + " posSingleVectorMapJoinSmallTable " + (int) posSingleVectorMapJoinSmallTable);

            LOG.info(getLoggingPrefix() + " bigTableKeyColumnMap " + Arrays.toString(bigTableKeyColumnMap));
            LOG.info(getLoggingPrefix() + " bigTableKeyColumnNames " + Arrays.toString(bigTableKeyColumnNames));
            LOG.info(getLoggingPrefix() + " bigTableKeyTypeInfos " + Arrays.toString(bigTableKeyTypeInfos));

            LOG.info(getLoggingPrefix() + " bigTableValueColumnMap " + Arrays.toString(bigTableValueColumnMap));
            LOG.info(getLoggingPrefix() + " bigTableValueColumnNames " + Arrays.toString(bigTableValueColumnNames));
            LOG.info(getLoggingPrefix() + " bigTableValueTypeNames " + Arrays.toString(bigTableValueTypeInfos));

            LOG.info(getLoggingPrefix() + " getBigTableRetainColumnMap " + Arrays.toString(bigTableRetainColumnMap));
            LOG.info(getLoggingPrefix() + " bigTableRetainTypeInfos " + Arrays.toString(bigTableRetainTypeInfos));

            LOG.info(getLoggingPrefix() + " nonOuterSmallTableKeyColumnMap " + Arrays.toString(nonOuterSmallTableKeyColumnMap));
            LOG.info(getLoggingPrefix() + " nonOuterSmallTableKeyTypeInfos " + Arrays.toString(nonOuterSmallTableKeyTypeInfos));

            LOG.info(getLoggingPrefix() + " outerSmallTableKeyMapping " + outerSmallTableKeyMapping.toString());

            LOG.info(getLoggingPrefix() + " fullOuterSmallTableKeyMapping " + fullOuterSmallTableKeyMapping.toString());

            LOG.info(getLoggingPrefix() + " smallTableValueMapping " + smallTableValueMapping.toString());

            LOG.info(getLoggingPrefix() + " bigTableByteColumnVectorColumns " + Arrays.toString(bigTableByteColumnVectorColumns));
            LOG.info(getLoggingPrefix() + " smallTableByteColumnVectorColumns " + Arrays.toString(smallTableByteColumnVectorColumns));

            LOG.info(getLoggingPrefix() + " outputProjection " + Arrays.toString(outputProjection));
            LOG.info(getLoggingPrefix() + " outputTypeInfos " + Arrays.toString(outputTypeInfos));

            LOG.info(getLoggingPrefix() + " mapJoinDesc.getKeysString " + conf.getKeysString());
            if (conf.getValueIndices() != null)
            {
                for (Entry<Byte, int[]> entry : conf.getValueIndices().entrySet())
                {
                    LOG.info(getLoggingPrefix() + " mapJoinDesc.getValueIndices +" + (int) entry.getKey() + " " + Arrays.toString(entry.getValue()));
                }
            }
            LOG.info(getLoggingPrefix() + " mapJoinDesc.getExprs " + conf.getExprs().toString());
            LOG.info(getLoggingPrefix() + " mapJoinDesc.getRetainList " + conf.getRetainList().toString());
    ...
    protected void setupVOutContext(List<String> outputColumnNames)
    {
        ...
        for (int i = 0; i < outputColumnNames.size(); ++i)
        {
            ...
            if (LOG.isDebugEnabled())
            {
                LOG.debug(getLoggingPrefix() + " addProjectionColumn " + i + " columnName " + columnName + " outputColumn " + outputColumn);
            }
        }
    }
    ...
    private void initializeFullOuterObjects() throws HiveException
    {
        ...
        if (hashTableKeyType == HashTableKeyType.MULTI_KEY && outputKeysSize > 0)
        {

            smallTableKeyOuterVectorDeserializeRow = new VectorDeserializeRow<BinarySortableDeserializeRead>(BinarySortableDeserializeRead.with(smallTableKeyTypeInfos, true, getConf().getKeyTblDesc().getProperties()));
            smallTableKeyOuterVectorDeserializeRow.init(allSmallTableKeyColumnNums, allSmallTableKeyColumnIncluded);
        }
    }
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        {
            smallTableValueVectorDeserializeRow = new VectorDeserializeRow<LazyBinaryDeserializeRead>(new LazyBinaryDeserializeRead(smallTableValueMapping.getTypeInfos(),
    ...
    protected void completeInitializationOp(Object[] os) throws HiveException
    {
        ...
        MapJoinTableContainer mapJoinTableContainer = mapJoinTables[posSingleVectorMapJoinSmallTable];
