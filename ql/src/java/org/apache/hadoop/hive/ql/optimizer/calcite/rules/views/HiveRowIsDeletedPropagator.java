...
public class HiveRowIsDeletedPropagator implements ReflectiveVisitor
{
    ...
    public HiveRowIsDeletedPropagator(RelBuilder relBuilder)
    {
        ...
        this.dispatcher = ReflectUtil.createMethodDispatcher(RelNode.class, this, "visit", RelNode.class, Context.class);
    }
    ...
    public RelNode visit(HiveTableScan scan, Context context)
    {
        ...
        } else
        {
            ...
            projects.add(rexBuilder.makeCall(SqlStdOperatorTable.AND, rexBuilder.makeCall(SqlStdOperatorTable.NOT, rowIsDeleted), rowIdPredicate));
        }
        ...
        // Project is created.
        return relBuilder.push(scan.setTableScanTrait(HiveTableScan.HiveTableScanTrait.FetchDeletedRows)).project(projects, projectNames).build();
    }
    ...
    public RelNode visit(HiveProject project, Context context)
    {
        ...
        return relBuilder.push(projectInput).project(newProjects).build();
    }
    ...
    public RelNode visit(HiveJoin join, Context context)
    {
        ...
        for (Map.Entry<Integer, RexNode> entry : context.rowIdPredicates.entrySet())
        {
            ...
            {
                rightContext.rowIdPredicates.put(entry.getKey() - originalLeftFieldCount, new InputRefShifter(originalLeftFieldCount, -originalLeftFieldCount, relBuilder).apply(entry.getValue()));
            }
        }
        ...
        RexNode rightDeleted = rexBuilder.makeInputRef(newRightRowType.getFieldList().get(rightAnyDeletedIndex).getType(), newLeftRowType.getFieldCount() + rightAnyDeletedIndex);
        RexNode rightInserted = rexBuilder.makeInputRef(newRightRowType.getFieldList().get(rightAnyInsertedIndex).getType(), newLeftRowType.getFieldCount() + rightAnyInsertedIndex);
        ...
        RexNode filterCondition = rexBuilder.makeCall(SqlStdOperatorTable.NOT, RexUtil.composeConjunction(rexBuilder, asList(anyDeleted, anyInserted)));

        return relBuilder.push(newLeftInput).push(newRightInput).join(join.getJoinType(), newJoinCondition).project(projects, projectNames).filter(filterCondition).build();
    }
    ...
    private RelDataTypeField getVirtualColumnField(RelDataType tableRowType, VirtualColumn virtualColumn, HiveTableScan scan)
    {
        ...
        if (field == null)
        {
            ...
            throw new ColumnPropagationException("TableScan " + scan + " row schema does not contain " + virtualColumn.getName() + " virtual column");
        }
        return field;
    }
    ...
    private RexNode createInputRef(RelNode relNode, int negativeOffset)
    {
        ...
        return relBuilder.getRexBuilder().makeInputRef(relNode.getRowType().getFieldList().get(index).getType(), index);
    }
