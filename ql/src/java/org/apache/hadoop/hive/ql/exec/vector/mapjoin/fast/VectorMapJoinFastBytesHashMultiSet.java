...
public abstract class VectorMapJoinFastBytesHashMultiSet extends VectorMapJoinFastBytesHashTable implements VectorMapJoinBytesHashMultiSet
{
    ...
    public void add(byte[] keyBytes, int keyStart, int keyLength, BytesWritable currentValue, long hashCode)
    {
        ...
        if (isNewKey)
        {
            slots[slot] = hashMultiSetStore.addFirst(partialHashCode, keyBytes, keyStart, keyLength);
            keysAssigned++;
        } else
        {
            final long newRefWord = hashMultiSetStore.bumpCount(refWord, unsafeReadPos);
    ...
    public JoinUtil.JoinResult contains(byte[] keyBytes, int keyStart, int keyLength, VectorMapJoinHashMultiSetResult hashMultiSetResult)
    {
        ...
        doHashMultiSetContains(keyBytes, keyStart, keyLength, hashCode, fastHashMultiSetResult);
