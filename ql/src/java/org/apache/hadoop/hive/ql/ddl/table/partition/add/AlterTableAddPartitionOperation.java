...
public class AlterTableAddPartitionOperation extends DDLOperation<AlterTableAddPartitionDesc>
{
    ...
    private Partition convertPartitionSpecToMetaPartition(Table table, AlterTableAddPartitionDesc.PartitionDesc partitionSpec) throws HiveException
    {
        ...
        Partition partition = org.apache.hadoop.hive.ql.metadata.Partition.createMetaPartitionObject(table, partitionSpec.getPartSpec(), location);
    ...
    private List<org.apache.hadoop.hive.ql.metadata.Partition> addPartitionsNoReplication(Table table, List<Partition> partitions) throws HiveException
    {
    ...
    private List<org.apache.hadoop.hive.ql.metadata.Partition> addPartitionsWithReplication(Table table, List<Partition> partitions, long writeId) throws HiveException
    {
        ...
        if (!partitionsToAlter.isEmpty())
        {
            ...
            context.getDb().alterPartitions(desc.getDbName(), desc.getTableName(), partitionsToAlter, ec, validWriteIdList, writeId);

            for (Partition outPartition : context.getDb().getPartitionsByNames(desc.getDbName(), desc.getTableName(), partitionNames, table))
            {
                outPartitions.add(new org.apache.hadoop.hive.ql.metadata.Partition(table, outPartition));
            }
        }
    ...
    private String getValidWriteIdList(Table table, long writeId) throws LockException
    {
        if (desc.getReplicationSpec().isInReplicationScope() && desc.getPartitions().get(0).getWriteId() > 0)
        {
            ...
            return new ValidReaderWriteIdList(TableName.getDbTable(table.getDbName(), table.getTableName()), new long[0], new BitSet(), writeId).writeToString();
        } else
        {
            AcidUtils.TableSnapshot tableSnapshot = AcidUtils.getTableSnapshot(context.getConf(), table, true);
            if (tableSnapshot != null && tableSnapshot.getWriteId() > 0)
