...
public class ConvertAstToSearchArg
{
    ...
    private static enum BoxType
    {
        ...
        DATE(PredicateLeaf.Type.DATE), DECIMAL(PredicateLeaf.Type.DECIMAL), TIMESTAMP(PredicateLeaf.Type.TIMESTAMP), BOOLEAN(PredicateLeaf.Type.BOOLEAN);
    ...
    private static Object boxLiteral(ExprNodeConstantDesc constantDesc, BoxType boxType)
    {
        ...
        switch (boxType)
        {
            ...
            case TIMESTAMP:
                ...
                {
                    ts = ((org.apache.hadoop.hive.common.type.Timestamp) lit).toSqlTimestamp();
                ...
                {
                    ts = org.apache.hadoop.hive.common.type.Timestamp.valueOf(lit.toString()).toSqlTimestamp();
                }
                return ts;
            case DATE:
     ...
     */
    private static Object findLiteral(Configuration conf, ExprNodeGenericFuncDesc expr, final BoxType boxType)
    {
        List<ExprNodeDesc> children = expr.getChildren();
        if (children.size() != 2)
        {
        ...
        try
        {
            switch (operator)
            {
                ...
                case LESS_THAN_EQUALS:
                    ...
                    break;
                case IN:
                    builder.in(columnName, boxType.type, getLiteralList(expression, boxType, variable + 1));
                    break;
                case BETWEEN:
                    builder.between(columnName, boxType.type, getLiteral(conf, expression, boxType, variable + 1), getLiteral(conf, expression, boxType, variable + 2));
                    break;
            }
        } catch (Exception e)
        {
    ...
    private static synchronized Cache<String, SearchArgument> initializeAndGetSargsCache(Configuration conf)
    {
        if (sargsCache == null)
        {
            sargsCache = CacheBuilder.newBuilder().weigher((String key, SearchArgument value) -> key.length()).maximumWeight(HiveConf.getIntVar(conf, HiveConf.ConfVars.HIVE_IO_SARG_CACHE_MAX_WEIGHT_MB) * 1024 * 1024).build(); // Can't use CacheLoader because SearchArguments may be built either from Kryo strings,
            // or from expressions.
        }
        return sargsCache;
    }
    ...
    private static SearchArgument getSearchArgumentFromString(Configuration conf, String sargString)
    {
        ...
        try
        {
            return isSargsCacheEnabled(conf) ? getSargsCache(conf).get(sargString, () -> create(sargString)) : create(sargString);
    ...
    private static SearchArgument getSearchArgumentFromExpression(Configuration conf, String sargString)
    {
        ...
        try
        {
            return isSargsCacheEnabled(conf) ? getSargsCache(conf).get(sargString, () -> create(conf, SerializationUtilities.deserializeExpression(sargString))) : create(conf, SerializationUtilities.deserializeExpression(sargString));
        } catch (ExecutionException exception)
        {
            throw new RuntimeException(exception);
        }
