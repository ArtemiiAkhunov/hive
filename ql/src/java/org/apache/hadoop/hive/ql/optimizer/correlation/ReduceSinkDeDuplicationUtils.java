...
public class ReduceSinkDeDuplicationUtils
{
    ...
    public static boolean merge(HiveConf hiveConf, ReduceSinkOperator cRS, ReduceSinkOperator pRS, int minReducer) throws SemanticException
    {
        ...
        if (result[2] > 0)
        {
            ...
            if (result[0] <= 0)
            {
                ...
                throw new SemanticException("Sorting columns and order don't match. " + "Try set " + HiveConf.ConfVars.HIVE_OPT_REDUCE_DEDUPLICATION + "=false;");
            }
        ...
        if (result[4] > 0)
        {
            ...
            } else if (cRS.getConf().getKeyCols() != null && cRS.getConf().getKeyCols().size() > 0)
            {
                ...
                List<FieldSchema> fields = PlanUtils.getFieldSchemasFromColumnList(pRS.getConf().getKeyCols(), keyColNames, 0, "");
                TableDesc keyTable = PlanUtils.getReduceKeyTableDesc(fields, pRS.getConf().getOrder(), pRS.getConf().getNullOrder());
     ...
     * @return true if the partitioning columns of the child RS can replace the columns of the parent RS and false otherwise
     */
