...
public class VectorMapJoinOuterStringOperator extends VectorMapJoinOuterGenerateResultOperator
{
    ...
    public VectorMapJoinOuterStringOperator(CompilationOpContext ctx, OperatorDesc conf, VectorizationContext vContext, VectorDesc vectorDesc) throws HiveException
    {
        ...
        try
        {
            ...
            if (allKeyInputColumnsRepeating)
            {
                ...
                } else
                {
                    ...
                    joinResult = hashMap.lookup(keyBytes, keyStart, keyLength, hashMapResults[0], matchTracker);
                }
                ...
                for (int logical = 0; logical < batch.size; logical++)
                {
                    ...
                    } else
                    {
                        ...
                        if (!haveSaveKey || StringExpr.equal(vector[saveKeyBatchIndex], start[saveKeyBatchIndex], length[saveKeyBatchIndex], vector[batchIndex], start[batchIndex], length[batchIndex]) == false)
                        {
                            ...
                            saveJoinResult = hashMap.lookup(keyBytes, keyStart, keyLength, hashMapResults[hashMapResultCount], matchTracker);
