...
public abstract class GenericUDAFStreamingEvaluator<T1> extends GenericUDAFEvaluator implements ISupportStreamingModeForWindowing
{
    ...
    public static abstract class SumAvgEnhancer<T1, T2> extends GenericUDAFStreamingEvaluator<T1>
    {
        ...
        class SumAvgStreamingState extends StreamingState
        {
            ...
            {
                if (!wFrameDef.getStart().isUnbounded() && !this.intermediateVals.isEmpty() && this.numRows >= wFrameDef.getWindowSize())
                {
        ...
        protected abstract T1 getNextResult(SumAvgStreamingState ss) throws HiveException;

        protected abstract T2 getCurrentIntermediateResult(SumAvgStreamingState ss) throws HiveException;
