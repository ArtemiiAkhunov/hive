...
public class ClearDanglingScratchDir implements Runnable
{
    ...
    static Options createOptions()
    {
        ...
        result.addOption(OptionBuilder.withLongOpt("dry-run").withDescription("Generate a list of dangling scratch dir, printed on console").create('r'));

        // add -s and --scratchdir to specify a non-default scratch dir
        result.addOption(OptionBuilder.withLongOpt("scratchdir").withDescription("Specify a non-default location of the scratch dir").hasArg().create('s'));
    ...
    private void removeLocalTmpFiles(String sessionName, String localTmpdir)
    {
        ...
        if (files != null)
        {
            for (File file : files)
            {
                ...
                {
                    consoleMessage("While removing '" + sessionName + "' dangling scratch dir from HDFS, " + "local tmp session file '" + file.getPath() + "' has been cleaned as well.");
                } else
                {
                    consoleMessage("Even though '" + sessionName + "' is marked as dangling session dir, " + "local tmp session file '" + file.getPath() + "' could not be removed.");
                }
            }
        }
    }
