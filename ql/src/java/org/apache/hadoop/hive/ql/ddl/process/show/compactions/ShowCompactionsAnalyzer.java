...
public class ShowCompactionsAnalyzer extends BaseSemanticAnalyzer
{
    ...
    public void analyzeInternal(ASTNode root) throws SemanticException
    {
        ...
        ShowCompactionsDesc desc = new ShowCompactionsDesc(ctx.getResFile(), compactionId, dbName, tblName, poolName, compactionType, compactionStatus, partitionSpec, limit, orderBy);
    ...
    private String processSortOrderSpec(ASTNode sortNode)
    {
        ...
        Map<String, String> orderByAttributes = orderExp.stream().collect(Collectors.toMap(x -> getDbColumnName(x.getExpression()), x -> x.getOrder().toString()));
    ...
    private String getDbColumnName(ASTNode expression)
    {
        ...
        return EnumUtils.isValidEnum(CompactionColumn.class, dbColumnName) ? CompactionColumn.valueOf(dbColumnName).toString() : "\"" + dbColumnPrefix + dbColumnName + "\"";
    }

    private enum CompactionColumn
    {
        COMPACTIONID("\"CC_ID\""), DBNAME("\"CC_DATABASE\""), TABNAME("\"CC_TABLE\""), PARTNAME("\"CC_PARTITION\""), ENQUEUETIME("\"CC_ENQUEUE_TIME\""), STARTTIME("\"CC_START\""), POOLNAME("\"CC_POOL_NAME\""), NEXTTXNID("\"CC_NEXT_TXN_ID\""), HADOOPJOBID("\"CC_HADOOP_JOB_ID\""), WORKERHOST("\"CC_WORKER_ID\""), WORKERID("\"CC_WORKER_ID\""), DURATION("\"CC_END\""), TXNID("\"CC_TXN_ID\""), COMMITTIME("CC_COMMIT_TIME"), HIGHESTWRITEID("CC_HIGHEST_WRITE_ID");
