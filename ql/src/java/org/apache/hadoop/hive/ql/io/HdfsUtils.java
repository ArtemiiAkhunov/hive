...
public class HdfsUtils
{
     ...
     * @throws IOException
     */
    public static List<FileStatus> listLocatedFileStatus(final FileSystem fs, final Path path, final PathFilter filter, final boolean recursive) throws IOException
    {
    ...
    public static List<Path> listPath(final FileSystem fs, final Path path, final PathFilter filter, final boolean recursive) throws IOException
    {
    ...
    public static boolean isDefaultFs(DistributedFileSystem fs)
    {
        ...
        if (scheme == null)
        {
            return true; // Assume that relative URI resolves to default FS.
        }
        ...
        if (authority == null)
        {
            return true; // Schemes match, no authority - assume default.
        }
        if (defaultAuthority == null)
        {
            return false; // TODO: What does this even mean?
        }
        ...
        if (port == -1)
        {
            return true; // No port, assume default.
        }
        ...
        if (childrenWithId != null)
        {
            for (HadoopShims.HdfsFileStatusWithId child : childrenWithId)
            {
                if (child.getFileStatus().isDirectory())
                {
                    if (recursive)
                    {
                        originals.addAll(listFileStatusWithId(fs, child.getFileStatus().getPath(), useFileIds, true, filter));
                    }
                } else
                {
