...
public class ExplainSemanticAnalyzer extends BaseSemanticAnalyzer
{
    ...
    public void analyzeInternal(ASTNode ast) throws SemanticException
    {
        ...
        config.setUserLevelExplain(!config.isExtended() && !config.isFormatted() && !config.isDependency() && !config.isCbo() && !config.isLogical() && !config.isVectorization() && !config.isAuthorize() && HiveConf.getBoolVar(ctx.getConf(), HiveConf.ConfVars.HIVE_EXPLAIN_USER) && HiveConf.getVar(conf, HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("tez"));
        ...
        work.setAppendTaskType(HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_EXPLAIN_DEPENDENCY_APPEND_TASK_TYPES));
    ...
    public boolean skipAuthorization()
    {
        ...
        if (task instanceof ExplainTask && ((ExplainTask) task).getWork().isAuthorize())
        {
            return true;
        }
