...
public class ReOptimizePlugin implements IReExecutionPlugin
{
                ...
                {
                    {
                        ...
                        if (message != null)
                        {
                            boolean isOOM = message.contains(MapJoinMemoryExhaustionError.class.getName()) || message.contains(OutOfMemoryError.class.getName());
    ...
    public void prepareToReExecute()
    {
        ...
        {
            coreDriver.setStatsSource(StatsSources.getStatsSourceContaining(coreDriver.getStatsSource(), coreDriver.getPlanMapper()));
        }
    }
    ...
    public void afterExecute(PlanMapper planMapper, boolean success)
    {
        if (alwaysCollectStats)
        {
            coreDriver.setStatsSource(StatsSources.getStatsSourceContaining(coreDriver.getStatsSource(), planMapper));
        }
    }
