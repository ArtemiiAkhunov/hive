...
public abstract class BaseVectorizedColumnReader implements VectorizedColumnReader
{
    ...
    public BaseVectorizedColumnReader(ColumnDescriptor descriptor, PageReader pageReader, boolean skipTimestampConversion, ZoneId writerTimezone, boolean skipProlepticConversion, boolean legacyConversionEnabled, Type parquetType, TypeInfo hiveType) throws IOException
    {
        ...
        if (dictionaryPage != null)
        {
            try
            {
                this.dictionary = ParquetDataColumnReaderFactory.getDataColumnReaderByTypeOnDictionary(parquetType.asPrimitiveType(), hiveType, dictionaryPage.getEncoding().initDictionary(descriptor, dictionaryPage), skipTimestampConversion, writerTimezone, legacyConversionEnabled);
    ...
    private void initDataReader(Encoding dataEncoding, ByteBufferInputStream in, int valueCount) throws IOException
    {
        ...
        if (dataEncoding.usesDictionary())
        {
            ...
            dataColumn = ParquetDataColumnReaderFactory.getDataColumnReaderByType(type.asPrimitiveType(), hiveType, dataEncoding.getDictionaryBasedValuesReader(descriptor, VALUES, dictionary.getDictionary()), skipTimestampConversion, writerTimezone, legacyConversionEnabled);
            this.isCurrentPageDictionaryEncoded = true;
        } else
        {
            dataColumn = ParquetDataColumnReaderFactory.getDataColumnReaderByType(type.asPrimitiveType(), hiveType, dataEncoding.getValuesReader(descriptor, VALUES), skipTimestampConversion, writerTimezone, legacyConversionEnabled);
            this.isCurrentPageDictionaryEncoded = false;
        }
