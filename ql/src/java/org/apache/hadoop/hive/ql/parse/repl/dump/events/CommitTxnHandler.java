...
class CommitTxnHandler extends AbstractEventHandler<CommitTxnMessage>
{
    ...
    private void writeDumpFiles(Table qlMdTable, Partition ptn, Iterable<String> files, Context withinContext, Path dataPath) throws IOException, LoginException, MetaException, HiveFatalException, SemanticException
    {
    ...
    private void createDumpFile(Context withinContext, org.apache.hadoop.hive.ql.metadata.Table qlMdTable, List<Partition> qlPtns, List<List<String>> fileListArray) throws IOException, SemanticException, LoginException, MetaException, HiveFatalException
    {
        ...
        } else
        {
            ...
            {
                Path dataPath = new Path(withinContext.eventRoot, EximUtil.DATA_PATH_NAME + File.separator + qlPtns.get(idx).getName());
                writeDumpFiles(qlMdTable, qlPtns.get(idx), fileListArray.get(idx), withinContext, dataPath);
            }
        }
    }

    private void createDumpFileForTable(Context withinContext, org.apache.hadoop.hive.ql.metadata.Table qlMdTable, List<Partition> qlPtns, List<List<String>> fileListArray) throws IOException, SemanticException, LoginException, MetaException, HiveFatalException
    {
    ...
    public void handle(Context withinContext) throws Exception
    {
        ...
        if (!withinContext.hiveConf.getBoolVar(HiveConf.ConfVars.REPL_DUMP_METADATA_ONLY))
        {
            ...
            if (replicatingAcidEvents)
            {
                ...
                if (ReplUtils.filterTransactionOperations(withinContext.hiveConf) && (writeEventInfoList == null || writeEventInfoList.size() == 0))
                {
