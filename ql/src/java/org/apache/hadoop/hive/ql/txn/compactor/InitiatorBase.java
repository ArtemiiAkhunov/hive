...
public class InitiatorBase extends MetaStoreCompactorThread
{
    ...
    private List<CompactionResponse> initiateCompactionForMultiplePartitions(Table table, Map<String, Partition> partitions, CompactionRequest request)
    {
        ...
        partitions.entrySet().parallelStream().forEach(entry ->
        {
            try
            {
                ...
                CompactionInfo ci = new CompactionInfo(table.getDbName(), table.getTableName(), entry.getKey(), request.getType());
                ...
                LOG.info("Checking to see if we should compact partition " + entry.getKey() + " of table " + table.getDbName() + "." + table.getTableName());
                CollectionUtils.addIgnoreNull(compactionResponses, scheduleCompactionIfRequired(ci, table, entry.getValue(), runAs, false));
            } catch (IOException | InterruptedException | MetaException e)
            {
                LOG.error("Error occurred while Checking if we should compact partition " + entry.getKey() + " of table " + table.getDbName() + "." + table.getTableName() + " Exception: " + e.getMessage());
    ...
    private CompactionType determineCompactionType(CompactionInfo ci, AcidDirectory dir, Map<String, String> tblProperties, long baseSize, long deltaSize)
    {
    ...
    private CompactionType checkForCompaction(final CompactionInfo ci, final ValidWriteIdList writeIds, final StorageDescriptor sd, final Map<String, String> tblProperties, final String runAs) throws IOException, InterruptedException
    {
        ...
        } else
        {
            ...
            try
            {
                compactionType = ugi.doAs((PrivilegedExceptionAction<CompactionType>) () -> determineCompactionType(ci, acidDirectory, tblProperties, baseSize, deltaSize));
            } finally
            {
                try
                {
    ...
    protected CompactionResponse scheduleCompactionIfRequired(CompactionInfo ci, Table t, Partition p, String runAs, boolean metricsEnabled) throws MetaException
    {
