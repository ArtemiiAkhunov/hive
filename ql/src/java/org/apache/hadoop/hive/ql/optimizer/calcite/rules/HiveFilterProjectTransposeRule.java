...
public class HiveFilterProjectTransposeRule extends FilterProjectTransposeRule
{
    ...
    public static final HiveFilterProjectTransposeRule DETERMINISTIC_WINDOWING_ON_NON_FILTERING_JOIN = new HiveFilterProjectTransposeRule(operand(Filter.class, operand(Project.class, operand(Join.class, any()))), HiveRelFactories.HIVE_BUILDER, true, true, ProjectMergeRule.DEFAULT_BLOAT);

    public static final HiveFilterProjectTransposeRule DETERMINISTIC_WINDOWING = new HiveFilterProjectTransposeRule(Filter.class, HiveProject.class, HiveRelFactories.HIVE_BUILDER, true, true, ProjectMergeRule.DEFAULT_BLOAT);

    public static final HiveFilterProjectTransposeRule DETERMINISTIC_ON_NON_FILTERING_JOIN = new HiveFilterProjectTransposeRule(operand(Filter.class, operand(Project.class, operand(Join.class, any()))), HiveRelFactories.HIVE_BUILDER, true, false, ProjectMergeRule.DEFAULT_BLOAT);

    public static final HiveFilterProjectTransposeRule DETERMINISTIC = new HiveFilterProjectTransposeRule(Filter.class, HiveProject.class, HiveRelFactories.HIVE_BUILDER, true, false, ProjectMergeRule.DEFAULT_BLOAT);

    public static final HiveFilterProjectTransposeRule INSTANCE = new HiveFilterProjectTransposeRule(Filter.class, HiveProject.class, HiveRelFactories.HIVE_BUILDER, false, false, ProjectMergeRule.DEFAULT_BLOAT);
    ...
    public boolean matches(RelOptRuleCall call)
    {
        ...
        RexNode condition = HiveRelOptUtil.pushPastProjectUnlessBloat(filterRel.getCondition(), call.rel(1), bloat);
        if (condition == null)
        {
            return false;
        }
    ...
    private RelNode getNewProject(RexNode filterCondToPushBelowProj, RexNode unPushedFilCondAboveProj, Project oldProj, RelDataTypeFactory typeFactory, RelBuilder relBuilder)
    {
        ...
        RexNode newPushedCondition = HiveRelOptUtil.pushPastProjectUnlessBloat(filterCondToPushBelowProj, oldProj, bloat);
    ...
    private static class RedundancyChecker extends RelVisitor
    {
        ...
        {
            RexNode filterCondition = filter2newConditionMap.isEmpty() ? newCondition : filter2newConditionMap.get(node);
            ...
            } else
            {
                ...
                {
                    RexNode condition = HiveRelOptUtil.pushPastProjectUnlessBloat(filterCondition, (Project) node, bloat);
                    if (condition != null)
                    {
        ...
        private void check(Filter filter)
        {
            ...
            final RexSimplify simplify = new RexSimplify(rexBuilder, RelOptPredicateList.EMPTY, Util.first(cluster.getPlanner().getExecutor(), RexUtil.EXECUTOR));
            ...
            if (newCondition instanceof RexCall && ((RexCall) newCondition).getOperands().get(0).isA(SqlKind.INPUT_REF))
            {
                return;
            }
            ...
            final RexInputRef rexInputRef = rexBuilder.makeInputRef(filter.getInput(), inputRefs.iterator().next());
            ...
            isRedundant = !isPredicateIncluded(newCondition, filterCondition) && isPredicateIncluded(baseCondition, filterCondition);
        }
    }
