...
public class HiveSemiJoinRule
{
    ...
    private abstract static class HiveSemiJoinRuleBase<T extends RelNode> extends RelOptRule
    {
        ...
        {
            super(operand(clazz, operand(Join.class, operand(RelNode.class, any()), operand(Aggregate.class, operand(RelNode.class, any())))), relBuilder, null);
        }
        ...
        protected void perform(final RelOptRuleCall call, final ImmutableBitSet topRefs, final T topOperator, final Join join, final RelNode left, final Aggregate aggregate, final RelNode aggregateInput)
        {
            ...
            final RelNode newLeft = updateLeft ? buildProjectLeftInput(left, leftNeededRefs, rexBuilder, call.builder()) : left;
            final RelNode newRight = needProject(aggregateInput, aggregate) ? buildProjectRightInput(aggregate, rexBuilder, call.builder()) : aggregateInput;
            ...
            final RexNode newCondition = RelOptUtil.createEquiJoinCondition(newLeft, leftKeys, newRight, joinInfo.rightKeys, rexBuilder);
            ...
            call.transformTo(recreateTopOperatorUnforced(call.builder(), rexBuilder, adjustments, topOperator, semi));
        }

        private RelNode buildProjectLeftInput(final RelNode node, final ImmutableBitSet neededRefs, final RexBuilder rexBuilder, final RelBuilder builder)
        {
        ...
        private RelNode buildProjectRightInput(final Aggregate aggregate, final RexBuilder rexBuilder, final RelBuilder relBuilder)
        {
        ...
        protected abstract T recreateTopOperator(RelBuilder builder, RexBuilder rexBuilder, int[] adjustments, T topOperator, RelNode inputOperator);

        protected abstract RelNode recreateTopOperatorUnforced(RelBuilder builder, RexBuilder rexBuilder, int[] adjustments, T topOperator, RelNode inputOperator);
    }
    ...
    protected static class HiveProjectJoinToSemiJoinRule extends HiveSemiJoinRuleBase<Project>
    {
        ...
        {
            return (Project) recreateProjectOperator(builder, rexBuilder, adjustments, topProject, newInputOperator, true);
        }
        ...
        {
            return recreateProjectOperator(builder, rexBuilder, adjustments, topProject, newInputOperator, false);
        }
    }
    ...
    private abstract static class HiveToSemiJoinRuleSwapInputs<T extends RelNode> extends HiveSemiJoinRuleBase<T>
    {
        ...
        {
            super(operand(clazz, operand(Join.class, operand(Aggregate.class, operand(RelNode.class, any())), operand(RelNode.class, any()))), relBuilder);
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            if (!joinInfo.leftSet().equals(ImmutableBitSet.range(aggregate.getGroupCount())))
            {
                // Rule requires that aggregate key to be the same as the join key.
                // By the way, neither a super-set nor a sub-set would work.
                return;
            ...
            final ImmutableBitSet leftBits = ImmutableBitSet.range(0, join.getLeft().getRowType().getFieldCount());
        ...
        protected T swapInputs(final Join join, final T topOperator, final RelBuilder builder)
        {
            ...
            RexNode newJoinCond = join.getCondition().accept(new RelOptUtil.RexInputConverter(rexBuilder, joinFields, joinFields, adjustments));
    ...
    protected static RelNode recreateProjectOperator(RelBuilder builder, RexBuilder rexBuilder, int[] adjustments, Project topProject, RelNode newInputOperator, boolean force)
    {
        ...
        {
            RexNode newProject = project.accept(new RelOptUtil.RexInputConverter(rexBuilder, swappedJoinFields, swappedJoinFields, adjustments));
            newProjects.add(newProject);
        }
        return builder.push(newInputOperator).project(newProjects, ImmutableList.of(), force).build();
    }
