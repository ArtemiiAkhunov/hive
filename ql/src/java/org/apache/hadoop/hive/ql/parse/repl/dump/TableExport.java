...
public class TableExport
{
    ...
    public TableExport(Paths paths, TableSpec tableSpec, ReplicationSpec replicationSpec, Hive db, String distCpDoAsUser, HiveConf conf, MmContext mmCtx)
    {
    ...
    protected void write(boolean isExportTask, FileList fileList, boolean dataCopyAtLoad) throws SemanticException
    {
        ...
        } else if (shouldExport())
        {
            ...
            if (!replicationSpec.isMetadataOnly() && !(replicationSpec.isRepl() && tableSpec.tableHandle.getTableType().equals(TableType.EXTERNAL_TABLE)))
            {
            ...
            if (tableSpec != null && tableSpec.tableHandle != null && tableSpec.tableHandle.isPartitioned())
            {
                if (tableSpec.specType == TableSpec.SpecType.TABLE_ONLY)
                {
                    ...
                    } else
                    {
                        ...
                        return new PartitionIterable(Hive.get(conf), tableSpec.tableHandle, null, MetastoreConf.getIntVar(conf, MetastoreConf.ConfVars.BATCH_RETRIEVE_MAX), true);
                    }
                } else
                {
    ...
    {
        return Utils.shouldReplicate(replicationSpec, tableSpec.tableHandle, false, null, null, conf);
    }
    ...
    public static class Paths
    {
        ...
        private Path exportDir(Path exportDir) throws SemanticException
        {
            ...
            {
                throw new SemanticException("error while creating directory for partition at " + exportDir, e);
            }
        }
        ...
        private void validateTargetDir(URI rootDirExportFile) throws SemanticException
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!tgt.isDirectory())
                    {
                        throw new SemanticException(astRepresentationForErrorMsg + ": " + "Target is not a directory : " + rootDirExportFile);
                    } else
                    {
                        FileStatus[] files = fs.listStatus(toPath, FileUtils.HIDDEN_FILES_PATH_FILTER);
                        if (files != null && files.length != 0)
                        {
                            throw new SemanticException(astRepresentationForErrorMsg + ": " + "Target is not an empty directory : " + rootDirExportFile);
                        }
                    }
