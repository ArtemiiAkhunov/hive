...
public class DruidSqlOperatorConverter
{
        ...
        public String toDruidExpression(RexNode rexNode, RelDataType rowType, DruidQuery query)
        {
            ...
            if (call.getOperands().size() == 1)
            {
                ...
                if (SqlTypeUtil.isDatetime(call.getOperands().get(0).getType()))
                {
                    ...
                    return applyTimestampFormat(DruidExpressions.applyTimestampFloor(arg, Period.days(1).toString(), "", tz), YYYY_MM_DD, tz);
                }
                return null;
            } else if (call.getOperands().size() == 2)
            {
                ...
                final TimeZone tz = timezoneId(query, call.getOperands().get(0));
                return applyTimestampFormat(DruidExpressions.applyTimestampFloor(arg, unit, "", tz), YYYY_MM_DD, tz);
            }
            return null;
        }
    }
    ...
    public static class DruidToDateOperatorConversion implements org.apache.calcite.adapter.druid.DruidSqlOperatorConverter
    {
        ...
        public String toDruidExpression(RexNode rexNode, RelDataType rowType, DruidQuery query)
        {
            ...
            return DruidExpressions.applyTimestampFloor(arg, Period.days(1).toString(), "", timezoneId(query, call.getOperands().get(0)));
        }
    }
    ...
    public static class DruidUnixTimestampOperatorConversion implements org.apache.calcite.adapter.druid.DruidSqlOperatorConverter
    {
        ...
        public String toDruidExpression(RexNode rexNode, RelDataType rowType, DruidQuery query)
        {
            ...
            final String format = call.getOperands().size() == 2 ? DruidExpressions.toDruidExpression(call.getOperands().get(1), rowType, query) : DEFAULT_TS_FORMAT;
    ...
    public static class DruidFormUnixTimeOperatorConversion implements org.apache.calcite.adapter.druid.DruidSqlOperatorConverter
    {
        ...
        public String toDruidExpression(RexNode rexNode, RelDataType rowType, DruidQuery query)
        {
            ...
            final String format = call.getOperands().size() == 1 ? DruidExpressions.stringLiteral(DEFAULT_TS_FORMAT) : DruidExpressions.toDruidExpression(call.getOperands().get(1), rowType, query);
            return DruidExpressions.functionCall("timestamp_format", ImmutableList.of(numMillis, format, DruidExpressions.stringLiteral(TimeZone.getTimeZone("UTC").getID())));
        }
    }
    ...
    public static class DruidDateArithmeticOperatorConversion implements org.apache.calcite.adapter.druid.DruidSqlOperatorConverter
    {
        ...
        public String toDruidExpression(RexNode rexNode, RelDataType rowType, DruidQuery query)
        {
            ...
            return DruidExpressions.functionCall("timestamp_shift", ImmutableList.of(arg0, DruidExpressions.stringLiteral("P1D"), steps, DruidExpressions.stringLiteral(timezoneId(query, call.getOperands().get(0)).getID())));
        }
    }
    ...
    {
        return arg.getType().getSqlTypeName() == SqlTypeName.TIMESTAMP_WITH_LOCAL_TIME_ZONE ? TimeZone.getTimeZone(query.getTopNode().getCluster().getPlanner().getContext().unwrap(CalciteConnectionConfig.class).timeZone()) : TimeZone.getTimeZone("UTC");
    }

    private static String applyTimestampFormat(String arg, String format, TimeZone timeZone)
    {
        return DruidExpressions.functionCall("timestamp_format", ImmutableList.of(arg, DruidExpressions.stringLiteral(format), DruidExpressions.stringLiteral(timeZone.getID())));
    }
