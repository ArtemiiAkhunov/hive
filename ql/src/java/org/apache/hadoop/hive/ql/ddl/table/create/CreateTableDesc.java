...
public class CreateTableDesc implements DDLDesc, Serializable
{
    ...
    /**
     * @param serdeProps the serde properties to set
     */
    ...
    { // only for displaying plan
        return PlanUtils.getPropertiesForExplain(tblProps, hive_metastoreConstants.TABLE_IS_CTAS, hive_metastoreConstants.TABLE_BUCKETING_VERSION);
    }

    /**
     * @param tblProps the table properties to set
     */
    ...
    public void validate(HiveConf conf) throws SemanticException
    {
        ...
        if ((this.getCols() == null) || (this.getCols().size() == 0))
        {
            ...
            if (Table.hasMetastoreBasedSchema(conf, serName) && StringUtils.isEmpty(getStorageHandler()) && this.getLikeFile() == null)
            {
    ...
    public Table toTable(HiveConf conf) throws HiveException
    {
        ...
        if (replicationSpec == null || !replicationSpec.isInReplicationScope())
        {
            ...
            if (!this.isCTAS && !tbl.isPartitioned() && !tbl.isTemporary() && conf.getBoolVar(HiveConf.ConfVars.HIVESTATSAUTOGATHER))
            {
                ...
                String statsSetup = StatsSetupConst.ColumnStatsSetup.getStatsSetupAsString(true,
                        ...
                        storageHandler != null && storageHandler.isTableMetaRefSupported(), MetaStoreUtils.getColumnNames(tbl.getCols()));
