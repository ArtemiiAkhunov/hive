...
final class MmMinorQueryCompactor extends QueryCompactor
{
    ...
    public boolean run(CompactorContext context) throws IOException
    {
        ...
        List<String> createTableQueries = getCreateQueries(tmpTableName, table, storageDescriptor, dir, writeIds, resultDeltaDir);
        ...
        runCompactionQueries(driverConf, tmpTableName, storageDescriptor, writeIds, context.getCompactionInfo(), Lists.newArrayList(resultDeltaDir), createTableQueries, compactionQueries, dropQueries, table.getParameters());
        return true;
    }
    ...
    {
        List<String> queries = Lists.newArrayList(getCreateQuery(tmpTableBase, t, sd, null, true), getCreateQuery(tmpTableBase + "_result", t, sd, resultDeltaDir.toString(), false));
    ...
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.CREATE, true, newTableName).setSourceTab(t).setStorageDescriptor(sd).setLocation(location).setPartitioned(isPartitioned).build();
    }
    ...
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.ALTER, true, tableName).setDir(dir).setValidWriteIdList(validWriteIdList).build();
    }
    ...
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.INSERT, true, resultTmpTableName).setSourceTabForInsert(sourceTmpTableName).setSourceTab(sourceTable).build());
    }
    ...
    {
        return Lists.newArrayList(getDropQuery(tmpTableBase), getDropQuery(tmpTableBase + "_result"));
    }

    private String getDropQuery(String tableToDrop)
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.DROP, true, tableToDrop).build();
    }
