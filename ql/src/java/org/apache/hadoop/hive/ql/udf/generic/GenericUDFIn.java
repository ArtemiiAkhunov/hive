...
public class GenericUDFIn extends GenericUDF
{
    ...
    private void prepareInSet(DeferredObject[] arguments) throws HiveException
    {
        ...
        if (compareOI.getCategory().equals(ObjectInspector.Category.PRIMITIVE))
        {
            for (int i = 1; i < arguments.length; ++i)
            {
                Object constant = ((PrimitiveObjectInspector) compareOI).getPrimitiveJavaObject(conversionHelper.convertIfNecessary(arguments[i].get(), argumentOIs[i]));
    ...
    public Object evaluate(DeferredObject[] arguments) throws HiveException
    {
        ...
        if (isInSetConstant)
        {
            ...
            switch (compareOI.getCategory())
            {
                case PRIMITIVE:
                {
                    Object arg = ((PrimitiveObjectInspector) compareOI).getPrimitiveJavaObject(conversionHelper.convertIfNecessary(arguments[0].get(), argumentOIs[0]));
                    if (compareOI.getTypeName().equals(serdeConstants.BINARY_TYPE_NAME))
                    {
        ...
        } else
        {
            for (int i = 1; i < arguments.length; i++)
            {
                if (ObjectInspectorUtils.compare(conversionHelper.convertIfNecessary(arguments[0].get(), argumentOIs[0]), compareOI, conversionHelper.convertIfNecessary(arguments[i].get(), argumentOIs[i], false), compareOI) == 0)
                {
                    bw.set(true);
                    return bw;
                }
