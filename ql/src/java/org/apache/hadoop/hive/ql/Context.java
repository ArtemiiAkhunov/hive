...
public class Context
{
    ...
    private Context(Configuration conf, String executionId)
    {
        ...
        enableUnparse = HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING_SQL) || HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING_SUBQUERY_SQL);
    ...
    public Path getMRScratchDir(boolean mkDir)
    {
        ...
        try
        {
            ...
            Path newScratchDir = getScratchDir(uri.getScheme(), uri.getAuthority(), mkDir, uri.getPath());
    ...
    public Path getTempDirForInterimJobPath(Path path)
    {
        ...
        boolean isBlobstorageOptimized = BlobStorageUtils.isBlobStoragePath(conf, path) && !BlobStorageUtils.isBlobStorageAsScratchDir(conf) && BlobStorageUtils.areOptimizationsEnabled(conf);
        ...
        for (Path p : fsScratchDirs.values())
        {
            try
            {
                ...
                if (resultCacheDir == null || !p.toUri().getPath().contains(resultCacheDir))
                {
                    ...
                    CleanupService cleanupService = sessionState != null ? sessionState.getCleanupService() : SyncCleanupService.INSTANCE;
    ...
    public void removeMaterializedCTEs()
    {
        ...
        for (Table materializedTable : cteTables.values())
        {
            ...
            try
            {
                ...
                LOG.info("Removed " + location + " for materialized " + materializedTable.getTableName() + ", status=" + status);
            } catch (IOException e)
            {
                // ignore
    ...
    {
        return (uriStr.indexOf(executionId) != -1) && (uriStr.indexOf(MR_PREFIX) != -1);
    }
    ...
    {
        return new Path(getStagingDir(new Path(uri), !isExplainSkipExecution() && !isLoadingMaterializedView()), MR_PREFIX + nextPathId());
    }

    public Path getMRTmpPath(boolean mkDir)
    {
        return new Path(getMRScratchDir(mkDir), MR_PREFIX + nextPathId());
    }
    ...
    public void clear(boolean deleteResultDir) throws IOException
    {
        ...
        if (resDir != null && !isInScratchDir(resDir))
        { // resDir is inside the scratch dir, removeScratchDir will take care of removing it
            try
            {
        ...
        if (resFile != null && !isInScratchDir(resFile.getParent()))
        { // resFile is inside the scratch dir, removeScratchDir will take care of removing it
            try
            {
    ...
    {
        return path.toUri().getPath().startsWith(localScratchDir) || path.toUri().getPath().startsWith(nonLocalScratchPath.toUri().getPath());
    }
    ...
    {
        assert (this.tokenRewriteStream == null || this.getExplainAnalyze() == AnalyzeState.RUNNING || skipTableMasking);
