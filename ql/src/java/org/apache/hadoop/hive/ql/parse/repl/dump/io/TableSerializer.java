...
public class TableSerializer implements JsonWriter.Serializer
{
    ...
    public TableSerializer(org.apache.hadoop.hive.ql.metadata.Table tableHandle, Iterable<Partition> partitions, HiveConf hiveConf)
    {
    ...
    private Table updatePropertiesInTable(Table table, ReplicationSpec additionalPropertiesProvider)
    {
        ...
        if (parameters != null)
        {
            parameters.entrySet().removeIf(e -> (e.getKey().equals(ReplConst.REPL_TARGET_DB_PROPERTY) || e.getKey().equals(ReplConst.REPL_FIRST_INC_PENDING_FLAG)));
        }
        ...
        if (additionalPropertiesProvider.isInReplicationScope())
        {
            ...
            {
                table.putToParameters(ReplicationSpec.KEY.CURR_STATE_ID_SOURCE.toString(), additionalPropertiesProvider.getCurrentReplicationState());
            }
        } else
        {
