...
public abstract class VectorMapJoinFastHashTable implements VectorMapJoinHashTable
{
    ...
    {
        throw new MapJoinMemoryExhaustionError("Vector MapJoin " + dataTypeName + " Hash Table cannot grow any more -- use a smaller container size. " + "Current logical size is " + logicalHashBucketCount + " and " + "the limit is " + limit + ". " + "Estimated key count was " + (estimatedKeyCount == -1 ? "not available" : estimatedKeyCount) + ".");
    }
    ...
    public VectorMapJoinFastHashTable(boolean isFullOuter, int initialCapacity, float loadFactor, int writeBuffersSize, long estimatedKeyCount)
    {

        this.isFullOuter = isFullOuter;

        this.logicalHashBucketCount = (Long.bitCount(initialCapacity) == 1) ? initialCapacity : nextHighestPowerOfTwo(initialCapacity);
    ...
    protected final boolean checkResize()
    {
        ...
        return (resizeThreshold <= keysAssigned) || (logicalHashBucketCount <= FIRST_SIZE_UP && largestNumberOfSteps > 1);
    }
