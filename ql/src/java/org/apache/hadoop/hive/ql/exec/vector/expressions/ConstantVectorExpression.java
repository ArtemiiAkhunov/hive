        ...
        switch (category)
        {
            case PRIMITIVE:
            {
                ...
                switch (primitiveCategory)
                {
                    ...
                    case DATE:
                        return new ConstantVectorExpression(outputColumnNum, DateWritableV2.dateToDays((Date) constantValue), outputTypeInfo);
                    case TIMESTAMP:
                    ...
                    case STRING:
                        return new ConstantVectorExpression(outputColumnNum, ((String) constantValue).getBytes(StandardCharsets.UTF_8), outputTypeInfo);
                    case VARCHAR:
        ...
        } else
        {
            switch (type)
            {
                ...
                case DECIMAL:
                    ...
                    break;
                case TIMESTAMP:
                    value = org.apache.hadoop.hive.common.type.Timestamp.ofEpochMilli(timestampValue.getTime(), timestampValue.getNanos()).toString();
                    break;
                case INTERVAL_DAY_TIME:
