...
public class MemoryDecider implements PhysicalPlanResolver
{
    ...
    public class MemoryCalculator implements SemanticDispatcher
    {
        ...
        public MemoryCalculator(PhysicalContext pctx)
        {
            ...
            this.minimumHashTableSize = HiveConf.getIntVar(pctx.conf, HiveConf.ConfVars.HIVE_HYBRIDGRACE_HASHJOIN_MIN_NUM_PARTITIONS) * HiveConf.getIntVar(pctx.conf, HiveConf.ConfVars.HIVE_HYBRIDGRACE_HASHJOIN_MIN_WB_SIZE);
        ...
        private void evaluateOperators(BaseWork w, PhysicalContext pctx) throws SemanticException
        {
            ...
            rules.put(new RuleRegExp("Map join memory estimator", MapJoinOperator.getOperatorName() + "%"), new SemanticNodeProcessor()
            {
                @Override
