...
public class HiveDruidRules
{
    ...
    public static final DruidFilterRule FILTER = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Filter.class).oneInput(b1 -> b1.operand(DruidQuery.class).noInputs())).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidFilterRule.Config.class).toRule();

    public static final DruidProjectRule PROJECT = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Project.class).oneInput(b1 -> b1.operand(DruidQuery.class).noInputs())).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidProjectRule.Config.class).toRule();

    public static final DruidAggregateRule AGGREGATE = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Aggregate.class).oneInput(b1 -> b1.operand(DruidQuery.class).noInputs())).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidAggregateRule.Config.class).toRule();

    public static final DruidAggregateProjectRule AGGREGATE_PROJECT = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Aggregate.class).oneInput(b1 -> b1.operand(Project.class).oneInput(b2 -> b2.operand(DruidQuery.class).noInputs()))).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidAggregateProjectRule.Config.class).toRule();

    public static final DruidSortRule SORT = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Sort.class).oneInput(b1 -> b1.operand(DruidQuery.class).noInputs())).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidSortRule.Config.class).toRule();

    public static final SortProjectTransposeRule SORT_PROJECT_TRANSPOSE = SortProjectTransposeRule.Config.DEFAULT.withOperandFor(Sort.class, Project.class, DruidQuery.class).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(SortProjectTransposeRule.Config.class).toRule();

    public static final ProjectFilterTransposeRule PROJECT_FILTER_TRANSPOSE = ProjectFilterTransposeRule.Config.DEFAULT.withOperandFor(Project.class, Filter.class, DruidQuery.class).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(ProjectFilterTransposeRule.Config.class).toRule();

    public static final FilterProjectTransposeRule FILTER_PROJECT_TRANSPOSE = CoreRules.FILTER_PROJECT_TRANSPOSE.config.withOperandFor(Filter.class, Project.class, DruidQuery.class).withCopyFilter(true).withCopyProject(true).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(FilterProjectTransposeRule.Config.class).toRule();

    public static final AggregateFilterTransposeRule AGGREGATE_FILTER_TRANSPOSE = CoreRules.AGGREGATE_FILTER_TRANSPOSE.config.withOperandFor(Aggregate.class, Filter.class, DruidQuery.class).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(AggregateFilterTransposeRule.Config.class).toRule();

    public static final FilterAggregateTransposeRule FILTER_AGGREGATE_TRANSPOSE = CoreRules.FILTER_AGGREGATE_TRANSPOSE.config.withOperandFor(Filter.class, Aggregate.class, DruidQuery.class).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(FilterAggregateTransposeRule.Config.class).toRule();

    public static final DruidPostAggregationProjectRule POST_AGGREGATION_PROJECT = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Project.class).oneInput(b1 -> b1.operand(DruidQuery.class).noInputs())).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidPostAggregationProjectRule.Config.class).toRule();

    public static final DruidHavingFilterRule HAVING_FILTER_RULE = RelRule.Config.EMPTY.withOperandSupplier(b0 -> b0.operand(Filter.class).oneInput(b1 -> b1.operand(DruidQuery.class).noInputs())).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).as(DruidHavingFilterRule.Config.class).toRule();
    ...
    public static final DateRangeRules.FilterDateRangeRule FILTER_DATE_RANGE_RULE = new DateRangeRules.FilterDateRangeRule(HiveRelFactories.HIVE_BUILDER);
    ...
    public static class AggregateExpandDistinctAggregatesDruidRule extends RelOptRule
    {
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            final long numCountDistinct = aggregate.getAggCallList().stream().filter(aggregateCall -> aggregateCall.getAggregation().getKind().equals(SqlKind.COUNT) && aggregateCall.isDistinct()).count();
