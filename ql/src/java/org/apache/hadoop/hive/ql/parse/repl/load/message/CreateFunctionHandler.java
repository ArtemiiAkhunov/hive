...
public class CreateFunctionHandler extends AbstractMessageHandler
{
    ...
    private static class FunctionDescBuilder
    {
        ...
        {
            replCopyTasks.clear();
            PrimaryToReplicaResourceFunction conversionFunction = new PrimaryToReplicaResourceFunction(context, metadata, destinationDbName);
            ...
            List<ResourceUri> transformedUris = (metadata.function.getResourceUris() == null) ? null : ImmutableList.copyOf(Lists.transform(metadata.function.getResourceUris(), conversionFunction));
            replCopyTasks.addAll(conversionFunction.replCopyTasks);
            String fullQualifiedFunctionName = FunctionUtils.qualifyFunctionName(metadata.function.getFunctionName(), destinationDbName);
            ...
            return new CreateFunctionDesc(fullQualifiedFunctionName, metadata.function.getClassName(), false, transformedUris, replSpec);
        }
    }

    static class PrimaryToReplicaResourceFunction implements Function<ResourceUri, ResourceUri>
    {
        private final Context context;
        private final MetaData metadata;
        private final List<Task<?>> replCopyTasks = new ArrayList<>();
        ...
        PrimaryToReplicaResourceFunction(Context context, MetaData metadata, String destinationDbName)
        {
            this.context = context;
        ...
        ResourceUri destinationResourceUri(ResourceUri resourceUri) throws IOException, SemanticException
        {
            ...
            Path qualifiedDestinationPath = PathBuilder.fullyQualifiedHDFSUri(pathBuilder.addDescendant(destinationDbName.toLowerCase()).addDescendant(metadata.function.getFunctionName().toLowerCase()).addDescendant(String.valueOf(Time.monotonicNowNanos())).addDescendant(split[split.length - 1]).build(), new Path(functionsRootDir).getFileSystem(context.hiveConf));
        ...
        private Task<?> getCopyTask(String sourceUri, Path dest)
        {
            ...
            if (copyAtLoad)
            {
                return ReplCopyTask.getLoadCopyTask(metadata.getReplicationSpec(), new Path(sourceUri), dest, context.hiveConf, context.getDumpDirectory(), context.getMetricCollector());
            } else
            {
                //CopyTask expects the destination directory, hence we pass the parent of the actual destination path
                return TaskFactory.get(new CopyWork(new Path(sourceUri), dest.getParent(), true, false, context.getDumpDirectory(), context.getMetricCollector(), true), context.hiveConf);
            }
        }
    }
}
