...
public class ExternalCache implements FooterCache
{
    ...
    public void configure(HiveConf queryConfig)
    {
        ...
        this.isPpdEnabled = HiveConf.getBoolVar(conf, ConfVars.HIVE_OPT_INDEX_FILTER) && HiveConf.getBoolVar(conf, ConfVars.HIVE_ORC_MS_FOOTER_CACHE_PPD);
    ...
    public void getAndValidate(List<HdfsFileStatusWithId> files, boolean isOriginal, OrcTail[] result, ByteBuffer[] ppdResult) throws IOException, HiveException
    {
        ...
        {
            Iterator<Entry<Long, MetadataPpdResult>> iter = cache.getFileMetadataByExpr(fileIds, serializedSarg, false); // don't fetch the footer, PPD happens in MS.
            while (iter.hasNext())
            {
    ...
    private int getAndVerifyIndex(HashMap<Long, Integer> posMap, List<HdfsFileStatusWithId> files, OrcTail[] result, Long fileId)
    {
    ...
    private boolean processBbResult(ByteBuffer bb, int ix, HdfsFileStatusWithId file, OrcTail[] result) throws IOException
    {
        if (bb == null)
        {
            return true;
    ...
    private void processPpdResult(MetadataPpdResult mpr, HdfsFileStatusWithId file, int ix, OrcTail[] result, ByteBuffer[] ppdResult) throws IOException
    {
        if (mpr == null)
        {
    ...
    private List<Long> determineFileIdsToQuery(List<HdfsFileStatusWithId> files, OrcTail[] result, HashMap<Long, Integer> posMap)
    {
    ...
    public interface ExternalFooterCachesByConf
    {
        ...
        {
            Iterator<Map.Entry<Long, MetadataPpdResult>> getFileMetadataByExpr(List<Long> fileIds, ByteBuffer serializedSarg, boolean doGetFooters) throws HiveException;

            void clearFileMetadata(List<Long> fileIds) throws HiveException;

            Iterator<Map.Entry<Long, ByteBuffer>> getFileMetadata(List<Long> fileIds) throws HiveException;

            void putFileMetadata(ArrayList<Long> keys, ArrayList<ByteBuffer> values) throws HiveException;
        }
