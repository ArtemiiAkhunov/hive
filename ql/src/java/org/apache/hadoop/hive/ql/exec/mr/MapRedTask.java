...
public class MapRedTask extends ExecDriver implements Serializable
{
    ...
    public int execute()
    {
        ...
        try
        {
            ...
            if (!ctx.isLocalOnlyExecutionMode() && conf.getBoolVar(HiveConf.ConfVars.LOCAL_MODE_AUTO))
            {
    ...
    static void configureDebugVariablesForChildJVM(Map<String, String> environmentVariables)
    {
        ...
        if (environmentVariables.get(HIVE_DEBUG_RECURSIVE).equals("y"))
        {
            ...
            assert environmentVariables.containsKey(HADOOP_OPTS_KEY) && environmentVariables.get(HADOOP_OPTS_KEY) != null : HADOOP_OPTS_KEY + " environment variable must have been set.";
            environmentVariables.put(HADOOP_OPTS_KEY, environmentVariables.get(HADOOP_OPTS_KEY).replace(environmentVariables.get(HADOOP_CLIENT_OPTS), ""));
    ...
    private void setNumberOfReducers() throws IOException
    {
        ...
        } else
        {
            ...
            console.printInfo("  set " + HiveConf.ConfVars.BYTES_PER_REDUCER.varname + "=<number>");
            console.printInfo("In order to limit the maximum number of reducers:");
            console.printInfo("  set " + HiveConf.ConfVars.MAX_REDUCERS.varname + "=<number>");
            console.printInfo("In order to set a constant number of reducers:");
            console.printInfo("  set " + HiveConf.ConfVars.HADOOP_NUM_REDUCERS + "=<number>");
        }
    }
    ...
    public static String isEligibleForLocalMode(HiveConf conf, int numReducers, long inputLength, long inputFileCount)
    {
        ...
        {
            return "Input Size (= " + inputLength + ") is larger than " + HiveConf.ConfVars.LOCAL_MODE_MAX_BYTES.varname + " (= " + maxBytes + ")";
        }
        ...
        {
            return "Number of Input Files (= " + inputFileCount + ") is larger than " + HiveConf.ConfVars.LOCAL_MODE_MAX_INPUT_FILES.varname + "(= " + maxInputFiles + ")";
        }
    ...
    {
        if (queryDisplay != null && conf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_SHOW_STATS) && conf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_SHOW_GRAPH) && conf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_WEBUI_EXPLAIN_OUTPUT))
        {
            try
            {
