...
public class HadoopJobExecHelper
{
    ...
    public HadoopJobExecHelper(JobConf job, LogHelper console, Task<?> task, HadoopJobExecHook hookCallBack)
    {
    ...
    public boolean checkFatalErrors(Counters ctrs, StringBuilder errMsg)
    {
        ...
        Counters.Counter cntr = ctrs.findCounter(HiveConf.getVar(job, ConfVars.HIVE_COUNTER_GROUP), Operator.HIVE_COUNTER_CREATED_FILES);
    ...
    private MapRedStats progress(ExecDriverTaskHandle th) throws IOException, LockException
    {
        ...
        if (ss != null)
        {
            ...
            if (ctrs != null)
            {
                Counter counter = ctrs.findCounter(ss.getConf().getVar(HiveConf.ConfVars.HIVE_COUNTER_GROUP), FileSinkOperator.TOTAL_TABLE_ROWS_WRITTEN);
                if (counter != null)
                {
    ...
    public void jobInfo(RunningJob rj)
    {
        ...
        } else
        {
            ...
            console.printInfo("Kill Command = " + HiveConf.getVar(job, ConfVars.MAPRED_BIN) + " job  -kill " + rj.getID());
        }
    }
