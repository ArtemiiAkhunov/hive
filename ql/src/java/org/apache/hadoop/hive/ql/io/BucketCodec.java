    ...
    V1(1)
            {
                ...
                public int encode(AcidOutputFormat.Options options)
                {
                    ...
                    if (maxStatementId < 0)
                    {              // uninitialized, use the default
                        maxStatementId = MAX_STATEMENT_ID;
                    } else if (maxStatementId == 0)
                    {      // single statement tx, id starts from zero, set it to one to make the below logic work
                        maxStatementId = 1;
                    }
                    Preconditions.checkArgument(maxStatementId >= 0 && maxStatementId <= MAX_STATEMENT_ID, "Max Statement ID out of range: " + bucketId);
                    Preconditions.checkArgument(statementId >= 0 && statementId <= MAX_STATEMENT_ID, "Statement ID out of range: " + statementId);
                    ...
                    return this.version << (1 + NUM_BUCKET_ID_BITS + 4 + NUM_STATEMENT_ID_BITS) | bucketId << (4 + NUM_STATEMENT_ID_BITS) | Math.max(0, statementId);
                }
            };
