...
public class WindowingSpec
{
    ...
    private void setAndValidateOrderSpec(WindowFunctionSpec wFn) throws SemanticException
    {
        ...
        if (wFrame.getWindowType() == WindowType.RANGE)
        {
            ...
            boolean defaultPreceding = start.getDirection() == Direction.PRECEDING && !start.isBounded() && end.getDirection() == Direction.CURRENT;
            boolean defaultFollowing = start.getDirection() == Direction.CURRENT && end.getDirection() == Direction.FOLLOWING && !end.isBounded();
            boolean defaultPrecedingFollowing = start.getDirection() == Direction.PRECEDING && !start.isBounded() && end.getDirection() == Direction.FOLLOWING && !end.isBounded();
    ...
    public static class WindowSpec
    {
        ...
        protected void ensureOrderSpec(WindowFunctionSpec wFn) throws SemanticException
        {
            if (getOrder() == null)
            {
                 ...
                 */
                if (windowFrame.getWindowType() == WindowType.RANGE && (windowFrame.getStart().isBoundedNotCurrent() || windowFrame.getEnd().isBoundedNotCurrent()))
                {
                    throw new SemanticException("RANGE with offset PRECEDING/FOLLOWING requires at least one ORDER BY column");
                }
        ...
        public String toString()
        {
            return String.format("Window Spec=[%s%s%s]", sourceId == null ? "" : "Name='" + sourceId + "'", partitioning == null ? "" : partitioning, windowFrame == null ? "" : windowFrame);
        }
    }
