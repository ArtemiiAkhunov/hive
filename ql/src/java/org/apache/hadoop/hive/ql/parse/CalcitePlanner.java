...
public class CalcitePlanner extends SemanticAnalyzer
{
    ...
    private static final CommonToken FROM_TOKEN = new ImmutableCommonToken(HiveParser.TOK_FROM, "TOK_FROM");
    private static final CommonToken DEST_TOKEN = new ImmutableCommonToken(HiveParser.TOK_DESTINATION, "TOK_DESTINATION");
    private static final CommonToken DIR_TOKEN = new ImmutableCommonToken(HiveParser.TOK_DIR, "TOK_DIR");
    private static final CommonToken TMPFILE_TOKEN = new ImmutableCommonToken(HiveParser.TOK_TMP_FILE, "TOK_TMP_FILE");
    private static final CommonToken SELECT_TOKEN = new ImmutableCommonToken(HiveParser.TOK_SELECT, "TOK_SELECT");
    private static final CommonToken SELEXPR_TOKEN = new ImmutableCommonToken(HiveParser.TOK_SELEXPR, "TOK_SELEXPR");
    private static final CommonToken TABLEORCOL_TOKEN = new ImmutableCommonToken(HiveParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL");
    private static final CommonToken INSERT_TOKEN = new ImmutableCommonToken(HiveParser.TOK_INSERT, "TOK_INSERT");
    private static final CommonToken QUERY_TOKEN = new ImmutableCommonToken(HiveParser.TOK_QUERY, "TOK_QUERY");
    private static final CommonToken SUBQUERY_TOKEN = new ImmutableCommonToken(HiveParser.TOK_SUBQUERY, "TOK_SUBQUERY");
     ...
     */
    private static final List<Class<? extends RelNode>> HIVE_REL_NODE_CLASSES = ImmutableList.of(RelNode.class, AbstractRelNode.class, RelSubset.class, HepRelVertex.class, ConverterImpl.class, AbstractConverter.class,

            HiveTableScan.class, HiveAggregate.class, HiveAntiJoin.class, HiveExcept.class, HiveFilter.class, HiveIntersect.class, HiveJoin.class, HiveMultiJoin.class, HiveProject.class, HiveRelNode.class, HiveSemiJoin.class, HiveSortExchange.class, HiveSortLimit.class, HiveTableFunctionScan.class, HiveUnion.class,

            DruidQuery.class,

            HiveJdbcConverter.class, JdbcHiveTableScan.class, JdbcAggregate.class, JdbcFilter.class, JdbcJoin.class, JdbcProject.class, JdbcSort.class, JdbcUnion.class);
    ...
    {
        final Double maxSplitSize = (double) HiveConf.getLongVar(conf, HiveConf.ConfVars.MAPRED_MAX_SPLIT_SIZE);
        final Double maxMemory = (double) HiveConf.getLongVar(conf, HiveConf.ConfVars.HIVE_CONVERT_JOIN_NOCONDITIONAL_TASK_THRESHOLD);
        ...
        HivePlannerContext confContext = new HivePlannerContext(algorithmsConf, registry, calciteConfig, new HiveConfPlannerContext(isCorrelatedColumns, heuristicMaterializationStrategy, isExplainPlan), statsSource);
     ...
     * @param qb     top level QB corresponding to the AST
     * @param cboCtx
     * @return boolean
     * <p>
     * Assumption:<br>
     ...
     * @param conf
     * @param topLevelQB      Does QB corresponds to top most query block?
    ...
    ASTNode reAnalyzeViewAfterCbo(ASTNode newAst) throws SemanticException
    {
        ...
        if (newAst == null)
        {
            LOG.error("analyzeCreateTable failed to initialize materialized view after CBO;" + " new ast is " + getAST().dump());
    ...
    public String getOptimizedSql(RelNode optimizedOptiqPlan)
    {
        ...
        SqlDialect dialect = new HiveSqlDialect(SqlDialect.EMPTY_CONTEXT.withDatabaseProduct(SqlDialect.DatabaseProduct.HIVE).withDatabaseMajorVersion(4) // TODO: should not be hardcoded
                .withDatabaseMinorVersion(0).withIdentifierQuoteString("`").withDataTypeSystem(new HiveTypeSystemImpl()).withNullCollation(nullCollation))
        {
            @Override
    ...
    {
        ASTNode optiqOptimizedAST = ASTConverter.convert(optimizedOptiqPlan, resultSchema, HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_COLUMN_ALIGNMENT), ctx.getPlanMapper());
    ...
    {
        private static final Field CAUSE_FIELD = getField(Throwable.class, "cause"), TARGET_FIELD = getField(InvocationTargetException.class, "target"), MESSAGE_FIELD = getField(Throwable.class, "detailMessage");
    ...
    {
        JOIN_REORDERING, WINDOWING_POSTPROCESSING, REFERENTIAL_CONSTRAINTS;
    }
    ...
    public class CalcitePlannerAction implements Frameworks.PlannerAction<RelNode>
    {
        ...
        public RelNode apply(RelOptCluster cluster, RelOptSchema relOptSchema, SchemaPlus rootSchema)
        {
            ...
            try
            {
                ...
                resultSchema = convertRowSchemaToResultSetSchema(relToHiveRR.get(calcitePlan), (forViewCreation || getQB().isMaterializedView()) ? false : HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_RESULTSET_USE_UNIQUE_COLUMN_NAMES));
            } catch (SemanticException e)
            {
                semanticException = e;
                throw new RuntimeException(e);
            ...
            calcitePlan = applyMaterializedViewRewritingByText(ast, calcitePlan, optCluster, mdProvider.getMetadataProvider());
            ...
            setMaterializationValidationResult(materializationValidator.getAutomaticRewritingValidationResult());
            ...
            if (conf.getBoolVar(ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING) && !getQB().isMaterializedView() && !ctx.isLoadingMaterializedView() && !getQB().isCTAS() && getQB().hasTableDefined() && !forViewCreation)
            {
                calcitePlan = applyMaterializedViewRewriting(planner, calcitePlan, mdProvider.getMetadataProvider(), executorProvider);
                if (LOG.isDebugEnabled())
                {
        ...
        protected RelNode applyPreJoinOrderingTransforms(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            //0. SetOp rewrite
            generatePartialProgram(program, true, HepMatchOrder.BOTTOM_UP, HiveProjectOverIntersectRemoveRule.INSTANCE, HiveIntersectMergeRule.INSTANCE, HiveUnionMergeRule.INSTANCE);
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveIntersectRewriteRule.INSTANCE);
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveExceptRewriteRule.INSTANCE);
            ...
            if (!conf.getVar(HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("mr") && conf.getBoolVar(HiveConf.ConfVars.HIVE_OPTIMIZE_DISTINCT_REWRITE))
            {
                ...
                generatePartialProgram(program, true, HepMatchOrder.TOP_DOWN, HiveExpandDistinctAggregatesRule.INSTANCE);
            }
            ...
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, new HivePreFilteringRule(maxCNFNodeCount));
            ...
            generatePartialProgram(program, true, HepMatchOrder.BOTTOM_UP, rules.toArray(new RelOptRule[0]));
            ...
            if (conf.getBoolVar(HiveConf.ConfVars.HIVE_OPTIMIZE_LIMIT_TRANSPOSE))
            {
                ...
                generatePartialProgram(program, true, HepMatchOrder.TOP_DOWN, HiveSortMergeRule.INSTANCE, HiveSortProjectTransposeRule.INSTANCE, HiveSortJoinReduceRule.INSTANCE, HiveSortUnionReduceRule.INSTANCE);
                generatePartialProgram(program, true, HepMatchOrder.BOTTOM_UP, new HiveSortRemoveRule(reductionProportion, reductionTuples), HiveProjectSortTransposeRule.INSTANCE);
            }
      ...
      perfLogger.PerfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER,
        "Calcite: Prejoin ordering transformation, Push Down Semi Joins"); */
            ...
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveSortLimitRemoveRule.INSTANCE);

            // 6. Apply Partition Pruning
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, new HivePartitionPruneRule(conf));

            // 7. Projection Pruning (this introduces select above TS & hence needs to be run last due to PP)
            generatePartialProgram(program, false, HepMatchOrder.TOP_DOWN, new HiveFieldTrimmerRule(true));
            ...
            generatePartialProgram(program, true, HepMatchOrder.TOP_DOWN, HiveFilterProjectTSTransposeRule.INSTANCE, HiveFilterProjectTSTransposeRule.INSTANCE_DRUID, HiveProjectFilterPullUpConstantsRule.INSTANCE, HiveProjectMergeRule.INSTANCE, ProjectRemoveRule.Config.DEFAULT.toRule(), HiveSortMergeRule.INSTANCE);

            // 9. Get rid of sq_count_check if group by key is constant
            if (conf.getBoolVar(ConfVars.HIVE_REMOVE_SQ_COUNT_CHECK))
            {
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveRemoveSqCountCheck.INSTANCE);
            }
            ...
            {
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveAntiSemiJoinRule.INSTANCE);
            }
            ...
            perfLogger.perfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER, "Calcite: Prejoin ordering transformation");
        ...
        {
            return mvRebuildMode != MaterializationRebuildMode.NONE || ctx.isLoadingMaterializedView() || getQB().isMaterializedView();
        }
        ...
        protected RelNode applyMaterializedViewRewriting(RelOptPlanner planner, RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            final boolean useMaterializedViewsRegistry = !conf.get(HiveConf.ConfVars.HIVE_SERVER2_MATERIALIZED_VIEWS_REGISTRY_IMPL.varname).equals("DUMMY");
            ...
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveInBetweenExpandRule.FILTER_INSTANCE, HiveInBetweenExpandRule.JOIN_INSTANCE, HiveInBetweenExpandRule.PROJECT_INSTANCE);
            basePlan = executeProgram(basePlan, program.build(), mdProvider, executorProvider);
            ...
            if (!ruleExclusionRegex.isEmpty())
            {
                LOG.info("The CBO rules matching the following regex are excluded from planning: {}", ruleExclusionRegex);
        ...
        {
            return conf.getBoolVar(ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING_SQL) && !HiveMaterializedViewsRegistry.get().isEmpty() && mvRebuildMode == MaterializationRebuildMode.NONE && !rootQB.isMaterializedView() && !ctx.isLoadingMaterializedView() && !rootQB.isCTAS() && rootQB.getIsQuery() && rootQB.hasTableDefined() && !forViewCreation;
        }
        ...
        private RelNode applyMaterializedViewRewritingByText(ASTNode queryToRewriteAST, RelNode originalPlan, RelOptCluster optCluster, RelMetadataProvider metadataProvider)
        {
            ...
            try
            {
                ...
                return new HiveMaterializedViewASTSubQueryRewriteShuttle(subQueryMap, queryToRewriteAST, expandedAST, HiveRelFactories.HIVE_BUILDER.create(optCluster, null), db, tablesUsedByOriginalPlan, getTxnMgr()).rewrite(originalPlan);
            } catch (Exception e)
            {
                LOG.warn("Automatic materialized view query rewrite failed. expanded query text: {} AST string {} ", expandedQueryText, queryToRewriteAST.toStringTree(), e);
                return originalPlan;
            }
        }
        ...
        private RelNode applyJoinOrderingTransform(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            if (profilesCBO.contains(ExtendedCBOProfile.REFERENTIAL_CONSTRAINTS))
            {
                rules.add(conf.getBoolVar(HiveConf.ConfVars.HIVE_OPT_PPD_WINDOWING) ? HiveFilterProjectTransposeRule.DETERMINISTIC_WINDOWING_ON_NON_FILTERING_JOIN : HiveFilterProjectTransposeRule.DETERMINISTIC_ON_NON_FILTERING_JOIN);
                rules.add(HiveFilterJoinRule.FILTER_ON_NON_FILTERING_JOIN);
            }
            generatePartialProgram(program, true, HepMatchOrder.BOTTOM_UP, rules.toArray(new RelOptRule[0]));
            // Join reordering
            generatePartialProgram(program, false, HepMatchOrder.BOTTOM_UP, new JoinToMultiJoinRule(HiveJoin.class), new LoptOptimizeJoinRule(HiveRelFactories.HIVE_BUILDER));
        ...
        private RelNode applyPostJoinOrderingTransform(RelNode basePlan, RelMetadataProvider mdProvider, RexExecutor executorProvider)
        {
            ...
            if (factor > 0.0)
            {
                generatePartialProgram(program, false, HepMatchOrder.TOP_DOWN, new HiveCardinalityPreservingJoinRule(factor));
            }

            // 1. Run other optimizations that do not need stats
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, ProjectRemoveRule.Config.DEFAULT.toRule(), HiveUnionMergeRule.INSTANCE, new HiveUnionSimpleSelectsToInlineTableRule(dummyTableScan), HiveAggregateProjectMergeRule.INSTANCE, HiveProjectMergeRule.INSTANCE_NO_FORCE, HiveJoinCommuteRule.INSTANCE, new HiveAggregateSortLimitRule(conf.getBoolVar(ConfVars.HIVE_DEFAULT_NULLS_LAST)));
            ...
            {
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, new HiveAggregateJoinTransposeRule(noColsMissingStats, conf.getBoolVar(ConfVars.AGGR_JOIN_TRANSPOSE), conf.getBoolVar(ConfVars.AGGR_JOIN_TRANSPOSE_UNIQUE)));
            }
            ...
            {
                generatePartialProgram(program, true, HepMatchOrder.DEPTH_FIRST, HiveSemiJoinRule.INSTANCE_PROJECT, HiveSemiJoinRule.INSTANCE_PROJECT_SWAPPED, HiveSemiJoinRule.INSTANCE_AGGREGATE, HiveSemiJoinRule.INSTANCE_AGGREGATE_SWAPPED);
            }

            // 4. convert SemiJoin + GBy to SemiJoin
            generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveRemoveGBYSemiJoinRule.INSTANCE);
            ...
            {
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveWindowingFixRule.INSTANCE);
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveWindowingLastValueRewrite.INSTANCE);
            }
            ...
            if (conf.getBoolVar(ConfVars.HIVE_ENABLE_JDBC_PUSHDOWN))
            {
                ...
                generatePartialProgram(program, true, HepMatchOrder.TOP_DOWN, rules.toArray(new RelOptRule[rules.size()]));
            }
            ...
            if (HiveConf.getBoolVar(conf, ConfVars.HIVE_CBO_RETPATH_HIVEOP))
            {
                ...
                generatePartialProgram(program, true, HepMatchOrder.BOTTOM_UP, HiveJoinProjectTransposeRule.BOTH_PROJECT_INCLUDE_OUTER, HiveJoinProjectTransposeRule.LEFT_PROJECT_INCLUDE_OUTER, HiveJoinProjectTransposeRule.RIGHT_PROJECT_INCLUDE_OUTER, HiveJoinToMultiJoinRule.INSTANCE, HiveProjectMergeRule.INSTANCE);
                ...
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, ProjectRemoveRule.Config.DEFAULT.toRule(), new ProjectMergeRule(false, HiveRelFactories.HIVE_BUILDER));
                generatePartialProgram(program, true, HepMatchOrder.TOP_DOWN, HiveFilterProjectTSTransposeRule.INSTANCE, HiveFilterProjectTSTransposeRule.INSTANCE_DRUID, HiveProjectFilterPullUpConstantsRule.INSTANCE);

                // 9.2.  Introduce exchange operators below join/multijoin operators
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveInsertExchange4JoinRule.EXCHANGE_BELOW_JOIN, HiveInsertExchange4JoinRule.EXCHANGE_BELOW_MULTIJOIN);
            } else
            {
                generatePartialProgram(program, false, HepMatchOrder.DEPTH_FIRST, HiveProjectSortExchangeTransposeRule.INSTANCE, HiveProjectMergeRule.INSTANCE);
            }
            ...
            perfLogger.perfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
            basePlan = executeProgram(basePlan, program.build(), mdProvider, executorProvider);
            perfLogger.perfLogEnd(this.getClass().getName(), PerfLogger.OPTIMIZER, "Calcite: Postjoin ordering transformation");
        ...
        private RelNode removeSubqueries(RelNode basePlan, RelMetadataProvider mdProvider)
        {
            ...
            builder.addRuleCollection(ImmutableList.of(HiveSubQueryRemoveRule.forFilter(conf), HiveSubQueryRemoveRule.forProject(conf)));
        ...
        protected RelNode executeProgram(RelNode basePlan, HepProgram program, RelMetadataProvider mdProvider, RexExecutor executorProvider, List<HiveRelOptMaterialization> materializations)
        {

            final String ruleExclusionRegex = conf.get(ConfVars.HIVE_CBO_RULE_EXCLUSION_REGEX.varname, "");

            // Create planner and copy context
            HepPlanner planner = new HepPlanner(program, basePlan.getCluster().getPlanner().getContext());
            planner.addListener(new RuleEventLogger());
        ...
        @SuppressWarnings("nls")
        private RelNode genSetOpLogicalPlan(Opcode opcode, String alias, String leftalias, RelNode leftRel, String rightalias, RelNode rightRel) throws SemanticException
        {
            ...
            for (int i = 0; i < leftRowDT.size(); i++)
            {
                ...
                {
                    unionFieldDT = TypeConverter.convert(setOpOutRR.getColumnInfos().get(i).getType(), cluster.getTypeFactory());
            ...
            switch (opcode)
            {
                case UNION:
                    ...
                    break;
                case INTERSECT:
                    setOpRel = new HiveIntersect(cluster, TraitsUtil.getDefaultTraitSet(cluster), bldr.build(), false);
                    break;
                case INTERSECTALL:
                    setOpRel = new HiveIntersect(cluster, TraitsUtil.getDefaultTraitSet(cluster), bldr.build(), true);
                    break;
                case EXCEPT:
                    setOpRel = new HiveExcept(cluster, TraitsUtil.getDefaultTraitSet(cluster), bldr.build(), false);
                    break;
                case EXCEPTALL:
                    setOpRel = new HiveExcept(cluster, TraitsUtil.getDefaultTraitSet(cluster), bldr.build(), true);
                    break;
                default:
        ...
        private RelNode genJoinRelNode(RelNode leftRel, String leftTableAlias, RelNode rightRel, String rightTableAlias, JoinType hiveJoinType, ASTNode joinCond, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR) throws SemanticException
        {
            ...
            if (joinCond != null)
            {
                ...
                if (joinCond.getType() == HiveParser.TOK_TABCOLNAME && !hiveJoinType.equals(JoinType.LEFTSEMI))
                {
             ...
             */
                ...
                Map<ASTNode, RexNode> exprNodes = RexNodeTypeCheck.genExprNodeJoinCond(joinCond, jCtx, cluster.getRexBuilder());
            ...
            if (leftSemiJoin)
            {
                ...
                RexNode nonEquiConds = HiveRelOptUtil.splitHiveJoinCondition(sysFieldList, ImmutableList.of(leftRel, rightRel), calciteJoinCond, ImmutableList.of(leftJoinKeys, rightJoinKeys), null, null);
                ...
                {
                    nonEquiConds = RexUtil.shift(nonEquiConds, leftRel.getRowType().getFieldCount(), inputRels[0].getRowType().getFieldCount() - leftRel.getRowType().getFieldCount());
                }
                calciteJoinCond = remainingEquiCond != null ? RexUtil.composeConjunction(cluster.getRexBuilder(), ImmutableList.of(remainingEquiCond, nonEquiConds), false) : nonEquiConds;
                final RelDataType combinedRowType = SqlValidatorUtil.createJoinType(cluster.getTypeFactory(), inputRels[0].getRowType(), inputRels[1].getRowType(), null, ImmutableList.of());

                if (hiveJoinType == JoinType.LEFTSEMI)
                {
                    topRel = HiveSemiJoin.getSemiJoin(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), inputRels[0], inputRels[1], HiveCalciteUtil.fixNullability(cluster.getRexBuilder(), calciteJoinCond, RelOptUtil.getFieldTypeList(combinedRowType)));
                } else
                {
                    topRel = HiveAntiJoin.getAntiJoin(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), inputRels[0], inputRels[1], HiveCalciteUtil.fixNullability(cluster.getRexBuilder(), calciteJoinCond, RelOptUtil.getFieldTypeList(combinedRowType)));
                }
            ...
            {
                final RelDataType combinedRowType = SqlValidatorUtil.createJoinType(cluster.getTypeFactory(), leftRel.getRowType(), rightRel.getRowType(), null, ImmutableList.of());
                topRR = RowResolver.getCombinedRR(leftRR, rightRR);
                final ImmutableMap<String, Integer> hiveColNameCalcitePosMap = buildHiveToCalciteColumnMap(topRR);
                calciteJoinCond = new CorrelationConverter(new InputContext(combinedRowType, hiveColNameCalcitePosMap, topRR), outerNameToPosMap, outerRR, subqueryId).apply(calciteJoinCond);
                topRel = HiveJoin.getJoin(cluster, leftRel, rightRel, HiveCalciteUtil.fixNullability(cluster.getRexBuilder(), calciteJoinCond, RelOptUtil.getFieldTypeList(combinedRowType)), calciteJoinType);
         ...
         *
         * @param aliasToRel Alias(Table/Relation alias) to RelNode; only read and not
        ...
        private RelNode genJoinLogicalPlan(QB qb, ASTNode joinParseTree, Map<String, RelNode> aliasToRel, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR) throws SemanticException
        {
            ...
            // 5. Create Join rel
            return genJoinRelNode(leftRel, leftTableAlias, rightRel, rightTableAlias, hiveJoinType, joinCond, outerNameToPosMap, outerRR);
        }
        ...
        private RelNode genTableLogicalPlan(String tableAlias, QB qb) throws SemanticException
        {
            ...
            try
            {
                ...
                if (qb.getParseInfo().getTabSample(tableAlias) != null || getNameToSplitSampleMap().containsKey(tableAlias) || (conf.getBoolVar(HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP)) && (conf.getBoolVar(HiveConf.ConfVars.HIVE_TEST_MODE)))
                {
                ...
                for (StructField structField : fields)
                {
                    ...
                    colInfo = new ColumnInfo(structField.getFieldName(), TypeInfoUtils.getTypeInfoFromObjectInspector(structField.getFieldObjectInspector()), isNullable(colName, nnc, pkc), tableAlias, false);
                ...
                for (FieldSchema part_col : tabMetaData.getPartCols())
                {
                    ...
                    colInfo = new ColumnInfo(colName, TypeInfoFactory.getPrimitiveTypeInfo(part_col.getType()), isNullable(colName, nnc, pkc), tableAlias, true);
                    rr.put(tableAlias, colName, colInfo);
                    cInfoLst.add(colInfo);
                ...
                if (tableType == TableType.NATIVE)
                {
                    ...
                    if (tabMetaData.isNonNative() && tabMetaData.getStorageHandler().areSnapshotsSupported() && isBlank(tabMetaData.getMetaTable()))
                    {
                ...
                if (tableType == TableType.DRUID || (tableType == TableType.JDBC && tabMetaData.getProperty(Constants.JDBC_TABLE) != null))
                {
                    ...
                    if (tableType == TableType.DRUID)
                    {
                        ...
                        optTable = new RelOptHiveTable(relOptSchema, relOptSchema.getTypeFactory(), fullyQualifiedTabName, rowType, tabMetaData, nonPartitionColumns, partitionColumns, virtualCols, conf, db, tabNameToTabObject, partitionCache, colStatsCache, noColsMissingStats);
                        final TableScan scan = new HiveTableScan(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), optTable, null == tableAlias ? tabMetaData.getTableName() : tableAlias, getAliasId(tableAlias, qb), HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP), qb.isInsideView() || qb.getAliasInsideView().contains(tableAlias.toLowerCase()), tableScanTrait);
                        tableRel = DruidQuery.create(cluster, cluster.traitSetOf(BindableConvention.INSTANCE), optTable, druidTable, ImmutableList.of(scan), DruidSqlOperatorConverter.getDefaultMap());
                    } else
                    {
                        optTable = new RelOptHiveTable(relOptSchema, relOptSchema.getTypeFactory(), fullyQualifiedTabName, rowType, tabMetaData, nonPartitionColumns, partitionColumns, virtualCols, conf, db, tabNameToTabObject, partitionCache, colStatsCache, noColsMissingStats);
                        ...
                        List<String> schemaKey = ImmutableNullableList.of(url, driver, user, pswd, dialectName, dataBaseType, catalogName, schemaName);
                        ...
                        tableRel = new HiveJdbcConverter(cluster, jdbcTableRel.getTraitSet().replace(HiveRelNode.CONVENTION), jdbcTableRel, jc, url, user);
                    }
                } else
                {
                    // Build row type from field <type, name>
        ...
        {
            RexNode filterExpression = genRexNode(filterNode, relToHiveRR.get(srcRel), outerRR, null, useCaching, cluster.getRexBuilder());

            return genFilterRelNode(filterExpression, srcRel, outerNameToPosMap, outerRR);
        }

        private RelNode genFilterRelNode(RexNode filterExpression, RelNode srcRel, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR) throws SemanticException
        {
            if (RexUtil.isLiteral(filterExpression, false) && filterExpression.getType().getSqlTypeName() != SqlTypeName.BOOLEAN)
            {
            ...
            final ImmutableMap<String, Integer> hiveColNameCalcitePosMap = this.relToHiveColNameCalcitePosMap.get(srcRel);
            filterExpression = new CorrelationConverter(new InputContext(srcRel.getRowType(), hiveColNameCalcitePosMap, relToHiveRR.get(srcRel)), outerNameToPosMap, outerRR, subqueryId).apply(filterExpression);
            RexNode factoredFilterExpression = RexUtil.pullFactors(cluster.getRexBuilder(), filterExpression);
            RelNode filterRel = new HiveFilter(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), srcRel, HiveCalciteUtil.fixNullability(cluster.getRexBuilder(), factoredFilterExpression, RelOptUtil.getFieldTypeList(srcRel.getRowType())));
        ...
        private RelNode genLateralViewPlans(QB qb, ASTNode lateralView, Map<String, RelNode> aliasToRel) throws SemanticException
        {
            ...
            // recursively)
            RelNode inputRel = next.getToken().getType() == HiveParser.TOK_LATERAL_VIEW ? genLateralViewPlans(qb, next, aliasToRel) : aliasToRel.get(getTableAlias(next));

            LateralViewPlan lateralViewPlan = new LateralViewPlan(lateralView, this.cluster, inputRel, this.relToHiveRR.get(inputRel), unparseTranslator, conf, functionHelper);

            qb.addAlias(lateralViewPlan.lateralTableAlias);
            this.relToHiveColNameCalcitePosMap.put(lateralViewPlan.lateralViewRel, buildHiveToCalciteColumnMap(lateralViewPlan.outputRR));
                ...
                while (!stack.isEmpty())
                {
                    ...
                    switch (next.getType())
                    {
                        case HiveParser.TOK_SUBQUERY_EXPR:
                            ...
                            SubQueryUtils.subqueryRestrictionCheck(qb, next, srcRel, forHavingClause, ctx, this.relToHiveRR);
        ...
        private RelNode genFilterRelNode(QB qb, ASTNode searchCond, RelNode srcRel, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR, boolean forHavingClause) throws SemanticException
        {
            ...
            if (isSubQuery)
            {
                RexNode filterExpression = genRexNode(searchCond, relToHiveRR.get(srcRel), outerRR, subQueryToRelNode, forHavingClause, cluster.getRexBuilder());

                ImmutableMap<String, Integer> hiveColNameCalcitePosMap = this.relToHiveColNameCalcitePosMap.get(srcRel);
                filterExpression = new CorrelationConverter(new InputContext(srcRel.getRowType(), hiveColNameCalcitePosMap, relToHiveRR.get(srcRel)), outerNameToPosMap, outerRR, subqueryId).apply(filterExpression);

                RelNode filterRel = new HiveFilter(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), srcRel, filterExpression);
        ...
        private RelNode genFilterLogicalPlan(QB qb, RelNode srcRel, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR, boolean forHavingClause) throws SemanticException
        {
            ...
            {
                filterRel = genFilterRelNode(qb, (ASTNode) whereClauseIterator.next().getChild(0), srcRel, outerNameToPosMap, outerRR, forHavingClause);
            }
         ...
         */
        private Pair<RelNode, RowResolver> genConstraintFilterLogicalPlan(QB qb, Pair<RelNode, RowResolver> selPair, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR) throws SemanticException
        {
            if (qb.getIsQuery())
            {
                return null;
            ...
            RexNode constraintUDF = RexNodeTypeCheck.genConstraintsExpr(conf, cluster.getRexBuilder(), getTargetTable(qb, dest), updating(dest), inputRR);
        ...
        private AggregateCall convertGBAgg(AggregateInfo agg, List<RexNode> gbChildProjLst, HashMap<String, Integer> rexNodeToPosMap, Integer childProjLstIndx) throws SemanticException
        {
            ...
            // type
            final SqlAggFunction aggregation = SqlFunctionConverter.getCalciteAggFn(agg.getAggregateName(), agg.isDistinct(), aggArgRelDTBldr.build(), aggFnRetType);
            ...
            for (FunctionHelper.FieldCollation fieldCollation : agg.getCollation())
            {
                ...
                collationList.add(new RelFieldCollation(inputIndx, DirectionUtils.codeToDirection(fieldCollation.getSortDirection()), fieldCollation.getNullOrdering().getDirection()));
            }

            return AggregateCall.create(aggregation, agg.isDistinct(), false, false, argList, -1, RelCollations.of(collationList), aggFnRetType, null);
        }

        private RelNode genGBRelNode(List<RexNode> gbExprs, List<AggregateInfo> aggInfoLst, List<Long> groupSets, RelNode srcRel) throws SemanticException
        {
            ...
            if (hasGroupSets)
            {
                Set<ImmutableBitSet> setTransformedGroupSets = new HashSet<>(groupSets.size());
            ...
            {
                aggregateCalls.add(convertGBAgg(agg, gbChildProjLst, rexNodeToPosMap, gbChildProjLst.size()));
            }
            if (hasGroupSets)
            {
            ...
            RelNode gbInputRel = HiveProject.create(srcRel, HiveCalciteUtil.fixNullability(cluster.getRexBuilder(), gbChildProjLst, RelOptUtil.getFieldTypeList(srcRel.getRowType())), null);

            HiveRelNode aggregateRel = new HiveAggregate(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), gbInputRel, groupSet, transformedGroupSets, aggregateCalls);
        ...
        private void addAlternateGByKeyMappings(ASTNode gByExpr, ColumnInfo colInfo, RowResolver gByInputRR, RowResolver gByRR)
        {
            ...
            {
                String tab_alias = unescapeIdentifier(gByExpr.getChild(0).getChild(0).getText().toLowerCase());
        ...
        private AggregateInfo getHiveAggInfo(ASTNode aggAst, int aggFnLstArgIndx, RowResolver inputRR) throws SemanticException
        {
            List<RexNode> aggParameters = new ArrayList<>();
            for (int i = 1; i <= aggFnLstArgIndx; i++)
            {
                RexNode parameterExpr = genRexNode((ASTNode) aggAst.getChild(i), inputRR, cluster.getRexBuilder());
            ...
            AggregateInfo aInfo = functionHelper.getWindowAggregateFunctionInfo(isDistinct, isAllColumns, aggName, aggParameters);
            ...
            if (aInfo == null)
            {
                ...
                aInfo = new AggregateInfo(aggParameters, TypeConverter.convert(exp.getType()), aggName, isDistinct);
            }
        ...
        private RelNode genGBLogicalPlan(QB qb, RelNode srcRel) throws SemanticException
        {
            ...
            if (selExprList.getToken().getType() == HiveParser.TOK_SELECTDI && !qb.getAllWindowingSpecs().isEmpty())
            {
                return null;
            }
            ...
            // 2. Sanity check
            if (conf.getBoolVar(HiveConf.ConfVars.HIVE_GROUPBY_SKEW) && qbp.getDistinctFuncExprsForClause(destClauseName).size() > 1)
            {
            ...
            if (hasGrpByAstExprs || hasAggregationTrees)
            {
                ...
                if (hasGrpByAstExprs)
                {
                    ...
                    for (int i = 0; i < groupByNodes.size(); ++i)
                    {
                        ...
                        Map<ASTNode, RexNode> astToRexNodeMap = genAllRexNode(groupByNode, groupByInputRowResolver, cluster.getRexBuilder());
                        RexNode groupByExpression = astToRexNodeMap.get(groupByNode);
                        if (groupByExpression == null)
                        {
                            throw new CalciteSemanticException("Invalid Column Reference: " + groupByNode.dump(), UnsupportedFeature.Invalid_column_reference);
                        }

                        addToGBExpr(groupByOutputRowResolver, groupByInputRowResolver, groupByNode, groupByExpression, groupByExpressions, outputColumnNames);
                    }
                }
                    ...
                    for (ASTNode value : aggregationTrees.values())
                    {
                        ...
                        for (int i = 1; i < value.getChildCount(); i++)
                        {
                            if (value.getChild(i).getType() == HiveParser.TOK_WITHIN_GROUP)
                            {
                                ...
                                {
                                    throw new SemanticException(ErrorMsg.WITHIN_GROUP_PARAMETER_MISMATCH, Integer.toString(aggParameters.size()), Integer.toString(orderByNode.getChildCount()));
                                }
                            ...
                            RexNode parameterExpr = genRexNode((ASTNode) value.getChild(i), groupByInputRowResolver, cluster.getRexBuilder());
                            aggParameters.add(parameterExpr);
                        }

                        AggregateInfo aInfo = functionHelper.getAggregateFunctionInfo(isDistinct, isAllColumns, aggName, aggParameters, fieldCollations);
        ...
        private RelNode genOBLogicalPlan(QB qb, Pair<RelNode, RowResolver> selPair, boolean outermostOB) throws SemanticException
        {
            ...
            if (limit != null)
            {
                ...
                RexNode offsetRN = (offset == null || offset == 0) ? null : cluster.getRexBuilder().makeExactLiteral(BigDecimal.valueOf(offset));
                RexNode fetchRN = cluster.getRexBuilder().makeExactLiteral(BigDecimal.valueOf(limit));
                sortRel = new HiveSortLimit(cluster, traitSet, obLogicalPlanGenState.getObInputRel(), canonizedCollation, offsetRN, fetchRN);
            } else
            {
        ...
        private RelNode genSBLogicalPlan(QB qb, Pair<RelNode, RowResolver> selPair, boolean outermostOB) throws SemanticException
        {
            QBParseInfo qbp = getQBParseInfo(qb);
            String dest = qbp.getClauseNames().iterator().next();
            ...
            RelNode sortRel = HiveSortExchange.create(obLogicalPlanGenState.getObInputRel(),
                    ...
                    new HiveRelDistribution(RelDistribution.Type.ANY, RelDistributions.ANY.getKeys()), canonizedCollation, builder.build());
        ...
        private OBLogicalPlanGenState beginGenOBLogicalPlan(ASTNode obAST, Pair<RelNode, RowResolver> selPair, boolean outermostOB) throws SemanticException
        {
            ...
            for (int i = 0; i < obASTExprLst.size(); i++)
            {
                ...
                if (ref != null && ref.getToken().getType() == HiveParser.Number)
                {
                    ...
                    { // if not using position alias and it is a number.
                        LOG.warn("Using constant number " + ref.getText() + " in order by. If you try to use position alias when hive.orderby.position.alias is false, " + "the position alias will be ignored.");
                    }
                } else
                {
            ...
            if (!newVCLst.isEmpty())
            {
                ...
                {
                    obSyntheticProjectRR.putExpression(astTypePair.getKey(), new ColumnInfo(SemanticAnalyzer.getColumnInternalName(vcolPos), astTypePair.getValue(), null, false));
                    vcolPos++;
                }
        ...
        private RexNode getOrderByExpression(RowResolver selectOutputRR, RowResolver inputRR, ASTNode orderByNode, ASTNode ref) throws SemanticException
        {
            ...
            if (selectOutputRR != null)
            {
                ...
                } catch (SemanticException ex)
                {
                    ...
                    LOG.debug("Can not find column in " + ref.getText() + ". The error msg is " + ex.getMessage());
                }
            }
        ...
        public RelNode endGenOBLogicalPlan(OBLogicalPlanGenState obLogicalPlanGenState, RelNode sortRel) throws CalciteSemanticException
        {
            ...
            ImmutableMap<String, Integer> hiveColNameCalcitePosMap = buildHiveToCalciteColumnMap(obLogicalPlanGenState.getOutputRR());
            ...
            if (obLogicalPlanGenState.getSelectOutputRR() != null)
            {
                ...
                List<RexNode> selectedRefs = originalInputRefs.subList(0, obLogicalPlanGenState.getSelectOutputRR().getColumnInfos().size());
        ...
        private List<RexNode> getPartitionKeys(PartitionSpec ps, RowResolver inputRR) throws SemanticException
        {
        ...
        private List<RexFieldCollation> getOrderKeys(OrderSpec os, RowResolver inputRR) throws SemanticException
        {
        ...
        private RexWindowBound getBound(BoundarySpec bs)
        {
            ...
            if (bs != null)
            {
                ...
                SqlNode amt = bs.getAmt() == 0 || bs.getAmt() == BoundarySpec.UNBOUNDED_AMOUNT ? null : SqlLiteral.createExactNumeric(String.valueOf(bs.getAmt()), new SqlParserPos(2, 2));
                RexNode amtLiteral = null;
                SqlCall sc = null;

                if (amt != null)
                {
                    amtLiteral = cluster.getRexBuilder().makeLiteral(Integer.valueOf(bs.getAmt()), cluster.getTypeFactory().createSqlType(SqlTypeName.INTEGER), true);
                }
        ...
        private Pair<RexNode, TypeInfo> genWindowingProj(WindowExpressionSpec wExpSpec, RelNode srcRel) throws SemanticException
        {
            RexNode w = null;
            ...
            if (wExpSpec instanceof WindowFunctionSpec)
            {
                ...
                AggregateInfo hiveAggInfo = getHiveAggInfo(windowProjAst, wndSpecASTIndx - 1, this.relToHiveRR.get(srcRel));

                // 3. Get Calcite Return type for Agg Fn
                wHiveRetType = hiveAggInfo.getReturnType();
                RelDataType calciteAggFnRetType = TypeConverter.convert(hiveAggInfo.getReturnType(), this.cluster.getTypeFactory());
                ...
                final SqlAggFunction calciteAggFn = SqlFunctionConverter.getCalciteAggFn(hiveAggInfo.getAggregateName(), hiveAggInfo.isDistinct(), calciteAggFnArgsType, calciteAggFnRetType);
                ...
                w = cluster.getRexBuilder().makeOver(calciteAggFnRetType, calciteAggFn, calciteAggFnArgs, partitionKeys, ImmutableList.<RexFieldCollation>copyOf(orderKeys), lowerBound, upperBound, isRows, true, false, hiveAggInfo.isDistinct(), !wndFuncSpec.isRespectNulls());
            } else
            {
        ...
        private RelNode genSelectForWindowing(QB qb, RelNode srcRel, HashSet<ColumnInfo> newColumns) throws SemanticException
        {
            ...
            for (WindowExpressionSpec wExprSpec : windowExpressions)
            {
                if (!qbp.getDestToGroupBy().isEmpty())
                {
                    ...
                    wExprSpec.setExpression(rewriteGroupingFunctionAST(getGroupByForClause(qbp, selClauseName), wExprSpec.getExpression(), !cubeRollupGrpSetPresent));
                }
        ...
        private RelNode genSelectRelNode(List<RexNode> calciteColLst, RowResolver out_rwsch, RelNode srcRel, List<WindowExpressionSpec> windowExpressions) throws CalciteSemanticException
        {
            ...
            HiveRelNode selRel = HiveProject.create(srcRel, HiveCalciteUtil.fixNullability(cluster.getRexBuilder(), calciteColLst, RelOptUtil.getFieldTypeList(srcRel.getRowType())), columnNames);
        ...
        private void setQueryHints(QB qb) throws SemanticException
        {
            ...
            String hint = ctx.getTokenRewriteStream().toString(selExpr0.getTokenStartIndex(), selExpr0.getTokenStopIndex());
        ...
        {
            Pair<RelNode, RowResolver> retNodeRR = internalGenSelectLogicalPlan(qb, srcRel, starSrcRel, outerNameToPosMap, outerRR, isAllColRefRewrite);
         ...
         *
         * @param isAllColRefRewrite when it is true, it means that it is called from group by *, where we use
         ...
         */
        private Pair<RelNode, RowResolver> internalGenSelectLogicalPlan(QB qb, RelNode srcRel, RelNode starSrcRel, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR, boolean isAllColRefRewrite) throws SemanticException
        {
            ...
            if (exprType == HiveParser.TOK_FUNCTION || exprType == HiveParser.TOK_FUNCTIONSTAR)
            {
                ...
                if (fi != null && fi.getGenericUDTF() != null)
                {
                    ...
                    {
                        genRexNodeRegex(".*", null, (ASTNode) expr.getChild(0), columnList, null, inputRR, starRR, pos, outputRR, qb.getAliases(), false);
                    }
                }
            }
            ...
            for (int i = startPosn; i < exprList.getChildCount(); ++i)
            {
                ...
                } else
                {
                    ...
                    String[] colRef = getColAlias(child, getAutogenColAliasPrfxLbl(), inputRR, autogenColAliasPrfxIncludeFuncName(), i);
                    tabAlias = colRef[0];
                    colAlias = colRef[1];
                    if (hasAsClause)
                    {
                ...
                if (isSubQuery)
                {
                    RexNode subQueryExpr = genRexNode(expr, relToHiveRR.get(srcRel), outerRR, subQueryToRelNode, true, cluster.getRexBuilder());
                    columnList.add(subQueryExpr);
                    ColumnInfo colInfo = new ColumnInfo(SemanticAnalyzer.getColumnInternalName(pos), TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(TypeConverter.convert(subQueryExpr.getType())), tabAlias, false);
                ...
                } else
                {
                    ...
                    {
                        pos = genRexNodeRegex(".*", expr.getChildCount() == 0 ? null : getUnescapedName((ASTNode) expr.getChild(0)).toLowerCase(), expr, columnList, excludedColumns, inputRR, starRR, pos, outputRR, qb.getAliases(), true);
                    } else if (expr.getType() == HiveParser.TOK_TABLE_OR_COL && !hasAsClause && !inputRR.getIsExprResolver() && isRegex(unescapeIdentifier(expr.getChild(0).getText()), conf))
                    {
                        ...
                        pos = genRexNodeRegex(unescapeIdentifier(expr.getChild(0).getText()), null, expr, columnList, excludedColumns, inputRR, starRR, pos, outputRR, qb.getAliases(), true);
                    ...
                    } else if (ParseUtils.containsTokenOfType(expr, HiveParser.TOK_FUNCTIONDI) && !ParseUtils.containsTokenOfType(expr, HiveParser.TOK_WINDOWSPEC) && !(srcRel instanceof HiveAggregate || (srcRel.getInputs().size() == 1 && srcRel.getInput(0) instanceof HiveAggregate)))
                    {
                        ...
                        throw new CalciteSemanticException("Distinct without an aggregation.", UnsupportedFeature.Distinct_without_an_aggregation);
                    } else
                    {
            ...
            ImmutableMap<String, Integer> hiveColNameCalcitePosMap = buildHiveColNameToInputPosMap(columnList, inputRR);
            CorrelationConverter cc = new CorrelationConverter(new InputContext(srcRel.getRowType(), hiveColNameCalcitePosMap, relToHiveRR.get(srcRel)), outerNameToPosMap, outerRR, subqueryId);
            columnList = columnList.stream().map(cc::apply).collect(Collectors.toList());
            ...
            if (genericUDTF != null)
            {
                ...
                return genUDTFPlan(genericUDTF, genericUDTFName, udtfTableAlias, udtfColAliases, qb, columnList, outputRR, srcRel);
            } else
            {
                String dest = qbp.getClauseNames().iterator().next();
                ASTNode obAST = qbp.getOrderByForClause(dest);
                ...
                if ((obAST != null || sbAST != null) && !(selForWindow != null && selExprList.getToken().getType() == HiveParser.TOK_SELECTDI) && !isAllColRefRewrite)
                {
                    // 1. OB Expr sanity test
                    // in strict mode, in the presence of order by, limit must be
                    // specified
                    ...
                    for (int i = 0; i < obOrSbAST.getChildCount(); ++i)
                    {
                        ...
                        ObjectInspector oi = TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(TypeConverter.convert(obRex.getType()));
                ...
                } else
                {
                    if (qbp.getQualifyExprForClause(dest) != null)
                    {
                        ...
                        {
                            topProjectColumnList.add(qualifyRel.getCluster().getRexBuilder().makeInputRef(qualifyRel.getRowType().getFieldList().get(i).getType(), i));
                        }
            ...
            if (selForWindow != null && selExprList.getToken().getType() == HiveParser.TOK_SELECTDI)
            {
                ...
                outputRel = new HiveAggregate(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), outputRel, groupSet, null, new ArrayList<AggregateCall>());
                RowResolver groupByOutputRowResolver = new RowResolver();
        ...
        {
            List<RexNode> originalInputRefs = Lists.transform(srcRel.getRowType().getFieldList(), input -> new RexInputRef(input.getIndex(), input.getType()));
        ...
        Integer genRexNodeRegex(String colRegex, String tabAlias, ASTNode sel, List<RexNode> exprList, Set<ColumnInfo> excludeCols, RowResolver input, RowResolver colSrcRR, Integer pos, RowResolver output, List<String> aliases, boolean ensureUniqueCols) throws SemanticException
        {
            ...
            Integer i = genColListRegex(colRegex, tabAlias, sel, colList, excludeCols, input, colSrcRR, pos, output, aliases, ensureUniqueCols);
        ...
        private Pair<RelNode, RowResolver> genUDTFPlan(GenericUDTF genericUDTF, String genericUDTFName, String outputTableAlias, ArrayList<String> colAliases, QB qb, List<RexNode> selectColLst, RowResolver selectRR, RelNode input) throws SemanticException
        {
            ...
            // UDTF
            StructTypeInfo type = (StructTypeInfo) TypeConverter.convert(functionHelper.getReturnType(functionHelper.getFunctionInfo(genericUDTFName), selectColLst));
        ...
        private Pair<RelNode, RowResolver> genGBSelectDistinctPlan(Pair<RelNode, RowResolver> srcNodeRR) throws SemanticException
        {
            RelNode srcRel = srcNodeRR.left;

            RelDataType inputRT = srcRel.getRowType();
            List<Integer> groupSetPositions = IntStream.range(0, inputRT.getFieldCount()).boxed().collect(Collectors.toList());

            HiveAggregate distAgg = new HiveAggregate(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), srcRel, ImmutableBitSet.of(groupSetPositions), null, new ArrayList<AggregateCall>());
        ...
        private RelNode genLogicalPlan(QBExpr qbexpr) throws SemanticException
        {
            switch (qbexpr.getOpcode())
            {
                ...
                case EXCEPTALL:
                    ...
                    return genSetOpLogicalPlan(qbexpr.getOpcode(), qbexpr.getAlias(), qbexpr.getQBExpr1().getAlias(), qbexpr1Ops, qbexpr.getQBExpr2().getAlias(), qbexpr2Ops);
                default:
                    return null;
            }
        }
        ...
        private RelNode genLogicalPlan(QB qb, boolean outerMostQB, ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR) throws SemanticException
        {
            ...
            for (String subqAlias : qb.getSubqAliases())
            {
                ...
                if (subqueryRoot != null && conf.getBoolVar(ConfVars.HIVE_MATERIALIZED_VIEW_ENABLE_AUTO_REWRITING_SUBQUERY_SQL) && relNode instanceof HiveProject)
                {
            ...
            Pair<RelNode, RowResolver> constraintPair = genConstraintFilterLogicalPlan(qb, selPair, outerNameToPosMap, outerRR);
            ...
            if (qb.getParseInfo().getAlias() != null)
            {
                ...
                {
                    throw new SemanticException(ErrorMsg.WITH_COL_LIST_NUM_OVERFLOW, alias, Integer.toString(rr.getColumnInfos().size()), Integer.toString(targetColNames.size()));
                }
        ...
        private RelNode genGBHavingLogicalPlan(QB qb, RelNode srcRel) throws SemanticException
        {
            ...
            if (havingClause != null)
            {
                if (!(srcRel instanceof HiveAggregate))
                {
                    ...
                    throw new CalciteSemanticException("Having clause without any group-by.", UnsupportedFeature.Having_clause_without_any_groupby);
                }
        ...
        {
            final ImmutableBitSet refs = RelOptUtil.InputFinder.bits(columnList, null);
            ImmutableMap.Builder<String, Integer> hiveColNameToInputPosMapBuilder = new ImmutableMap.Builder<>();
            for (int ref : refs)
            {
                hiveColNameToInputPosMapBuilder.put(inputRR.getColumnInfos().get(ref).getInternalName(), ref);
            }
    ...
    private static class OBLogicalPlanGenState
    {
        ...
        OBLogicalPlanGenState(RelNode obInputRel, List<RelFieldCollation> canonizedCollation, RowResolver selectOutputRR, RowResolver outputRR, RelNode srcRel)
        {
