...
public class GenericUDAFRank extends AbstractGenericUDAFResolver
{
    ...
    public static class GenericUDAFHypotheticalSetRankEvaluator extends GenericUDAFEvaluator
    {
        ...
        public static final ObjectInspector PARTIAL_RANK_OI = ObjectInspectorFactory.getStandardStructObjectInspector(asList(RANK_FIELD, COUNT_FIELD), asList(writableLongObjectInspector, writableLongObjectInspector));
        ...
        protected static class RankAssets
        {
            ...
            {
                return ObjectInspectorUtils.compare(inputConverter.convert(inputValue), commonInputOI, directArgumentConverter.convert(directArgumentValue), commonInputOI, new FullMapEqualComparer(), nullOrdering.getNullValueOption());
            }
        }
        ...
        public ObjectInspector init(Mode m, ObjectInspector[] parameters) throws HiveException
        {
            ...
            if (mode == Mode.PARTIAL1 || mode == Mode.COMPLETE)
            {
                ...
                for (int i = 0; i < parameters.length / 4; ++i)
                {
                    ...
                    rankAssetsList.add(new RankAssets(commonInputOI, ObjectInspectorConverters.getConverter(parameters[4 * i], commonInputOI), ObjectInspectorConverters.getConverter(parameters[4 * i + 1], commonInputOI), ((WritableConstantIntObjectInspector) parameters[4 * i + 2]).getWritableConstantValue().get(), NullOrdering.fromCode(((WritableConstantIntObjectInspector) parameters[4 * i + 3]).getWritableConstantValue().get())));
                }
            } else
            {
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException
        {
            ...
            if (compareResult.getOrder() == ASCENDING_CODE && compareResult.getCompareResult() < 0 || compareResult.getOrder() == DESCENDING_CODE && compareResult.getCompareResult() > 0)
            {
                rankBuffer.rank++;
            }
        }
