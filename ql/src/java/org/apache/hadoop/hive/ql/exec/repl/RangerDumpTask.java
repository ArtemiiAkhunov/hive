...
public class RangerDumpTask extends Task<RangerDumpWork> implements Serializable
{
    ...
    public int execute()
    {
        try
        {
            ...
            {
                throw new SemanticException(ErrorMsg.REPL_EXTERNAL_SERVICE_CONNECTION_ERROR.format(ReplUtils.REPL_RANGER_SERVICE, "Ranger endpoint is not reachable " + rangerEndpoint));
            }
            RangerExportPolicyList rangerExportPolicyList = rangerRestClient.exportRangerPolicies(rangerEndpoint, work.getDbName(), rangerHiveServiceName, conf);
            ...
            if (!CollectionUtils.isEmpty(rangerPolicies))
            {
                ...
                filePath = rangerRestClient.saveRangerPoliciesToFile(rangerExportPolicyList, work.getCurrentDumpPath(), ReplUtils.HIVE_RANGER_POLICIES_FILE_NAME, conf);
                if (filePath != null)
                {
