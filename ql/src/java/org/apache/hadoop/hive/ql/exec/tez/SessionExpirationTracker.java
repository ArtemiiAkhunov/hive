...
class SessionExpirationTracker
{
    ...
    {
        long sessionLifetimeMs = conf.getTimeVar(ConfVars.HIVE_SERVER2_TEZ_SESSION_LIFETIME, TimeUnit.MILLISECONDS);
        if (sessionLifetimeMs == 0)
        {
            return null;
        }
        return new SessionExpirationTracker(sessionLifetimeMs, conf.getTimeVar(ConfVars.HIVE_SERVER2_TEZ_SESSION_LIFETIME_JITTER, TimeUnit.MILLISECONDS), restartImpl);
    }
    ...
    private SessionExpirationTracker(long sessionLifetimeMs, long sessionLifetimeJitterMs, RestartImpl restartImpl)
    {
        ...
        LOG.debug("Session expiration is enabled; session lifetime is {} [0, {})ms", sessionLifetimeMs, sessionLifetimeJitterMs);
