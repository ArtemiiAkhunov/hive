...
class CompactionCleaner extends TaskHandler
{
    ...
    public List<Runnable> getTasks() throws MetaException
    {
        ...
        long retentionTime = HiveConf.getBoolVar(conf, HIVE_COMPACTOR_DELAYED_CLEANUP_ENABLED) ? HiveConf.getTimeVar(conf, HIVE_COMPACTOR_CLEANER_RETENTION_TIME, TimeUnit.MILLISECONDS) : 0;
    ...
    private void clean(CompactionInfo ci, long minOpenTxn, boolean metricsEnabled) throws MetaException
    {
        ...
        String cleanerMetric = MetricsConstants.COMPACTION_CLEANER_CYCLE + "_" + (!isNull(ci.type) ? ci.type.toString().toLowerCase() : null);
        try
        {
            if (metricsEnabled)
            {
            ...
            if (isNull(location))
            {
                ...
                if (isNull(t))
                {
                    ...
                    LOG.info("Unable to find table {}, assuming it was dropped. {}", ci.getFullTableName(), idWatermark(ci));
                ...
                if (!isNull(ci.partName))
                {
                    ...
                    if (isNull(p))
                    {
                        ...
                        LOG.info("Unable to find partition {}, assuming it was dropped. {}", ci.getFullPartitionName(), idWatermark(ci));
                        txnHandler.markCleaned(ci);
                        return;
                    }
            ...
            {
                String path = isNull(location) ? CompactorUtil.resolveStorageDescriptor(t, p).getLocation() : location;
        ...
        {
            LOG.error("Caught exception when cleaning, unable to complete cleaning of {} due to {}", ci, e.getMessage());
            if (metricsEnabled)
            {
    ...
    {
        ValidTxnList validTxnList = TxnUtils.createValidTxnListForCleaner(txnHandler.getOpenTxns(), minOpenTxn, false);
    ...
    protected LockRequest createLockRequest(CompactionInfo ci)
    {
        ...
        LockComponentBuilder lockCompBuilder = new LockComponentBuilder().setLock(LockType.EXCL_WRITE).setOperationType(DataOperationType.DELETE).setDbName(ci.dbname).setTableName(ci.tableName).setIsTransactional(true);
    ...
    @Override
    protected ValidReaderWriteIdList getValidCleanerWriteIdList(CompactionInfo ci, ValidTxnList validTxnList) throws NoSuchTxnException, MetaException
    {
    ...
    private CleanupRequest getCleaningRequestBasedOnLocation(CompactionInfo ci, String location)
    {
        ...
        return new CleanupRequestBuilder().setLocation(location).setDbName(ci.dbname).setFullPartitionName(ci.getFullPartitionName()).setRunAs(ci.runAs).setPurge(ifPurge).setObsoleteDirs(Collections.singletonList(obsoletePath)).build();
    }
}
