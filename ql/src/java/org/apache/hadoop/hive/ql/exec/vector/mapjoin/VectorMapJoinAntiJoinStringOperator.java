        ...
        try
        {
            ...
            } else
            {
                ...
                for (int logical = 0; logical < inputLogicalSize; logical++)
                {
                    ...
                    if (isNull || !haveSaveKey || !StringExpr.equal(vector[saveKeyBatchIndex], start[saveKeyBatchIndex], length[saveKeyBatchIndex], vector[batchIndex], start[batchIndex], length[batchIndex]))
                    {
                ...
                if (LOG.isDebugEnabled())
                {
                    LOG.debug(CLASS_NAME + " allMatches " + intArrayToRangesString(allMatches, allMatchCount) + " spills " + intArrayToRangesString(spills, spillCount) + " spillHashMapResultIndices " + intArrayToRangesString(spillHashMapResultIndices, spillCount) + " hashMapResults " + Arrays.toString(Arrays.copyOfRange(hashSetResults, 0, hashSetResultCount)));
                }
