...
public abstract class VectorPTFEvaluatorAbstractLeadLag extends VectorPTFEvaluatorBase
{
    ...
    public VectorPTFEvaluatorAbstractLeadLag(WindowFrameDef windowFrameDef, VectorExpression inputVecExpr, int outputColumnNum, Type type, int amt, VectorExpression defaultValueExpression)
    {
    ...
    public void mapCustomColumns(int[] bufferedColumnMap)
    {
        if (defaultValueColumn > -1)
        {
            defaultValueColumn = IntStream.range(0, bufferedColumnMap.length).filter(j -> bufferedColumnMap[j] == defaultValueColumn).findFirst().orElseGet(() -> defaultValueColumn);
        }
    }
}
