...
public class HiveRelDistribution implements RelDistribution
{
    ...
    public static HiveRelDistribution from(List<RelFieldCollation> fieldCollations, RelDistribution.Type distributionType)
    {
    ...
    public RelDistribution apply(TargetMapping mapping)
    {
        ...
        {
            throw new AssertionError("Remove logic in HiveRelDistribution when [CALCITE-4166] " + "has been fixed and use newKeys.add(mapping.getTargetOpt(key)); instead.");
        }
    ...
    public int compareTo(RelMultipleTrait o)
    {
        ...
        if (type == distribution.getType() && (type == Type.HASH_DISTRIBUTED || type == Type.RANGE_DISTRIBUTED))
        {
