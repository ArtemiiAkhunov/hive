...
public class AtlasRestClientImpl extends RetryingClientTimeBased implements AtlasRestClient
{
    ...
    public String getEntityGuid(final String entityType, final String attributeName, final String qualifiedName) throws SemanticException
    {
        ...
        try
        {
            AtlasEntity.AtlasEntityWithExtInfo entityWithExtInfo = runWithTimeout(() -> clientV2.getEntityByAttribute(entityType, attributes), entityApiTimeOut, TimeUnit.SECONDS);

            if (entityWithExtInfo == null || entityWithExtInfo.getEntity() == null)
            {
                LOG.warn("Atlas entity cannot be retrieved using: type: {} and {} - {}", entityType, attributeName, qualifiedName);
                return null;
            }
        ...
        } catch (AtlasServiceException e)
        {
            ...
            {
                throw new SemanticException(ErrorMsg.REPL_INVALID_INTERNAL_CONFIG_FOR_SERVICE.format("Exception " + "while getEntityGuid ", ReplUtils.REPL_ATLAS_SERVICE), e.getCause());
            }
            LOG.warn("getEntityGuid: Could not retrieve entity guid for: {}-{}-{}", entityType, attributeName, qualifiedName, e.getMessage());
            return null;
        } catch (Exception e)
        {
