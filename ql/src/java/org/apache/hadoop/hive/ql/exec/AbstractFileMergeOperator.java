...
public abstract class AbstractFileMergeOperator<T extends FileMergeDesc> extends Operator<T> implements Serializable
{
    ...
    private void updatePaths(Path tp, Path ttp)
    {
        ...
        {
            Utilities.FILE_OP_LOGGER.trace("Paths for merge " + taskId + ": tmp " + tmpPath + ", task " + taskTmpPath + ", final " + finalPath + ", out " + outPath);
        }
    }
    ...
    protected void checkPartitionsMatch(Path inputPath) throws IOException
    {
        if (!dpPath.equals(inputPath))
        {
            ...
            String msg = "Multiple partitions for one merge mapper: " + dpPath + " NOT EQUAL TO " + inputPath;
            LOG.error(msg);
        ...
        try
        {
            ...
            if (fs.exists(outPath))
            {
                ...
                if (!isMmTable)
                {
                    ...
                    LOG.info("Renamed path " + outPath + " to " + finalPath + "(" + fss.getLen() + " bytes).");
                } else
                {
                    ...
                    Utilities.writeCommitManifest(Lists.newArrayList(outPath), tmpPath.getParent(), fs, taskId, conf.getWriteId(), conf.getStmtId(), null, false, hasDynamicPartitions, new HashSet<>(), null, false);
    ...
    public void jobCloseOp(Configuration hconf, boolean success) throws HiveException
    {
        try
        {
            ...
            if (!isMmTable)
            {
                ...
                Utilities.mvFileToFinalPath(outputDir, null, hconf, success, LOG, conf.getDpCtx(), null, reporter);
                if (success)
                {
            ...
            {
                int dpLevels = dpCtx == null ? 0 : dpCtx.getNumDPCols(), lbLevels = conf.getListBucketingDepth();
                ...
                Utilities.handleDirectInsertTableFinalPath(outputDir.getParent(), null, hconf, success, dpLevels, lbLevels, null, mmWriteId, stmtId, reporter, isMmTable, false, false, false, null, AcidUtils.Operation.NOT_ACID, null);
            }
