...
public class VectorizedPrimitiveColumnReader extends BaseVectorizedColumnReader
{
    ...
    private void readDate(int total, DateColumnVector c, int rowId) throws IOException
    {
        ...
        while (left > 0)
        {
            ...
            if (definitionLevel >= maxDefLevel)
            {
                c.vector[rowId] = skipProlepticConversion ? dataColumn.readLong() : CalendarUtils.convertDateToProleptic((int) dataColumn.readLong());
                if (dataColumn.isValid())
                {
                    c.isNull[rowId] = false;
                    c.isRepeating = c.isRepeating && (c.vector[0] == c.vector[rowId]);
            ...
            if (definitionLevel >= maxDefLevel)
            {
                switch (descriptor.getType())
                {
                    ...
                    case INT64:
                        ...
                        break;
                    default:
                        throw new IOException("Unsupported parquet logical type: " + type.getLogicalTypeAnnotation().toString() + " for timestamp");
                }
                c.isNull[rowId] = false;
     ...
     */
    private void fillDecimalPrecisionScale(DecimalLogicalTypeAnnotation decimalLogicalType, DecimalColumnVector decimalColumnVector)
    {
        ...
        } else if (TypeInfoUtils.getBaseName(hiveType.getTypeName()).equalsIgnoreCase(serdeConstants.DECIMAL_TYPE_NAME))
        {
        ...
        {
            throw new UnsupportedOperationException("The underlying Parquet type cannot be converted to Hive Decimal type: " + type);
        }
    }
}
