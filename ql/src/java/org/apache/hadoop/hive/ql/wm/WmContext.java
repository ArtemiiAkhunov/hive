...
public class WmContext implements PrintSummary
{
    ...
    public void print(final SessionState.LogHelper console)
    {
        try
        {
            ...
            for (final WmEvent wmEvent : queryWmEvents)
            {
                ...
                String row = String.format(WM_EVENTS_HEADER_FORMAT, wmEvent.getEventType(), Instant.ofEpochMilli(wmEvent.getEventStartTimestamp()).toString(), Instant.ofEpochMilli(wmEvent.getEventEndTimestamp()).toString(), wmEvent.getElapsedTime(), DECIMAL_FORMAT.format(wmTezSessionInfo.getClusterPercent()), wmTezSessionInfo.getPoolName());
    ...
    public void printJson(final SessionState.LogHelper console)
    {
        try
        {
            ...
            objectMapper.setVisibility(objectMapper.getSerializationConfig().getDefaultVisibilityChecker().withSetterVisibility(JsonAutoDetect.Visibility.NONE));
    ...
    public void shortPrint(final SessionState.LogHelper console) throws ExecutionException, InterruptedException
    {
        ...
        {
            console.printInfo("Event: " + wmEvent.getEventType() + " Pool: " + wmEvent.getWmTezSessionInfo().getPoolName() + " Cluster %: " + WmContext.DECIMAL_FORMAT.format(wmEvent.getWmTezSessionInfo().getClusterPercent()));
        }
    }
