...
public class HiveWindowingLastValueRewrite extends RelOptRule
{
    ...
    private static class LastValueRewriteRexShuttle extends RexShuttle
    {
        ...
        {
            if (over.op.getName().equals(LAST_VALUE_FUNC) && over.getWindow().getLowerBound().isUnbounded() && over.getWindow().getUpperBound().isUnbounded())
            {
                ...
                SqlFunctionConverter.CalciteUDAF newSqlAggFunction = new SqlFunctionConverter.CalciteUDAF(over.isDistinct(), FIRST_VALUE_FUNC, s.getReturnTypeInference(), s.getOperandTypeInference(), s.getOperandTypeChecker());
                List<RexNode> clonedOperands = visitList(over.operands, new boolean[]{false});
                RexWindow window = visitWindow(over.getWindow());
                return rexBuilder.makeOver(over.type, newSqlAggFunction, clonedOperands, window.partitionKeys, newOrderKeys.build(), window.getLowerBound(), window.getUpperBound(), window.isRows(), true, false, over.isDistinct(), over.ignoreNulls());
            }
            return over;
        }
    }
