...
public class ExportSemanticAnalyzer extends BaseSemanticAnalyzer
{
    ...
    static Task<ExportWork> analyzeExport(ASTNode ast, @Nullable String acidTableName, Hive db, HiveConf conf, Set<ReadEntity> inputs, Set<WriteEntity> outputs) throws SemanticException
    {
        ...
        TableExport.Paths exportPaths = new TableExport.Paths(ASTErrorUtils.getMsg(ErrorMsg.INVALID_PATH.getMsg(), ast), tmpPath, conf, false);
        ...
        Utilities.FILE_OP_LOGGER.debug("Exporting table {}: MM context {}", ts == null ? null : ts.getTableName(), mmCtx);
        // Configure export work
        ExportWork exportWork = new ExportWork(exportRootDirName, ts, replicationSpec, ASTErrorUtils.getMsg(ErrorMsg.INVALID_PATH.getMsg(), ast), acidTableName, mmCtx);
