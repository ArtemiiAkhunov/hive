...
public class HiveSchemaConverter
{
    ...
    public static MessageType convert(final List<String> columnNames, final List<TypeInfo> columnTypes, Configuration conf)
    {
        final MessageType schema = new MessageType("hive_schema", convertTypes(columnNames, columnTypes, conf));
        return schema;
    }

    private static Type[] convertTypes(final List<String> columnNames, final List<TypeInfo> columnTypes, Configuration conf)
    {
    ...
    private static Type convertType(final String name, final TypeInfo typeInfo, Configuration conf, final Repetition repetition)
    {
        if (typeInfo.getCategory().equals(Category.PRIMITIVE))
        {
            if (typeInfo.equals(TypeInfoFactory.stringTypeInfo))
            {
                return Types.primitive(PrimitiveTypeName.BINARY, repetition).as(LogicalTypeAnnotation.stringType()).named(name);
            } else if (typeInfo.equals(TypeInfoFactory.intTypeInfo))
            {
                return Types.primitive(PrimitiveTypeName.INT32, repetition).named(name);
            } else if (typeInfo.equals(TypeInfoFactory.shortTypeInfo))
            {
                return Types.primitive(PrimitiveTypeName.INT32, repetition).as(LogicalTypeAnnotation.intType(16, true)).named(name);
            } else if (typeInfo.equals(TypeInfoFactory.byteTypeInfo))
            {
                return Types.primitive(PrimitiveTypeName.INT32, repetition).as(LogicalTypeAnnotation.intType(8, true)).named(name);
            ...
            } else if (typeInfo.equals(TypeInfoFactory.timestampTypeInfo))
            {
                ...
                if (useInt64)
                {
                    LogicalTypeAnnotation.TimeUnit timeUnit = LogicalTypeAnnotation.TimeUnit.valueOf(timeUnitVal.toUpperCase());
                    return Types.primitive(PrimitiveTypeName.INT64, repetition).as(LogicalTypeAnnotation.timestampType(false, timeUnit)).named(name);
                } else
                {
    ...
    private static GroupType convertArrayType(final String name, final ListTypeInfo typeInfo, final Configuration conf)
    {
        ...
        GroupType groupType = Types.optionalGroup().as(LogicalTypeAnnotation.listType()).addField(Types.repeatedGroup().addField(convertType("array_element", subType, conf)).named(ParquetHiveSerDe.ARRAY.toString())).named(name);
        return groupType;
    }

    // An optional group containing multiple elements
    private static GroupType convertStructType(final String name, final StructTypeInfo typeInfo, final Configuration conf)
    {
    ...
    private static GroupType convertMapType(final String name, final MapTypeInfo typeInfo, final Configuration conf)
    {
        final Type keyType = convertType(ParquetHiveSerDe.MAP_KEY.toString(), typeInfo.getMapKeyTypeInfo(), conf, Repetition.REQUIRED);
        final Type valueType = convertType(ParquetHiveSerDe.MAP_VALUE.toString(), typeInfo.getMapValueTypeInfo(), conf);
