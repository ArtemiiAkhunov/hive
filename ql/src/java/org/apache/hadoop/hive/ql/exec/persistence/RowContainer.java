...
public class RowContainer<ROW extends List<Object>> implements AbstractRowContainer<ROW>, AbstractRowContainer.RowIterator<ROW>
{
    ...
    private JobConf getLocalFSJobConfClone(Configuration jc)
    {
        if (this.jobCloneUsingLocalFs == null)
        {
            ...
            jobCloneUsingLocalFs.set(CommonConfigurationKeysPublic.FS_DEFAULT_NAME_KEY, Utilities.HADOOP_LOCAL_FS);
        }
        ...
        try
        {
            ...
            } else
            {
                ...
                if (inputSplits == null)
                {
                    ...
                    localJc.set(FileInputFormat.INPUT_DIR, org.apache.hadoop.util.StringUtils.escapeString(parentDir.getAbsolutePath()));
    ...
    protected void setupWriter() throws HiveException
    {
        try
        {
            ...
            JobConf localJc = getLocalFSJobConfClone(jc);
            rw = hiveOutputFormat.getHiveRecordWriter(localJc, tempOutPath, serde.getSerializedClass(), false, tblDesc.getProperties(), reporter);
        } catch (Exception e)
        {
            clearRows();
            LOG.error(e.toString(), e);
