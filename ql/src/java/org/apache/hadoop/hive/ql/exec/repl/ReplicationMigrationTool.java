...
public class ReplicationMigrationTool implements Tool
{
    ...
    public int run(String[] args) throws Exception
    {
        ...
        if (isValidateChecksum)
        {
            ...
            LOG.info("Using a ThreadPoolExecutor with {} threads and {} as Queue size for checksum computation.", checksumNumThreads, checksumQueueSize);
        }
    ...
    private boolean validateOpenFilesAtSource(Path srcPath, FileSystem srcFileSystem) throws IOException
    {
        ...
        if (srcFileSystem instanceof DistributedFileSystem)
        {
            ...
            if (srcDFS.listOpenFiles(EnumSet.of(ALL_OPEN_FILES), Path.getPathWithoutSchemeAndAuthority(srcPath).toString()).hasNext())
            {
                System.out.println("There are open files in " + srcPath);
                return false;
            } else
            {
                LOG.error("Open file check is ignored since the source filesystem is not of type of " + "DistributedFileSystem. The source file system is of " + srcFileSystem.getClass() + " type.");
            }
        }
        return true;
    }
    ...
    private boolean validateAtDirectoryLevel(Path srcPath, Path trgPath, FileSystem srcFileSystem, FileSystem tgtFileSystem) throws IOException
    {
        ...
        {
            System.err.println("Directory Count mismatch in source directory " + srcPath + " and target directory " + trgPath);
            return false;
        }
    ...
    public boolean validateAtFileLevel(Path srcDir, Path tgtDir, FileSystem srcFs, FileSystem tgtFs, List<Pattern> filtersPattern, boolean verifyChecksum, ThreadPoolExecutor threadPoolChecksum) throws Exception
    {
        ...
        while (srcListing.hasNext() && tgtListing.hasNext())
        {
            ...
            String sourcePath = Path.getPathWithoutSchemeAndAuthority(sourceFile.getPath()).toString().replaceFirst(Path.getPathWithoutSchemeAndAuthority(srcDir).toString(), "/");
            String targetPath = Path.getPathWithoutSchemeAndAuthority(tgtFile.getPath()).toString().replaceFirst(Path.getPathWithoutSchemeAndAuthority(tgtDir).toString(), "/");
            if (!sourcePath.equals(targetPath))
            {
                System.err.println("Entries mismatch between source: " + srcDir + " and target: " + tgtDir + " for " + "sourceFile: " + sourceFile.getPath() + " at target with " + tgtFile.getPath() + " Mismatched subPaths: " + "source: " + sourcePath + " and target: " + targetPath + "Either Source Or Target has an extra/less files.");
            ...
            {
                System.err.println("File Size mismatch in source directory " + srcDir + " and target directory " + tgtDir + " " + "for source: " + sourceFile.getPath() + " and target " + tgtFile.getPath());
            ...
            if (verifyChecksum)
            {
                ChecksumVerifier checksumVerifier = new ChecksumVerifier(sourceFile.getPath(), tgtFile.getPath(), srcFs, tgtFs, srcDir, tgtDir);
    ...
    private boolean validateChecksumStatus(Path srcDir, Path tgtDir, ArrayList<Future<Boolean>> futures) throws InterruptedException, java.util.concurrent.ExecutionException
    {
        int numFailed = 0;
        int numSuccess = 0;
        for (Future<Boolean> f : futures)
        ...
        if (numFailed > 0)
        {
            LOG.warn("{} files failed checksum validation for source: {} and target: {}, numSuccess: {} numFailed: {}", srcDir, tgtDir, numSuccess, numFailed);
            return true;
        }
        return false;
    }
        ...
        public Boolean call() throws Exception
        {
            ...
            try
            {
                ...
                if (checkAtFileLevel)
                {
                    ...
                    if (!validateAtFileLevel(srcPath, trgPath, srcFileSystem, tgtFileSystem, filtersPattern, isValidateChecksum, threadPoolChecksum))
                    {
                        return false;
                    }
                }
            ...
            } catch (Exception e)
            {
                ...
                System.err.println("Failed to verify source: " + srcPath + " with target: " + trgPath + " error:" + e.getMessage());
                return false;
            }
            return true;
        }
    ...
    private class ChecksumVerifier implements Callable<Boolean>
    {
        ...
        ChecksumVerifier(Path sourcePath, Path targetPath, FileSystem sourceFs, FileSystem targetFs, Path srcDir, Path tgtDir)
        {
        ...
        public Boolean call() throws Exception
        {
            ...
            try
            {
                ...
                if (!response)
                {
                    System.err.println("File Checksum mismatch in source directory " + srcDir + " and target directory " + tgtDir + " " + "for source: " + sourcePath + " and target " + targetPath);
                }

                return response;
            } catch (Exception e)
            {
                System.out.println("Failed Checksum for: " + sourcePath + " and target: " + targetPath + " reason:" + e.getMessage());
