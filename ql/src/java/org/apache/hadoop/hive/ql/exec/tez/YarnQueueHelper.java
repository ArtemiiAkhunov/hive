...
public class YarnQueueHelper
{
    ...
    public YarnQueueHelper(HiveConf conf)
    {
        ...
        if (isHA)
        {
            ...
            if (sslForYarn == true)
            {
                ...
                {
                    nodeList.addAll(Arrays.asList(conf.getTrimmedStrings(YarnConfiguration.RM_WEBAPP_HTTPS_ADDRESS + "." + rmid)));
                }
                Preconditions.checkArgument(nodeList.size() > 0, "yarn.resourcemanager.ha.rm-ids must be set to enable queue access checks in Yarn HA mode");
            } else
            {
                for (String rmid : rmids)
                {
        ...
        } else
        {
            if (sslForYarn == true)
            {
                ...
                Preconditions.checkArgument((rmNodes != null && rmNodes.length > 0), "yarn.resourcemanager.webapp.https.address must be set to enable queue access checks using TLS");
            } else
            {
                rmNodes = conf.getTrimmedStrings(YarnConfiguration.RM_WEBAPP_ADDRESS);
                Preconditions.checkArgument((rmNodes != null && rmNodes.length > 0), "yarn.resourcemanager.webapp.address must be set to enable queue access checks");
            }
        }
        lastKnownGoodUrl = 0;
    }
    ...
    private void checkQueueAccessInternal(String queueName, String userName) throws IOException, HiveException
    {
        ...
        while (true)
        {
            ...
            if (isCallOk)
            {
                ...
                if (error == null)
                {
                    return; // null error message here means the user has access.
                }
    ...
    private String checkQueueAccessFromSingleRm(String urlString) throws IOException
    {
        ...
        HttpURLConnection connection = UserGroupInformation.isSecurityEnabled() ? getSecureConnection(url) : (HttpURLConnection) url.openConnection();
        ...
        switch (statusCode)
        {
            ...
            case HttpStatus.SC_FORBIDDEN:
            {
                ...
                throw new IOException(handleUnexpectedStatusCode(connection, statusCode, "check that the " + "HiveServer2 principal is in the administrator list of the root YARN queue"));
            }
            default:
                throw new IOException(handleUnexpectedStatusCode(connection, statusCode, null));
        }
    }
    ...
    private String processResponse(HttpURLConnection connection) throws IOException
    {
        ...
        if (stream == null)
        {
            throw new IOException(handleUnexpectedStatusCode(connection, HttpStatus.SC_OK, "No input on successful API call"));
        }
