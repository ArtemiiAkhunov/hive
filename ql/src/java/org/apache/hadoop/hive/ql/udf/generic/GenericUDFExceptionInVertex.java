 ...
 */
@Description(name = "exception_in_vertex_udf", value = "_FUNC_(hintColumn, vertexName, taskNumberExpression, taskAttemptNumberExpression" + "Throws exception in Map tasks, where UDF running is possible", extended = "hintColumn is needed for easy-locating mapper stage" + " please refer to full examples and explanation in exception_in_vertex_udf.q: " + "exception_in_vertex_udf (src1.value, 'Map 1', 0, 0)          -> Map1, first task, first attempt" + "exception_in_vertex_udf (src1.value, 'Map 1', '0,1,2', '*')  -> Map1, tasks: 0,1,2, all attempts" + "exception_in_vertex_udf (src1.value, 'Map 1', '*', 0)        -> Map1, all tasks, first attempt" + "exception_in_vertex_udf (src1.value, 'Map 1', '0-2', '*')    -> Map1, tasks: 0,1,2, all attempts" + "exception_in_vertex_udf (src1.value, 'Map 1', '*', '*')      -> Map1, all tasks, all attempts" + "exception_in_vertex_udf (src1.value, 'Map 1', '*')           -> Map1, all tasks, all attempts" + "exception_in_vertex_udf (src1.value, 'Map 1')                -> Map1, all tasks, all attempts")
public class GenericUDFExceptionInVertex extends GenericUDF
{
    ...
    public static String getVertexName(ObjectInspector[] parameters, int index) throws UDFArgumentTypeException
    {
        ...
        {
            throw new UDFArgumentTypeException(index, String.format("This argument takes only constant STRING, got %s", parameters[index].getTypeName()));
        }
    }
    ...
    private static String getExpressionAtIndex(ObjectInspector[] parameters, int index) throws UDFArgumentTypeException
    {
        if (parameters.length > index)
        {
            ...
            {
                throw new UDFArgumentTypeException(index, String.format("This argument takes only constant STRING or INT, got %s", parameters[index].getTypeName()));
            }
        } else
        {
            return "*";
    ...
    public static boolean numberFitsExpression(int number, String expression)
    {
        ...
        { // 1,2,3,4
            List<Integer> numbers = Arrays.asList(expression.split(",")).stream().map(x -> Integer.valueOf(x)).collect(Collectors.toList());
    ...
    public void configure(MapredContext mapredContext)
    {
        ...
        this.currentTaskNumber = mapredContext.getJobConf().getInt(TezProcessor.HIVE_TEZ_TASK_INDEX, -1);
        this.currentTaskAttemptNumber = mapredContext.getJobConf().getInt(TezProcessor.HIVE_TEZ_TASK_ATTEMPT_NUMBER, -1);

        LOG.debug("configure vertex: {}, task: {}, attempt: {} <-> current vertex {}, task: {}, attempt: {}", vertexName, taskNumberExpr, taskAttemptNumberExpr, currentVertexName, currentTaskNumber, currentTaskAttemptNumber);
    }
    ...
    public Object evaluate(DeferredObject[] arguments) throws HiveException
    {
        ...
        LOG.debug("evaluate: vertex {}, task: {}, attempt: {} <-> vertex {}, task: {}, attempt: {}", currentVertexName, currentTaskNumber, currentTaskAttemptNumber, vertexName, taskNumberExpr, taskAttemptNumberExpr);

        if (vertexName.equals(currentVertexName) && GenericUDFExceptionInVertex.numberFitsExpression(currentTaskNumber, taskNumberExpr) && GenericUDFExceptionInVertex.numberFitsExpression(currentTaskAttemptNumber, taskAttemptNumberExpr))
        {
            String message = String.format("GenericUDFExceptionInVertex: found condition for throwing exception (vertex/task/attempt):" + "current %s / %d / %d matches criteria %s / %s / %s", currentVertexName, currentTaskNumber, currentTaskAttemptNumber, vertexName, taskNumberExpr, taskAttemptNumberExpr);
            LOG.info(message);
