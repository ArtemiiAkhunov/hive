...
public class HiveTableScan extends TableScan implements HiveRelNode
{
    ...
    private HiveTableScan(RelOptCluster cluster, RelTraitSet traitSet, RelOptHiveTable table, String alias, String concatQbIDAlias, RelDataType newRowtype, boolean useQBIdInDigest, boolean insideView, HiveTableScanTrait tableScanTrait)
    {
        ...
        Triple<ImmutableList<Integer>, ImmutableSet<Integer>, ImmutableSet<Integer>> colIndxPair = buildColIndxsFrmReloptHT(table, newRowtype);
    ...
    {
        return new HiveTableScan(getCluster(), getTraitSet(), ((RelOptHiveTable) table), this.tblAlias, this.concatQbIDAlias, newRowtype, this.useQBIdInDigest, this.insideView, this.tableScanTrait);
    }

    public HiveTableScan setTableScanTrait(HiveTableScanTrait tableScanTrait)
    {
        return new HiveTableScan(getCluster(), getTraitSet(), ((RelOptHiveTable) table), this.tblAlias, this.concatQbIDAlias, this.rowType, this.useQBIdInDigest, this.insideView, tableScanTrait);
    }
    ...
    {
        return new HiveTableScan(getCluster(), getTraitSet(), ((RelOptHiveTable) table).copy(newRowtype), this.tblAlias, this.concatQbIDAlias, newRowtype, this.useQBIdInDigest, this.insideView, this.tableScanTrait);
    }
    ...
    {
        return super.explainTerms(pw).itemIf("qbid:alias", concatQbIDAlias, this.useQBIdInDigest).itemIf("htColumns", this.neededColIndxsFrmReloptHT, pw.getDetailLevel() == SqlExplainLevel.DIGEST_ATTRIBUTES).itemIf("insideView", this.isInsideView(), pw.getDetailLevel() == SqlExplainLevel.DIGEST_ATTRIBUTES).itemIf("plKey", ((RelOptHiveTable) table).getPartitionListKey(), pw.getDetailLevel() == SqlExplainLevel.DIGEST_ATTRIBUTES).itemIf("table:alias", tblAlias, !this.useQBIdInDigest).itemIf("tableScanTrait", this.tableScanTrait, pw.getDetailLevel() == SqlExplainLevel.DIGEST_ATTRIBUTES).itemIf("fromVersion", ((RelOptHiveTable) table).getHiveTableMD().getVersionIntervalFrom(), isNotBlank(((RelOptHiveTable) table).getHiveTableMD().getVersionIntervalFrom()));
    }
