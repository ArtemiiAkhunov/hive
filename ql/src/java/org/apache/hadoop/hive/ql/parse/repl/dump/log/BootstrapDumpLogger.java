...
public class BootstrapDumpLogger extends ReplLogger<String>
{
    ...
    {
        (new BootstrapDumpBegin(dbName, estimatedNumTables, estimatedNumFunctions)).log(LogTag.START);
    }
    ...
    {
        tableSeqNo++;
        (new BootstrapDumpTable(dbName, tableName, tableType, tableSeqNo, estimatedNumTables)).log(LogTag.TABLE_DUMP);
    }
    ...
    {
        functionSeqNo++;
        (new BootstrapDumpFunction(dbName, funcName, functionSeqNo, estimatedNumFunctions)).log(LogTag.FUNCTION_DUMP);
    }

    @Override
    public void endLog(String lastReplId)
    {
        (new BootstrapDumpEnd(dbName, tableSeqNo, functionSeqNo, dumpDir, lastReplId)).log(LogTag.END);
    }
}
