...
public class SyslogInputFormat extends TextInputFormat
{
    ...
    protected FileStatus[] listStatus(final JobConf job) throws IOException
    {
        ...
        final long timeSliceSeconds = HiveConf.getTimeVar(job, HiveConf.ConfVars.SYSLOG_INPUT_FORMAT_FILE_TIME_SLICE, TimeUnit.SECONDS);
    ...
    @VisibleForTesting
    public List<FileStatus> pruneFiles(final SearchArgument sarg, final long timeSliceSeconds, final List<FileStatus> inputFiles)
    {
        int tsExpr = 0;
        ...
        if (tsPred.getOperator() == PredicateLeaf.Operator.BETWEEN)
        {
            ...
            for (FileStatus fileStatus : inputFiles)
            {
                ...
                if (fileLowerBound != null)
                {
                    ...
                    LOG.info("file: {} -> [{}, {}] against predicate [{}({}), {}({})]. selected? {}", fileStatus.getPath(), fileLowerBound.toInstant(), fileUpperBound.toInstant(), predLowerBound.toInstant(), predLowerLocation, predUpperBound.toInstant(), predUpperLocation, selected);
                } else
                {
    ...
    @Override
    public RecordReader<LongWritable, Text> getRecordReader(final InputSplit genericSplit, final JobConf job, final Reporter reporter) throws IOException
    {
