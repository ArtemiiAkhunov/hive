...
public class TruncateTableAnalyzer extends AbstractBaseAlterTableAnalyzer
{
    ...
    public void analyzeInternal(ASTNode root) throws SemanticException
    {
        ...
        if (table.getStorageHandler() != null && !table.getStorageHandler().canUseTruncate(table, partitionSpec))
        {
            ...
            StringBuilder rewrittenQuery = constructDeleteQuery(getFullTableNameForSQL((ASTNode) tableNode.getChild(0)), table, partitionSpec);
    ...
    private void checkTruncateEligibility(ASTNode ast, ASTNode root, String tableName, Table table) throws SemanticException
    {
        ...
        if (!isForce && table.getTableType() != TableType.MANAGED_TABLE && table.getParameters().getOrDefault(MetaStoreUtils.EXTERNAL_TABLE_PURGE, "FALSE").equalsIgnoreCase("FALSE"))
        {
    ...
    {
        boolean truncateUseBase = (HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_ACID_TRUNCATE_USE_BASE) || HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_ACID_LOCKLESS_READS_ENABLED)) && AcidUtils.isTransactionalTable(table);

        WriteEntity.WriteType writeType = truncateUseBase ? WriteEntity.WriteType.DDL_EXCL_WRITE : WriteEntity.WriteType.DDL_EXCLUSIVE;
    ...
    private Task<?> truncatePartitionedTableWithColumnNames(ASTNode root, TableName tableName, Table table, Map<String, String> partitionSpec, List<String> columnNames) throws HiveException
    {
        ...
        ListBucketingCtx lbCtx = constructListBucketingCtx(partition.getSkewedColNames(), partition.getSkewedColValues(), partition.getSkewedColValueLocationMaps(), partition.isStoredAsSubDirectories());
        boolean isListBucketed = partition.isStoredAsSubDirectories();

        return createTasks(root, tableName, table, partitionSpec, columnNames, bucketColumns, inputFormatClass, isArchived, newPartitionLocation, oldPartitionLocation, columns, lbCtx, isListBucketed, listBucketColumns);
    }
    ...
    private Task<?> truncateUnpartitionedTableWithColumnNames(ASTNode root, TableName tableName, Table table, Map<String, String> partitionSpec, List<String> columnNames) throws SemanticException
    {
        ...
        ListBucketingCtx lbCtx = constructListBucketingCtx(table.getSkewedColNames(), table.getSkewedColValues(), table.getSkewedColValueLocationMaps(), table.isStoredAsSubDirectories());
        boolean isListBucketed = table.isStoredAsSubDirectories();

        return createTasks(root, tableName, table, partitionSpec, columnNames, bucketColumns, inputFormatClass, false, newPartitionLocation, oldPartitionLocation, columns, lbCtx, isListBucketed, listBucketColumns);
    }
    ...
    private Task<?> createTasks(ASTNode root, TableName tableName, Table table, Map<String, String> partitionSpec, List<String> columnNames, List<String> bucketColumns, Class<? extends InputFormat> inputFormatClass, boolean isArchived, Path newPartitionLocation, Path oldPartitionLocation, List<FieldSchema> columns, ListBucketingCtx lbCtx, boolean isListBucketed, List<String> listBucketColumns) throws SemanticException
    {
        ...
        Set<Integer> columnIndexes = getColumnIndexes(columnNames, bucketColumns, columns, isListBucketed, listBucketColumns);
        ...
        Task<?> truncateTask = createTruncateTask(tableName, table, partitionSpec, oldPartitionLocation, lbCtx, columnIndexes, queryTmpdir);

        addMoveTask(root, table, partitionSpec, oldPartitionLocation, newPartitionLocation, lbCtx, queryTmpdir, truncateTask, tableDesc);
    ...
    public StringBuilder constructDeleteQuery(String qualifiedTableName, Table table, Map<String, String> partitionSpec) throws SemanticException
    {
        StringBuilder sb = new StringBuilder().append("delete from ").append(qualifiedTableName).append(" where ");
        boolean first = true;
        for (String key : partitionSpec.keySet())
        {
            ...
            if (columnInfo.getObjectInspector() instanceof PrimitiveObjectInspector)
            {
                ...
                if (value != null)
                {
                    sb.append(first ? "" : " and ").append(HiveUtils.unparseIdentifier(key, conf)).append(" = ").append(TypeInfoUtils.convertStringToLiteralForSQL(value, primitiveObjInspector.getPrimitiveCategory()));
                }
            } else
            {
                throw new SemanticException(String.format("Only primitive partition column type is supported via TRUNCATE operation " + " but column %s is not a primitive category.", key));
            }
            first = false;
        }
        return sb;
