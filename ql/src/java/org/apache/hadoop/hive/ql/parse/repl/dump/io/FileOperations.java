...
public class FileOperations
{
    ...
    public FileOperations(List<Path> dataPathList, Path exportRootDataDir, String distCpDoAsUser, HiveConf hiveConf, MmContext mmCtx) throws IOException
    {
    ...
    private void copyMmPath() throws LoginException, IOException
    {
        ...
        for (Path fromPath : dataPathList)
        {
            ...
            HiveInputFormat.processPathsForMmRead(dataPathList, hiveConf, ids, validPaths, dirsWithOriginals);
            ...
            for (Path dirWithOriginals : dirsWithOriginals)
            {
                ...
                Utilities.FILE_OP_LOGGER.debug("Exporting originals from {} to {}", dirWithOriginals, exportRootDataDir);
                new CopyUtils(distCpDoAsUser, hiveConf, exportRootDataDir.getFileSystem(hiveConf)).doCopy(exportRootDataDir, srcPaths);
            }
        }
    }
    ...
    private BufferedWriter writer() throws IOException
    {
        ...
        return new BufferedWriter(new OutputStreamWriter(exportFileSystem.create(exportToFile)));
    }
