 ...
 * FetchWork.
 */
@Explain(displayName = "Fetch Operator", explainLevels = {Level.USER, Level.DEFAULT, Level.EXTENDED}, vectorization = Vectorization.SUMMARY_PATH)
public class FetchWork implements Serializable
{
    ...
    {
        return isPartitioned() ? partDir == null ? null : new ArrayList<Path>(partDir) : Arrays.asList(tblDir);
    }
    ...
    public ArrayList<PartitionDesc> getPartDescOrderedByPartDir()
    {
        ...
        if (partDir != null && partDir.size() > 1)
        {
            ...
            {
                throw new RuntimeException("Partition Directory list size doesn't match Partition Descriptor list size");
            }
