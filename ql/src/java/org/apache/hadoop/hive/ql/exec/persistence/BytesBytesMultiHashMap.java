...
public final class BytesBytesMultiHashMap implements MemoryEstimate
{
    ...
    public BytesBytesMultiHashMap(int initialCapacity, float loadFactor, int wbSize, long maxProbeSize)
    {
        ...
        int maxCapacity = (maxProbeSize <= 0) ? DEFAULT_MAX_CAPACITY : (int) Math.min((long) DEFAULT_MAX_CAPACITY, maxProbeSize / 8);
    ...
    public static class Result
    {
         ...
         *
         * @param hashMap                     The hash map we found the key in.
         * @param firstOffset                 The absolute offset of the first record in the write buffers.
         * @param hasList                     Whether there are multiple values (true) or just a single value (false).
         * @param offsetAfterListRecordKeyLen The offset of just after the key length in the list record.  Or, 0 when single row.
         */
     ...
     */
    public byte getValueResult(byte[] key, int offset, int length, Result hashMapResult, MatchTracker matchTracker)
    {
     ...
     */
    public int findNextNonMatched(int currentSlotNum, WriteBuffers.ByteSegmentRef keyRef, Result hashMapResult, MatchTracker matchTracker)
    {
        currentSlotNum++;
    ...
    public void expandAndRehashToTarget(int estimateNewRowCount)
    {
        ...
        {
            newRefsCount = (Long.bitCount(newRefsCount) == 1) ? newRefsCount : nextHighestPowerOfTwo(newRefsCount);
    ...
    private static void validateCapacity(long capacity)
    {
        ...
        {
            throw new RuntimeException("Attempting to expand the hash table to " + capacity + " that overflows maximum array size. For this query, you may want to disable " + ConfVars.HIVE_DYNAMIC_PARTITION_HASHJOIN.varname + " or reduce " + ConfVars.HIVE_CONVERT_JOIN_NOCONDITIONAL_TASK_THRESHOLD.varname);
        }
    }
     ...
     */
    private long findKeyRefToRead(byte[] key, int offset, int length, WriteBuffers.Position readPos, MatchTracker matchTracker)
    {
        final int bucketMask = (refs.length - 1);
    ...
    {
        LOG.info("Map metrics: keys allocated " + this.refs.length + ", keys assigned " + keysAssigned + ", write conflict " + metricPutConflict + ", write max dist " + largestNumberOfSteps + ", read conflict " + metricGetConflict + ", expanded " + metricExpands + " times in " + metricExpandsMs + "ms");
    }
