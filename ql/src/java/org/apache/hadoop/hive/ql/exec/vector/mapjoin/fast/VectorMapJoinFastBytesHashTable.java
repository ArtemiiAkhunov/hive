...
public abstract class VectorMapJoinFastBytesHashTable extends VectorMapJoinFastHashTable implements VectorMapJoinBytesHashTable
{
    ...
    public abstract void add(byte[] keyBytes, int keyStart, int keyLength, BytesWritable currentValue, long hashCode);
    ...
    protected void expandAndRehash()
    {
        ...
        for (int slot = 0; slot < logicalHashBucketCount; slot++)
        {
            ...
            if (refWord != 0)
            {
                final long hashCode = VectorMapJoinFastBytesHashKeyRef.calculateHashCode(refWord, writeBuffers, unsafeReadPos);
