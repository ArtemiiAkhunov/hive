...
public class OrcRawRecordMerger implements AcidInputFormat.RawReader<OrcStruct>
{
    ...
    public final static class ReaderKey extends RecordIdentifier
    {
        ...
        {
            return super.equals(other) && currentWriteId == ((ReaderKey) other).currentWriteId;
        }
        ...
        {
            return "{originalWriteId: " + getWriteId() + ", " + bucketToString(getBucketProperty()) + ", row: " + getRowId() + ", currentWriteId " + currentWriteId + "}";
        }
    ...
    final static class ReaderPairAcid implements ReaderPair
    {
         ...
         */
        @VisibleForTesting
        ReaderPairAcid(ReaderKey key, Reader reader, RecordIdentifier minKey, RecordIdentifier maxKey, ReaderImpl.Options options, final Configuration conf) throws IOException
        {
            this.reader = reader;
            this.key = key;
            ...
            {
                next(nextRecord());
            } while (nextRecord() != null && (minKey != null && key.compareRow(getMinKey()) <= 0));
        }
        ...
        public void next(OrcStruct next) throws IOException
        {
            if (getRecordReader().hasNext())
            {
                ...
                // set the key
                getKey().setValues(OrcRecordUpdater.getOriginalTransaction(nextRecord()), OrcRecordUpdater.getBucket(nextRecord()), OrcRecordUpdater.getRowId(nextRecord()), OrcRecordUpdater.getCurrentTransaction(nextRecord()), OrcRecordUpdater.getOperation(nextRecord()) == OrcRecordUpdater.DELETE_OPERATION);
        ...
        final boolean nextFromCurrentFile(OrcStruct next) throws IOException
        {
            if (getRecordReader().hasNext())
            {
                ...
                if (next == null)
                {
                    ...
                    nextRecord().setFieldValue(OrcRecordUpdater.CURRENT_WRITEID, new LongWritable(writeId));
                    nextRecord().setFieldValue(OrcRecordUpdater.ORIGINAL_WRITEID, new LongWritable(writeId));
                    nextRecord().setFieldValue(OrcRecordUpdater.BUCKET, new IntWritable(bucketProperty));
                    nextRecord().setFieldValue(OrcRecordUpdater.ROW_ID, new LongWritable(nextRowId));
                    nextRecord().setFieldValue(OrcRecordUpdater.ROW, getRecordReader().next(null));
                } else
                {
                    nextRecord = next;
                    ((IntWritable) next.getFieldValue(OrcRecordUpdater.OPERATION)).set(OrcRecordUpdater.INSERT_OPERATION);
                    ((LongWritable) next.getFieldValue(OrcRecordUpdater.ORIGINAL_WRITEID)).set(writeId);
                    ((IntWritable) next.getFieldValue(OrcRecordUpdater.BUCKET)).set(bucketProperty);
                    ((LongWritable) next.getFieldValue(OrcRecordUpdater.CURRENT_WRITEID)).set(writeId);
                    ((LongWritable) next.getFieldValue(OrcRecordUpdater.ROW_ID)).set(nextRowId);
                    nextRecord().setFieldValue(OrcRecordUpdater.ROW, getRecordReader().next(OrcRecordUpdater.getRow(next)));
                }
    ...
    final static class OriginalReaderPairToRead extends OriginalReaderPair
    {
        ...
        OriginalReaderPairToRead(ReaderKey key, Reader reader, int bucketId, final RecordIdentifier minKey, final RecordIdentifier maxKey, Reader.Options options, Options mergerOptions, Configuration conf, ValidWriteIdList validWriteIdList, int statementId) throws IOException
        {
            ...
            long rowIdOffsetTmp = 0;
            {
                ...
                AcidDirectory directoryState = AcidUtils.getAcidState(null, mergerOptions.getRootPath(), conf, validWriteIdList, Ref.from(false), true);
            ...
            if (!isLastFileForThisBucket && maxKey == null)
            {
                ...
                newMaxKey = new RecordIdentifier(writeId, bucketProperty, rowIdOffset + reader.getNumberOfRows() - 1);
            }
            ...
            {
                next(nextRecord());
            } while (nextRecord() != null && (getMinKey() != null && this.getKey().compareRow(getMinKey()) <= 0));
        }
    ...
    static final class KeyInterval
    {
        ...
        public boolean equals(Object other)
        {
            ...
            return Objects.equals(minKey, otherInterval.getMinKey()) && Objects.equals(maxKey, otherInterval.getMaxKey());
        }
        ...
        {
            return (minKey == null || other.maxKey == null || minKey.compareTo(other.maxKey) <= 0) && (maxKey == null || other.minKey == null || maxKey.compareTo(other.minKey) >= 0);
        }
    }
    ...
    private KeyInterval discoverKeyBounds(Reader reader, Reader.Options options)
    {
        ...
        if (keyIndex == null)
        {
            LOG.warn("Missing '{}' metadata in ORC file, can't compute min/max keys", OrcRecordUpdater.ACID_KEY_INDEX_NAME);
    ...
    {
        this(conf, collapseEvents, reader, isOriginal, bucket, validWriteIdList, options, deltaDirectory, mergerOptions, null);
    }
     ...
     * @throws IOException
     */
    OrcRawRecordMerger(Configuration conf, boolean collapseEvents, Reader reader, boolean isOriginal, int bucket, ValidWriteIdList validWriteIdList, Reader.Options options, Path[] deltaDirectory, Options mergerOptions, Map<String, Integer> deltasToAttemptId) throws IOException
    {
        ...
        objectInspector = OrcRecordUpdater.createEventObjectInspector(OrcStruct.createObjectInspector(0, OrcUtils.getOrcTypes(typeDescr)));
        ...
        boolean isMajorNoBase = mergerOptions.isCompacting() && mergerOptions.isMajorCompaction() && mergerOptions.getBaseDir() == null;
        ...
        } else
        {
            ...
            if (isOriginal)
            {
                ...
                if (mergerOptions.isCompacting())
                {
                    ...
                    {
                        readerPairOptions = modifyForNonAcidSchemaRead(mergerOptions, AcidUtils.ParsedBaseLight.parseBase(mergerOptions.getBaseDir()).getWriteId(), mergerOptions.getBaseDir());
                    }
                    pair = new OriginalReaderPairToCompact(baseKey, bucket, options, readerPairOptions, conf, validWriteIdList, 0);//0 since base_x doesn't have a suffix (neither does pre acid write)
                } else
                {
                    ...
                    {
                        readerPairOptions = modifyForNonAcidSchemaRead(mergerOptions, tfp.syntheticWriteId, tfp.folder);
                    }
        ...
        if (deltaDirectory != null && deltaDirectory.length > 0)
        {
             ...
             * */
            Reader.Options deltaEventOptions = eventOptions.clone().searchArgument(null, null).range(0, Long.MAX_VALUE);
            for (Path delta : deltaDirectory)
            {
                ...
                if (deltaDir.isRawFormat())
                {
                    ...
                    ReaderPair deltaPair = new OriginalReaderPairToCompact(key, bucket, options, rawCompactOptions, conf, validWriteIdList, deltaDir.getStatementId());
    ...
    static final class TransactionMetaData
    {
        ...
        static TransactionMetaData findWriteIDForSynthetcRowIDs(Path splitPath, Path rootPath, Configuration conf) throws IOException
        {
            ...
            while (parent != null && !rootPath.equals(parent))
            {
                ...
                boolean isDelta = parent.getName().startsWith(AcidUtils.DELTA_PREFIX) || parent.getName().startsWith(AcidUtils.DELETE_DELTA_PREFIX);
                if (isBase || isDelta)
                {
                    if (isBase)
                    {
                        return new TransactionMetaData(AcidUtils.ParsedBaseLight.parseBase(parent).getWriteId(), parent);
                    } else
                    {
    ...
    {
        assert (!mergerOptions.isCompacting && deltaDirectory.getName().startsWith(AcidUtils.DELETE_DELTA_PREFIX)) || mergerOptions.isCompacting : "Unexpected delta: " + deltaDirectory + "(isCompacting=" + mergerOptions.isCompacting() + ")";
    ...
    public boolean next(RecordIdentifier recordIdentifier, OrcStruct prev) throws IOException
    {
        ...
        while (keysSame && primary != null)
        {
            ...
            if (primary.nextRecord() == null || primary.getKey().compareTo(secondaryKey) > 0)
            {
            ...
            if (!validWriteIdList.isWriteIdValid(((ReaderKey) recordIdentifier).getCurrentWriteId()))
            {
                continue;
            }
