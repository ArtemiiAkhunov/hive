...
public class HiveFilterSortPredicates extends RelHomogeneousShuttle
{
    ...
    private static class RexSortPredicatesShuttle extends RexShuttle
    {
        ...
        public RexNode visitCall(final RexCall call)
        {
            ...
            {
                case AND:
                    List<RexNode> newAndOperands = call.getOperands().stream().map(pred -> new Pair<>(pred, rankingAnd(pred))).sorted(Comparator.comparing(Pair::getValue, Comparator.nullsLast(Double::compare))).map(Pair::getKey).collect(Collectors.toList());
                    ...
                    break;
                case OR:
                    List<RexNode> newOrOperands = call.getOperands().stream().map(pred -> new Pair<>(pred, rankingOr(pred))).sorted(Comparator.comparing(Pair::getValue, Comparator.nullsLast(Double::compare))).map(Pair::getKey).collect(Collectors.toList());
    ...
    private static class RexFunctionCost extends RexVisitorImpl<Double>
    {
        ...
        public Double visitCall(RexCall call)
        {
            ...
            for (RexNode operand : call.operands)
            {
                ...
                {
                    size = HiveRelMdSize.INSTANCE.typeValueSize(operand.getType(), ((RexLiteral) operand).getValueAs(Comparable.class));
                } else
                {
