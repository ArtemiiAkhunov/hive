    ...
    public void analyzeInternal(ASTNode root) throws SemanticException
    {
        ...
        if (cascade)
        {
            try
            {
                ...
                isDbLevelLock = !isSoftDelete || tables.stream().allMatch(table -> AcidUtils.isTableSoftDeleteEnabled(table, conf));
                for (Table table : tables)
                {
                    ...
                    if (!isDbLevelLock)
                    {
                        lockType = AcidUtils.isTableSoftDeleteEnabled(table, conf) ? WriteEntity.WriteType.DDL_EXCL_WRITE : WriteEntity.WriteType.DDL_EXCLUSIVE;
                    }
