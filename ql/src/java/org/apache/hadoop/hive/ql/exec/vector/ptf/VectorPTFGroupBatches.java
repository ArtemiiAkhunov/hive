...
public class VectorPTFGroupBatches extends PTFPartition
{
    ...
    class PartitionResults
    {
        ...
        {
            return String.format("PartitionResults (evaluators: %d, rows: %d): %s", results.length, results[0].length, Arrays.deepToString(results));
        }
    ...
    class RowPositionInBatch
    {
        ...
        public String toString()
        {
            return String.format("RowPositionInBatch: batchIndex: %d, rowIndexInBatch: %d, blockIndex: %d, block: %s", batchIndex, rowIndexInBatch, block == null ? 0 : block.blockIndex, block);
        }
    }
    ...
    public void init(VectorPTFEvaluatorBase[] evaluators, int[] outputProjectionColumnMap, int[] bufferedColumnMap, TypeInfo[] bufferedTypeInfos, int[] orderColumnMap, int[] keyWithoutOrderColumnMap, VectorizedRowBatch overflowBatch)
    {
        ...
        this.blocks = new VectorSpillBlockContainer(spillLimitBufferedBatchCount, spillLocalDirs, bufferedColumnMap, bufferedTypeInfos);
    }
    ...
    public RowPositionInBatch getPosition(int i) throws HiveException
    {
        ...
        {
            return toPositionCache(i, new RowPositionInBatch(lastBlock, /* batchIndex */ lastBlock.startRowIndex.length - 1,
                    /* rowIndexInBatch */ (int) (lastBlock.startRowIndex[0] + lastBlock.spillRowCount - 1 - lastBlock.startRowIndex[lastBlock.spillBatchCount - 1])));
        }
        ...
        return toPositionCache(i, new RowPositionInBatch(block, batchIndex, i - block.startRowIndex[batchIndex]));
    }
    ...
    private RowPositionInBatch getRowPositionFromBufferedBatches(int row) throws HiveException
    {
        ...
        return new RowPositionInBatch(null, /* batchIndex */batchIndex, row - inMemoryStartRowIndex.get(batchIndex));
    }
    ...
    private Object getValueFromBatch(VectorizedRowBatch vectorizedRowBatch, int column, int rowIndexInBatch)
    {
        ...
        switch (columnVector.type)
        {
            ...
            default:
                throw new RuntimeException("Unexpected column vector type while getting value from ordering col: " + columnVector.type);
        }
    }
    ...
    private void prepareBufferedBatchForForwarding(BufferedVectorizedRowBatch bufferedBatch, Object[] partitionKey) throws HiveException
    {
        ...
        for (int i = 0; i < bufferedColumnCount; i++)
        {
            ...
            VectorizedBatchUtil.copyNonSelectedColumnVector(bufferedBatch, i, overflowBatch, bufferedColumnMap[i], size);
        }
    ...
    private void forwardSpilledBatches(VectorPTFOperator vecPTFOperator, Object[] partitionKey) throws HiveException
    {
        ...
        try
        {
            for (int b = 0; b < blocks.size(); b++)
            {
                ...
                {
                    if (batchIndex < block.startRowIndex.length - 1 && overflowBatch.size == block.startRowIndex[batchIndex + 1] - block.startRowIndex[batchIndex])
                    {
                ...
                Preconditions.checkState(spillRowsRead == block.spillRowCount, "error while checking forwardSpilledBatches state for spillRowsRead == block.spillRowCount, %s != %s (block %s/%s)", spillRowsRead, block.spillRowCount, b, blocks.size() - 1);
            }
        } catch (Exception e)
        {
    ...
    private void refreshInMemoryStartRowIndex(VectorSpillBlock block)
    {
        ...
        if (block == null || !block.isFullySpilled())
        {
            ...
            {
                inMemoryStartRowIndex.add(inMemoryStartRowIndex.get(i - 1) + bufferedBatches.get(i - 1).size);
            }
        } else
        {
            ...
            inMemoryStartRowIndex = Arrays.stream(block.startRowIndex).boxed().collect(Collectors.toList());
    ...
    private void runEvaluatorForRow(int evaluatorIndex, VectorPTFEvaluatorBase evaluator, int rowNum, Range range) throws HiveException
    {
        ...
        { // fallback to batch-by-batch manner over the whole range
    ...
    private void runEvaluator(VectorPTFEvaluatorBase evaluator, Range range) throws HiveException
    {
        ...
        for (int i = Math.max(0, batchStartIndex); i < Math.min(batchEndIndex + 1, bufferedBatches.size()); i++)
        {
            ...
            if (range.getStart() > currentBatchStartRow || range.getEnd() - 1 < currentBatchEndRow)
            {
                 ...
                 */
                evaluator.evaluateGroupBatch(createTruncatedBufferedBatch(bufferedBatch, Math.max(range.getStart(), currentBatchStartRow) - currentBatchStartRow, Math.min(range.getEnd() - 1, currentBatchEndRow) - currentBatchStartRow));
            } else
            {
    ...
    private VectorizedRowBatch createTruncatedBufferedBatch(BufferedVectorizedRowBatch bufferedBatch, int startIndexWithinBatch, int endIndexWithinBatch)
    {
        ...
        {
            VectorizedBatchUtil.copyNonSelectedColumnVector(bufferedBatch, i, partialBatch, i, size, startIndexWithinBatch);
        }
    ...
    private BufferedVectorizedRowBatch newBufferedBatch(VectorizedRowBatch batch) throws HiveException
    {
        ...
        for (int i = 0; i < bufferedColumnCount; i++)
        {
            newBatch.cols[i] = VectorizedBatchUtil.makeLikeColumnVector(batch.cols[bufferedColumnMap[i]]);
        ...
        if (partialBatch == null)
        {
            ...
            for (int i = 0; i < bufferedColumnCount; i++)
            {
                partialBatch.cols[i] = VectorizedBatchUtil.makeLikeColumnVector(batch.cols[bufferedColumnMap[i]]);
    ...
    boolean nextBlock() throws HiveException
    {
        ...
        if (!block.didSpillToDisk)
        {
            ...
            throw new HiveException("Current block (index: " + block.blockIndex + ", " + block + ") is not spilled while trying to move forward. Number of current blocks: " + blocks.size());
        }
    ...
    private void readBlockFromSpillToMemory(VectorSpillBlock block) throws HiveException
    {
        ...
        try
        {
            ...
            while (rowBytesContainer.readNext())
            {
                if (batchIndex < block.startRowIndex.length - 1 && bufferedBatches.get(batchIndex).size == block.startRowIndex[batchIndex + 1] - block.startRowIndex[batchIndex])
                {
                    batchIndex += 1;
                    bufferedBatches.get(batchIndex).reset();
                }
    ...
    private void spillAndResetCurrentBufferedBatches() throws HiveException
    {
         ...
         */
        LOG.debug("Spilling batches from {} to {} in block: {}", block.spillBatchCount, currentBufferedBatchCount, block);
        BufferedVectorizedRowBatch previousBatch = block.spillBatchCount > 0 ? bufferedBatches.get(block.spillBatchCount - 1) : null;
