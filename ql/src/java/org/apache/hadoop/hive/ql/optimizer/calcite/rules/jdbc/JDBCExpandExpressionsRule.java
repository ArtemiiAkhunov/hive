...
public abstract class JDBCExpandExpressionsRule extends RelOptRule
{
    ...
    public static final JDBCExpandExpressionsRule.FilterCondition FILTER_INSTANCE = new JDBCExpandExpressionsRule.FilterCondition();
    public static final JDBCExpandExpressionsRule.JoinCondition JOIN_INSTANCE = new JDBCExpandExpressionsRule.JoinCondition();
    public static final JDBCExpandExpressionsRule.ProjectionExpressions PROJECT_INSTANCE = new JDBCExpandExpressionsRule.ProjectionExpressions();
    ...
    private static class FilterCondition extends JDBCExpandExpressionsRule
    {
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            RexNode newCondition = analyzeRexNode(filter.getCluster().getRexBuilder(), condition);
    ...
    private static class JoinCondition extends JDBCExpandExpressionsRule
    {
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            final RexNode condition = RexUtil.pullFactors(join.getCluster().getRexBuilder(), join.getCondition());

            RexNode newCondition = analyzeRexNode(join.getCluster().getRexBuilder(), condition);
    ...
    protected static class RexTransformIntoOrAndClause extends RexShuttle
    {
        ...
        private RexNode transformIntoOrAndClause(RexBuilder rexBuilder, RexCall expression)
        {
            ...
            final List<RexNode> disjuncts = RexNodeConverter.transformInToOrOperands(expression.getOperands(), rexBuilder);
            if (disjuncts == null)
            {
