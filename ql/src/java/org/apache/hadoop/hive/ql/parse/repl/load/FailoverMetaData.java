...
public class FailoverMetaData
{
    ...
    private synchronized void loadMetadataFromFile() throws SemanticException
    {
        if (!initialized)
        {
            ...
            try
            {
                ...
                {
                    throw new IOException("Unable to read valid values from failover Metadata file:" + metadataFile.toUri().toString());
                }
    ...
    {
        initializeIfNot();
        return openTxns != null && abortedTxns != null && failoverEventId != null && cursorPoint != null && txnsWithoutLock != null;
    }
