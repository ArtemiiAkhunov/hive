...
public class ExprNodeDescUtils
{
    ...
    {
        if (origin instanceof ExprNodeConstantDesc && ((ExprNodeConstantDesc) origin).getValue() != null && ((ExprNodeConstantDesc) origin).getValue() instanceof String && ((ExprNodeConstantDesc) origin).getValue().equals(defaultPartitionName))
        {
            return true;
        } else
        {
    ...
    public static void replaceEqualDefaultPartition(ExprNodeDesc origin, String defaultPartitionName) throws SemanticException
    {
        ...
        {
            if (isDefaultPartition(origin.getChildren().get(0), defaultPartitionName))
            {
                defaultPartition = (ExprNodeConstantDesc) origin.getChildren().get(0);
                column = (ExprNodeColumnDesc) origin.getChildren().get(1);
            } else if (isDefaultPartition(origin.getChildren().get(1), defaultPartitionName))
            {
        ...
        if (column != null)
        {
            ...
            String fnName;
            if (((ExprNodeGenericFuncDesc) origin).getGenericUDF() instanceof GenericUDFOPEqual)
            {
                fnName = "isnull";
            } else
            {
    ...
    public static boolean containsPredicate(ExprNodeDesc source, ExprNodeDesc predicate)
    {
        ...
        {
            if (containsPredicate(source.getChildren().get(0), predicate) || containsPredicate(source.getChildren().get(1), predicate))
            {
                return true;
            }
        }
    ...
    {
        if ((value instanceof ExprNodeConstantDesc) && ((ExprNodeConstantDesc) value).getValue() == null)
        {
            return true;
        }
        return false;
    ...
    public static PrimitiveTypeInfo deriveMinArgumentCast(TypeInfo childTi, TypeInfo targetType)
    {
        ...
        if ((pti.getPrimitiveCategory() != PrimitiveCategory.DECIMAL) || (!(childTi instanceof PrimitiveTypeInfo)))
        {
            return pti;
        }
     ...
     * @param startPos starting position in the input operator schema; must be &gt;=0 and &lt;=
     *                 endPos
    ...
    public static boolean isIntegerType(ExprNodeDesc expr)
    {
        ...
        if (typeInfo.getCategory() == ObjectInspector.Category.PRIMITIVE)
        {
            ...
            if (primitiveCategory == PrimitiveCategory.INT || primitiveCategory == PrimitiveCategory.SHORT || primitiveCategory == PrimitiveCategory.BYTE || primitiveCategory == PrimitiveCategory.LONG)
            {
                return true;
            }
        }
