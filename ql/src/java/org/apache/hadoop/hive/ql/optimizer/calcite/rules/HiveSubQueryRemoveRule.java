...
public class HiveSubQueryRemoveRule extends RelOptRule
{
    ...
    {
        return new HiveSubQueryRemoveRule(RelOptRule.operandJ(HiveProject.class, null, RexUtil.SubQueryFinder::containsSubQuery, any()), "SubQueryRemoveRule:Project", conf);
    }

    public static RelOptRule forFilter(HiveConf conf)
    {
        return new HiveSubQueryRemoveRule(RelOptRule.operandJ(HiveFilter.class, null, RexUtil.SubQueryFinder::containsSubQuery, any()), "SubQueryRemoveRule:Filter", conf);
    }
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        if (relNode instanceof HiveFilter)
        {
            ...
            final RelOptUtil.Logic logic = LogicVisitor.find(RelOptUtil.Logic.TRUE, ImmutableList.of(filter.getCondition()), correlationInfo.rexSubQuery);
            builder.push(filter.getInput());
            final int fieldCount = builder.peek().getRowType().getFieldCount();

            boolean isCorrScalarQuery = correlationInfo.isCorrScalarQuery();

            final RexNode target = apply(call.getMetadataQuery(), correlationInfo.rexSubQuery, correlationInfo.correlationIds, logic, builder, 1, fieldCount, isCorrScalarQuery);
        ...
        } else if (relNode instanceof HiveProject)
        {
            ...
            final RelOptUtil.Logic logic = LogicVisitor.find(RelOptUtil.Logic.TRUE_FALSE_UNKNOWN, project.getProjects(), correlationInfo.rexSubQuery);
