...
public class DynamicPartitionPruner
{
    ...
    protected ExprNodeDesc prunePartitionSingleSource(JobConf jobConf, String source, SourceInfo si) throws HiveException
    {
        ...
        PrimitiveObjectInspector oi = PrimitiveObjectInspectorFactory.getPrimitiveWritableObjectInspector(TypeInfoFactory.getPrimitiveTypeInfo(si.columnType));
        ...
        if (si.predicate != null)
        {
            ...
            List<ExprNodeConstantDesc> dynArgs = values.stream().map(v -> new ExprNodeConstantDesc(oi.getPrimitiveJavaObject(v))).collect(Collectors.toList());
    ...
    private void applyFilterToPartitions(Converter converter, ExprNodeEvaluator eval, String columnName, Set<Object> values, boolean mustKeepOnePartition) throws HiveException
    {
    ...
    private void processEvents() throws SerDeException, IOException, InterruptedException
    {
        ...
        while (true)
        {
            ...
            } else
            {
                ...
                numEventsSeenPerSource.computeIfAbsent(event.getSourceVertexName(), vn -> new MutableInt(0)).increment();

                totalEventCount++;
                LOG.info("Input event ({} -> {} {}), event payload size: {}", event.getSourceVertexName(), event.getTargetVertexName(), event.getTargetInputName(), (payload.limit() - payload.position()));
    ...
    private void calculateFinishCondition(String sourceName)
    {
        ...
        Preconditions.checkState(prevValInt < 0, "Invalid value for numExpectedEvents for source: " + sourceName + ", oldVal=" + prevValInt);
