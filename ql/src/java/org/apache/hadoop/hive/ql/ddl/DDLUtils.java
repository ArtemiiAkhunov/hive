...
public final class DDLUtils
{
    ...
    public static boolean addIfAbsentByName(WriteEntity newWriteEntity, Set<WriteEntity> outputs)
    {
        for (WriteEntity writeEntity : outputs)
        {
            ...
            {
                LOG.debug("Ignoring request to add {} because {} is present", newWriteEntity.toStringDetail(), writeEntity.toStringDetail());
                return false;
            }
        }
    ...
    public static String getFQName(ASTNode node)
    {
        ...
        {
            return getFQName((ASTNode) node.getChild(0)) + "." + getFQName((ASTNode) node.getChild(1)) + "." + getFQName((ASTNode) node.getChild(2));
        } else
        {
            return null;
        }
    ...
    public static void validateTableIsIceberg(org.apache.hadoop.hive.ql.metadata.Table table) throws SemanticException
    {
    ...
    {
        return table.isNonNative() && table.getStorageHandler().getType() == StorageHandlerTypes.ICEBERG;
    }

    public static boolean isIcebergStatsSource(HiveConf conf)
    {
        return conf.get(HIVE_ICEBERG_STATS_SOURCE.varname, HiveMetaHook.ICEBERG).equalsIgnoreCase(HiveMetaHook.ICEBERG);
    }
}
