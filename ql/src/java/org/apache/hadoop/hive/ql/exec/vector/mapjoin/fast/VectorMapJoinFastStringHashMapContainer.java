...
public class VectorMapJoinFastStringHashMapContainer extends VectorMapJoinFastHashTableContainerBase implements VectorMapJoinBytesHashMap
{
    ...
    public VectorMapJoinFastStringHashMapContainer(boolean isFullOuter, int initialCapacity, float loadFactor, int writeBuffersSize, long estimatedKeyCount, TableDesc tableDesc, int numHTs)
    {
        ...
        {
            vectorMapJoinFastStringHashMaps[i] = new VectorMapJoinFastStringHashMap(isFullOuter, initialCapacity, loadFactor, writeBuffersSize, estimatedKeyCount, tableDesc);
        }
        PrimitiveTypeInfo[] primitiveTypeInfos = {TypeInfoFactory.stringTypeInfo};
        this.keyBinarySortableDeserializeRead = BinarySortableDeserializeRead.with(primitiveTypeInfos, false, tableDesc.getProperties());
    ...
    private static class NonMatchedBytesHashMapIterator extends VectorMapJoinFastNonMatchedIterator
    {
        ...
        NonMatchedBytesHashMapIterator(MatchTracker matchTracker, VectorMapJoinFastStringHashMap[] hashMaps, int numThreads)
        {
            super(matchTracker);
            ...
            {
                hashMapIterators[i] = new VectorMapJoinFastBytesHashMap.NonMatchedBytesHashMapIterator(matchTracker.getPartition(i), hashMaps[i]);
            }
            index = 0;
    ...
    public long getHashCode(BytesWritable currentKey) throws HiveException, IOException
    {
        ...
        {
            throw new HiveException("DeserializeRead details: " + keyBinarySortableDeserializeRead.getDetailedReadPositionString(), e);
        }
        return HashCodeUtil.murmurHash(keyBinarySortableDeserializeRead.currentBytes, keyBinarySortableDeserializeRead.currentBytesStart, keyBinarySortableDeserializeRead.currentBytesLength);
    }
    ...
    public JoinUtil.JoinResult lookup(byte[] keyBytes, int keyStart, int keyLength, VectorMapJoinHashMapResult hashMapResult, MatchTracker matchTracker) throws IOException
    {
        ...
        return vectorMapJoinFastStringHashMaps[partition].lookup(keyBytes, keyStart, keyLength, hashMapResult, childMatchTracker);
    }
