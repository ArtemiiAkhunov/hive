...
public class ReplLoadWork implements Serializable, ReplLoadWorkMBean
{
    ...
    */
    ...
    public ReplLoadWork(HiveConf hiveConf, String dumpDirectory, String sourceDbName, String dbNameToLoadIn, ReplScope currentReplScope, LineageState lineageState, boolean isIncrementalDump, Long eventTo, Long dumpExecutionId, ReplicationMetricCollector metricCollector, boolean replScopeModified) throws IOException, SemanticException
    {
        ...
        if (isIncrementalDump)
        {
            ...
            if (replStatsTracker == null)
            {
                ...
                if (numEvents < 0)
                {
                    LOG.warn("Invalid value configured for {}, Using default of {}", REPL_STATS_TOP_EVENTS_COUNTS, REPL_STATS_TOP_EVENTS_COUNTS.defaultIntVal);
            ...
            shouldFailover = hiveConf.getBoolVar(HiveConf.ConfVars.HIVE_REPL_FAILOVER_START) && fs.exists(failoverReadyMarker);
            Path dumpDirParent = new Path(dumpDirectory).getParent();
            isFirstFailover = checkFileExists(dumpDirParent, hiveConf, EVENT_ACK_FILE);
            isSecondFailover = !isFirstFailover && checkFileExists(dumpDirParent, hiveConf, BOOTSTRAP_TABLES_LIST);
            ...
            if (isSecondFailover)
            {
                ...
                LOG.info("Optimised bootstrap load for database {} with initial bootstrapped table list as {}", dbNameToLoadIn, tablesToBootstrap);
                ...
                if (fs.exists(incBootstrapDir))
                {
                    ...
                    tablesToBootstrap = Stream.of(fs.listStatus(tableMetaPath)).map(st -> st.getPath().getName()).collect(Collectors.toList());
                } else
                {
                ...
                LOG.info("Optimised bootstrap for database {} with drop table list as {} and bootstrap table list as {}", dbNameToLoadIn, tablesToDrop, tablesToBootstrap);
            }
            if (fs.exists(incBootstrapDir))
            {
                this.bootstrapIterator = new BootstrapEventsIterator(new Path(incBootstrapDir, EximUtil.METADATA_PATH_NAME).toString(), dbNameToLoadIn, true, hiveConf, metricCollector);
            ...
            try
            {
                incrementalLoadTasksBuilder = new IncrementalLoadTasksBuilder(dbNameToLoadIn, dumpDirectory, new IncrementalLoadEventsIterator(dumpDirectory, hiveConf), hiveConf, eventTo, metricCollector, replStatsTracker, shouldFailover, tablesToBootstrap.size());
        ...
        {
            this.bootstrapIterator = new BootstrapEventsIterator(new Path(dumpDirectory, EximUtil.METADATA_PATH_NAME).toString(), dbNameToLoadIn, true, hiveConf, metricCollector);
            this.constraintsIterator = new ConstraintEventsIterator(new Path(dumpDirectory, EximUtil.METADATA_PATH_NAME).toString(), hiveConf);
    ...
    private void unRegisterMBeanIfRegistered(String serviceName, String nameName, Map<String, String> additionalParameters)
    {
        ...
        String additionalKeys = additionalParameters.entrySet().stream().map(entry -> entry.getKey() + "=" + entry.getValue()).collect(Collectors.joining(","));

        String nameStr = "Hadoop:" + "service=" + serviceName + "," + "name=" + nameName + (additionalKeys.isEmpty() ? "" : "," + additionalKeys);
        try
        {
    ...
    {
        return (((bootstrapIterator != null) && bootstrapIterator.hasNext()) || ((constraintsIterator != null) && constraintsIterator.hasNext()));
    }
