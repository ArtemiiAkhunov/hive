...
public class LogDivertAppender
{
    ...
    private static class NameFilter extends AbstractFilter
    {
         ...
         */
        private static final Pattern verboseExcludeNamePattern = Pattern.compile(Joiner.on("|").join(new String[]{LOG.getName(), OperationLog.class.getName()}));

        /* Patterns that are included in execution logging level.
         * In execution mode, show only select logger messages.
         */
        private static final Pattern executionIncludeNamePattern = Pattern.compile(Joiner.on("|").join(new String[]{"org.apache.hadoop.mapreduce.JobSubmitter", "org.apache.hadoop.mapreduce.Job", "SessionState", "ReplState", Task.class.getName(), TezTask.class.getName(), Driver.class.getName(), BasicStatsTask.class.getName()}));
        ...
        @PluginFactory
        public static NameFilter createFilter(@PluginAttribute("loggingLevel") final String loggingLevel)
        {
            // Name required for routing. Error out if it is not set.
            Preconditions.checkNotNull(loggingLevel, "loggingLevel must be specified for " + NameFilter.class.getName());
