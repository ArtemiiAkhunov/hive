...
public class SortedDynPartitionTimeGranularityOptimizer extends Transform
{
    ...
    class SortedDynamicPartitionProc implements SemanticNodeProcessor
    {
        ...
        public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx, Object... nodeOutputs) throws SemanticException
        {
            ...
            if (parseCtx.getQueryProperties().isQuery() || sh == null || !sh.equals(Constants.DRUID_HIVE_OUTPUT_FORMAT))
            {
            ...
            if (table != null)
            {
                ...
                targetShardsProperty = table.getParameters().getOrDefault(Constants.DRUID_TARGET_SHARDS_PER_GRANULARITY, "0");

            } else if (parseCtx.getCreateViewDesc() != null)
            {
                // case the statement is a CREATE MATERIALIZED VIEW AS
                segmentGranularity = parseCtx.getCreateViewDesc().getTblProps().get(Constants.DRUID_SEGMENT_GRANULARITY);
                targetShardsProperty = parseCtx.getCreateViewDesc().getTblProps().getOrDefault(Constants.DRUID_TARGET_SHARDS_PER_GRANULARITY, "0");
            } else if (parseCtx.getCreateTable() != null)
            {
                // case the statement is a CREATE TABLE AS
                segmentGranularity = parseCtx.getCreateTable().getTblProps().get(Constants.DRUID_SEGMENT_GRANULARITY);
                targetShardsProperty = parseCtx.getCreateTable().getTblProps().getOrDefault(Constants.DRUID_TARGET_SHARDS_PER_GRANULARITY, "0");
            } else
            {
                throw new SemanticException("Druid storage handler used but not an INSERT, " + "CMVAS or CTAS statement");
            }
            segmentGranularity = Strings.isNullOrEmpty(segmentGranularity) ? HiveConf.getVar(parseCtx.getConf(), HiveConf.ConfVars.HIVE_DRUID_INDEXING_GRANULARITY) : segmentGranularity;
            ...
            final Operator<? extends OperatorDesc> granularitySelOp = getGranularitySelOp(fsParent, segmentGranularity);

            // Create ReduceSinkOp operator
            final ArrayList<ColumnInfo> parentCols = Lists.newArrayList(granularitySelOp.getSchema().getSignature());
            ...
            ReduceSinkOperator rsOp = getReduceSinkOp(keyPositions, sortOrder, sortNullOrder, allRSCols, granularitySelOp, fsOp.getConf().getWriteType());
            ...
            final ColumnInfo granularityColumnInfo = new ColumnInfo(granularitySelOp.getSchema().getSignature().get(granularityKeyPos));
            fsOp.getSchema().getSignature().add(granularityColumnInfo);
            if (targetShardsPerGranularity > 0)
            {
                final ColumnInfo partitionKeyColumnInfo = new ColumnInfo(granularitySelOp.getSchema().getSignature().get(partitionKeyPos));
                fsOp.getSchema().getSignature().add(partitionKeyColumnInfo);
            }

            LOG.info("Inserted " + granularitySelOp.getOperatorId() + ", " + rsOp.getOperatorId() + " and " + backtrackSelOp.getOperatorId() + " as parent of " + fsOp.getOperatorId() + " and child of " + fsParent.getOperatorId());
        ...
        {
            final ArrayList<ColumnInfo> parentCols = Lists.newArrayList(fsParent.getSchema().getSignature());
            ...
            for (int i = 0; i < parentCols.size(); i++)
            {
                ...
                if (columnDesc.getTypeInfo().getCategory() == ObjectInspector.Category.PRIMITIVE && (((PrimitiveTypeInfo) columnDesc.getTypeInfo()).getPrimitiveCategory() == PrimitiveCategory.TIMESTAMP || ((PrimitiveTypeInfo) columnDesc.getTypeInfo()).getPrimitiveCategory() == PrimitiveCategory.TIMESTAMPLOCALTZ))
                {
                    if (timestampPos != -1)
                    {
            ...
            if (timestampType == PrimitiveCategory.TIMESTAMPLOCALTZ)
            {
                ...
                expr = new ExprNodeGenericFuncDesc(TypeInfoFactory.longTypeInfo, new GenericUDFEpochMilli(), Lists.newArrayList(expr));
                // #3 - Cast to timestamp
                expr = new ExprNodeGenericFuncDesc(TypeInfoFactory.timestampTypeInfo, new GenericUDFTimestamp(), Lists.newArrayList(expr));
            }
            // #4 - We apply the granularity function
            expr = new ExprNodeGenericFuncDesc(TypeInfoFactory.timestampTypeInfo, new GenericUDFBridge(udfName, false, udfClass.getName()), Lists.newArrayList(expr));
            descs.add(expr);
            colNames.add(Constants.DRUID_TIMESTAMP_GRANULARITY_COL_NAME);
            // Add granularity to the row schema
            final ColumnInfo ci = new ColumnInfo(Constants.DRUID_TIMESTAMP_GRANULARITY_COL_NAME, TypeInfoFactory.timestampTypeInfo, selRS.getSignature().get(0).getTabAlias(), false, false);
            selRS.getSignature().add(ci);
            if (targetShardsPerGranularity > 0)
            {
                // add another partitioning key based on floor(1/rand) % targetShardsPerGranularity
                final ColumnInfo partitionKeyCi = new ColumnInfo(Constants.DRUID_SHARD_KEY_COL_NAME, TypeInfoFactory.longTypeInfo, selRS.getSignature().get(0).getTabAlias(), false, false);
                final ExprNodeDesc targetNumShardDescNode = new ExprNodeConstantDesc(TypeInfoFactory.intTypeInfo, targetShardsPerGranularity);
                final ExprNodeGenericFuncDesc randomFn = ExprNodeGenericFuncDesc.newInstance(new GenericUDFBridge("rand", false, UDFRand.class.getName()), Lists.newArrayList());

                final ExprNodeGenericFuncDesc random = ExprNodeGenericFuncDesc.newInstance(new GenericUDFFloor(), Lists.newArrayList(ExprNodeGenericFuncDesc.newInstance(new GenericUDFOPDivide(), Lists.newArrayList(new ExprNodeConstantDesc(TypeInfoFactory.doubleTypeInfo, 1.0), randomFn))));
                final ExprNodeGenericFuncDesc randModMax = ExprNodeGenericFuncDesc.newInstance(new GenericUDFOPMod(), Lists.newArrayList(random, targetNumShardDescNode));
            ...
            final SelectOperator selOp = (SelectOperator) OperatorFactory.getAndMakeChild(selConf, selRS, fsParent);
        ...
        private ReduceSinkOperator getReduceSinkOp(List<Integer> keyPositions, List<Integer> sortOrder, List<Integer> sortNullOrder, ArrayList<ExprNodeDesc> allCols, Operator<? extends OperatorDesc> parent, AcidUtils.Operation writeType)
        {
            ...
            final ArrayList<ExprNodeDesc> keyCols = keyPositions.stream().map(id -> allCols.get(id).clone()).collect(Collectors.toCollection(ArrayList::new));
            ...
            final ArrayList<ExprNodeDesc> partCols = keyPositions.stream().map(id -> allCols.get(id).clone()).collect(Collectors.toCollection(ArrayList::new));
            ...
            final List<FieldSchema> fields = PlanUtils.getFieldSchemasFromColumnList(keyCols, keyColNames, 0, "");
            ...
            final ReduceSinkDesc rsConf = new ReduceSinkDesc(keyCols, keyCols.size(), valCols, keyColNames, distinctColumnIndices, valColNames, -1, partCols, -1, keyTable, valueTable, writeType);

            final ArrayList<ColumnInfo> signature = parent.getSchema().getSignature().stream().map(e -> new ColumnInfo(e)).map(columnInfo ->
            {
                columnInfo.setInternalName(nameMapping.get(columnInfo.getInternalName()));
                return columnInfo;
            }).collect(Collectors.toCollection(ArrayList::new));
            final ReduceSinkOperator op = (ReduceSinkOperator) OperatorFactory.getAndMakeChild(rsConf, new RowSchema(signature), parent);
