...
public class CopyOnWriteMergeRewriter extends MergeRewriter
{
    ...
    public ParseUtils.ReparseResult rewrite(Context ctx, MergeStatement mergeStatement) throws SemanticException
    {
        ...
        Optional<String> extraPredicate = whenClauses.stream().filter(whenClause -> !(whenClause instanceof MergeStatement.InsertClause)).map(MergeStatement.WhenClause::getExtraPredicate).map(Strings::nullToEmpty).reduce((p1, p2) -> isNotBlank(p2) ? p1 + " OR " + p2 : p2);
    ...
    static class CopyOnWriteMergeWhenClauseSqlGenerator extends MergeRewriter.MergeWhenClauseSqlGenerator
    {
        ...
        public void appendWhenNotMatchedInsertClause(MergeStatement.InsertClause insertClause)
        {
            ...
            sqlGenerator.append(replaceColumnRefsWithTargetPrefix(targetAlias, whereClause.toString()));
            sqlGenerator.append('\n');
        }
        ...
        public void appendWhenMatchedUpdateClause(MergeStatement.UpdateClause updateClause)
        {
            ...
            addWhereClauseOfUpdate(onClauseAsString, updateClause.getExtraPredicate(), updateClause.getDeleteExtraPredicate(), sqlGenerator, columnRefsFunc);
            sqlGenerator.append("\n");
        }
        ...
        protected void handleWhenMatchedDelete(String onClauseAsString, String extraPredicate, String updateExtraPredicate, String hintStr, MultiInsertSqlGenerator sqlGenerator)
        {
            ...
            sqlGenerator.append("\n").indent();
            sqlGenerator.append("( NOT(%s) OR (%s) IS NULL )".replace("%s", columnRefsFunc.apply(whereClause.toString())));
