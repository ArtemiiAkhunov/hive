...
class TezSessionPool<SessionType extends TezSessionPoolSession>
{
    ...
    TezSessionPool(HiveConf initConf, int numSessionsTotal, boolean useAmRegistryIfPresent, SessionObjectFactory<SessionType> sessionFactory)
    {
        ...
        this.executorService = MoreExecutors.listeningDecorator(new ThreadPoolExecutor(0, threadCount, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), new ThreadFactoryBuilder().setDaemon(true).setNameFormat("tez-session-init-%d").build()));
    }
    ...
    private boolean returnSessionInternal(SessionType session, boolean isAsync)
    {
        ...
        {
            return true; // The session will be restarted and return to us.
        }
        ...
        if (isAsync)
        {
            return false; // The caller is responsible for destroying the session.
        }
        try
        {
            session.close(false);
    ...
    private boolean putSessionBack(SessionType session, boolean isFirst)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                {
                    break; // No need to kill anything.
                }
    ...
    private final class ChangeListener implements ServiceInstanceStateChangeListener<TezAmInstance>
    {
    ...
    private ListenableFuture<List<Boolean>> createSessions(final int sessionCount)
    {
        ...
        Collection<Callable<Boolean>> tasks = Stream.generate(() -> (new CreateSessionCallable(Optional.ofNullable(parentSessionState), sessionObjFactory, deltaRemaining))).limit(sessionCount).collect(Collectors.toList());
    ...
    private final class CreateSessionCallable implements Callable<Boolean>
    {
        ...
        public Boolean call() throws Exception
        {
            ...
            while (true)
            {
                ...
                if (!backlog.compareAndSet(oldVal, oldVal - 1))
                {
                    continue;
                }
