...
public class EventUtils
{
    ...
    public interface NotificationFetcher
    {
        ...
        long getDbNotificationEventsCount(long fromEventId, String dbName, Long toEventId, int limit) throws IOException;

        List<NotificationEvent> getNextNotificationEvents(long pos, IMetaStoreClient.NotificationFilter filter) throws IOException;
    }
    ...
    public static class MSClientNotificationFetcher implements NotificationFetcher
    {
        ...
        public int getBatchSize() throws IOException
        {
            if (batchSize == null)
            {
                try
                {
                    batchSize = Integer.parseInt(hiveDb.getMSC().getConfigValue(MetastoreConf.ConfVars.BATCH_RETRIEVE_MAX.toString(), "50"));
