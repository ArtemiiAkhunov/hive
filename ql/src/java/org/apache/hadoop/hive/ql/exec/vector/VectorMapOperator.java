...
public class VectorMapOperator extends AbstractMapOperator
{
    ...
    protected abstract class VectorPartitionContext
    {
         ...
         */
        public abstract void init(Configuration hconf) throws SerDeException, Exception;
        ...
        public void init(Configuration hconf) throws SerDeException, HiveException
        {
            ...
            switch (vectorPartDesc.getVectorDeserializeType())
            {
                case LAZY_SIMPLE:
                {
                    ...
                    LazySimpleDeserializeRead lazySimpleDeserializeRead = new LazySimpleDeserializeRead(minimalDataTypeInfos, batchContext.getRowdataTypePhysicalVariations(),
                            /* useExternalBuffer */ true, simpleSerdeParams);
    ...
    {
        if (!abort && oneRootOperator != null && !oneRootOperator.getDone() && currentReadType != VectorMapOperatorReadType.VECTORIZED_INPUT_FILE_FORMAT)
        {
            if (deserializerBatch.size > 0)
            {
                numRows += deserializerBatch.size;
