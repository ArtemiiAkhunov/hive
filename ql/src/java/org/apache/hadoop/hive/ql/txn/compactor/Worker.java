...
public class Worker extends RemoteCompactorThread implements MetaStoreThread
{
    ...
    protected Boolean findNextCompactionAndExecute(boolean collectGenericStats, boolean collectMrStats)
    {
        ...
        try
        {
            ...
            {
                LOG.warn("The returned compaction request ({}) belong to a different pool. Although the worker is assigned to the {} pool," + " it will process the request.", ci, getPoolName());
            }
        ...
        {
            statsUpdater.gatherStats(ci, conf, CompactorUtil.runJobAsSelf(ci.runAs) ? ci.runAs : table.getOwner(), CompactorUtil.getCompactorJobQueueName(conf, ci, table), msc);
        }
