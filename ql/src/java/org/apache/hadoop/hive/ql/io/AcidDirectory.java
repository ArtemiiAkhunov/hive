...
public final class AcidDirectory implements AcidUtils.Directory
{
    ...
    public List<FileInfo> getFiles() throws IOException
    {
        ...
        } else
        {
            ...
            {
                baseAndDeltaFiles.add(new FileInfo(fileId, isBaseInRawFormat() ? ORIGINAL_BASE : ACID_SCHEMA));
            }
        }
        for (AcidUtils.ParsedDelta parsedDelta : currentDirectories)
        {
            ...
            if (parsedDelta.isRawFormat() && parsedDelta.getMinWriteId() != parsedDelta.getMaxWriteId())
            {
                ...
                throw new IllegalStateException("Delta in " + ORIGINAL_BASE + " format but txnIds are out of range: " + parsedDelta.getPath());
            }

            AcidUtils.AcidBaseFileType deltaType = parsedDelta.isRawFormat() ? ORIGINAL_BASE : ACID_SCHEMA;
    ...
    {
        return "Aborted Directories: " + abortedDirectories + "; aborted writeIds: " + abortedWriteIds + "; original: " + originalFiles + "; obsolete: " + obsolete + "; currentDirectories: " + currentDirectories + "; base: " + base;
    }
