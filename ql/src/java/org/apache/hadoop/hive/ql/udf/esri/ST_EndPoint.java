...
@Description(name = "ST_EndPoint", value = "_FUNC_(geometry) - returns the last point of an ST_Linestring", extended = "Example:\n" + "  > SELECT _FUNC_(ST_LineString(1.5,2.5, 3.0,2.2)) FROM src LIMIT 1;  -- POINT(3.0 2.0)\n")
...
public class ST_EndPoint extends ST_GeometryAccessor
{
    ...
    public BytesWritable evaluate(BytesWritable geomref)
    {
        ...
        if (GeometryUtils.getType(geomref) == GeometryUtils.OGCType.ST_LINESTRING)
        {
            ...
            return GeometryUtils.geometryToEsriShapeBytesWritable(OGCGeometry.createFromEsriGeometry(lines.getPoint(lines.getPointCount() - 1), spatialReference));
        } else
        {
