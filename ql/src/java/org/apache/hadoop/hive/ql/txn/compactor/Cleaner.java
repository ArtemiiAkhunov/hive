...
public class Cleaner extends MetaStoreCompactorThread
{
    ...
    public void init(AtomicBoolean stop) throws Exception
    {
        ...
        cleanerExecutor = CompactorUtil.createExecutorWithThreadFactory(conf.getIntVar(HiveConf.ConfVars.HIVE_COMPACTOR_CLEANER_THREADS_NUM), COMPACTOR_CLEANER_THREAD_NAME_FORMAT);
        FSRemover fsRemover = new FSRemover(conf, ReplChangeManager.getInstance(conf), metadataCache);
        cleanupHandlers = TaskHandlerFactory.getInstance().getHandlers(conf, txnHandler, metadataCache, metricsEnabled, fsRemover);
    }
        ...
        try
        {
            do
            {
                ...
                try
                {
                    ...
                    {
                        stopCycleUpdater();
                        startCycleUpdater(HiveConf.getTimeVar(conf, HiveConf.ConfVars.HIVE_COMPACTOR_CLEANER_DURATION_UPDATE_INTERVAL, TimeUnit.MILLISECONDS), new CleanerCycleUpdater(MetricsConstants.COMPACTION_CLEANER_CYCLE_DURATION, startedAt));
                    }
                    ...
                    for (TaskHandler cleanupHandler : cleanupHandlers)
                    {
                        try
                        {
                            ...
                            {
                                CompletableFuture<Void> asyncTask = CompletableFuture.runAsync(task, cleanerExecutor).exceptionally(t ->
                                {
                        ...
                        {
                            LOG.error("Caught an exception while executing RequestHandler loop : {} of compactor cleaner, {}", cleanupHandler.getClass().getName(), t.getMessage());
                            throw t;
                        }
                    }
                    CompactorUtil.checkInterrupt(CLASS_NAME);
    ...
    {
        return MetastoreConf.getBoolVar(conf, MetastoreConf.ConfVars.COMPACTOR_CLEANER_TABLECACHE_ON);
    }
