    ...
    protected void postProcess(TableName tableName, Table table, AlterTableAddPartitionDesc desc, Task<DDLWork> ddlTask) throws SemanticException
    {
        ...
        for (AlterTableAddPartitionDesc.PartitionDesc partitionDesc : desc.getPartitions())
        {
            if (partitionDesc.getLocation() != null)
            {
                ...
                LoadTableDesc loadTableWork = new LoadTableDesc(new Path(partitionDesc.getLocation()), Utilities.getTableDesc(table), partitionDesc.getPartSpec(), LoadTableDesc.LoadFileType.KEEP_EXISTING, //not relevant - creating new partition
                        writeId);
                ...
                Task<MoveWork> moveTask = TaskFactory.get(new MoveWork(getInputs(), getOutputs(), loadTableWork, null, true, //make sure to check format
                        false)); //is this right?
