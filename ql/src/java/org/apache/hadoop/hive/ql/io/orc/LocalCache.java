...
class LocalCache implements OrcInputFormat.FooterCache
{
    ...
    {
        CacheBuilder<Path, TailAndFileData> builder = CacheBuilder.newBuilder().initialCapacity(DEFAULT_CACHE_INITIAL_CAPACITY).concurrencyLevel(numThreads).maximumWeight(cacheMemSize).weigher(new Weigher<Path, TailAndFileData>()
        {
            @Override
    ...
    public void put(Path path, OrcTail tail)
    {
        ...
        cache.put(path, new TailAndFileData(tail.getFileTail().getFileLength(), tail.getFileModificationTime(), bb.duplicate()));
    }

    @Override
    public void getAndValidate(final List<HadoopShims.HdfsFileStatusWithId> files, final boolean isOriginal, final OrcTail[] result, final ByteBuffer[] ppdResult) throws IOException, HiveException
    {
    ...
    @Override
    public void put(final OrcInputFormat.FooterCacheKey cacheKey, final OrcTail orcTail) throws IOException
    {
