...
public class FileSinkOperator extends TerminalOperator<FileSinkDesc> implements Serializable, IConfigureJobConf
{
    ...
    public class FSPaths implements Cloneable
    {
        ...
        public FSPaths(Path specPath, boolean isMmTable, boolean isDirectInsert, boolean isInsertOverwrite, AcidUtils.Operation acidOperation)
        {
            ...
            } else
            {
                ...
                if (isMmTable)
                {
                    subdirForTxn = AcidUtils.baseOrDeltaSubdir(conf.getInsertOverwrite(), conf.getTableWriteId(), conf.getTableWriteId(), conf.getStatementId());
                } else
                {
                    /**
            ...
            {
                Utilities.FILE_OP_LOGGER.trace("new FSPaths for " + numFiles + " files, dynParts = " + bDynParts + " (spec path " + specPath + ")");
            }
        ...
        public void closeWriters(boolean abort, List<Path> deleteDeltas) throws HiveException
        {
            ...
            for (int i = 0; i < updaters.length; i++)
            {
                if (updaters[i] != null)
                {
                    ...
                    if (isDirectInsert && (stats.getRowCount() > 0 || isInsertOverwrite || AcidUtils.Operation.DELETE.equals(acidOperation) || AcidUtils.Operation.UPDATE.equals(acidOperation)))
                    {
                        ...
                        LOG.debug("The following path has been added to the outPathsCommitted array: " + outPathsCommitted[i].toString());
                    }
                    try
                    {
        ...
        private void commit(FileSystem fs, List<Path> commitPaths, List<Path> deleteDeltas) throws HiveException
        {
            for (int idx = 0; idx < outPaths.length; ++idx)
            {
                ...
                {
                    throw new HiveException("Unable to commit output from: " + outPaths[idx] + " to: " + finalPaths[idx], e);
                }
            }
        ...
        private void commitOneOutPath(int idx, FileSystem fs, List<Path> commitPaths) throws IOException, HiveException
        {
            ...
            if (outPaths[idx] != null && fs.exists(outPaths[idx]))
            {
                if (Utilities.FILE_OP_LOGGER.isTraceEnabled())
                {
                    Utilities.FILE_OP_LOGGER.trace("committing " + outPaths[idx] + " to " + finalPaths[idx] + " (mm table =" + isMmTable + ", direct insert = " + isDirectInsert + ")");
                }
                if (isMmTable)
                {
                ...
                } else if (isDirectInsert && outPathsCommitted[idx] != null)
                {
                    if (Utilities.FILE_OP_LOGGER.isTraceEnabled())
                    {
                        Utilities.FILE_OP_LOGGER.trace("committing " + outPathsCommitted[idx] + " (direct insert = " + isDirectInsert + ")");
                    }
        ...
        public int createDynamicBucket(int bucketNum)
        {
            ...
            if (this.finalPaths[writerOffset] == null)
            {
                ...
                } else
                {
                    ...
                    String bucketName = Utilities.replaceTaskIdFromFilename(Utilities.getTaskId(hconf), bucketNum);
    ...
    private void initializeSpecPath()
    {
        ...
        if (Utilities.FILE_OP_LOGGER.isTraceEnabled())
        {
            Utilities.FILE_OP_LOGGER.trace("Setting up FSOP " + System.identityHashCode(this) + " (" + conf.isLinkedFileSink() + ") with " + taskId + " and " + specPath + " + " + unionPath);
        }
    }
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        try
        {
            ...
            setWriteOperationIsSorted(jc, getConf().getTableInfo().getTableName(), dpCtx != null && dpCtx.hasCustomSortExprs());
    ...
    {
        return Optional.ofNullable(conf).map(FileSinkDesc::getTableInfo).map(TableDesc::getProperties).map(props -> props.getProperty(hive_metastoreConstants.META_TABLE_STORAGE)).map(handler ->
        {
            try
            {
        ...
        }).map(HiveStorageHandler::alwaysUnpartitioned).orElse(Boolean.FALSE);
    }
    ...
    protected void createBucketForFileIdx(FSPaths fsp, int filesIdx) throws HiveException
    {
        try
        {
            ...
            {
                fsp.initializeBucketPaths(filesIdx, AcidUtils.BUCKET_PREFIX + String.format(AcidUtils.BUCKET_DIGITS, bucketId), isNativeTable(), isSkewedStoredAsSubDirectories);
            } else
            {
            ...
            if (Utilities.FILE_OP_LOGGER.isTraceEnabled())
            {
                Utilities.FILE_OP_LOGGER.trace("createBucketForFileIdx " + filesIdx + ": final path " + fsp.finalPaths[filesIdx] + "; out path " + fsp.outPaths[filesIdx] + " (spec path " + specPath + ", tmp path " + fsp.buildTmpPath() + ", task " + taskId + ")");
            }
            ...
            {
                Path outPath = fsp.outPaths[filesIdx];
                if (conf.isDirectInsert() && !FileUtils.mkdir(fs, outPath.getParent(), hconf))
                {
                ...
                fsp.updaters[filesIdx] = HiveFileFormatUtils.getAcidRecordUpdater(jc, conf.getTableInfo(), acidBucketNum, conf, fsp.outPaths[filesIdx], inspector, reporter, -1, attemptId); // outPath.getParent()
            }
    ...
    public void process(Object row, int tag) throws HiveException
    {
        ...
        try
        {
            ...
            if ((++numRows == cntr) && LOG.isInfoEnabled())
            {
                ...
                LOG.info("{}: {} written - {}", this, conf.isDeleteOfSplitUpdate() ? "delete delta records" : "records", numRows);
            }
    ...
    private int findWriterOffset(Object row) throws HiveException
    {
        ...
        {
            assert getConf().getWriteType() != AcidUtils.Operation.DELETE && getConf().getWriteType() != AcidUtils.Operation.UPDATE : "Unexpected operation type: " + getConf().getWriteType();
    ...
    {
        FSPaths fsp2 = new FSPaths(specPath, conf.isMmTable(), conf.isDirectInsert(), conf.getInsertOverwrite(), conf.getAcidOperation());
        ...
        if (Utilities.FILE_OP_LOGGER.isTraceEnabled())
        {
            Utilities.FILE_OP_LOGGER.trace("creating new paths {} for {}, childSpec {}: tmpPath {}," + " task path {}", System.identityHashCode(fsp2), pathKey, unionPath, fsp2.buildTmpPath(), fsp2.buildTaskOutputTempPath());
        }
    ...
    protected String generateListBucketingDirName(Object row)
    {
        ...
        if (row != null)
        {
            ...
            ObjectInspectorUtils.copyToStandardObject(standObjs, row, (StructObjectInspector) inputObjInspectors[0], ObjectInspectorCopyOption.WRITABLE);
    ...
    public void jobCloseOp(Configuration hconf, boolean success) throws HiveException
    {
        try
        {
            if ((conf != null) && isNativeTable())
            {
                ...
                {
                    int dpLevels = dpCtx == null ? 0 : dpCtx.getNumDPCols(), lbLevels = lbCtx == null ? 0 : lbCtx.calculateListBucketingLevel();
                    // TODO: why is it stored in both table and dpCtx?
                    int numBuckets = (conf.getTable() != null) ? conf.getTable().getNumBuckets() : (dpCtx != null ? dpCtx.getNumBuckets() : 0);
                    MissingBucketsContext mbc = new MissingBucketsContext(conf.getTableInfo(), numBuckets, conf.getCompressed());
                    Utilities.handleDirectInsertTableFinalPath(specPath, unionSuffix, hconf, success, dpLevels, lbLevels, mbc, conf.getTableWriteId(), conf.getStatementId(), reporter, conf.isMmTable(), conf.isMmCtas(), conf.getInsertOverwrite(), conf.isDirectInsert(), conf.getStaticSpec(), conf.getAcidOperation(), conf);
                }
            }
        } catch (IOException e)
        {
