...
public class HiveProtoLoggingHook implements ExecuteWithHookContext
{
    ...
    {
        QUERY, STATUS, TEZ, MAPRED, INVOKER_INFO, SESSION_ID, THREAD_NAME, VERSION, CLIENT_IP_ADDRESS, HIVE_ADDRESS, HIVE_INSTANCE_TYPE, CONF, PERF, LLAP_APP_ID, ERROR_MESSAGE, QUERY_TYPE
    }
    ...
    static class EventLogger
    {
        ...
        EventLogger(HiveConf conf, Clock clock)
        {
            ...
            this.queueCapacity = conf.getInt(ConfVars.HIVE_PROTO_EVENTS_QUEUE_CAPACITY.varname, ConfVars.HIVE_PROTO_EVENTS_QUEUE_CAPACITY.defaultIntVal);
            ...
            try
            {
                if (baseDir != null)
                {
                    tmpLogger = new DatePartitionedLogger<>(HiveHookEventProto.PARSER, new Path(baseDir), conf, clock);
                }
            } catch (IOException e)
            {
            ...
            logWriter.scheduleWithFixedDelay(() -> handleTick(), rolloverInterval, rolloverInterval, TimeUnit.MICROSECONDS);
        }
        ...
        void handle(HookContext hookContext)
        {
            ...
            if (event != null)
            {
                try
                {
                    ...
                    {
                        LOG.warn("Writer queue full ignoring event {} for query {}", hookContext.getHookType(), plan.getQueryId());
                    }
        ...
        private void writeEvent(HiveHookEventProtoPartialBuilder builder)
        {
            ...
            for (int retryCount = 0; retryCount <= MAX_RETRIES; ++retryCount)
            {
                ...
                } catch (IOException e)
                {
                    ...
                    {
                        LOG.warn("Error writing proto message for query {}, eventType: {}, retryCount: {}," + " error: {} ", event.getHiveQueryId(), event.getEventType(), retryCount, e.getMessage(), e);
        ...
        private HiveHookEventProtoPartialBuilder getPreHookEvent(HookContext hookContext)
        {
            ...
            return new HiveHookEventProtoPartialBuilder(builder, explainWork, otherInfo, plan.getQueryStr(), conf.getVar(ConfVars.HIVE_STAGE_ID_REARRANGE));
        }
        ...
        public static void addMapEntry(HiveHookEventProto.Builder builder, OtherInfoType key, String value)
        {
            if (value != null)
            {
                builder.addOtherInfo(MapFieldEntry.newBuilder().setKey(key.name()).setValue(value).build());
            }
        }
