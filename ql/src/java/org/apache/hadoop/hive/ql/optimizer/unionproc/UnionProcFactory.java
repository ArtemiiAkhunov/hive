...
public final class UnionProcFactory
{
    ...
    public static int getPositionParent(UnionOperator union, Stack<Node> stack)
    {
        ...
        List<Operator<? extends OperatorDesc>> parUnion = union.getParentOperators();
    ...
    public static class UnknownUnion implements SemanticNodeProcessor
    {
        ...
        public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx, Object... nodeOutputs) throws SemanticException
        {
            ...
            while (start >= 0)
            {
                Operator<? extends OperatorDesc> parent = (Operator<? extends OperatorDesc>) stack.get(start);
        ...
        private void pushOperatorsAboveUnion(UnionOperator union, Stack<Node> stack, int pos) throws SemanticException
        {
            ...
            try
            {
                ...
                for (Operator<? extends OperatorDesc> parent : parents)
                {
                    ...
                    {
                        Utilities.FILE_OP_LOGGER.trace("Created LinkedFileSink for union " + fileSinkDesc.getDirName() + "; parent " + parentDirName);
                    }
                    parent.setChildOperators(null);
