...
public class ReExecuteLostAMQueryPlugin implements IReExecutionPlugin
{
    ...
    private static final Pattern LOST_AM_CONTAINER_ERROR_PATTERN = Pattern.compile(".*AM Container for .* exited .* exitCode: -100.*");
        ...
        public void run(HookContext hookContext) throws Exception
        {
            if (hookContext.getHookType() == HookContext.HookType.ON_FAILURE_HOOK)
            {
                ...
                if (message != null)
                {
                    ...
                    if (LOST_AM_CONTAINER_ERROR_PATTERN.matcher(message).matches() || message.contains(UNMANAGED_AM_FAILURE) || message.contains(DAG_LOST_FAILURE))
                    {
                        retryPossible = true;
                    }
                    LOG.info("Got exception message: {} retryPossible: {}, dags seen so far: {}", message, retryPossible, dagIds);
                }
            }
        }
    }
