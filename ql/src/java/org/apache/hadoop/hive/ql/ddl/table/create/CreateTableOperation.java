...
public class CreateTableOperation extends DDLOperation<CreateTableDesc>
{
    ...
    public int execute() throws HiveException
    {
        ...
        if (desc.getReplicationSpec().isInReplicationScope())
        {
            ...
            if (existingTable != null)
            {
                ...
                if (desc.getReplicationSpec().allowEventReplacementInto(dbParams) || desc.getReplicationSpec().isForceOverwrite())
                {
                    ...
                    if (existingTable.getTableType().equals(TableType.MANAGED_TABLE) && tbl.getTableType().equals(TableType.EXTERNAL_TABLE) && (!existingTable.getDataLocation().equals(tbl.getDataLocation())))
                    {
        ...
        } else
        {
            ...
            if (desc.getReplicationSpec().isInReplicationScope() && !tbl.getParameters().containsKey(ReplConst.REPL_TARGET_TABLE_PROPERTY))
            {
    ...
    {
        if (CollectionUtils.isNotEmpty(desc.getPrimaryKeys()) || CollectionUtils.isNotEmpty(desc.getForeignKeys()) || CollectionUtils.isNotEmpty(desc.getUniqueConstraints()) || CollectionUtils.isNotEmpty(desc.getNotNullConstraints()) || CollectionUtils.isNotEmpty(desc.getDefaultConstraints()) || CollectionUtils.isNotEmpty(desc.getCheckConstraints()))
        {
            context.getDb().createTable(tbl, desc.getIfNotExists(), desc.getPrimaryKeys(), desc.getForeignKeys(), desc.getUniqueConstraints(), desc.getNotNullConstraints(), desc.getDefaultConstraints(), desc.getCheckConstraints());
        } else
        {
        ...
        if (desc.isCTAS())
        {
            ...
            if (!createdTable.isPartitioned() && AcidUtils.isTransactionalTable(createdTable))
            {
                ...
                context.getDb().addWriteNotificationLog(createdTable, null, newFilesList, tTable.getWriteId(), null);
            }
        }
    }
    ...
    public static boolean doesTableNeedLocation(Table tbl)
    {
        ...
        if (tbl.getStorageHandler() != null)
        {
            ...
            retval = !"org.apache.hadoop.hive.hbase.HBaseStorageHandler".equals(sh) && !Constants.DRUID_HIVE_STORAGE_HANDLER_ID.equals(sh) && !Constants.JDBC_HIVE_STORAGE_HANDLER_ID.equals(sh) && !"org.apache.hadoop.hive.accumulo.AccumuloStorageHandler".equals(sh);
        }
        return retval;
    }
