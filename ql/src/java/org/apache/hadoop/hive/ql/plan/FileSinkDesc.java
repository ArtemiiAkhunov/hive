...
public class FileSinkDesc extends AbstractOperatorDesc implements IStatsGatherDesc
{
     ...
     */
    public FileSinkDesc(final Path dirName, final TableDesc tableInfo, final boolean compressed, final int destTableId, final boolean multiFileSpray, final boolean canBeMerged, final int numFiles, final int totalFiles, final List<ExprNodeDesc> partitionCols, final DynamicPartitionCtx dpCtx, Path destPath, Long mmWriteId, boolean isMmCtas, boolean isInsertOverwrite, boolean isQuery, boolean isCTASorCM, boolean isDirectInsert, AcidUtils.Operation acidOperation, boolean deleteOfSplitUpdate)
    {
    ...
    public FileSinkDesc(final Path dirName, final TableDesc tableInfo, final boolean compressed)
    {
    ...
    {
        FileSinkDesc ret = new FileSinkDesc(dirName, tableInfo, compressed, destTableId, multiFileSpray, canBeMerged, numFiles, totalFiles, partitionCols, dpCtx, destPath, mmWriteId, isMmCtas, isInsertOverwrite, isQuery, isCTASorCM, isDirectInsert, acidOperation, deleteOfSplitUpdate);
    ...
    public boolean isFullAcidTable()
    {
        ...
        {
            return AcidUtils.isTablePropertyTransactional(getTableInfo().getProperties()) && !AcidUtils.isInsertOnlyTable(getTableInfo().getProperties());
        }
    }
    ...
    {
        return getTable() != null ? AcidUtils.isCompactionTable(table.getParameters()) : AcidUtils.isCompactionTable(getTableInfo().getProperties());
    }
    ...
    {
        String compactionType = getTable() != null ? table.getParameters().get(COMPACTOR_TABLE_PROPERTY) : getTableInfo().getProperties().getProperty(COMPACTOR_TABLE_PROPERTY);
        return StringUtils.isNotBlank(compactionType) && CompactionType.valueOf(compactionType).equals(CompactionType.REBALANCE);
    }
