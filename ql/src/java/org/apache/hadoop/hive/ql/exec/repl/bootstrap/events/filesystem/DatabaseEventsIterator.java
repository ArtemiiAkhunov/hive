...
class DatabaseEventsIterator implements Iterator<BootstrapEvent>
{
        ...
        try
        {
            ...
            if (replicationState == null && next == null)
            {
                while (remoteIterator.hasNext())
                {
                    ...
                    if (next.getPath().toString().endsWith(EximUtil.FILE_LIST_EXTERNAL) || next.getPath().toString().endsWith(ReplUtils.REPL_TABLE_LIST_DIR_NAME))
                    {
                        continue;
                    }
                    if (next.getPath().toString().endsWith(EximUtil.METADATA_NAME))
                    {
                        ...
                        List<String> filteredNames = Arrays.stream(replacedString.split(Path.SEPARATOR)).filter(StringUtils::isNotBlank).collect(Collectors.toList());
    ...
    public BootstrapEvent next()
    {
        ...
        return postProcessing(new FSTableEvent(hiveConf, next.toString(), new Path(getDbLevelDataPath(), next.getName()).toString()));
    }
    ...
    private BootstrapEvent eventForReplicationState()
    {
        ...
        {
            BootstrapEvent bootstrapEvent = new FSPartitionEvent(hiveConf, previous.toString(), new Path(getDbLevelDataPath(), previous.getName()).toString(), replicationState);
            replicationState = null;
            return bootstrapEvent;
        } else if (replicationState.lastTableReplicated != null)
        {
            FSTableEvent event = new FSTableEvent(hiveConf, previous.toString(), new Path(new Path(dbLevelPath, EximUtil.DATA_PATH_NAME), previous.getName()).toString());
            replicationState = null;
            return event;
        }
