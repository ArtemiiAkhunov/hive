...
public class WmTezSession extends TezSessionPoolSession implements AmPluginNode
{
    ...
    @VisibleForTesting
    WmTezSession(String sessionId, Manager testParent, SessionExpirationTracker expiration, HiveConf conf)
    {
    ...
    {
        AmPluginInfo info = si == null ? null : new AmPluginInfo(si.getHost(), si.getPluginPort(), si.getPluginToken(), si.getPluginTokenJobId());
        synchronized (amPluginInfoLock)
        {
            ...
            if ((amPluginEndpointVersion != null) && ((amPluginEndpointVersion > ephSeqVersion) || (amPluginEndpointVersion == ephSeqVersion && info != null)))
            {
    ...
    private void handleGuaranteedTasksChange(int guaranteedCount)
    {
        ...
        synchronized (actualState)
        {
            ...
            if (actualState.sending != -1 || actualState.sent == guaranteedCount)
            {
                return;
            }
    ...
    Integer setSendingGuaranteed(Integer intAlloc)
    {
        ...
        synchronized (actualState)
        {
            ...
            if (actualState.sending != -1)
            {
                return null; // The sender will take care of this.
            }
            if (actualState.sent == intAlloc)
            {
                return null; // The value didn't change.
            }
    ...
    public String getAllocationState()
    {
        synchronized (actualState)
        {
            return "actual/target " + actualState.sent + "/" + actualState.target + (actualState.sending >= 0 ? "; sending" : "");
        }
    }
    ...
    private final class TimeoutRunnable implements Runnable
    {
        ...
        public void run()
        {
            ...
            {
                timeoutTimer = null;
                if (amRegistryFuture == null || amRegistryFuture.isDone())
                {
                    return;
                }
    ...
    {
        return super.toString() + ", WM state poolName=" + poolName + ", clusterFraction=" + clusterFraction + ", queryId=" + queryId + ", killReason=" + killReason;
    }
