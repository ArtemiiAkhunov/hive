...
public class RangerLoadTask extends Task<RangerLoadWork> implements Serializable
{
    ...
    public int execute()
    {
        try
        {
            ...
            if (url == null)
            {
                throw new SemanticException(ErrorMsg.REPL_INVALID_CONFIG_FOR_SERVICE.format("Ranger configuration is not valid " + ReplUtils.RANGER_CONFIGURATION_RESOURCE_NAME, ReplUtils.REPL_RANGER_SERVICE));
            }
            ...
            {
                throw new SemanticException(ErrorMsg.REPL_INVALID_CONFIG_FOR_SERVICE.format("Ranger endpoint is not valid " + rangerEndpoint, ReplUtils.REPL_RANGER_SERVICE));
            }
            if (!rangerRestClient.checkConnection(rangerEndpoint, conf))
            {
                throw new SemanticException(ErrorMsg.REPL_EXTERNAL_SERVICE_CONNECTION_ERROR.format(ReplUtils.REPL_RANGER_SERVICE, "Ranger endpoint is not valid " + rangerEndpoint));
            }
            if (work.getCurrentDumpPath() != null)
            {
                LOG.info("Importing Ranger Metadata from {} ", work.getCurrentDumpPath());
                rangerExportPolicyList = rangerRestClient.readRangerPoliciesFromJsonFile(new Path(work.getCurrentDumpPath(), ReplUtils.HIVE_RANGER_POLICIES_FILE_NAME), conf);
                int expectedPolicyCount = rangerExportPolicyList == null ? 0 : rangerExportPolicyList.getListSize();
                replLogger = new RangerLoadLogger(work.getSourceDbName(), work.getTargetDbName(), work.getCurrentDumpPath().toString(), expectedPolicyCount);
                replLogger.startLog();
            ...
            List<RangerPolicy> updatedRangerPolicies = rangerRestClient.changeDataSet(rangerPolicies, work.getSourceDbName(), work.getTargetDbName());
            ...
            if (!CollectionUtils.isEmpty(updatedRangerPolicies))
            {
                ...
                rangerRestClient.importRangerPolicies(rangerExportPolicyList, work.getTargetDbName(), rangerEndpoint, rangerHiveServiceName, conf);
        ...
        } catch (RuntimeException e)
        {
            ...
            try
            {
                ReplUtils.handleException(true, e, work.getCurrentDumpPath().getParent().toString(), work.getMetricCollector(), getName(), conf);
            } catch (Exception ex)
            {
        ...
        } catch (Exception e)
        {
            ...
            try
            {
                return ReplUtils.handleException(true, e, work.getCurrentDumpPath().getParent().toString(), work.getMetricCollector(), getName(), conf);
            } catch (Exception ex)
            {
                LOG.error("Failed to collect replication metrics: ", ex);
                return errorCode;
