...
public class AcidExportSemanticAnalyzer extends RewriteSemanticAnalyzer<Object>
{
    ...
    private void analyzeAcidExport(ASTNode ast, Table exportTable, ASTNode tokRefOrNameExportTable) throws SemanticException
    {
        ...
        CreateTableLikeDesc ctlt = new CreateTableLikeDesc(newTableName, false, true, null, null, location, null, null, tblProps, true, //important so we get an exception on name collision
        ...
        StringBuilder rewrittenQueryStr = generateExportQuery(newTable.getPartCols(), tokRefOrNameExportTable, (ASTNode) tokRefOrNameExportTable.parent, newTableName);
    ...
    private StringBuilder generateExportQuery(List<FieldSchema> partCols, ASTNode tokRefOrNameExportTable, ASTNode tableTree, String newTableName) throws SemanticException
    {
        ...
        if (exportTableSpec.getPartSpec() != null)
        {
            ...
            for (Map.Entry<String, String> ent : exportTableSpec.getPartSpec().entrySet())
            {
                ...
                whereClause.append(HiveUtils.unparseIdentifier(ent.getKey(), conf)).append(" = ").append(genPartValueString(partCols.get(partColsIdx).getType(), ent.getValue()));
            }
            if (whereClause != null)
            {
     ...
     */
    private void addExportTask(List<Task<?>> rootTasks, Task<ExportWork> exportTask, Task<DDLWork> alterTable)
    {
    ...
    private List<Task<?>> findStatsTasks(List<Task<?>> rootTasks, List<Task<?>> statsTasks)
    {
        for (Task<? extends Serializable> t : rootTasks)
        {
