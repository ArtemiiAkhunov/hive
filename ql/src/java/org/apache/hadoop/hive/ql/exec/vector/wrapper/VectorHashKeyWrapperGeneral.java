...
public class VectorHashKeyWrapperGeneral extends VectorHashKeyWrapperBase
{
    ...
    public VectorHashKeyWrapperGeneral(HashContext ctx, int longValuesCount, int doubleValuesCount, int byteValuesCount, int decimalValuesCount, int timestampValuesCount, int intervalDayTimeValuesCount, int keyCount)
    {
        super();
        hashCtx = ctx;
    ...
    public void setHashKey()
    {
        ...
        int hash = Arrays.hashCode(longValues) ^ Arrays.hashCode(doubleValues) ^ Arrays.hashCode(isNull);
    ...
    private boolean bytesEquals(VectorHashKeyWrapperGeneral keyThat)
    {
        ...
        for (int i = 0; i < byteValues.length; ++i)
        {
            ...
            if (byteLengths[i] != -1)
            {
                if (!StringExpr.equal(byteValues[i], byteStarts[i], byteLengths[i], keyThat.byteValues[i], keyThat.byteStarts[i], keyThat.byteLengths[i]))
                {
                    return false;
                }
            }
    ...
    protected Object clone()
    {
        ...
        if (byteLengths.length > 0)
        {
            ...
            for (int i = 0; i < byteValues.length; ++i)
            {
                ...
                if (byteLengths[i] != -1)
                {
                    clone.byteValues[i] = Arrays.copyOfRange(byteValues[i], byteStarts[i], byteStarts[i] + byteLengths[i]);
                }
            }
        } else
        {
