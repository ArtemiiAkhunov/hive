 ...
 * UDFToDouble.
 */
@VectorizedExpressions({CastTimestampToDouble.class, CastLongToDouble.class, CastDecimalToDouble.class, CastStringToDouble.class})
...
public class UDFToDouble extends UDF
{
    ...
    public DoubleWritable evaluate(TimestampWritableV2 i)
    {
        ...
        } else
        {
            try
            {
                doubleWritable.set(TimestampTZUtil.convertTimestampTZToDouble(UDFUtils.getTimestampTZFromTimestamp(i.getTimestamp())));
                return doubleWritable;
            } catch (NumberFormatException e)
            {
