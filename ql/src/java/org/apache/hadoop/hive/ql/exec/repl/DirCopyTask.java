...
public class DirCopyTask extends Task<DirCopyWork> implements Serializable
{
    ...
    private boolean createAndSetPathOwner(Path destPath, Path sourcePath, HiveConf clonedConf) throws IOException
    {
        ...
        LOG.info("Setting permission for path dest {} from source {} owner {} : {} : {}", destPath, sourcePath, status.getOwner(), status.getGroup(), status.getPermission());
    ...
    private void setAclsToTarget(FileStatus sourceStatus, Path sourcePath, Path destPath, HiveConf clonedConf) throws IOException
    {
        ...
        if (sourceAcls != null && sourceAcls.getEntries().size() > 0)
        {
            ...
            List<AclEntry> effectiveAclEntries = AclUtil.getAclFromPermAndEntries(sourceStatus.getPermission(), sourceAcls.getEntries());
    ...
    private boolean setTargetPathOwner(Path targetPath, Path sourcePath, UserGroupInformation proxyUser, HiveConf clonedConf) throws IOException, InterruptedException
    {
        ...
        return proxyUser.doAs((PrivilegedExceptionAction<Boolean>) () -> createAndSetPathOwner(targetPath, sourcePath, clonedConf));
    }
    ...
    public int execute()
    {
        LOG.info("Started DirCopyTask for table {} from source: {} to target: {}", work.getTableName(), work.getFullyQualifiedSourcePath(), work.getFullyQualifiedTargetPath());
    ...
    private boolean runFallbackDistCp(Path sourcePath, Path targetPath, UserGroupInformation proxyUser, HiveConf clonedConf) throws IOException
    {
        ...
        boolean response = FileUtils.distCp(sourcePath.getFileSystem(clonedConf), // source file system
                Collections.singletonList(sourcePath),  // list of source paths
                targetPath, false, proxyUser, clonedConf, ShimLoader.getHadoopShims());
        return response;
    }
}
