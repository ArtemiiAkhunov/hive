...
public class GuaranteedTasksAllocator implements QueryAllocationManager
{
    ...
    protected int getExecutorCount(boolean allowUpdate)
    {
        ...
        {
            LOG.warn("Failed to get LLAP cluster information for " + HiveConf.getTrimmedVar(this.conf, ConfVars.LLAP_DAEMON_SERVICE_HOSTS) + "; we may rely on outdated cluster status");
        }
        ...
        if (unknownNodes > 0)
        {
            LOG.error("There are " + unknownNodes + " nodes with unknown executor count; only " + clusterState.getKnownExecutorCount() + " guaranteed tasks will be allocated");
        }
    ...
    private void updateSessionAsync(final WmTezSession session, final Integer intAlloc)
    {
        ...
        UpdateQueryRequestProto request = UpdateQueryRequestProto.newBuilder().setGuaranteedTaskCount(valueToSend.intValue()).build();
