...
public class TestListPartitionsWithXIncludeParams extends TestListPartitions
{
    ...
    public TestListPartitionsWithXIncludeParams(String name, AbstractMetaStoreService metaStore)
    {
        ...
        MetastoreConf.setVar(hiveConf, MetastoreConf.ConfVars.EXPRESSION_PROXY_CLASS, PartitionExpressionForMetastoreTest.class.getName());
        MetastoreConf.setVar(hiveConf, MetastoreConf.ConfVars.METASTORE_PARTITIONS_PARAMETERS_INCLUDE_PATTERN, "%k_y_");
        MetastoreConf.setVar(hiveConf, MetastoreConf.ConfVars.METASTORE_PARTITIONS_PARAMETERS_EXCLUDE_PATTERN, "%exclu%");
    }
    ...
    protected void assertPartitionsHaveCorrectParams(List<Partition> partitions)
    {
        for (int i = 0; i < partitions.size(); i++)
        {
            ...
            assertTrue("included parameter key is not found in the partition", parameters.keySet().containsAll(includeKeys));
            assertFalse("excluded parameter key is found in the partition", parameters.keySet().stream().anyMatch(key -> excludeKeys.contains(key)));
    ...
    public void testGetPartitionsByNames() throws Exception
    {
        ...
        GetPartitionsByNamesRequest request = convertToGetPartitionsByNamesRequest(MetaStoreUtils.prependCatalogToDbName(t.getCatName(), t.getDbName(), hiveConf), t.getTableName(), part_names);
        ...
        request = convertToGetPartitionsByNamesRequest(MetaStoreUtils.prependCatalogToDbName(t.getCatName(), t.getDbName(), hiveConf), t.getTableName(), part_names);
    ...
    public void testGetPartitionsRequest() throws Exception
    {
        ...
        assertEquals(Arrays.asList(Arrays.asList("2017", "10", "26"), Arrays.asList("2017", "11", "27")), returnTable.testValues.subList(2, 4));
    }
    ...
    private void checkExpr(int numParts, ExprNodeGenericFuncDesc expr) throws Exception
    {
        ...
        getClient().listPartitionsByExpr(DB_NAME, TABLE_NAME, exprBytes, null, (short) -1, partitions);
        assertEquals("Partition check failed: " + expr.getExprString(), numParts, partitions.size());
        assertPartitionsHaveCorrectParams(partitions);

        PartitionsByExprRequest req = new PartitionsByExprRequest(DB_NAME, TABLE_NAME, ByteBuffer.wrap(exprBytes));
        ...
        {
            assertTrue(partitionSpec.getPartitionList() == null || partitionSpec.getPartitionList().getPartitions() == null || partitionSpec.getPartitionList().getPartitions().isEmpty());
