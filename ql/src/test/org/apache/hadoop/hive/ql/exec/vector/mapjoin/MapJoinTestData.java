...
public class MapJoinTestData
{
    ...
    public MapJoinTestData(int rowCount, MapJoinTestDescription testDesc, long randomSeed) throws HiveException
    {
        ...
        generationSpecList = generationSpecListFromTypeInfos(testDesc.bigTableTypeInfos, testDesc.bigTableKeyColumnNums.length, isOuterJoin);

        bigTableRowSource = new VectorRandomRowSource();

        bigTableRowSource.initGenerationSpecSchema(random, generationSpecList, /* maxComplexDepth */ 0,
                /* allowNull */ true, /* isUnicodeOk */ true, null);

        // UNDONE: 100000
        bigTableRandomRows = bigTableRowSource.randomRows(10);

        bigTableBatchSource = VectorRandomBatchSource.createInterestingBatches(random, bigTableRowSource, bigTableRandomRows, null);
        ...
        final int smallTableRetainKeySize = (isOuterJoin ? testDesc.smallTableRetainKeyColumnNums.length : 0);
        ColumnVector[] newCols = new ColumnVector[bigTableBatch.cols.length + smallTableRetainKeySize + testDesc.smallTableValueTypeInfos.length];
        ...
        if (isOuterJoin)
        {
            for (int s = 0; s < smallTableRetainKeySize; s++)
            {
                ...
                newCols[colIndex++] = VectorizedBatchUtil.createColumnVector(testDesc.smallTableKeyTypeInfos[smallTableKeyColumnNum]);
            }
        }
        for (int s = 0; s < testDesc.smallTableValueTypeInfos.length; s++)
        {
            newCols[colIndex++] = VectorizedBatchUtil.createColumnVector(testDesc.smallTableValueTypeInfos[s]);
        }
        ...
        if (valueOption != ValueOption.NO_REGULAR_SMALL_KEYS)
        {
            ...
            while (bigTableBatchSource.fillNextBatch(bigTableBatch))
            {
                ...
                {
                    final int batchIndex = (bigTableBatch.selectedInUse ? bigTableBatch.selected[logical] : logical);

                    RowTestObjects testKey = getTestKey(bigTableBatch, batchIndex, keyVectorExtractRow, testDesc.bigTableKeyTypeInfos.length, testDesc.bigTableObjectInspectors);
        ...
        altBigTableRowSource.initGenerationSpecSchema(random, generationSpecList, /* maxComplexDepth */ 0,
                /* allowNull */ true, /* isUnicodeOk */ true, null);

        Object[][] altBigTableRandomRows = altBigTableRowSource.randomRows(10000);

        VectorRandomBatchSource altBigTableBatchSource = VectorRandomBatchSource.createInterestingBatches(random, altBigTableRowSource, altBigTableRandomRows, null);
        ...
        while (altBigTableBatchSource.fillNextBatch(bigTableBatch))
        {
            ...
            for (int logical = 0; logical < size; logical++)
            {
                final int batchIndex = (bigTableBatch.selectedInUse ? bigTableBatch.selected[logical] : logical);
                RowTestObjects testKey = getTestKey(bigTableBatch, batchIndex, keyVectorExtractRow, testDesc.bigTableKeyTypeInfos.length, testDesc.bigTableObjectInspectors);
                if (bigTableKeyHashMap.containsKey(testKey) || smallTableKeyHashMap.containsKey(testKey))
                {
                    continue;
                }
    ...
    public VectorizedRowBatch createBigTableBatch(MapJoinTestDescription testDesc)
    {
        ...
        {
            batch.cols[i] = VectorizedBatchUtil.createColumnVector(testDesc.bigTableTypeInfos[i]);
        }
        return batch;
    }
    ...
    private static List<GenerationSpec> generationSpecListFromTypeInfos(TypeInfo[] typeInfos, int keyCount, boolean isOuterJoin)
    {
