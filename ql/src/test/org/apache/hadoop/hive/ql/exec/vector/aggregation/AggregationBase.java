...
public class AggregationBase
{
    ...
    */
        ...
        VectorizedRowBatchCtx outputBatchContext = new VectorizedRowBatchCtx(outputColumnNames, outputTypeInfos, new DataTypePhysicalVariation[]{vecAggrExpr.getOutputDataTypePhysicalVariation()},
                /* dataColumnNums */ null,
                /* partitionColumnCount */ 0,
                /* virtualColumnCount */ 0,
                /* neededVirtualColumns */ null, new String[0], new DataTypePhysicalVariation[0]);
    ...
    protected void executeAggregationTests(String aggregationName, TypeInfo typeInfo, GenericUDAFEvaluator evaluator, TypeInfo outputTypeInfo, GenericUDAFEvaluator.Mode udafEvaluatorMode, int maxKeyCount, List<String> columns, String[] columnNames, List<ExprNodeDesc> parameters, Object[][] randomRows, VectorRandomRowSource rowSource, VectorRandomBatchSource batchSource, boolean tryDecimal64, Object[] resultsArray) throws Exception
    {
        ...
        for (int i = 0; i < AggregationTestMode.count; i++)
        {
            ...
            {
                case ROW_MODE:
                    if (!doRowTest(typeInfo, evaluator, outputTypeInfo, udafEvaluatorMode, maxKeyCount, columns, parameters, randomRows, rowSource.rowStructObjectInspector(), results))
                    {
                        return;
                    }
                    break;
                case VECTOR_EXPRESSION:
                    if (!doVectorTest(aggregationName, typeInfo, evaluator, outputTypeInfo, udafEvaluatorMode, maxKeyCount, columns, columnNames, rowSource.typeInfos(), rowSource.dataTypePhysicalVariations(), parameters, batchSource, results))
                    {
                        return;
                    }
                    break;
