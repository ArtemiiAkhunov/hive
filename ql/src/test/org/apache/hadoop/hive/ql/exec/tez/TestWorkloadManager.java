...
public class TestWorkloadManager
{
    ...
    public static class WorkloadManagerForTest extends WorkloadManager
    {
        ...
        {
            WMFullResourcePlan plan = new WMFullResourcePlan(new WMResourcePlan("rp"), Lists.newArrayList(pool("llap", numSessions, 1.0f)));
    ...
    private static void verifyMapping(WorkloadManager wm, HiveConf conf, MappingInput mi, String result) throws Exception
    {
    ...
    public void testReuseWithQueueing() throws Exception
    {
        ...
        wm.start();
        WmTezSession session1 = (WmTezSession) wm.getSession(null, mappingInput("user"), conf);
        // First, try to reuse from the same pool - should "just work".
        WmTezSession session1a = (WmTezSession) wm.getSession(session1, mappingInput("user"), conf);
        ...
        WmTezSession session2a = (WmTezSession) wm.getSession(session2, mappingInput("user"), conf);
        ...
        Thread t1 = new Thread(new GetSessionRunnable(session3, wm, error, conf, cdl, null), "t1"), t2 = new Thread(new GetSessionRunnable(session4, wm, error, conf, cdl2, null), "t2");
        waitForThreadToBlock(cdl, t1);
    ...
    public void testApplyPlanUserMapping() throws Exception
    {
        final HiveConf conf = createConf();
        MockQam qam = new MockQam();
        WMFullResourcePlan plan = new WMFullResourcePlan(plan(), Lists.newArrayList(pool("A", 1, 0.5f), pool("B", 1, 0.5f)));
        ...
        plan = new WMFullResourcePlan(plan(), Lists.newArrayList(pool("A", 1, 0.6f), pool("B", 1, 0.4f)));
    ...
    public void testAmPoolInteractions() throws Exception
    {
        ...
        plan = new WMFullResourcePlan(plan(), Lists.newArrayList(pool("A", 4, 1.0f)));
        plan.setMappings(Lists.newArrayList(mapping("A", "A")));
        wm.updateResourcePlanAsync(plan);
        WmTezSession oob2 = pool.getSession(), oob3 = pool.getSession(), oob4 = pool.getSession();
