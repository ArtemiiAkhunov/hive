...
public class TestBoundaryCache
{
    ...
    private static final List<LinkedList<List<IntWritable>>> TEST_PARTITIONS = Lists.newArrayList(TEST_PARTITION, TEST_PARTITION_NO_NULLS);
        ...
        for (LinkedList<List<IntWritable>> partition : TEST_PARTITIONS)
        {
            ...
            for (PTFInvocationSpec.Order order : ORDERS)
            {
                for (Integer cacheSize : CACHE_SIZES)
                {
                    ...
                    Pair<PTFPartition, ValueBoundaryScanner> mocks = setupMocks(partition, ORDER_BY_COL, startBoundary, endBoundary, order, cache, readCounter);
                    ...
                    for (int i = 0; i < partition.size(); ++i)
                    {
                        ...
                        assertEquals("start ranges are not equal with/without binary pre-search", startWithoutBinaryPreSearch, start);
                        assertEquals("end ranges are not equal with/without binary pre-search", endWithoutBinaryPreSearch, end);
                        ...
                        {
                            assertEquals(String.format("expected boundary start doesn't match, order: %s, cache size: %d, i: %d", order, cacheSize, i), expectedBoundaryStarts[i], start);
                            assertEquals(String.format("expected boundary end doesn't match, order: %s, cache size: %d, i: %d", order, cacheSize, i), expectedBoundaryEnds[i], end);
                        }
                    }
                    if (cache == null)
                    {
     ...
     * @throws Exception
     */
    private static Pair<PTFPartition, ValueBoundaryScanner> setupMocks(LinkedList<List<IntWritable>> partition, int orderByCol, BoundaryDef start, BoundaryDef end, PTFInvocationSpec.Order order, BoundaryCache cache, AtomicInteger readCounter) throws Exception
    {
    ...
    @SuppressWarnings("unchecked")
    private static void addRow(Integer col0, Integer col1, Integer col2, List<List<IntWritable>>... partitions)
    {
        for (List<List<IntWritable>> partition : partitions)
        {
            partition.add(Lists.newArrayList(col0 != null ? new IntWritable(col0) : null, col1 != null ? new IntWritable(col1) : null, col2 != null ? new IntWritable(col2) : null));
        }
     ...
     * @param order
     */
    private static void setOrderOnTestPartitions(LinkedList<List<IntWritable>> partition, PTFInvocationSpec.Order order)
    {
        LinkedList<List<IntWritable>> notNulls = partition.stream().filter(r -> r.get(ORDER_BY_COL) != null).collect(toCollection(LinkedList::new));
        List<List<IntWritable>> nulls = partition.stream().filter(r -> r.get(ORDER_BY_COL) == null).collect(toList());

        boolean isAscCurrently = notNulls.getFirst().get(ORDER_BY_COL).get() < notNulls.getLast().get(ORDER_BY_COL).get();
