...
public class TestVectorSerDeRow
{
    ...
    {
        NONE, BINARY_SORTABLE, LAZY_BINARY, LAZY_SIMPLE
    }
    ...
    private void verifyRead(DeserializeRead deserializeRead, TypeInfo typeInfo, Object expectedObject) throws IOException
    {
        ...
        } else
        {
            ...
            if (expectedObject == null)
            {
                ...
                {
                    fail("Field reports not null but object is null (class " + complexFieldObj.getClass().getName() + ", " + complexFieldObj.toString() + ")");
                }
            } else
            {
                if (complexFieldObj == null)
                {
                    ...
                    fail("Field reports null but object is not null (class " + expectedObject.getClass().getName() + ", " + expectedObject.toString() + ")");
                }
            }
    ...
    void innerTestVectorSerializeRow(Random r, SerializationType serializationType) throws HiveException, IOException, SerDeException
    {
        ...
        switch (serializationType)
        {
            ...
            case LAZY_SIMPLE:
            {
                ...
                deserializeRead = new LazySimpleDeserializeRead(source.typeInfos(),
                        /* useExternalBuffer */ false, lazySerDeParams);
    ...
    void examineBatch(VectorizedRowBatch batch, VectorExtractRow vectorExtractRow, TypeInfo[] typeInfos, Object[][] randomRows, int firstRandomRowIndex, String title)
    {
        ...
        for (int i = 0; i < batch.size; i++)
        {
            ...
            for (int c = 0; c < rowSize; c++)
            {
                ...
                if (rowObj == null)
                {
                    ...
                    fail("Unexpected NULL from extractRow.  Expected class " + typeInfos[c].getCategory() + " value " + expectedObj + " batch index " + i + " firstRandomRowIndex " + firstRandomRowIndex);
                }
                if (!rowObj.equals(expectedObj))
                {
                    ...
                    fail("Row " + (firstRandomRowIndex + i) + " and column " + c + " mismatch (" + typeInfos[c].getCategory() + " actual value '" + actualValueString + "'" + " and expected value '" + expectedValueString + "')" + " difference info " + differentInfoString + " typeInfos " + Arrays.toString(typeInfos) + " title " + title);
                }
            }
        }
    }
    ...
    void testVectorDeserializeRow(Random r, SerializationType serializationType, boolean testDecimal64, boolean alternate1, boolean alternate2, boolean useExternalBuffer) throws HiveException, IOException, SerDeException
    {

        for (int i = 0; i < 20; i++)
        {
            innerTestVectorDeserializeRow(r, i, serializationType, testDecimal64, alternate1, alternate2, useExternalBuffer);
        }
    }

    void innerTestVectorDeserializeRow(Random r, int iteration, SerializationType serializationType, boolean testDecimal64, boolean alternate1, boolean alternate2, boolean useExternalBuffer) throws HiveException, IOException, SerDeException
    {
    ...
    {
        Random r = new Random(8732);
        testVectorDeserializeRow(r, SerializationType.BINARY_SORTABLE,
                /* testDecimal64 */ false,
    ...
    {
        Random r = new Random(8732);
        testVectorDeserializeRow(r, SerializationType.LAZY_BINARY,
                /* testDecimal64 */ false,
    ...
    {
        Random r = new Random(8732);
        testVectorDeserializeRow(r, SerializationType.LAZY_SIMPLE,
                /* testDecimal64 */ false,
