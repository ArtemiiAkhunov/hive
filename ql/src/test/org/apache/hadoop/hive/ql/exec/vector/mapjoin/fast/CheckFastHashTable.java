...
public class CheckFastHashTable
{
    ...
    public static class VerifyFastLongHashMap
    {
        ...
        // And, keys are *UNIQUE*.
        public void verifyNonMatched(VectorMapJoinFastLongHashMap map, Random random) throws HiveException
        {
            int mapSize = map.size();
            if (mapSize != count)
            {
            ...
            VectorMapJoinNonMatchedIterator nonMatchedIterator = map.createNonMatchedIterator(matchTracker);
    ...
    public static class VerifyFastBytesHashMap
    {
        ...
        private void verifyOne(VectorMapJoinFastBytesHashMap map, int index, MatchTracker matchTracker)
        {
        ...
        private int findKeyInArray(byte[] searchKeyBytes, int searchKeyOffset, int searchKeyLength)
        {
            ...
            for (int index = 0; index < count; index++)
            {
                ...
                if (keyBytes.length == searchKeyLength && StringExpr.equal(keyBytes, 0, keyBytes.length, searchKeyBytes, searchKeyOffset, searchKeyLength))
                {
                    return index;
                }
            }
        ...
        // We assume there have been no reads/lookups before this call.
        // And, keys are *UNIQUE*.
        public void verifyNonMatched(VectorMapJoinFastBytesHashMap map, Random random) throws HiveException
        {
            int mapSize = map.size();
            if (mapSize != count)
            {
