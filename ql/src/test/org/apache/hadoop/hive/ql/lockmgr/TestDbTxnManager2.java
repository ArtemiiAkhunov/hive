...
public class TestDbTxnManager2 extends DbTxnManagerEndToEndTestBase
{
    ...
    private void testLocksInSubquery(boolean sharedWrite) throws Exception
    {
        ...
        Assert.assertEquals("Unexpected lock count", 2, locks.size());
        checkLock(LockType.SHARED_READ, LockState.ACQUIRED, "default", "T", null, locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "S", null, locks);
        txnMgr.rollbackTxn();
        ...
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "R", null, locks);
        txnMgr.rollbackTxn();
    }
    ...
    private void testInsertOverwriteCreate(boolean isTransactional, boolean sharedWrite) throws Exception
    {
        ...
        checkLock((isTransactional && sharedWrite) ? LockType.EXCL_WRITE : LockType.EXCLUSIVE, LockState.ACQUIRED, "default", "T3", null, locks);
        txnMgr.commitTxn();
        Assert.assertEquals("Lock remained", 0, getLocks().size());
    ...
    private void testInsertOverwritePartitionedCreate(boolean isTransactional, boolean sharedWrite) throws Exception
    {
        ...
        checkLock((isTransactional && sharedWrite) ? LockType.EXCL_WRITE : LockType.EXCLUSIVE, LockState.ACQUIRED, "default", "T4", null, locks);
        txnMgr.commitTxn();
        Assert.assertEquals("Lock remained", 0, getLocks().size());
    ...
    private void testUpdateSelectUpdate(boolean sharedWrite) throws Exception
    {
        ...
        List<ShowLocksResponseElement> locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 3, locks.size());
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "T8", null, locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "T8", null, locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "T8", null, locks);
    ...
    public void testMetastoreTablesCleanup() throws Exception
    {
        ...
        int count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " + "where \"CTC_DATABASE\"='temp' and \"CTC_TABLE\" in ('t10', 't11')");
        Assert.assertEquals(4, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " + "where \"CTC_DATABASE\"='temp' and \"CTC_TABLE\" in ('t12p', 't13p')");
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" " + "where \"TC_DATABASE\"='temp' and \"TC_TABLE\" in ('t10', 't11', 't12p', 't13p')");
        Assert.assertEquals(4, count);
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" " + "where \"TC_DATABASE\"='temp' and \"TC_TABLE\"='t10'");
        Assert.assertEquals(1, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " + "where \"CTC_DATABASE\"='temp' and \"CTC_TABLE\"='t10'");
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" " + "where \"TC_DATABASE\"='temp' and \"TC_TABLE\"='t12p' and \"TC_PARTITION\"='ds=today/hour=1'");
        Assert.assertEquals(1, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " + "where \"CTC_DATABASE\"='temp' and \"CTC_TABLE\"='t12p' and \"CTC_PARTITION\"='ds=today/hour=1'");
        Assert.assertEquals(1, count);
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t11' and \"CQ_STATE\"='i' and \"CQ_TYPE\"='i'");
        Assert.assertEquals(1, count);
        runWorker(conf);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t11' and \"CQ_STATE\"='r' and \"CQ_TYPE\"='i'");
        Assert.assertEquals(1, count);
        runCleaner(conf);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t11'");
        Assert.assertEquals(0, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\"='t11' and \"CC_STATE\"='s' and \"CC_TYPE\"='i'");
        Assert.assertEquals(1, count);

        driver.run("alter table temp.T12p partition (ds='tomorrow', hour='2') compact 'minor'");
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t12p' and \"CQ_PARTITION\"='ds=tomorrow/hour=2' " + "and \"CQ_STATE\"='i' and \"CQ_TYPE\"='i'");
        Assert.assertEquals(1, count);
        runWorker(conf);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t12p' and \"CQ_PARTITION\"='ds=tomorrow/hour=2' " + "and \"CQ_STATE\"='r' and \"CQ_TYPE\"='i'");
        Assert.assertEquals(1, count);
        runCleaner(conf);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t12p'");
        Assert.assertEquals(0, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\"='t12p' and \"CC_STATE\"='s' and \"CC_TYPE\"='i'");
        Assert.assertEquals(1, count);
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t11' and \"CQ_STATE\"='i' and \"CQ_TYPE\"='a'");
        Assert.assertEquals(1, count);
        runWorker(conf); // will fail
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t11' and \"CQ_STATE\"='i' and \"CQ_TYPE\"='a'");
        Assert.assertEquals(0, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\"='t11' and \"CC_STATE\"='f' and \"CC_TYPE\"='a'");
        Assert.assertEquals(1, count);

        driver.run("alter table temp.T12p partition (ds='tomorrow', hour='2') compact 'major'");
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t12p' and \"CQ_PARTITION\"='ds=tomorrow/hour=2' " + "and \"CQ_STATE\"='i' and \"CQ_TYPE\"='a'");
        Assert.assertEquals(1, count);
        runWorker(conf); // will fail
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t12p' and \"CQ_PARTITION\"='ds=tomorrow/hour=2' " + "and \"CQ_STATE\"='i' and \"CQ_TYPE\"='a'");
        Assert.assertEquals(0, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\"='t12p' and \"CC_STATE\"='f' and \"CC_TYPE\"='a'");
        Assert.assertEquals(1, count);
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t11'");
        Assert.assertEquals(0, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\"='t11'");
        Assert.assertEquals(0, count);

        driver.run("alter table temp.T12p drop partition (ds='tomorrow', hour='2')");
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t12p'");
        Assert.assertEquals(0, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\"='t12p'");
        Assert.assertEquals(0, count);
        ...
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\"='t13p' and \"CQ_STATE\"='i' and \"CQ_TYPE\"='a'");
        Assert.assertEquals(1, count);

        // Drop database, everything in all 4 meta tables should disappear
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" " + "where \"TC_DATABASE\"='temp' and \"TC_TABLE\" in ('t10', 't11', 't12p', 't13p')");
        Assert.assertEquals(1, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " + "where \"CTC_DATABASE\"='temp' and \"CTC_TABLE\" in ('t10', 't11', 't12p', 't13p')");
        Assert.assertEquals(2, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPACTION_QUEUE\" " + "where \"CQ_DATABASE\"='temp' and \"CQ_TABLE\" in ('t10', 't11', 't12p', 't13p')");
        Assert.assertEquals(1, count);
        count = TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_COMPACTIONS\" " + "where \"CC_DATABASE\"='temp' and \"CC_TABLE\" in ('t10', 't11', 't12p', 't13p')");
        Assert.assertEquals(0, count);
    ...
    private void testCheckExpectedLocks(boolean sharedWrite) throws Exception
    {
        ...
        locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 1, locks.size());
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "acidPart", null, locks);
        txnMgr.rollbackTxn();
        ...
        locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 1, locks.size());
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "acidPart", "p=1", locks);
        txnMgr.rollbackTxn();
        ...
        locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 1, locks.size());
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "acidPart", null, locks);
        txnMgr.rollbackTxn();
    ...
    public void testCheckExpectedLocks2() throws Exception
    {
        ...
        driver.run("create table if not exists tab_acid (a int, b int) partitioned by (p string) " + "clustered by (a) into 2  buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        driver.run("create table if not exists tab_not_acid (na int, nb int) partitioned by (np string) " + "clustered by (na) into 2  buckets stored as orc TBLPROPERTIES ('transactional'='false')");
    ...
    public void testLockingOnInsertIntoNonNativeTables() throws Exception
    {
        ...
        driver.run("create table if not exists tab_not_acid (a int, b int) " + " STORED BY 'org.apache.hadoop.hive.ql.metadata.StorageHandlerMock'");
        txnMgr.openTxn(ctx, "T1");
    ...
    {
        dropTable(new String[]{"tab_not_acid"});
        driver.run("create external table if not exists tab_not_acid (na int, nb int) partitioned by (np string) " + "clustered by (na) into 2  buckets stored as orc TBLPROPERTIES ('transactional'='false')");
        driver.run("insert into tab_not_acid partition(np) (na,nb,np) values(1,2,'blah'),(3,4,'doh')");
    ...
    {
        dropTable(new String[]{"tab_not_acid"});
        driver.run("create external table if not exists tab_not_acid (na int, nb int) " + "stored as orc TBLPROPERTIES ('transactional'='false')");
    ...
    private static ShowLocksResponseElement checkLock(LockType expectedType, LockState expectedState, String expectedDb, String expectedTable, String expectedPartition, List<ShowLocksResponseElement> actuals, boolean skipFirst)
    {
        ...
        {
            if (expectedType == actual.getType() && expectedState == actual.getState() && StringUtils.equals(normalizeCase(expectedDb), normalizeCase(actual.getDbname())) && StringUtils.equals(normalizeCase(expectedTable), normalizeCase(actual.getTablename())) && StringUtils.equals(normalizeCase(expectedPartition), normalizeCase(actual.getPartname())))
            {
                if (!skip)
                {
                    return actual;
        ...
        Assert.fail("Could't find {" + expectedType + ", " + expectedState + ", " + expectedDb + ", " + expectedTable + ", " + expectedPartition + "} in " + actuals);
    ...
    public void testWriteSetTracking5() throws Exception
    {
        ...
        AddDynamicPartitions adp = new AddDynamicPartitions(txnId, writeIds.getTxnToWriteIds().get(0).getWriteId(), "default", "TAB_PART", Collections.singletonList("p=blah"));
        adp.setOperationType(DataOperationType.UPDATE);
        txnHandler.addDynamicPartitions(adp);
        Assert.assertEquals(0, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\""));
    ...
    public void testWriteSetTracking7() throws Exception
    {
        ...
        adp = new AddDynamicPartitions(txnMgr.getCurrentTxnId(), writeId, "default", "tab2", Collections.singletonList("p=one"));
        adp.setOperationType(DataOperationType.UPDATE);
        txnHandler.addDynamicPartitions(adp);
        txnMgr.commitTxn(); //txnid:idTxnUpdate2
        //now both txns concurrently updated TAB2 but different partitions.

        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=one' and \"WS_OPERATION_TYPE\"='u'"));
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=two' and \"WS_OPERATION_TYPE\"='u'"));
        //2 from txnid:1, 1 from txnid:2, 1 from txnid:3
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TABLE\"='tab2' and \"CTC_PARTITION\" is not null"));

        //================
        //test with predicates such that partition pruning doesn't kick in
        driver.run("drop table if exists tab1");
        driver.run("create table if not exists tab1 (a int, b int) partitioned by (p string) " + "clustered by (a) into 2  buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        ...
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=one' and \"WS_OPERATION_TYPE\"='u' and \"WS_TABLE\"='tab1'"));
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=two' and \"WS_OPERATION_TYPE\"='u' and \"WS_TABLE\"='tab1'"));
        //2 from insert + 1 for each update stmt
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TABLE\"='tab1' and \"CTC_PARTITION\" is not null"));
    }
    ...
    public void testWriteSetTracking9() throws Exception
    {
        ...
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + (idTxnUpdate1 - 1) + " and \"CTC_TABLE\"='tab1'"));
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + idTxnUpdate1 + " and \"CTC_TABLE\"='tab1' and \"CTC_PARTITION\"='p=one'"));
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + idTxnDelete1 + " and \"CTC_TABLE\"='tab1' and \"CTC_PARTITION\"='p=two'"));
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=one' and \"WS_OPERATION_TYPE\"='u' and \"WS_TABLE\"='tab1'"));
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=two' and \"WS_OPERATION_TYPE\"='d' and \"WS_TABLE\"='tab1'"));
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TABLE\"='tab1' and \"CTC_PARTITION\" is not null"));
    }
    ...
    public void testWriteSetTracking10() throws Exception
    {
        ...
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=two' and \"WS_OPERATION_TYPE\"='u' and \"WS_TABLE\"='tab1'"));
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 3, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TABLE\"='tab1' and \"CTC_PARTITION\" is not null"));
    }
    ...
    public void testWriteSetTracking11() throws Exception
    {
        ...
        Assert.assertEquals("Transaction manager has aborted the transaction txnid:5.  " + "Reason: Aborting [txnid:5,5] due to a write conflict on default/tab1/p=two " + "committed by [txnid:4,5] d/d", expectedException.getMessage());
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=two' and \"WS_OPERATION_TYPE\"='d' " + "and \"WS_TABLE\"='tab1' and \"WS_TXNID\"=" + txnIdDelete));
        Assert.assertEquals("WRITE_SET mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 0, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_PARTITION\"='p=two' and \"WS_OPERATION_TYPE\"='d' " + "and \"WS_TABLE\"='tab1' and \"WS_TXNID\"=" + txnIdSelect));
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch: " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 3, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TABLE\"='tab1' and \"CTC_PARTITION\" is not null"));
    }
    ...
    public void testCompletedTxnComponents() throws Exception
    {
        ...
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\""));
        //only expect transactional components to be in COMPLETED_TXN_COMPONENTS
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=6 and \"CTC_TABLE\"='tab1'"));
    }
    ...
    public void testMultiInsert() throws Exception
    {
        ...
        //only expect transactional components to be in COMPLETED_TXN_COMPONENTS
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=9"));
        Assert.assertEquals(TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=9 and \"CTC_TABLE\"='tab1'"));
    }
    ...
    public void testMultiInsertOnDynamicallyPartitionedMmTable() throws Exception
    {
        ...
        driver.run("create table if not exists tabMmDp (a int, b int) partitioned by (p string) " + "stored as orc " + "TBLPROPERTIES ('transactional'='true', 'transactional_properties'='insert_only')");
        driver.run("create table if not exists tab_not_acid (a int, b int, p string)");
        driver.run("insert into tab_not_acid values (1 ,1, 'one'), (2, 2, 'two')");
        // insert 2 rows twice into the MM table
        driver.run("from tab_not_acid " + "insert into tabMmDp select a,b,p " + "insert into tabMmDp select a,b,p"); //txnid: 6 (2 drops, 2 creates, 2 inserts)

        final String completedTxnComponentsContents = TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\"");
        Assert.assertEquals(completedTxnComponentsContents, 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\""));
        Assert.assertEquals(completedTxnComponentsContents, 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=6"));
        Assert.assertEquals(completedTxnComponentsContents, 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=6 " + "and \"CTC_TABLE\"='tabmmdp'"));
        // ctc_update_delete value should be "N" for both partitions since these are inserts
        Assert.assertEquals(completedTxnComponentsContents, 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=6 " + "and \"CTC_TABLE\"='tabmmdp' and \"CTC_UPDATE_DELETE\"='N'"));
        dropTable(new String[]{"tabMmDp", "tab_not_acid"});
    }
    ...
    private void testMerge3Way(boolean causeConflict, boolean sharedWrite) throws Exception
    {
        ...
        driver.run("create table target (a int, b int) " + "partitioned by (p int, q int) clustered by (a) into 2  buckets " + "stored as orc TBLPROPERTIES ('transactional'='true')");
        ...
        driver.compileAndRespond("merge into target t using source s on t.a=s.b " + "when matched and t.a=5 then update set b=s.b " + //updates p=1/q=3
        ...
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "target", null, locks);
        checkLock(LockType.SHARED_READ, LockState.ACQUIRED, "default", "source", null, locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "target", "p=1/q=2", locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "target", "p=1/q=3", locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "target", "p=2/q=2", locks);
        ...
        long extLockId = checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "target", null, locks, sharedWrite).getLockid();
        checkLock(LockType.SHARED_READ, (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "source2", null, locks);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "target", "p=1/q=2", locks, sharedWrite);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "target", "p=1/q=3", locks, sharedWrite);
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "target", "p=2/q=2", locks, sharedWrite);

        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId1));

        //complete 1st txn
        long writeId = txnMgr.getTableWriteId("default", "target");
        AddDynamicPartitions adp = new AddDynamicPartitions(txnId1, writeId, "default", "target", Collections.singletonList("p=1/q=3")); //update clause
        adp.setOperationType(DataOperationType.UPDATE);
        txnHandler.addDynamicPartitions(adp);
        adp = new AddDynamicPartitions(txnId1, writeId, "default", "target", Arrays.asList("p=1/q=2", "p=2/q=2")); //delete clause
        adp.setOperationType(DataOperationType.DELETE);
        txnHandler.addDynamicPartitions(adp);
        adp = new AddDynamicPartitions(txnId1, writeId, "default", "target", Arrays.asList("p=1/q=2", "p=1/q=3", "p=1/q=1")); //insert clause
        adp.setOperationType(DataOperationType.INSERT);
        txnHandler.addDynamicPartitions(adp);
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId1 + " and \"TC_OPERATION_TYPE\"='u'"));
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId1 + " and \"TC_OPERATION_TYPE\"='d'"));
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 3, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId1 + " and \"TC_OPERATION_TYPE\"='i'"));

        txnMgr.commitTxn(); //commit T1
        Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 6, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + txnId1));
        Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnId1 + " and \"WS_OPERATION_TYPE\"='u'"));
        Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnId1 + " and \"WS_OPERATION_TYPE\"='d'"));
        ...
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId2));

        //complete 2nd txn
        writeId = txnMgr2.getTableWriteId("default", "target");
        adp = new AddDynamicPartitions(txnId2, writeId, "default", "target", Collections.singletonList(causeConflict ? "p=1/q=3" : "p=1/p=2")); //update clause
        adp.setOperationType(DataOperationType.UPDATE);
        txnHandler.addDynamicPartitions(adp);

        if (causeConflict)
        {
            adp = new AddDynamicPartitions(txnId2, writeId, "default", "target", Arrays.asList("p=1/q=2", "p=2/q=2")); //delete clause
            adp.setOperationType(DataOperationType.DELETE);
            txnHandler.addDynamicPartitions(adp);
        }
        adp = new AddDynamicPartitions(txnId2, writeId, "default", "target", Arrays.asList("p=1/q=2", "p=1/q=3", "p=1/q=1")); //insert clause
        adp.setOperationType(DataOperationType.INSERT);
        txnHandler.addDynamicPartitions(adp);
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId2 + " and \"TC_OPERATION_TYPE\"='u'"));
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), (causeConflict ? 2 : 0), TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId2 + " and \"TC_OPERATION_TYPE\"='d'"));
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 3, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId2 + " and \"TC_OPERATION_TYPE\"='i'"));
        ...
        if (causeConflict && sharedWrite)
        {
            ...
            try
            {
                Assert.assertEquals("Transaction manager has aborted the transaction txnid:11.  Reason: " + "Aborting [txnid:11,11] due to a write conflict on default/target/p=1/q=2 " + "committed by [txnid:10,11] d/d", expectedException.getMessage());
            } catch (ComparisonFailure ex)
            {
                ...
                try
                {
                    Assert.assertEquals("Transaction manager has aborted the transaction txnid:11.  Reason: " + "Aborting [txnid:11,11] due to a write conflict on default/target/p=2/q=2 " + "committed by [txnid:10,11] d/d", expectedException.getMessage());
                } catch (ComparisonFailure ex2)
                {
                    Assert.assertEquals("Transaction manager has aborted the transaction txnid:11.  Reason: " + "Aborting [txnid:11,11] due to a write conflict on default/target/p=1/q=3 " + "committed by [txnid:10,11] u/u", expectedException.getMessage());
                }
            }
            Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 0, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + txnId2));
            Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 0, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnId2));
        } else
        {
            Assert.assertNull("Unexpected exception " + expectedException, expectedException);
            Assert.assertEquals("COMPLETED_TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), causeConflict ? 6 : 4, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + txnId2));
            Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnId2 + " and \"WS_OPERATION_TYPE\"='u'"));
            Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnId2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), causeConflict ? 2 : 0, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnId2 + " and \"WS_OPERATION_TYPE\"='d'"));
        }
        dropTable(new String[]{"target", "source", "source2"});
    }
    ...
    private void testMergeUnpartitioned(boolean causeConflict, boolean sharedWrite) throws Exception
    {
        ...
        driver.run("create table target (a int, b int) " + "clustered by (a) into 2  buckets " + "stored as orc TBLPROPERTIES ('transactional'='true')");
        ...
        long txnid1 = txnMgr.getCurrentTxnId();
        txnMgr.acquireLocks(driver.getPlan(), ctx, "T1");
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, //no DP, so it's populated from lock info
        ...
        {
            Assert.assertEquals("Unexpected lock count", 1, locks.size());
            checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "target", null, locks);
        } else
        {
            Assert.assertEquals("Unexpected lock count", 1, locks.size());
            checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "target", null, locks);
        ...
        driver.compileAndRespond("merge into target t using source s " + "on t.a=s.a " + "when matched then delete " + "when not matched then insert values(s.a,s.b)", true);
        long txnid2 = txnMgr2.getCurrentTxnId();
        ...
        Assert.assertEquals("Unexpected lock count", 3, locks.size());
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), LockState.ACQUIRED, "default", "target", null, locks);
        checkLock(LockType.SHARED_READ, (causeConflict && !sharedWrite) ? LockState.WAITING : LockState.ACQUIRED, "default", "source", null, locks);
        long extLockId = checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), (causeConflict && !sharedWrite) ? LockState.WAITING : LockState.ACQUIRED, "default", "target", null, locks, sharedWrite).getLockid();
        txnMgr.commitTxn(); //commit T1

        Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnid1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), causeConflict ? 1 : 0, //Inserts are not tracked by WRITE_SET
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnid1 + " and \"WS_OPERATION_TYPE\"=" + (causeConflict ? "'d'" : "'i'")));
        ...
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, //
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnid2));
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, //
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnid2 + " and \"TC_OPERATION_TYPE\"='d'"));
        ...
        if (causeConflict && sharedWrite)
        {
            Assert.assertNotNull("Didn't get exception", expectedException);
            Assert.assertEquals("Got wrong message code", ErrorMsg.TXN_ABORTED, expectedException.getCanonicalErrorMsg());
            Assert.assertEquals("Exception msg didn't match", "Aborting [txnid:7,7] due to a write conflict on default/target committed by [txnid:6,7] d/d", expectedException.getCause().getMessage());
        } else
        {
            Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnid1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, //Unpartitioned table: 1 row for Delete; Inserts are not tracked in WRITE_SET
                    TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnid2 + " and \"WS_OPERATION_TYPE\"='d'"));
        }
    }
    ...
    private void testMergeInsertLocking(boolean sharedWrite) throws Exception
    {
        ...
        driver.compileAndRespond("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b)");
    ...
    {
        testConcurrentMergeInsert("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b)", false, false, false);
    }

    @Test
    public void testConcurrent2MergeInsertsSharedWriteNoDuplicates() throws Exception
    {
        testConcurrentMergeInsert("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b)", true, false, false);
    }

    @Test
    public void testConcurrent2MergeInsertsNoDuplicatesSlowCompile() throws Exception
    {
        testConcurrentMergeInsert("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b)", false, true, false);
    }

    @Test
    public void testConcurrent2MergeInsertsSharedWriteNoDuplicatesSlowCompile() throws Exception
    {
        testConcurrentMergeInsert("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b)", true, true, false);
    }
    ...
    private void testConcurrentMergeInsert(String query, boolean sharedWrite, boolean slowCompile, boolean extectedDuplicates) throws Exception
    {
        ...
        DbTxnManager txnMgr2 = (DbTxnManager) TxnManagerFactory.getTxnManagerFactory().getTxnManager(conf);
        swapTxnManager(txnMgr2);
        driver2.compileAndRespond("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b)");
        ...
        Assert.assertEquals("Duplicate records " + (extectedDuplicates ? "" : "not") + "found", extectedDuplicates ? 5 : 4, res.size());
        dropTable(new String[]{"target", "source"});
    }
    ...
    private void testConcurrentUpdateMergeUpdateConflict(boolean slowCompile) throws Exception
    {
        ...
        DbTxnManager txnMgr2 = (DbTxnManager) TxnManagerFactory.getTxnManagerFactory().getTxnManager(conf);
        swapTxnManager(txnMgr2);
        driver2.compileAndRespond("merge into target t using source s on t.a = s.a " + "when matched then update set b=8");
    ...
    private void testConcurrent2MergeUpdatesConflict(boolean slowCompile) throws Exception
    {
        ...
        if (!slowCompile)
        {
            driver.compileAndRespond("merge into target t using source s on t.name = s.name " + "when matched then update set age=10");
        }

        DbTxnManager txnMgr2 = (DbTxnManager) TxnManagerFactory.getTxnManagerFactory().getTxnManager(conf);
        swapTxnManager(txnMgr2);
        driver2.compileAndRespond("merge into target t using source s on t.age = s.age " + "when matched then update set age=10");
        ...
        {
            driver.run("merge into target t using source s on t.name = s.name " + "when matched then update set age=10");
        }
    ...
    public void testConcurrentConflictMaxRetryCount() throws Exception
    {
        ...
        } catch (CommandProcessorException cpe)
        {
            ...
            Assert.assertTrue(cpe.getMessage().contains("Operation could not be executed, snapshot was outdated when locks were acquired."));
        }
    ...
    public void testMergeMultipleBranchesOptimistic() throws Exception
    {
        ...
        driver.run("insert into target values " + "('0', 'orig_FyZl'), " + "(5, 'orig_VsbLsaG'), " + "(10, 'orig_dhhCassOoV')");

        driver.run("create table src1 (id int, txt string) stored as orc TBLPROPERTIES('transactional'='true')");
        driver.run("insert into src1 values " + "(0, 'new1_tnlGat'), " + "(1, 'new1_KulBf'), " + "(2, 'new1_zkLGuU'), " + "(3, 'new1_jznZVac')," + "(4, 'new1_hdyazJXL')," + "(5, 'new1_gxclXFtP')," + "(6, 'new1_CNZr')," + "(7, 'new1_GoBjjuow')," + "(8, 'new1_vRfY')," + "(9, 'new1_bdnQA')," + "(10, 'new1_FNboL')");

        driver.run("create table src2 (id int, txt string) stored as orc TBLPROPERTIES('transactional'='true')");
        driver.run("insert into src2 values " + "(0, 'new2_Cjdj'), " + "(1, 'new2_GysxGF'), " + "(2, 'new2_ToHyf'), " + "(3, 'new2_HZjkahVJ')," + "(4, 'new2_qcySYYUul')," + "(5, 'new2_FupKyDcVcJ')," + "(6, 'new2_DAcCwakVr')," + "(7, 'new2_nZozPAZKI')," + "(8, 'new2_bjdEmdRp')," + "(9, 'new2_PkRAwdJeLX')," + "(10, 'new2_aGSuZHx')");

        driver.compileAndRespond("MERGE INTO target t USING src1 s ON t.id = s.id " + "WHEN MATCHED THEN UPDATE SET txt = CONCAT_WS(' ',t.txt,s.txt) " + "WHEN NOT MATCHED THEN INSERT VALUES (s.id, s.txt)");

        DbTxnManager txnMgr2 = (DbTxnManager) TxnManagerFactory.getTxnManagerFactory().getTxnManager(conf);
        swapTxnManager(txnMgr2);
        driver2.compileAndRespond("MERGE INTO target t USING src2 s ON t.id = s.id " + "WHEN MATCHED THEN UPDATE SET txt = CONCAT_WS(' ',t.txt,s.txt) " + "WHEN NOT MATCHED THEN INSERT VALUES (s.id, s.txt)");
        ...
        Assert.assertEquals("[0\torig_FyZl new1_tnlGat, " + "1\tnew1_KulBf, " + "2\tnew1_zkLGuU, " + "3\tnew1_jznZVac, " + "4\tnew1_hdyazJXL, " + "5\torig_VsbLsaG new1_gxclXFtP, " + "6\tnew1_CNZr, " + "7\tnew1_GoBjjuow, " + "8\tnew1_vRfY, " + "9\tnew1_bdnQA, " + "10\torig_dhhCassOoV new1_FNboL]", res.toString());
    }
    ...
    private void testConcurrent2InsertOverwrites(boolean conflict) throws Exception
    {
        ...
        DbTxnManager txnMgr2 = (DbTxnManager) TxnManagerFactory.getTxnManagerFactory().getTxnManager(conf);
        swapTxnManager(txnMgr2);
        driver2.compileAndRespond("insert overwrite table target partition (b='" + (conflict ? 2 : 4) + "') select * from source");
    ...
    public void testInsertOverwriteMergeInsertDynamicPartitioningSequential() throws Exception
    {
        ...
        // txn3 merge
        driver.run("merge into target t using source s on t.a = s.a " + "when not matched then insert values (s.a, s.b, s.c)");
        driver.run("select * from target");
        List<String> res = new ArrayList<>();
        driver.getFetchTask().fetch(res);
    ...
    private void testDynamicPartitionInsert(boolean sharedWrite) throws Exception
    {
        ...
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), LockState.ACQUIRED, "default", "target", null, locks);
        Assert.assertEquals("HIVE_LOCKS mismatch(" + JavaUtils.txnIdToString(txnid1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"HIVE_LOCKS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"HIVE_LOCKS\" where \"HL_TXNID\"=" + txnid1));
        txnMgr.rollbackTxn();
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnid1));
        ...
        Assert.assertEquals(//look in COMPLETED_TXN_COMPONENTS because driver.run() committed!!!!
                "COMPLETED_TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid1 + 1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""), 2, //2 distinct partitions created
                //txnid+1 because we want txn used by previous driver.run("insert....)
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" where \"CTC_TXNID\"=" + (txnid1 + 1)));
        ...
        adp.setOperationType(DataOperationType.INSERT);
        txnHandler.addDynamicPartitions(adp);
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 2, //2 distinct partitions modified
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnid2));
        txnMgr.commitTxn();
    }
    ...
    private void testMergePartitioned(boolean causeConflict, boolean sharedWrite) throws Exception
    {
        ...
        swapTxnManager(txnMgr2);
        //start a 2nd (overlapping) txn
        driver.compileAndRespond("merge into target using source " + "on target.p=source.p1 and target.a=source.a1 " + "when matched then update set b = 11 " + "when not matched then insert values(a1,b1,p1,q1)", true);
        long txnid2 = txnMgr2.getCurrentTxnId();
        txnMgr2.acquireLocks(driver.getPlan(), ctx, "T2", false);
        locks = getLocks(txnMgr);
         ...
         */
        checkLock(LockType.SHARED_READ, (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "source", null, locks);
        long extLockId = checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.SHARED_READ), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "target", null, locks, sharedWrite).getLockid();
        ...
        checkLock((sharedWrite ? LockType.SHARED_WRITE : LockType.EXCL_WRITE), (sharedWrite ? LockState.ACQUIRED : LockState.WAITING), "default", "target", "p=2/q=2", locks);

        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, //because it's using a DP write
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId1));
        ...
        adp.setOperationType(DataOperationType.UPDATE);
        txnHandler.addDynamicPartitions(adp);
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnId1 + " and \"TC_OPERATION_TYPE\"='u'"));

        txnMgr.commitTxn(); //commit T1
        Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnId1) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 2, //2 partitions updated
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnId1 + " and \"WS_OPERATION_TYPE\"='u'"));
        ...
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 1, //because it's using a DP write
                TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnid2));
        ...
        adp.setOperationType(DataOperationType.INSERT);
        txnHandler.addDynamicPartitions(adp);
        Assert.assertEquals("TXN_COMPONENTS mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"TXN_COMPONENTS\""), 2, TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"TXN_COMPONENTS\" where \"TC_TXNID\"=" + txnid2 + " and \"TC_OPERATION_TYPE\"='i'"));
        ...
        {
            Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, //1 partitions updated
                    TestTxnDbUtil.countQueryAgent(conf, "select count(*) from \"WRITE_SET\" where \"WS_TXNID\"=" + txnid2 + " and \"WS_OPERATION_TYPE\"='u'"));
            Assert.assertEquals("WRITE_SET mismatch(" + JavaUtils.txnIdToString(txnid2) + "): " + TestTxnDbUtil.queryToString(conf, "select * from \"WRITE_SET\""), 1, //1 partitions updated (and no other entries)
    ...
    public void testShowTablesLock() throws Exception
    {
        ...
        swapTxnManager(txnMgr);
        driver.run("create table T2 (a int, b int) partitioned by (p int) clustered by (a) " + "into 2  buckets stored as orc TBLPROPERTIES ('transactional'='false')");
    ...
    private void testFairness(boolean zeroWaitRead) throws Exception
    {
        ...
        } catch (LockException ex)
        {
            ...
            Assert.assertEquals("Exception msg didn't match", ErrorMsg.LOCK_CANNOT_BE_ACQUIRED.getMsg() + " LockResponse(lockid:" + (extLockId + 1) + ", state:NOT_ACQUIRED, errorMessage:Unable to acquire read lock due to an existing exclusive lock" + " {lockid:" + extLockId + " intLockId:1 txnid:" + txnMgr2.getCurrentTxnId() + " db:default table:t6 partition:null state:WAITING type:EXCLUSIVE})", ex.getMessage());
        }
        locks = getLocks();
    ...
    private void testFairness2(boolean zeroWaitRead) throws Exception
    {
        ...
        conf.setBoolVar(HiveConf.ConfVars.TXN_WRITE_X_LOCK, !zeroWaitRead);
        driver.run("create table if not exists T7 (a int) " + "partitioned by (p int) stored as orc TBLPROPERTIES ('transactional'='true')");
        ...
        {
            Assert.assertTrue(zeroWaitRead);
            Assert.assertEquals("Exception msg didn't match", ErrorMsg.LOCK_CANNOT_BE_ACQUIRED.getMsg() + " LockResponse(lockid:" + (extLockId + 1) + ", state:NOT_ACQUIRED, errorMessage:Unable to acquire read lock due to an existing exclusive lock" + " {lockid:" + extLockId + " intLockId:1 txnid:" + txnMgr2.getCurrentTxnId() + " db:default table:t7 partition:p=1 state:WAITING type:EXCLUSIVE})", ex.getMessage());
        }
        locks = getLocks();
    ...
    private void testLoadData(boolean sharedWrite) throws Exception
    {
        ...
        List<ShowLocksResponseElement> locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 1, locks.size());
        checkLock((sharedWrite ? LockType.EXCL_WRITE : LockType.EXCLUSIVE), LockState.ACQUIRED, "default", "T2", null, locks);
        txnMgr.commitTxn();
    }
    ...
    public void testMmConversionLocks() throws Exception
    {
        ...
        driver.compileAndRespond("ALTER TABLE T set tblproperties" + "('transactional'='true', 'transactional_properties'='insert_only')", true);
    ...
    public void testAnalyze() throws Exception
    {
        ...
        driver.run("create table tab_not_acid (key string, value string) partitioned by (ds string, hr string) " + "stored as textfile");
        ...
        driver.run("create table tab_acid (key string, value string) partitioned by (ds string, hr string) " + "stored as orc TBLPROPERTIES ('transactional'='true')");
    ...
    public void testFullTableReadLock() throws Exception
    {
        ...
        driver.run("create table if not exists tab_not_acid (na int, nb int) partitioned by (np string) " + "stored as orc TBLPROPERTIES ('transactional'='false')");
        driver.run("insert into tab_acid partition(p) (a,b,p) values(1,2,'foo'),(3,4,'bar')");
        driver.run("insert into tab_not_acid partition(np) (na,nb,np) values(1,2,'blah'),(3,4,'doh')");
    ...
    public void testInsertSnapshotIsolation() throws Exception
    {
        ...
        driver.run("create table if not exists tab_acid (a int, b int) " + "stored as orc TBLPROPERTIES ('transactional'='true')");
    ...
    private void testDropPartition(boolean blocking) throws Exception
    {
        ...
        if (blocking)
        {
            ...
            ShowLocksResponseElement checkLock = checkLock(LockType.EXCLUSIVE, LockState.WAITING, "default", "tab_acid", "p=foo", locks);
        ...
        checkLock(blocking ? LockType.EXCLUSIVE : LockType.EXCL_WRITE, LockState.ACQUIRED, "default", "tab_acid", "p=foo", locks);
        ...
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), "tab_acid" + (blocking ? "" : "/p=foo")), (blocking ? path -> path.getName().equals("p=foo") : AcidUtils.baseFileFilter));
    ...
    public void testDropTableNonBlocking2() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(conf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir()), t -> t.getName().matches("tab_acid" + SOFT_DELETE_TABLE_PATTERN));
        if (1 != stat.length)
        {
    ...
    private void testRenameTable(boolean blocking) throws Exception
    {
        ...
        locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 2, locks.size());
        checkLock(LockType.SHARED_READ, LockState.ACQUIRED, "default", null, null, locks);//destination database
        checkLock(blocking ? LockType.EXCLUSIVE : LockType.EXCL_WRITE, LockState.ACQUIRED, "default", "tab_acid", null, locks);//source table
        ...
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir()), t -> t.getName().matches("tab_acid" + (blocking ? "_v2" : SOFT_DELETE_TABLE_PATTERN)));
        if (1 != stat.length)
        {
    ...
    public void testDropAlterViewNoLocks() throws Exception
    {
        ...
        driver.run("create view v_tab_acid partitioned on (p) " + "as select a, p from tab_acid where b > 1");
    ...
    private void testDropMaterializedView(boolean blocking) throws Exception
    {
        ...
        driver.run("create materialized view mv_tab_acid partitioned on (p) " + "stored as orc TBLPROPERTIES ('transactional'='true') as select a, p from tab_acid where b > 1");
        ...
        {
            checkLock(LockType.SHARED_READ, LockState.ACQUIRED, "default", "tab_acid", null, locks);
            checkLock(LockType.SHARED_READ, LockState.ACQUIRED, "default", "mv_tab_acid", null, locks);
        }
        ...
        if (blocking)
        {
            ...
            ShowLocksResponseElement checkLock = checkLock(LockType.EXCLUSIVE, LockState.WAITING, "default", "mv_tab_acid", null, locks);
        ...
        checkLock(blocking ? LockType.EXCLUSIVE : LockType.EXCL_WRITE, LockState.ACQUIRED, "default", "mv_tab_acid", null, locks);
        ...
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir()), t -> t.getName().matches("mv_tab_acid" + (blocking ? "" : SOFT_DELETE_TABLE_PATTERN)));
        if ((blocking ? 0 : 1) != stat.length)
        {
            Assert.fail("Table data was " + (blocking ? "not" : "") + "removed from FS");
        }
    ...
    public void testDbConnectorNoLocks() throws Exception
    {
        ...
        driver.run("CREATE CONNECTOR IF NOT EXISTS derby_auth " + "TYPE 'derby' " + "URL 'jdbc:derby:./target/tmp/junit_metastore_db;create=true' " + "WITH DCPROPERTIES ( " + "   'hive.sql.dbcp.username'='APP', " + "   'hive.sql.dbcp.password'='mine')");
    ...
    private void dropDatabaseNonBlocking(boolean allTablesWithSuffix, boolean cascade) throws Exception
    {
        ...
        driver.run("create table " + database + "." + tableName + "1 (a int, b int) " + "partitioned by (ds string) stored as orc TBLPROPERTIES ('transactional'='true')");
        driver.run("create materialized view " + database + ".mv_" + tableName + "1 " + "partitioned on (ds) stored as orc TBLPROPERTIES ('transactional'='true')" + "as select a, ds from " + database + "." + tableName + "1 where b > 1");

        HiveConf.setBoolVar(conf, HiveConf.ConfVars.HIVE_ACID_LOCKLESS_READS_ENABLED, true);
        // Create transactional table/materialized view with lockless-reads feature enabled
        driver.run("create table " + database + "." + tableName + "2 (a int, b int) " + "partitioned by (ds string) stored as orc TBLPROPERTIES ('transactional'='true')");
        driver.run("create materialized view " + database + ".mv_" + tableName + "2 " + "partitioned on (ds) stored as orc TBLPROPERTIES ('transactional'='true')" + "as select a, ds from " + database + "." + tableName + "2 where b > 1");

        if (!allTablesWithSuffix)
        {
            // Create external table
            driver.run("create external table " + database + ".tab_ext (a int, b int) " + "partitioned by (ds string) stored as parquet");
            // Create managed table
            driver.run("create table " + database + ".tab_nonacid (a int, b int) " + "partitioned by (ds string) stored as parquet");
        }
        ...
        {
            checkLock(LockType.EXCLUSIVE, LockState.ACQUIRED, database, tableName + "1", null, locks);
            checkLock(LockType.EXCLUSIVE, LockState.ACQUIRED, database, "mv_" + tableName + "1", null, locks);
            checkLock(LockType.EXCL_WRITE, LockState.ACQUIRED, database, tableName + "2", null, locks);
            checkLock(LockType.EXCL_WRITE, LockState.ACQUIRED, database, "mv_" + tableName + "2", null, locks);
            checkLock(LockType.EXCLUSIVE, LockState.ACQUIRED, database, "tab_nonacid", null, locks);

        } else
        {
            checkLock(LockType.EXCL_WRITE, LockState.ACQUIRED, database, null, null, locks);
        }
    }
    ...
    public void testAddPartitionIfNotExists() throws Exception
    {
        ...
        driver.run("create table if not exists T (a int, b int) partitioned by (p string) " + "stored as orc TBLPROPERTIES ('transactional'='true')");
        //bucketed just so that we get 2 files
        driver.run("create table if not exists Tstage (a int, b int) clustered by (a) into 2 " + "buckets stored as orc TBLPROPERTIES ('transactional'='false')");
        ...
        checkLock(LockType.EXCL_WRITE, LockState.ACQUIRED, "default", "T", null, locks);
    ...
    public void testMsckRepair() throws Exception
    {
        ...
        driver.run("create table tab_acid (a int, b int) partitioned by (p string) " + "stored as orc TBLPROPERTIES ('transactional'='true')");
        ...
        // Create target table
        driver.run("create table tab_acid_msck (a int, b int) partitioned by (p string) " + " stored as orc TBLPROPERTIES ('transactional'='true')");

        // copy files on fs
        FileSystem fs = FileSystem.get(conf);
        FileUtil.copy(fs, new Path(getWarehouseDir() + "/tab_acid/p=p1"), fs, new Path(getWarehouseDir(), "tab_acid_msck"), false, conf);
    ...
    public void testSetSerdeAndFileFormatNonBlocking() throws Exception
    {
        ...
        driver.run("create table if not exists tab_acid (a int, b int) partitioned by (p string) " + "TBLPROPERTIES ('transactional'='true', 'transactional_properties'='insert_only')");
        driver.run("insert into tab_acid partition(p) (a,b,p) values(1,2,'foo'),(3,4,'bar')");
    ...
    public void testMaterializedViewEnableRewriteNonBlocking() throws Exception
    {
        ...
        List<ShowLocksResponseElement> locks = getLocks();
        Assert.assertEquals("Unexpected lock count", 1, locks.size());
        checkLock(LockType.EXCL_WRITE, LockState.ACQUIRED, "default", "mv_tab_acid", null, locks);
        // cleanup
        txnMgr.rollbackTxn();
        driver.run("drop materialized view mv_tab_acid");
    }
