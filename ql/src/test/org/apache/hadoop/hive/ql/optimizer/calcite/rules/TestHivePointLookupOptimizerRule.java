...
public class TestHivePointLookupOptimizerRule
{
    ...
    // @formatter:on
        ...
        assertEquals("AND(IN($0, 1, 2), =($0, 1.0E0:DOUBLE), IN($3, 3.0E0:DOUBLE, 4.1E0:DOUBLE))", condition.toString());
    }
    ...
    // @formatter:on
        ...
        RexNode condition = filter.getCondition();
        System.out.println(condition);
        assertEquals("AND(IN($0, 1, 2), OR(AND(IN($1, 1, 2), IN($2, 1, 2)), " + "AND(IN($1, 3, 4), IN($2, 3, 4))))", condition.toString());
    }
    ...
    public void testSameVarcharLiteralDifferentPrecision()
    {
        ...
        final RelNode basePlan = relBuilder.scan("t").filter(and(relBuilder, relBuilder.call(SqlStdOperatorTable.IN, relBuilder.field("f2"), lita30, litb30), relBuilder.call(SqlStdOperatorTable.IN, relBuilder.field("f2"), lita14, litb14))).build();
        ...
        RexNode condition = filter.getCondition();
        System.out.println(condition);
        assertEquals("IN($1, " + "_UTF-16LE'AAA111':VARCHAR(30) CHARACTER SET \"UTF-16LE\", " + "_UTF-16LE'BBB222':VARCHAR(30) CHARACTER SET \"UTF-16LE\")", condition.toString());
    }
    ...
    public void testSameVarcharLiteralDifferentPrecisionValueOverflow()
    {
        ...
        RexNode litaOverflow = rexBuilder.makeLiteral(RexNodeExprFactory.makeHiveUnicodeString("AAA111"), stringType4, true);
        RexNode litbOverflow = rexBuilder.makeLiteral(RexNodeExprFactory.makeHiveUnicodeString("BBB222"), stringType4, true);

        final RelNode basePlan = relBuilder.scan("t").filter(and(relBuilder, relBuilder.call(SqlStdOperatorTable.IN, relBuilder.field("f2"), lita30, litb30), relBuilder.call(SqlStdOperatorTable.IN, relBuilder.field("f2"), litaOverflow, litbOverflow))).build();
    ...
    public void testSameVarcharLiteralsDifferentPrecisionInOrExpression()
    {
        ...
        final RelNode basePlan = relBuilder.scan("t").filter(or(relBuilder, relBuilder.call(SqlStdOperatorTable.IN, relBuilder.field("f2"), lita30, litb30), relBuilder.call(SqlStdOperatorTable.IN, relBuilder.field("f2"), lita14, litb14))).build();
