...
public class TestDeltaFilesMetrics extends CompactorTest
{
    ...
    private void setUpHiveConf()
    {
        ...
        MetastoreConf.setTimeVar(conf, MetastoreConf.ConfVars.METASTORE_ACIDMETRICS_CHECK_INTERVAL, 1, TimeUnit.SECONDS);
    ...
    {
        verifyDeltaMetricsMatch(expected, gaugeToMap(metricName), Metrics.getOrCreateMapMetrics(metricName).get());
    }
    ...
    {
        assertThat("Actual mBean metrics " + actualMBeanMetric + " don't match expected: " + expected, actualMapMetric, is(expected));
        assertThat("Actual map metrics " + actualMapMetric + " don't match expected: " + expected, actualMapMetric, is(expected));
    }
