...
public class TestReplicationMetricCollector
{
    ...
    public void testFailureNoScheduledId() throws Exception
    {
        ...
        conf = new HiveConf();
        MetricCollector.getInstance().init(conf);
        ReplicationMetricCollector bootstrapDumpMetricCollector = new BootstrapDumpMetricCollector("db", "dummyDir", conf, 0L);
    ...
    {
        ReplicationMetricCollector incrDumpMetricCollector = new IncrementalDumpMetricCollector("db", "dummyDir", conf, 0L);
        Map<String, Long> metricMap = new HashMap<>();
        metricMap.put(ReplUtils.MetricName.TABLES.name(), (long) 10);
        metricMap.put(ReplUtils.MetricName.FUNCTIONS.name(), (long) 1);
    ...
    {
        ReplicationMetricCollector preOptimizedBootstrapDumpMetricCollector = new PreOptimizedBootstrapDumpMetricCollector("db", "dummyDir", conf, (long) -1, MetaStoreUtils.FailoverEndpoint.SOURCE.toString(), ReplConst.FailoverType.UNPLANNED.toString());
        Map<String, Long> metricMap = new HashMap<>();
        ...
        preOptimizedBootstrapDumpMetricCollector.reportStageEnd("dump", Status.SUCCESS, -1, new SnapshotUtils.ReplSnapshotCount(), new ReplStatsTracker(0));
    ...
    {
        ReplicationMetricCollector optimizedBootstrapDumpMetricCollector = new OptimizedBootstrapDumpMetricCollector("db", "dummyDir", conf, 0L, MetaStoreUtils.FailoverEndpoint.SOURCE.toString(), ReplConst.FailoverType.UNPLANNED.toString());
        Map<String, Long> metricMap = new HashMap<>();
        metricMap.put(ReplUtils.MetricName.TABLES.name(), (long) 10);
        metricMap.put(ReplUtils.MetricName.FUNCTIONS.name(), (long) 1);
        ...
        optimizedBootstrapDumpMetricCollector.reportStageEnd("dump", Status.SUCCESS, 10, new SnapshotUtils.ReplSnapshotCount(), new ReplStatsTracker(0));
    ...
    private void checkSuccess(ReplicationMetric actual, ReplicationMetric expected, String stageName, List<String> metricNames)
    {
        ...
        Assert.assertEquals(expected.getProgress().getStageByName(stageName).getStatus(), actual.getProgress().getStageByName(stageName).getStatus());
        for (String metricName : metricNames)
        {
            Assert.assertEquals(expected.getProgress().getStageByName(stageName).getMetricByName(metricName).getTotalCount(), actual.getProgress().getStageByName(stageName).getMetricByName(metricName).getTotalCount());
            Assert.assertEquals(expected.getProgress().getStageByName(stageName).getMetricByName(metricName).getCurrentCount(), actual.getProgress().getStageByName(stageName).getMetricByName(metricName).getCurrentCount());
        }
    }
