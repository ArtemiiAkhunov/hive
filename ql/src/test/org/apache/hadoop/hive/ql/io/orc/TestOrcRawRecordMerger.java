...
public class TestOrcRawRecordMerger
{
    ...
    private static void checkIntersect(OrcRawRecordMerger.KeyInterval ki1, OrcRawRecordMerger.KeyInterval ki2, boolean isIntersect)
    {
        if (isIntersect)
        {
    ...
    private static void setRow(OrcStruct event, int operation, long originalTransaction, int bucket, long rowId, long currentTransaction, String value)
    {
        ...
        event.setFieldValue(OrcRecordUpdater.ORIGINAL_WRITEID, new LongWritable(originalTransaction));
        ...
        event.setFieldValue(OrcRecordUpdater.CURRENT_WRITEID, new LongWritable(currentTransaction));
    ...
    {
        long offset = 0;
        List<StripeInformation> result = new ArrayList<>(rowCounts.length);
        int stripeCount = 0;
        for (long count : rowCounts)
        {
            OrcProto.StripeInformation.Builder stripe = OrcProto.StripeInformation.newBuilder();
            stripe.setDataLength(800).setIndexLength(100).setFooterLength(100).setNumberOfRows(count).setOffset(offset);
            offset += 1000;
    ...
    private Reader createMockReader() throws IOException
    {
        ...
        when(reader.rowsOptions(any(Reader.Options.class), any(HiveConf.class))).thenReturn(recordReader);
    ...
    private static <T> void checkReaderRecord(int writeId, int bucketProperty, int rowId, int currWriteId, T value, ReaderKey key, ReaderPair pair)
    {
    ...
    private static <T> void checkMergerRecord(long writeId, int bucketProperty, long rowId, T value, RecordIdentifier key, OrcStruct event)
    {
    ...
    private static <T> void checkMergerRecord(int operationType, T value, ReaderKey key, RecordIdentifier id, OrcStruct event)
    {
        checkEvent(operationType, event);
        checkMergerRecord(key.getWriteId(), key.getBucketProperty(), key.getRowId(), value, id, event);
    }
    ...
    private Reader buildMockReaderMerger(List<String> columnNames, List<OrcProto.Type.Kind> columnKinds, boolean addHiveAcidKeyIndexMetadata) throws IOException
    {
        Reader reader = Mockito.mock(Reader.class, settings);
        RecordReader recordReader = Mockito.mock(RecordReader.class, settings);
        ...
        } else
        {
            ...
            when(recordReader.hasNext()).thenReturn(true, true, true, true, true, true, true, true, true, false);
        ...
        when(reader.hasMetadataValue(OrcRecordUpdater.ACID_KEY_INDEX_NAME)).thenReturn(addHiveAcidKeyIndexMetadata);
        if (addHiveAcidKeyIndexMetadata)
        {
            when(reader.getMetadataValue(OrcRecordUpdater.ACID_KEY_INDEX_NAME)).thenReturn(ByteBuffer.wrap("10,20,30;40,50,60;40,50,61".getBytes(StandardCharsets.UTF_8)));
        }
    ...
    public void testNewBase() throws Exception
    {
        ...
        final OrcRawRecordMerger merger = new OrcRawRecordMerger(conf, false, reader, false, 10, createMaximalTxnList(), new Reader.Options().range(1000, 1000), null, new OrcRawRecordMerger.Options());
    ...
    public void testNewBaseNoHiveAcidKeyIndexMetadata() throws Exception
    {
        ...
        final OrcRawRecordMerger merger = new OrcRawRecordMerger(conf, true, reader, false, 10, createMaximalTxnList(), new Reader.Options().range(1000, 1000), null, new OrcRawRecordMerger.Options());
        final RecordReader rr = merger.getCurrentReader().getRecordReader();
        assertEquals(0, merger.getOtherReaders().size());
    ...
    private static void assertRecordMergerStructFields(OrcRawRecordMerger merger, List<String> columns)
    {
        ...
        {
            assertEquals(columns.get(i), rowObjectInspector.getAllStructFieldRefs().get(i).getFieldName());
        }
    }
    ...
    public void testGetLogicalLength() throws Exception
    {
        ...
        AcidOutputFormat.Options options = new AcidOutputFormat.Options(conf).inspector(inspector).bucket(BUCKET).writingBase(false).minimumWriteId(1).maximumWriteId(2).finalDestination(root);
        Path delta1_2_0 = new Path(root, AcidUtils.deltaSubdir(options.getMinimumWriteId(), options.getMaximumWriteId(), options.getStatementId()));
    ...
    private void testNewBaseAndDelta(boolean use130Format) throws Exception
    {
        ...
        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.DELETE_OPERATION, null, new ReaderKey(0, BUCKET_PROPERTY, 0, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "second", new ReaderKey(0, BUCKET_PROPERTY, 1, 0), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.DELETE_OPERATION, null, new ReaderKey(0, BUCKET_PROPERTY, 2, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.DELETE_OPERATION, null, new ReaderKey(0, BUCKET_PROPERTY, 3, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "fifth", new ReaderKey(0, BUCKET_PROPERTY, 4, 0), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "sixth", new ReaderKey(0, BUCKET_PROPERTY, 5, 0), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "seventh", new ReaderKey(0, BUCKET_PROPERTY, 6, 0), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.DELETE_OPERATION, null, new ReaderKey(0, BUCKET_PROPERTY, 7, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.DELETE_OPERATION, null, new ReaderKey(0, BUCKET_PROPERTY, 8, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "tenth", new ReaderKey(0, BUCKET_PROPERTY, 9, 0), id, event);
        ...
        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "update 1", new ReaderKey(200, BUCKET_PROPERTY, 0, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "update 2", new ReaderKey(200, BUCKET_PROPERTY, 1, 200), id, event);

        assertTrue(merger.next(id, event));
        checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, "update 3", new ReaderKey(200, BUCKET_PROPERTY, 2, 200), id, event);
        ...
        for (int i = 0; i < values.length; ++i)
        {
            ...
            checkMergerRecord(OrcRecordUpdater.INSERT_OPERATION, values[i], new ReaderKey(0, BUCKET_PROPERTY, i, 0), id, event);
        }
        assertFalse(merger.next(id, event));
        merger.close();
    ...
    public void testRecordReaderOldBaseAndDelta() throws Exception
    {
        ...
        Writer writer = OrcFile.createWriter(new Path(root, "0000010_0"), OrcFile.writerOptions(conf).inspector(inspector).fileSystem(fs).blockPadding(false).bufferSize(10000).compress(CompressionKind.NONE).stripeSize(1).batchSize(2).version(OrcFile.Version.V_0_11));
        ...
        job.set(ValidTxnList.VALID_TXNS_KEY, new ValidReadTxnList(new long[0], new BitSet(), 1000, Long.MAX_VALUE).writeToString());
    ...
    public void testRecordReaderNewBaseAndDelta() throws Exception
    {
        ...
        options.orcOptions(OrcFile.writerOptions(conf).stripeSize(1).blockPadding(false).compress(CompressionKind.NONE).batchSize(2));
