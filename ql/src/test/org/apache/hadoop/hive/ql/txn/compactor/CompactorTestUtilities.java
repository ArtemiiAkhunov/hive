...
public class CompactorTestUtilities
{
    ...
    private static int getAcidVersionFromDataFile(Path dataFile, FileSystem fs) throws IOException
    {
        ...
        try (Reader orcReader = OrcFile.createReader(dataFile, OrcFile.readerOptions(fs.getConf()).filesystem(fs)
                ...
                .maxLength(AcidUtils.getLogicalLength(fs, fileStatus))))
        {
            ...
            {
                char[] versionChar = UTF8.decode(orcReader.getMetadataValue(AcidUtils.OrcAcidVersion.ACID_VERSION_KEY)).array();
    ...
    public static void checkAcidVersion(RemoteIterator<LocatedFileStatus> files, FileSystem fs, boolean hasVersionFile, String[] expectedTypes) throws IOException
    {
        ...
        while (files.hasNext())
        {
            ...
            if (!path.getName().equals(AcidUtils.OrcAcidVersion.ACID_FORMAT))
            {
                ...
                Assert.assertEquals("Unexpected version marker in " + path.getName(), AcidUtils.OrcAcidVersion.ORC_ACID_VERSION, version);
            }
            ...
            if (!foundDirectories.contains(parent))
            {
                ...
                if (hasVersionFile)
                {
                    Assert.assertTrue("Version marker should exists", fs.exists(AcidUtils.OrcAcidVersion.getVersionFilePath(parent)));
                    int versionFromMetaFile = getAcidVersionFromMetaFile(parent, fs);
                    Assert.assertEquals("Unexpected version marker in " + parent, AcidUtils.OrcAcidVersion.ORC_ACID_VERSION, versionFromMetaFile);
                } else
                {
                    Assert.assertFalse("Version marker should not exists", fs.exists(AcidUtils.OrcAcidVersion.getVersionFilePath(parent)));
                }
            }
        }
        Assert.assertEquals("Did not found all types of directories", new HashSet<>(Arrays.asList(expectedTypes)), foundPrefixes);
    }
}
