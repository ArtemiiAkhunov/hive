...
public class TestZookeeperLockManager
{
    ...
    public void testGetQuorumServers()
    {
        ...
        conf.setVar(HiveConf.ConfVars.HIVE_ZOOKEEPER_CLIENT_PORT, "9999");
        Assert.assertEquals("node1:9999,node2:9999,node3:9999", conf.getZKConfig().getQuorumServers());

        conf.setVar(HiveConf.ConfVars.HIVE_ZOOKEEPER_QUORUM, "node1:5666,node2,node3");
        conf.setVar(HiveConf.ConfVars.HIVE_ZOOKEEPER_CLIENT_PORT, "9999");
        Assert.assertEquals("node1:5666,node2:9999,node3:9999", conf.getZKConfig().getQuorumServers());
    }
    ...
    static class LockTesterThread extends Thread
    {
        ...
        public LockTesterThread(CyclicBarrier barrier, Semaphore semaphore, ZooKeeperHiveLockManager zMgr, HiveLockObject hiveLock, HiveLockMode semiShared)
        {
    ...
    private void testLockModeTiming(ZooKeeperHiveLockManager zMgr, HiveLockMode lock1mode, HiveLockMode lock2mode, int minT, int maxT) throws Exception, LockException, InterruptedException, BrokenBarrierException
    {
