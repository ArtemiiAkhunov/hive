...
public class TestFixAcidKeyIndex
{
    ...
    public void openFileSystem() throws Exception
    {
        ...
        testFilePath = new Path(workDir, "TestFixAcidKeyIndex." + testCaseName.getMethodName() + ".orc");
        fs.delete(testFilePath, false);
    }

    static abstract class TestKeyIndexBuilder extends OrcRecordUpdater.KeyIndexBuilder implements OrcFile.WriterCallback
    {
    ...
    void createTestAcidFile(Path path, int numRows, TestKeyIndexBuilder indexBuilder) throws Exception
    {
        ...
        Writer writer = OrcFile.createWriter(path, OrcFile.writerOptions(conf).fileSystem(fs).inspector(OrcStruct.createObjectInspector(typeInfo)).compress(CompressionKind.NONE).callback(indexBuilder).stripeSize(128));
    ...
    static class MissingKeyIndexBuilder extends TestKeyIndexBuilder
    {
        ...
        public void preFooterWrite(OrcFile.WriterContext context) throws IOException
        {
            ...
            context.getWriter().addUserMetadata(OrcAcidUtils.ACID_STATS, StandardCharsets.UTF_8.encode(acidStats.serialize()));
