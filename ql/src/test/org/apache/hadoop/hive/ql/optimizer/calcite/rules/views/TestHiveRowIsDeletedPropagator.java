...
public class TestHiveRowIsDeletedPropagator extends TestRuleBase
{
    ...
    public void testJoining3TablesAndAllChanged()
    {
        ...
        RelNode join1 = relBuilder.push(ts1).filter(REX_BUILDER.makeCall(SqlStdOperatorTable.IS_NOT_NULL, REX_BUILDER.makeInputRef(ts1, 0))).push(ts2).filter(REX_BUILDER.makeCall(SqlStdOperatorTable.IS_NOT_NULL, REX_BUILDER.makeInputRef(ts2, 0))).join(JoinRelType.INNER, joinCondition).build();

        RexNode joinCondition2 = REX_BUILDER.makeCall(SqlStdOperatorTable.EQUALS, REX_BUILDER.makeInputRef(ts3.getRowType().getFieldList().get(0).getType(), 10), REX_BUILDER.makeInputRef(join1.getRowType().getFieldList().get(5).getType(), 5));

        RelDataType bigIntType = relBuilder.getTypeFactory().createSqlType(SqlTypeName.BIGINT);

        RexNode writeIdFilter = REX_BUILDER.makeCall(SqlStdOperatorTable.OR, REX_BUILDER.makeCall(SqlStdOperatorTable.LESS_THAN, REX_BUILDER.makeLiteral(1, bigIntType, false), rowIdFieldAccess(ts1, 3)), REX_BUILDER.makeCall(SqlStdOperatorTable.LESS_THAN, REX_BUILDER.makeLiteral(1, bigIntType, false), rowIdFieldAccess(ts2, 8)), REX_BUILDER.makeCall(SqlStdOperatorTable.LESS_THAN, REX_BUILDER.makeLiteral(1, bigIntType, false), rowIdFieldAccess(ts3, 13)));

        RelNode root = relBuilder.push(join1).push(ts3).filter(REX_BUILDER.makeCall(SqlStdOperatorTable.IS_NOT_NULL, REX_BUILDER.makeInputRef(ts3, 0))).join(JoinRelType.INNER, joinCondition2).filter(writeIdFilter).build();
    ...
    private RexNode rowIdFieldAccess(RelNode tableScan, int posInTarget)
    {
        ...
        return REX_BUILDER.makeFieldAccess(REX_BUILDER.makeInputRef(tableScan.getTable().getRowType().getFieldList().get(rowIDPos).getType(), posInTarget), 0);
    }
}