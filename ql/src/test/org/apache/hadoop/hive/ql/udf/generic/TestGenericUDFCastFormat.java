...
public class TestGenericUDFCastFormat
{
    ...
    private void testCast(int typeCode, int length, ObjectInspector inputOI, Object input, String format, String expOutput) throws HiveException
    {
        ...
        ConstantObjectInspector typeCodeOI = PrimitiveObjectInspectorFactory.getPrimitiveWritableConstantObjectInspector(TypeInfoFactory.getPrimitiveTypeInfo("int"), new IntWritable(typeCode));
        ConstantObjectInspector formatOI = PrimitiveObjectInspectorFactory.getPrimitiveWritableConstantObjectInspector(TypeInfoFactory.getPrimitiveTypeInfo("string"), new Text(format));
        ConstantObjectInspector lengthOI = PrimitiveObjectInspectorFactory.getPrimitiveWritableConstantObjectInspector(TypeInfoFactory.getPrimitiveTypeInfo("int"), new IntWritable(length));
        ...
        if (output == null)
        {
            fail("Cast " + inputOI.getTypeName() + " \"" + input + "\" to " + GenericUDFCastFormat.OUTPUT_TYPES.get(typeCode) + " failed, output null");
        }
        assertEquals("Cast " + inputOI.getTypeName() + " \"" + input + "\" to " + GenericUDFCastFormat.OUTPUT_TYPES.get(typeCode) + " failed ", expOutput, output.toString());

        // Try with null input
        GenericUDF.DeferredObject[] nullArgs = {typeCodeObj, new GenericUDF.DeferredJavaObject(null), formatObj, lengthObj};
