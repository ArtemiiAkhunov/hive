...
public class TestParseUtils
{
    ...
    {
        return Arrays.asList(new Object[][]{{"SELECT current_timestamp()", TxnType.READ_ONLY}, {"SELECT count(*) FROM a", TxnType.READ_ONLY}, {"SELECT count(*) FROM a JOIN b ON a.id = b.id", TxnType.READ_ONLY},

                {"WITH a AS (SELECT current_timestamp()) " + "  SELECT * FROM a", TxnType.READ_ONLY},

                {"INSERT INTO a VALUES (1, 2)", TxnType.DEFAULT}, {"INSERT INTO a SELECT * FROM b", TxnType.DEFAULT}, {"INSERT OVERWRITE TABLE a SELECT * FROM b", TxnType.DEFAULT},

                {"FROM b INSERT OVERWRITE TABLE a SELECT *", TxnType.DEFAULT},

                {"WITH a AS (SELECT current_timestamp()) " + "  INSERT INTO b SELECT * FROM a", TxnType.DEFAULT},

                {"UPDATE a SET col_b = 1", TxnType.DEFAULT}, {"DELETE FROM a WHERE col_b = 1", TxnType.DEFAULT},

                {"CREATE TABLE a (col_b int)", TxnType.DEFAULT}, {"CREATE TABLE a AS SELECT * FROM b", TxnType.DEFAULT}, {"DROP TABLE a", TxnType.DEFAULT},

                {"LOAD DATA LOCAL INPATH './examples/files/kv.txt' " + "  OVERWRITE INTO TABLE a", TxnType.DEFAULT},

                {"REPL LOAD a INTO a", TxnType.DEFAULT}, {"REPL DUMP a", TxnType.DEFAULT}, {"REPL STATUS a", TxnType.DEFAULT},

                {"MERGE INTO a trg using b src " + "  ON src.col_a = trg.col_a " + "WHEN MATCHED THEN " + "  UPDATE SET col_b = src.col_b " + "WHEN NOT MATCHED THEN " + "  INSERT VALUES (src.col_a, src.col_b)", TxnType.DEFAULT},

                {"CREATE MATERIALIZED VIEW matview AS SELECT * FROM b", TxnType.DEFAULT}, {"ALTER MATERIALIZED VIEW matview REBUILD", TxnType.MATER_VIEW_REBUILD}, {"ALTER MATERIALIZED VIEW matview DISABLE REWRITE", TxnType.DEFAULT},
    ...
    public void testTxnTypeWithDisabledReadOnlyFeature() throws Exception
    {
        ...
        Assert.assertEquals(AcidUtils.getTxnType(conf, ParseUtils.parse(query, new Context(conf))), txnType == TxnType.READ_ONLY ? TxnType.DEFAULT : txnType);
    }
