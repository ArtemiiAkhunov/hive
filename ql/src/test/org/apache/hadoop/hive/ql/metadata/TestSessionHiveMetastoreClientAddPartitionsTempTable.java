...
public class TestSessionHiveMetastoreClientAddPartitionsTempTable extends TestAddPartitions
{
    ...
    public void setUp() throws Exception
    {
        ...
        new DatabaseBuilder().setName(DB_NAME).create(getClient(), conf);
    }
    ...
    {
        new TableBuilder().setDbName(DB_NAME).setTableName(tableName).addCol("test_id", "int", "test col id").addCol("test_value", DEFAULT_COL_TYPE, "test col value").addPartCol(YEAR_COL_NAME, DEFAULT_COL_TYPE).addTableParam("EXTERNAL", "TRUE").setLocation(location).setTemporary(true).create(getClient(), conf);
    }
    ...
    protected void verifyPartition(Table table, String name, List<String> values, int index) throws Exception
    {
        ...
        assertEquals("The partition's parameter map should contain the partparamkey - partparamvalue pair.", DEFAULT_PARAM_VALUE + index, part.getParameters().get(DEFAULT_PARAM_KEY + index));
        ...
        assertEquals("The parameter map of the partition's storage descriptor should contain the partsdkey - partsdvalue pair.", "partsdvalue" + index, sd.getParameters().get("partsdkey" + index));
        assertEquals("The parameter's location is not correct.", getMetaStore().getWarehouseRoot() + "/" + TABLE_NAME + "/" + name, sd.getLocation());
        ...
        Assert.assertFalse("The partition should not inherit the table parameters.", part.getParameters().keySet().contains(table.getParameters().keySet()));
    }
    ...
    protected void verifyPartitionAttributesDefaultValues(Partition partition, String tableLocation)
    {
        ...
        assertEquals("The input format doesn't have the default value.", "org.apache.hadoop.hive.ql.io.HiveInputFormat", sd.getInputFormat());
        assertEquals("The output format doesn't have the default value.", "org.apache.hadoop.hive.ql.io.HiveOutputFormat", sd.getOutputFormat());
        ...
        assertEquals("The serde's 'serializationLib' attribute doesn't have the default value.", "org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe", serdeInfo.getSerializationLib());
    ...
    public void testAddPartitionNullLocationInTableToo() throws Exception
    {
        ...
        assertEquals(SessionState.get().getTempTables().get(DB_NAME).get(TABLE_NAME).getSd().getLocation() + "/year=2017", part.getSd().getLocation());
    }
    ...
    public void testAddPartitionForExternalTableNullLocation() throws Exception
    {
        ...
        assertEquals(SessionState.get().getTempTables().get(DB_NAME).get(tableName).getSd().getLocation() + "/year=2017", resultPart.getSd().getLocation());
    }
    ...
    public void testAddPartitionsForExternalTableNullLocation() throws Exception
    {
        ...
        List<Partition> resultParts = getClient().getPartitionsByNames(DB_NAME, tableName, Lists.newArrayList("year=2017", "year=2018"));
