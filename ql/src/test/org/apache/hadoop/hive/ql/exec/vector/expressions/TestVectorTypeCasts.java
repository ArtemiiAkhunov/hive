...
public class TestVectorTypeCasts
{
    ...
    public void testCastLongToTimestamp() throws HiveException
    {
        ...
        {
            Timestamp timestamp = resultV.asScratchTimestamp(i);
            long actual = TimestampWritableV2.getLong(org.apache.hadoop.hive.common.type.Timestamp.ofEpochMilli(timestamp.getTime(), timestamp.getNanos()));
    ...
    public void testCastTimestampToString() throws HiveException
    {
        ...
        {
            expected = org.apache.hadoop.hive.common.type.Timestamp.ofEpochSecond(epochSecondValues[i], nanoValues[i]).toString();
            byte[] subbyte = Arrays.copyOfRange(resultV.vector[i], resultV.start[i], resultV.start[i] + resultV.length[i]);
            result = new String(subbyte, StandardCharsets.UTF_8);
            Assert.assertEquals("Index: " + i + " Seconds since epoch: " + epochSecondValues[i] + " nanoseconds: " + nanoValues[i], expected, result);
        }
    }
