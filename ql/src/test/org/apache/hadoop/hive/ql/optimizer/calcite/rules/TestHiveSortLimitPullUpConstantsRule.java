...
public class TestHiveSortLimitPullUpConstantsRule
{
    ...
    {
        planner = buildPlanner(Arrays.asList(HiveSortPullUpConstantsRule.SORT_LIMIT_INSTANCE, HiveProjectMergeRule.INSTANCE));
    ...
    public void testNonNullableFields()
    {
        ...
        final RelNode plan = relBuilder.scan("t").filter(eq(relBuilder, "f1", 1)).sort(relBuilder.field("f1"), relBuilder.field("f2")).project(relBuilder.field("f1"), relBuilder.field("f2")).build();
