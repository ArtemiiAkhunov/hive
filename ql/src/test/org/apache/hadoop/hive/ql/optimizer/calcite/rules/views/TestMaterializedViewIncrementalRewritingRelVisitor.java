...
public class TestMaterializedViewIncrementalRewritingRelVisitor extends TestRuleBase
{
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasUnsupportedOperator()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).sort(1) // Order by is not supported
                .build();
    ...
    public void testIncrementalRebuildIsInsertOnlyWhenPlanHasFilter()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).filter(REX_BUILDER.makeCall(SqlStdOperatorTable.IS_NOT_NULL, REX_BUILDER.makeInputRef(ts1, 0))).build();
    ...
    public void testIncrementalRebuildIsInsertOnlyWhenPlanHasInnerJoin()
    {
        ...
        RexNode joinCondition = REX_BUILDER.makeCall(SqlStdOperatorTable.EQUALS, REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0), REX_BUILDER.makeInputRef(ts2.getRowType().getFieldList().get(0).getType(), 5));

        RelNode mvQueryPlan = REL_BUILDER.push(ts1).push(ts2).join(JoinRelType.INNER, joinCondition).build();
    ...
    {
        RexNode joinCondition = REX_BUILDER.makeCall(SqlStdOperatorTable.EQUALS, REX_BUILDER.makeInputRef(left.getRowType().getFieldList().get(0).getType(), 0), REX_BUILDER.makeInputRef(right.getRowType().getFieldList().get(0).getType(), 5));

        RelNode mvQueryPlan = REL_BUILDER.push(left).push(right).join(JoinRelType.INNER, joinCondition).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.COUNT)).build();

        MaterializedViewIncrementalRewritingRelVisitor visitor = new MaterializedViewIncrementalRewritingRelVisitor();
        assertThat(String.format("leftInput=%s, rightInput=%s, expected incremental rebuild mode=%s", left, right, expected), visitor.go(mvQueryPlan).getIncrementalRebuildMode(), is(expected));
    }
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasJoinOtherThanInner()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).push(ts2).join(JoinRelType.LEFT, joinCondition).build();
    ...
    public void testIncrementalRebuildIsInsertOnlyWhenPlanHasAggregate()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.SUM, REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0))).build();
    ...
    public void testIncrementalRebuildIsAvailableWhenPlanHasAggregateAndCountStar()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.SUM, REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0)), REL_BUILDER.aggregateCall(SqlStdOperatorTable.COUNT)).build();
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasAggregateAvg()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.AVG, REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0))).build();
    ...
    public void testIncrementalRebuildIsInsertOnlyWhenPlanHasAggregateAvgCountSumOfTheSameColumn()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.COUNT, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.SUM, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.AVG, rexInputRef)).build();
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasAggregateAvgCountSumButOfDifferentColumns()
    {
        ...
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.COUNT, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.SUM, rexInputRef2), REL_BUILDER.aggregateCall(SqlStdOperatorTable.AVG, rexInputRef)).build();
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasNotSupportedAggregateCall()
    {
        ...
        RexInputRef rexInputRef = REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0);
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.STDDEV, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.SUM, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.COUNT, rexInputRef)).build();
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasBothSupportedAndNotSupportedAggregateCall()
    {
        ...
        RexInputRef rexInputRef = REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0);
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.STDDEV, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.MIN, rexInputRef)).build();
    ...
    public void testIncrementalRebuildIsInsertOnlyWhenPlanHasBothSupportedAggregateCallAndOneThatSupportsInsertOnly()
    {
        ...
        RexInputRef rexInputRef = REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0);
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), REL_BUILDER.aggregateCall(SqlStdOperatorTable.MIN, rexInputRef), REL_BUILDER.aggregateCall(SqlStdOperatorTable.COUNT)).build();
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasCountDistinct()
    {
        ...
        AggregateCall aggregateCall = AggregateCall.create(SqlStdOperatorTable.COUNT, true, false, false, Collections.emptyList(), -1, RelCollations.EMPTY, countRetType, null);
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(0), Collections.singletonList(aggregateCall)).build();
    ...
    public void testIncrementalRebuildIsNotAvailableWhenPlanHasUnsupportedAggregateOnSubPlanSupportsInsertOnly()
    {
        ...
        RexInputRef rexInputRef = REX_BUILDER.makeInputRef(ts1.getRowType().getFieldList().get(0).getType(), 0);
        RelNode mvQueryPlan = REL_BUILDER.push(ts1).aggregate(REL_BUILDER.groupKey(), REL_BUILDER.aggregateCall(SqlStdOperatorTable.STDDEV, rexInputRef)).build();
