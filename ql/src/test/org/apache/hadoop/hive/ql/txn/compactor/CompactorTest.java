...
public abstract class CompactorTest
{
    ...
    protected void burnThroughTransactions(String dbName, String tblName, int num, Set<Long> open, Set<Long> aborted, LockRequest lockReq) throws MetaException, NoSuchTxnException, TxnAbortedException
    {
        ...
        for (long tid : rsp.getTxn_ids())
        {
            ...
            {
                txnHandler.addWriteIdsToMinHistory(tid, Collections.singletonMap(dbName + "." + tblName, minOpenWriteId));
            }
        }
    }
    ...
    private void addFile(Table t, Partition p, long minTxn, long maxTxn, int numRecords, FileType type, int numBuckets, boolean allBucketsPresent, long visibilityId) throws Exception
    {
        ...
        {
            case BASE:
                filename = AcidUtils.addVisibilitySuffix(AcidUtils.BASE_PREFIX + maxTxn, visibilityId);
                break;
            case LENGTH_FILE: // Fall through to delta
            case DELTA:
                filename = AcidUtils.addVisibilitySuffix(makeDeltaDirName(minTxn, maxTxn), visibilityId);
                break;
            case LEGACY:
                break; // handled below
        }
    ...
    static class MockInputFormat implements AcidInputFormat<WritableComparable, Text>
    {
        ...
        @Override
        public AcidInputFormat.RowReader<Text> getReader(InputSplit split, Options options) throws IOException
        {
            return null;
        }

        @Override
        public RawReader<Text> getRawReader(Configuration conf, boolean collapseEvents, int bucket, ValidWriteIdList validWriteIdList, Path baseDirectory, Path[] deltaDirectory, Map<String, Integer> deltaToAttemptId) throws IOException
        {
