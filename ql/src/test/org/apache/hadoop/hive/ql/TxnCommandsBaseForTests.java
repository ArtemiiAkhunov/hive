...
public abstract class TxnCommandsBaseForTests
{
    ...
    public enum Table
    {
        ACIDTBL("acidTbl"), ACIDTBLPART("acidTblPart"), ACIDTBLNESTEDPART("acidTblNestedPart"), ACIDTBL2("acidTbl2"), NONACIDORCTBL("nonAcidOrcTbl"), NONACIDORCTBL2("nonAcidOrcTbl2"), NONACIDNONBUCKET("nonAcidNonBucket");
    ...
    void checkExpected(List<String> rs, String[][] expected, String msg, Logger LOG, boolean checkFileName)
    {
        ...
        logResult(LOG, rs);
        Assert.assertEquals(testName.getMethodName() + ": " + msg + "; " + rs, expected.length, rs.size());
        ...
        for (int i = 0; i < expected.length; i++)
        {
            ...
            if (checkFileName)
            {
                Assert.assertTrue("Actual line(file) " + i + " file: " + rs.get(i), rs.get(i).endsWith(expected[i][1]) || rs.get(i).matches(expected[i][1]));
            }
        }
    }
