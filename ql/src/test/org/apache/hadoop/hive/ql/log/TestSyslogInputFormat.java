...
public class TestSyslogInputFormat
{
    ...
    public void testTimestampFilePruningAllBefore()
    {
        ...
        try
        {
            ...
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2018-01-03 10:00:00.000"), Timestamp.valueOf("2019-01-03 10:00:00.000")).build();
            ...
            List<String> gotFiles = selectedFiles.stream().map(f -> f.getPath().toString()).collect(Collectors.toList());
    ...
    public void testTimestampFilePruningAllAfter()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2020-01-03 10:00:00.000"), Timestamp.valueOf("2020-01-03 10:00:00.000")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-10_0.log.gz"));
            FileStatus fs2 = new FileStatus();
    ...
    public void testTimestampFilePruningSimple()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 10:15:00.000"), Timestamp.valueOf("2019-01-03 11:15:00.000")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            ...
            fs6.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-11-35_0.log.gz"));
            List<FileStatus> inputFiles = Lists.newArrayList(fs1, fs2, fs3, fs4, fs5, fs6);
            List<String> expectedFiles = Lists.newArrayList(fs2, fs3, fs4, fs5).stream().map(f -> f.getPath().toString()).collect(Collectors.toList());
            List<FileStatus> selectedFiles = syslogInputFormat.pruneFiles(sarg, 300, inputFiles);
            List<String> gotFiles = selectedFiles.stream().map(f -> f.getPath().toString()).collect(Collectors.toList());
            Assert.assertEquals(expectedFiles, gotFiles);
        } finally
    ...
    public void testTimestampFilePruningTimeSlice()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 10:16:00.000"), Timestamp.valueOf("2019-01-03 11:16:00.000")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-10_0.log.gz"));
            FileStatus fs2 = new FileStatus();
    ...
    public void testTimestampFilePruningPredicateWithSeconds()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 10:15:03.600"), Timestamp.valueOf("2019-01-03 11:15:04.320")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-00_0.log.gz"));
            FileStatus fs2 = new FileStatus();
    ...
    public void testTimestampFilePruningPredicateWithSecondsFlipRange()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 11:15:04.320"), Timestamp.valueOf("2019-01-03 10:15:03.600")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-00_0.log.gz"));
            FileStatus fs2 = new FileStatus();
    ...
    public void testTimestampNoFilePruningUnsupportedColumnName()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().between("ts1", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 11:15:04.320"), Timestamp.valueOf("2019-01-03 10:15:03.600")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-10_0.log.gz"));
            FileStatus fs2 = new FileStatus();
            ...
            fs6.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-11-35_0.log.gz"));
            List<FileStatus> inputFiles = Lists.newArrayList(fs1, fs2, fs3, fs4, fs5, fs6);
            List<String> expectedFiles = Lists.newArrayList(fs1, fs2, fs3, fs4, fs5, fs6).stream().map(f -> f.getPath().toString()).collect(Collectors.toList());
            List<FileStatus> selectedFiles = syslogInputFormat.pruneFiles(sarg, 300, inputFiles);
            List<String> gotFiles = selectedFiles.stream().map(f -> f.getPath().toString()).collect(Collectors.toList());
            Assert.assertEquals(expectedFiles, gotFiles);
        } finally
    ...
    public void testTimestampNoFilePruningUnsupportedExpression()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().equals("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 11:15:04.320")).build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-10_0.log.gz"));
            FileStatus fs2 = new FileStatus();
    ...
    public void testTimestampNoFilePruningUnsupportedMultiExpression()
    {
        ...
        {
            TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
            SearchArgument sarg = SearchArgumentFactory.newBuilder().startOr().equals("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2019-01-03 11:15:04.320")).equals("ts", PredicateLeaf.Type.TIMESTAMP, Timestamp.valueOf("2020-01-03 11:15:04.320")).end().build();
            SyslogInputFormat syslogInputFormat = new SyslogInputFormat();
            FileStatus fs1 = new FileStatus();
            fs1.setPath(new Path("s3a://bucket/logs/dt=2019-01-01/ns=foo/app=hs2/2019-01-03-10-10_0.log.gz"));
            FileStatus fs2 = new FileStatus();
