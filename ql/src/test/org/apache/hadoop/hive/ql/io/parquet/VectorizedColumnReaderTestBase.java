...
public class VectorizedColumnReaderTestBase
{
    ...
    protected static ParquetWriter<Group> initWriterFromFile() throws IOException
    {
        ...
        return new ParquetWriter<>(file, new GroupWriteSupport(), GZIP, 1024 * 1024, 1024, 1024 * 1024, true, false, PARQUET_1_0, conf);
    }
    ...
    protected void longReadInt(boolean isDictionaryEncoding) throws InterruptedException, HiveException, IOException
    {
    ...
    private void intRead(boolean isDictionaryEncoding, Configuration conf) throws InterruptedException, HiveException, IOException
    {
        VectorizedParquetRecordReader reader = createTestParquetReader("message test { required int32 int32_field;}", conf);
        VectorizedRowBatch previous = reader.createValue();
        try
    ...
    {
        VectorizedParquetRecordReader reader = createTestParquetReader("message test { required double double_field;}", conf);
        VectorizedRowBatch previous = reader.createValue();
        try
        {
            int c = 0;
            while (reader.next(NullWritable.get(), previous))
            {
                ...
                for (int i = 0; i < vector.vector.length; i++)
                {
                    ...
                    assertEquals("Failed at " + c, getDoubleValue(isDictionaryEncoding, c), vector.vector[i], 0);
                    assertFalse(vector.isNull[i]);
                    c++;
                }
            }
    ...
    protected void binaryRead(boolean isDictionaryEncoding) throws Exception
    {
        ...
        conf.setBoolean(ColumnProjectionUtils.READ_ALL_COLUMNS, false);
        conf.set(ColumnProjectionUtils.READ_COLUMN_IDS_CONF_STR, "0");
        VectorizedParquetRecordReader reader = createTestParquetReader("message test { required binary binary_field_some_null;}", conf);
        VectorizedRowBatch previous = reader.createValue();
        int c = 0;
        try
        {
