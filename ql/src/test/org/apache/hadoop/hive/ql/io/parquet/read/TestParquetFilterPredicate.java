...
public class TestParquetFilterPredicate
{
    ...
    public void testFilterColumnsThatDoNoExistOnSchema()
    {
        ...
        SearchArgument sarg = SearchArgumentFactory.newBuilder().startNot().startOr().isNull("a", PredicateLeaf.Type.LONG).between("y", PredicateLeaf.Type.LONG, 10L, 20L) // Column will be removed from filter
                .in("z", PredicateLeaf.Type.LONG, 1L, 2L, 3L) // Column will be removed from filter
                .nullSafeEquals("stinger", PredicateLeaf.Type.STRING, "stinger").end().end().build();
    ...
    {
        SearchArgument sarg = SearchArgumentFactory.newBuilder().between("bCol", PredicateLeaf.Type.TIMESTAMP, new Timestamp(1667586661), new Timestamp(1667932199)).build();
    ...
    {
        SearchArgument sarg = SearchArgumentFactory.newBuilder().between("bCol", PredicateLeaf.Type.DECIMAL, new HiveDecimalWritable("12.21"), new HiveDecimalWritable(1667932199)).build();
    ...
    {
        SearchArgument sarg = SearchArgumentFactory.newBuilder().between("bCol", PredicateLeaf.Type.DATE, new Date(1667586661), new Date(1667932199)).build();
    ...
    private void testFilterBetweenHelper(SearchArgument sarg, PredicateLeaf.Type type)
    {
        ...
        HiveException exception = Assert.assertThrows(HiveException.class, () -> leafFilterFactory.getLeafFilterBuilderByType(leaf.getType(), parquetType));
