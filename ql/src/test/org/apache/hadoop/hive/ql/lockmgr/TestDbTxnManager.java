...
public class TestDbTxnManager
{
    ...
    public void testSingleReadTable() throws Exception
    {
        ...
        Assert.assertEquals(1, TestTxnDbUtil.countLockComponents(conf, ((DbLockManager.DbHiveLock) locks.get(0)).lockId));
        txnMgr.commitTxn();
    ...
    public void testSingleReadMultiPartition() throws Exception
    {
        ...
        List<HiveLock> locks = ctx.getHiveLocks();
        Assert.assertEquals(1, locks.size());
        Assert.assertEquals(3, TestTxnDbUtil.countLockComponents(conf, ((DbLockManager.DbHiveLock) locks.get(0)).lockId));
        txnMgr.commitTxn();
        locks = txnMgr.getLockManager().getLocks(false, false);
        Assert.assertEquals(0, locks.size());
    }
    ...
    public void testJoin() throws Exception
    {
        ...
        List<HiveLock> locks = ctx.getHiveLocks();
        Assert.assertEquals(1, locks.size());
        Assert.assertEquals(4, TestTxnDbUtil.countLockComponents(conf, ((DbLockManager.DbHiveLock) locks.get(0)).lockId));
        txnMgr.commitTxn();
        locks = txnMgr.getLockManager().getLocks(false, false);
        Assert.assertEquals(0, locks.size());
    }
    ...
    */// Make sure we're locking the whole table, since this is dynamic partitioning
    ...
    public void testHeartbeater() throws Exception
    {
        ...
        ((DbTxnManager) txnMgr).openTxn(ctx, "tom", TxnType.DEFAULT, MetastoreConf.getTimeVar(conf, timeThresholdConfVar, TimeUnit.MILLISECONDS) / 2);
        ...
        ((DbTxnManager) txnMgr).openTxn(ctx, "jerry", TxnType.DEFAULT, MetastoreConf.getTimeVar(conf, timeThresholdConfVar, TimeUnit.MILLISECONDS) * 2);
