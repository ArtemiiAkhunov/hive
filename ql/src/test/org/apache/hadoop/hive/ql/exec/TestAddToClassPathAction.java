...
public class TestAddToClassPathAction
{
    ...
    public void testNullPaths()
    {
        ...
        assertURLsMatch("When newPaths is null, loader shall be created normally with no extra paths.", Collections.emptyList(), childLoader.getURLs());
    }
    ...
    public void testUseExisting()
    {
        ...
        assertSame("Normally, the existing class loader should be reused (not closed, no force new).", parentLoader, childLoader);
        assertURLsMatch("The class path of the class loader should be updated.", Arrays.asList("file:/a/1", "file:/c/3", "file:/b/2", "file:/d/4"), childLoader.getURLs());
    }
    ...
    public void testClosed() throws IOException
    {
        ...
        AddToClassPathAction action2 = new AddToClassPathAction(parentLoader, Arrays.asList("/b/2", "/d/4"));
        UDFClassLoader childLoader = action2.run();
        assertNotSame("When the parent class loader is closed, a new instance must be created.", parentLoader, childLoader);
    ...
    public void testForceNew()
    {
        ...
        UDFClassLoader childLoader = action2.run();
        assertNotSame("When forceNewClassLoader is set, a new instance must be created.", parentLoader, childLoader);
        assertURLsMatch(Arrays.asList("file:/b/2", "file:/d/4"), childLoader.getURLs());
    }
