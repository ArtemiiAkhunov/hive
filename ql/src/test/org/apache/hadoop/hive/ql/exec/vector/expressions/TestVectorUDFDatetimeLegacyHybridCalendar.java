...
public class TestVectorUDFDatetimeLegacyHybridCalendar
{
    ...
    private void verifyVectorUDFDatetimeLegacyHybridCalendarTimestamp(VectorizedRowBatch batch) throws HiveException
    {
        ...
        for (int i = 0; i < batch.size; i++)
        {
            ...
            {
                java.sql.Timestamp input = ((TimestampColumnVector) batch.cols[in]).asScratchTimestamp(i);
                java.sql.Timestamp result = ((TimestampColumnVector) batch.cols[out]).asScratchTimestamp(i);
    ...
    {
        return getVectorizedRowBatch(new Long[]{(long) Date.valueOf("0001-01-01").toEpochDay(), (long) Date.valueOf("1400-01-01").toEpochDay(), (long) Date.valueOf("1500-01-01").toEpochDay(), (long) Date.valueOf("1583-01-01").toEpochDay(),}, size);
    }
