...
public class TestHiveSortExchangePullUpConstantsRule
{
    ...
    {
        planner = buildPlanner(Arrays.asList(HiveSortPullUpConstantsRule.SORT_EXCHANGE_INSTANCE, HiveProjectMergeRule.INSTANCE));
    ...
    public void testNonNullableFields()
    {
        ...
        final RelNode plan = relBuilder.scan("t").filter(eq(relBuilder, "f1", 1)).sortExchange(RelDistributions.ROUND_ROBIN_DISTRIBUTED, RelCollations.of(0)).project(relBuilder.field("f1"), relBuilder.field("f2")).build();
