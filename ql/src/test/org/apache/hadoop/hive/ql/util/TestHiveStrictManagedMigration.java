...
public class TestHiveStrictManagedMigration extends TxnCommandsBaseForTests
{
    ...
    public void testUpgrade() throws Exception
    {
        ...
        Hive.get(hiveConf).alterTable(table, false, new EnvironmentContext(ImmutableMap.of(StatsSetupConst.DO_NOT_UPDATE_STATS, StatsSetupConst.TRUE)), false);

        String oldWarehouse = getWarehouseDir();
        String[] args = {"--hiveconf", "hive.strict.managed.tables=true", "-m", "automatic", "--modifyManagedTables", "--oldWarehouseRoot", oldWarehouse};
...
*/
    ...
    public void testUsingControlFileUrl() throws Throwable
    {
        setupExternalTableTest();
        String oldWarehouse = getWarehouseDir();
        String[] args = {"-m", "external", "--oldWarehouseRoot", oldWarehouse, "--controlFileUrl", "src/test/resources/hsmm/hsmm_cfg_01.yaml"};
        HiveConf newConf = new HiveConf(hiveConf);

        runMigrationTool(newConf, args);

        verifySubsetOfTablesBecameExternal(Sets.newHashSet("manwhwh", "manwhnone"));
    }
    ...
    public void testUsingControlDirUrl() throws Throwable
    {
        setupExternalTableTest();
        String oldWarehouse = getWarehouseDir();
        String[] args = {"-m", "external", "--oldWarehouseRoot", oldWarehouse, "--controlFileUrl", "src/test/resources/hsmm"};
        HiveConf newConf = new HiveConf(hiveConf);

        runMigrationTool(newConf, args);

        verifySubsetOfTablesBecameExternal(Sets.newHashSet("manwhwh", "manwhnone", "custdb.custmanwhwh"));
    }

    private void verifySubsetOfTablesBecameExternal(Set<String> expectedExternals) throws Throwable
    {
        Set<String> allTables = Sets.newHashSet("manwhnone", "manoutnone", "manwhwh", "manwhout", "manwhmixed", "manoutout", "custdb.custmanwhwh");
    ...
    {
        Set<String> actualDirs = Files.find(Paths.get(exteralWarehouseDir.toURI()), Integer.MAX_VALUE, (p, a) -> true).map(p -> p.toString().replaceAll(exteralWarehouseDir.getAbsolutePath(), "")).filter(s -> !s.isEmpty()).collect(toSet());
        ...
        assertEquals("Unexpected external warehouse directory structure in " + exteralWarehouseDir, expectedDirs, actualDirs);

        actualDirs = Files.find(Paths.get(externalNonWhDir.toURI()), Integer.MAX_VALUE, (p, a) -> true).map(p -> p.toString().replaceAll(externalNonWhDir.getAbsolutePath(), "")).filter(s -> !s.isEmpty()).collect(toSet());
        expectedDirs.clear();
        ...
        assertEquals("Unexpected external (non-warehouse) directory structure in " + externalNonWhDir, expectedDirs, actualDirs);
    }
