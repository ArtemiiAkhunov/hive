...
public class TestCompactionMetrics extends CompactorTest
{
    ...
    public void testOldestReadyForCleaningAge() throws Exception
    {
        ...
        int oldestAgeInSeconds = Metrics.getOrCreateGauge(MetricsConstants.OLDEST_READY_FOR_CLEANING_AGE).intValue();
    ...
    {
        long cleanerCyclesMinor = Objects.requireNonNull(Metrics.getOrCreateTimer(CLEANER_CYCLE_KEY + "_" + CompactionType.MINOR.toString().toLowerCase())).getCount();
        long cleanerCyclesMajor = Objects.requireNonNull(Metrics.getOrCreateTimer(CLEANER_CYCLE_KEY + "_" + CompactionType.MAJOR.toString().toLowerCase())).getCount();
        ...
        Assert.assertEquals(cleanerCyclesMinor + 10, Objects.requireNonNull(Metrics.getOrCreateTimer(CLEANER_CYCLE_KEY + "_" + CompactionType.MINOR.toString().toLowerCase())).getCount());
        ...
        Assert.assertEquals(cleanerCyclesMajor + 10, Objects.requireNonNull(Metrics.getOrCreateTimer(CLEANER_CYCLE_KEY + "_" + CompactionType.MAJOR.toString().toLowerCase())).getCount());
    }
    ...
    public void testCleanerPerfMetricsDisabled() throws Exception
    {
        ...
        long cleanerCyclesMinor = Objects.requireNonNull(Metrics.getOrCreateTimer(CLEANER_CYCLE_KEY + "_" + CompactionType.MAJOR.toString().toLowerCase())).getCount();
        ...
        Assert.assertEquals(cleanerCyclesMinor, Objects.requireNonNull(Metrics.getOrCreateTimer(CLEANER_CYCLE_KEY + "_" + CompactionType.MAJOR.toString().toLowerCase())).getCount());
    }
    ...
    public void testWorkerPerfMetrics() throws Exception
    {
        ...
        MetricsTestUtils.verifyMetricsJson(json, MetricsTestUtils.TIMER, WORKER_CYCLE_KEY + "_" + CompactionType.MINOR.toString().toLowerCase(), 1);
        ...
        Assert.assertEquals(TxnStore.SUCCEEDED_RESPONSE, rsp.getCompacts().stream().filter(c -> c.getType().equals(CompactionType.MINOR)).findFirst().orElseThrow(() -> new RuntimeException("Could not found minor compaction")).getState());
        Assert.assertEquals(TxnStore.CLEANING_RESPONSE, rsp.getCompacts().stream().filter(c -> c.getType().equals(CompactionType.MAJOR)).findFirst().orElseThrow(() -> new RuntimeException("Could not found minor compaction")).getState());

        json = metrics.dumpJson();
        MetricsTestUtils.verifyMetricsJson(json, MetricsTestUtils.TIMER, WORKER_CYCLE_KEY + "_" + CompactionType.MAJOR.toString().toLowerCase(), 1);
    }
    ...
    public void testUpdateCompactionMetrics()
    {
        ...
        elements.add(generateElement(6, "db1", "tb", null, CompactionType.MINOR, TxnStore.FAILED_RESPONSE, System.currentTimeMillis(), true, WORKER_VERSION, WORKER_VERSION, 10));
        ...
        elements.add(generateElement(15, "db3", "tb5", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, System.currentTimeMillis(), true, WORKER_VERSION, WORKER_VERSION, 30));
        elements.add(generateElement(16, "db3", "tb6", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE));
        elements.add(generateElement(17, "db3", "tb7", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, System.currentTimeMillis(), true, WORKER_VERSION, WORKER_VERSION, 40));
    ...
    public void testWorkingAgeMetrics()
    {
        ...
        long start = System.currentTimeMillis() - 1000L;
        List<ShowCompactResponseElement> elements = ImmutableList.of(generateElement(17, "db3", "tb7", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, System.currentTimeMillis(), true, WORKER_VERSION, WORKER_VERSION, start));
    ...
    public void testCleaningAgeMetrics()
    {
        ...
        long start = System.currentTimeMillis() - 1000L;
        List<ShowCompactResponseElement> elements = ImmutableList.of(generateElement(19, "db3", "tb7", null, CompactionType.MINOR, TxnStore.CLEANING_RESPONSE, System.currentTimeMillis(), true, WORKER_VERSION, WORKER_VERSION, -1L, start));
    ...
    public void testWorkingAgeMetricsOrder()
    {
        ...
        List<ShowCompactResponseElement> elements = ImmutableList.of(generateElement(15, "db3", "tb5", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, start, false, WORKER_VERSION, WORKER_VERSION, start - 1_000L), generateElement(16, "db3", "tb6", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, start, false, WORKER_VERSION, WORKER_VERSION, start - 15_000L));
        ...
        // Check the reverse order
        elements = ImmutableList.of(generateElement(16, "db3", "tb6", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, start, false, WORKER_VERSION, WORKER_VERSION, start - 25_000L), generateElement(15, "db3", "tb5", null, CompactionType.MINOR, TxnStore.WORKING_RESPONSE, start, false, WORKER_VERSION, WORKER_VERSION, start - 1_000L));
        scr.setCompacts(elements);
        AcidMetricService.updateMetricsFromShowCompact(scr);
    ...
    public void testTxnHandlerCounters() throws Exception
    {
        ...
        Assert.assertEquals(MetricsConstants.TOTAL_NUM_ABORTED_TXNS + " value incorrect", 2, Metrics.getOrCreateCounter(MetricsConstants.TOTAL_NUM_ABORTED_TXNS).getCount());
        Assert.assertEquals(MetricsConstants.TOTAL_NUM_COMMITTED_TXNS + " value incorrect", 1, Metrics.getOrCreateCounter(MetricsConstants.TOTAL_NUM_COMMITTED_TXNS).getCount());

        burnThroughTransactions(t.getDbName(), t.getTableName(), 3, null, new HashSet<>(Collections.singletonList(4L)));
        Assert.assertEquals(MetricsConstants.TOTAL_NUM_ABORTED_TXNS + " value incorrect", 3, Metrics.getOrCreateCounter(MetricsConstants.TOTAL_NUM_ABORTED_TXNS).getCount());
        Assert.assertEquals(MetricsConstants.TOTAL_NUM_COMMITTED_TXNS + " value incorrect", 3, Metrics.getOrCreateCounter(MetricsConstants.TOTAL_NUM_COMMITTED_TXNS).getCount());
    }
    ...
    public void testTablesWithXAbortedTxns() throws Exception
    {
        ...
        Assert.assertEquals(MetricsConstants.TABLES_WITH_X_ABORTED_TXNS + " value incorrect", 2, Metrics.getOrCreateGauge(MetricsConstants.TABLES_WITH_X_ABORTED_TXNS).intValue());
    }
    ...
    public void testWritesToDisabledCompactionTable() throws Exception
    {
        ...
        burnThroughTransactions(disabledTbl.getDbName(), disabledTbl.getTableName(), 1, null, new HashSet<>(Collections.singletonList(2L)));
        ...
        Assert.assertEquals(MetricsConstants.WRITES_TO_DISABLED_COMPACTION_TABLE + " value incorrect", 2, Metrics.getOrCreateGauge(MetricsConstants.WRITES_TO_DISABLED_COMPACTION_TABLE).intValue());
    }
    ...
    public void testCleanerDurationMeasuredCorrectly() throws Exception
    {
        ...
        int cleanerDurationFromMetric = Metrics.getOrCreateGauge(MetricsConstants.COMPACTION_CLEANER_CYCLE_DURATION).intValue();
        Assert.assertTrue("Cleaner duration must be withing the limits", (0 < cleanerDurationFromMetric) && (cleanerDurationFromMetric <= durationUpperLimit));
    }
    ...
    public void testCleanerFailuresCountedCorrectly() throws Exception
    {
        ...
        Assert.assertEquals(EXPECTED_FAIL_COUNT, Metrics.getOrCreateCounter(MetricsConstants.COMPACTION_CLEANER_FAILURE_COUNTER).getCount());
    }
    ...
    {
        return generateElement(id, db, table, partition, type, state, enqueueTime, manuallyInitiatedCompaction, null, null, -1);
    }
    ...
    {
        return generateElement(id, db, table, partition, type, state, enqueueTime, manuallyInitiatedCompaction, initiatorVersion, workerVersion, startTime, -1L);
    }
