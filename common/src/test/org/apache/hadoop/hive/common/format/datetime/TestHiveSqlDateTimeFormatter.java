...
public class TestHiveSqlDateTimeFormatter
{
    ...
    {
        verifyPatternParsing(" ---yyyy-\'-:-  -,.;/MM-dd--", Arrays.asList(null, // represents separator, which has no temporal field
                ChronoField.YEAR, null, ChronoField.MONTH_OF_YEAR, null, ChronoField.DAY_OF_MONTH, null));

        verifyPatternParsing("ymmdddhh24::mi:ss A.M. pm", 25, "ymmdddhh24::mi:ss A.M. pm", Arrays.asList(ChronoField.YEAR, ChronoField.MONTH_OF_YEAR, ChronoField.DAY_OF_YEAR, ChronoField.HOUR_OF_DAY, null, ChronoField.MINUTE_OF_HOUR, null, ChronoField.SECOND_OF_MINUTE, null, ChronoField.AMPM_OF_DAY, null, ChronoField.AMPM_OF_DAY));
    }
    ...
    {
        formatter = new HiveSqlDateTimeFormatter(pattern, false, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
        assertEquals("Format timestamp to string failed with pattern: " + pattern, expectedOutput, formatter.format(Timestamp.valueOf(input)));
    }
    ...
    {
        formatter = new HiveSqlDateTimeFormatter(pattern, false, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
        assertEquals("Format date to string failed with pattern: " + pattern, expectedOutput, formatter.format(Date.valueOf(input)));
    }
    ...
    public void testParseTimestamp()
    {
        ...
        checkParseTimestamp("MI DD-TZM-YYYY-MM TZHPM SS:HH12.FF9", "59 03-30-2017-05 01PM 01:08.123456789", "2017-05-03 20:59:01.123456789");
    ...
    {
        formatter = new HiveSqlDateTimeFormatter(pattern, true, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
        assertEquals("Parse string to timestamp failed. Pattern: " + pattern, Timestamp.valueOf(expectedOutput), formatter.parseTimestamp(input));
    }

    private void checkParseTimestampIso(String parsePattern, String input, String formatPattern, String expectedOutput)
    {
        formatter = new HiveSqlDateTimeFormatter(parsePattern, true, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
        Timestamp ts = formatter.parseTimestamp(input);
        formatter = new HiveSqlDateTimeFormatter(formatPattern, false, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
    ...
    {
        formatter = new HiveSqlDateTimeFormatter(pattern, true, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
        assertEquals("Parse string to date failed. Pattern: " + pattern, Date.valueOf(expectedOutput), formatter.parseDate(input));
    }
    ...
    private void verifyBadPattern(String string, boolean forParsing)
    {
        try
        {
            formatter = new HiveSqlDateTimeFormatter(string, forParsing, Optional.of(LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC)));
            fail("Bad pattern " + string + " should have thrown IllegalArgumentException but didn't");
        } catch (Exception e)
        {
            assertEquals("Expected IllegalArgumentException, got another exception.", e.getClass().getName(), IllegalArgumentException.class.getName());
        }
    }
    ...
    public void testText()
    {
        ...
        checkParseTimestamp("\"Year \"YYYY \"month\" MM \"day\" DD.\"!\"", "YEaR 3000 mOnTh 3 DaY 1...!", "3000-03-01 00:00:00");
        checkParseDate("\"Year \"YYYY \"month\" MM \"day\" DD.\"!\"", "YEaR 3000 mOnTh 3 DaY 1...!", "3000-03-01");
        ...
        checkParseTimestamp("\"Y\'ear \"YYYY \' \"month\" MM \"day\" DD.\"!\"", "Y'EaR 3000 ' mOnTh 3 DaY 1...!", "3000-03-01 00:00:00");
        checkParseDate("\"Y\'ear \"YYYY \' \"month\" MM \"day\" DD.\"!\"", "Y'EaR 3000 ' mOnTh 3 DaY 1...!", "3000-03-01");
        // literal double quotes are escaped
        checkFormatTs("\"the \\\"DATE\\\" is\" yyyy-mm-dd", "2018-01-01 00:00:00", "the \"DATE\" is 2018-01-01");
    ...
    private void verifyPatternParsing(String pattern, int expectedPatternLength, String expectedPattern, List<TemporalField> temporalFields)
    {
        ...
        {
            assertEquals("Generated list of tokens not correct", temporalFields.get(i), formatter.getTokens().get(i).temporalField);
    ...
    private void verifyBadParseString(String pattern, String string)
    {
        ...
        {
            assertEquals("Expected IllegalArgumentException, got another exception:" + e, IllegalArgumentException.class.getName(), e.getClass().getName());
        }
    }
}
