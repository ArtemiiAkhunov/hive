...
public class RetryTestRunner extends BlockJUnit4ClassRunner
{
    ...
    private void retry(final Description description, final EachTestNotifier notifier, final Statement statement, final Throwable currentThrowable)
    {
        ...
        while (retryCount > failedAttempts)
        {
            try
            {
                System.out.println(description + " Caught: " + (currentThrowable == null ? "exception" : currentThrowable.getMessage()) + ". Retrying test " + failedAttempts + "/" + retryCount);
                statement.evaluate();
                return;
            } catch (Throwable t)
            {
