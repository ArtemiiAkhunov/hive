...
public class HiveAuthUtils
{
    ...
    public static <T extends TTransport> T configureThriftMaxMessageSize(T transport, int maxMessageSize)
    {
        if (maxMessageSize > 0)
        {
            ...
            {
                LOG.warn("TTransport {} is returning a null Configuration, Thrift max message size is not getting configured", transport.getClass().getName());
                return transport;
            }
    ...
    public static TTransport getSSLSocket(String host, int port, int loginTimeout, TSSLTransportParameters params, int maxMessageSize) throws TTransportException
    {
    ...
    {
        return getSSLSocket(host, port, loginTimeout, trustStorePath, trustStorePassWord, trustStoreType, trustStoreAlgorithm, /* maxMessageSize */ -1);
    }
