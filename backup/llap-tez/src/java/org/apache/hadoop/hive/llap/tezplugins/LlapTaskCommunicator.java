...
public class LlapTaskCommunicator extends TezTaskCommunicatorImpl
{
    ...
    private boolean isInvalidTokenError(Throwable cause)
    {
         ...
         */
        return ((cause instanceof InvalidToken && cause.getMessage() != null) || (cause instanceof RemoteException && cause.getCause() == null && cause.getMessage() != null && cause.getMessage().contains(LLAP_TOKEN_NAME) && (cause.getMessage().contains("InvalidToken") || cause.getMessage().contains("can't be found in cache"))));
    }
    ...
    public void initialize() throws Exception
    {
        ...
        this.deleteDelayOnDagComplete = HiveConf.getTimeVar(conf, ConfVars.LLAP_FILE_CLEANUP_DELAY_SECONDS, TimeUnit.SECONDS);
    ...
    protected void startRpcServer()
    {
        ...
        try
        {
            ...
            boolean isHadoopSecurityAuthorizationEnabled = conf.getBoolean(CommonConfigurationKeysPublic.HADOOP_SECURITY_AUTHORIZATION, false);
            ...
            if (portRange.length == 1)
            {
                ...
                startServerInternal(conf, minPort, numHandlers, jobTokenSecretManager, isHadoopSecurityAuthorizationEnabled);
                portFound = true;
            ...
            } else
            {
                ...
                for (int i = minPort; i < maxPort; i++)
                {
                    try
                    {
                        startServerInternal(conf, i, numHandlers, jobTokenSecretManager, isHadoopSecurityAuthorizationEnabled);
                        portFound = true;
    ...
    {
        server = new RPC.Builder(conf).setProtocol(LlapTaskUmbilicalProtocol.class).setBindAddress("0.0.0.0").setPort(umbilicalPort).setInstance(umbilical).setNumHandlers(numHandlers).setSecretManager(jobTokenSecretManager).build();
    ...
    public boolean registerDag(NodeInfo node, final OperationCallback<QueryIdentifierProto, Void> callback)
    {
        ...
        try
        {
            RegisterDagRequestProto request = builder.setQueryIdentifier(currentQueryIdentifierProto).setUser(user).setCredentialsBinary(getCredentials(getContext().getCurrentDagInfo().getCredentials())).build();
            communicator.registerDag(request, node.getHost(), node.getRpcPort(), new LlapProtocolClientProxy.ExecuteRequestCallback<RegisterDagResponseProto>()
            {
                @Override
                ...
                {
                    LOG.info("Error registering dag with" + " appId=" + currentQueryIdentifierProto.getApplicationIdString() + " dagId=" + currentQueryIdentifierProto.getDagIndex() + " to node " + node.getHost());
                    processSendError(t);
    ...
    public <T> void startUpdateGuaranteed(TezTaskAttemptID attemptId, NodeInfo assignedNode, boolean newState, final OperationCallback<Boolean, T> callback, final T ctx)
    {
        ...
        UpdateFragmentRequestProto request = UpdateFragmentRequestProto.newBuilder().setIsGuaranteed(newState).setFragmentIdentifierString(attemptId.toString()).setQueryIdentifier(constructQueryIdentifierProto(attemptId.getDAGID().getId())).build();

        communicator.sendUpdateFragment(request, nodeId.getHostname(), nodeId.getPort(), new LlapProtocolClientProxy.ExecuteRequestCallback<UpdateFragmentResponseProto>()
        {
            @Override
    ...
    private void sendTaskTerminated(final TezTaskAttemptID taskAttemptId, boolean invokedByContainerEnd)
    {
        ...
        if (nodeId != null)
        {
            TerminateFragmentRequestProto request = TerminateFragmentRequestProto.newBuilder().setQueryIdentifier(constructQueryIdentifierProto(taskAttemptId.getDAGID().getId())).setFragmentIdentifierString(taskAttemptId.toString()).build();
    ...
    public void onVertexStateUpdated(VertexStateUpdate vertexStateUpdate)
    {
        ...
        sourceStateTracker.sourceStateUpdated(vertexStateUpdate.getVertexName(), vertexStateUpdate.getVertexState());
    }
    ...
    public void sendStateUpdate(final LlapNodeId nodeId, final SourceStateUpdatedRequestProto request)
    {
    ...
    private String constructLlapLogUrl(final TezTaskAttemptID attemptID, final String containerIdString, final boolean isDone, final String nmAddress)
    {
        String dagId = attemptID.getDAGID().toString();
        String filename = JOINER.join(currentHiveQueryId, "-", dagId, ".log", (isDone ? ".done" : ""), "?nm.id=", nmAddress);
        String url = PATH_JOINER.join(timelineServerUri, "ws", "v1", "applicationhistory", "containers", containerIdString, "logs", filename);
        return url;
    }
    ...
    private void resetCurrentDag(int newDagId, String hiveQueryId)
    {
        ...
        nodesForQuery.clear();
        LOG.info("CurrentDagId set to: " + newDagId + ", name=" + getContext().getCurrentDagInfo().getName() + ", queryId=" + hiveQueryId);
    ...
    private SubmitWorkRequestProto constructSubmitWorkRequest(ContainerId containerId, TaskSpec taskSpec, FragmentRuntimeInfo fragmentRuntimeInfo, String hiveQueryId) throws IOException
    {
        ...
        Preconditions.checkState(currentQueryIdentifierProto.getDagIndex() == taskSpec.getDAGID().getId());


        builder.setCredentialsBinary(getCredentials(getContext().getCurrentDagInfo().getCredentials()));
    ...
    protected class LlapTaskUmbilicalProtocolImpl implements LlapTaskUmbilicalProtocol
    {
        ...
        @Override
        public void nodeHeartbeat(Text hostname, Text uniqueId, int port, TezAttemptArray aw, BooleanArray guaranteed) throws IOException
        {
        ...
        public void taskKilled(TezTaskAttemptID taskAttemptId) throws IOException
        {
            ...
            getContext().taskKilled(taskAttemptId, TaskAttemptEndReason.EXTERNAL_PREEMPTION, "Attempt preempted");
            entityTracker.unregisterTaskAttempt(taskAttemptId);
        }
    ...
    {
        return QueryIdentifierProto.newBuilder().setApplicationIdString(getContext().getCurrentAppIdentifier()).setDagIndex(dagIdentifier).setAppAttemptNumber(getContext().getApplicationAttemptId().getAttemptId()).build();
    }
