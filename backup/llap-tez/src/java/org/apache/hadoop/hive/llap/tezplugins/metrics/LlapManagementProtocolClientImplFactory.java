...
public class LlapManagementProtocolClientImplFactory
{
    ...
    {
        return new LlapManagementProtocolClientImplFactory(conf, RetryPolicies.retryUpToMaximumCountWithFixedSleep(5, 3000L, TimeUnit.MILLISECONDS), NetUtils.getDefaultSocketFactory(conf));
    }

    public LlapManagementProtocolClientImpl create(LlapServiceInstance serviceInstance)
    {
        return new LlapManagementProtocolClientImpl(conf, serviceInstance.getHost(), serviceInstance.getManagementPort(), retryPolicy, socketFactory);
    }
}
