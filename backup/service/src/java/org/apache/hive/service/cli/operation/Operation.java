...
public abstract class Operation
{
    ...
    protected Operation(HiveSession parentSession, Map<String, String> confOverlay, OperationType opType, boolean embedded)
    {
        ...
        operationTimeout = HiveConf.getTimeVar(parentSession.getHiveConf(), HiveConf.ConfVars.HIVE_SERVER2_IDLE_OPERATION_TIMEOUT, TimeUnit.MILLISECONDS);

        currentStateScope = updateOperationStateMetrics(null, MetricsConstant.OPERATION_PREFIX, MetricsConstant.COMPLETED_OPERATION_PREFIX, state);
        queryState = new QueryState.Builder().withConfOverlay(confOverlay).withGenerateNewQueryId(!embedded).withHiveConf(parentSession.getHiveConf()).build();
    }
    ...
    {
        ShimLoader.getHadoopShims().setHadoopQueryContext(String.format(USER_ID, queryState.getQueryId(), parentSession.getUserName()));
        if (!embedded)
        {
            createOperationLog();
        ...
        log.info("[opType={}, queryId={}, startTime={}, sessionId={}, createTime={}, userName={}, ipAddress={}]", opHandle.getOperationType(), queryState.getQueryId(), beginTime, parentSession.getSessionState().getSessionId(), parentSession.getCreationTime(), parentSession.getUserName(), parentSession.getIpAddress());
    }
    ...
    protected void afterRun()
    {
        ...
        ShimLoader.getHadoopShims().setHadoopSessionContext(String.format(USER_ID, parentSession.getSessionState().getSessionId(), parentSession.getUserName()));
    }
        ...
        if (isOperationLogEnabled)
        {
            ...
            } else
            {
                if (operationLogCleanupDelayMs > 0)
                {
                    ...
                    scheduledExecutorService.schedule(new OperationLogCleaner(this, operationLog), operationLogCleanupDelayMs, TimeUnit.MILLISECONDS);
    ...
    {
        HiveSQLException ex = new HiveSQLException(prefix + ": " + e.getMessage(), e.getSqlState(), e.getResponseCode());
        if (e.getCause() != null)
        {
