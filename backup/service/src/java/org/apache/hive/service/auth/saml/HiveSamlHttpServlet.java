...
public class HiveSamlHttpServlet extends HttpServlet
{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            LOG.info("RelayState = {}. Driver side port on loopback address is {}", relayState, port);
        ...
        } catch (HttpSamlAuthenticationException e)
        {
            ...
            generateFormData(response, HiveSamlUtils.getLoopBackAddress(port), null, false, "SAML assertion could not be validated. Check server logs for more details.");
            return;
        }
        Preconditions.checkState(nameId != null);
        LOG.info("Successfully validated saml response for user {}. Forwarding the token to port {}", nameId, port);
        generateFormData(response, HiveSamlUtils.getLoopBackAddress(port), tokenGenerator.get(nameId, relayState), true, "");
    }
    ...
    private void generateFormData(HttpServletResponse response, String url, String token, boolean success, String msg)
    {
        ...
        sb.append(String.format("<input type='hidden' name='%s' value='%s'>", HiveSamlUtils.TOKEN_KEY, token));
        sb.append(String.format("<input type='hidden' name='%s' value='%s'>", HiveSamlUtils.STATUS_KEY, success));
        sb.append(String.format("<input type='hidden' name='%s' value='%s'>", HiveSamlUtils.MESSAGE_KEY, msg));
        sb.append("</form>");
        sb.append("</body>");
        sb.append("</html>");
        ...
        {
            LOG.error("Could not generate the form data for sending a response to url " + url, e);
