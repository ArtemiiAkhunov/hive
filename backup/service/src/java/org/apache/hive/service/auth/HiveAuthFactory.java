...
public class HiveAuthFactory
{
    ...
    public HiveAuthFactory(HiveConf conf, boolean isHttpMode) throws TTransportException
    {
        ...
        {
            saslServer = HadoopThriftAuthBridge.getBridge().createServer(conf.getVar(ConfVars.HIVE_SERVER2_KERBEROS_KEYTAB), conf.getVar(ConfVars.HIVE_SERVER2_KERBEROS_PRINCIPAL), conf.getVar(ConfVars.HIVE_SERVER2_CLIENT_KERBEROS_PRINCIPAL));
    ...
    {
        return "kerberos".equalsIgnoreCase(hadoopAuth) && !authType.isEnabled(HiveAuthConstants.AuthTypes.NOSASL);
    }

    public boolean isSASLKerberosUser()
    {
        return AuthMethod.KERBEROS.getMechanismName().equals(getUserAuthMechanism()) || AuthMethod.TOKEN.getMechanismName().equals(getUserAuthMechanism());
    }
    ...
    public String getDelegationToken(String owner, String renewer, String remoteAddr) throws HiveSQLException
    {
        ...
        try
        {
            String tokenStr = delegationTokenManager.getDelegationTokenWithService(owner, renewer, HiveAuthConstants.HS2_CLIENT_TOKEN, remoteAddr);
