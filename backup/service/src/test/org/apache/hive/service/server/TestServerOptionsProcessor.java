...
public class TestServerOptionsProcessor
{
    ...
    public void test()
    {
        ...
        Assert.assertEquals("checking system property before processing options", null, HiveConf.overrides.get(key));

        optProcessor.parse(args);

        Assert.assertEquals("checking system property after processing options", value, HiveConf.overrides.get(key));
    }
