...
public class TestHS2HttpServer
{
    ...
    {
        webUIPort = MetaStoreTestUtils.findFreePortExcepting(Integer.valueOf(ConfVars.HIVE_SERVER2_WEBUI_PORT.getDefaultValue()));
        ...
        for (int tryCount = 0; (tryCount < MetaStoreTestUtils.RETRY_COUNT); tryCount++)
        {
            ...
            {
                HiveConf.setIntVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_THRIFT_PORT, MetaStoreTestUtils.findFreePort());
                HiveConf.setIntVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_THRIFT_HTTP_PORT, MetaStoreTestUtils.findFreePort());
                HiveConf.setIntVar(hiveConf, HiveConf.ConfVars.HIVE_SERVER2_WEBUI_PORT, MetaStoreTestUtils.findFreePort());
