...
public class LlapExtClientJwtHelper
{
    ...
    {
        return Jwts.builder().setSubject(LLAP_JWT_SUBJECT).setIssuedAt(new Date()).setId(UUID.randomUUID().toString()).claim(LLAP_EXT_CLIENT_APP_ID, extClientAppId.toString()).signWith(jwtSecretProvider.getEncryptionSecret()).compact();
    }
    ...
    {
        return Jwts.parser().setSigningKey(jwtSecretProvider.getDecryptionSecret()).parseClaimsJws(jwt);
    }
