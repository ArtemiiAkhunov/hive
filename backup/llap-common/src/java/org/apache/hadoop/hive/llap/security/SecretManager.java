...
public class SecretManager extends ZKDelegationTokenSecretManager<LlapTokenIdentifier> implements SigningSecretManager
{
    ...
    private static final String DISABLE_MESSAGE = "Set " + ConfVars.LLAP_VALIDATE_ACLS.varname + " to false to disable ACL validation (note" + " that invalid ACLs on secret key paths would mean that security is compromised)";
    ...
    public void startThreads() throws IOException
    {
        ...
        super.startThreads();
        if (!HiveConf.getBoolVar(conf, ConfVars.LLAP_VALIDATE_ACLS) || !UserGroupInformation.isSecurityEnabled())
        {
            return;
        }
    ...
    private void checkForZKDTSMBug()
    {
        ...
        if (expectedRenewTimeSec == -1)
        {
            return; // The default works, no bug.
        }
    ...
    private static LlapZkConf createLlapZkConf(Configuration conf, String llapPrincipal, String llapKeytab, String clusterId)
    {
        ...
        long tokenRenewInterval = HiveConf.getTimeVar(conf, ConfVars.LLAP_DELEGATION_TOKEN_RENEW_INTERVAL, TimeUnit.SECONDS);
        zkConf.setLong(DelegationTokenManager.RENEW_INTERVAL, tokenRenewInterval);

        LOG.info("SecretManager ZkConf created: tokenLifetime {}, tokenRenewInterval: {}", tokenLifetime, tokenRenewInterval);

        try
        {
            zkConf.set(ZK_DTSM_ZK_KERBEROS_PRINCIPAL, SecurityUtil.getServerPrincipal(llapPrincipal, "0.0.0.0"));
        } catch (IOException e)
        {
        ...
        long sessionTimeoutMs = HiveConf.getTimeVar(zkConf, ConfVars.LLAP_ZKSM_ZK_SESSION_TIMEOUT, TimeUnit.MILLISECONDS);
        long newRetryCount = (ZK_DTSM_ZK_NUM_RETRIES_DEFAULT * sessionTimeoutMs) / ZK_DTSM_ZK_SESSION_TIMEOUT_DEFAULT;
        ...
        setZkConfIfNotSet(zkConf, ZK_DTSM_ZK_CONNECTION_STRING, HiveConf.getVar(zkConf, ConfVars.LLAP_ZKSM_ZK_CONNECTION_STRING));
