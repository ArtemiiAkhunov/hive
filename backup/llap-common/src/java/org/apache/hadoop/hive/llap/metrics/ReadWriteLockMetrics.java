...
public class ReadWriteLockMetrics implements ReadWriteLock
{
    ...
    /// < wrapper around original read lock
    ...
    public static class MetricsComparator implements Comparator<MetricsSource>, Serializable
    {
        ...
        public int compare(MetricsSource o1, MetricsSource o2)
        {
            if (o1 != null && o2 != null && o1 instanceof LockMetricSource && o2 instanceof LockMetricSource)
            {
                ...
                long totalMs2 = (lms2.readLockWaitTimeTotal.value() / 1000000L) + (lms2.writeLockWaitTimeTotal.value() / 1000000L);
    ...
    public static ReadWriteLock wrap(Configuration conf, ReadWriteLock lock, MetricsSource metrics)
    {
        ...
        if (null != conf)
        {
            needsWrap = HiveConf.getBoolVar(conf, HiveConf.ConfVars.LLAP_COLLECT_LOCK_METRICS);
        }
        ...
        Preconditions.checkNotNull(metrics, "Caller has to procide group specific metrics source");
    ...
    {
        ReadLockWaitTimeTotal("The total wait time for read locks in nanoseconds"), ReadLockWaitTimeMax("The maximum wait time for a read lock in nanoseconds"), ReadLockCount("Total amount of read lock requests"), WriteLockWaitTimeTotal("The total wait time for write locks in nanoseconds"), WriteLockWaitTimeMax("The maximum wait time for a write lock in nanoseconds"), WriteLockCount("Total amount of write lock requests");
    ...
    private static class LockMetricSource implements MetricsSource
    {
        ...
        {
            lockLabel = label;
            readLockWaitTimeTotal = new MutableCounterLong(LockMetricInfo.ReadLockWaitTimeTotal, 0);
            readLockWaitTimeMax = new MutableCounterLong(LockMetricInfo.ReadLockWaitTimeMax, 0);
            readLockCounts = new MutableCounterLong(LockMetricInfo.ReadLockCount, 0);
            writeLockWaitTimeTotal = new MutableCounterLong(LockMetricInfo.WriteLockWaitTimeTotal, 0);
            writeLockWaitTimeMax = new MutableCounterLong(LockMetricInfo.WriteLockWaitTimeMax, 0);
            writeLockCounts = new MutableCounterLong(LockMetricInfo.WriteLockCount, 0);
        ...
        {
            collector.addRecord(this.lockLabel).setContext("Locking").addCounter(LockMetricInfo.ReadLockWaitTimeTotal, readLockWaitTimeTotal.value()).addCounter(LockMetricInfo.ReadLockWaitTimeMax, readLockWaitTimeMax.value()).addCounter(LockMetricInfo.ReadLockCount, readLockCounts.value()).addCounter(LockMetricInfo.WriteLockWaitTimeTotal, writeLockWaitTimeTotal.value()).addCounter(LockMetricInfo.WriteLockWaitTimeMax, writeLockWaitTimeMax.value()).addCounter(LockMetricInfo.WriteLockCount, writeLockCounts.value());
        }
        ...
        public String toString()
        {
            ...
            totalMillis = (readLockWaitTimeTotal.value() / 1000000L) + (writeLockWaitTimeTotal.value() / 1000000L);
    ...
    private ReadWriteLockMetrics(ReadWriteLock lock, MetricsSource metrics)
    {
        ...
        Preconditions.checkArgument(metrics instanceof LockMetricSource, "Invalid MetricsSource");

        LockMetricSource lms = (LockMetricSource) metrics;
        readLock = new LockWrapper(lock.readLock(), lms.readLockWaitTimeTotal, lms.readLockWaitTimeMax, lms.readLockCounts);
        writeLock = new LockWrapper(lock.writeLock(), lms.writeLockWaitTimeTotal, lms.writeLockWaitTimeMax, lms.writeLockCounts);
    }
