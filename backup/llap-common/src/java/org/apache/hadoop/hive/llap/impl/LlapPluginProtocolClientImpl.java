...
public class LlapPluginProtocolClientImpl implements LlapPluginProtocolPB
{
    ...
    public LlapPluginProtocolClientImpl(Configuration conf, String hostname, int port, RetryPolicy retryPolicy, SocketFactory socketFactory, UserGroupInformation ugi)
    {
        protobuf = new ProtobufProxy<>(conf, ugi, hostname, port, retryPolicy, socketFactory, LlapPluginProtocolPB.class);
    }

    @Override
    public UpdateQueryResponseProto updateQuery(RpcController controller, UpdateQueryRequestProto request) throws ServiceException
    {
        try
        {
