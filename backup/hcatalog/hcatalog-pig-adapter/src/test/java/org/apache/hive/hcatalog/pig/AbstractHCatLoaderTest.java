...
public abstract class AbstractHCatLoaderTest extends HCatBaseTest
{
    ...
    private void createTable(String db, String tablename, String schema, String partitionedBy) throws Exception
    {
        createTable(db, tablename, schema, partitionedBy, driver, storageFormat);
    }

    private void createTableDefaultDB(String tablename, String schema, String partitionedBy) throws Exception
    {
        createTable(null, tablename, schema, partitionedBy, driver, storageFormat);
    }

    static void createTableDefaultDB(String tablename, String schema, String partitionedBy, IDriver driver, String storageFormat) throws Exception
    {
        createTable(null, tablename, schema, partitionedBy, driver, storageFormat);
    }

    static void createTable(String db, String tablename, String schema, String partitionedBy, IDriver driver, String storageFormat) throws Exception
    {
        String createTable;
    ...
    private static void checkProjection(FieldSchema fs, String expectedName, byte expectedPigType)
    {
        ...
        assertEquals("Expected " + DataType.findTypeName(expectedPigType) + "; got " + DataType.findTypeName(fs.type), expectedPigType, fs.type);
    }
    ...
    public void tearDown() throws Exception
    {
        try
        {
            if (driver != null)
            {
                ...
                executeStatementOnDriver("drop database if exists " + SPECIFIC_DATABASE + " cascade", driver);
            }
        } finally
        {
    ...
    public void testGetInputBytesMultipleTables() throws Exception
    {
        ...
        randomAccessFile.close();
        file = new File(TEST_WAREHOUSE_DIR + "/" + SPECIFIC_DATABASE + ".db/" + SPECIFIC_SIZE_TABLE_2 + "/part-m-00000");
        file.deleteOnExit();
        ...
        assertTrue(Math.abs((specificTableSize + specificTableSize2 + partitionedTableSize) - (987654321 + 12345678 + 1010)) < 1010);
    }
    ...
    public void testConvertBooleanToInt() throws Exception
    {
        ...
        driver.run("create external table " + tbl + " (a string, b boolean) row format delimited fields terminated by '\t'" + " stored as textfile location 'file:///" + inputDataDir.getPath().replaceAll("\\\\", "/") + "'");
            ...
            while (XIter.hasNext())
            {
                ...
                for (Object referenceData : primitiveRows[numTuplesRead])
                {
                    ...
                    } else if (referenceData instanceof Date)
                    {
                        ...
                        assertTrue("rowNum=" + numTuplesRead + " colNum=" + colPos + " Reference data=" + ((Date) referenceData).toEpochMilli() + " actual=" + ((DateTime) t.get(colPos)).getMillis() + "; types=(" + referenceData.getClass() + "," + t.get(colPos).getClass() + ")", ((Date) referenceData).toEpochMilli() == ((DateTime) t.get(colPos)).getMillis());
                    } else if (referenceData instanceof Timestamp)
                    {
                        // Note that here we ignore nanos part of Hive Timestamp since nanos are dropped when
                        // reading Hive from Pig by design.
                        assertTrue("rowNum=" + numTuplesRead + " colNum=" + colPos + " Reference data=" + ((Timestamp) referenceData).toEpochMilli() + " actual=" + ((DateTime) t.get(colPos)).getMillis() + "; types=(" + referenceData.getClass() + "," + t.get(colPos).getClass() + ")", ((Timestamp) referenceData).toEpochMilli() == ((DateTime) t.get(colPos)).getMillis());
                    } else
                    {
