...
public class HcatDelegator extends LauncherDelegator
{
    ...
    public Response createDatabase(String user, DatabaseDesc desc) throws HcatException, NotAuthorizedException, BusyException, ExecuteException, IOException
    {
        ...
        {
            exec += String.format(" with dbproperties (%s)", makePropertiesStatement(desc.properties));
        }
        exec += ";";
    ...
    private String makeCreateTable(String db, TableDesc desc)
    {
        ...
        {
            exec += String.format(" tblproperties (%s)", makePropertiesStatement(desc.tableProperties));
        }
        exec += ";";
    ...
    {
        String res = makeTermBy(desc.fieldsTerminatedBy, "fields") + makeTermBy(desc.collectionItemsTerminatedBy, "collection items") + makeTermBy(desc.mapKeysTerminatedBy, "map keys") + makeTermBy(desc.linesTerminatedBy, "lines");
    ...
    private String makeSerdeFormat(TableDesc.SerdeDesc desc)
    {
        ...
        if (TempletonUtils.isset(desc.properties))
        {
            res += String.format(" with serdeproperties (%s)", makePropertiesStatement(desc.properties));
        }
        return res;
    }
    ...
    public Response descOnePartition(String user, String db, String table, String partition) throws HcatException, NotAuthorizedException, BusyException, ExecuteException, IOException
    {
        ...
        } catch (HcatException e)
        {
            if (e.execBean.stderr.contains("SemanticException") && e.execBean.stderr.contains("Partition not found"))
            {
                ...
                return JsonBuilder.create().put("error", emsg)
                        //this error should really be produced by Hive (DDLTask)
                        .put("errorCode", ErrorMsg.INVALID_PARTITION.getErrorCode()).put("database", db).put("table", table).put("partition", partition).build();
            }
    ...
    public Response addOnePartition(String user, String db, String table, PartitionDesc desc) throws HcatException, NotAuthorizedException, BusyException, ExecuteException, IOException
    {
        ...
        try
        {
            ...
            {
                return JsonBuilder.create().put("error", "Partition already exists")
                        //This error code should really be produced by Hive
                        .put("errorCode", ErrorMsg.PARTITION_EXISTS.getErrorCode()).put("database", db).put("table", table).put("partition", desc.partition).build();
            }
    ...
    public Response descOneColumn(String user, String db, String table, String column) throws SimpleWebException, NotAuthorizedException, BusyException, ExecuteException, IOException
    {
        ...
        if (fields == null)
        {
            throw new SimpleWebException(HttpStatus.NOT_FOUND_404, "Internal error, unable to find column " + column);
        }
        ...
        if (found == null)
        {
            throw new SimpleWebException(HttpStatus.NOT_FOUND_404, "unable to find column " + column, new HashMap<String, Object>()
            {
                {
