...
public class LauncherDelegator extends TempletonDelegator
{
     ...
     */
    private static JobRequestExecutor<EnqueueBean> jobRequest = new JobRequestExecutor<EnqueueBean>(JobRequestExecutor.JobRequestType.Submit, AppConfig.JOB_SUBMIT_MAX_THREADS, AppConfig.JOB_SUBMIT_TIMEOUT, false);
     ...
     */
    public EnqueueBean enqueueController(final String user, final Map<String, Object> userArgs, final String callback, final List<String> args) throws NotAuthorizedException, BusyException, IOException, QueueException, TooManyRequestsException
    {
        ...
        {
            JobCallable<EnqueueBean> jobExecuteCallable = getJobSubmitTask(user, userArgs, callback, args, controllerJob);
            try
            {
    ...
    private JobCallable<EnqueueBean> getJobSubmitTask(final String user, final Map<String, Object> userArgs, final String callback, final List<String> args, final TempletonControllerJob controllerJob)
    {
        return new JobCallable<EnqueueBean>()
        {
            ...
            public EnqueueBean execute() throws NotAuthorizedException, BusyException, IOException, QueueException
            {
                 ...
                 * to debug job issues.
                 */
                Thread.currentThread().setName(String.format("%s-%s-%s", JOB_SUBMIT_EXECUTE_THREAD_PREFIX, submitThreadId, Thread.currentThread().getId()));
        ...
        if (StringUtils.startsWith(jobId, "job_"))
        {
            ...
            do
            {
                ...
                } catch (Exception e)
                {
                    ...
                    LOG.info("Waiting for " + jobTimeoutTaskRetryIntervalInSec + "s before retrying " + "the operation. Iteration: " + count);
                    try
                    {
