...
public class AlterPartitionReplicationTask extends ReplicationTask
{
    ...
    public Iterable<? extends Command> getDstWhCommands()
    {
        ...
        return Collections.singletonList(new ImportCommand(dstDbName, dstTableName, alterPartitionMessage.getKeyValues(), dstStagingDirProvider.getStagingDirectory(ReplicationUtils.getUniqueKey(getEvent().getEventId(), alterPartitionMessage.getDB(), // Note - important to retain the same key as the export
                alterPartitionMessage.getTable(), alterPartitionMessage.getKeyValues())), true, event.getEventId()));
    }
}
