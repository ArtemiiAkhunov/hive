...
public class HCatClientHMSImpl extends HCatClient
{
    ...
    public HCatDatabase getDatabase(String dbName) throws HCatException
    {
        ...
        {
            throw new ObjectNotFoundException("NoSuchObjectException while fetching database", exp);
        } catch (MetaException exp)
        {
            throw new HCatException("MetaException while fetching database", exp);
        } catch (TException exp)
        {
            throw new ConnectionFailureException("TException while fetching database", exp);
        }
        return db;
    }
    ...
    public void createDatabase(HCatCreateDBDesc dbInfo) throws HCatException
    {
        ...
        } catch (AlreadyExistsException exp)
        {
            ...
            {
                throw new HCatException("AlreadyExistsException while creating database", exp);
            }
        } catch (InvalidObjectException exp)
        {
            throw new HCatException("InvalidObjectException while creating database", exp);
        } catch (MetaException exp)
        {
            throw new HCatException("MetaException while creating database", exp);
        } catch (TException exp)
        {
            throw new ConnectionFailureException("TException while creating database", exp);
        }
    }

    @Override
    public void dropDatabase(String dbName, boolean ifExists, DropDBMode mode) throws HCatException
    {
    ...
    public void createTable(HCatCreateTableDesc createTableDesc) throws HCatException
    {
        ...
        } catch (AlreadyExistsException e)
        {
            ...
            {
                throw new HCatException("AlreadyExistsException while creating table.", e);
            }
        } catch (InvalidObjectException e)
        {
            throw new HCatException("InvalidObjectException while creating table.", e);
        } catch (MetaException e)
        {
            throw new HCatException("MetaException while creating table.", e);
        } catch (NoSuchObjectException e)
        {
            throw new ObjectNotFoundException("NoSuchObjectException while creating table.", e);
        } catch (TException e)
        {
            throw new ConnectionFailureException("TException while creating table.", e);
        } catch (IOException e)
        {
    ...
    public void dropTable(String dbName, String tableName, boolean ifExists) throws HCatException
    {
        ...
        } catch (NoSuchObjectException e)
        {
            if (!ifExists)
            {
                throw new ObjectNotFoundException("NoSuchObjectException while dropping table.", e);
            }
        } catch (MetaException e)
        {
        ...
        {
            throw new ConnectionFailureException("TException while dropping table.", e);
        }
    }
    ...
    public HCatPartitionSpec getPartitionSpecs(String dbName, String tableName, int maxPartitions) throws HCatException
    {
        try
        {
            return new HCatPartitionSpec(getTable(dbName, tableName), hmsClient.listPartitionSpecs(dbName, tableName, maxPartitions));
        } catch (NoSuchObjectException e)
        {
            throw new ObjectNotFoundException("NoSuchObjectException while retrieving partition.", e);
        } catch (MetaException e)
    ...
    private static class ExpressionBuilder
    {
        ...
        public ExpressionBuilder(Table table, Map<String, String> partSpecs)
        {
            ...
            {
                partColumnTypesMap.put(partField.getName().toLowerCase(), TypeInfoFactory.getPrimitiveTypeInfo(partField.getType()));
            }
        }
        ...
        {
            ObjectInspectorConverters.Converter converter = ObjectInspectorConverters.getConverter(TypeInfoUtils.getStandardJavaObjectInspectorFromTypeInfo(TypeInfoFactory.stringTypeInfo), TypeInfoUtils.getStandardJavaObjectInspectorFromTypeInfo(type));
        ...
        public ExprNodeGenericFuncDesc equalityPredicate(String partColumn, String value) throws SemanticException
        {
            ...
            ExprNodeConstantDesc valueExpr = new ExprNodeConstantDesc(partColumnType, getTypeAppropriateValueFor(partColumnType, value));
        ...
        {
            return new ExprNodeGenericFuncDesc(TypeInfoFactory.booleanTypeInfo, FunctionRegistry.getFunctionInfo(function).getGenericUDF(), Lists.newArrayList(lhs, rhs));
        }
    ...
    private void dropPartitionsUsingExpressions(Table table, Map<String, String> partitionSpec, boolean ifExists, boolean deleteData) throws SemanticException, TException
    {
        ...
        Pair<Integer, byte[]> serializedPartitionExpression = Pair.of(partitionSpec.size(), SerializationUtilities.serializeObjectWithTypeInformation(partitionExpression));
        hmsClient.dropPartitions(table.getDbName(), table.getTableName(), Arrays.asList(serializedPartitionExpression), deleteData && !isExternal(table),  // Delete data?
    ...
    private void dropPartition(Partition partition, boolean ifExists, boolean deleteData) throws HCatException, MetaException, TException
    {
        try
        {
    ...
    @InterfaceAudience.LimitedPrivate({"Hive"})
    @InterfaceStability.Evolving
    public HCatPartitionSpec listPartitionSpecsByFilter(String dbName, String tblName, String filter, int maxPartitions) throws HCatException
    {
        try
        {
    ...
    public String getDelegationToken(String owner, String renewerKerberosPrincipalName) throws HCatException
    {
        ...
        try
        {
            token = hmsClient.getDelegationToken(owner, renewerKerberosPrincipalName);
        } catch (MetaException e)
        {
            throw new HCatException("MetaException while getting delegation token.", e);
        } catch (TException e)
        {
            throw new ConnectionFailureException("TException while getting delegation token.", e);
        }
    ...
    public long renewDelegationToken(String tokenStrForm) throws HCatException
    {
        ...
        {
            throw new ConnectionFailureException("TException while renewing delegation token.", e);
        }
    ...
    public String getMessageBusTopicName(String dbName, String tableName) throws HCatException
    {
        ...
        {
            throw new ConnectionFailureException("TException while retrieving JMS Topic name.", e);
        }
    }
