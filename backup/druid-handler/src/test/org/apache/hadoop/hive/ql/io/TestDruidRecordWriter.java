...
public class TestDruidRecordWriter
{
    ...
    final List<ImmutableMap<String, Object>> expectedRows = ImmutableList.of(ImmutableMap.of(DruidConstants.DEFAULT_TIMESTAMP_COLUMN, DateTime.parse("2014-10-22T00:00:00.000Z").getMillis(), "host", ImmutableList.of("a.example.com"), "visited_sum", 190L, "unique_hosts", 1.0d), ImmutableMap.of(DruidConstants.DEFAULT_TIMESTAMP_COLUMN, DateTime.parse("2014-10-22T01:00:00.000Z").getMillis(), "host", ImmutableList.of("b.example.com"), "visited_sum", 175L, "unique_hosts", 1.0d), ImmutableMap.of(DruidConstants.DEFAULT_TIMESTAMP_COLUMN, DateTime.parse("2014-10-22T02:00:00.000Z").getMillis(), "host", ImmutableList.of("c.example.com"), "visited_sum", 270L, "unique_hosts", 1.0d));

    @Test
    public void testTimeStampColumnName()
    {
        Assert.assertEquals("Time column name need to match to ensure serdeser compatibility", DruidConstants.DEFAULT_TIMESTAMP_COLUMN, DruidTable.DEFAULT_TIMESTAMP_COLUMN);
    }
    ...
    private void verifyRows(List<ImmutableMap<String, Object>> expectedRows, List<InputRow> actualRows)
    {
        ...
        for (int i = 0; i < expectedRows.size(); i++)
        {
            ...
            Assert.assertEquals(expected.get(DruidConstants.DEFAULT_TIMESTAMP_COLUMN), actual.getTimestamp().getMillis());
            ...
            Assert.assertEquals((Double) expected.get("unique_hosts"), (Double) HyperUniquesAggregatorFactory.estimateCardinality(actual.getRaw("unique_hosts"), false), 0.001);
        }
    }
