...
public class TestHiveCli
{
    ...
    private final static String SOURCE_CONTEXT = "create table if not exists test.testSrcTbl(sc1 string);";
    private final static String SOURCE_CONTEXT2 = "create table if not exists test.testSrcTbl2(sc2 string);";
    private final static String SOURCE_CONTEXT3 = "create table if not exists test.testSrcTbl3(sc3 string);";
    ...
    {
        verifyCMD("\nshow tables;\nquit;\n", "Database does not exist: invalidDB", err, new String[]{"--database", "invalidDB"}, ERRNO_OK, true);
    }

    @Test
    public void testDatabaseOptions()
    {
        verifyCMD("\nshow tables;\nquit;", "testtbl", out, new String[]{"--database", "test"}, ERRNO_OK, true);
    }
    ...
    public void testSourceCmd3()
    {
        ...
        verifyCMD("source " + f.getPath() + ";" + "desc testSrcTbl4;\nquit;\n", "src", out, new String[]{"--database", "test"}, ERRNO_OTHER, true);
        f.delete();
    }
    ...
    public void testSourceCmd4()
    {
        ...
        verifyCMD("source " + f.getPath() + ";", "testtbl", out, new String[]{"--database", "test"}, ERRNO_OK, true);
        f.delete();
    }
    ...
    {
        verifyCMD(null, "testtbl", out, new String[]{"-e", "show tables;", "--database", "test"}, ERRNO_OK, true);
    }

    @Test
    public void testSqlFromCmdWithEmbeddedQuotes()
    {
        verifyCMD(null, "hive", out, new String[]{"-e", "select \"hive\""}, ERRNO_OK, true);
    }

    @Test
    public void testInvalidOptions()
    {
        verifyCMD(null, "The '-e' and '-f' options cannot be specified simultaneously", err, new String[]{"-e", "show tables;", "-f", "path/to/file"}, ERRNO_ARGS, true);
    }

    @Test
    public void testInvalidOptions2()
    {
        verifyCMD(null, "Unrecognized option: -k", err, new String[]{"-k"}, ERRNO_ARGS, true);
    }
    ...
    public void testVariablesForSource()
    {
        ...
        verifyCMD("set hiveconf:zzz=" + f.getAbsolutePath() + ";\nsource ${hiveconf:zzz};\ndesc testSrcTbl2;", "sc2", out, new String[]{"--database", "test"}, ERRNO_OK, true);
        f.delete();
    }
    ...
    {
        verifyCMD("show tables;set system:xxx=5;set system:yyy=${system:xxx};\nlss;", "cannot recognize input near 'lss' '<EOF>' '<EOF>'", err, null, ERRNO_OTHER, true);
    }
    ...
    {
        verifyCMD("set hive.cli.print.current.db=true;use invalidDB;", "hive (invalidDB)>", out, null, ERRNO_OTHER, false);
    }

    @Test
    public void testNoErrorDB()
    {
        verifyCMD(null, "Error: Method not supported (state=,code=0)", err, new String[]{"-e", "show tables;"}, ERRNO_OK, false);
    }
    ...
    private void verifyCMD(String CMD, String keywords, OutputStream os, String[] options, int retCode, boolean contains)
    {
        ...
        if (contains)
        {
            Assert.assertTrue("The expected keyword \"" + keywords + "\" occur in the output: " + output, output.contains(keywords));
        } else
        {
            Assert.assertFalse("The expected keyword \"" + keywords + "\" should be excluded occurred in the output: " + output, output.contains(keywords));
        }
    }
