...
public class BeelineSiteParser implements HS2ConnectionFileParser
{
    ...
    public BeelineSiteParser()
    {
        ...
        {
            locations.add(System.getenv("HIVE_CONF_DIR") + File.separator + DEFAULT_BEELINE_SITE_FILE_NAME);
        }
    ...
    public Properties getConnectionProperties(String propertyValue) throws BeelineSiteParseException
    {
        ...
        try
        {
            ...
            {
                String key = kv.getKey();
                if (key.startsWith(BEELINE_CONNECTION_NAMED_JDBC_URL_PREFIX) && (propertyValue.equalsIgnoreCase(kv.getValue())))
                {
                    props.setProperty(key.substring(BEELINE_CONNECTION_NAMED_JDBC_URL_PREFIX.length()), conf.get(key));
                }
            }
