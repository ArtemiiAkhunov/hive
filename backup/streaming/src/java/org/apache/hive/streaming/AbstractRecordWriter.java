...
public abstract class AbstractRecordWriter implements RecordWriter
{
    ...
    protected static class OrcMemoryPressureMonitor implements HeapMemoryMonitor.Listener
    {
        ...
        public void memoryUsageAboveThreshold(final long usedMemory, final long maxMemory)
        {
            ...
            {
                LOG.debug("Orc memory pressure notified! usedMemory: {} maxMemory: {}.", LlapUtil.humanReadableByteCount(usedMemory), LlapUtil.humanReadableByteCount(maxMemory));
            }
    ...
    protected void setupMemoryMonitoring()
    {
        ...
        LOG.info("Memory monitoring settings - autoFlush: {} memoryUsageThreshold: {} ingestSizeThreshold: {}", autoFlush, memoryUsageThreshold, ingestSizeBytes);
        ...
        if (tenuredMemUsage != null)
        {
            ...
            {
                LOG.warn("LOW MEMORY ALERT! Tenured gen memory is already low. Increase memory to improve performance." + " Used: {} Max: {}", LlapUtil.humanReadableByteCount(tenuredMemUsage.getUsed()), LlapUtil.humanReadableByteCount(tenuredMemUsage.getMax()));
            }
        }
    }
    ...
    protected int getBucket(Object row)
    {
        ...
        int bucketingVersion = Utilities.getBucketingVersion(table.getParameters().get(hive_metastoreConstants.TABLE_BUCKETING_VERSION));

        return bucketingVersion == 2 ? ObjectInspectorUtils.getBucketNumber(bucketFields, bucketObjInspectors, totalBuckets) : ObjectInspectorUtils.getBucketNumberOld(bucketFields, bucketObjInspectors, totalBuckets);
    }
    ...
    protected List<String> getPartitionValues(final Object row)
    {
        ...
        if (conn.isPartitionedTable() && conn.isDynamicPartitioning())
        {
            ...
            for (int i = 0; i < partitionObjInspectors.length; i++)
            {
                ...
                Object partitionValue = ObjectInspectorUtils.copyToStandardObject(field, oi, ObjectInspectorUtils.ObjectInspectorCopyOption.WRITABLE);
    ...
    protected void checkAutoFlush() throws StreamingIOFailure
    {
        ...
        if (lowMemoryCanary != null)
        {
            if (lowMemoryCanary.get() && ingestSizeBytes > ingestSizeThreshold)
            {
                if (LOG.isDebugEnabled())
                {
                    LOG.debug("Low memory canary is set and ingestion size (buffered) threshold '{}' exceeded. " + "Flushing all record updaters..", LlapUtil.humanReadableByteCount(ingestSizeThreshold));
                }
                flush();
        ...
        } else
        {
            if (ingestSizeBytes > ingestSizeThreshold)
            {
                ...
                if (memUsedFraction > memoryUsageThreshold)
                {
                    if (LOG.isDebugEnabled())
                    {
                        LOG.info("Memory usage threshold '{}' and ingestion size (buffered) threshold '{}' exceeded. " + "Flushing all record updaters..", memUsedFraction, LlapUtil.humanReadableByteCount(ingestSizeThreshold));
                    }
                    flush();
                    conn.getConnectionStats().incrementAutoFlushCount();
                }
    ...
    protected RecordUpdater createRecordUpdater(List<String> partitionValues, final Path partitionPath, int bucketId, Long minWriteId, Long maxWriteID) throws IOException
    {
        ...
        AcidOutputFormat.Options options = new AcidOutputFormat.Options(conf).filesystem(fs).inspector(outputRowObjectInspector).bucket(bucketId).tableProperties(tblProperties).minimumWriteId(minWriteId).maximumWriteId(maxWriteID).statementId(statementId).finalDestination(partitionPath);
    ...
    public Path getDeltaFileLocation(List<String> partitionValues, Integer bucketId, Long minWriteId, Long maxWriteId, Integer statementId, Table table) throws StreamingException
    {
        ...
        {
            Map<String, String> partSpec = Warehouse.makeSpecFromValues(table.getPartitionKeys(), partitionValues);
            try
            {
        ...
        AcidOutputFormat.Options options = new AcidOutputFormat.Options(conf).filesystem(fs).inspector(outputRowObjectInspector).bucket(bucketId).minimumWriteId(minWriteId).maximumWriteId(maxWriteId).statementId(statementId).finalDestination(destLocation);
    ...
    protected RecordUpdater getRecordUpdater(List<String> partitionValues, int bucketId) throws StreamingIOFailure
    {
        ...
        if (recordUpdater == null)
        {
            try
            {
                recordUpdater = createRecordUpdater(partitionValues, destLocation, bucketId, curBatchMinWriteId, curBatchMaxWriteId);
            } catch (IOException e)
            {
    ...
    {
        int openRecordUpdaters = updaters.values().stream().mapToInt(List::size).sum();
        long bufferedRecords = updaters.values().stream().flatMap(List::stream).filter(Objects::nonNull).mapToLong(RecordUpdater::getBufferedRowCount).sum();
        MemoryUsage memoryUsage = heapMemoryMonitor == null ? null : heapMemoryMonitor.getTenuredGenMemoryUsage();
        String oldGenUsage = "NA";
        if (memoryUsage != null)
        {
            oldGenUsage = "used/max => " + LlapUtil.humanReadableByteCount(memoryUsage.getUsed()) + "/" + LlapUtil.humanReadableByteCount(memoryUsage.getMax());
        }
        LOG.debug("{} [record-updaters: {}, partitions: {}, buffered-records: {} total-records: {} " + "buffered-ingest-size: {}, total-ingest-size: {} tenured-memory-usage: {}]", prefix, openRecordUpdaters, partitionPaths.size(), bufferedRecords, conn == null ? 0 : conn.getConnectionStats().getRecordsWritten(), LlapUtil.humanReadableByteCount(ingestSizeBytes), LlapUtil.humanReadableByteCount(conn == null ? 0 : conn.getConnectionStats().getRecordsSize()), oldGenUsage);
    }
}
