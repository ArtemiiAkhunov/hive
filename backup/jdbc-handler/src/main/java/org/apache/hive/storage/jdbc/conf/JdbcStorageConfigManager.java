...
public class JdbcStorageConfigManager
{
    ...
    public static void copyConfigurationToJob(Properties props, Map<String, String> jobProps) throws HiveException, IOException
    {
        ...
        for (Entry<Object, Object> entry : props.entrySet())
        {
            ...
            if (!key.equals(CONFIG_PWD) && !key.equals(CONFIG_PWD_KEYSTORE) && !key.equals(CONFIG_PWD_KEY) && !key.equals(CONFIG_PWD_URI))
            {
    ...
    private static void checkRequiredPropertiesAreDefined(Properties props)
    {
        ...
        for (JdbcStorageConfig configKey : (DatabaseType.METASTORE.equals(dbType) ? METASTORE_REQUIRED_PROPERTIES : DEFAULT_REQUIRED_PROPERTIES))
        {
    ...
    private static void resolveMetadata(Properties props) throws HiveException, IOException
    {
        ...
        if (dbType == DatabaseType.METASTORE)
        {
            ...
            props.setProperty(JdbcStorageConfig.JDBC_URL.getPropertyName(), getMetastoreConnectionURL(hconf));
            props.setProperty(JdbcStorageConfig.JDBC_DRIVER_CLASS.getPropertyName(), getMetastoreDriver(hconf));
            ...
            props.setProperty(JdbcStorageConfig.DATABASE_TYPE.getPropertyName(), getMetastoreDatabaseType(hconf));
        }
    }
    ...
    {
        return ShimLoader.getHadoopShims().getPassword(conf, HiveConf.ConfVars.METASTORE_PWD.varname);
    }
}
