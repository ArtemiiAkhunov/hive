...
public class TestProactiveEviction
{
    ...
    public void testCachetagAndRequestMatching() throws Exception
    {
        ...
        assertMatchOnTags(Builder.create().addPartitionOfATable("fx", "futures", buildParts("ccy", "JPY")), "000000110000000000");
        assertMatchOnTags(Builder.create().addPartitionOfATable("equity", "prices", buildParts("ex", "NYSE")).addPartitionOfATable("equity", "prices", buildParts("ex", "NYSE")), "000000000000110000");
        assertMatchOnTags(Builder.create().addTable("fx", "rates").addTable("fx", "futures"), "111111111000000000");
        assertMatchOnTags(Builder.create().addPartitionOfATable("fx", "rates", buildParts("from", "PLN")), "000000000000000000");
        assertMatchOnTags(Builder.create().addTable("fixedincome", "bonds"), "000000000000000110");
        assertMatchOnTags(Builder.create().addPartitionOfATable("fx", "rates", buildParts("from", "EUR", "to", "HUF")), "000010000000000000");
    }
    ...
    public void testProactiveSweep() throws Exception
    {
        ...
        LowLevelCacheMemoryManager mm = new LowLevelCacheMemoryManager(1024L, policy, LlapDaemonCacheMetrics.create("test", "1"));
        ...
        LlapDataBuffer[] buffs = IntStream.range(0, 10).mapToObj(i -> LowLevelCacheImpl.allocateFake()).toArray(LlapDataBuffer[]::new);
