...
public class TestLlapStatusServiceCommandLine
{
    ...
    public void testArgumentParsingDefault() throws Exception
    {
        ...
        assertEquals("findAppTimeout should be the default value if not specified otherwise", cl.getFindAppTimeoutMs(), LlapStatusServiceCommandLine.DEFAULT_FIND_YARN_APP_TIMEOUT_MS);

        assertEquals("refreshInterval should be the default value if not specified otherwise", cl.getRefreshIntervalMs(), LlapStatusServiceCommandLine.DEFAULT_STATUS_REFRESH_INTERVAL_MS);

        assertEquals("watchTimeout should be the default value if not specified otherwise", cl.getWatchTimeoutMs(), LlapStatusServiceCommandLine.DEFAULT_WATCH_MODE_TIMEOUT_MS);

        assertEquals("runningNodesThreshold should be the default value if not specified otherwise", cl.getRunningNodesThreshold(), LlapStatusServiceCommandLine.DEFAULT_RUNNING_NODES_THRESHOLD);

        assertEquals("hiveConf should be empty properties if not specified otherwise", cl.getHiveConf(), new Properties());
