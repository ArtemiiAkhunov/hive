...
public class SerDeEncodedDataReader extends CallableWithNdc<Void> implements ConsumerFeedback<OrcEncodedColumnBatch>, TezCounterSource
{

    public static final FixedSizedObjectPool<ColumnStreamData> CSD_POOL = new FixedSizedObjectPool<>(8192, new PoolObjectHelper<ColumnStreamData>()
    {
        @Override
    ...
    public static final FixedSizedObjectPool<OrcEncodedColumnBatch> ECB_POOL = new FixedSizedObjectPool<>(1024, new PoolObjectHelper<OrcEncodedColumnBatch>()
    {
        @Override
    ...
    public SerDeEncodedDataReader(SerDeLowLevelCacheImpl cache, BufferUsageManager bufferManager, Configuration daemonConf, FileSplit split, List<Integer> columnIds, OrcEncodedDataConsumer consumer, JobConf jobConf, Reporter reporter, InputFormat<?, ?> sourceInputFormat, Deserializer sourceSerDe, QueryFragmentCounters counters, TypeDescription schema, Map<Path, PartitionDesc> parts, ExecutorService encodeExecutor) throws IOException
    {
        ...
        this.targetSliceRowCount = HiveConf.getIntVar(sliceConf, ConfVars.LLAP_IO_ENCODE_SLICE_ROW_COUNT);
        ...
        cacheTag = HiveConf.getBoolVar(daemonConf, ConfVars.LLAP_TRACK_CACHE_USAGE) ? LlapHiveUtils.getDbAndTableNameForMetrics(split.getPath(), true, partitionDesc) : null;
        this.sourceInputFormat = sourceInputFormat;
        this.sourceSerDe = sourceSerDe;
        this.reporter = reporter;
        this.jobConf = jobConf;
        final boolean useDecimal64ColumnVectors = HiveConf.getVar(jobConf, ConfVars.HIVE_VECTORIZED_INPUT_FORMAT_SUPPORTS_ENABLED).equalsIgnoreCase("decimal_64");
        ...
        SchemaEvolution evolution = new SchemaEvolution(schema, null, new Reader.Options(jobConf).include(writerIncludes));
    ...
    public static class CacheWriter implements PhysicalWriter
    {
        ...
        private static class CacheStreamData
        {
            ...
            {
                return "CacheStreamData [name=" + name + ", isSuppressed=" + isSuppressed + ", data=" + toString(data) + "]";
            }
        ...
        private static class CacheStripeData
        {
            ...
            public String toString()
            {
                return ("{disk data knownTornStart=" + knownTornStart + ", firstRowStart=" + firstRowStart + ", lastRowStart=" + lastRowStart + ", lastRowEnd=" + lastRowEnd + ", rowCount=" + rowCount + ", encodings=" + encodings + ", streams=" + colStreams + "}").replace('\n', ' ');
            }

            public String toCoordinateString()
            {
                return "knownTornStart=" + knownTornStart + ", firstRowStart=" + firstRowStart + ", lastRowStart=" + lastRowStart + ", lastRowEnd=" + lastRowEnd;
            }
        }
        ...
        public void validateIncludes(OrcProto.Footer footer) throws IOException
        {
            ...
            boolean[] translatedIncludes = columnIds == null ? null : OrcInputFormat.genIncludedColumns(OrcUtils.convertTypeFromProtobuf(footer.getTypesList(), 0), columnIds);
        ...
        {
            String s = "Includes derived from the original table: " + DebugUtils.toString(writerIncludes) + " but the ones derived from writer types are: " + DebugUtils.toString(translated);
        ...
        public void discardData()
        {
            ...
            {
                if (stripe.colStreams == null || stripe.colStreams.isEmpty())
                {
                    continue;
                }
        ...
        public void finalizeStripe(OrcProto.StripeFooter.Builder footer, OrcProto.StripeInformation.Builder dirEntry) throws IOException
        {
            ...
            if (LlapIoImpl.LOG.isTraceEnabled())
            {
                LlapIoImpl.LOG.trace(("Finalizing stripe " + footer.build() + " => " + si).replace('\n', ' '));
            }
            if (doesSourceHaveIncludes)
            {
            ...
            for (Map.Entry<Integer, List<CacheOutputReceiver>> e : colStreams.entrySet())
            {
                ...
                for (CacheOutputReceiver receiver : streams)
                {
                    ...
                    data.add(new CacheStreamData(receiver.suppressed, receiver.name, buffers == null ? new ArrayList<MemoryBuffer>() : new ArrayList<>(buffers)));
                    receiver.clear();
                }
    ...
    private static final class CacheOutputReceiver implements CacheOutput, OutputReceiver
    {
        ...
        public CacheOutputReceiver(BufferUsageManager bufferManager, StreamName name, BufferObjectFactory bufferFactory, AtomicBoolean isStopped)
        {
            this.bufferManager = bufferManager;
            this.bufferFactory = bufferFactory;
    ...
    protected Void performDataRead() throws IOException, InterruptedException
    {
        ...
        try
        {
            try
            {
                ...
                if (isFromCache == null)
                {
                    return null; // Stop requested, and handled inside.
                }
                if (!isFromCache)
                {
    ...
    private static boolean determineSplitIncludes(StripeData slice, boolean[] splitIncludes, boolean[] writerIncludes)
    {
        ...
        if (LlapIoImpl.CACHE_LOGGER.isTraceEnabled())
        {
            LlapIoImpl.LOG.trace("Includes accounting for cached data: before " + DebugUtils.toString(writerIncludes) + ", after " + DebugUtils.toString(splitIncludes));
        }
        return hasAllData;
    }
    ...
    private boolean processOneSlice(CacheWriter.CacheStripeData diskData, boolean[] splitIncludes, int stripeIx, StripeData cacheData, long startTime) throws IOException, InterruptedException
    {
        ...
        for (int colIx = 1; colIx < writerIncludes.length; ++colIx)
        {
            ...
            if (!hasAllData && splitIncludes[colIx])
            {
                ...
                if (streams == null)
                {
                    continue; // Struct column, such as root?
                }
    ...
    private void validateCacheAndDisk(StripeData cacheData, long rowCount, long encodingCount, Object diskDataLog) throws IOException
    {
        if (rowCount != cacheData.getRowCount())
        {
            throw new IOException("Row count mismatch; disk " + rowCount + ", cache " + cacheData.getRowCount() + " from " + diskDataLog + " and " + cacheData);
        }
        if (encodingCount > 0 && encodingCount != cacheData.getEncodings().length)
        {
            throw new IOException("Column count mismatch; disk " + encodingCount + ", cache " + cacheData.getEncodings().length + " from " + diskDataLog + " and " + cacheData);
        }
    }
    ...
    private static LlapSerDeDataBuffer[][] createArrayToCache(StripeData sliceToCache, int colIx, List<CacheWriter.CacheStreamData> streams)
    {
        if (LlapIoImpl.LOG.isTraceEnabled())
        {
            LlapIoImpl.LOG.trace("Processing streams for column " + colIx + ": " + streams);
        }
        LlapSerDeDataBuffer[][] newCacheDataForCol = sliceToCache.getData()[colIx] = new LlapSerDeDataBuffer[OrcEncodedColumnBatch.MAX_DATA_STREAMS][];
        return newCacheDataForCol;
    }

    private static int setStreamDataToCache(LlapSerDeDataBuffer[][] newCacheDataForCol, CacheWriter.CacheStreamData stream)
    {
    ...
    private void processColumnCacheData(LlapSerDeDataBuffer[][][] cacheBuffers, OrcEncodedColumnBatch ecb, int colIx)
    {
    ...
    private static class Vectors
    {
        ...
        public String toString()
        {
            return "Vectors {isSupported=" + isSupported + ", rowCount=" + rowCount + ", data=" + Arrays.toString(data) + "}";
        }
    }
            ...
            try
            {
                while (offsetReader.next())
                {
                    ...
                    if (maySplitTheSplit && ++rowsPerSlice == targetSliceRowCount)
                    {
                        ...
                        {
                            throw new AssertionError("Unable to get offsets from " + offsetReader.getClass().getSimpleName());
                        }
                        writer.setCurrentStripeOffsets(currentKnownTornStart, firstStartOffset, lastStartOffset, fileOffset);
                ...
                try
                {
                    if (rowsPerSlice > 0 || (!maySplitTheSplit && hasAnyData))
                    {
                        ...
                        if (!offsetReader.hasOffsets())
                        {
                            ...
                            {
                                LlapIoImpl.CACHE_LOGGER.debug("Cache offsets based on the split - 'first row' at " + firstStartOffset + "; 'last row' at " + lastStartOffset + ", " + fileOffset);
                            }
                        } else
                        {
    ...
    static abstract class EncodingWriter
    {
        ...
        public abstract void setCurrentStripeOffsets(long currentKnownTornStart, long firstStartOffset, long lastStartOffset, long fileOffset);
    ...
    static class DeserializerOrcWriter extends EncodingWriter
    {
        ...
        {
            cacheWriter.setCurrentStripeOffsets(currentKnownTornStart, firstStartOffset, lastStartOffset, fileOffset);
        }
    ...
    static WriterOptions createOrcWriterOptions(ObjectInspector sourceOi, Configuration conf, CacheWriter cacheWriter, int allocSize) throws IOException
    {
        return OrcFile.writerOptions(conf).stripeSize(Long.MAX_VALUE).blockSize(Long.MAX_VALUE).rowIndexStride(Integer.MAX_VALUE) // For now, do not limit this - one RG per split
                .blockPadding(false).compress(CompressionKind.NONE).version(Version.CURRENT).encodingStrategy(EncodingStrategy.SPEED).bloomFilterColumns(null).inspector(sourceOi).physicalWriter(cacheWriter).memory(MEMORY_MANAGER).bufferSize(allocSize);
    }
    ...
    private ReaderWithOffsets createOffsetReader(RecordReader<?, ?> sourceReader, TableDesc tableDesc, FileSplit split) throws IOException
    {
        ...
        if (LlapIoImpl.LOG.isDebugEnabled())
        {
            LlapIoImpl.LOG.debug("Using {} to read data with skip.header.line.count {} and skip.footer.line.count {}", sourceReader.getClass().getSimpleName(), headerCount, footerCount);
        }
    ...
    private static String[] extractHosts(FileSplit split, boolean isInMemory) throws IOException
    {
        ...
        {
            if (locInfo[i].isInMemory() != isInMemory)
            {
                continue;
            }
            if (hosts == null)
    ...
    private boolean sendEcbToConsumer(OrcEncodedColumnBatch ecb, boolean hasCachedSlice, CacheWriter.CacheStripeData diskData) throws InterruptedException
    {
        if (ecb == null)
