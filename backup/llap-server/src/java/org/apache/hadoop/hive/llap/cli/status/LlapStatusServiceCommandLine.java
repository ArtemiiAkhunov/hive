...
public class LlapStatusServiceCommandLine
{
    ...
    private static final Option NAME = OptionBuilder.withLongOpt("name").withDescription("LLAP cluster name").withArgName("name").hasArg().create('n');

    @SuppressWarnings("static-access")
    private static final Option FIND_APP_TIMEOUT = OptionBuilder.withLongOpt("findAppTimeout").withDescription("Amount of time(s) that the tool will sleep to wait for the YARN application to start." + "negative values=wait forever, 0=Do not wait. default=" + (DEFAULT_FIND_YARN_APP_TIMEOUT_MS / 1000) + "s").withArgName("findAppTimeout").hasArg().create('f');

    @SuppressWarnings("static-access")
    private static final Option OUTPUT_FILE = OptionBuilder.withLongOpt("outputFile").withDescription("File to which output should be written (Default stdout)").withArgName("outputFile").hasArg().create('o');

    @SuppressWarnings("static-access")
    private static final Option WATCH_MODE = OptionBuilder.withLongOpt("watch").withDescription("Watch mode waits until all LLAP daemons are running or subset of the nodes are running " + "(threshold can be specified via -r option) (Default wait until all nodes are running)").withArgName("watch").create('w');

    @SuppressWarnings("static-access")
    private static final Option NOT_LAUNCHED = OptionBuilder.withLongOpt("notLaunched").withDescription("In watch mode, do not assume that the application was already launched if there's doubt " + "(e.g. if the last application instance has failed).").withArgName("notLaunched").create('l');

    @SuppressWarnings("static-access")
    private static final Option RUNNING_NODES_THRESHOLD = OptionBuilder.withLongOpt("runningNodesThreshold").withDescription("When watch mode is enabled (-w), wait until the specified threshold of nodes are running " + "(Default 1.0 which means 100% nodes are running)").withArgName("runningNodesThreshold").hasArg().create('r');

    @SuppressWarnings("static-access")
    private static final Option REFRESH_INTERVAL = OptionBuilder.withLongOpt("refreshInterval").withDescription("Amount of time in seconds to wait until subsequent status checks in watch mode. Valid only " + "for watch mode. (Default " + (DEFAULT_STATUS_REFRESH_INTERVAL_MS / 1000) + "s)").withArgName("refreshInterval").hasArg().create('i');

    @SuppressWarnings("static-access")
    private static final Option WATCH_TIMEOUT = OptionBuilder.withLongOpt("watchTimeout").withDescription("Exit watch mode if the desired state is not attained until the specified timeout. (Default " + (DEFAULT_WATCH_MODE_TIMEOUT_MS / 1000) + "s)").withArgName("watchTimeout").hasArg().create('t');

    @SuppressWarnings("static-access")
    private static final Option HIVECONF = OptionBuilder.withLongOpt("hiveconf").withDescription("Use value for given property. Overridden by explicit parameters").withArgName("property=value").hasArgs(2).create();

    @SuppressWarnings("static-access")
    private static final Option HELP = OptionBuilder.withLongOpt("help").withDescription("Print help information").withArgName("help").create('h');
