...
class VectorDeserializeOrcWriter extends EncodingWriter implements Runnable
{
    ...
    private VectorDeserializeOrcWriter(Configuration conf, Properties tblProps, StructObjectInspector sourceOi, List<Integer> sourceIncludes, boolean[] cacheIncludes, int allocSize, ExecutorService encodeExecutor) throws IOException
    {
        ...
        deserializeRead = new LazySimpleDeserializeRead(vrbCtx.getRowColumnTypeInfos(), vrbCtx.getRowdataTypePhysicalVariations(),/* useExternalBuffer */ true, createSerdeParams(conf, tblProps));
        ...
        if (usesSourceIncludes)
        {
            ...
            destinationOi = new LazySimpleStructObjectInspector(childNames, childOis, null, (byte) 0, null);
    ...
    {
        if (destinationBatch == sourceBatch)
        {
            return;
        }
    ...
    public void setCurrentStripeOffsets(long currentKnownTornStart, long firstStartOffset, long lastStartOffset, long fileOffset)
    {
        if (isAsync)
        {
            addWriteOp(new SetStripeDataOperation(currentKnownTornStart, firstStartOffset, lastStartOffset, fileOffset));
        } else
        {
