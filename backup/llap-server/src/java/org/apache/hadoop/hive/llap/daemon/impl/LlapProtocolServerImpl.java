...
public class LlapProtocolServerImpl extends AbstractService implements LlapProtocolBlockingPB, LlapManagementProtocolPB
{
    ...
    public LlapProtocolServerImpl(SecretManager secretManager, int numHandlers, ContainerRunner containerRunner, AtomicReference<InetSocketAddress> srvAddress, AtomicReference<InetSocketAddress> mngAddress, int srvPort, int externalClientsRpcPort, int mngPort, DaemonId daemonId, LlapDaemonExecutorMetrics executorMetrics)
    {
    ...
    @Override
    public SubmitWorkResponseProto submitWork(RpcController controller, SubmitWorkRequestProto request) throws ServiceException
    {
        try
        {
    ...
    @Override
    public SourceStateUpdatedResponseProto sourceStateUpdated(RpcController controller, SourceStateUpdatedRequestProto request) throws ServiceException
    {
        try
        {
    ...
    @Override
    public QueryCompleteResponseProto queryComplete(RpcController controller, QueryCompleteRequestProto request) throws ServiceException
    {
        try
        {
    ...
    @Override
    public TerminateFragmentResponseProto terminateFragment(RpcController controller, TerminateFragmentRequestProto request) throws ServiceException
    {
        try
        {
    ...
    @Override
    public UpdateFragmentResponseProto updateFragment(RpcController controller, UpdateFragmentRequestProto request) throws ServiceException
    {
        try
        {
    ...
    {
        String signSetting = HiveConf.getVar(conf, ConfVars.LLAP_REMOTE_TOKEN_REQUIRES_SIGNING).toLowerCase();
        switch (signSetting)
        {
            case "true":
            ...
            default:
            {
                throw new RuntimeException("Invalid value for " + ConfVars.LLAP_REMOTE_TOKEN_REQUIRES_SIGNING.varname + ": " + signSetting);
            }
        }
    }
    ...
    {
        return HiveConf.getBoolVar(conf, ConfVars.LLAP_VALIDATE_ACLS) && AccessControlList.WILDCARD_ACL_VALUE.equals(HiveConf.getVar(conf, ConfVars.LLAP_MANAGEMENT_ACL)) && "".equals(HiveConf.getVar(conf, ConfVars.LLAP_MANAGEMENT_ACL_DENY));
    }
    ...
    private void startProtocolServers(Configuration conf, BlockingService daemonImpl, BlockingService managementImpl)
    {
        ...
        server = LlapUtil.startProtocolServer(srvPort, numHandlers, srvAddress, conf, daemonImpl, LlapProtocolBlockingPB.class, secretManager, pp, ConfVars.LLAP_SECURITY_ACL, ConfVars.LLAP_SECURITY_ACL_DENY);
        ...
        {
            externalClientsRpcServer = LlapUtil.startProtocolServer(externalClientsRpcPort, numHandlers, null, conf, daemonImpl, LlapProtocolBlockingPB.class, secretManager, pp, ConfVars.LLAP_SECURITY_ACL, ConfVars.LLAP_SECURITY_ACL_DENY);
        ...
        mngServer = LlapUtil.startProtocolServer(mngPort, 2, mngAddress, conf, managementImpl, LlapManagementProtocolPB.class, secretManager, pp, ConfVars.LLAP_MANAGEMENT_ACL, ConfVars.LLAP_MANAGEMENT_ACL_DENY);
    }
    ...
    @Override
    public LlapDaemonProtocolProtos.PurgeCacheResponseProto purgeCache(final RpcController controller, final LlapDaemonProtocolProtos.PurgeCacheRequestProto request) throws ServiceException
    {
    ...
    @Override
    public LlapDaemonProtocolProtos.GetDaemonMetricsResponseProto getDaemonMetrics(final RpcController controller, final LlapDaemonProtocolProtos.GetDaemonMetricsRequestProto request) throws ServiceException
    {
        ...
        if (executorMetrics != null)
        {
            ...
            data.forEach((key, value) -> responseProtoBuilder.addMetrics(LlapDaemonProtocolProtos.MapEntry.newBuilder().setKey(key).setValue(value).build()));
        }
        return responseProtoBuilder.build();
    }

    @Override
    public LlapDaemonProtocolProtos.SetCapacityResponseProto setCapacity(final RpcController controller, final LlapDaemonProtocolProtos.SetCapacityRequestProto request) throws ServiceException
    {
        try
        {
    ...
    @Override
    public GetCacheContentResponseProto getCacheContent(RpcController controller, GetCacheContentRequestProto request)
    {
    ...
    private boolean determineIfSigningIsRequired(UserGroupInformation callingUser)
    {
        switch (isSigningRequiredConfig)
        {
            ...
            case EXCEPT_OWNER:
                return !clusterUser.equals(callingUser.getShortUserName());
            default:
                throw new AssertionError("Unknown value " + isSigningRequiredConfig);
        }
    }
