...
public class ContainerRunnerImpl extends CompositeService implements ContainerRunner, FragmentCompletionHandler, QueryFailedHandler
{
    ...
    public ContainerRunnerImpl(Configuration conf, int numExecutors, AtomicReference<Integer> localShufflePort, AtomicReference<InetSocketAddress> localAddress, long totalMemoryAvailableBytes, LlapDaemonExecutorMetrics metrics, AMReporter amReporter, QueryTracker queryTracker, Scheduler<TaskRunnerCallable> executorService, DaemonId daemonId, UgiFactory fsUgiFactory, SocketFactory socketFactory)
    {
    ...
    @Override
    public RegisterDagResponseProto registerDag(RegisterDagRequestProto request) throws IOException
    {
        QueryIdentifierProto identifier = request.getQueryIdentifier();
        Credentials credentials;
        if (request.hasCredentialsBinary())
        {
            credentials = LlapUtil.credentialsFromByteArray(request.getCredentialsBinary().toByteArray());
        } else
        {
            credentials = new Credentials();
        }
        queryTracker.registerDag(identifier.getApplicationIdString(), identifier.getDagIndex(), request.getUser(), credentials);
        LOG.info("Application with  id={}, dagId={} registered", identifier.getApplicationIdString(), identifier.getDagIndex());
    ...
    private void verifyJwtForExternalClient(SubmitWorkRequestProto request, String extClientAppIdFromSplit, String fragmentIdString)
    {
        LOG.info("Checking if request[{}] is from llap external client in a cloud based deployment", extClientAppIdFromSplit);
        if (request.getIsExternalClientRequest() && LlapUtil.isCloudDeployment(getConfig()))
        {
            ...
            Preconditions.checkState(extClientAppIdFromJwt.equals(extClientAppIdFromSplit), String.format("applicationId[%s] in request does not match to applicationId[%s] in JWT", extClientAppIdFromSplit, extClientAppIdFromJwt));
    ...
    private void checkSignature(SignableVertexSpec vertex, ByteString vertexBinary, SubmitWorkRequestProto request, String tokenUserName) throws SecurityException, IOException
    {
    ...
    private void logSecurityErrorRarely(String userName)
    {
        ...
        if (oldTime != 0 && (time - oldTime) < 1000000000L)
        {
            return; // 1 second
        }
        if (!lastLoggedError.compareAndSet(oldTime, time))
        {
            return;
        }
        String tokens = null;
    ...
    {
        QueryIdentifier queryIdentifier = new QueryIdentifier(request.getQueryIdentifier().getApplicationIdString(), request.getQueryIdentifier().getDagIndex());
        ...
        if (queryInfo != null)
        {
            ...
            {
                LOG.info("Issuing killFragment for completed query {} {}", queryIdentifier, fragmentInfo.getFragmentIdentifierString());
    ...
    public UpdateFragmentResponseProto updateFragment(UpdateFragmentRequestProto request) throws IOException
    {
        ...
        return UpdateFragmentResponseProto.newBuilder().setResult(result).setIsGuaranteed(isGuaranteed).build();
    }

    @Override
    public SetCapacityResponseProto setCapacity(SetCapacityRequestProto request)
    {
    ...
    private class KilledTaskHandlerImpl implements KilledTaskHandler
    {
        ...
        @Override
        public void taskKilled(String amLocation, int port, String umbilicalUser, Token<JobTokenIdentifier> jobToken, QueryIdentifier queryIdentifier, TezTaskAttemptID taskAttemptId)
        {
    ...
    static class UgiPool
    {
        ...
        private final Cache<String, BlockingQueue<UserGroupInformation>> ugiPool = CacheBuilder.newBuilder().removalListener(new RemovalListener<String, BlockingQueue<UserGroupInformation>>()
        {
            @Override
            public void onRemoval(RemovalNotification<String, BlockingQueue<UserGroupInformation>> notification)
            {
         ...
         */
        public UserGroupInformation getUmbilicalUgi(String appTokenIdentifier, Token<JobTokenIdentifier> appToken) throws ExecutionException
        {
            BlockingQueue<UserGroupInformation> queue = ugiPool.get(appTokenIdentifier, new Callable<BlockingQueue<UserGroupInformation>>()
            {
                @Override
