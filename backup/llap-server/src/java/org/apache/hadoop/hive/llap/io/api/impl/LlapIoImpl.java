...
public class LlapIoImpl implements LlapIo<VectorizedRowBatch>, LlapIoDebugDump
{
    ...
    private LlapIoImpl(Configuration conf) throws IOException
    {
        ...
        boolean isEncodeEnabled = useLowLevelCache && HiveConf.getBoolVar(conf, ConfVars.LLAP_IO_ENCODE_ENABLED);
        if (useLowLevelCache)
        {
            ...
            realCachePolicy = useLrfu ? new LowLevelLrfuCachePolicy(minAllocSize, totalMemorySize, conf) : new LowLevelFifoCachePolicy();
            ...
            if (isEncodeEnabled)
            {
                SerDeLowLevelCacheImpl serdeCacheImpl = new SerDeLowLevelCacheImpl(cacheMetrics, cachePolicyWrapper, allocator);
            ...
            EvictionDispatcher e = new EvictionDispatcher(dataCache, serdeCache, metadataCache, allocator);
        ...
        executor = new StatsRecordingThreadPool(numThreads, numThreads, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadFactoryBuilder().setNameFormat("IO-Elevator-Thread-%d").setDaemon(true).setThreadFactory(r -> new LlapPooledIOThread(r)).build());
        ...
        if (isEncodeEnabled)
        {
            ...
            encodeExecutor = new StatsRecordingThreadPool(encodeThreads, encodeThreads, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadFactoryBuilder().setNameFormat("IO-Elevator-Thread-OrcEncode-%d").setDaemon(true).setThreadFactory(r -> new LlapPooledIOThread(r)).build());
        } else
        {
            encodeExecutor = null;
        }
        ...
        this.orcCvp = new OrcColumnVectorProducer(metadataCache, dataCache, pathCache, bufferManagerOrc, conf, cacheMetrics, ioMetrics, tracePool);
        this.genericCvp = isEncodeEnabled ? new GenericColumnVectorProducer(serdeCache, bufferManagerGeneric, conf, cacheMetrics, ioMetrics, tracePool, encodeExecutor) : null;
    ...
    public long evictEntity(LlapDaemonProtocolProtos.EvictEntityRequestProto protoRequest)
    {
        ...
        final ProactiveEviction.Request request = ProactiveEviction.Request.Builder.create().fromProtoRequest(protoRequest).build();
        Predicate<CacheTag> predicate = tag -> request.isTagMatch(tag);
        boolean isInstantDeallocation = HiveConf.getBoolVar(daemonConf, HiveConf.ConfVars.LLAP_IO_PROACTIVE_EVICTION_INSTANT_DEALLOC);
    ...
    private class GenericDataCache implements DataCache, BufferObjectFactory
    {
        ...
        {
            return lowLevelCache.putFileData(fileKey, ranges, data, baseOffset, Priority.NORMAL, null, tag);
        }
    ...
    public MemoryBufferOrBuffers getParquetFooterBuffersFromCache(Path path, JobConf conf, @Nullable Object fileKey) throws IOException
    {
        ...
        MemoryBufferOrBuffers footerData = (fileKey == null) ? null : fileMetadataCache.getFileMetadata(fileKey);
        if (footerData != null)
        {
    ...
    public LlapDaemonProtocolProtos.CacheEntryList fetchCachedContentInfo()
    {
        if (useLowLevelCache)
        {
            ...
            LlapCacheMetadataSerializer serializer = new LlapCacheMetadataSerializer(fileMetadataCache, cache, daemonConf, pathCache, tracePool, realCachePolicy);
