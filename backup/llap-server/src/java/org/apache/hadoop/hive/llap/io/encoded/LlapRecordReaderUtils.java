...
public class LlapRecordReaderUtils
{
    ...
    {
        return codec == null || (codec instanceof DirectDecompressionCodec && ((DirectDecompressionCodec) codec).isAvailable()) ? SHIMS.getZeroCopyReader(file, pool) : null;
    }
    ...
    private static class DefaultLLapDataReader implements LlapDataReader
    {
        ...
        public OrcIndex readRowIndex(StripeInformation stripe, TypeDescription fileSchema, OrcProto.StripeFooter footer, boolean ignoreNonUtf8BloomFilter, boolean[] included, OrcProto.RowIndex[] indexes, boolean[] sargColumns, OrcFile.WriterVersion version, OrcProto.Stream.Kind[] bloomFilterKinds, OrcProto.BloomFilterIndex[] bloomFilterIndices) throws IOException
        {
            ...
            DiskRangeList ranges = planIndexReading(fileSchema, footer, ignoreNonUtf8BloomFilter, included, sargColumns, version, bloomFilterKinds);
                    ...
                    switch (stream.getKind())
                    {
                        case ROW_INDEX:
                            if (included == null || included[column])
                            {
                                ...
                                indexes[column] = OrcProto.RowIndex.parseFrom(InStream.createCodedInputStream(InStream.create("index", new BufferChunk(bb, 0), 0, stream.getLength(), compression)));
                            }
                            break;
                        case BLOOM_FILTER:
                        case BLOOM_FILTER_UTF8:
                            if (sargColumns != null && sargColumns[column])
                            {
                                ...
                                bb.position((int) (offset - range.getOffset()));
                                bb.limit((int) (bb.position() + stream.getLength()));
                                bloomFilterIndices[column] = OrcProto.BloomFilterIndex.parseFrom(InStream.createCodedInputStream(InStream.create("bloom_filter", new BufferChunk(bb, 0), 0, stream.getLength(), compression)));
                            }
                            break;
                        default:
                            break;
        ...
        public OrcProto.StripeFooter readStripeFooter(StripeInformation stripe) throws IOException
        {
            ...
            return OrcProto.StripeFooter.parseFrom(InStream.createCodedInputStream(InStream.create("footer", new BufferChunk(tailBuf, 0), 0, tailLength, compression)));
        }
