...
public final class LowLevelLrfuCachePolicy extends ProactiveEvictingCachePolicy.Impl implements LowLevelCachePolicy
{
    ...
    public LowLevelLrfuCachePolicy(int minBufferSize, long maxSize, Configuration conf)
    {
        ...
        LlapIoImpl.LOG.info("LRFU cache policy with min buffer size {} and lambda {} (heap size {})", minBufferSize, lambda, maxHeapSize);
    ...
    private void doNotifyUnderHeapLock(LlapCacheableBuffer buffer)
    {
        ...
        buffer.priority = (buffer.lastUpdate == -1) ? F0 : touchPriority(time, buffer.lastUpdate, buffer.priority);
        buffer.lastUpdate = time;
    ...
    private long evictOrPurge(boolean isPurge)
    {
        ...
        try
        {
            ...
            while (current != null)
            {
                ...
                if (invalidateResult == LlapCacheableBuffer.INVALIDATE_ALREADY_INVALID && instantProactiveEviction && current.isMarkedForEviction())
                {
        ...
        try
        {
            ...
            for (int i = 0; i < oldHeapSize; ++i)
            {
                ...
                if (invalidateResult != LlapCacheableBuffer.INVALIDATE_OK)
                {
                    if (invalidateResult == LlapCacheableBuffer.INVALIDATE_ALREADY_INVALID && instantProactiveEviction && result.isMarkedForEviction())
                    {
                        // Found a marked and instantly deallocated buffer. If this is a purge we need to do a proper cleanup of
                        // this buffer. If this is a proactive sweep cleanup will be done later in this method.
                        if (isPurge)
        ...
        {
            LlapIoImpl.LOG.info("PROACTIVE_EVICTION: evicted {} from LRFU policy", LlapUtil.humanReadableByteCount(evicted));
        }
        return evicted;
    }
        ...
        try
        {
            ...
            while (evicted < memoryToReserve && nextCandidate != null)
            {
                ...
                if (LlapCacheableBuffer.INVALIDATE_OK != invalidateResult)
                {
                    ...
                    if (instantProactiveEviction && LlapCacheableBuffer.INVALIDATE_ALREADY_INVALID == invalidateResult && lockedBuffer.isMarkedForEviction())
                    {
    ...
    private LlapCacheableBuffer evictHeapElementUnderLock(long time, int ix)
    {
        ...
        if (instantProactiveEviction && LlapCacheableBuffer.INVALIDATE_ALREADY_INVALID == invalidateResult && result.isMarkedForEviction())
        {
            // Cleanup an already marked and deallocated buffer - this call is needed for administration purposes
            evictionListener.notifyProactivelyEvicted(result);
        }
    ...
    public void debugDumpShort(StringBuilder sb)
    {
        ...
        sb.append("\nLRFU eviction list: ").append(metricData[PolicyMetrics.LISTSIZE]).append(" items");
        sb.append("\nLRFU eviction heap: ").append(heapSize).append(" items (of max ").append(maxHeapSize).append(")");
        sb.append("\nLRFU data on heap: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.DATAONHEAP]));
        sb.append("\nLRFU metadata on heap: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.METAONHEAP]));
        sb.append("\nLRFU data on eviction list: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.DATAONLIST]));
        sb.append("\nLRFU metadata on eviction list: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.METAONLIST]));
        sb.append("\nLRFU data locked: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.LOCKEDDATA]));
        sb.append("\nLRFU metadata locked: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.LOCKEDMETA]));
        sb.append("\nLRFU BP wrapper: ").append(metricData[PolicyMetrics.BPWRAPCNT]).append(" total items, ").append(metricData[PolicyMetrics.BPWRAPDISTINCT]).append(" distinct buffers, that use cache space: ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.BPWRAPDATA])).append(" for data, ").append(LlapUtil.humanReadableByteCount(metricData[PolicyMetrics.BPWRAPMETA])).append(" for metadata.");
    }

    @Override
    public List<LlapCacheableBuffer> getHotBuffers()
    {
        ...
        long t = timer.get();
        Arrays.sort(copy, nullsLast(comparing(b -> b.lastUpdate == t ? b.priority : expirePriority(t, b.lastUpdate, b.priority))));
    ...
    {
        PolicyMetrics("LRFU cache policy based metrics"), DataOnHeap("Amount of bytes used for data on min-heap"), DataOnList("Amount of bytes used for data on eviction short list"), MetaOnHeap("Amount of bytes used for meta data on min-heap"), MetaOnList("Amount of bytes used for meta data on eviction short list"), DataLocked("Amount of locked data in bytes (in use)"), MetaLocked("Amount of locked meta data in bytes (in use)"), HeapSize("Number of buffers on the min-heap"), HeapSizeMax("Capacity (number of buffers) of the min-heap"), ListSize("Number of buffers on the eviction short list"), BPWrapperCount("Number of all buffers in BPWrapper threadlocals"), BPWrapperDistinct("Number of distinct buffers in BPWrapper threadlocals"), BPWrapperData("Amount of bytes for data buffers in BPWrapper threadlocals"), BPWrapperMeta("Amount of bytes for metadata buffers in BPWrapper threadlocals"), TotalData("Total amount of bytes, used for data"), TotalMeta("Total amount of bytes, used for meta data");
    ...
    class PolicyMetrics implements MetricsSource
    {
        ...
        public synchronized void getMetrics(MetricsCollector collector, boolean all)
        {
            ...
            // start a new record
            MetricsRecordBuilder mrb = collector.addRecord(PolicyInformation.PolicyMetrics).setContext("cache").tag(MsInfo.ProcessName, MetricsUtils.METRICS_PROCESS_NAME).tag(MsInfo.SessionId, session);

            // add the values to the new record
            mrb.addCounter(PolicyInformation.DataOnHeap, usageStats[DATAONHEAP]).addCounter(PolicyInformation.DataOnList, usageStats[DATAONLIST]).addCounter(PolicyInformation.MetaOnHeap, usageStats[METAONHEAP]).addCounter(PolicyInformation.MetaOnList, usageStats[METAONLIST]).addCounter(PolicyInformation.DataLocked, usageStats[LOCKEDDATA]).addCounter(PolicyInformation.MetaLocked, usageStats[LOCKEDMETA]).addCounter(PolicyInformation.HeapSize, heapSize).addCounter(PolicyInformation.HeapSizeMax, maxHeapSize).addCounter(PolicyInformation.ListSize, usageStats[LISTSIZE]).addCounter(PolicyInformation.BPWrapperCount, usageStats[BPWRAPCNT]).addCounter(PolicyInformation.BPWrapperDistinct, usageStats[BPWRAPDISTINCT]).addCounter(PolicyInformation.BPWrapperData, usageStats[BPWRAPDATA]).addCounter(PolicyInformation.TotalData, usageStats[DATAONHEAP] + usageStats[DATAONLIST] + usageStats[BPWRAPDATA]).addCounter(PolicyInformation.TotalMeta, usageStats[METAONHEAP] + usageStats[METAONLIST] + usageStats[BPWRAPMETA]);
        }
    }
}
