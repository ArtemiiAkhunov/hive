 ...
 */
public final class BuddyAllocator implements EvictionAwareAllocator, StoppableAllocator, BuddyAllocatorMXBean, LlapIoDebugDump
{
    ...
    private static long getMaxTotalMemorySize(Configuration conf)
    {
        ...
        throw new RuntimeException("Allocator space is too small for reasonable operation; " + ConfVars.LLAP_IO_MEMORY_MAX_SIZE.varname + "=" + maxSize + ", but at least " + MIN_TOTAL_MEMORY_SIZE + " is required. If you cannot spare any memory, you can " + "disable LLAP IO entirely via " + ConfVars.LLAP_IO_ENABLED.varname);
    }

    @VisibleForTesting
    public BuddyAllocator(boolean isDirectVal, boolean isMappedVal, int minAllocVal, int maxAllocVal, int arenaCount, long maxSizeVal, long defragHeadroom, String mapPath, MemoryManager memoryManager, LlapDaemonCacheMetrics metrics, String discardMethod, boolean doPreallocate)
    {
        this(isDirectVal, isMappedVal, minAllocVal, maxAllocVal, arenaCount, maxSizeVal, defragHeadroom, mapPath, memoryManager, metrics, discardMethod, doPreallocate, MAX_FORCED_EVICTION_SIZE);
    }

    @VisibleForTesting
    public BuddyAllocator(boolean isDirectVal, boolean isMappedVal, int minAllocVal, int maxAllocVal, int arenaCount, long maxSizeVal, long defragHeadroom, String mapPath, MemoryManager memoryManager, LlapDaemonCacheMetrics metrics, String discardMethod, boolean doPreallocate, int maxForcedEvictionSize)
    {
    ...
    @Override
    public void allocateMultiple(MemoryBuffer[] dest, int size, BufferObjectFactory factory) throws AllocatorOutOfMemoryException
    {
        allocateMultiple(dest, size, factory, null);
    }

    @Override
    public void allocateMultiple(MemoryBuffer[] dest, int size, BufferObjectFactory factory, AtomicBoolean isStopped) throws AllocatorOutOfMemoryException
    {
        ...
        int destAllocIx = allocateFast(dest, null, 0, dest.length, freeListIx, allocationSize, (int) (threadId % arenaCount), arenaCount);
        ...
        while (totalForceEvictedBytes < maxForcedEvictionSize && emptyAttempt < MAX_DISCARD_ATTEMPTS)
        {
            ...
            if (allocationAttempt >= MAX_FAST_ATTEMPT)
            {
                ...
                currentEvicted = memoryManager.evictMemory(dest.length - destAllocIx + allocationAttempt - MAX_FAST_ATTEMPT << allocLog2);
            ...
            destAllocIx = allocateFast(dest, null, destAllocIx, dest.length, freeListIx, allocationSize, startArenaIx, arenaCount);
            if (destAllocIx == dest.length)
            {
                return;
            }
            destAllocIx = allocateWithSplit(dest, null, destAllocIx, dest.length, freeListIx, allocationSize, startArenaIx, arenaCount, -1);
            if (destAllocIx == dest.length)
            {
                return;
            }
        ...
        LlapIoImpl.LOG.warn(FAILED_TO_ALLOCATE_MSG, dest.length, allocationSize, allocationAttempt, totalForceEvictedBytes, destAllocIx << allocLog2);
        ...
        } finally
        {
            ...
            {
                LlapIoImpl.LOG.info("Allocation of " + dest.length + " buffers of size " + size + " took " + discardsAttempt + " attempts to free enough memory; force-released " + memoryForceReleased);
            }
        }
    }
    ...
    private static final class DiscardContext
    {
        ...
        {
            return "[victimHeaders=" + Arrays.toString(victimHeaders) + ", victimCount=" + victimCount + ", baseHeaders=" + Arrays.toString(baseHeaders) + ", baseCount=" + baseCount + ", remainingToFind=" + remainingToFind + "]";
        }
    }
    ...
    private int allocateWithSplit(MemoryBuffer[] dest, long[] destHeaders, int destIx, int destCount, int freeListIx, int allocSize, int startArenaIx, int arenaCount, int maxSplitFreeListIx)
    {
        ...
        do
        {
            int newDestIx = arenas[arenaIx].allocateWithSplit(freeListIx, dest, destHeaders, destIx, destCount, allocSize, maxSplitFreeListIx);
            if (newDestIx == destCount)
            {
                return newDestIx;
            }
    ...
    private int allocateFromDiscardResult(MemoryBuffer[] dest, int destAllocIx, int freeListIx, int allocationSize, DiscardContext discardResult)
    {
        for (int i = 0; i < discardResult.resultCount; ++i)
        {
            ...
            destAllocIx = arenas[getFirstInt(result)].allocateFromDiscard(dest, destAllocIx, getSecondInt(result), freeListIx, allocationSize);
        }
        return destAllocIx;
    }
    ...
    private class Arena
    {
        ...
        public void reserveDiscardBruteForce(int freeListIx, DiscardContext ctx, int startHeaderIx)
        {
            ...
            do
            {
                long reserveResult = reserveBlockContents(freeListIx, headerIx, ctx.victimHeaders, ctx.victimCount, true);
        ...
        private int allocateWithSplit(int freeListIx, MemoryBuffer[] dest, long[] destHeaders, int destIx, int destCount, int allocSize, int maxSplitFreeListIx)
        {
            ...
            try
            {
                ...
                destIx = allocateFromFreeListUnderLock(freeList, freeListIx, dest, destHeaders, destIx, destCount, allocSize);
        ...
        public void deallocate(LlapAllocatorBuffer buffer, boolean isAfterMove)
        {
            ...
            if (buffer != null && buffer.byteBuffer != null)
            {
                ...
                if (assertsEnabled && !isAfterMove)
                {
                    ...
                    if (buf != buffer)
                    {
                        failWithLog(arenaIx + ":" + headerIx + " => " + toDebugString(buffer) + ", " + toDebugString(buf));
                    }
        ...
        private void addToFreeListWithMerge(int headerIx, int freeListIx, CasLog.Src src)
        {
            while (true)
            {
                ...
                try
                {
                    if ((freeListIx == freeLists.length - 1) || headers[bHeaderIx] != makeHeader(freeListIx, false))
                    {
        ...
        {
            int bOffset = offsetFromHeaderIndex(headerIx), bpHeaderIx = getPrevFreeListItem(bOffset), bnHeaderIx = getNextFreeListItem(bOffset);
    ...
    private void assertBufferLooksValid(int freeListIx, LlapAllocatorBuffer buf, int arenaIx, int headerIx)
    {
        ...
        failWithLog("Race; allocation size " + buf.allocSize + ", not " + allocSizeFromFreeList(freeListIx) + " for free list " + freeListIx + " at " + arenaIx + ":" + headerIx);
    }
    ...
    private void checkHeaderByte(int arenaIx, int headerIx, int freeListIx, boolean isLocked, byte header)
    {
        ...
        {
            failWithLog("Expected " + arenaIx + ":" + headerIx + " " + (isLocked ? "" : "not ") + "locked: " + header);
        }
        if (freeListIx < 0)
        {
            return;
        ...
        {
            failWithLog("Expected " + arenaIx + ":" + headerIx + " in list " + freeListIx + ": " + freeListFromHeader(header));
        }
    }
    ...
    private final static class CasLog
    {
        ...
        public enum Src
        {
            NEWLY_CLEARED, SPLIT_AFTER_BUF, SPLIT_AFTER_DEFRAG, ALLOC_SPLIT_DEFRAG, ALLOC_SPLIT_BUF, ALLOC_DEFRAG, EMPTY_V, NEW_BASE, CTOR, MOVE_TO_NESTED, MOVE_TO_ALLOC, ABANDON_MOVE, ABANDON_AT_END, ABANDON_BASE, CLEARED_BASE, CLEARED_VICTIM, UNUSABLE_NESTED, ABANDON_NESTED, DEALLOC, ALLOC_FREE_DEFRAG, ALLOC_FREE_BUF
        }
        ...
        public static final int START_MOVE = 0, SET_NB = 1, SET_BUF = 2, SET_FREE = 3, ADD_TO_LIST = 4, REMOVE_FROM_LIST = 5, ERROR = 6, UNSET = 7;
        ...
        private int dumpOneLine(int ix)
        {
            ...
            switch (event)
            {
                case START_MOVE:
                {
                    LlapIoImpl.LOG.info(prefix(ix) + " started to move " + header(log[ix + 2]) + " " + Integer.toHexString(getSecondInt(log[ix])));
                    return ix + 3;
                }
                case SET_NB:
                {
                    LlapIoImpl.LOG.info(prefix(ix) + " " + src(getSecondInt(log[ix])) + " set " + header(log[ix + 2]) + " to taken of size " + log[ix + 3]);
                    return ix + 4;
                }
                case SET_FREE:
                {
                    LlapIoImpl.LOG.info(prefix(ix) + " " + src(getSecondInt(log[ix])) + " set " + header(log[ix + 2]) + " to free of size " + log[ix + 3]);
                    return ix + 4;
                }
                case UNSET:
                {
                    LlapIoImpl.LOG.info(prefix(ix) + " " + src(getSecondInt(log[ix])) + " unset [" + header(log[ix + 2]) + ", " + header(log[ix + 3]) + "]");
                    return ix + 4;
                }
                case SET_BUF:
                {
                    LlapIoImpl.LOG.info(prefix(ix) + " " + src(getSecondInt(log[ix])) + " set " + header(log[ix + 2]) + " to " + Integer.toHexString((int) log[ix + 3]));
                    return ix + 4;
                }
                case ADD_TO_LIST:
                {
