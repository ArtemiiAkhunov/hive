...
public class GenericColumnVectorProducer implements ColumnVectorProducer
{
    ...
    @Override
    public ReadPipeline createReadPipeline(Consumer<ColumnVectorBatch> consumer, FileSplit split, Includes includes, SearchArgument sarg, QueryFragmentCounters counters, SchemaEvolutionFactory sef, InputFormat<?, ?> sourceInputFormat, Deserializer sourceSerDe, Reporter reporter, JobConf job, Map<Path, PartitionDesc> parts) throws IOException
    {
        ...
        SerDeEncodedDataReader reader = new SerDeEncodedDataReader(cache, bufferManager, conf, split, includes.getPhysicalColumnIds(), edc, job, reporter, sourceInputFormat, sourceSerDe, counters, fm.getSchema(), parts, encodeExecutor);
    ...
    private static final class SerDeFileMetadata implements ConsumerFileMetadata
    {
        ...
        {
            TypeDescription schema = OrcInputFormat.convertTypeInfo(TypeInfoUtils.getTypeInfoFromObjectInspector(sourceSerDe.getObjectInspector()));
            this.schema = schema;
