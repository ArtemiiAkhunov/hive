...
public class MetadataCache implements LlapIoDebugDump, FileMetadataCache
{
    ...
    public DiskRangeList getIncompleteCbs(Object fileKey, DiskRangeList ranges, long baseOffset, BooleanRef gotAllData)
    {
        if (estimateErrors == null)
        {
            return ranges;
    ...
    {
        sb.append("\nMetadata cache state: ").append(metadata.size()).append(" files and stripes, ").append(metadata.values().parallelStream().mapToLong(value ->
        {
            ...
            return sum;
        }).sum()).append(" total used bytes, ").append(estimateErrors.size()).append(" files w/ORC estimate");
    }
    ...
    private <T extends LlapBufferOrBuffers> boolean lockOldVal(Object key, T newVal, T oldVal)
    {
        ...
        {
            LlapIoImpl.CACHE_LOGGER.trace("Trying to cache when metadata is already cached for" + " {}; old {}, new {}", key, oldVal, newVal);
        }
    ...
    private <T> LlapBufferOrBuffers wrapBb(LlapBufferOrBuffers result, T key, ByteBuffer tailBuffer, CacheTag tag, AtomicBoolean isStopped)
    {
        ...
        } else
        {
            ...
            @SuppressWarnings("unchecked") LlapMetadataBuffer<T>[] results = new LlapMetadataBuffer[allocCount];
    ...
    private boolean lockBuffer(LlapBufferOrBuffers buffers, boolean doNotifyPolicy)
    {
        ...
        {
            if (lockOneBuffer(bufferArray[i], doNotifyPolicy))
            {
                continue;
            }
            for (int j = 0; j < i; ++j)
    ...
    private void discardMultiBuffer(LlapBufferOrBuffers removed)
    {
        ...
        for (LlapAllocatorBuffer buf : removed.getMultipleLlapBuffers())
        {
            ...
            switch (result)
            {
                case LlapAllocatorBuffer.INVALIDATE_ALREADY_INVALID:
    ...
    private final static class StripeKey
    {
        ...
        public boolean equals(Object obj)
        {
            ...
            return ((fileKey == null) == (other.fileKey == null)) && (fileKey == null || fileKey.equals(other.fileKey)) && (stripeIx == other.stripeIx);
        }
    }
    ...
    public final static class LlapMetadataBuffer<T> extends LlapAllocatorBuffer implements LlapBufferOrBuffers
    {
        private final T key;
