...
public abstract class LlapAllocatorBuffer extends LlapCacheableBuffer implements MemoryBuffer
{
    ...
    public String toString()
    {
        ...
        return "0x" + Integer.toHexString(System.identityHashCode(this)) + "(" + State.getRefCount(state) + (flags == 0 ? "" : (", " + State.toFlagString(flags))) + ")";
    }
    ...
    {
        return "0x" + Integer.toHexString(System.identityHashCode(this)) + "(" + State.getArena(state) + ":" + State.getHeader(state) + "; " + allocSize + "; " + State.toFlagString(State.getAllFlags(state)) + ")";
    }
    ...
    {
        public static final int FLAG_MOVING = 0b000001, // Locked by someone to move or force-evict.
        ...
        private static final int FLAGS_WIDTH = 6, REFCOUNT_WIDTH = 18, ARENA_WIDTH = 16, HEADER_WIDTH = 24;

        public static final long MAX_REFCOUNT = (1 << REFCOUNT_WIDTH) - 1;

        private static final int REFCOUNT_SHIFT = FLAGS_WIDTH, ARENA_SHIFT = REFCOUNT_SHIFT + REFCOUNT_WIDTH, HEADER_SHIFT = ARENA_SHIFT + ARENA_WIDTH;

        private static final long FLAGS_MASK = (1L << FLAGS_WIDTH) - 1, REFCOUNT_MASK = ((1L << REFCOUNT_WIDTH) - 1) << REFCOUNT_SHIFT, ARENA_MASK = ((1L << ARENA_WIDTH) - 1) << ARENA_SHIFT, HEADER_MASK = ((1L << HEADER_WIDTH) - 1) << HEADER_SHIFT;
