...
public class WmFragmentCounters
{
    ...
    {
        return fixedCounters.get(LlapWmCounters.GUARANTEED_QUEUED_NS.ordinal()) + fixedCounters.get(LlapWmCounters.SPECULATIVE_QUEUED_NS.ordinal());
    }

    public long getRunningTime()
    {
        return fixedCounters.get(LlapWmCounters.GUARANTEED_RUNNING_NS.ordinal()) + fixedCounters.get(LlapWmCounters.SPECULATIVE_RUNNING_NS.ordinal());
    }
        ...
        synchronized (this)
        {
            ...
            switch (currentState)
            {
                ...
                case NONE:
                    return;
                case QUEUED:
                    counter = getQueuedCounter(isGuaranteed);
                    break;
                case RUNNING:
                    counter = getRunningCounter(isGuaranteed);
                    break;
                default:
                    throw new AssertionError(currentState);
            }
