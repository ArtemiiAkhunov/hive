...
class FileCache<T>
{
    ...
    boolean incRef()
    {
        while (true)
        {
            ...
            if (value == EVICTED_REFCOUNT)
            {
                return false;
            }
            if (value == EVICTING_REFCOUNT)
            {
                continue; // spin until it resolves; extremely rare
            }
            assert value >= 0;
    ...
    boolean startEvicting()
    {
        while (true)
        {
            ...
            if (refCount.compareAndSet(value, EVICTING_REFCOUNT))
            {
                return true;
            }
        }
    ...
    public static <T> FileCache<T> getOrAddFileSubCache(ConcurrentHashMap<Object, FileCache<T>> cache, Object fileKey, Function<Void, T> createFunc, CacheTag tag)
    {
        ...
        { // Overwhelmingly executes once.
            ...
            if (subCache != null)
            {
                if (subCache.incRef())
                {
                    return subCache; // Main path - found it, incRef-ed it.
                }
                if (newSubCache == null)
                {
                ...
                if (cache.replace(fileKey, subCache, newSubCache))
                {
                    return newSubCache;
                }
            ...
            if (oldSubCache == null)
            {
                return newSubCache; // Main path 2 - created a new file cache.
            }
            if (oldSubCache.incRef())
            {
                return oldSubCache; // Someone created one in parallel.
            }
            // Someone created one in parallel and then it went stale.
            if (cache.replace(fileKey, oldSubCache, newSubCache))
            {
                return newSubCache;
            }
