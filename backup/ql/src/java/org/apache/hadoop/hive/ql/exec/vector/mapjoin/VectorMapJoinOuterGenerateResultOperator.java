...
public abstract class VectorMapJoinOuterGenerateResultOperator extends VectorMapJoinGenerateResultOperator
{
     ...
     * @param inputSelectedInUse Whether the (original) input batch is selectedInUse.
     * @param inputLogicalSize   The (original) input batch size.
     */
     ...
     *
     * @param batch    The vectorized row batch.
     * @param selected The (physical) batch indices to apply the expression to.
     * @param size     The size of selected.
     */
     ...
     * @param inputSelectedInUse Whether the (original) input batch is selectedInUse.
     * @param inputLogicalSize   The (original) input batch size.
     * @param remove             The indices to remove.  They must all be present in input selected array.
     * @param removeSize         The size of remove.
     * @param difference         The resulting difference -- the input selected array indices not in the
     *                           remove array.
     * @return The resulting size of the difference array.
     ...
     * a difference array.
     *
     * @param all        The selected array containing all members.
     * @param allSize    The size of all.
     * @param remove     The indices to remove.  They must all be present in input selected array.
     * @param removeSize The size of remove.
     * @param difference The resulting difference -- the all array indices not in the
     *                   remove array.
     * @return The resulting size of the difference array.
     * @throws HiveException
     */
     ...
     * @param sortMerged     The resulting sort merge of selected1 and selected2.
     * @return The resulting size of the sortMerged array.
     * @throws HiveException
     */
     ...
     *
     * @param batch               The big table batch with any matching and any non matching rows both as
     *                            selected in use.
     * @param allMatchCount       Number of matches in allMatches.
     * @param equalKeySeriesCount Number of single value matches.
     * @param atLeastOneNonMatch  Whether at least one row was a non-match.
     * @param inputSelectedInUse  A copy of the batch's selectedInUse flag on input to the process method.
     * @param inputLogicalSize    The batch's size on input to the process method.
     * @param spillCount          Number of spills in spills.
     * @param hashMapResultCount  Number of entries in hashMapResults.
     */
    public void finishOuter(VectorizedRowBatch batch, int allMatchCount, int equalKeySeriesCount, boolean atLeastOneNonMatch, boolean inputSelectedInUse, int inputLogicalSize, int spillCount, int hashMapResultCount) throws IOException, HiveException
    {
        ...
        if (spillCount > 0)
        {
            ...
            {
                noMatchCount = subtract(nonSpills, nonSpillCount, allMatches, allMatchCount, noMatchs);
        ...
        } else
        {
            ...
            if (atLeastOneNonMatch)
            {
                noMatchCount = subtractFromInputSelected(inputSelectedInUse, inputLogicalSize, allMatches, allMatchCount, noMatchs);
        ...
        if (allMatchCount > 0)
        {
            ...
            for (int i = 0; i < equalKeySeriesCount; i++)
            {
                ...
                if (isSingleValue)
                {
                    numSel = generateHashMapResultSingleValue(batch, hashMapResult, allMatches, allMatchesIndex, duplicateCount, numSel);
                } else
                {
                    generateHashMapResultMultiValue(batch, hashMapResult, allMatches, allMatchesIndex, duplicateCount);
                }
            }
     ...
     * @param batch       The big table batch with any matching and any non matching rows both as
     *                    selected in use.
     * @param noMatchs    A subset of the rows of the batch that are non matches.
     * @param noMatchSize Number of non matches in noMatchs.
     */
     ...
     * @param batch               The big table batch with any matching and any non matching rows both as
     *                            selected in use.
     * @param joinResult          The hash map lookup result for the repeated key.
     * @param hashMapResult       The array of all hash map results for the batch.
     * @param someRowsFilteredOut Whether some rows of the repeated key batch were knocked out by the filter.
     * @param inputSelectedInUse  A copy of the batch's selectedInUse flag on input to the process method.
     * @param inputLogicalSize    The batch's size on input to the process method.
     */
    ...
    protected void generateFullOuterLongKeySmallTableNoMatches() throws HiveException
    {
        ...
        {
            singleSmallTableKeyOutputColumnVector = (LongColumnVector) overflowBatch.cols[allSmallTableKeyColumnNums[0]];
        } else
        {
        ...
        VectorMapJoinNonMatchedIterator nonMatchedIterator = hashMap.createNonMatchedIterator(matchTracker);
    ...
    protected void generateFullOuterMultiKeySmallTableNoMatches() throws HiveException
    {
        ...
        while (nonMatchedIterator.findNextNonMatched())
        {
            ...
            while (byteSegmentRef != null)
            {
                ...
                {
                    doSmallTableKeyDeserializeRow(overflowBatch, overflowBatch.size, keyBytes, keyOffset, keyLength);
                }
    ...
    protected void generateFullOuterStringKeySmallTableNoMatches() throws HiveException
    {
        ...
        if (allSmallTableKeyColumnIncluded[0])
        {
            singleSmallTableKeyOutputColumnVector = (BytesColumnVector) overflowBatch.cols[allSmallTableKeyColumnNums[0]];
        } else
        {
            singleSmallTableKeyOutputColumnVector = null;
        }
        ...
        while (nonMatchedIterator.findNextNonMatched())
        {
            ...
            while (byteSegmentRef != null)
            {
                ...
                if (singleSmallTableKeyOutputColumnVector != null)
                {
                    ...
                    {
                        singleSmallTableKeyOutputColumnVector.setVal(overflowBatch.size, keyBytes, keyOffset, keyLength);
                        singleSmallTableKeyOutputColumnVector.isNull[overflowBatch.size] = false;
                    }
                }
