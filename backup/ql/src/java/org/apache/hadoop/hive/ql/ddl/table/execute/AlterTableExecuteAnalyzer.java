...
public class AlterTableExecuteAnalyzer extends AbstractAlterTableAnalyzer
{
    ...
    private static AlterTableExecuteDesc getFastForwardDesc(TableName tableName, Map<String, String> partitionSpec, ASTNode command) throws SemanticException
    {
        ...
        AlterTableExecuteSpec spec = new AlterTableExecuteSpec(FAST_FORWARD, new FastForwardSpec(branchName, targetBranchName));
        return new AlterTableExecuteDesc(tableName, partitionSpec, spec);
    }
    ...
    {
        AlterTableExecuteSpec<AlterTableExecuteSpec.SetCurrentSnapshotSpec> spec = new AlterTableExecuteSpec(SET_CURRENT_SNAPSHOT, new AlterTableExecuteSpec.SetCurrentSnapshotSpec(childNode.getText()));
        return new AlterTableExecuteDesc(tableName, partitionSpec, spec);
    }

    private static AlterTableExecuteDesc getExpireSnapshotDesc(TableName tableName, Map<String, String> partitionSpec, List<Node> children) throws SemanticException
    {
        ...
        } else if (children.size() == 3)
        {
            ...
            spec = new AlterTableExecuteSpec(EXPIRE_SNAPSHOT, new ExpireSnapshotsSpec(fromTime.toEpochMilli(), toTime.toEpochMilli()));
