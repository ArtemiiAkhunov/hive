...
public class GroupByDesc extends AbstractOperatorDesc
{
    ...
    public GroupByDesc(final Mode mode, final List<String> outputColumnNames, final List<ExprNodeDesc> keys, final List<AggregationDesc> aggregators, final float groupByMemoryUsage, final float memoryThreshold, final float minReductionHashAggr, final float minReductionHashAggrLowerBound, final float hashAggrFlushPercent, final List<Long> listGroupingSets, final boolean groupingSetsPresent, final int groupingSetsPosition, final boolean isDistinct)
    {
        this(mode, outputColumnNames, keys, aggregators, false, groupByMemoryUsage, memoryThreshold, minReductionHashAggr, minReductionHashAggrLowerBound, hashAggrFlushPercent, listGroupingSets, groupingSetsPresent, groupingSetsPosition, isDistinct);
    }

    public GroupByDesc(final Mode mode, final List<String> outputColumnNames, final List<ExprNodeDesc> keys, final List<AggregationDesc> aggregators, final boolean bucketGroup, final float groupByMemoryUsage, final float memoryThreshold, final float minReductionHashAggr, final float minReductionHashAggrLowerBound, final float hashAggrFlushPercent, final List<Long> listGroupingSets, final boolean groupingSetsPresent, final int groupingSetsPosition, final boolean isDistinct)
    {
        this.mode = mode;
    ...
    {
        return groupingSetPosition >= 0 && outputColumnNames.size() != keys.size() + aggregators.size();
    }
    ...
    public Object clone()
    {
        ...
        return new GroupByDesc(this.mode, outputColumnNames, keys, aggregators, this.groupByMemoryUsage, this.memoryThreshold, this.minReductionHashAggr, this.minReductionHashAggrLowerBound, this.hashAggrFlushPercent, listGroupingSets, this.groupingSetsPresent, this.groupingSetPosition, this.isDistinct);
    }
    ...
    public class GroupByOperatorExplainVectorization extends OperatorExplainVectorization
    {
        ...
        public List<String> getVectorOutputConditionsNotMet()
        {
            ...
            results.add(getComplexTypeWithGroupByEnabledCondition(isVectorizationComplexTypesEnabled, isVectorizationGroupByComplexTypesEnabled));
            return results;
        }
    ...
    public boolean isSame(OperatorDesc other)
    {
        if (getClass().getName().equals(other.getClass().getName()))
        {
            ...
            return Objects.equals(getModeString(), otherDesc.getModeString()) && Objects.equals(getKeyString(), otherDesc.getKeyString()) && Objects.equals(getOutputColumnNames(), otherDesc.getOutputColumnNames()) && pruneGroupingSetId() == otherDesc.pruneGroupingSetId() && Objects.equals(getAggregatorStrings(), otherDesc.getAggregatorStrings()) && getBucketGroup() == otherDesc.getBucketGroup();
        }
        return false;
    }
}
