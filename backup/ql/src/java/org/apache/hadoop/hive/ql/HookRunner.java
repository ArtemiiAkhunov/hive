...
public class HookRunner
{
    ...
    void runBeforeParseHook(String command)
    {
        ...
        {
            QueryLifeTimeHookContext qhc = new QueryLifeTimeHookContextImpl.Builder().withHiveConf(conf).withCommand(command).build();
     ...
     */
    void runAfterCompilationHook(DriverContext driverContext, Context analyzerContext, Throwable compileException) throws Exception
    {
        List<QueryLifeTimeHook> queryHooks = hooks.getHooks(QUERY_LIFETIME_HOOKS);
        if (!queryHooks.isEmpty())
        {
            ...
            QueryLifeTimeHookContext qhc = new QueryLifeTimeHookContextImpl.Builder().withHiveConf(conf).withCommand(analyzerContext.getCmd()).withHookContext(hookContext).build();
    ...
    void runBeforeExecutionHook(String command, HookContext hookContext)
    {
        ...
        if (!queryHooks.isEmpty())
        {
            QueryLifeTimeHookContext qhc = new QueryLifeTimeHookContextImpl.Builder().withHiveConf(conf).withCommand(command).withHookContext(hookContext).build();
    ...
    public ASTNode runPreAnalyzeHooks(HiveSemanticAnalyzerHookContext hookCtx, ASTNode tree) throws HiveException
    {
        try
        {
            for (HiveSemanticAnalyzerHook hook : hooks.getHooks(SEMANTIC_ANALYZER_HOOK, HiveSemanticAnalyzerHook.class))
            {
