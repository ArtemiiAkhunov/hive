...
public class SubQueryUtils
{
    ...
    static public void subqueryRestrictionCheck(QB qb, ASTNode subqueryExprNode, RelNode srcRel, boolean forHavingClause, Context ctx, LinkedHashMap<RelNode, RowResolver> relToHiveRR) throws SemanticException
    {
         ...
         */
        if (subqueryExprNode.getChildren().size() == 3 && subqueryExprNode.getChild(2).getType() == HiveParser.TOK_SUBQUERY_EXPR)
        {
            throw new CalciteSubquerySemanticException(ASTErrorUtils.getMsg(ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(), subqueryExprNode.getChild(2), "SubQuery on left hand side is not supported."));
        }

        // avoid subquery restrictions for SOME/ALL for now
        if (subqueryExprNode.getChild(0).getChildCount() > 1 && (subqueryExprNode.getChild(0).getChild(1).getType() == HiveParser.KW_SOME || subqueryExprNode.getChild(0).getChild(1).getType() == HiveParser.KW_ALL))
        {
            return;
        }
        ...
        {
            throw new CalciteSubquerySemanticException(ASTErrorUtils.getMsg(ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(), outerQueryExpr, "IN/EXISTS/SOME/ALL subqueries are not allowed in LHS"));
        }

        QBSubQuery subQuery = SubQueryUtils.buildSubQuery(qb.getId(), sqIdx, subqueryExprNode, subqueryExprNode, ctx);
    ...
    static class ParentQueryWhereClauseRewrite
    {
        ...
        boolean removeSubQuery(ASTNode node)
        {
            ...
            } else if (node.getType() == HiveParser.KW_NOT)
            {
                ...
                if (child == subQuery)
                {
                    ...
                    {
                        newSqOpType = new ASTNode(new CommonToken(HiveParser.TOK_SUBQUERY_OP_NOTEXISTS, "TOK_SUBQUERY_OP_NOTEXISTS"));
                    } else
                    {
                        newSqOpType = new ASTNode(new CommonToken(HiveParser.TOK_SUBQUERY_OP_NOTIN, "TOK_SUBQUERY_OP_NOTIN"));
                    }
        ...
        ASTNode remove() throws SemanticException
        {
             ...
             */
            throw new SemanticException(ASTErrorUtils.getMsg(ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(), subQuery, "Only SubQuery expressions that are top level conjuncts are allowed"));
        }
    }
    ...
    static QBSubQuery buildSubQuery(String outerQueryId, int sqIdx, ASTNode sqAST, ASTNode originalSQAST, Context ctx) throws SemanticException
    {
        ...
        return new QBSubQuery(outerQueryId, sqIdx, sq, outerQueryExpr, buildSQOperator(sqOp), originalSQAST, ctx);
    }
    ...
    {
        if (insertClause.getChildCount() > 2 && insertClause.getChild(2).getType() == HiveParser.TOK_WHERE)
        {
    ...
    {
        ASTNode tabAst = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL");
    ...
    {
        ASTNode selectItem = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_SELEXPR, "TOK_SELEXPR");
    ...
    {
        ASTNode insert = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_INSERT, "TOK_INSERT");
        ASTNode dest = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_DESTINATION, "TOK_DESTINATION");
        ASTNode dir = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_DIR, "TOK_DIR");
        ASTNode tfile = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_TMP_FILE, "TOK_TMP_FILE");
    ...
    {
        ASTNode select = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_SELECT, "TOK_SELECT");
        ASTNode selectExpr = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_SELEXPR, "TOK_SELEXPR");
        ASTNode countStar = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_FUNCTIONSTAR, "TOK_FUNCTIONSTAR");
    ...
    public static void checkForTopLevelSubqueries(ASTNode selExprList) throws SemanticException
    {
        ...
        assert (selExprList.getType() == HiveParser.TOK_SELECT || selExprList.getType() == HiveParser.TOK_SELECTDI);
        for (int i = 0; i < selExprList.getChildCount(); i++)
        {
            ...
            assert (selExpr.getType() == HiveParser.TOK_SELEXPR || selExpr.getType() == HiveParser.QUERY_HINT);
            ...
            if (selExpr.getChildCount() == 1 && selExpr.getChild(0).getType() == HiveParser.TOK_SUBQUERY_EXPR)
            {
