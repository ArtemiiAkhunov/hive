...
class CompactionQueryBuilder
{
     ...
     */
    CompactionQueryBuilder(CompactionType compactionType, Operation operation, boolean insertOnly, String resultTableName)
    {
    ...
    private void buildAddClauseForAlter(StringBuilder query)
    {
        ...
        List<AcidUtils.ParsedDelta> deltas = dir.getCurrentDirectories().stream().filter(delta -> delta.isDeleteDelta() == isDeleteDelta && delta.getMaxWriteId() <= highWatermark && delta.getMinWriteId() >= minWriteID).collect(Collectors.toList());
        if (deltas.isEmpty())
        {
    ...
    private void buildSelectClauseForInsert(StringBuilder query)
    {
        ...
        if (CompactionType.REBALANCE.equals(compactionType) || CompactionType.MAJOR.equals(compactionType) && (!insertOnly || sourcePartition != null) || CompactionType.MINOR.equals(compactionType) && insertOnly)
        {
            if (sourceTab == null)
            {
        ...
        switch (compactionType)
        {
            case REBALANCE:
            {
                query.append("0, t2.writeId, t2.rowId DIV CEIL(numRows / ").append(numberOfBuckets).append("), t2.rowId, t2.writeId, t2.data from (select ").append("count(ROW__ID.writeId) over() as numRows, ");
            ...
            case MAJOR:
            {
                if (insertOnly)
                {
                    ...
                    { //mmmajor and partitioned
                        appendColumns(query, cols, false);
                    } else
                    { // mmmajor and unpartitioned
                        query.append("*");
                    }
                } else
                {
    ...
    private void buildWhereClauseForInsert(StringBuilder query)
    {
        ...
        if (CompactionType.MINOR.equals(compactionType) && !insertOnly && validWriteIdList != null)
        {
            ...
            {
                query.append(" where `originalTransaction` not in (").append(StringUtils.join(ArrayUtils.toObject(invalidWriteIds), ",")).append(")");
            }
        }
    }
    ...
    {
        if (isBucketed && sourceTab != null)
        { // skip if sourceTab is null to avoid NPEs
            int numBuckets = 1;
            try
            {
    ...
    private void copySerdeFromSourceTable(StringBuilder query)
    {
        ...
        query.append(" ROW FORMAT SERDE '").append(HiveStringUtils.escapeHiveCommand(serdeInfo.getSerializationLib())).append("'");
        // WITH SERDEPROPERTIES
        if (!serdeParams.isEmpty())
        {
            DDLPlanUtils.appendSerdeParams(query, serdeParams);
        }
        query.append("STORED AS INPUTFORMAT '").append(HiveStringUtils.escapeHiveCommand(storageDescriptor.getInputFormat())).append("'").append(" OUTPUTFORMAT '").append(HiveStringUtils.escapeHiveCommand(storageDescriptor.getOutputFormat())).append("'");
    }
    ...
    private void addTblProperties(StringBuilder query, int bucketingVersion)
    {
        ...
        if (sourceTab != null)
        { // to avoid NPEs, skip this part if sourceTab is null
            if (insertOnly)
            {
    ...
    private void ensureTableToCompactIsNative()
    {
        ...
        {
            String message = "Table " + sourceTab.getTableName() + "has a storage handler (" + storageHandler + "). Failing compaction for this non-native table.";
            LOG.error(message);
