...
public class CommonMergeJoinOperator extends AbstractMapJoinOperator<CommonMergeJoinDesc> implements Serializable
{
    ...
    public void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        for (byte pos = 0; pos < order.length; pos++)
        {
            ...
            RowContainer<List<Object>> unmatchedRC = JoinUtil.getRowContainer(hconf, rowContainerStandardObjectInspectors[pos], pos, bucketSize, spillTableDesc, conf, !hasFilter(pos), reporter);
        ...
        for (byte pos = 0; pos < order.length; pos++)
        {
            if (pos != posBigTable)
            {
                if ((parentOperators != null) && !parentOperators.isEmpty() && (parentOperators.get(pos) instanceof TezDummyStoreOperator))
                {
        ...
        if (sources[0] instanceof ReduceRecordSource)
        {
            ...
            if (reduceRecordSource.getKeyTableDesc() != null && reduceRecordSource.getKeyTableDesc().getProperties() != null)
            {
                String nullSortOrder = reduceRecordSource.getKeyTableDesc().getProperties().getProperty(serdeConstants.SERIALIZATION_NULL_SORT_ORDER);
    ...
    public void process(Object row, int tag) throws HiveException
    {
        ...
        } else
        {
            if ((tag == posBigTable) && (candidateStorage[tag].rowCount() == joinEmitInterval))
            {
                ...
                if (canEmit)
                {
                    LOG.info("We are emitting rows since we hit the join emit interval of " + joinEmitInterval);
                    joinOneGroup(false);
        ...
        if (nextKeyGroup)
        {
            ...
            } while (listOfFetchNeeded != null && listOfFetchNeeded.size() > 0 && !listOfFetchNeeded.contains(this.posBigTable));
            return;
        }
    ...
    private int compareKeys(byte alias, List<Object> k1, List<Object> k2)
    {
        ...
        {
            return compareKey(comparators, 0, k1.get(0), k2.get(0), nullsafes != null ? nullsafes[0] : false);
        }
    }
    ...
    private int compareKeysMany(WritableComparator[] comparators, final List<Object> k1, final List<Object> k2)
    {
        ...
        {
            ret = compareKey(comparators, i, k1.get(i), k2.get(i), nullsafes != null ? nullsafes[i] : false);
            if (ret != 0)
            {
                return ret;
            }
    ...
    @SuppressWarnings("rawtypes")
    private int compareKey(final WritableComparator comparators[], final int pos, final Object key_1, final Object key_2, final boolean nullsafe)
    {
    ...
    private List<Object> mergeJoinComputeKeys(Object row, Byte alias) throws HiveException
    {
        ...
        } else
        {
            ...
            ObjectInspectorUtils.partialCopyToStandardObject(key, row, Utilities.ReduceField.KEY.position, 1, (StructObjectInspector) inputObjInspectors[alias], ObjectInspectorCopyOption.WRITABLE);
    ...
    public void initializeLocalWork(Configuration hconf) throws HiveException
    {
        ...
        {
            Map<Integer, DummyStoreOperator> dummyOps = ((TezContext) (MapredContext.get())).getDummyOpsMap();
            for (Entry<Integer, DummyStoreOperator> connectOp : dummyOps.entrySet())
            {
                if (connectOp.getValue().getChildOperators() == null || connectOp.getValue().getChildOperators().isEmpty())
                {
