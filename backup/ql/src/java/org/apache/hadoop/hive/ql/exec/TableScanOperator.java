 ...
 **/
public class TableScanOperator extends Operator<TableScanDesc> implements Serializable, VectorizationContextRegion
{
    ...
    public static class ProbeDecodeContext
    {
        ...
        {
            return "cacheKey:" + mjSmallTableCacheKey + ", bigKeyColName:" + mjBigTableKeyColName + ", smallTablePos:" + mjSmallTablePos + ", keyRatio:" + keyRatio;
        }
    }
    ...
    private void gatherStats(Object row)
    {
        ...
        if ((currentStat == null) || inputFileChanged)
        {
            ...
            {
                if (soi.getAllStructFieldRefs().get(i).getFieldName().equals(VirtualColumn.RAWDATASIZE.getName().toLowerCase()))
                {
                    uSizeColumn = i;
                    break;
                }
        ...
        if (rdSizeColumn != -1)
        {
            ...
            ObjectInspectorUtils.partialCopyToStandardObject(rdSize, row, rdSizeColumn, 1, (StructObjectInspector) inputObjInspectors[0], ObjectInspectorCopyOption.WRITABLE);
