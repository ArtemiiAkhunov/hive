...
public abstract class TaskHandler
{
    ...
    protected ValidReaderWriteIdList getValidCleanerWriteIdList(CompactionInfo info, ValidTxnList validTxnList) throws NoSuchTxnException, MetaException
    {
        ...
        return new ValidCleanerWriteIdList(TxnCommonUtils.createValidReaderWriteIdList(rsp.getTblValidWriteIds().get(0)));
    }
    ...
    protected boolean cleanAndVerifyObsoleteDirectories(CompactionInfo info, String location, ValidReaderWriteIdList validWriteIdList, Table table) throws MetaException, IOException
    {
        ...
        AcidDirectory dir = AcidUtils.getAcidState(fs, path, conf, validWriteIdList, Ref.from(false), false, dirSnapshots);
        ...
        List<Path> deleted = fsRemover.clean(new CleanupRequest.CleanupRequestBuilder().setLocation(location).setDbName(info.dbname).setFullPartitionName(info.getFullPartitionName()).setRunAs(info.runAs).setObsoleteDirs(obsoleteDirs).setPurge(true).build());

        if (!deleted.isEmpty())
        {
            AcidMetricService.updateMetricsFromCleaner(info.dbname, info.tableName, info.partName, dir.getObsolete(), conf, txnHandler);
        }
        ...
        dir = AcidUtils.getAcidState(fs, path, conf, new ValidCleanerWriteIdList(info.getFullTableName(), info.highestWriteId), Ref.from(false), false, dirSnapshots);
