...
public class VectorHashKeyWrapperBatch extends VectorColumnSetInfo
{
    ...
    private void assignStringNullsNoRepeatingSelection(int keyIndex, int index, int size, BytesColumnVector columnVector, int[] selected)
    {
        for (int i = 0; i < size; ++i)
        {
            ...
            {
                vectorHashKeyWrappers[i].assignString(index, columnVector.vector[row], columnVector.start[row], columnVector.length[row]);
            }
        }
    }
    ...
    private void assignStringNullsRepeating(int keyIndex, int index, int size, BytesColumnVector columnVector)
    {
        ...
        } else
        {
            for (int i = 0; i < size; ++i)
            {
                vectorHashKeyWrappers[i].assignString(index, columnVector.vector[0], columnVector.start[0], columnVector.length[0]);
            }
        }
    }
    ...
    public static VectorHashKeyWrapperBatch compileKeyWrapperBatch(VectorExpression[] keyExpressions, TypeInfo[] typeInfos) throws HiveException
    {
        ...
        compiledKeyWrapperBatch.vectorHashKeyWrappers = new VectorHashKeyWrapperBase[VectorizedRowBatch.DEFAULT_SIZE];
    ...
    {
        return VectorHashKeyWrapperFactory.allocate(hashCtx, longIndices.length, doubleIndices.length, stringIndices.length, decimalIndices.length, timestampIndices.length, intervalDayTimeIndices.length, keyCount);
    }
    ...
    public void setLongValue(VectorHashKeyWrapperBase kw, int keyIndex, Long value) throws HiveException
    {
    ...
    {
        VectorHashKeyWrapperGeneralComparator comparator = new VectorHashKeyWrapperGeneralComparator(columnVectorTypes.length);
        for (int i = 0; i < columnVectorTypes.length; ++i)
        {
            ...
            comparator.addColumnComparator(i, columnTypeSpecificIndex, columnVectorType, columnSortOrder.charAt(i), nullOrder.charAt(i));
        }
        if (comparator.getComparators().length == 1)
        { // don't use the composite comparator for n=1
