...
public class OrcInputFormat implements InputFormat<NullWritable, OrcStruct>, InputFormatChecker, VectorizedInputFormatInterface, LlapWrappableInputFormatInterface, SelfDescribingInputFormatInterface, AcidInputFormat<NullWritable, OrcStruct>, CombineHiveInputFormat.AvoidSplitCombination, BatchToRowInputFormat
{
    ...
    private static class OrcRecordReader implements org.apache.hadoop.mapred.RecordReader<NullWritable, OrcStruct>, StatsProvidingRecordReader
    {
        ...
        OrcRecordReader(Reader file, Configuration conf, InputSplit inputSplit) throws IOException
        {
            this.file = file;
    ...
    public static RecordReader createReaderFromFile(Reader file, Configuration conf, long offset, long length, boolean isOriginal) throws IOException
    {
    ...
    {
        return !CollectionUtils.isEqualCollection(file.getSchema().getFieldNames(), OrcRecordUpdater.ALL_ACID_ROW_NAMES);
    }
    ...
    {
        return !CollectionUtils.isEqualCollection(footer.getTypesList().get(0).getFieldNamesList(), OrcRecordUpdater.ALL_ACID_ROW_NAMES);
    }
    ...
    public static boolean[] genIncludedColumns(TypeDescription readerSchema, List<Integer> included, Integer recursiveStruct)
    {
    ...
    public static TypeDescription[] genIncludedTypes(TypeDescription fileSchema, List<Integer> included, Integer recursiveStruct)
    {
    ...
    // Mostly dup of genIncludedColumns
    public static String[] genIncludedColNames(TypeDescription fileSchema, List<Integer> included, Integer recursiveStruct)
    {
     ...
     */
    static boolean[] genIncludedColumns(TypeDescription readerSchema, Configuration conf)
    {
    ...
    private static String[] getSargColumnNames(String[] originalColumnNames, List<OrcProto.Type> types, boolean[] includedColumns, boolean isOriginal)
    {
    ...
    {
        return conf.getBoolean(ColumnProjectionUtils.READ_ALL_COLUMNS, true) ? null : conf.get(ColumnProjectionUtils.READ_COLUMN_IDS_CONF_STR);
    }
    ...
    public boolean validateInput(FileSystem fs, HiveConf conf, List<FileStatus> files) throws IOException
    {
        ...
        for (FileStatus file : files)
        {
            ...
            try (Reader notUsed = OrcFile.createReader(file.getPath(), OrcFile.readerOptions(conf).filesystem(fs).maxLength(file.getLen())))
            {
    ...
    static class Context
    {
        ...
        @VisibleForTesting
        Context(Configuration conf, final int minSplits, ExternalFooterCachesByConf efc) throws IOException
        {
            this.conf = conf;
            ...
            long cacheMemSize = HiveConf.getSizeVar(conf, ConfVars.HIVE_ORC_CACHE_STRIPE_DETAILS_MEMORY_SIZE);
            int numThreads = HiveConf.getIntVar(conf, ConfVars.HIVE_COMPUTE_SPLITS_NUM_THREADS);
            boolean useSoftReference = HiveConf.getBoolVar(conf, ConfVars.HIVE_ORC_CACHE_USE_SOFT_REFERENCES);
            ...
            this.acidOperationalProperties = isTxnTable ? AcidOperationalProperties.parseString(txnProperties) : null;
            ...
            LOG.info("Context:: " + "isAcid: {} " + "isVectorMode: {} " + "sarg: {} " + "minSplitSize: {} " + "maxSplitSize: {} " + "splitStrategy: {} " + "footerInSplits: {} " + "numBuckets: {} " + "numThreads: {} " + "cacheMemSize: {} " + "cacheStripeDetails: {} " + "useSoftReference: {} " + "writeIdList: {} " + "isTransactionalTable: {} " + "txnProperties: {} ", isAcid, isVectorMode, sarg, minSize, maxSize, splitStrategyKind, footerInSplits, numBuckets, numThreads, cacheMemSize, cacheStripeDetails, useSoftReference, writeIdList, isTxnTable, txnProperties);
        }
    ...
    static final class SplitInfo extends ACIDSplitStrategy
    {
        ...
        SplitInfo(Context context, FileSystem fs, HdfsFileStatusWithId fileWithId, OrcTail orcTail, List<OrcProto.Type> readerTypes, boolean isOriginal, List<DeltaMetaData> deltas, boolean hasBase, Path dir, boolean[] covered, ByteBuffer ppdResult) throws IOException
        {
        ...
        @VisibleForTesting
        public SplitInfo(Context context, FileSystem fs, FileStatus fileStatus, OrcTail orcTail, List<OrcProto.Type> readerTypes, boolean isOriginal, ArrayList<DeltaMetaData> deltas, boolean hasBase, Path dir, boolean[] covered) throws IOException
        {
            this(context, fs, new HdfsUtils.HdfsFileStatusWithoutId(fileStatus), orcTail, readerTypes, isOriginal, deltas, hasBase, dir, covered, null);
        }
    }
    ...
    static final class ETLSplitStrategy implements SplitStrategy<SplitInfo>, Callable<Void>
    {
        ...
        public ETLSplitStrategy(Context context, FileSystem fs, Path dir, List<HdfsFileStatusWithId> children, List<OrcProto.Type> readerTypes, boolean isOriginal, List<DeltaMetaData> deltas, boolean[] covered, UserGroupInformation ugi, boolean allowSyntheticFileIds, boolean isDefaultFs)
        {
        ...
        public CombineResult combineWith(FileSystem fs, Path dir, List<HdfsFileStatusWithId> otherFiles, boolean isOriginal)
        {
            if ((files.size() + otherFiles.size()) > ETL_COMBINE_FILE_LIMIT || this.isOriginal != isOriginal)
            {//todo: what is this checking????
                return (files.size() > otherFiles.size()) ? CombineResult.NO_AND_SWAP : CombineResult.NO_AND_CONTINUE;
            }
        ...
        private void runGetSplitsSync(List<Future<List<OrcSplit>>> splitFutures, List<OrcSplit> splits, UserGroupInformation ugi) throws IOException
        {
            ...
            {
                SplitGenerator sg = new SplitGenerator(splitInfo, tpUgi, allowSyntheticFileIds, isDefaultFs);
                if (!sg.isBlocking())
                {
                    if (localListS == null)
                    {
    ...
    static final class BISplitStrategy extends ACIDSplitStrategy
    {
         ...
         */
        public BISplitStrategy(Context context, FileSystem fs, Path dir, List<HdfsFileStatusWithId> fileStatuses, boolean isOriginal, List<DeltaMetaData> deltas, boolean[] covered, boolean allowSyntheticFileIds, boolean isDefaultFs)
        {
            super(dir, context.numBuckets, deltas, covered, context.acidOperationalProperties);
        ...
        public List<OrcSplit> getSplits() throws IOException
        {
            ...
            for (HdfsFileStatusWithId file : fileStatuses)
            {
                ...
                {
                    offsetAndBucket = VectorizedOrcAcidRowBatchReader.computeOffsetAndBucket(file.getFileStatus(), dir, isOriginal, !deltas.isEmpty(), conf);
                }
                ...
                if (logicalLen != 0)
                {
                    ...
                    if (BlobStorageUtils.isBlobStorageFileSystem(conf, fs))
                    {
                        ...
                        {
                            OrcSplit orcSplit = new OrcSplit(fileStatus.getPath(), fileKey, start, Math.min(splitSize, logicalLen - start), null, null, isOriginal, true, deltas, -1, logicalLen, dir, offsetAndBucket);
                            splits.add(orcSplit);
                        }
                    } else
                    {
                        ...
                        for (Map.Entry<Long, BlockLocation> entry : blockOffsets.entrySet())
                        {
                            ...
                            OrcSplit orcSplit = new OrcSplit(fileStatus.getPath(), fileKey, blockOffset, splitLength, entry.getValue().getHosts(), null, isOriginal, true, deltas, -1, logicalLen, dir, offsetAndBucket);
                            splits.add(orcSplit);
                        }
                    }
                }
    ...
    static class ACIDSplitStrategy implements SplitStrategy<OrcSplit>
    {
         ...
         * @param dir root of partition dir
         */
        ACIDSplitStrategy(Path dir, int numBuckets, List<DeltaMetaData> deltas, boolean[] covered, AcidOperationalProperties acidOperationalProperties)
        {
            this.dir = dir;
            this.deltas = deltas;
    ...
    static final class FileGenerator implements Callable<Directory>
    {
        ...
        @VisibleForTesting
        FileGenerator(Context context, Supplier<FileSystem> fs, Path dir, boolean useFileIds, UserGroupInformation ugi)
        {
            this(context, fs, dir, Ref.from(useFileIds), ugi);
        }

        FileGenerator(Context context, Supplier<FileSystem> fs, Path dir, Ref<Boolean> useFileIds, UserGroupInformation ugi)
        {
            this.context = context;
            this.fs = fs;
            this.dir = dir;
        ...
        private AcidDirectory getAcidState() throws IOException
        {
            ...
            {
                return AcidUtils.getAcidStateFromCache(fs, dir, context.conf, context.writeIdList, useFileIds, true);
            } else
            {
                return AcidUtils.getAcidState(fs.get(), dir, context.conf, context.writeIdList, useFileIds, true);
            }
        }
        ...
        {
            if (context.acidOperationalProperties == null || context.acidOperationalProperties.isInsertOnly())
            {
        ...
        {
            boolean isRecursive = context.conf.getBoolean(FileInputFormat.INPUT_DIR_RECURSIVE, context.conf.getBoolean("mapred.input.dir.recursive", false));

            List<FileInfo> originals = HdfsUtils.listFileStatusWithId(fs.get(), dir, Ref.from(fs instanceof DistributedFileSystem), isRecursive, null).stream().map(f -> new FileInfo(f, ORIGINAL_BASE)).collect(Collectors.toList());
    ...
    static final class SplitGenerator implements Callable<List<OrcSplit>>
    {
        ...
        public SplitGenerator(SplitInfo splitInfo, UserGroupInformation ugi, boolean allowSyntheticFileIds, boolean isDefaultFs) throws IOException
        {
            this.ugi = ugi;
        ...
        OrcSplit createSplit(long offset, long length, OrcTail orcTail) throws IOException
        {
            ...
            final long scaledProjSize = projColsUncompressedSize > 0 ? (long) (splitRatio * projColsUncompressedSize) : fileLen;
            Object fileKey = fsFileId;
            if (fileKey == null && allowSyntheticFileIds)
            {
                fileKey = new SyntheticFileId(file);
            }
            return new OrcSplit(file.getPath(), fileKey, offset, length, hosts, orcTail, isOriginal, hasBase, deltas, scaledProjSize, fileLen, rootDir, offsetAndBucket);
        }
        ...
        private List<OrcSplit> callInternal() throws IOException
        {
            ...
            if (isOriginal && isAcid && vectorMode)
            {
                offsetAndBucket = VectorizedOrcAcidRowBatchReader.computeOffsetAndBucket(file, rootDir, isOriginal, !deltas.isEmpty(), context.conf);
            }
            ...
            if (ppdResult != null)
            {
                ...
                CodedInputStream cis = CodedInputStream.newInstance(ppdResult.array(), ppdResult.arrayOffset(), ppdResult.remaining());
            ...
            } else
            {
                ...
                {
                    String[] colNames = extractNeededColNames((readerTypes == null ? fileTypes : readerTypes), context.conf, readerIncluded, isOriginal);
                    if (colNames == null)
                    {
                        LOG.warn("Skipping split elimination for {} as column names is null", file.getPath());
                    } else
                    {
                        includeStripe = pickStripes(context.sarg, writerVersion, stripeStats, stripes.size(), file.getPath(), evolution);
                    }
                }
        ...
        private void populateAndCacheStripeDetails() throws IOException
        {
            ...
            if (orcTail == null)
            {
                try (Reader orcReader = OrcFile.createReader(file.getPath(), OrcFile.readerOptions(context.conf).filesystem(fs).maxLength(context.isAcid ? AcidUtils.getLogicalLength(fs, file) : file.getLen())))
                {
            ...
            } else
            {
                ...
                try (org.apache.orc.Reader dummyReader = new org.apache.orc.impl.ReaderImpl(null, org.apache.orc.OrcFile.readerOptions(org.apache.orc.OrcFile.readerOptions(context.conf).getConfiguration()).useUTCTimestamp(true).convertToProlepticGregorian(true).orcTail(orcTail)))
                {
        ...
        private long computeProjectionSize(List<OrcProto.Type> fileTypes, List<OrcProto.ColumnStatistics> stats, boolean[] fileIncluded) throws FileFormatException
        {
            ...
            {
                internalColIds = IntStream.range(1, fileTypes.size()).boxed().collect(Collectors.toList());
            } else
            {
                internalColIds = IntStream.range(1, fileTypes.size()).filter(i -> fileIncluded[i]).filter(i -> i != OrcRecordUpdater.ROW + 1 || isOriginal).boxed().collect(Collectors.toList());
            }
    ...
    static List<OrcSplit> generateSplitsInfo(Configuration conf, Context context) throws IOException
    {
        ...
        try
        {
            ...
            while (resultsLeft > 0)
            {
                ...
                List<SplitStrategy<?>> splitStrategies = determineSplitStrategies(combinedCtx, context, directory.getFs(), directory.getPath(), directory.getFiles(), directory.getDeleteDeltas(), readerTypes, ugi, allowSyntheticFileIds);
    ...
    private static SplitStrategy<?> combineOrCreateETLStrategy(CombinedCtx combinedCtx, Context context, FileSystem fs, Path dir, List<HdfsFileStatusWithId> files, List<DeltaMetaData> deltas, boolean[] covered, List<OrcProto.Type> readerTypes, boolean isOriginal, UserGroupInformation ugi, boolean allowSyntheticFileIds, boolean isDefaultFs)
    {
        if (!deltas.isEmpty() || combinedCtx == null)
        {
            //why is this checking for deltas.isEmpty() - HIVE-18110
            return new ETLSplitStrategy(context, fs, dir, files, readerTypes, isOriginal, deltas, covered, ugi, allowSyntheticFileIds, isDefaultFs);
        } else if (combinedCtx.combined == null)
        {
            combinedCtx.combined = new ETLSplitStrategy(context, fs, dir, files, readerTypes, isOriginal, deltas, covered, ugi, allowSyntheticFileIds, isDefaultFs);
    ...
    {
        return (org.apache.hadoop.mapred.RecordReader) new VectorizedOrcInputFormat().getRecordReader(split, conf, reporter);
    }
    ...
    public org.apache.hadoop.mapred.RecordReader<NullWritable, OrcStruct> getRecordReader(InputSplit inputSplit, JobConf conf, Reporter reporter) throws IOException
    {
        ...
        if (!isAcidRead)
        {
            ...
            } else
            {
                ...
                return new OrcRecordReader(OrcFile.createReader(((FileSplit) inputSplit).getPath(), readerOptions), conf, inputSplit);
            }
        }
        ...
        if (vectorMode)
        {
            return (org.apache.hadoop.mapred.RecordReader) new VectorizedOrcAcidRowBatchReader(split, conf, reporter);
        }
    ...
    private static class OrcRowReader implements RowReader<OrcStruct>
    {
        ...
        @Override
        public boolean next(OrcRawRecordMerger.ReaderKey recordIdentifier, OrcStruct orcStruct) throws IOException
        {
            boolean result;
    ...
    private static boolean[] pickStripes(SearchArgument sarg, OrcFile.WriterVersion writerVersion, List<StripeStatistics> stripeStats, int stripeCount, Path filePath, final SchemaEvolution evolution)
    {
        if (stripeStats == null || writerVersion == OrcFile.WriterVersion.ORIGINAL)
        {
            return null; // only do split pruning if HIVE-8732 has been fixed in the writer
        ...
        // eliminate stripes that doesn't satisfy the predicate condition
        List<PredicateLeaf> sargLeaves = sarg.getLeaves();
        int[] filterColumns = RecordReaderImpl.mapSargColumnsToOrcInternalColIdx(sargLeaves, evolution);
        return pickStripesInternal(sarg, filterColumns, stripeStats, stripeCount, filePath, evolution);
    }

    private static boolean[] pickStripesInternal(SearchArgument sarg, int[] filterColumns, List<StripeStatistics> stripeStats, int stripeCount, Path filePath, final SchemaEvolution evolution)
    {
    ...
    private static SplitStrategy<?> determineSplitStrategy(CombinedCtx combinedCtx, Context context, FileSystem fs, Path dir, List<HdfsFileStatusWithId> baseFiles, boolean isOriginal, List<ParsedDelta> parsedDeltas, List<OrcProto.Type> readerTypes, UserGroupInformation ugi, boolean allowSyntheticFileIds, boolean isDefaultFs) throws IOException
    {
        ...
        if (!baseFiles.isEmpty())
        {
            ...
            switch (context.splitStrategyKind)
            {
                case BI:
                    ...
                    return new BISplitStrategy(context, fs, dir, baseFiles, isOriginal, deltas, covered, allowSyntheticFileIds, isDefaultFs);
                case ETL:
                    // ETL strategy requested through config
                    return combineOrCreateETLStrategy(combinedCtx, context, fs, dir, baseFiles, deltas, covered, readerTypes, isOriginal, ugi, allowSyntheticFileIds, isDefaultFs);
                default:
                    // HYBRID strategy
     ...
     */
    @Override
    public RawReader<OrcStruct> getRawReader(Configuration conf, boolean collapseEvents, int bucket, ValidWriteIdList validWriteIdList, Path baseDirectory, Path[] deltaDirectory, Map<String, Integer> deltasToAttemptId) throws IOException
    {
        boolean isOriginal = false;
        OrcRawRecordMerger.Options mergerOptions = new OrcRawRecordMerger.Options().isCompacting(true).isMajorCompaction(collapseEvents);
        if (baseDirectory != null)
        {//this is NULL for minor compaction
        ...
        return new OrcRawRecordMerger(conf, collapseEvents, null, isOriginal, bucket, validWriteIdList, new Reader.Options(conf), deltaDirectory, mergerOptions, deltasToAttemptId);
    }
    ...
    public interface FooterCache
    {
        ...
        void getAndValidate(List<HdfsFileStatusWithId> files, boolean isOriginal, OrcTail[] result, ByteBuffer[] ppdResult) throws IOException, HiveException;
    ...
    public static TypeDescription getDesiredRowTypeDescr(Configuration conf, boolean isAcidRead, int dataColumns)
    {
        ...
        if (haveSchemaEvolutionProperties)
        {
            if (LOG.isDebugEnabled())
            {
                LOG.debug("Using schema evolution configuration variables schema.evolution.columns " + schemaEvolutionColumnNames.toString() + " / schema.evolution.columns.types " + schemaEvolutionTypeDescrs.toString() + " (isAcidRead " + isAcidRead + ")");
            }
        } else
        {
            ...
            if (LOG.isDebugEnabled())
            {
                LOG.debug("Using column configuration variables columns " + schemaEvolutionColumnNames.toString() + " / columns.types " + schemaEvolutionTypeDescrs.toString() + " (isAcidRead " + isAcidRead + ")");
            }
        }
    ...
    public static boolean[] firstLevelFileIncludes(SchemaEvolution evolution)
    {
        ...
        {
            result[IntStream.range(0, topLevelFields.size()).filter(i -> typeDescriptionId == topLevelFields.get(i).getId()).findFirst().getAsInt() + 1] = true;
        }
     ...
     * @param parentIdMap
     */
    private static void idToFieldSchemaMap(TypeDescription typeDescription, Map<Integer, TypeDescription> idMap, Map<Integer, Integer> parentIdMap)
    {
    ...
    @Override
    public BatchToRowReader<?, ?> getWrapper(org.apache.hadoop.mapred.RecordReader<NullWritable, VectorizedRowBatch> vrr, VectorizedRowBatchCtx vrbCtx, List<Integer> includedCols)
    {
