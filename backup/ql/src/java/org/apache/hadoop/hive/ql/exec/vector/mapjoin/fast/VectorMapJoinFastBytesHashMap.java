...
public abstract class VectorMapJoinFastBytesHashMap extends VectorMapJoinFastBytesHashTable implements VectorMapJoinBytesHashMap
{
    ...
    public static class NonMatchedBytesHashMapIterator extends VectorMapJoinFastNonMatchedIterator
    {
        ...
        public boolean readNonMatchedBytesKey() throws HiveException
        {
            ...
            hashMap.hashMapStore.getKey(hashMap.slots[nonMatchedLogicalSlotNum], nonMatchedKeyByteSegmentRef, nonMatchedReadPos);
            return true;
        }
    ...
    public void add(byte[] keyBytes, int keyStart, int keyLength, BytesWritable currentValue, long hashCode)
    {
        ...
        boolean isNewKey;
        long refWord;
        final long partialHashCode = VectorMapJoinFastBytesHashKeyRef.extractPartialHashCode(hashCode);
        while (true)
        {
            refWord = slots[slot];
            if (refWord == 0)
            ...
            if (VectorMapJoinFastBytesHashKeyRef.getPartialHashCodeFromRefWord(refWord) == partialHashCode && VectorMapJoinFastBytesHashKeyRef.equalKey(refWord, keyBytes, keyStart, keyLength, writeBuffers, unsafeReadPos))
            {
                isNewKey = false;
                break;
            }
        ...
        if (isNewKey)
        {
            slots[slot] = hashMapStore.addFirst(partialHashCode, keyBytes, keyStart, keyLength, valueBytes, 0, valueLength);
            keysAssigned++;
        } else
        {
            final long newRefWord = hashMapStore.addMore(refWord, valueBytes, 0, valueLength, unsafeReadPos);
    ...
    public JoinUtil.JoinResult lookup(byte[] keyBytes, int keyStart, int keyLength, VectorMapJoinHashMapResult hashMapResult)
    {
        ...
        doHashMapMatch(keyBytes, keyStart, keyLength, hashCode, fastHashMapResult);
    ...
    public JoinUtil.JoinResult lookup(byte[] keyBytes, int keyStart, int keyLength, VectorMapJoinHashMapResult hashMapResult, MatchTracker matchTracker)
    {
        ...
        final int slot = doHashMapMatch(keyBytes, keyStart, keyLength, hashCode, fastHashMapResult);
    ...
    protected final int doHashMapMatch(byte[] keyBytes, int keyStart, int keyLength, long hashCode, VectorMapJoinFastBytesHashMapStore.HashMapResult fastHashMapResult)
    {
        ...
        while (true)
        {
            ...
            if (refWord == 0)
            {
                ...
                return -1;
            } else if (VectorMapJoinFastBytesHashKeyRef.getPartialHashCodeFromRefWord(refWord) == partialHashCode)
            {
    ...
    public void addFullOuterNullKeyValue(BytesWritable currentValue)
    {
        ...
        {
            fullOuterNullKeyRefWord = hashMapStore.addFirst(
                    /* partialHashCode */ 0, EMPTY_BYTES, 0, 0, valueBytes, 0, valueLength);
        } else
        {

            // Add another value.
            fullOuterNullKeyRefWord = hashMapStore.addMore(fullOuterNullKeyRefWord, valueBytes, 0, valueLength, unsafeReadPos);
        }
    }
