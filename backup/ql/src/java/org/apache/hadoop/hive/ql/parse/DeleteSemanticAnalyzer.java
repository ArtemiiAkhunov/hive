...
public class DeleteSemanticAnalyzer extends RewriteSemanticAnalyzer<DeleteStatement>
{
    ...
    protected ASTNode getTargetTableNode(ASTNode tree)
    {
        ...
        assert tabName.getToken().getType() == HiveParser.TOK_TABNAME : "Expected tablename as first child of " + Context.Operation.DELETE + " but found " + tabName.getName();
        return tabName;
    }
    ...
    protected void analyze(ASTNode tree, Table table, ASTNode tableName) throws SemanticException
    {
        ...
        boolean shouldTruncate = HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_OPTIMIZE_REPLACE_DELETE_WITH_TRUNCATE) && where == null;
        if (shouldTruncate)
        {
    ...
    private boolean tryMetadataUpdate(Table table, ASTNode tabNameNode, ASTNode whereNode) throws SemanticException
    {
        ...
        String whereClause = ctx.getTokenRewriteStream().toString(whereNode.getChild(0).getTokenStartIndex(), whereNode.getChild(0).getTokenStopIndex());
        StringBuilder sb = new StringBuilder("select * from ").append(getFullTableNameForSQL(tabNameNode)).append(" where ").append(whereClause);
    ...
    private DDLWork createDDLWorkOfMetadataUpdate(TableName tableName, SearchArgument sarg) throws SemanticException
    {
        ...
        AlterTableExecuteSpec<AlterTableExecuteSpec.DeleteMetadataSpec> executeSpec = new AlterTableExecuteSpec<>(AlterTableExecuteSpec.ExecuteOperationType.DELETE_METADATA, deleteMetadataSpec);
