...
public class Compiler
{
    ...
    {
        if (DriverUtils.checkConcurrency(driverContext) && startImplicitTxn(driverContext.getTxnManager()) && !driverContext.getTxnManager().isTxnOpen() && !MetaStoreServerUtils.isCompactionTxn(txnType))
        {
            String userFromUGI = DriverUtils.getUserFromUGI(driverContext);
            if (HiveOperation.REPLDUMP.equals(driverContext.getQueryState().getHiveOperation()) || HiveOperation.REPLLOAD.equals(driverContext.getQueryState().getHiveOperation()))
            {
    ...
    private QueryPlan createPlan(BaseSemanticAnalyzer sem)
    {
        ...
        setSchema(sem);
        QueryPlan plan = new QueryPlan(driverContext.getQueryString(), sem, driverContext.getQueryDisplay().getQueryStartTime(), driverContext.getQueryId(), driverContext.getQueryState().getHiveOperation(), driverContext.getSchema());
    ...
    private void setSchema(BaseSemanticAnalyzer sem)
    {
        ...
        } else if (sem.getFetchTask() != null)
        {
            ...
            } else
            {
                ...
                try
                {
                    lst = HiveMetaStoreUtils.getFieldsFromDeserializer(tableName, td.getDeserializer(driverContext.getConf()), driverContext.getConf());
                } catch (Exception e)
                {
    ...
    {
        if (driverContext.getConf().getBoolVar(ConfVars.HIVE_LOG_EXPLAIN_OUTPUT) || driverContext.getConf().getBoolVar(ConfVars.HIVE_SERVER2_WEBUI_EXPLAIN_OUTPUT))
        {
            String explainOutput = ExplainTask.getExplainOutput(sem, plan, tree, driverContext.getQueryState(), context, driverContext.getConf());
            if (explainOutput != null)
            {
                ...
                if (driverContext.getConf().isWebUiQueryInfoCacheEnabled() && driverContext.getConf().getBoolVar(ConfVars.HIVE_SERVER2_WEBUI_EXPLAIN_OUTPUT))
                {
    ...
    private void cleanUp(Throwable compileException, boolean parsed, boolean deferClose)
    {
        ...
        if (driverState.isAborted())
        {
            ...
            LOG.info("Compiling command(queryId={}) has been interrupted after {} seconds", driverContext.getQueryId(), duration);
        } else
        {
            driverState.compilationFinishedWithLocking(compileException != null);
            LOG.info("Completed compiling command(queryId={}); Time taken: {} seconds", driverContext.getQueryId(), duration);
        }
    }
}
