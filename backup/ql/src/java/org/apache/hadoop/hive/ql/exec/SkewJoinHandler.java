...
public class SkewJoinHandler
{
    ...
    public void initiliaze(Configuration hconf)
    {
        ...
        for (int i = 0; i < numAliases; i++)
        {
            ...
            try
            {
                AbstractSerDe serializer = (AbstractSerDe) ReflectionUtils.newInstance(tblDesc.get(alias).getSerDeClass(), null);
    ...
    private void commit() throws IOException
    {
        for (int bigKeyTbl = 0; bigKeyTbl < numAliases; bigKeyTbl++)
        {
            ...
            Path specPath = conf.getBigKeysDirMap().get(Byte.valueOf((byte) bigKeyTbl));
