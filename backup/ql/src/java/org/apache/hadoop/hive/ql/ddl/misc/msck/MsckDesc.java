...
public class MsckDesc implements DDLDesc, Serializable
{
    ...
    public MsckDesc(String tableName, byte[] filterExp, Path resFile, boolean repairPartitions, boolean addPartitions, boolean dropPartitions)
    {
    ...
    @Explain(displayName = "repair partition", displayOnlyOnTrue = true, explainLevels = {Level.USER, Level.DEFAULT, Level.EXTENDED})
    public boolean isRepairPartitions()
    {
        return repairPartitions;
    }

    @Explain(displayName = "add partition", displayOnlyOnTrue = true, explainLevels = {Level.USER, Level.DEFAULT, Level.EXTENDED})
    public boolean isAddPartitions()
    {
        return addPartitions;
    }

    @Explain(displayName = "drop partition", displayOnlyOnTrue = true, explainLevels = {Level.USER, Level.DEFAULT, Level.EXTENDED})
