 ...
 */
public abstract class HiveAggregateIncrementalRewritingRuleBase<T extends HiveAggregateIncrementalRewritingRuleBase.IncrementalComputePlan> extends RelOptRule
{
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        for (int i = 0, leftPos = groupCount, rightPos = totalCount + 1 + groupCount; leftPos < totalCount; i++, leftPos++, rightPos++)
        {
            ...
            RexNode caseExpression = rexBuilder.makeCall(SqlStdOperatorTable.CASE, leftNull, rightRef, rightNull, leftRef, elseReturn);
            RexNode cast = rexBuilder.makeCast(call.rel(0).getRowType().getFieldList().get(projExprs.size()).getType(), caseExpression);
            projExprs.add(cast);
        }
    ...
    protected RexNode createAggregateNode(SqlAggFunction aggCall, RexNode leftRef, RexNode rightRef, RexBuilder rexBuilder)
    {
        switch (aggCall.getKind())
        {
            ...
            case COUNT:
                ...
                return rexBuilder.makeCall(SqlStdOperatorTable.PLUS, ImmutableList.of(rightRef, leftRef));
            default:
                throw new AssertionError("Found an aggregation that could not be" + " recognized: " + aggCall);
        }
    }
