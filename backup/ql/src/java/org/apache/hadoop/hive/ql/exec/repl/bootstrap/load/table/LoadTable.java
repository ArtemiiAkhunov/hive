...
public class LoadTable
{
    ...
    public LoadTable(TableEvent event, Context context, ReplLogger replLogger, TableContext tableContext, TaskTracker limiter, ReplicationMetricCollector metricCollector)
    {
        this.event = event;
    ...
    */
    ...
    private ReplLoadOpType getLoadTableType(Table table, boolean isBootstrapDuringInc, boolean isSecondFailover) throws InvalidOperationException, HiveException
    {
    ...
    private void newTableTasks(ImportTableDesc tblDesc, Task<?> tblRootTask, TableLocationTuple tuple, boolean setLoc) throws Exception
    {
    ...
    private Task<?> dropTableTask(Table table)
    {
        ...
        return TaskFactory.get(new DDLWork(new HashSet<>(), new HashSet<>(), dropTblDesc, true, (new Path(context.dumpDirectory)).getParent().toString(), this.metricCollector), context.hiveConf);
    }
}
