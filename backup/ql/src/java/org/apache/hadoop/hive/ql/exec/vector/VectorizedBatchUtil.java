...
public class VectorizedBatchUtil
{
            ...
            case PRIMITIVE:
            {
                ...
                switch (primitiveTypeInfo.getPrimitiveCategory())
                {
                    ...
                    case DECIMAL:
                        ...
                        {
                            return new Decimal64ColumnVector(VectorizedRowBatch.DEFAULT_SIZE, tInfo.precision(), tInfo.scale());
                        } else
                        {
    ...
    public static ColumnVector makeLikeColumnVector(ColumnVector source) throws HiveException
    {
        if (source instanceof Decimal64ColumnVector)
        {
            ...
            return new Decimal64ColumnVector(dec64ColVector.vector.length, dec64ColVector.precision, dec64ColVector.scale);
    ...
    {
        List<Integer> columnsToReset = IntStream.concat(IntStream.range(0, rowBatch.getDataColumnCount()), IntStream.range(rowBatch.getDataColumnCount() + rowBatch.getPartitionColumnCount(), rowBatch.cols.length)).boxed().collect(toList());
