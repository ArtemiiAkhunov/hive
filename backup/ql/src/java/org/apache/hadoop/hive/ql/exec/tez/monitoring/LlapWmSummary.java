...
public class LlapWmSummary implements PrintSummary
{
    ...
    private static final String LLAP_SUMMARY_HEADER = String.format(LLAP_SUMMARY_HEADER_FORMAT, "VERTICES", "SPECULATIVE QUEUED", "SPECULATIVE RUNNING", "GUARANTEED QUEUED", "GUARANTEED RUNNING");
    ...
    {
        final long sq = getCounterValueByGroupName(vertexCounters, counterGroup, LlapWmCounters.SPECULATIVE_QUEUED_NS.name());
        final long sr = getCounterValueByGroupName(vertexCounters, counterGroup, LlapWmCounters.SPECULATIVE_RUNNING_NS.name());
        final long gq = getCounterValueByGroupName(vertexCounters, counterGroup, LlapWmCounters.GUARANTEED_QUEUED_NS.name());
        final long gr = getCounterValueByGroupName(vertexCounters, counterGroup, LlapWmCounters.GUARANTEED_RUNNING_NS.name());


        return String.format(LLAP_SUMMARY_HEADER_FORMAT, vertexName, secondsFormatter.format(sq / 1000_000_000.0) + "s", secondsFormatter.format(sr / 1000_000_000.0) + "s", secondsFormatter.format(gq / 1000_000_000.0) + "s", secondsFormatter.format(gr / 1000_000_000.0) + "s");
    }
