...
class TextDescTableFormatter extends DescTableFormatter
{
    ...
    private void addStatsData(DataOutputStream out, HiveConf conf, String columnPath, List<FieldSchema> columns, boolean isFormatted, List<ColumnStatisticsObj> columnStats, boolean isOutputPadded) throws IOException
    {
        ...
        {
            metaDataTable.addRow(ShowUtils.extractColumnValues(column, needColStats, getColumnStatisticsObject(column.getName(), column.getType(), columnStats), histogramEnabled));
        }
        if (needColStats)
        {
    ...
    private ColumnStatisticsObj getColumnStatisticsObject(String columnName, String columnType, List<ColumnStatisticsObj> columnStats)
    {
        if (CollectionUtils.isNotEmpty(columnStats))
        {
            ...
            {
                if (columnStat.getColName().equalsIgnoreCase(columnName) && columnStat.getColType().equalsIgnoreCase(columnType))
                {
                    return columnStat;
                }
            }
    ...
    private static void getMaterializedViewInfo(StringBuilder tableInfo, Table table, boolean isOutputPadded)
    {
        ...
        MaterializationSnapshotFormatter snapshotFormatter = createMaterializationSnapshotFormatter(table.getMVMetadata().getSnapshot());
    ...
    private void getStorageDescriptorInfo(StringBuilder tableInfo, Table table, StorageDescriptor storageDesc)
    {
        ...
        if (storageDesc.getSkewedInfo() != null)
        {
            ...
            {
                List<String> skewedCoumnNames = storageDesc.getSkewedInfo().getSkewedColNames().stream().sorted().collect(Collectors.toList());
            ...
            {
                List<List<String>> skewedColumnValues = storageDesc.getSkewedInfo().getSkewedColValues().stream().sorted(new VectorComparator<String>()).collect(Collectors.toList());
    ...
    private void displayAllParameters(Map<String, String> params, StringBuilder tableInfo, boolean escapeUnicode, boolean isOutputPadded)
    {
        ...
        for (String key : keys)
        {
            ...
            formatOutput(key, escapeUnicode ? StringEscapeUtils.escapeJava(value) : HiveStringUtils.escapeJava(value), tableInfo, isOutputPadded);
        }
    }
    ...
    private void getForeignKeyRelInformation(StringBuilder constraintsInfo, String constraintName, List<ForeignKeyCol> columns)
    {
        ...
        if (CollectionUtils.isNotEmpty(columns))
        {
            for (ForeignKeyCol column : columns)
            {
                ...
                fields[0] = "Parent Column Name:" + column.parentDatabaseName + "." + column.parentTableName + "." + column.parentColName;
