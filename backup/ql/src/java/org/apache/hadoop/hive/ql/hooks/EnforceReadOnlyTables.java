...
public class EnforceReadOnlyTables implements ExecuteWithHookContext
{
    ...
    public void run(SessionState sess, Set<ReadEntity> inputs, Set<WriteEntity> outputs, UserGroupInformation ugi, boolean isExplain) throws Exception
    {
        ...
        for (WriteEntity w : outputs)
        {
            ...
            {
                Table t = w.getTable();
                if (DEFAULT_DATABASE_NAME.equalsIgnoreCase(t.getDbName()) && readOnlyTables.contains(t.getTableName()) && !isExplain)
                {
