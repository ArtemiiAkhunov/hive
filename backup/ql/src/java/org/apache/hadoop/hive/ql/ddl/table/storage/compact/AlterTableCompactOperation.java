...
public class AlterTableCompactOperation extends DDLOperation<AlterTableCompactDesc>
{
    ...
    public int execute() throws Exception
    {
        ...
        CompactionRequest compactionRequest = new CompactionRequest(table.getDbName(), table.getTableName(), compactionTypeStr2ThriftType(desc.getCompactionType()));
        ...
        Map<String, org.apache.hadoop.hive.metastore.api.Partition> partitionMap = convertPartitionsFromThriftToDB(getPartitions(table, desc, context));
        ...
        List<CompactionResponse> compactionResponses = initiatorBase.initiateCompactionForTable(compactionRequest, table.getTTable(), partitionMap);
        for (CompactionResponse compactionResponse : compactionResponses)
        {
            if (!compactionResponse.isAccepted())
            {
                ...
                if (compactionResponse.isSetErrormessage())
                {
                    ...
                    throw new HiveException(ErrorMsg.COMPACTION_REFUSED, table.getDbName(), table.getTableName(), "CompactionId: " + compactionResponse.getId(), message);
                }
                context.getConsole().printInfo("Compaction already enqueued with id " + compactionResponse.getId() + "; State is " + compactionResponse.getState());
                continue;
            }
    ...
    private List<Partition> getPartitions(Table table, AlterTableCompactDesc desc, DDLOperationContext context) throws HiveException
    {
