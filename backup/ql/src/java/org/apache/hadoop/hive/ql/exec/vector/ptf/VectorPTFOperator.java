...
public class VectorPTFOperator extends Operator<PTFDesc> implements VectorizationOperator, VectorizationContextRegion
{
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
         ...
         */
        overflowBatch = setupOverflowBatch(vContext.firstOutputColumnIndex(), vOutContext.getScratchColumnTypeNames(), outputProjectionColumnMap, outputTypeInfos);
        ...
        groupBatches.init(evaluators, outputProjectionColumnMap, bufferedColumnMap, bufferedTypeInfos, orderColumnMap, keyWithoutOrderColumnMap, overflowBatch);
    ...
    private void initBufferedColumns()
    {
         ...
         */
        int bufferedColumnCount = orderColumnMap.length + nonKeyInputColumnMap.length + streamingEvaluatorNums.length + scratchColumnPositions.length;
        ...
        for (int i = 0; i < orderColumnMap.length; i++)
        {
            ...
            bufferedTypeInfos[i] = reducerBatchTypeInfos.length <= columnNum ? columnVectorTypeToTypeInfo(orderColumnVectorTypes[i]) : reducerBatchTypeInfos[columnNum];
        }
    ...
    private void initExpressionColumns()
    {
        for (int i = 0; i < evaluators.length; i++)
        {
            ...
            {
                evaluator.inputColumnNum = IntStream.range(0, bufferedColumnMap.length).filter(j -> bufferedColumnMap[j] == evaluator.inputColumnNum).findFirst().orElseGet(() -> evaluator.inputColumnNum);
                ...
                if (evaluator.inputVecExpr != null)
                {
                    for (int j = 0; j < evaluator.inputVecExpr.inputColumnNum.length; j++)
                    {
                        ...
                        evaluator.inputVecExpr.inputColumnNum[jj] = IntStream.range(0, bufferedColumnMap.length).filter(k -> bufferedColumnMap[k] == evaluator.inputVecExpr.inputColumnNum[jj]).findFirst().orElseGet(() -> evaluator.inputVecExpr.inputColumnNum[jj]);
                    }
                    evaluator.inputVecExpr.outputColumnNum = IntStream.range(0, bufferedColumnMap.length).filter(j -> bufferedColumnMap[j] == evaluator.inputVecExpr.outputColumnNum).findFirst().orElseGet(() -> evaluator.inputVecExpr.outputColumnNum);
                }
    ...
    {
        List<Integer> keyColumnsWithoutOrderColumns = IntStream.of(keyInputColumnMap).boxed().collect(Collectors.toList());
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            switch (partitionColumnVectorTypes[i])
            {
                ...
                case BYTES:
                    ...
                    System.arraycopy(currentPartitionByteArrays[i], 0, key[i], 0, currentPartitionByteLengths[i]);
                    break;
                case DECIMAL:
