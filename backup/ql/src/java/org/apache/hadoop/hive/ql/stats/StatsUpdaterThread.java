...
public class StatsUpdaterThread extends Thread implements MetaStoreThread
{
    ...
    public void setConf(Configuration conf)
    {
        ...
        noUpdatesWaitMs = MetastoreConf.getTimeVar(conf, ConfVars.STATS_AUTO_UPDATE_NOOP_WAIT, TimeUnit.MILLISECONDS);
    ...
    public void init(AtomicBoolean stop) throws MetaException
    {
        ...
        rs = RawStoreProxy.getProxy(conf, conf, MetastoreConf.getVar(conf, MetastoreConf.ConfVars.RAW_STORE_IMPL));
    ...
    private List<AnalyzeWork> processOneTable(TableName fullTableName, Map<String, Boolean> dbsToSkip) throws MetaException, NoSuchTxnException, NoSuchObjectException
    {
        if (isAnalyzeTableInProgress(fullTableName))
        {
            return null;
        }
        ...
        if (isTxn)
        {
            ...
            {
                return null; // Skip transactional tables.
            }
        ...
        if (table.getPartitionKeysSize() == 0)
        {
            ...
            if (isExistingOnly)
            {
                ...
                colsToUpdate = getExistingNonPartTableStatsToUpdate(fullTableName, cat, db, tbl, params, writeId, allCols, writeIdString);
            } else
            {
                colsToUpdate = getAnyStatsToUpdate(db, tbl, allCols, params, writeId, writeIdString);
            }

            LOG.debug("Columns to update are {}; existing only: {}, out of: {} based on {}", colsToUpdate, isExistingOnly, allCols, params);
        ...
        } else
        {
            ...
            List<String> colsForAllParts = findPartitionsToAnalyze(fullTableName, cat, db, tbl, allCols, partsToAnalyze, writeIdString);
            LOG.debug("Columns to update are {} for all partitions; {} individual partitions." + " Existing only: {}, out of: {}", colsForAllParts, partsToAnalyze.size(), isExistingOnly, allCols);
            ...
            if (colsForAllParts != null)
            {
                ...
                return Lists.newArrayList(new AnalyzeWork(fullTableName, null, buildPartColStr(table), colsForAllParts));
            }
    ...
    private List<String> findPartitionsToAnalyze(TableName fullTableName, String cat, String db, String tbl, List<String> allCols, Map<String, List<String>> partsToAnalyze, String writeIdString) throws MetaException, NoSuchObjectException
    {
        ...
        while (true)
        {
            ...
            if (isAnalyzePartInProgress(fullTableName, partName) || "true".equalsIgnoreCase(skipParam))
            {
                if (isAllParts)
                {
                    addPreviousPartitions(t, partNames, currentBatchStart, currentBatch, currentIxInBatch, colsToUpdateForAll, partsToAnalyze);
                }
                isAllParts = false;
                continue;
            }
            ...
            List<String> colsToUpdate = getAnyStatsToUpdate(db, tbl, colsToMaybeUpdate, params, writeIdString == null ? -1 : part.getWriteId(), writeIdString);
    ...
    private ValidReaderWriteIdList getWriteIds(TableName fullTableName) throws NoSuchTxnException, MetaException
    {
        ...
        return TxnCommonUtils.createValidReaderWriteIdList(txnHandler.getValidWriteIds(req).getTblValidWriteIds().get(0));
    }
    ...
    private final static class AnalyzeWork
    {
        ...
        {
            return "AnalyzeWork [tableName=" + tableName + ", partName=" + partName + ", allParts=" + allParts + ", cols=" + cols + "]";
        }
    }
