...
public class HiveFilterTableFunctionTransposeRule extends RelOptRule
{
    ...
    {
        super(operand(HiveFilter.class, operand(HiveTableFunctionScan.class, any())), relBuilderFactory, null);
    }
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        final RexNode filterCondToPushBelowProj = RexUtil.composeConjunction(filter.getCluster().getRexBuilder(), newPartKeyFilterConditions, true);
        ...
        final RexNode unpushedFilCondAboveProj = unpushedFilterConditions.isEmpty() ? null : RexUtil.composeConjunction(filter.getCluster().getRexBuilder(), unpushedFilterConditions, true);

        // Generate the new TableFunctionScanNode with the Filter InputRel
        final RelNode tableFunctionScanNode = tfs.copy(tfs.getTraitSet(), ImmutableList.of(builder.build()), tfs.getCall(), tfs.getElementType(), tfs.getRowType(), tfs.getColumnMappings());
