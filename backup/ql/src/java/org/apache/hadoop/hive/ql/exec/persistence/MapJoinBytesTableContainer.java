...
public class MapJoinBytesTableContainer implements MapJoinTableContainer, MapJoinTableContainerDirectAccess
{
    ...
    private class GetAdaptor implements ReusableGetAdaptor, ReusableGetAdaptorDirectAccess
    {
        ...
        public JoinUtil.JoinResult setFromVectorNoNulls(VectorHashKeyWrapperBase kw, VectorExpressionWriter[] keyOutputWriters, VectorHashKeyWrapperBatch keyWrapperBatch, MatchTracker matchTracker) throws HiveException
        {
            ...
            return currentValue.setFromOutput(MapJoinKey.serializeRow(output, currentKey, vectorKeyOIs, sortableSortOrders, nullMarkers, notNullMarkers), matchTracker);
        }
        ...
        public JoinUtil.JoinResult setFromRowNoNulls(Object row, List<ExprNodeEvaluator> fields, List<ObjectInspector> ois, MatchTracker matchTracker) throws HiveException
        {
            ...
            return currentValue.setFromOutput(MapJoinKey.serializeRow(output, currentKey, ois, sortableSortOrders, nullMarkers, notNullMarkers), matchTracker);
        }
    ...
    private class ReusableRowContainer implements MapJoinRowContainer, AbstractRowContainer.RowIterator<List<Object>>
    {
        ...
        public JoinUtil.JoinResult setFromOutput(Output output)
        {
            ...
            aliasFilter = hashMap.getValueResult(output.getData(), 0, output.getLength(), hashMapResult, /* matchTracker */ null);
            dummyRow = null;
        ...
        public JoinUtil.JoinResult setFromOutput(Output output, MatchTracker matchTracker)
        {
            ...
            aliasFilter = hashMap.getValueResult(output.getData(), 0, output.getLength(), hashMapResult, matchTracker);
            dummyRow = null;
            wasFirstCalledOnDummyRow = false;
            if (hashMapResult.hasRows())
            {
    ...
    private class NonMatchedSmallTableIteratorImpl implements NonMatchedSmallTableIterator
    {
        ...
        public boolean isNext()
        {
            ...
            currentIndex = hashMap.findNextNonMatched(currentIndex, keyRef, currentValue.getHashMapResult(), matchTracker);
        ...
        {
            List<Object> deserializedList = MapJoinKey.deserializeRow(keyRef.getBytes(), (int) keyRef.getOffset(), keyRef.getLength(), bytesWritable, keySerde);
