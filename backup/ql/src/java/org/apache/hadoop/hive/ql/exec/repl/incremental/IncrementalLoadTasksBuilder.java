...
public class IncrementalLoadTasksBuilder
{
    ...
    public IncrementalLoadTasksBuilder(String dbName, String loadPath, IncrementalLoadEventsIterator iterator, HiveConf conf, Long eventTo, ReplicationMetricCollector metricCollector, ReplStatsTracker replStatsTracker, boolean shouldFailover, int bootstrapTableSize) throws SemanticException
    {
        ...
        if (shouldFailover)
        {
            ...
            this.metricCollector.reportFailoverStart("REPL_LOAD", metricMap, new FailoverMetaData(new Path(dumpDirectory, ReplUtils.REPL_HIVE_BASE_DIR), conf), dbFailoverEndPoint, ReplConst.FailoverType.PLANNED.toString());
        } else
        {
    ...
    public Task<?> build(Context context, Hive hive, Logger log, TaskTracker tracker) throws Exception
    {
        ...
        if (!hasMoreWork())
        {
            ...
            dbProps.put(ReplicationSpec.KEY.CURR_STATE_ID_SOURCE.toString(), String.valueOf((lastReplayedEvent == null) ? eventTo : lastReplayedEvent));
    ...
    private boolean isEventNotReplayed(Map<String, String> params, FileStatus dir, DumpType dumpType)
    {
        if (params != null && (params.containsKey(ReplicationSpec.KEY.CURR_STATE_ID_SOURCE.toString())))
        {
            ...
            {
                log.debug("Event " + dumpType + " with replId " + Long.parseLong(dir.getPath().getName()) + " is already replayed. LastReplId - " + Long.parseLong(replLastId));
                return false;
            }
        }
        return true;
