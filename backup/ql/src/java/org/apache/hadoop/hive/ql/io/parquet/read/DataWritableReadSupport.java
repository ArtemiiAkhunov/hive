...
public class DataWritableReadSupport extends ReadSupport<ArrayWritable>
{
    ...
    {
        return (List<String>) VirtualColumn.removeVirtualColumns(StringUtils.getStringCollection(columns));
    }
    ...
    public static MessageType getSchemaByName(MessageType schema, List<String> colNames, List<TypeInfo> colTypes)
    {
        ...
        return Types.buildMessage().addFields(typesArray).named(schema.getName());
    }
     ...
     * @return
     */
    public static MessageType getRequestedSchema(boolean indexAccess, List<String> columnNamesList, List<TypeInfo> columnTypesList, MessageType fileSchema, Configuration configuration)
    {
        MessageType tableSchema = getRequestedSchemaForIndexAccess(indexAccess, columnNamesList, columnTypesList, fileSchema);
        ...
        {
            return DataWritableReadSupport.getSchemaByIndex(tableSchema, columnNamesList, indexColumnsWanted);
        } else
        {
            return tableSchema;
        }
    ...
    private static MessageType getRequestedSchemaForIndexAccess(boolean indexAccess, List<String> columnNamesList, List<TypeInfo> columnTypesList, MessageType fileSchema)
    {
        if (indexAccess)
        {
    ...
    private static MessageType getRequestedPrunedSchema(List<String> columnNamesList, MessageType fileSchema, Configuration configuration)
    {
    ...
    public RecordMaterializer<ArrayWritable> prepareForRead(final Configuration configuration, final Map<String, String> keyValueMetaData, final MessageType fileSchema, final org.apache.parquet.hadoop.api.ReadSupport.ReadContext readContext)
    {
        ...
        {
            throw new IllegalStateException("Metadata contains a writer time zone that does not match " + "file footer's writer time zone.");
        }
        ...
        {
            throw new IllegalStateException("Metadata contains a writer proleptic property value that does not match " + "file footer's value.");
        }
        ...
        {
            metadata.put(prolepticDefault, String.valueOf(HiveConf.getBoolVar(configuration, HiveConf.ConfVars.HIVE_PARQUET_DATE_PROLEPTIC_GREGORIAN_DEFAULT)));
        }
        ...
        } else
        {
            ...
            {
                throw new IllegalStateException("Different values for " + DataWritableWriteSupport.WRITER_ZONE_CONVERSION_LEGACY + " metadata: context [" + ctxMeta + "], file [" + fileMeta + "].");
            }
        }
