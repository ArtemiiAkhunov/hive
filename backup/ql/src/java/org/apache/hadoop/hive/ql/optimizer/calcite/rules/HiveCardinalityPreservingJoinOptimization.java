...
public class HiveCardinalityPreservingJoinOptimization extends HiveRelFieldTrimmer
{
    ...
    private RexNode joinCondition(RelNode leftInput, Mapping leftInputMapping, TableToJoinBack tableToJoinBack, RelNode rightInput, Mapping rightInputKeyMapping, RexBuilder rexBuilder)
    {
        ...
        {
            if (usedKeys.get(tableInputRefHolder.tableInputRef.getIndex()) || !tableToJoinBack.keys.get(tableInputRefHolder.tableInputRef.getIndex()))
            {
                continue;
            }
            ...
            equalsConditions.add(rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, rexBuilder.makeInputRef(leftKeyField.getValue(), leftKeyField.getIndex()), rexBuilder.makeInputRef(rightKeyField.getValue(), leftInput.getRowType().getFieldCount() + rightKeyIndex)));
        }
    ...
    private static class JoinedBackFieldsBuilder
    {
        ...
        {
            return new JoinedBackFields(relTableRef, fieldsInOriginalRowTypeBuilder.build(), fieldsInSourceTableBuilder.build(), mapping);
        }
    }
