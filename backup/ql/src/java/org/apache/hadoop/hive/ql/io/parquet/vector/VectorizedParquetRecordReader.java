 ...
 */
public class VectorizedParquetRecordReader extends ParquetRecordReaderBase implements RecordReader<NullWritable, VectorizedRowBatch>, RowPositionAwareVectorizedRecordReader
{
    ...
    public void initialize(ParquetInputSplit split, JobConf configuration) throws IOException, InterruptedException, HiveException
    {
        ...
        this.writerTimezone = DataWritableReadSupport.getWriterTimeZoneId(parquetMetadata.getFileMetaData().getKeyValueMetaData());
        ...
        this.reader = new ParquetFileReader(configuration, parquetMetadata.getFileMetaData(), path, blocks, requestedSchema.getColumns());
    }
    ...
    private ParquetMetadata readFooterFromFile(final Path file, final FileSystem fs, final FileStatus stat, MetadataFilter filter) throws IOException
    {
    ...
    private boolean nextBatch(VectorizedRowBatch columnarBatch) throws IOException
    {
        ...
        if (colsToInclude.size() > 0)
        {
            for (int i = 0; i < columnReaders.length; ++i)
            {
                ...
                columnReaders[i].readBatch(num, columnarBatch.cols[colsToInclude.get(i)], columnTypesList.get(colsToInclude.get(i)));
            }
        }
