...
class DriverTxnHandler
{
    ...
    private void allocateWriteIdForAcidAnalyzeTable() throws LockException
    {
        if (driverContext.getPlan().getAcidAnalyzeTable() != null)
        {
            ...
            driverContext.getTxnManager().setTableWriteId(table.getDbName(), table.getTableName(), driverContext.getAnalyzeTableWriteId());
        }
    }
    ...
    /* It's imperative that {@code acquireLocks()} is called for all commands so that
       HiveTxnManager can transition its state machine correctly */
        ...
        LOG.info("Operation {} obtained {} locks", driverContext.getPlan().getOperation(), ((locks == null) ? 0 : locks.size()));
        // This check is for controlling the correctness of the current state
        if (driverContext.getTxnManager().recordSnapshot(driverContext.getPlan()) && !driverContext.isValidTxnListsGenerated())
        {
            throw new IllegalStateException("Need to record valid WriteID list but there is no valid TxnID list (" + JavaUtils.txnIdToString(driverContext.getTxnManager().getCurrentTxnId()) + ", queryId: " + driverContext.getPlan().getQueryId() + ")");
        }
    }
    ...
    ValidTxnWriteIdList recordValidWriteIds() throws LockException
    {
        ...
        {
            throw new IllegalStateException("calling recordValidWriteIds() without initializing ValidTxnList " + JavaUtils.txnIdToString(driverContext.getTxnManager().getCurrentTxnId()));
        }
        ...
        LOG.debug("Encoding valid txn write ids info {} txnid: {}", txnWriteIds.toString(), driverContext.getTxnManager().getCurrentTxnId());
        return txnWriteIds;
    }
    ...
    private ValidTxnWriteIdList getTxnWriteIds(String txnString) throws LockException
    {
        ...
        {
            throw new IllegalStateException(String.format("Inferred transaction type '%s' doesn't conform to the actual query string '%s'", driverContext.getTxnType(), driverContext.getQueryState().getQueryString()));
        }
        return txnWriteIds;
    }
        ...
        for (HiveLock lock : context.getHiveLocks())
        {
            if (lock.mayContainComponents())
            {
                ...
                for (LockComponent lockComponent : lock.getHiveLockComponents())
                {
                    ...
                    if ((lockComponent.getType() == LockType.EXCLUSIVE || lockComponent.getType() == LockType.EXCL_WRITE) && lockComponent.getTablename() != null && !DbTxnManager.GLOBAL_LOCKS.equals(lockComponent.getDbname()))
                    {
            ...
            } else
            {
                ...
                if ((lock.getHiveLockMode() == HiveLockMode.EXCLUSIVE || lock.getHiveLockMode() == HiveLockMode.SEMI_SHARED) && lock.getHiveLockObject().getPaths().length == 2)
                {
                    nonSharedLockedTables.add(TableName.getDbTable(lock.getHiveLockObject().getPaths()[0], lock.getHiveLockObject().getPaths()[1]));
                }
            }
        }
    ...
    void handleTransactionAfterExecution() throws CommandProcessorException
    {
        try
        {
            ...
            if (driverContext.getTxnManager().isImplicitTransactionOpen(context) || driverContext.getPlan().getOperation() == HiveOperation.COMMIT)
            {
                endTransactionAndCleanup(true);
            } else if (driverContext.getPlan().getOperation() == HiveOperation.ROLLBACK)
            {
                endTransactionAndCleanup(false);
            } else if (!driverContext.getTxnManager().isTxnOpen() && driverContext.getQueryState().getHiveOperation() == HiveOperation.REPLLOAD)
            {
    ...
    {
        return tables.entrySet().stream().filter(entry -> AcidUtils.isTransactionalTable(entry.getValue())).map(Map.Entry::getKey).collect(Collectors.toList());
    }
    ...
    void destroy(String queryIdFromDriver)
    {
        ...
        boolean isTxnOpen = driverContext != null && driverContext.getTxnManager() != null && driverContext.getTxnManager().isTxnOpen() && org.apache.commons.lang3.StringUtils.equals(queryIdFromDriver, driverContext.getTxnManager().getQueryid());
    ...
    private void commitOrRollback(boolean commit, HiveTxnManager txnManager) throws LockException
    {
        if (commit)
        {
            if (driverContext.getConf().getBoolVar(ConfVars.HIVE_IN_TEST) && driverContext.getConf().getBoolVar(ConfVars.HIVE_TEST_MODE_ROLLBACK_TXN))
            {
