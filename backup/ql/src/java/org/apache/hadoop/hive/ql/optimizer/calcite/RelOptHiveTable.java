...
public class RelOptHiveTable implements RelOptTable
{
    ...
    public RelOptHiveTable(RelOptSchema calciteSchema, RelDataTypeFactory typeFactory, List<String> qualifiedTblName, RelDataType rowType, Table hiveTblMetadata, List<ColumnInfo> hiveNonPartitionCols, List<ColumnInfo> hivePartitionCols, List<VirtualColumn> hiveVirtualCols, HiveConf hconf, Hive db, ParsedQueryTables tabNameToTabObject, Map<String, PrunedPartitionList> partitionCache, Map<String, ColumnStatsList> colStatsCache, AtomicInteger noColsMissingStats)
    {
        ...
        if (foreignKeyInfo != null && !foreignKeyInfo.getForeignKeys().isEmpty())
        {
            for (List<ForeignKeyCol> fkCols : foreignKeyInfo.getForeignKeys().values())
            {
                ...
                if (parentDatabaseName != null && !parentDatabaseName.isEmpty())
                {
                    ...
                    qualifiedName = TableName.getDbTable(parentDatabaseName, parentTableName);
                } else
                {
                    parentTableQualifiedName.add(parentTableName);
                ...
                builder.add(RelReferentialConstraintImpl.of(qualifiedTblName, parentTableQualifiedName, keys.build()));
            }
        }
    ...
    public double getRowCount()
    {
        if (rowCount == -1)
        {
            ...
            rowCount = StatsUtils.getNumRows(hiveConf, getNonPartColumns(), hiveTblMetadata, partitionList, noColsMissingStats);
        }
    ...
    public void computePartitionList(HiveConf conf, RexNode pruneNode, Set<Integer> partOrVirtualCols)
    {
        try
        {
            ...
            ExprNodeDesc pruneExpr = pruneNode.accept(new ExprNodeConverter(getName(), getRowType(), partOrVirtualCols, getTypeFactory()));
    ...
    {
        return obj instanceof RelOptHiveTable && this.rowType.equals(((RelOptHiveTable) obj).getRowType()) && this.getHiveTableMD().equals(((RelOptHiveTable) obj).getHiveTableMD());
    }
