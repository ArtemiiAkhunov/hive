...
public class AlterTableArchiveOperation extends DDLOperation<AlterTableArchiveDesc>
{
    ...
    public int execute() throws HiveException
    {
        ...
        Path intermediateArchivedDir = AlterTableArchiveUtils.getInterMediateDir(originalDir, context.getConf(), ConfVars.METASTORE_INT_ARCHIVED);
        Path intermediateOriginalDir = AlterTableArchiveUtils.getInterMediateDir(originalDir, context.getConf(), ConfVars.METASTORE_INT_ORIGINAL);
        ...
        } else
        {
            ...
            {
                context.getConsole().printInfo("Intermediate archive directory " + intermediateArchivedDir + " already exists. Assuming it contains an archived version of the partition");
            }
        }
    ...
    private Path getOriginalDir(Table table, PartSpecInfo partitionSpecInfo, List<Partition> partitions) throws HiveException
    {
    ...
    {
        if (HdfsUtils.pathExists(intermediateArchivedDir, context.getConf()) || HdfsUtils.pathExists(intermediateOriginalDir, context.getConf()))
        {
    ...
    private Path createArchiveInTmpDir(Table table, PartSpecInfo partitionSpecInfo, Path originalDir) throws HiveException
    {
        ...
        try
        {
            ...
            List<String> args = ImmutableList.of("-archiveName", ARCHIVE_NAME, "-p", originalDir.toString(), tmpPath.toString());
    ...
    private void moveOriginalDirToIntermediateDir(Path originalDir, Path intermediateOriginalDir, FileSystem fs) throws HiveException
    {
        ...
        {
            context.getConsole().printInfo(intermediateOriginalDir + " already exists. " + "Assuming it contains the original files in the partition");
        }
    }
    ...
    private void moveIntermediateArchivedDirToOriginalParent(Path originalDir, Path intermediateArchivedDir, FileSystem fs) throws HiveException
    {
        ...
        {
            context.getConsole().printInfo(originalDir + " already exists. Assuming it contains the archived version of " + "the partition");
        }
    }
     ...
     */
    private void writeArchivationToMetastore(PartSpecInfo partitionSpecInfo, List<Partition> partitions, Path originalDir) throws HiveException
    {
        try
        {
    ...
    private void deleteIntermediateOriginalDir(Table table, Path intermediateOriginalDir) throws HiveException
    {
        if (HdfsUtils.pathExists(intermediateOriginalDir, context.getConf()))
        {
            boolean shouldEnableCm = ReplChangeManager.shouldEnableCm(context.getDb().getDatabase(table.getDbName()), table.getTTable());
