...
final class MinorQueryCompactor extends QueryCompactor
{
    ...
    public boolean run(CompactorContext context) throws IOException
    {
        ...
        List<String> createQueries = getCreateQueries(table, tmpTableName, dir, writeIds, resultDeltaDir, resultDeleteDeltaDir);
        ...
        runCompactionQueries(conf, tmpTableName, storageDescriptor, writeIds, context.getCompactionInfo(), Lists.newArrayList(resultDeltaDir, resultDeleteDeltaDir), createQueries, compactionQueries, dropQueries, table.getParameters());
        return true;
    }
    ...
    private List<String> getCreateQueries(Table table, String tempTableBase, AcidDirectory dir, ValidWriteIdList writeIds, Path tmpTableResultLocation, Path tmpTableDeleteResultLocation)
    {
        ...
        queries.add(buildCreateTableQuery(table, tmpTableName + "_result", false, true, tmpTableResultLocation.toString()));
        ...
        queries.add(buildCreateTableQuery(table, tmpDeleteTableName + "_result", false, true, tmpTableDeleteResultLocation.toString()));
        return queries;
    }
    ...
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.CREATE, false, newTableName).setSourceTab(table).setBucketed(isBucketed).setPartitioned(isPartitioned).setLocation(location).build();
    }
    ...
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.ALTER, false, tableName).setDir(dir).setValidWriteIdList(validWriteIdList).setIsDeleteDelta(isDeleteDelta).build();
    }
    ...
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.INSERT, false, resultTableName).setSourceTabForInsert(sourceTableName).setSourceTab(table).setValidWriteIdList(validWriteIdList).build();
    }
    ...
    {
        return Lists.newArrayList(getDropQuery(AcidUtils.DELTA_PREFIX + tmpTableBase), getDropQuery(AcidUtils.DELETE_DELTA_PREFIX + tmpTableBase), getDropQuery(AcidUtils.DELTA_PREFIX + tmpTableBase + "_result"), getDropQuery(AcidUtils.DELETE_DELTA_PREFIX + tmpTableBase + "_result"));
    }

    private String getDropQuery(String tableToDrop)
    {
        return new CompactionQueryBuilder(CompactionType.MINOR, CompactionQueryBuilder.Operation.DROP, false, tableToDrop).build();
    }
}
