...
public class ReplicationSemanticAnalyzer extends BaseSemanticAnalyzer
{
    ...
    public void analyzeInternal(ASTNode ast) throws SemanticException
    {
        ...
        switch (ast.getToken().getType())
        {
            case TOK_REPL_DUMP:
            {
                ...
                {
                    ReplUtils.reportStatusInReplicationMetrics("REPL_DUMP", ReplUtils.isErrorRecoverable(e) ? Status.FAILED_ADMIN : Status.FAILED, null, conf, null, null);
                    throw e;
                }
                break;
            }
            case TOK_REPL_LOAD:
            {
                ...
                } catch (SemanticException e)
                {
                    ...
                    {
                        ReplUtils.reportStatusInReplicationMetrics("REPL_LOAD", ReplUtils.isErrorRecoverable(e) ? Status.FAILED_ADMIN : Status.FAILED, null, conf, null, null);
                    }
                    throw e;
                }
                break;
    ...
    private void initReplDump(ASTNode ast) throws HiveException
    {
        ...
        for (String dbName : databases)
        {
            ...
            if (database != null)
            {
                ...
                {
                    LOG.info("Triggering optimized bootstrap for database {} since it " + "is marked as target of replication (repl.target.for) " + "with {} set to {}", dbName, ReplConst.REPL_FAILOVER_ENDPOINT, Objects.nonNull(dbParams) ? dbParams.get(ReplConst.REPL_FAILOVER_ENDPOINT) : null);
                }
            } else
            {
    ...
    private void analyzeReplLoad(ASTNode ast) throws SemanticException
    {
        ...
        try
        {
            ...
            Path latestDumpPath = ReplUtils.getLatestDumpPath(ReplUtils.getEncodedDumpRootPath(conf, sourceDbNameOrPattern.toLowerCase()), conf);
            if (ReplUtils.failedWithNonRecoverableError(latestDumpPath, conf))
            {
                ...
                ReplUtils.reportStatusInReplicationMetrics("REPL_LOAD", Status.SKIPPED, nonRecoverableFile.toString(), conf, sourceDbNameOrPattern, null);
            ...
            if (loadPath != null)
            {
                ...
                ReplLoadWork replLoadWork = new ReplLoadWork(conf, loadPath.toString(), sourceDbNameOrPattern, replScope.getDbName(), dmd.getReplScope(), queryState.getLineageState(), evDump, dmd.getEventTo(), dmd.getDumpExecutionId(), metricCollector, dmd.isReplScopeModified());
    ...
    private ReplicationMetricCollector initReplicationLoadMetricCollector(String dumpDirectory, String dbNameToLoadIn, DumpMetaData dmd) throws SemanticException
    {
        ...
        if (dmd.isPreOptimizedBootstrapDump() || dmd.isOptimizedBootstrapDump())
        {
            ...
            if (dmd.isPreOptimizedBootstrapDump())
            {
                collector = new PreOptimizedBootstrapLoadMetricCollector(dbNameToLoadIn, dumpDirectory, dmd.getDumpExecutionId(), conf, MetaStoreUtils.FailoverEndpoint.TARGET.toString(), failoverType);
            } else
            {
                // db property ReplConst.FAILOVER_ENDPOINT is only set during planned failover.
                collector = new OptimizedBootstrapLoadMetricCollector(dbNameToLoadIn, dumpDirectory, dmd.getDumpExecutionId(), conf, MetaStoreUtils.FailoverEndpoint.TARGET.toString(), failoverType);
            }
    ...
    private Path getCurrentLoadPath() throws IOException, SemanticException
    {
        ...
        if (fs.exists(loadPathBase))
        {
            ...
            if (statuses.length > 0)
            {
                ...
                if (loadPathBase.getFileSystem(conf).exists(new Path(hiveDumpPath, ReplAck.DUMP_ACKNOWLEDGEMENT.toString())) && !loadPathBase.getFileSystem(conf).exists(new Path(hiveDumpPath, LOAD_ACKNOWLEDGEMENT.toString())))
                {
                    return hiveDumpPath;
                }
            }
    ...
    private void analyzeReplStatus(ASTNode ast) throws SemanticException
    {
        ...
        LOG.debug("ReplicationSemanticAnalyzer.analyzeReplStatus: writing repl.last.id={} out to {} using configuration {}", replLastId, ctx.getResFile(), conf);
    }
