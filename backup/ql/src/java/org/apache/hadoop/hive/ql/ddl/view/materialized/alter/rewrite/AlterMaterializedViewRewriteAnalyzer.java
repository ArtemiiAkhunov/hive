    ...
    public void analyzeInternal(ASTNode root) throws SemanticException
    {
        ...
        if (rewriteEnable)
        {
            for (SourceTable sourceTable : materializedViewTable.getMVMetadata().getSourceTables())
            {
                ...
                if (!AcidUtils.isTransactionalTable(sourceTable.getTable()) && !(table.isNonNative() && table.getStorageHandler().areSnapshotsSupported()))
                {
        ...
        if (MetaStoreUtils.isNonNativeTable(materializedViewTable.getTTable()) && materializedViewTable.getStorageHandler().areSnapshotsSupported())
        {
            type = WriteEntity.WriteType.DDL_SHARED;
        } else
        {
            type = AcidUtils.isLocklessReadsEnabled(materializedViewTable, conf) ? WriteEntity.WriteType.DDL_EXCL_WRITE : WriteEntity.WriteType.DDL_EXCLUSIVE;
        }
