...
public class OrcNewInputFormat extends InputFormat<NullWritable, OrcStruct>
{
    ...
    public RecordReader<NullWritable, OrcStruct> createRecordReader(InputSplit inputSplit, TaskAttemptContext context) throws IOException, InterruptedException
    {
        ...
        return new OrcRecordReader(OrcFile.createReader(path, OrcFile.readerOptions(conf)), ShimLoader.getHadoopShims().getConfiguration(context), fileSplit.getStart(), fileSplit.getLength(), inputSplit);
    }
    ...
    private static class OrcRecordReader extends RecordReader<NullWritable, OrcStruct>
    {
        ...
        OrcRecordReader(Reader file, Configuration conf, long offset, long length, InputSplit inputSplit) throws IOException
        {
            numColumns = file.getSchema().getChildren().size();
            value = new OrcStruct(numColumns);
            if (inputSplit instanceof OrcNewSplit)
            {
                this.reader = OrcInputFormat.createReaderFromFile(file, conf, offset, length, ((OrcNewSplit) inputSplit).isOriginal());
            } else
            {
