...
public class CopyTask extends Task<CopyWork> implements Serializable
{
    ...
    protected int copyOnePath(Path fromPath, Path toPath)
    {
        ...
        try
        {
            ...
            {
                Utilities.FILE_OP_LOGGER.debug("Recursing into a single child directory {}", srcs[0].getPath().getName());
            ...
            for (FileStatus oneSrc : srcs)
            {
                ...
                if (!FileUtils.copy(srcFs, oneSrc.getPath(), dstFs, toPath, false, // delete source
                        work.isOverwrite(), // overwrite destination
                        conf, copyStatistics))
                {
                    console.printError("Failed to copy: '" + oneSrcPathStr + "to: '" + toPath.toString() + "'");
                    return 1;
                }
