 ...
 */
public class VectorGroupByOperator extends Operator<GroupByDesc> implements VectorizationOperator, VectorizationContextRegion, IConfigureJobConf
{
    ...
    private abstract class ProcessingModeBase implements IProcessingMode
    {
        ...
        protected abstract void doProcessBatch(VectorizedRowBatch batch, boolean isFirstGroupingSet, boolean[] currentGroupingSetsOverrideIsNulls) throws HiveException;
        ...
        protected void processAggregators(VectorizedRowBatch batch) throws HiveException
        {
            ...
            VectorAggregationBufferRow[] aggregationBufferSets = aggregationBatchInfo.getAggregationBuffers();
            ...
            } else
            {
                for (int i = 0; i < aggregators.length; ++i)
                {
                    aggregators[i].aggregateInputSelection(aggregationBufferSets, i, batch);
                }
            }
        }
        ...
        {
            VectorAggregateExpression.AggregationBuffer[] aggregationBuffers = new VectorAggregateExpression.AggregationBuffer[aggregators.length];
            for (int i = 0; i < aggregators.length; ++i)
            {
    ...
    final class ProcessingModeHashAggregate extends ProcessingModeBase
    {
        ...
        private Queue<VectorAggregationBufferRow> reusableAggregationBufferRows = new ArrayDeque<>(VectorizedRowBatch.DEFAULT_SIZE);
        ...
        public void initialize(Configuration hconf) throws HiveException
        {
            ...
            if (null != hconf)
            {
                ...
                this.numRowsCompareHashAggr = HiveConf.getIntVar(hconf, HiveConf.ConfVars.HIVE_GROUPBY_MAP_INTERVAL);
            } else
            {
                ...
                this.numRowsCompareHashAggr = HiveConf.ConfVars.HIVE_GROUPBY_MAP_INTERVAL.defaultIntVal;
            }
            ...
            if (groupingSets != null && groupingSets.length > 0)
            {
                ...
                LOG.info("New maxHtEntries: {}, groupingSets len: {}, numRowsCompareHashAggr: {}, " + "minReductionHashAggr:{} ", maxHtEntries, groupingSets.length, numRowsCompareHashAggr, minReductionHashAggr);
            }
            computeMemoryLimits();
        ...
        public void doProcessBatch(VectorizedRowBatch batch, boolean isFirstGroupingSet, boolean[] currentGroupingSetsOverrideIsNulls) throws HiveException
        {
            ...
            while (shouldFlush(batch))
            {
                ...
                {
                    LOG.debug("Flush did not progress: {} entries before, {} entries after", preFlushEntriesCount, numEntriesHashTable);
                    break;
                }
        ...
        private void computeMemoryLimits()
        {
            ...
            {
                LOG.debug("GBY memory limits - isLlap: {} maxMemory: {} ({} * {}) fixSize:{} (key:{} agg:{})", isLlap, LlapUtil.humanReadableByteCount(maxHashTblMemory), LlapUtil.humanReadableByteCount(maxMemory), memoryThreshold, fixedHashEntrySize, keyWrappersBatch.getKeysFixedSize(), aggregationBatchInfo.getAggregatorsFixedSize());
            }
        }
        ...
        int computeAvgAccess()
        {
            ...
            LOG.debug("totalAccessCount:{}, numEntries:{}, avgAccess:{}", totalAccessCount, numEntriesHashTable, avgAccess);
            return avgAccess;
        }
      ...
      /* Iterate the global (keywrapper,aggregationbuffers) map and emit
       a row for each key */
        ...
        private boolean shouldFlush(VectorizedRowBatch batch)
        {
            ...
            if (((numEntriesHashTable > this.maxHtEntries) && (currMemUsed > 0.3 * maxHashTblMemory)) || currMemUsed > maxHashTblMemory)
            {
                return true;
            }
            if (gcCanary.get() == null)
        ...
        private void updateAvgVariableSize(VectorizedRowBatch batch)
        {
            ...
            avgVariableSize = (int) ((avgVariableSize * sumBatchSize + keyVariableSize + aggVariableSize) / (sumBatchSize + batch.size));
        }
        ...
        private void checkHashModeEfficiency() throws HiveException
        {
            if (lastModeCheckRowCount > numRowsCompareHashAggr)
            {
                ...
                if (LOG.isDebugEnabled())
                {
                    LOG.debug(String.format("checkHashModeEfficiency: HT:%d RC:%d MIN:%d", numEntriesHashTable, sumBatchSize, (long) (sumBatchSize * minReductionHashAggr)));
                }
                /**
    ...
    final class ProcessingModeStreaming extends ProcessingModeBase
    {
         ...
         */
        private final VectorAggregationBufferRow[] rowsToFlush = new VectorAggregationBufferRow[VectorizedRowBatch.DEFAULT_SIZE];
    ...
    public VectorGroupByOperator(CompilationOpContext ctx, OperatorDesc conf, VectorizationContext vContext, VectorDesc vectorDesc) throws HiveException
    {
        this(ctx);
        ...
        outputKeyLength = this.conf.pruneGroupingSetId() ? keyExpressions.length - 1 : keyExpressions.length;
        ...
        for (int i = 0; i < aggregationCount; i++)
        {
            ...
            outputDataTypePhysicalVariations[i + outputKeyLength] = vecAggrDesc.getOutputDataTypePhysicalVariation();
        }
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        try
        {
            ...
            vrbCtx = new VectorizedRowBatchCtx(outputFieldNames.toArray(new String[0]), outputTypeInfos, outputDataTypePhysicalVariations,
                    /* dataColumnNums */ null,
                    /* partitionColumnCount */ 0,
                    /* virtualColumnCount */ 0,
                    /* neededVirtualColumns */ null, vOutContext.getScratchColumnTypeNames(), vOutContext.getScratchDataTypePhysicalVariations());
    ...
    VectorAggregateExpression instantiateExpression(VectorAggregationDesc vecAggrDesc) throws HiveException
    {
        ...
        final List<ConstantVectorExpression> constants = vecAggrDesc.getConstants() == null ? Collections.emptyList() : vecAggrDesc.getConstants();
        ...
        for (int i = 0; i < constants.size(); ++i)
        {
            ...
            PrimitiveObjectInspectorUtils.PrimitiveTypeEntry primitiveTypeEntry = PrimitiveObjectInspectorUtils.getTypeEntryFromTypeName(typeName);
            if (primitiveTypeEntry == null)
            {
                throw new IllegalArgumentException("Non-primitive type detected as " + i + "-th argument for a call to the vectorized aggregation class " + vecAggrClass.getSimpleName() + ", only primitive types are supported");
            }
        ...
        {
            throw new HiveException("Constructor " + vecAggrClass.getSimpleName() + "(" + Arrays.toString(ctorParamClasses) + ") not available", e);
        }
        try
        {
        ...
        {
            throw new HiveException("Failed to create " + vecAggrClass.getSimpleName() + "(" + Arrays.toString(ctorParamClasses) + ") object ", e);
        }
    }
     ...
     * @throws HiveException
     */
    private void writeSingleRow(VectorHashKeyWrapperBase kw, VectorAggregationBufferRow agg) throws HiveException
    {
        ...
        for (int i = 0; i < aggregators.length; ++i)
        {
            aggregators[i].assignRowColumn(outputBatch, batchIndex, colNum++, agg.getAggregationBuffer(i));
        }
        ++outputBatch.size;
