...
public class QueryPlan implements Serializable
{
     ...
     */
    public Integer getStatementIdForAcidWriteType(long writeId, String moveTaskId, AcidUtils.Operation acidOperation, Path path, int originalStatementId)
    {
        FileSinkDesc result = null;
        for (FileSinkDesc acidSink : acidSinks)
        {
            if (acidOperation.equals(acidSink.getAcidOperation()) && path.equals(acidSink.getDestPath()) && acidSink.getTableWriteId() == writeId && (moveTaskId == null || acidSink.getMoveTaskId() == null || moveTaskId.equals(acidSink.getMoveTaskId())))
            {
                if (result != null)
                {
                    return -1;
    ...
    public static String makeQueryId()
    {
        ...
        return userid + "_" + String.format("%1$4d%2$02d%3$02d%4$02d%5$02d%6$02d", gc.get(Calendar.YEAR), gc.get(Calendar.MONTH) + 1, gc.get(Calendar.DAY_OF_MONTH), gc.get(Calendar.HOUR_OF_DAY), gc.get(Calendar.MINUTE), gc.get(Calendar.SECOND)) + "_" + UUID.randomUUID().toString();
    }
     ...
     *               task is hanging
     */
    private void populateOperatorGraph(org.apache.hadoop.hive.ql.plan.api.Task task, Collection<Operator<? extends OperatorDesc>> topOps)
    {
