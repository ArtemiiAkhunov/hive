...
public class MurmurHashStringColStringCol extends MurmurHashExpression
{
    ...
    protected void hash(ColumnVector inputColVector1, ColumnVector inputColVector2, LongColumnVector outputColVector, int i, ByteBuffer byteBuffer)
    {
        ...
        int hash = inV1.isNull[i] ? 0 : Murmur3.hash32(inV1.vector[i], inV1.start[i], inV1.length[i], Murmur3.DEFAULT_SEED);
        // hash of value from 2. column
        int hash2 = inV2.isNull[i] ? 0 : Murmur3.hash32(inV2.vector[i], inV2.start[i], inV2.length[i], Murmur3.DEFAULT_SEED);
    ...
    {
        builder.setArgumentTypes(VectorExpressionDescriptor.ArgumentType.STRING_FAMILY, VectorExpressionDescriptor.ArgumentType.STRING_FAMILY);
    }
}
