...
public class CustomPartitionVertex extends VertexManagerPlugin
{
    ...
    private final Map<String, Multimap<Integer, InputSplit>> inputToGroupedSplitMap = new HashMap<>();
    ...
    private final Map<String, InputSpecUpdate> inputNameInputSpecMap = new HashMap<>();
    ...
    public void onRootVertexInitialized(String inputName, InputDescriptor inputDescriptor, List<Event> events)
    {
        ...
        Preconditions.checkState(bucketToInitialSplitMap.keySet().stream().allMatch(i -> 0 <= i && i < numBuckets));
    ...
    private Multimap<Integer, InputSplit> getBucketSplitMapForPath(String inputName, Map<String, Set<FileSplit>> pathFileSplitsMap)
    {
        ...
        Preconditions.checkState(isMainWork || isSMBJoin && inputToBucketMap != null && inputToBucketMap.containsKey(inputName), "CustomPartitionVertex.inputToBucketMap is not defined for {}", inputName);
