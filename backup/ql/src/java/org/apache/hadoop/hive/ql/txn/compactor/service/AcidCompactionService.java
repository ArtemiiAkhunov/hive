...
public class AcidCompactionService extends CompactionService
{
    ...
    private AcidDirectory getAcidStateForWorker(CompactionInfo ci, StorageDescriptor sd, ValidCompactorWriteIdList tblValidWriteIds) throws IOException, InterruptedException
    {
        ...
        {
            return AcidUtils.getAcidState(null, new Path(sd.getLocation()), conf, tblValidWriteIds, Ref.from(false), true);
        }

        UserGroupInformation ugi = UserGroupInformation.createProxyUser(ci.runAs, UserGroupInformation.getLoginUser());
        try
        {
            return ugi.doAs((PrivilegedExceptionAction<AcidDirectory>) () -> AcidUtils.getAcidState(null, new Path(sd.getLocation()), conf, tblValidWriteIds, Ref.from(false), true));
        } finally
        {
            try
            {
    ...
    public void cleanupResultDirs(CompactionInfo ci)
    {
        ...
        Path resultDir = QueryCompactor.Util.getCompactionResultDir(sd, tblValidWriteIds, conf, ci.type == CompactionType.MAJOR, false, false, dir);
        ...
        try
        {
            ...
            {
                Path deleteDeltaDir = QueryCompactor.Util.getCompactionResultDir(sd, tblValidWriteIds, conf, false, true, false, dir);
    ...
    public Boolean compact(Table table, CompactionInfo ci) throws Exception
    {
        ...
        try (CompactionTxn compactionTxn = new CompactionTxn())
        {
            ...
            if (!ci.type.equals(CompactionType.REBALANCE) && ci.numberOfBuckets > 0)
            {
                ...
                {
                    LOG.warn("Only the REBALANCE compaction accepts the number of buckets clause (CLUSTERED INTO {N} BUCKETS). " + "Since the compaction request is {}, it will be ignored.", ci.type);
                }
            }
            ...
            try
            {
                p = CompactorUtil.resolvePartition(conf, msc, ci.dbname, ci.tableName, ci.partName, CompactorUtil.METADATA_FETCH_MODE.REMOTE);
            ...
            tblValidWriteIds = TxnUtils.createValidCompactWriteIdList(msc.getValidWriteIds(Collections.singletonList(fullTableName), validTxnList.writeToString()).get(0));
    ...
    static boolean isEnoughToCompact(CompactionInfo ci, AcidDirectory dir, StorageDescriptor sd)
    {
        ...
        } else
        {
            ...
            if (deltaCount == 2)
            {
                Map<String, Long> deltaByType = dir.getCurrentDirectories().stream().collect(Collectors.groupingBy(delta -> (delta.isDeleteDelta() ? AcidUtils.DELETE_DELTA_PREFIX : AcidUtils.DELTA_PREFIX), Collectors.counting()));
        ...
        {
            LOG.info("Not enough files in {} to compact; current base: {}, delta files: {}, originals: {}", sd.getLocation(), dir.getBaseDirectory(), deltaInfo, origCount);
        }
    ...
    public static boolean needsCleaning(AcidDirectory dir, StorageDescriptor sd)
    {
        ...
        {
            LOG.info("{} obsolete directories in {} found; marked for cleaning.", numObsoleteDirs, sd.getLocation());
        }
    ...
    class CompactionTxn implements AutoCloseable
    {
        ...
        void open(CompactionInfo ci) throws TException
        {
            ...
            {
                throw new TException("Unable to acquire lock(s) on {" + ci.getFullPartitionName() + "}, status {" + res.getState() + "}, reason {" + res.getErrorMessage() + "}");
            }
