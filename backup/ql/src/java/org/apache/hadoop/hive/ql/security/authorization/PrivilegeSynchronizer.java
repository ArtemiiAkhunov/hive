...
public class PrivilegeSynchronizer implements Runnable
{
    ...
    public PrivilegeSynchronizer(LeaderLatch privilegeSynchronizerLatch, PolicyProviderContainer policyProviderContainer, HiveConf hiveConf)
    {
    ...
    private void addACLsToBag(Map<String, Map<HiveResourceACLs.Privilege, HiveResourceACLs.AccessResult>> principalAclsMap, PrivilegeBag privBag, HiveObjectType objectType, String dbName, String tblName, String columnName, PrincipalType principalType, String authorizer)
    {
        ...
        for (Map.Entry<String, Map<HiveResourceACLs.Privilege, HiveResourceACLs.AccessResult>> principalAcls : principalAclsMap.entrySet())
        {
            ...
            for (Map.Entry<HiveResourceACLs.Privilege, HiveResourceACLs.AccessResult> acl : principalAcls.getValue().entrySet())
            {
                if (acl.getValue() == HiveResourceACLs.AccessResult.ALLOWED)
                {
                    ...
                    {
                        case DATABASE:
                            privBag.addToPrivileges(new HiveObjectPrivilege(new HiveObjectRef(HiveObjectType.DATABASE, dbName, null, null, null), principal, principalType, new PrivilegeGrantInfo(acl.getKey().toString(), (int) (System.currentTimeMillis() / 1000), GRANTOR, PrincipalType.USER, false), authorizer));
                            break;
                        case TABLE:
                            privBag.addToPrivileges(new HiveObjectPrivilege(new HiveObjectRef(HiveObjectType.TABLE, dbName, tblName, null, null), principal, principalType, new PrivilegeGrantInfo(acl.getKey().toString(), (int) (System.currentTimeMillis() / 1000), GRANTOR, PrincipalType.USER, false), authorizer));
                            break;
                        case COLUMN:
            ...
            if (columnUpdateFlag)
            {
                ...
                privBag.addToPrivileges(new HiveObjectPrivilege(new HiveObjectRef(HiveObjectType.COLUMN, dbName, tblName, null, columnName), principal, principalType, new PrivilegeGrantInfo(columnPrivilegeBitsString, (int) (System.currentTimeMillis() / 1000), GRANTOR, PrincipalType.USER, false), authorizer));
            ...
            try
            {
                for (HivePolicyProvider policyProvider : policyProviderContainer)
                {
                    ...
                    for (String dcName : hiveClient.getAllDataConnectorNames())
                    {
                        ...
                        addGrantPrivilegesToBag(policyProvider, grantDataConnectorBag, HiveObjectType.DATACONNECTOR, null, dcName, null, authorizer);
                    ...
                    for (String dbName : hiveClient.getAllDatabases())
                    {
                        ...
                        addGrantPrivilegesToBag(policyProvider, grantDatabaseBag, HiveObjectType.DATABASE, dbName, null, null, authorizer);
                        ...
                        for (String tblName : hiveClient.getAllTables(dbName))
                        {
                            ...
                            addGrantPrivilegesToBag(policyProvider, grantTableBag, HiveObjectType.TABLE, dbName, tblName, null, authorizer);
                            ...
                            try
                            {
                                ...
                                {
                                    addGrantPrivilegesToBag(policyProvider, grantColumnBag, HiveObjectType.COLUMN, dbName, tblName, fs.getName(), authorizer);
                                }
                    ...
                    LOG.info("Success synchronize privilege " + policyProvider.getClass().getName() + ":" + numDb + " databases, " + numTbl + " tables");
                }
            } catch (Exception e)
            {
