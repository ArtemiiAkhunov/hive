...
public class ReplLoadTask extends Task<ReplLoadWork> implements Serializable
{
    ...
    public int execute()
    {
        ...
        } catch (RuntimeException e)
        {
            ...
            try
            {
                ReplUtils.handleException(true, e, new Path(work.getDumpDirectory()).getParent().toString(), work.getMetricCollector(), STAGE_NAME, conf);
            } catch (Exception ex)
            {
        ...
        } catch (Exception e)
        {
            ...
            try
            {
                return ReplUtils.handleException(true, e, new Path(work.getDumpDirectory()).getParent().toString(), work.getMetricCollector(), STAGE_NAME, conf);
            } catch (Exception ex)
            {
                LOG.error("Failed to collect replication metrics: ", ex);
                return errorCode;
    ...
    private void initiateRangerDenyTask() throws SemanticException
    {
        if (RANGER_AUTHORIZER.equalsIgnoreCase(conf.getVar(HiveConf.ConfVars.REPL_AUTHORIZATION_PROVIDER_SERVICE)))
        {
            ...
            RangerDenyWork rangerDenyWork = new RangerDenyWork(new Path(work.getDumpDirectory()), work.getSourceDbName(), work.dbNameToLoadIn, work.getMetricCollector());
            if (childTasks == null)
            {
        ...
        {
            throw new SemanticException(ErrorMsg.REPL_INVALID_CONFIG_FOR_SERVICE.format("Authorizer: " + conf.getVar(HiveConf.ConfVars.REPL_AUTHORIZATION_PROVIDER_SERVICE) + " not supported for deny policy creation. Currently Only supports: ", ReplUtils.REPL_RANGER_SERVICE));
        }
    }
    ...
    private void initiateAuthorizationLoadTask() throws SemanticException
    {
        if (RANGER_AUTHORIZER.equalsIgnoreCase(conf.getVar(HiveConf.ConfVars.REPL_AUTHORIZATION_PROVIDER_SERVICE)))
        {
            ...
            RangerLoadWork rangerLoadWork = new RangerLoadWork(rangerLoadRoot, work.getSourceDbName(), targetDbName, work.getMetricCollector());
    ...
    private void addAtlasLoadTask() throws HiveException
    {
        ...
        String targetDbName = StringUtils.isEmpty(work.dbNameToLoadIn) ? work.getSourceDbName() : work.dbNameToLoadIn;
        AtlasLoadWork atlasLoadWork = new AtlasLoadWork(work.getSourceDbName(), targetDbName, atlasDumpDir, work.getMetricCollector());
    ...
    */
     ...
     */
       ...
       */
         ...
         */
                        ...
                        LoadPartitions loadPartitions = new LoadPartitions(loadContext, iterator.replLogger(), loadTaskTracker, tableEvent, work.dbNameToLoadIn, tableContext, work.getMetricCollector(), work.tablesToBootstrap);
     ...
     */
        ...
        if (conf.getBoolVar(REPL_SNAPSHOT_DIFF_FOR_EXTERNAL_TABLE_COPY))
        {
            ...
            try
            {
                SnapshotUtils.getDFS(snapPath, conf).rename(new Path(snapPath, EximUtil.FILE_LIST_EXTERNAL_SNAPSHOT_CURRENT), new Path(snapPath, EximUtil.FILE_LIST_EXTERNAL_SNAPSHOT_OLD), Options.Rename.OVERWRITE);
    ...
    {
        return conf.get(HiveConf.ConfVars.PRE_EXEC_HOOKS.varname) != null && conf.get(HiveConf.ConfVars.PRE_EXEC_HOOKS.varname).contains(READ_ONLY_HOOK);
    }
    ...
    private void setDbReadOnly()
    {
        ...
        AlterDatabaseSetPropertiesDesc setTargetReadOnly = new AlterDatabaseSetPropertiesDesc(work.dbNameToLoadIn, props, null);
        DDLWork alterDbPropWork = new DDLWork(new HashSet<>(), new HashSet<>(), setTargetReadOnly, true, work.dumpDirectory, work.getMetricCollector());
    ...
    private TaskTracker addLoadPartitionTasks(Context loadContext, BootstrapEvent next, TaskTracker dbTracker, BootstrapEventsIterator iterator, Scope scope, TaskTracker loadTaskTracker, TaskTracker tableTracker) throws Exception
    {
        ...
        TableContext tableContext = new TableContext(dbTracker, work.dbNameToLoadIn);
        LoadPartitions loadPartitions = new LoadPartitions(loadContext, iterator.replLogger(), tableContext, loadTaskTracker, event.asTableEvent(), work.dbNameToLoadIn, event.lastPartitionReplicated(), work.getMetricCollector(), event.lastPartSpecReplicated(), event.lastStageReplicated(), getWork().tablesToBootstrap);
        /*
    ...
    private void dropTablesExcludedInReplScope(ReplScope replScope) throws HiveException
    {
        ...
        Iterable<String> tableNames = Collections2.filter(db.getAllTables(dbName), tableName ->
        {
            assert (tableName != null);
            return !tableName.toLowerCase().startsWith(SemanticAnalyzer.VALUES_TMP_TABLE_NAME_PREFIX.toLowerCase()) && !replScope.tableIncludedInReplScope(tableName);
        });
   ...
   */
    ...
    private Database getSourceDbMetadata() throws IOException, SemanticException
    {
        ...
        BootstrapEventsIterator itr = new BootstrapEventsIterator(dbMetadata.toString(), work.dbNameToLoadIn, true, conf, work.getMetricCollector());
        if (!itr.hasNext())
        {
