...
public class PartitionIterable implements Iterable<Partition>
{
    ...
    private PartitionIterable(Hive db, Table table, Map<String, String> partialPartitionSpec, int batchSize, boolean getColStats, boolean isAuthRequired, String userName, List<String> groupNames) throws HiveException
    {
        ...
        {
            partitionNames = db.getPartitionNames(table.getDbName(), table.getTableName(), (short) -1);
        } else
        {
            partitionNames = db.getPartitionNames(table.getDbName(), table.getTableName(), partialPartitionSpec, (short) -1);
        }
    }
