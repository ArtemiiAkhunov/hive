 ...
 */
@Description(name = "exception_in_vertex_udaf", value = "_FUNC_(vertexName, taskNumberExpression, taskAttemptNumberExpression)" + "Throws exception in Reducer tasks, where UDF running is possible", extended = " please refer to full examples in exception_in_vertex_udf.q: " + "exception_in_vertex_udaf ('Reducer 2', 0, 0)          -> Reducer2, first task, first attempt" + "exception_in_vertex_udaf ('Reducer 2', '0,1,2', '*')  -> Reducer2, tasks: 0,1,2, all attempts" + "exception_in_vertex_udaf ('Reducer 2', '*', 0)        -> Reducer2, all tasks, first attempt" + "exception_in_vertex_udaf ('Reducer 2', '0-2', '*')    -> Reducer2, tasks: 0,1,2, all attempts" + "exception_in_vertex_udaf ('Reducer 2', '*', '*')      -> Reducer2, all tasks, all attempts" + "exception_in_vertex_udaf ('Reducer 2', '*')           -> Reducer2, all tasks, all attempts" + "exception_in_vertex_udaf ('Reducer 2')                -> Reducer2, all tasks, all attempts")
public class GenericUDAFExceptionInVertex implements GenericUDAFResolver2
{
    ...
    public static class GenericUDAFExceptionInVertexEvaluator extends GenericUDAFEvaluator implements Serializable
    {
        ...
        private void checkAndThrowException(String phase) throws HiveException
        {
            ...
            LOG.debug("{}: vertex {}, task: {}, attempt: {} <-> vertex {}, task: {}, attempt: {}", phase, currentVertexName, currentTaskNumber, currentTaskAttemptNumber, vertexName, taskNumberExpr, taskAttemptNumberExpr);
            ...
            {
                String message = String.format("GenericUDAFExceptionInVertex: found condition for throwing exception (vertex/task/attempt):" + "current %s / %d / %d matches criteria %s / %s / %s", currentVertexName, currentTaskNumber, currentTaskAttemptNumber, vertexName, taskNumberExpr, taskAttemptNumberExpr);
                LOG.info(message);
