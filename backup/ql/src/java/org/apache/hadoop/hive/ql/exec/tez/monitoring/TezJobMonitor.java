...
public class TezJobMonitor
{
    ...
    public TezJobMonitor(List<BaseWork> topSortedWorks, final DAGClient dagClient, HiveConf conf, DAG dag, Context ctx, final TezCounters counters, PerfLogger perfLogger)
    {
    ...
    {
        return InPlaceUpdate.canRenderInPlace(hiveConf) && !SessionState.getConsole().getIsSilent() && !SessionState.get().isHiveServerQuery() ? new RenderStrategy.InPlaceUpdateFunction(this, perfLogger) : new RenderStrategy.LogToFileFunction(this, perfLogger);
    }

    private boolean isProfilingEnabled()
    {
        return HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.TEZ_EXEC_SUMMARY) || Utilities.isPerfOrAboveLogging(hiveConf);
    }
    ...
    private void aggregateCountersSum(final Map<String, Long> updatedCounters, final List<String> vertexNames, final String prefixRemovedCounterName, final String desiredCounter, final TezCounter tezCounter)
    {
    ...
    private void aggregateCountersMax(final Map<String, Long> updatedCounters, final List<String> vertexNames, final String prefixRemovedCounterName, final String desiredCounter, final TezCounter tezCounter)
    {
        long counterValue = checkVertexSuffixAndGetValue(vertexNames, prefixRemovedCounterName, tezCounter);
        long currentMax = 0;
        if (updatedCounters.containsKey(desiredCounter))
    ...
    private long checkVertexSuffixAndGetValue(final List<String> vertexNames, final String counterName, final TezCounter tezCounter)
    {
    ...
    ProgressMonitor progressMonitor(DAGStatus status, Map<String, Progress> progressMap)
    {
        try
        {
            return new TezProgressMonitor(dagClient, status, topSortedWorks, progressMap, console, executionStartTime);
        } catch (IOException | TezException e)
        {
            console.printInfo("Getting  Progress Information: " + e.getMessage() + " stack trace: " + ExceptionUtils.getStackTrace(e));
        }
