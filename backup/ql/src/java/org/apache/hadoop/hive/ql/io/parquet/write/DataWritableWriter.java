...
public class DataWritableWriter
{
    ...
    public DataWritableWriter(final RecordConsumer recordConsumer, final GroupType schema, final boolean defaultDateProleptic)
    {
    ...
    public DataWritableWriter(final RecordConsumer recordConsumer, final GroupType schema, final boolean defaultDateProleptic, final Configuration conf)
    {
        this.recordConsumer = recordConsumer;
        this.schema = schema;
        this.defaultDateProleptic = defaultDateProleptic;
        this.conf = conf;
        this.isLegacyZoneConversion = HiveConf.getBoolVar(this.conf, HiveConf.ConfVars.HIVE_PARQUET_TIMESTAMP_WRITE_LEGACY_CONVERSION_ENABLED);
    }
    ...
    private void checkInspectorCategory(ObjectInspector inspector, ObjectInspector.Category category)
    {
        ...
        {
            throw new IllegalArgumentException("Invalid data type: expected " + category + " type, but found: " + inspector.getCategory());
        }
    }
    ...
    private class DateDataWriter implements DataWriter
    {
        ...
        public void write(Object value)
        {
            ...
            recordConsumer.addInteger(defaultDateProleptic ? DateWritableV2.dateToDays(vDate) : CalendarUtils.convertDateToHybrid(DateWritableV2.dateToDays(vDate)));
        }
    }
}
