 ...
 * closed.
 */
@SerDeSpec(schemaProps = {serdeConstants.LIST_COLUMNS, serdeConstants.LIST_COLUMN_TYPES, ParquetOutputFormat.COMPRESSION})
public class ParquetHiveSerDe extends AbstractSerDe implements SchemaInference
{
    ...
    static
    {
        for (int prec = 1; prec <= 38; prec++)
        {
            ...
            PRECISION_TO_BYTE_COUNT[prec - 1] = (int) Math.ceil((Math.log(Math.pow(10, prec) - 1) / Math.log(2) + 1) / 8);
        }
    }
    ...
    public void initialize(Configuration configuration, Properties tableProperties, Properties partitionProperties) throws SerDeException
    {
        ...
        StructTypeInfo completeTypeInfo = (StructTypeInfo) TypeInfoFactory.getStructTypeInfo(getColumnNames(), getColumnTypes());
    ...
    {
        Preconditions.checkArgument(prunedInfo != null, "PrunedStructTypeInfo for path '" + path + "' should not be null");
    ...
    private static class PrunedStructTypeInfo
    {
        ...
        PrunedStructTypeInfo(StructTypeInfo typeInfo)
        {
            ...
            for (int i = 0; i < typeInfo.getAllStructFieldTypeInfos().size(); ++i)
            {
                ...
                {
                    this.children.put(typeInfo.getAllStructFieldNames().get(i).toLowerCase(), new PrunedStructTypeInfo((StructTypeInfo) ti));
                }
            }
        }
    ...
    private String convertParquetMapLogicalTypeAnnotation(Type parquetType, boolean inferBinaryAsString) throws SerDeException
    {
        ...
        return createMapType(convertParquetTypeToFieldType(key, inferBinaryAsString), convertParquetTypeToFieldType(value, inferBinaryAsString));
    }
