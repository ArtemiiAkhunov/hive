...
public class SnapshotUtils
{
    ...
    public static boolean deleteSnapshotIfExists(DistributedFileSystem fs, Path snapshotPath, String snapshotName, HiveConf conf) throws IOException
    {
        ...
        try
        {
            retryable.executeCallable(() ->
            {
                ...
                {
                    if (e.getMessage().contains("the snapshot does not exist") || e.getMessage().contains("Directory is not a snapshottable directory"))
                    {
                        return true;
                    }
                }
        ...
        {
            throw new SnapshotException("Unable to delete snapshot for path: " + snapshotPath + " snapshot name: " + snapshotName, e);
        }
        return true;
    }
    ...
    public static void createSnapshot(FileSystem fs, Path snapshotPath, String snapshotName, HiveConf conf) throws IOException
    {
        ...
        {
            throw new SnapshotException("Unable to create snapshot for path: " + snapshotPath + " snapshot name: " + snapshotName, e);
        }
    }
    ...
    public static void renameSnapshot(FileSystem fs, Path snapshotPath, String sourceSnapshotName, String targetSnapshotName, HiveConf conf) throws IOException
    {
        ...
        try
        {
            retryable.executeCallable(() ->
            {
                ...
                {
                    LOG.warn("Couldn't rename the snapshot {} to {} under path {}", sourceSnapshotName, targetSnapshotName, snapshotPath, e);
                }
                return null;
            });
        } catch (Exception e)
        {
            throw new SnapshotException("Unable to rename snapshot " + sourceSnapshotName + " to " + targetSnapshotName + " for path: " + snapshotPath, e);
        }
    }
     ...
     * @throws IOException in case of any error.
     */
    private static void cleanUpSnapshots(ArrayList<String> diffList, String prefix, ReplSnapshotCount snapshotCount, HiveConf conf) throws IOException
    {
        DistributedFileSystem dfs = diffList.size() > 0 ? (DistributedFileSystem) new Path(diffList.get(0)).getFileSystem(conf) : null;
    ...
    public static void cleanupSnapshots(Path dumpRoot, String snapshotPrefix, HiveConf conf, ReplSnapshotCount snapshotCount, boolean isLoad) throws IOException, SemanticException
    {
        ...
        if (isLoad)
        {
            ...
            try
            {
                dumpDfs.rename(new Path(dumpRoot, EximUtil.FILE_LIST_EXTERNAL_SNAPSHOT_CURRENT), new Path(dumpRoot, EximUtil.FILE_LIST_EXTERNAL_SNAPSHOT_OLD), Options.Rename.OVERWRITE);
            } catch (FileNotFoundException fnf)
            {
                // ignore
