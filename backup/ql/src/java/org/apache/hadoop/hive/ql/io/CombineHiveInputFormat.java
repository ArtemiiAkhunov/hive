...
public class CombineHiveInputFormat<K extends WritableComparable, V extends Writable> extends HiveInputFormat<K, V>
{
    ...
    private class CheckNonCombinablePathCallable implements Callable<Set<Integer>>
    {
        ...
        public CheckNonCombinablePathCallable(Path[] paths, int start, int length, JobConf conf, boolean isMerge)
        {
            this.paths = paths;
            this.start = start;
            this.length = length;
        ...
        public Set<Integer> call() throws Exception
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    Utilities.FILE_OP_LOGGER.info("The path [" + paths[i + start] + "] is being parked for HiveInputFormat.getSplits");
    ...
    public static class CombineHiveInputSplit extends InputSplitShim
    {
        ...
        public CombineHiveInputSplit(JobConf job, CombineFileSplit inputSplitShim, Map<Path, PartitionDesc> pathToPartitionInfo) throws IOException
        {
            ...
            if (job != null)
            {
                ...
                if (ipaths.length > 0)
                {
                    PartitionDesc part = HiveFileFormatUtils.getFromPathRecursively(this.pathToPartitionInfo, ipaths[0], IOPrepareCache.get().getPartitionDescMap());
        ...
        public void write(DataOutput out) throws IOException
        {
            ...
            if (inputFormatClassName == null)
            {
                ...
                // extract all the inputFormatClass names for each chunk in the
                // CombinedSplit.
                PartitionDesc part = HiveFileFormatUtils.getFromPathRecursively(pathToPartitionInfo, inputSplitShim.getPath(0), IOPrepareCache.get().getPartitionDescMap());
    ...
    private static class CombinePathInputFormat
    {
        ...
        public boolean equals(Object o)
        {
            if (o instanceof CombinePathInputFormat)
            {
                ...
                return (opList.equals(mObj.opList)) && (inputFormatClassName.equals(mObj.inputFormatClassName)) && (deserializerClassName == null ? (mObj.deserializerClassName == null) : deserializerClassName.equals(mObj.deserializerClassName));
            }
            return false;
        }
    ...
    public Set<Integer> getNonCombinablePathIndices(JobConf job, Path[] paths, int numThreads) throws ExecutionException, InterruptedException
    {
        ...
        try
        {
            ...
            for (int i = 0; i < numThreads; i++)
            {
                ...
                futureList.add(executor.submit(new CheckNonCombinablePathCallable(paths, start, length, job, isMerge)));
            }
            Set<Integer> nonCombinablePathIndices = new HashSet<Integer>();
    ...
    private void processPaths(JobConf job, CombineFileInputFormatShim combine, List<CombineFileSplit> iss, Path... path) throws IOException
    {
    ...
    private List<CombineFileSplit> sampleSplits(List<CombineFileSplit> splits)
    {
        ...
        for (CombineFileSplit split : splits)
        {
            ...
            for (Path path : split.getPaths())
            {
                ...
                if (l.size() != 1 || !nameToSamples.containsKey(l.get(0)) || (alias != null && !alias.equals(l.get(0))))
                {
                    alias = null;
                    break;
                }
