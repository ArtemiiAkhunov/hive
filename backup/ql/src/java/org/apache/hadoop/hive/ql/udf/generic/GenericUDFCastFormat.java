...
public class GenericUDFCastFormat extends GenericUDF implements Serializable
{
    ...
    @VisibleForTesting
    static final Map<Integer, String> OUTPUT_TYPES = ImmutableMap.<Integer, String>builder().put(HiveParser.TOK_STRING, serdeConstants.STRING_TYPE_NAME).put(HiveParser.TOK_VARCHAR, serdeConstants.VARCHAR_TYPE_NAME).put(HiveParser.TOK_CHAR, serdeConstants.CHAR_TYPE_NAME).put(HiveParser.TOK_TIMESTAMP, serdeConstants.TIMESTAMP_TYPE_NAME).put(HiveParser.TOK_DATE, serdeConstants.DATE_TYPE_NAME).build();
    ...
    /**
     * @param arguments 0. const int, value of a HiveParser_IdentifiersParser constant which represents a TOK_[TYPE]
