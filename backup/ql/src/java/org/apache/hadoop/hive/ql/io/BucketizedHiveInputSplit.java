...
public class BucketizedHiveInputSplit extends HiveInputSplit
{
    ...
    public BucketizedHiveInputSplit(InputSplit[] inputSplits, String inputFormatClassName)
    {
        // This is the only public constructor of FileSplit
        super();
    ...
    public Path getPath()
    {
        if (inputSplits != null && inputSplits.length > 0 && inputSplits[0] instanceof FileSplit)
        {
    ...
    public void readFields(DataInput in) throws IOException
    {
        ...
        for (int i = 0; i < numSplits; i++)
        {
            try
            {
                inputSplits[i] = (InputSplit) ReflectionUtil.newInstance(conf.getClassByName(inputSplitClassName), conf);
            } catch (Exception e)
            {
