...
public class PerPoolTriggerValidatorRunnable implements Runnable
{
    ...
    PerPoolTriggerValidatorRunnable(final Map<String, SessionTriggerProvider> sessionTriggerProviders, final TriggerActionHandler triggerActionHandler, final long triggerValidationIntervalMs)
    {
    ...
    public void run()
    {
        try
        {
            ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(true).setNameFormat("PoolValidator %d").build();
            ScheduledExecutorService validatorExecutorService = Executors.newScheduledThreadPool(sessionTriggerProviders.size(), threadFactory);
