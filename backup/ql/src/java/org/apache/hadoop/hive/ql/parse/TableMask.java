        ...
        if (exprs != null)
        {
            ...
            for (int index = 0; index < exprs.size(); index++)
            {
                ...
                if (!expr.equals(colName))
                {
                    if (colType.contains("<"))
                    {
                        throw new SemanticException(ErrorMsg.MASKING_COMPLEX_TYPE_NOT_SUPPORTED, expr, colName, colType);
                    }
                    // CAST(expr AS COLTYPE) AS COLNAME
                    sb.append("CAST(" + expr + " AS " + colType + ") AS " + HiveUtils.unparseIdentifier(colName, conf));
