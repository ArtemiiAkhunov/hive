...
public abstract class AbstractFunctionAnalyzer extends BaseSemanticAnalyzer
{
    ...
    protected void addEntities(String functionName, String className, boolean isTemporary, List<ResourceUri> resources) throws SemanticException
    {
        ...
        if (database != null)
        {
            ...
            {
                function = new Function(functionName, database.getName(), className, SessionState.getUserFromAuthenticator(), PrincipalType.USER, (int) (System.currentTimeMillis() / 1000), FunctionType.JAVA, resources);
            } else
            {
                try
                {
