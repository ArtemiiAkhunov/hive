 ...
 */
public class VectorSMBMapJoinOperator extends SMBMapJoinOperator implements VectorizationOperator, VectorizationContextRegion
{
    ...
    protected List<Object> smbJoinComputeKeys(Object row, byte alias) throws HiveException
    {
        if (alias == this.posBigTable)
        {
            ...
            {
                result.add(ObjectInspectorUtils.copyToStandardObject(singletonListAndObjects.get(i), joinKeysObjectInspectors[alias].get(i), ObjectInspectorCopyOption.WRITABLE));
            }
            return result;
        } else
        {
