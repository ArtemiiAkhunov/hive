...
public class HiveInBetweenExpandRule
{
    ...
    private static class FilterRule extends RelOptRule
    {
        ...
        FilterRule()
        {
            super(operand(Filter.class, any()), HiveRelFactories.HIVE_BUILDER, "HiveInBetweenExpandRule(FilterRule)");
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            RelNode newFilter = filter.copy(filter.getTraitSet(), filter.getInput(), condition);
    ...
    private static class JoinRule extends RelOptRule
    {
        ...
        JoinRule()
        {
            super(operand(Join.class, any()), HiveRelFactories.HIVE_BUILDER, "HiveInBetweenExpandRule(JoinRule)");
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            RelNode newJoin = join.copy(join.getTraitSet(), condition, join.getLeft(), join.getRight(), join.getJoinType(), join.isSemiJoinDone());
    ...
    private static class ProjectRule extends RelOptRule
    {
        ...
        ProjectRule()
        {
            super(operand(Project.class, any()), HiveRelFactories.HIVE_BUILDER, "HiveInBetweenExpandRule(ProjectRule)");
        }
    ...
    private static final class RexInBetweenExpander extends RexShuttle
    {
        ...
        public RexNode visitCall(final RexCall call)
        {
            switch (call.getKind())
            {
                ...
                case BETWEEN:
                {
                    List<RexNode> newOperands = RexNodeConverter.rewriteBetweenChildren(call.getOperands(), rexBuilder);
                    modified = true;
