...
public class VectorMapJoinFastLongHashMap extends VectorMapJoinFastLongHashTable implements VectorMapJoinLongHashMap, MemoryEstimate
{
        ...
        public boolean findNextNonMatched()
        {
            ...
            while (true)
            {
                ...
                if (hashMap.slotPairs[nonMatchedDoubleIndex] != 0)
                {
                    ...
                    {
                        nonMatchedHashMapResult.set(hashMap.valueStore, hashMap.slotPairs[nonMatchedDoubleIndex]);
                        keyIsNull = false;
                        return true;
                    }
                }
            ...
            nonMatchedHashMapResult.set(hashMap.valueStore, hashMap.fullOuterNullKeyValueRef);
            noMore = true;
            keyIsNull = true;
            return true;
        }
    ...
    public void addFullOuterNullKeyValue(BytesWritable currentValue)
    {
        ...
        } else
        {
            ...
            // Add another value.
            fullOuterNullKeyValueRef = valueStore.addMore(fullOuterNullKeyValueRef, valueBytes, 0, valueLength);
        }
    }

    public VectorMapJoinFastLongHashMap(boolean isFullOuter, boolean minMaxEnabled, HashTableKeyType hashTableKeyType, int initialCapacity, float loadFactor, int writeBuffersSize, long estimatedKeyCount, TableDesc tableDesc)
    {
