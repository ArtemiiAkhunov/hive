...
public class DynamicPartitionPruningOptimization implements SemanticNodeProcessor
{
    ...
    static final private Logger LOG = LoggerFactory.getLogger(DynamicPartitionPruningOptimization.class.getName());
    ...
    public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx, Object... nodeOutputs) throws SemanticException
    {
        ...
        {
            throw new IllegalArgumentException("expected parseContext " + "OptimizeTezProcContext, but found " + procCtx.getClass().getName());
        }
    ...
    private boolean disableSemiJoinOptDueToExternalTable(HiveConf conf, TableScanOperator ts, DynamicListContext ctx)
    {
        ...
        if (conf.getBoolVar(HiveConf.ConfVars.HIVE_DISABLE_UNSAFE_EXTERNALTABLE_OPERATIONS))
        {
            ...
            {
                LOG.debug("Disabling semijoin optimzation on {} since it is an external table and also could not provide statistics.", ts.getConf().getTableMetadata().getFullyQualifiedName());
            ...
            } else
            {
                ...
                if (colExpr != null)
                {
                    ...
                    if (columnOrigin != null && columnOrigin.op instanceof TableScanOperator)
                    {
                        ...
                        {
                            LOG.debug("Join key {} is from {} which is an external table and also could not provide statistics. " + "Disabling semijoin optimization.", columnOrigin.col, joinKeyTs.getConf().getTableMetadata().getFullyQualifiedName());
                            disableSemiJoin = true;
                        }
                    }
                }
    ...
    private boolean processSemiJoinHints(ParseContext pCtx, DynamicListContext ctx, Map<String, List<SemiJoinHint>> hints, String tableAlias, String internalColName, String colName, TableScanOperator ts, String keyBaseAlias) throws SemanticException
    {
        if (hints.size() == 0)
        {
            return false;
        ...
        for (SemiJoinHint sjHint : hintList)
        {
            ...
            if (generateSemiJoinOperatorPlan(ctx, pCtx, ts, keyBaseAlias, internalColName, colName, sjHint))
            {
                return true;
            }
    ...
    private void generateEventOperatorPlan(DynamicListContext ctx, ParseContext parseContext, TableScanOperator ts, String column, String columnType, ExprNodeDesc predicate)
    {
        ...
        float groupByMemoryUsage = HiveConf.getFloatVar(parseContext.getConf(), HiveConf.ConfVars.HIVE_MAP_AGGR_HASH_MEMORY);
        float memoryThreshold = HiveConf.getFloatVar(parseContext.getConf(), HiveConf.ConfVars.HIVE_MAP_AGGR_MEMORY_THRESHOLD);
        float minReductionHashAggr = HiveConf.getFloatVar(parseContext.getConf(), ConfVars.HIVE_MAP_AGGR_HASH_MIN_REDUCTION);
        float minReductionHashAggrLowerBound = HiveConf.getFloatVar(parseContext.getConf(), ConfVars.HIVE_MAP_AGGR_HASH_MIN_REDUCTION_LOWER_BOUND);
        float hashAggrFlushPercent = HiveConf.getFloatVar(parseContext.getConf(), ConfVars.HIVE_MAP_AGGR_HASH_FLUSH_SIZE_PERCENT);
        ...
        GroupByDesc groupBy = new GroupByDesc(GroupByDesc.Mode.HASH, outputNames, groupByExprs, new ArrayList<AggregationDesc>(), false, groupByMemoryUsage, memoryThreshold, minReductionHashAggr, minReductionHashAggrLowerBound, hashAggrFlushPercent, null, false, -1, true);
    ...
    private boolean generateSemiJoinOperatorPlan(DynamicListContext ctx, ParseContext parseContext, TableScanOperator ts, String keyBaseAlias, String internalColName, String colName, SemiJoinHint sjHint) throws SemanticException
    {
        ...
        try
        {
            ...
            AggregationDesc bloomFilter = new AggregationDesc(BLOOM_FILTER_FUNCTION, FunctionRegistry.getGenericUDAFEvaluator(BLOOM_FILTER_FUNCTION, aggFnOIs, false, false), params, false, Mode.PARTIAL1);
        ...
        GroupByDesc groupBy = new GroupByDesc(GroupByDesc.Mode.HASH, gbOutputNames, new ArrayList<ExprNodeDesc>(), aggs, false, groupByMemoryUsage, memoryThreshold, minReductionHashAggr, minReductionHashAggrLowerBound, hashAggrFlushPercent, null, false, -1, false);
        ...
        {
            ExprNodeColumnDesc colExpr = new ExprNodeColumnDesc(groupbyColInfos.get(i).getType(), gbOutputNames.get(i), "", false);
        ...
        try
        {
            ...
            AggregationDesc bloomFilter = new AggregationDesc(BLOOM_FILTER_FUNCTION, FunctionRegistry.getGenericUDAFEvaluator(BLOOM_FILTER_FUNCTION, bloomFilterFinalFnOIs, false, false), bloomFilterFinalParams, false, Mode.FINAL);
            GenericUDAFBloomFilterEvaluator bloomFilterEval = (GenericUDAFBloomFilterEvaluator) bloomFilter.getGenericUDAFEvaluator();
            bloomFilterEval.setSourceOperator(selectOp);
            if (sjHint != null && sjHint.getNumEntries() > 0)
            {
        ...
        GroupByDesc groupByDescFinal = new GroupByDesc(GroupByDesc.Mode.FINAL, gbOutputNames, new ArrayList<ExprNodeDesc>(), aggsFinal, false, groupByMemoryUsage, memoryThreshold, minReductionHashAggr, minReductionHashAggrLowerBound, hashAggrFlushPercent, null, false, 0, false);
    ...
    private void createFinalRsForSemiJoinOp(ParseContext parseContext, TableScanOperator ts, GroupByOperator gb, ExprNodeDesc key, String keyBaseAlias, ExprNodeDesc colExpr, boolean isHint) throws SemanticException
    {
        ...
        ReduceSinkOperator rsOpFinal = (ReduceSinkOperator) OperatorFactory.getAndMakeChild(rsDescFinal, new RowSchema(rsColInfos), gb);
