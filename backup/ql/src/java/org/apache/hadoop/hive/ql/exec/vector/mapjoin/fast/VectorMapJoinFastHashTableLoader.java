...
public class VectorMapJoinFastHashTableLoader implements org.apache.hadoop.hive.ql.exec.HashTableLoader
{
    ...
    public VectorMapJoinFastHashTableLoader(TezContext context, Configuration hconf, MapJoinOperator joinOp)
    {
        ...
        this.htLoadCounter = this.tezContext.getTezProcessorContext().getCounters().findCounter(HiveConf.getVar(hconf, HiveConf.ConfVars.HIVE_COUNTER_GROUP), hconf.get(Operator.CONTEXT_NAME_KEY, ""));
    }
    ...
    private void initHTLoadingService(long estKeyCount)
    {
        ...
        } else
        {
            ...
            {
                LOG.info("Adjust the number of HT-loading-threads to {}. (Previous value: {})", adjustedValue, initialValue);
            }

            this.numLoadThreads = adjustedValue;
        }
        this.totalEntries = new LongAccumulator(Long::sum, 0L);
        this.loadExecService = Executors.newFixedThreadPool(numLoadThreads, new ThreadFactoryBuilder().setDaemon(true).setPriority(Thread.NORM_PRIORITY).setNameFormat("HT-Load-Thread-%d").build());
