...
public class HiveWrapper
{
    ...
    public Tuple<org.apache.hadoop.hive.metastore.api.Function> function(final String name) throws HiveException
    {
    ...
    public Tuple<Table> table(final String tableName, HiveConf conf) throws HiveException
    {
        ...
        boolean getColStats = !Utils.shouldDumpMetaDataOnlyForExternalTables(db.getTable(dbName, tableName), conf) && !Utils.shouldDumpMetaDataOnly(conf);
        return new Tuple<>(functionForSpec, () -> db.getTable(dbName, tableName, null, true, false, getColStats));
    }
