...
public class FSRemover
{
    ...
    public List<Path> clean(CleanupRequest cr) throws MetaException
    {
        ...
        try
        {
            ...
            } else
            {
                ...
                UserGroupInformation ugi = UserGroupInformation.createProxyUser(cr.runAs(), UserGroupInformation.getLoginUser());
                try
                {
                ...
                } finally
                {
                    ...
                    {
                        LOG.error("Could not clean up file-system handles for UGI: {} for {}", ugi, cr.getFullPartitionName(), exception);
                    }
                }
            }
        } catch (Exception ex)
        {
            LOG.error("Caught exception when cleaning, unable to complete cleaning of {} due to {}", cr, StringUtils.stringifyException(ex));
        }
    ...
    private List<Path> removeFiles(CleanupRequest cr) throws MetaException, IOException
    {
        ...
        LOG.info("About to remove {} obsolete directories from {}. {}", cr.getObsoleteDirs().size(), cr.getLocation(), CompactorUtil.getDebugInfo(cr.getObsoleteDirs()));
        boolean needCmRecycle;
        try
        {
            Database db = metadataCache.computeIfAbsent(cr.getDbName(), () -> CompactorUtil.resolveDatabase(conf, cr.getDbName()));
