...
public class HiveAggregateInsertIncrementalRewritingRule extends HiveAggregateIncrementalRewritingRuleBase<HiveAggregateIncrementalRewritingRuleBase.IncrementalComputePlan>
{
    ...
    {
        super(operand(Aggregate.class, operand(Union.class, any())), HiveRelFactories.HIVE_BUILDER, "HiveAggregateIncrementalRewritingRule", 0);
    }
    ...
    @Override
    protected RexNode createFilterCondition(IncrementalComputePlan incrementalComputePlan, RexNode flagNode, List<RexNode> projExprs, RelBuilder relBuilder)
    {
        RexBuilder rexBuilder = relBuilder.getRexBuilder();
        return rexBuilder.makeCall(SqlStdOperatorTable.OR, flagNode, rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, flagNode));
    }
    ...
    protected RexNode createAggregateNode(SqlAggFunction aggCall, RexNode leftRef, RexNode rightRef, RexBuilder rexBuilder)
    {
        ...
        switch (aggCall.getKind())
        {
            case MIN:
            {
                RexNode condInnerCase = rexBuilder.makeCall(SqlStdOperatorTable.LESS_THAN, ImmutableList.of(rightRef, leftRef));
                return rexBuilder.makeCall(SqlStdOperatorTable.CASE, ImmutableList.of(condInnerCase, rightRef, leftRef));
            }
            case MAX:
            {
                RexNode condInnerCase = rexBuilder.makeCall(SqlStdOperatorTable.GREATER_THAN, ImmutableList.of(rightRef, leftRef));
                return rexBuilder.makeCall(SqlStdOperatorTable.CASE, ImmutableList.of(condInnerCase, rightRef, leftRef));
            }
            default:
