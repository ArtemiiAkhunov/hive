...
abstract class AbstractDropPartitionAnalyzer extends AbstractAlterTableAnalyzer
{
    ...
    {
        WriteType writeType = dropPartUseBase ? WriteType.DDL_EXCL_WRITE : WriteType.DDL_EXCLUSIVE;
        ...
        for (List<ExprNodeGenericFuncDesc> specs : partitionSpecs)
        {
            for (ExprNodeGenericFuncDesc partitionSpec : specs)
            {
                ...
                {
                    throw new SemanticException("Error fetching partitions for " + partitionSpec.getExprString() + ", message: " + e.getMessage(), e);
                }
                if (hasUnknown)
                {
