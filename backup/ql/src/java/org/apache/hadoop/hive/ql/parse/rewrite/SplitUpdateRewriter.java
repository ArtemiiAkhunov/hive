...
public class SplitUpdateRewriter implements Rewriter<UpdateStatement>
{
    ...
    public ParseUtils.ReparseResult rewrite(Context context, UpdateStatement updateBlock) throws SemanticException
    {
        ...
        for (int i = 0; i < nonPartCols.size(); i++)
        {
            ...
            if (setCol != null)
            {
                if (setCol.getType() == HiveParser.TOK_TABLE_OR_COL && setCol.getChildCount() == 1 && setCol.getChild(0).getType() == HiveParser.TOK_DEFAULT_VALUE)
                {
        ...
        {
            updateBlock.getTargetTable().getPartCols().forEach(fieldSchema -> insertValues.add(sqlGenerator.qualify(HiveUtils.unparseIdentifier(fieldSchema.getName(), conf))));
        }
        ...
        ASTNode rewrittenInsert = new CalcitePlanner.ASTSearcher().simpleBreadthFirstSearch(rewrittenTree, HiveParser.TOK_FROM, HiveParser.TOK_SUBQUERY, HiveParser.TOK_INSERT);
