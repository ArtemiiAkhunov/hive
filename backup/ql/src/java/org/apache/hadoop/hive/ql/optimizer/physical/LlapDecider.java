...
public class LlapDecider implements PhysicalPlanResolver
{
    ...
    class LlapDecisionDispatcher implements SemanticDispatcher
    {
        ...
        public LlapDecisionDispatcher(PhysicalContext pctx, LlapMode mode)
        {
            ...
            minReducersPerExec = HiveConf.getFloatVar(conf, ConfVars.TEZ_LLAP_MIN_REDUCER_PER_EXECUTOR);
        ...
        private void adjustAutoParallelism(BaseWork work)
        {
            ...
            {
                executorCount = clusterState.getKnownExecutorCount() + executorsPerNode * clusterState.getNodeCountWithUnknownExecutors();
            }
            ...
            if (reduceWork.isAutoReduceParallelism())
            {
                ...
                if (newMin < reduceWork.getMaxReduceTasks())
                {
                    ...
                    reduceWork.getEdgePropRef().setAutoReduce(conf, true, newMin, reduceWork.getMaxReduceTasks(), conf.getLongVar(HiveConf.ConfVars.BYTES_PER_REDUCER), reduceWork.getMinSrcFraction(), reduceWork.getMaxSrcFraction());
                } else
                {
        ...
        private void convertWork(TezWork tezWork, BaseWork work) throws SemanticException
        {
            ...
            if (shouldUber)
            {
                ...
                if (tezWork.getChildren(work).isEmpty() && work instanceof ReduceWork && ((ReduceWork) work).getNumReduceTasks() == 1)
                {
      ...
      /*if (work instanceof MapWork && ((MapWork)work).isUseOneNullRowInputFormat()) {
        ...
        return false;
            ...
            if (!evaluateOperators(work))
            {
                ...
                if (mode == only)
                {
                    throw new RuntimeException("Cannot run all parts of query in llap. Failing since " + ConfVars.LLAP_EXECUTION_MODE.varname + " is set to " + only.name());
                }
            ...
            if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.LLAP_AUTO_ENFORCE_TREE) && !checkParentsInLlap(tezWork, work))
            {
            ...
            if (work instanceof MapWork && HiveConf.getBoolVar(conf, HiveConf.ConfVars.LLAP_AUTO_ENFORCE_VECTORIZED) && !checkInputsVectorized((MapWork) work))
            {
            ...
            if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.LLAP_AUTO_ENFORCE_STATS) && !checkPartialStatsAvailable(work))
            {
        ...
        private Map<SemanticRule, SemanticNodeProcessor> getRules()
        {
            ...
            opRules.put(new RuleRegExp("No scripts", ScriptOperator.getOperatorName() + "%"), new SemanticNodeProcessor()
            {
                @Override
            ...
            opRules.put(new RuleRegExp("No user code in select", SelectOperator.getOperatorName() + "%"), new SemanticNodeProcessor()
            {
                @Override
                public Object process(Node n, Stack<Node> s, NodeProcessorCtx c, Object... os)
                {
            ...
            {
                opRules.put(new RuleRegExp("Disable grace hash join if LLAP mode and not dynamic partition hash join", MapJoinOperator.getOperatorName() + "%"), new SemanticNodeProcessor()
                {
                    @Override
                    public Object process(Node n, Stack<Node> s, NodeProcessorCtx c, Object... os)
                    {
        ...
        private boolean checkInputsVectorized(MapWork mapWork)
        {
            ...
            {
                if ((Utilities.isInputFileFormatVectorized(pd) && !excludedInputFormats.contains(pd.getInputFileFormatClass())) || (mayWrap && HiveInputFormat.canWrapForLlap(pd.getInputFileFormatClass(), true)))
                {
                    continue;
                }
