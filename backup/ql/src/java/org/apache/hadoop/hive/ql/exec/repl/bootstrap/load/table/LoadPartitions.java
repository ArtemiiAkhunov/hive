...
public class LoadPartitions
{
    ...
    {
        this(context, replLogger, tableContext, tableTracker, event, dbNameToLoadIn, null, metricCollector, null, PartitionState.Stage.PARTITION, tablesToBootstrap);
    }

    public LoadPartitions(Context context, ReplLogger replLogger, TableContext tableContext, TaskTracker limiter, TableEvent event, String dbNameToLoadIn, AlterTableAddPartitionDesc lastReplicatedPartition, ReplicationMetricCollector metricCollector, AlterTableAddPartitionDesc.PartitionDesc lastReplicatedPartitionDesc, PartitionState.Stage lastReplicatedStage, List<String> tablesToBootstrap) throws HiveException
    {
    ...
    {
        return new ReplicationState(new PartitionState(tableDesc.getTableName(), lastReplicatedPartition));
    }

    private boolean isMetaDataOp()
    {
        return HiveConf.getBoolVar(context.hiveConf, REPL_DUMP_SKIP_IMMUTABLE_DATA_COPY) || HiveConf.getBoolVar(context.hiveConf, REPL_DUMP_METADATA_ONLY);
    }
    ...
    private Path locationOnReplicaWarehouse(Table table, AlterTableAddPartitionDesc.PartitionDesc partSpec) throws MetaException, HiveException
    {
        ...
        {
            String externalLocation = ReplExternalTables.externalTableLocation(context.hiveConf, partSpec.getLocation());
        ...
        if (tableDesc.getLocation() == null)
        {
            ...
            {
                Database parentDb = context.hiveDb.getDatabase(tableDesc.getDatabaseName());
                return new Path(context.warehouse.getDefaultTablePath(parentDb, tableDesc.getTableName(), tableDesc.isExternal()), child);
            } else
            {
