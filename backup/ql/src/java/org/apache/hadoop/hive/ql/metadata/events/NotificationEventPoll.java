    ...
    private NotificationEventPoll(Configuration conf) throws Exception
    {
        ...
        if (consumerClassNames != null && consumerClassNames.length > 0)
        {
            for (String consumerClassName : consumerClassNames)
            {
                ...
                EventConsumer consumer = (EventConsumer) ReflectionUtils.newInstance(consumerClass, conf);
        ...
        pollFuture = executorService.scheduleAtFixedRate(new Poller(), pollInterval, pollInterval, TimeUnit.MILLISECONDS);
    }
    ...
    class Poller implements Runnable
    {
        ...
        public void run()
        {
            ...
            try
            {
                ...
                EventUtils.MSClientNotificationFetcher evFetcher = new EventUtils.MSClientNotificationFetcher(Hive.get());
                EventUtils.NotificationEventIterator evIter = new EventUtils.NotificationEventIterator(evFetcher, lastCheckedEventId, 0, "*", null);
