...
public class HiveMaterializedViewRule
{
     ...
     */
    private static final HepProgram PROGRAM = new HepProgramBuilder().addRuleInstance(HiveHepExtractRelNodeRule.INSTANCE).addRuleInstance(HiveVolcanoExtractRelNodeRule.INSTANCE).addRuleInstance(HiveTableScanProjectInsert.INSTANCE).addRuleCollection(ImmutableList.of(HiveFilterProjectTransposeRule.INSTANCE, HiveJoinProjectTransposeRule.BOTH_PROJECT, HiveJoinProjectTransposeRule.LEFT_PROJECT, HiveJoinProjectTransposeRule.RIGHT_PROJECT, HiveProjectMergeRule.INSTANCE)).addRuleInstance(ProjectRemoveRule.Config.DEFAULT.toRule()).addRuleInstance(HiveRootJoinProjectInsert.INSTANCE).build();

    public static final MaterializedViewProjectFilterRule INSTANCE_PROJECT_FILTER = (MaterializedViewProjectFilterRule) MaterializedViewProjectFilterRule.Config.DEFAULT.withGenerateUnionRewriting(true).withFastBailOut(false).withUnionRewritingPullProgram(PROGRAM).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).toRule();

    public static final MaterializedViewOnlyFilterRule INSTANCE_FILTER = (MaterializedViewOnlyFilterRule) MaterializedViewOnlyFilterRule.Config.DEFAULT.withGenerateUnionRewriting(true).withFastBailOut(false).withUnionRewritingPullProgram(PROGRAM).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).toRule();

    public static final MaterializedViewProjectJoinRule INSTANCE_PROJECT_JOIN = (MaterializedViewProjectJoinRule) MaterializedViewProjectJoinRule.Config.DEFAULT.withGenerateUnionRewriting(true).withFastBailOut(false).withUnionRewritingPullProgram(PROGRAM).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).toRule();

    public static final MaterializedViewOnlyJoinRule INSTANCE_JOIN = (MaterializedViewOnlyJoinRule) MaterializedViewOnlyJoinRule.Config.DEFAULT.withGenerateUnionRewriting(true).withFastBailOut(false).withUnionRewritingPullProgram(PROGRAM).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).toRule();
    ...
    public static final RelOptRule[] MATERIALIZED_VIEW_REWRITING_RULES = new RelOptRule[]{HiveMaterializedViewRule.INSTANCE_PROJECT_FILTER, HiveMaterializedViewRule.INSTANCE_FILTER, HiveMaterializedViewRule.INSTANCE_PROJECT_JOIN, HiveMaterializedViewRule.INSTANCE_JOIN, HiveMaterializedViewRule.INSTANCE_PROJECT_AGGREGATE, HiveMaterializedViewRule.INSTANCE_AGGREGATE};


    protected static class HiveMaterializedViewProjectAggregateRule extends MaterializedViewProjectAggregateRule
    {
        public HiveMaterializedViewProjectAggregateRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting, HepProgram unionRewritingPullProgram)
        {
    ...
    {
        public HiveMaterializedViewOnlyAggregateRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting, HepProgram unionRewritingPullProgram)
        {
            super(relBuilderFactory, generateUnionRewriting, unionRewritingPullProgram);
        }
    ...
    private static class HiveVolcanoExtractRelNodeRule extends RelOptRule
    {
        ...
        private static final HiveVolcanoExtractRelNodeRule INSTANCE = new HiveVolcanoExtractRelNodeRule();
    ...
    private static class HiveTableScanProjectInsert extends RelOptRule
    {
        ...
        {
            super(operand(Filter.class, operand(TableScan.class, any())), HiveRelFactories.HIVE_BUILDER, "HiveTableScanProjectInsert");
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            relBuilder.push(rel);
            List<RexNode> identityFields = relBuilder.fields(ImmutableBitSet.range(0, rel.getRowType().getFieldCount()).asList());
            RelNode newRel = relBuilder.project(identityFields, ImmutableList.of(), true).build();
    ...
    private static class HiveRootJoinProjectInsert extends RelOptRule
    {
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            List<RexNode> identityFields = relBuilder.fields(ImmutableBitSet.range(0, join.getRowType().getFieldCount()).asList());
