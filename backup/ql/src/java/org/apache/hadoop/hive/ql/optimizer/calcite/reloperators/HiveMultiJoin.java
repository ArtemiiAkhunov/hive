...
public final class HiveMultiJoin extends AbstractRelNode
{
     ...
     */
    public HiveMultiJoin(RelOptCluster cluster, List<RelNode> inputs, RexNode condition, RelDataType rowType, List<Pair<Integer, Integer>> joinInputs, List<JoinRelType> joinTypes, List<RexNode> filters, JoinPredicateInfo joinPredicateInfo)
    {
    ...
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs)
    {
        ...
        return new HiveMultiJoin(getCluster(), inputs, condition, rowType, joinInputs, joinTypes, filters);
    }
    ...
    public RelWriter explainTerms(RelWriter pw)
    {
        ...
        for (int i = 0; i < joinInputs.size(); i++)
        {
            ...
            sb.append(joinInputs.get(i).left).append(" - ").append(joinInputs.get(i).right).append(" : ").append(joinTypes.get(i).name());
        ...
        return pw.item("condition", condition).item("joinsDescription", joinsString);
    }
    ...
    public RelNode accept(RexShuttle shuttle)
    {
        ...
        return new HiveMultiJoin(getCluster(), inputs, joinFilter, rowType, joinInputs, joinTypes, filters);
    }
