...
public class TopNKeyOperator extends Operator<TopNKeyDesc> implements Serializable
{
    ...
    protected void initializeOp(Configuration hconf) throws HiveException
    {
        ...
        partitionKeyWrapper = initObjectInspectors(hconf, conf.getPartitionKeyColumns(), rowInspector, partitionKeyObjectInspectors, partitionCurrentKeyObjectInspectors);

        keyWrapperComparator = new KeyWrapperComparator(keyObjectInspectors, currentKeyObjectInspectors, columnSortOrder, nullSortOrder);
    ...
    private KeyWrapper initObjectInspectors(Configuration hconf, List<ExprNodeDesc> keyColumns, ObjectInspector rowInspector, ObjectInspector[] keyObjectInspectors, ObjectInspector[] currentKeyObjectInspectors) throws HiveException
    {
        ...
        for (int i = 0; i < keyColumns.size(); i++)
        {
            ...
            currentKeyObjectInspectors[i] = ObjectInspectorUtils.getStandardObjectInspector(keyObjectInspectors[i], ObjectInspectorUtils.ObjectInspectorCopyOption.WRITABLE);
        }
    ...
    public void process(Object row, int tag) throws HiveException
    {
        ...
        if (disabledPartitions.contains(partitionKeyWrapper))
        ...
        if (runTimeNumRows % conf.getCheckEfficiencyNumRows() == 0)
        { // check the efficiency at every nth rows
