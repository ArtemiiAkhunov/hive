...
public abstract class ProtobufSerDe extends AbstractSerDe
{
    ...
    private Class<? extends Message> loadClass(String protoClass) throws SerDeException
    {
        ...
        try
        {
            ...
            @SuppressWarnings("unchecked") Class<? extends Message> serdeClass = (Class<? extends Message>) clazz;
            return serdeClass;
    ...
    private static Descriptor loadDescriptor(Class<? extends Message> protoClass) throws SerDeException
    {
        try
        {
        ...
        {
            throw new SerDeException("Error trying to get descriptor for class: " + protoClass.getName(), e);
        }
    }
    ...
    private ObjectInspector createStructObjectInspector(Descriptor descriptor, Map<Descriptor, ObjectInspector> cache) throws SerDeException
    {
        ...
        ObjectInspector oi = ObjectInspectorFactory.getStandardStructObjectInspector(columnNames, columnOI);
        cache.put(descriptor, oi);
        return oi;
    }

    private ObjectInspector createObjectInspector(FieldDescriptor descriptor, Map<Descriptor, ObjectInspector> cache) throws SerDeException
    {
        ObjectInspector oi;
    ...
    private ObjectInspector getMapObjectInspector(Descriptor descriptor, Map<Descriptor, ObjectInspector> cache) throws SerDeException
    {
    ...
    private ProtoToHiveConvertor createConvertor(Descriptor descriptor, Map<Descriptor, ProtoToHiveConvertor> cache) throws SerDeException
    {
        if (cache.containsKey(descriptor))
        {
            return cache.get(descriptor);
        ...
        for (FieldDescriptor field : descriptor.getFields())
        {
            ...
            if (field.isRepeated())
            {
                if (field.getJavaType() == JavaType.MESSAGE && mapTypes.contains(field.getMessageType().getFullName()))
                {
