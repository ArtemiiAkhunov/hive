...
public final class CompileLockFactory
{
    ...
    public static CompileLock newInstance(HiveConf conf, String command)
    {
        ...
        boolean isParallelEnabled = (conf != null) && HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_SERVER2_PARALLEL_COMPILATION);
        ...
        long timeout = HiveConf.getTimeVar(conf, HiveConf.ConfVars.HIVE_SERVER2_COMPILE_LOCK_TIMEOUT, TimeUnit.SECONDS);
    ...
    private enum SessionWithQuotaCompileLock implements Lock
    {
        ...
        public boolean tryLock(long time, TimeUnit unit) throws InterruptedException
        {
            ...
            try
            {
                result = compileLock.tryLock(time, unit) && globalCompileQuotas.tryAcquire(getRemainingTime(startTime, unit.toNanos(time)), TimeUnit.NANOSECONDS);
            } finally
            {
