...
public class HiveSplitGenerator extends InputInitializer
{
    ...
    public HiveSplitGenerator(Configuration conf, MapWork work, final boolean generateSingleSplit, Integer numSplits) throws IOException
    {
        super(null);
    ...
    public List<Event> initialize() throws Exception
    {
        ...
        try
        {
            ...
            if (groupingEnabled)
            {
                ...
                InputSplit[] splits;
                if (generateSingleSplit && conf.get(HiveConf.ConfVars.HIVE_TEZ_INPUT_FORMAT.varname).equals(HiveInputFormat.class.getName()))
                {
    ...
    {
        Class<?> clazz = Class.forName(HiveConf.getVar(conf, HiveConf.ConfVars.HIVE_SPLITS_AVAILABLE_SLOTS_CALCULATOR_CLASS), true, Utilities.getSessionSpecifiedClassLoader());
