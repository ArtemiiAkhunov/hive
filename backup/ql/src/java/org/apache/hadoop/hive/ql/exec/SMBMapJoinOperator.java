...
public class SMBMapJoinOperator extends AbstractMapJoinOperator<SMBJoinDesc> implements Serializable
{
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SMBMapJoinOperator.class.getName());
    ...
    public void initializeMapredLocalWork(MapJoinDesc mjConf, Configuration hconf, MapredLocalWork localWork, Logger l4j) throws HiveException
    {
        ...
        for (Map.Entry<String, FetchWork> entry : aliasToFetchWork.entrySet())
        {
            ...
            ColumnProjectionUtils.appendReadColumns(jobClone, ts.getNeededColumnIDs(), ts.getNeededColumns(), ts.getNeededNestedColumnPaths(), ts.conf.hasVirtualCols());
    ...
    public void process(Object row, int tag) throws HiveException
    {
        ...
        if (nextKeyGroup)
        {
            ...
            } while (smallestPos != null && smallestPos.size() > 0 && !smallestPos.contains(this.posBigTable));
    ...
    private int compareKeys(List<Object> k1, List<Object> k2)
    {
        ...
        {
            ret = WritableComparatorFactory.get(k1.get(i), nullsafes == null ? false : nullsafes[i], null).compare(k1.get(i), k2.get(i));
            if (ret != 0)
            {
                return ret;
            }
