...
public class SortedDynPartitionOptimizer extends Transform
{
    ...
    private static final Function<List<ExprNodeDesc>, ExprNodeDesc> BUCKET_SORT_EXPRESSION = cols ->
    {
        try
        {
            return ExprNodeGenericFuncDesc.newInstance(FunctionRegistry.getFunctionInfo("bucket_number").getGenericUDF(), new ArrayList<>());
    ...
    class SortedDynamicPartitionProc implements SemanticNodeProcessor
    {
        ...
        public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx, Object... nodeOutputs) throws SemanticException
        {
            ...
            if (destTable.isMaterializedView() && (destTable.getProperty(Constants.MATERIALIZED_VIEW_SORT_COLUMNS) != null || destTable.getProperty(Constants.MATERIALIZED_VIEW_DISTRIBUTE_COLUMNS) != null))
            {
                LOG.debug("Bailing out of sort dynamic partition optimization as destination is a materialized view" + "with CLUSTER/SORT/DISTRIBUTE spec");
                return null;
            }
            ...
            LinkedList<Function<List<ExprNodeDesc>, ExprNodeDesc>> customSortExprs = new LinkedList<>(dpCtx.getCustomSortExpressions());
            ...
            if (fsOp.getConf().getWriteType() == AcidUtils.Operation.UPDATE || fsOp.getConf().getWriteType() == AcidUtils.Operation.DELETE)
            {
                ...
                if (numBuckets > 0)
                {
                    ...
                    bucketColumns.add(ExprNodeTypeCheck.getExprNodeDefaultExprProcessor().createConversionCast(new ExprNodeColumnDesc(ci), TypeInfoFactory.intTypeInfo));
                }
            } else
            {
                ...
                } else if (HiveConf.getBoolVar(this.parseCtx.getConf(), HiveConf.ConfVars.HIVE_SORT_WHEN_BUCKETING) && !bucketPositions.isEmpty())
                {
            ...
            ReduceSinkOperator rsOp = getReduceSinkOp(partitionPositions, sortPositions, sortOrder, sortNullOrder, customSortExprs, customSortOrder, customNullOrder, allRSCols, bucketColumns, numBuckets, fsParent, fsOp.getConf().getWriteType());
            ...
            for (int i = 0; i < allRSCols.size(); i++)
            {
                ...
                if (i < fileSinkSchema.size())
                {
                    ...
                    {
                        newColumnExpr = ExprNodeTypeCheck.getExprNodeDefaultExprProcessor().createConversionCast(newColumnExpr, (PrimitiveTypeInfo) fsColInfo.getType());
                    }
                }
            ...
            for (Function<List<ExprNodeDesc>, ExprNodeDesc> customSortExpr : customSortExprs)
            {
                ...
                descs.add(new ExprNodeColumnDesc(customSortColTypeInfo, ReduceField.KEY + "." + customSortColName, null, false));
        ...
        private boolean allStaticPartitions(Operator<? extends OperatorDesc> op, List<ExprNodeDesc> allRSCols, final DynamicPartitionCtx dynPartCtx)
        {
        ...
        private boolean removeRSInsertedByEnforceBucketing(FileSinkOperator fsOp)
        {
            ...
            if (found)
            {
                ...
                LOG.info("Removed " + rsToRemove.getOperatorId() + " and " + rsChildToRemove.getOperatorId() + " as it was introduced by enforce bucketing/sorting.");
            }
            return true;
        }
        ...
        private boolean shouldDo(List<Integer> partitionPos, Operator<? extends OperatorDesc> fsParent)
        {
            ...
            int threshold = HiveConf.getIntVar(this.parseCtx.getConf(), HiveConf.ConfVars.HIVE_OPT_SORT_DYNAMIC_PARTITION_THRESHOLD);
            long MAX_WRITERS = -1;
            ...
            if (MAX_WRITERS < 0)
            {
                double orcMemPool = this.parseCtx.getConf().getDouble(OrcConf.MEMORY_POOL.getHiveConfName(), (Double) OrcConf.MEMORY_POOL.getDefaultValue());
                long orcStripSize = this.parseCtx.getConf().getLong(OrcConf.STRIPE_SIZE.getHiveConfName(), (Long) OrcConf.STRIPE_SIZE.getDefaultValue());
