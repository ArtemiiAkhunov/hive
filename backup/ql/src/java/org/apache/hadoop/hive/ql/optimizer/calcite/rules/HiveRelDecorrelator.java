...
public final class HiveRelDecorrelator implements ReflectiveVisitor
{
    ...
    private static final FilterFlattenCorrelatedConditionRule FLATTEN_CORRELATED_CONDITION_RULE = new FilterFlattenCorrelatedConditionRule(HiveFilter.class, HiveRelFactories.HIVE_BUILDER, "HiveFilterFlattenCorrelatedConditionRule");
    //~ Instance fields --------------------------------------------------------
    ...
    private RelNode decorrelate(RelNode root)
    {
        ...
        if (frame != null)
        {
            ...
            final HepProgram program2 = HepProgram.builder().addRuleInstance(HiveFilterJoinRule.FILTER_ON_JOIN).addRuleInstance(HiveFilterJoinRule.JOIN).build();
    ...
    protected RexNode removeCorrelationExpr(RexNode exp, boolean projectPulledAboveLeftCorrelator, RexInputRef nullIndicator, Set<Integer> isCount)
    {
        RemoveCorrelationRexShuttle shuttle = new RemoveCorrelationRexShuttle(rexBuilder, projectPulledAboveLeftCorrelator, nullIndicator, isCount);
        return exp.accept(shuttle);
    }
    ...
    public Frame decorrelateRel(Join rel)
    {
        ...
        if (cm != null && cm.mapRefRelToCorRef != null && cm.mapRefRelToCorRef.containsKey(rel) && rel.getJoinType().isOuterJoin())
        {
            throw new UnsupportedOperationException("Correlated subqueries in outer join conditions not supported yet." + " Join condition: " + rel.getCondition());
        }
    ...
    private RelNode createProjectWithAdditionalExprs(RelNode input, List<Pair<RexNode, String>> additionalExprs)
    {
        ...
        return RelOptUtil.createProject(input, Pair.left(projects), Pair.right(projects), false, relBuilder);
    }
        ...
        public RexNode visitCall(final RexCall call)
        {
            ...
            if (update[0])
            {
                ...
                if (!isSpecialCast)
                {
                    ...
                    newCall = rexBuilder.ensureType(call.getType(), rexBuilder.makeCall(rexBuilder.deriveReturnType(operator, clonedOperands), operator, clonedOperands), true);
                } else
                {
    ...
    private final class RemoveCorrelationForScalarProjectRule extends RelOptRule
    {
        ...
        {
            super(operand(LogicalCorrelate.class, operand(RelNode.class, any()), operand(Aggregate.class, operand(Project.class, operand(RelNode.class, any())))));
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            if ((right instanceof Filter) && cm.mapRefRelToCorRef.containsKey(right))
            {
            ...
            Join join = (Join) relBuilder.push(left).push(right).join(joinType, joinCond).build();
    ...
    private final class RemoveCorrelationForScalarAggregateRule extends RelOptRule
    {
        ...
        {
            super(operand(LogicalCorrelate.class, operand(RelNode.class, any()), operand(Project.class, operandJ(Aggregate.class, null, Aggregate::isSimple, operand(Project.class, operand(RelNode.class, any()))))));
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            RelNode joinOutputProject = RelOptUtil.createProject(join, joinOutputProjects, null, false, relBuilder);
            ...
            Aggregate newAggregate = (Aggregate) relBuilder.push(joinOutputProject).aggregate(relBuilder.groupKey(groupSet, null), newAggCalls).build();
            ...
            RelNode newAggOutputProject = RelOptUtil.createProject(newAggregate, newAggOutputProjectList, null, false, relBuilder);
    ...
    private final class AdjustProjectForCountAggregateRule extends RelOptRule
    {
        ...
        {
            super(flavor ? operand(LogicalCorrelate.class, operand(RelNode.class, any()), operand(Project.class, operand(Aggregate.class, any()))) : operand(LogicalCorrelate.class, operand(RelNode.class, any()), operand(Aggregate.class, any())));
            this.flavor = flavor;
        }
        ...
        public void onMatch(RelOptRuleCall call)
        {
            ...
            } else
            {
                ...
                aggOutputProject = (Project) RelOptUtil.createProject(aggregate, Pair.left(projects), Pair.right(projects), false, relBuilder);
            }
    ...
    static class CorRef implements Comparable<CorRef>
    {
        ...
        {
            return this == o || o instanceof CorRef && uniqueKey == ((CorRef) o).uniqueKey && corr == ((CorRef) o).corr && field == ((CorRef) o).field;
        }
    ...
    private static class CorelMapBuilder extends HiveRelShuttleImpl
    {
        ...
        private final Multimap<RelNode, CorRef> mapRefRelToCorRef = Multimaps.newListMultimap(new HashMap<>(), Lists::newArrayList);
    ...
    static class Frame
    {
        ...
        Frame(RelNode oldRel, RelNode r, SortedMap<CorDef, Integer> corDefOutputs, Map<Integer, Integer> oldToNewOutputs)
        {
            ...
            assert allLessThan(corDefOutputs.values(), r.getRowType().getFieldCount(), Litmus.THROW);
            assert allLessThan(oldToNewOutputs.keySet(), oldRel.getRowType().getFieldCount(), Litmus.THROW);
            assert allLessThan(oldToNewOutputs.values(), r.getRowType().getFieldCount(), Litmus.THROW);
        }
    }
}
