...
class HiveValuesVisitor extends HiveRelNodeVisitor<HiveValues>
{
    ...
    OpAttr visit(HiveValues valuesRel) throws SemanticException
    {
        ...
        LOG.debug("Translating operator rel#{}:{} with row type: [{}]", valuesRel.getId(), valuesRel.getRelTypeName(), valuesRel.getRowType());

        if (!Values.isEmpty(valuesRel))
        {
            LOG.error("Empty {} operator translation not supported yet in return path.", valuesRel.getClass().getCanonicalName());
            return null;
        }
    ...
    private Operator<?> createDummyScan() throws SemanticException
    {
        ...
        return OperatorFactory.get(hiveOpConverter.getSemanticAnalyzer().getOpContext(), tsd, new RowSchema(Collections.emptyList()));
    }
    ...
    private static Operator<?> createSelect(RelDataType relDataType, Operator<?> parentOp)
    {
        ...
        for (int i = 0; i < relDataType.getFieldList().size(); i++)
        {
            ...
            ColumnInfo ci = new ColumnInfo(typeField.getName(), TypeConverter.convert(typeField.getType()), SemanticAnalyzer.DUMMY_TABLE, false);
            colInfoList.add(ci);
