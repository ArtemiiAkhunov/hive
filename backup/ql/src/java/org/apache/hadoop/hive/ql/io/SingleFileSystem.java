...
public abstract class SingleFileSystem extends FileSystem
{
    ...
    {
        FileStatus newStatus = new FileStatus(status.getLen(), true, status.getReplication(), status.getBlockSize(), status.getModificationTime(), status.getAccessTime(), addExecute(status.getPermission()), status.getOwner(), status.getGroup(), (status.isSymlink() ? status.getSymlink() : null), status.getPath());
