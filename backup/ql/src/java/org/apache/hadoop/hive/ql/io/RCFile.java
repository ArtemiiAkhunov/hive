...
public class RCFile
{
    ...
    private static final byte[] ORIGINAL_MAGIC = new byte[]{(byte) 'S', (byte) 'E', (byte) 'Q'};
    ...
    private static final byte[] ORIGINAL_MAGIC_VERSION = new byte[]{(byte) 'S', (byte) 'E', (byte) 'Q', ORIGINAL_MAGIC_VERSION_WITH_METADATA};

    // The 'magic' bytes at the beginning of the RCFile
    private static final byte[] MAGIC = new byte[]{(byte) 'R', (byte) 'C', (byte) 'F'};
    ...
    public static class KeyBuffer implements WritableComparable
    {
         ...
         *
         * @param columnValueLen  this total bytes number of this column's values in this split
         * @param colValLenBuffer each cell's length of this column's in this split
         */
    ...
    public static class ValueBuffer implements WritableComparable
    {
        ...
        public ValueBuffer(KeyBuffer currentKey, int columnNumber, boolean[] skippedCols, CompressionCodec codec, boolean lazyDecompress) throws IOException
        {
            ...
            compressedColumnsValueBuffer = new NonSyncDataOutputBuffer[columnNumber - skipped];
            this.codec = codec;
            if (codec != null)
            {
    ...
    public static class Writer
    {
         ...
         * @throws IOException
         */
        public Writer(FileSystem fs, Configuration conf, Path name, Progressable progress, Metadata metadata, CompressionCodec codec) throws IOException
        {
         ...
         * @throws IOException
         */
        public Writer(FileSystem fs, Configuration conf, Path name, int bufferSize, short replication, long blockSize, Progressable progress, Metadata metadata, CompressionCodec codec) throws IOException
        {
        ...
        void init(Configuration conf, FSDataOutputStream out, CompressionCodec codec, Metadata metadata) throws IOException
        {
            this.conf = conf;
            this.out = out;
            this.codec = codec;
         ...
         */
        public void flushBlock(KeyBuffer keyBuffer, ValueBuffer valueBuffer, int recordLen, int keyLength, @SuppressWarnings("unused") int compressedKeyLen) throws IOException
        {
        ...
        public synchronized void close() throws IOException
        {
            ...
            for (int i = 0; i < columnNumber; i++)
            {
                LOG.info("Column#" + i + " : Plain Total Column Value Length: " + plainTotalColumnLength[i] + ",  Compr Total Column Value Length: " + comprTotalColumnLength[i]);
            }
        }
    }
    ...
    public static class Reader
    {
        ...
        {
            this(fs, file, conf.getInt("io.file.buffer.size", 4096), conf, 0, fs.getFileStatus(file).getLen());
        }
