...
final class RebalanceQueryCompactor extends QueryCompactor
{
    ...
    public boolean run(CompactorContext context) throws IOException, HiveException
    {
        ...
        if (numBuckets <= 0)
        {
            ...
            numBuckets = Streams.stream(new FileUtils.AdaptingIterator<>(FileUtils.listFiles(dir.getFs(), dir.getPath(), true, AcidUtils.bucketFileFilter))).map(f -> AcidUtils.parseBucketId(f.getPath())).collect(Collectors.toSet()).size();
        }


        List<String> createQueries = getCreateQueries(tmpTableName, table, tmpTablePath.toString());
        List<String> compactionQueries = getCompactionQueries(table, context.getPartition(), tmpTableName, numBuckets, context.getCompactionInfo().orderByClause);
        List<String> dropQueries = getDropQueries(tmpTableName);
        runCompactionQueries(conf, tmpTableName, storageDescriptor, writeIds, context.getCompactionInfo(), Lists.newArrayList(tmpTablePath), createQueries, compactionQueries, dropQueries, table.getParameters());
        return true;
    }

    private List<String> getCreateQueries(String fullName, Table t, String tmpTableLocation)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.REBALANCE, CompactionQueryBuilder.Operation.CREATE, false, fullName).setSourceTab(t).setLocation(tmpTableLocation).build());
    }

    private List<String> getCompactionQueries(Table t, Partition p, String tmpName, int numberOfBuckets, String orderByClause)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.REBALANCE, CompactionQueryBuilder.Operation.INSERT, false, tmpName).setSourceTab(t).setSourcePartition(p).setNumberOfBuckets(numberOfBuckets).setOrderByClause(orderByClause).build());
    }

    private List<String> getDropQueries(String tmpTableName)
    {
        return Lists.newArrayList(new CompactionQueryBuilder(CompactionType.REBALANCE, CompactionQueryBuilder.Operation.DROP, false, tmpTableName).build());
    }
}
