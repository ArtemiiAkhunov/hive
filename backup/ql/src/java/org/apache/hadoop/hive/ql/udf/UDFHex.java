...
public class UDFHex extends UDF implements IUDFUnaryString
{
    ...
    private Text evaluate(long num)
    {
        ...
        {
            len++;
            value[value.length - len] = (byte) Character.toUpperCase(Character.forDigit((int) (num & 0xF), 16));
            num >>>= 4;
        } while (num != 0);
    ...
    private Text evaluate(byte[] bytes, int length)
    {
        ...
        {
            value[i * 2] = (byte) Character.toUpperCase(Character.forDigit((bytes[i] & 0xF0) >>> 4, 16));
            value[i * 2 + 1] = (byte) Character.toUpperCase(Character.forDigit(bytes[i] & 0x0F, 16));
        }
