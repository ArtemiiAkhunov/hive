...
class RenderStrategy
{
    ...
    private abstract static class BaseUpdateFunction implements UpdateFunction
    {
        ...
        BaseUpdateFunction(TezJobMonitor monitor, PerfLogger perfLogger)
        {
            ...
            print_interval = HiveConf.getTimeVar(monitor.getHiveConf(), HiveConf.ConfVars.HIVE_LOG_INCREMENTAL_PLAN_PROGRESS_INTERVAL, TimeUnit.MILLISECONDS);
    ...
    static class LogToFileFunction extends BaseUpdateFunction
    {
        ...
        private static final DateTimeFormatter REPORT_DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss,SSS");

        private final boolean hiveServer2InPlaceProgressEnabled = SessionState.get().getConf().getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_INPLACE_PROGRESS);
