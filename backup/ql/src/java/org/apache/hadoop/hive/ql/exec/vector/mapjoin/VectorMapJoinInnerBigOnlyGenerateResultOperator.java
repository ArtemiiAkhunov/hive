...
public abstract class VectorMapJoinInnerBigOnlyGenerateResultOperator extends VectorMapJoinGenerateResultOperator
{
    ...
    public VectorMapJoinInnerBigOnlyGenerateResultOperator(CompilationOpContext ctx, OperatorDesc conf, VectorizationContext vContext, VectorDesc vectorDesc) throws HiveException
    {
     ...
     * @param hashTableResults    The array of all hash table results for the batch. We need the
    ...
    protected void finishInnerBigOnly(VectorizedRowBatch batch, int allMatchCount, int equalKeySeriesCount, int spillCount, VectorMapJoinHashTableResult[] hashTableResults, int hashMapResultCount) throws HiveException, IOException
    {
        ...
        for (int i = 0; i < equalKeySeriesCount; i++)
        {
            ...
            if (count == 1)
            {
                numSel = generateHashMultiSetResultSingleValue(batch, allMatches, allMatchesIndex, duplicateCount, numSel);
            } else
            {
                generateHashMultiSetResultMultiValue(batch, allMatches, allMatchesIndex, duplicateCount, count);
            }
        }
        batch.size = numSel;
     ...
     * @param allMatches      A subset of the rows of the batch that are matches.
     * @param allMatchesIndex The logical index into allMatches of the first equal key.
     * @param duplicateCount  The number of duplicates or equal keys.
     * @param numSel          The current count of rows in the rebuilding of the selected array.
     * @return The new count of selected rows.
     */
    private int generateHashMultiSetResultSingleValue(VectorizedRowBatch batch, int[] allMatches, int allMatchesIndex, int duplicateCount, int numSel) throws HiveException, IOException
    {
     ...
     */
    private void generateHashMultiSetResultMultiValue(VectorizedRowBatch batch, int[] allMatches, int allMatchesIndex, int duplicateCount, long count) throws HiveException, IOException
    {
     ...
     * @param batch              The big table batch with any matching and any non matching rows both as
     *                           selected in use.
     * @param hashMultiSetResult The hash multi-set results for the batch.
     */
