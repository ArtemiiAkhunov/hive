...
public class BootstrapEventsIterator implements Iterator<BootstrapEvent>
{
    ...
    public BootstrapEventsIterator(String dumpDirectory, String dbNameToLoadIn, boolean needLogger, HiveConf hiveConf, ReplicationMetricCollector metricCollector) throws IOException
    {
        ...
        List<FileStatus> dbsToCreate = Arrays.stream(fileStatuses).filter(f -> !f.getPath().getName().equals(ReplUtils.CONSTRAINTS_ROOT_DIR_NAME)).collect(Collectors.toList());
