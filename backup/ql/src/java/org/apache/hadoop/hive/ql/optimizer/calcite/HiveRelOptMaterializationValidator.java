...
{
    private static final String UNSUPPORTED_BY_CALCITE_FORMAT = "Only query text based automatic rewriting is available for materialized view. " + "Statement has unsupported %s: %s.";
    ...
    public RelNode visit(HiveTableScan hiveScan)
    {
        ...
        if (tab.getTableType() == TableType.EXTERNAL_TABLE && !(tab.getStorageHandler() != null && tab.getStorageHandler().areSnapshotsSupported()))
        {
    ...
    private void fail(String reason)
    {
        ...
        this.materializationValidationResult = new MaterializationValidationResult(EnumSet.noneOf(RewriteAlgorithm.class), "Cannot enable automatic rewriting for materialized view. " + reason);
        throw Util.FoundOne.NULL;
    }

    private RelNode fail(RelNode node)
    {
        setResultCacheInvalidReason("Unsupported RelNode type " + node.getRelTypeName() + " encountered in the query plan");
        this.materializationValidationResult = new MaterializationValidationResult(EnumSet.noneOf(RewriteAlgorithm.class), String.format("Cannot enable automatic rewriting for materialized view. " + "Unsupported RelNode type %s encountered in the query plan", node.getRelTypeName()));
        throw Util.FoundOne.NULL;
    }
    ...
    public void unsupportedByCalciteRewrite(String sqlPartType, String sqlPart)
    {
        if (isValidForAutomaticRewriting())
        {
            ...
            this.materializationValidationResult = new MaterializationValidationResult(EnumSet.of(TEXT), errorMessage);
        }
    }
