...
public class LBExprProcFactory extends PrunerExpressionOperatorFactory
{
    ...
    public static class LBPRColumnExprProcessor extends ColumnExprProcessor
    {
        ...
        protected ExprNodeDesc processColumnDesc(NodeProcessorCtx procCtx, ExprNodeColumnDesc cd)
        {
            ...
            if (cd.getTabAlias().equalsIgnoreCase(ctx.getTabAlias()) && isPruneForListBucketing(part, cd.getColumn()))
            {
                newcd = cd.clone();
            } else
            {
     ...
     *                 for pruning
     * @param pred     The predicate from which the list bucketing pruner needs to be
     *                 generated
     * @param part     The partition this walker is walking
