...
public class VectorUDAFBloomFilterMerge extends VectorAggregateExpression
{
    ...
    static final class Aggregation implements AggregationBuffer
    {
        ...
        public void mergeBloomFilterBytesFromInputColumn(BytesColumnVector inputColumn, int batchSize, boolean selectedInUse, int[] selected)
        {
            ...
            for (int j = 0; j < batchSize; j++)
            {
                if (!selectedInUse)
                {
                    ...
                    {
                        splitVectorAcrossWorkers(workers, inputColumn.vector[j], inputColumn.start[j], inputColumn.length[j]);
                ...
                {
                    int i = selected[j];
                    splitVectorAcrossWorkers(workers, inputColumn.vector[i], inputColumn.start[i], inputColumn.length[i]);
                } else
                {
                    int i = selected[j];
        ...
        private static void splitVectorAcrossWorkers(BloomFilterMergeWorker[] workers, byte[] bytes, int start, int length)
        {
             ...
             */
            int elementPerBatch = (int) Math.ceil((double) (length - START_OF_SERIALIZED_LONGS) / workers.length);

            for (int w = 0; w < workers.length; w++)
            {
                int modifiedStart = START_OF_SERIALIZED_LONGS + w * elementPerBatch;
                int modifiedLength = (w == workers.length - 1) ? length - (START_OF_SERIALIZED_LONGS + w * elementPerBatch) : elementPerBatch;
        ...
        public void shutdownAndWaitForMergeTasks(Aggregation agg, boolean aborted)
        {
            ...
            {
                LOG.warn("Bloom filter merge is interrupted while waiting to finish, this is unexpected", e);
            }
        }
    }
    ...
    private static class BloomFilterMergeWorker implements Runnable
    {
        ...
        public void run()
        {
            while (true)
            {
                ...
                {// Executor.shutdownNow() is called
                    if (!queue.isEmpty())
                    {
                        LOG.info("bloom filter merge was interrupted while processing and queue is still not empty (size: {})" + ", this is fine in case of shutdownNow", queue.size());
                    }
                    if (aborted.get())
                    {
        ...
        {
            BloomKFilter.mergeBloomFilterBytes(bfAggregation, bfAggregationStart, bfAggregationLength, bloomFilterWrapper.bytes, bloomFilterWrapper.start, bloomFilterWrapper.length, bloomFilterWrapper.modifiedStart, bloomFilterWrapper.modifiedStart + bloomFilterWrapper.modifiedLength);
        }
    }
    ...
    public void finish(AggregationBuffer myagg, boolean aborted)
    {
        ...
        if (agg.numThreads > 0)
        {
            LOG.info("bloom filter merge: finishing aggregation, waiting tasks: {}", agg.getNumberOfWaitingMergeTasks());
    ...
    void processValue(Aggregation myagg, ColumnVector columnVector, int i)
    {
        ...
        BloomKFilter.mergeBloomFilterBytes(myagg.bfBytes, 0, myagg.bfBytes.length, inputColumn.vector[i], inputColumn.start[i], inputColumn.length[i]);
    }
    ...
    public boolean matches(String name, ColumnVector.Type inputColVectorType, ColumnVector.Type outputColVectorType, Mode mode)
    {
         ...
         */
        return name.equals(BLOOM_FILTER_FUNCTION) && inputColVectorType == ColumnVector.Type.BYTES && outputColVectorType == ColumnVector.Type.BYTES && (mode == Mode.PARTIAL2 || mode == Mode.FINAL);
    }
