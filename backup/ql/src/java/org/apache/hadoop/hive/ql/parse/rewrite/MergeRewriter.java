...
public class MergeRewriter implements Rewriter<MergeStatement>, MergeStatement.DestClausePrefixSetter
{
    ...
    public ParseUtils.ReparseResult rewrite(Context ctx, MergeStatement mergeStatement) throws SemanticException
    {
        ...
        handleSource(mergeStatement.hasWhenNotMatchedInsertClause(), mergeStatement.getSourceAlias(), mergeStatement.getOnClauseAsText(), sqlGenerator);
    ...
    private void handleCardinalityViolation(String targetAlias, String onClauseAsString, MultiInsertSqlGenerator sqlGenerator) throws SemanticException
    {
        ...
        sqlGenerator.append("INSERT INTO ").append(tableName).append("\n  SELECT cardinality_violation(").append(StringUtils.join(sortKeys, ","));
        sqlGenerator.appendPartColsOfTargetTableWithComma(targetAlias);

        sqlGenerator.append(")\n WHERE ").append(onClauseAsString).append(" GROUP BY ").append(StringUtils.join(sortKeys, ","));
    ...
    protected static class MergeWhenClauseSqlGenerator implements MergeStatement.MergeSqlGenerator
    {
        ...
        public void appendWhenMatchedUpdateClause(MergeStatement.UpdateClause updateClause)
        {
            ...
            List<String> valuesAndAcidSortKeys = new ArrayList<>(targetTable.getCols().size() + targetTable.getPartCols().size() + 1);
            ...
            addWhereClauseOfUpdate(onClauseAsString, updateClause.getExtraPredicate(), updateClause.getDeleteExtraPredicate(), sqlGenerator);
        ...
        {
            UnaryOperator<String> formatter = name -> String.format("%s.%s", targetAlias, HiveUtils.unparseIdentifier(name, conf));
        ...
        {
            handleWhenMatchedDelete(mergeStatement.getOnClauseAsText(), deleteClause.getExtraPredicate(), deleteClause.getUpdateExtraPredicate(), hintStr, sqlGenerator);
            hintStr = null;
        }
