...
public class TezSessionState
{
    ...
    public static final class HiveResources
    {
        ...
        {
            return dagResourcesDir + "; " + additionalFilesNotFromConf.size() + " additional files, " + localizedResources.size() + " localized resources";
        }
    }
    ...
    public String toString()
    {
        return "sessionId=" + sessionId + ", queueName=" + queueName + ", user=" + user + ", doAs=" + doAsEnabled + ", isOpen=" + isOpen() + ", isDefault=" + defaultQueue;
    }
    ...
    public void open(HiveResources resources) throws LoginException, IOException, URISyntaxException, TezException
    {
        openInternal(null, false, null, resources);
    }

    public void beginOpen(String[] additionalFiles, LogHelper console) throws IOException, LoginException, URISyntaxException, TezException
    {
    ...
    private boolean isKerberosEnabled(Configuration conf)
    {
        try
        {
            return UserGroupInformation.getLoginUser().hasKerberosCredentials() && HiveConf.getBoolVar(conf, ConfVars.LLAP_USE_KERBEROS);
        } catch (IOException e)
        {
            return false;
        }
    ...
    private void setupTezParamsBasedOnMR(TezConfiguration conf)
    {
        ...
        conf.setIfUnset(TezConfiguration.TEZ_AM_LAUNCH_CMD_OPTS, org.apache.tez.mapreduce.hadoop.MRHelpers.getJavaOptsForMRAM(conf));
        ...
        conf.setIfUnset(TezConfiguration.TEZ_AM_MAX_APP_ATTEMPTS, "" + conf.getInt(MRJobConfig.MR_AM_MAX_ATTEMPTS, MRJobConfig.DEFAULT_MR_AM_MAX_ATTEMPTS));
        ...
        conf.setIfUnset(TezConfiguration.TEZ_AM_MODIFY_ACLS, conf.get(MRJobConfig.JOB_ACL_MODIFY_JOB, MRJobConfig.DEFAULT_JOB_ACL_MODIFY_JOB));
        ...
        for (Map<String, String> map : maps)
        {
            for (Map.Entry<String, String> dep : map.entrySet())
            {
                if (conf.get(dep.getKey()) != null)
                {
                    ...
                    LOG.info("Config: mr(unset):" + dep.getKey() + ", mr initial value=" + mrValue + ", tez(original):" + dep.getValue() + "=" + tezValue + ", tez(final):" + dep.getValue() + "=" + conf.get(dep.getValue()));
                }
            }
        }
    }
    ...
    private void setupSessionAcls(Configuration tezConf, HiveConf hiveConf) throws IOException
    {
        ...
        boolean addHs2User = HiveConf.getBoolVar(hiveConf, ConfVars.HIVE_TEZ_HS2_USER_ACCESS);

        String viewStr = Utilities.getAclStringWithHiveModification(tezConf, TezConfiguration.TEZ_AM_VIEW_ACLS, addHs2User, user, loginUser);
        String modifyStr = Utilities.getAclStringWithHiveModification(tezConf, TezConfiguration.TEZ_AM_MODIFY_ACLS, addHs2User, user, loginUser);

        if (LOG.isDebugEnabled())
        {
            // TODO: De-link from SessionState. A TezSession can be linked to different Hive Sessions via the pool.
            LOG.debug("Setting Tez Session access for sessionId={} with viewAclString={}, modifyStr={}", SessionState.get().getSessionId(), viewStr, modifyStr);
        }
    ...
    public void ensureLocalResources(Configuration conf, String[] newFilesNotFromConf) throws IOException, LoginException, URISyntaxException, TezException
    {
        ...
        if (session != null)
        {
            ...
            {
                session.addAppMasterLocalFiles(DagUtils.getResourcesUpdatableForAm(resources.localizedResources));
            }
        }
    }
     ...
     *
     * @param keepDagFilesDir whether or not to remove the scratch dir at the same time.
     * @throws Exception
     */
    ...
    private Path createTezDir(String sessionId, String suffix) throws IOException
    {
        ...
        String hdfsScratchDir = sessionState == null ? HiveConf.getVar(conf, HiveConf.ConfVars.SCRATCH_DIR) : sessionState.getHdfsScratchDirURIString();
     ...
     */
    @VisibleForTesting
    LocalResource createJarLocalResource(String localJarPath) throws IOException, LoginException, IllegalArgumentException
    {
    ...
    private void addJarLRByClassName(String className, final Map<String, LocalResource> lrMap) throws IOException, LoginException
    {
        Class<?> clazz;
        try
        {
    ...
    private void addJarLRByClass(Class<?> clazz, final Map<String, LocalResource> lrMap) throws IOException, LoginException
    {
    ...
    private String getSha(final Path localFile) throws IOException, IllegalArgumentException
    {
        ...
        if (sha256 == null)
        {
            ...
            try
            {
                ...
                LOG.info("Computed sha: {} for file: {} of length: {} in {} ms", sha256, localFile, LlapUtil.humanReadableByteCount(fileStatus.getLen()), end - start);
