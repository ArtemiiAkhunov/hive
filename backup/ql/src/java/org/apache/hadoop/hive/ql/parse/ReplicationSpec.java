...
public class ReplicationSpec
{
    ...
    public enum KEY
    {
        REPL_SCOPE("repl.scope"), EVENT_ID("repl.event.id"), CURR_STATE_ID_SOURCE(ReplConst.REPL_TARGET_TABLE_PROPERTY), NOOP("repl.noop"), IS_REPLACE("repl.is.replace"), VALID_WRITEID_LIST("repl.valid.writeid.list"), VALID_TXN_LIST("repl.valid.txnid.list"), CURR_STATE_ID_TARGET(REPL_TARGET_DATABASE_PROPERTY),
        ;
    ...
    public enum SCOPE
    {NO_REPL, MD_ONLY, REPL}

    public enum Type
    {DEFAULT, INCREMENTAL_DUMP, IMPORT}
    ...
    {
        return allowReplacement(getLastReplicatedStateFromParameters(params), getCurrentReplicationState());
    }
