...
public abstract class UDFDateFloor extends UDF
{
    ...
    public TimestampLocalTZWritable evaluate(TimestampLocalTZWritable t)
    {
        ...
        final long originalTimestampUTC = localZDT.withZoneSameLocal(ZoneOffset.UTC).toInstant().toEpochMilli(); // default -> utc
        final long newTimestampUTC = granularity.truncate(originalTimestampUTC); // utc
        final ZonedDateTime newLocalZDT = ZonedDateTime.of(LocalDateTime.ofInstant(Instant.ofEpochMilli(newTimestampUTC), ZoneOffset.UTC), localZDT.getZone()); // utc -> default
    ...
    private static abstract class QueryGranularity
    {
        ...
        {
            SECOND(1000), MINUTE(60 * 1000), FIFTEEN_MINUTE(15 * 60 * 1000), THIRTY_MINUTE(30 * 60 * 1000), HOUR(3600 * 1000), DAY(24 * 3600 * 1000);
    ...
    private static class PeriodGranularity extends BaseQueryGranularity
    {
        ...
        {
            return "PeriodGranularity{" + "period=" + period + ", timeZone=" + chronology.getZone() + ", origin=" + (hasOrigin ? origin : "null") + '}';
        }
    }
