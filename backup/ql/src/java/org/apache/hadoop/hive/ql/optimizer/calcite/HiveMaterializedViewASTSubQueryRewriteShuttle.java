...
public class HiveMaterializedViewASTSubQueryRewriteShuttle extends HiveRelShuttleImpl
{
    ...
    public HiveMaterializedViewASTSubQueryRewriteShuttle(Map<RelNode, ASTNode> subQueryMap, ASTNode originalAST, ASTNode expandedAST, RelBuilder relBuilder, Hive db, Set<TableName> tablesUsedByOriginalPlan, HiveTxnManager txnManager)
    {
    ...
    public RelNode visit(HiveProject project)
    {
        ...
        RelNode match = getMaterializedViewByAST(expandedSubqAST, relBuilder.getCluster(), NON_CALCITE, db, tablesUsedByOriginalPlan, txnManager);
        if (match != null)
        {
            return match;
        }
    ...
    public RelNode visit(HiveFilter filter)
    {
        ...
        return relBuilder.push(filter.getInput().accept(this)).filter(newCond).build();
    }
     ...
     */
    public static RelNode getMaterializedViewByAST(ASTNode expandedAST, RelOptCluster optCluster, Predicate<Set<RewriteAlgorithm>> filter, Hive db, Set<TableName> tablesUsedByOriginalPlan, HiveTxnManager txnManager)
    {
        try
        {
            List<HiveRelOptMaterialization> relOptMaterializationList = db.getMaterializedViewsByAST(expandedAST, tablesUsedByOriginalPlan, txnManager);
            for (HiveRelOptMaterialization relOptMaterialization : relOptMaterializationList)
            {
                ...
                {
                    LOG.debug("Filter out materialized view {} scope {}", relOptMaterialization.qualifiedTableName, relOptMaterialization.getScope());
                    continue;
                }
                ...
                try
                {
                    ...
                    if (HiveMaterializedViewUtils.checkPrivilegeForMaterializedViews(singletonList(hiveTableMD)))
                    {
                        ...
                        if (db.validateMaterializedViewsFromRegistry(singletonList(hiveTableMD), sourceTables, txnManager))
                        {
                            return relOptMaterialization.copyToNewCluster(optCluster).tableRel;
                        }
                    } else
                    {
                        LOG.debug("User does not have privilege to use materialized view {}", relOptMaterialization.qualifiedTableName);
                    }
                } catch (HiveException e)
                {
                    LOG.warn("Skipping materialized view due to validation failure: " + relOptMaterialization.qualifiedTableName, e);
                }
            }
        } catch (HiveException e)
        {
