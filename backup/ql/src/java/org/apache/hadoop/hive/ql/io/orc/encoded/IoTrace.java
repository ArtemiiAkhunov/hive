...
public final class IoTrace
{
    ...
    // Events.
    public static final int TREE_READER_NEXT_VECTOR = 1, READING_STRIPE = 2, SARG_RESULT = 4, RANGES = 5, COLUMN_READ = 6, SKIP_STREAM = 7, ADD_STREAM = 8, START_RG = 9, START_COL = 10, START_STRIPE_STREAM = 11, START_STREAM = 12, START_READ = 13, UNCOMPRESSED_DATA = 14, PARTIAL_UNCOMPRESSED_DATA = 15, VALID_UNCOMPRESSED_CHUNK = 16, CACHE_COLLISION = 17, ORC_CB = 18, INVALID_ORC_CB = 19, PARTIAL_CB = 20, COMPOSITE_ORC_CB = 21, SARG_RESULT2 = 22;
    ...
    private static int dumpOneLine(int ix, Logger logger, long[] log)
    {
        ...
        switch (event)
        {
            ...
            case READING_STRIPE:
            {
                logger.info(ix + ": Reading stripe " + getSecondInt(log[ix]) + " at " + log[ix + 1] + " length " + log[ix + 2]);
                return ix + 3;
            }
            case SARG_RESULT:
            {
            ...
            case SARG_RESULT2:
            {
                ...
                logger.info(ix + ": Reading filtered rgs for stripe " + getSecondInt(log[ix]) + ": " + DebugUtils.toString(rgs));
            ...
            case RANGES:
            {
                ...
                logger.info(ix + ": Ranges for file " + log[ix + 1] + " (base offset " + log[ix + 2] + ") after " + src + ": " + sb.toString());
                return ix + 3 + rangeCount * 3;
            }
            case COLUMN_READ:
            {
                logger.info(ix + ": Reading column " + getSecondInt(log[ix]) + " (included index " + getFirstInt(log[ix + 1]) + "; type " + ColumnEncoding.Kind.values()[getSecondInt(log[ix + 1])] + ")");
                return ix + 2;
            }
            case SKIP_STREAM:
            {
                long streamOffset = log[ix + 1];
                logger.info(ix + ": Skipping stream for col " + getSecondInt(log[ix]) + " [" + streamOffset + ", " + (streamOffset + getFirstInt(log[ix + 2])) + ") kind " + Kind.values()[getSecondInt(log[ix + 2])]);
                return ix + 3;
            }
            case ADD_STREAM:
            {
                long streamOffset = log[ix + 1];
                logger.info(ix + ": Adding stream for col " + getSecondInt(log[ix]) + " [" + streamOffset + ", " + (streamOffset + getFirstInt(log[ix + 2])) + ") kind " + Kind.values()[getSecondInt(log[ix + 2])] + ", index " + getFirstInt(log[ix + 3]) + ", entire stream " + (getSecondInt(log[ix + 3]) == 1));
                return ix + 4;
            }
            case START_RG:
            {
            ...
            case START_STREAM:
            {
                ...
                logger.info(ix + ": Starting on stream " + Kind.values()[getSecondInt(log[ix])] + "[" + offset + ", " + (offset + getSecondInt(log[ix + 2])) + ") " + unlockStr);
                return ix + 3;
            }
            case START_READ:
            {
            ...
            {
                long offset = log[ix + 1];
                logger.info(ix + ": Uncompressed data [" + offset + ", " + (offset + getSecondInt(log[ix])) + ")");
                return ix + 2;
            }
            case PARTIAL_UNCOMPRESSED_DATA:
            {
                long offset = log[ix + 1];
                logger.info(ix + ": Partial uncompressed data [" + offset + ", " + (offset + getSecondInt(log[ix])) + ")");
                return ix + 2;
            }
            case VALID_UNCOMPRESSED_CHUNK:
            {
                logger.info(ix + ": Combining uncompressed data for cache buffer of length " + getSecondInt(log[ix]) + " from 0x" + Integer.toHexString((int) log[ix + 1]));
                return ix + 2;
            }
            case CACHE_COLLISION:
            {
                logger.info(ix + ": Replacing " + printRange(ix + 1, log) + " with 0x" + Integer.toHexString(getSecondInt(log[ix])));
                return ix + 4;
            }
            case ORC_CB:
            {
                ...
                int cbLength = val >>> 1;
                logger.info(ix + ": Found " + (isUncompressed ? "un" : "") + "compressed ORC CB [" + offset + ", " + (offset + cbLength) + ")");
                return ix + 2;
            }
            case INVALID_ORC_CB:
            {
                long offset = log[ix + 1];
                logger.info(ix + ": Found incomplete ORC CB [" + offset + ", " + (offset + getSecondInt(log[ix])) + ")");
                return ix + 2;
            }
            case PARTIAL_CB:
            {
            ...
            {
                logger.info(ix + ": Combined ORC CB from multiple buffers " + printRange(ix + 2, log) + " last chunk taken " + getSecondInt(log[ix]) + ", remaining " + log[ix + 1]);
                return ix + 5;
            }
            default:
    ...
    public void logSargResult(int stripeIx, boolean[] rgsToRead)
    {
        ...
        if (offset + elements + 2 > log.length)
        {
            return;
        }
    ...
    public void logRanges(Object fileKey, long baseOffset, DiskRangeList range, RangesSrc src)
    {
        ...
        if (offset + 3 > log.length)
        {
            return; // At least the header should fit.
        }
    ...
    public void logAddStream(int colIx, Stream.Kind streamKind, long streamOffset, long length, int indexIx, boolean isEntire)
    {
        if (log == null)
        {
            return;
    ...
    public void logStartStream(Kind kind, long cOffset, long endCOffset, long unlockUntilCOffset)
    {
        if (log == null)
        {
            return;
        ...
        int unlockLenToSave = unlockLen >= 0 && unlockLen < Integer.MAX_VALUE ? (int) unlockLen : Integer.MAX_VALUE;
    ...
    public void logStartRead(DiskRangeList current)
    {
        ...
        log[offset] = makeIntPair(START_READ, current.hasData() ? System.identityHashCode(current.getData()) : 0);
        this.offset += 1;
    }
