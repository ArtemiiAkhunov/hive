 ...
 */
public class HiveAggregateInsertDeleteIncrementalRewritingRule extends HiveAggregateIncrementalRewritingRuleBase<HiveAggregateInsertDeleteIncrementalRewritingRule.IncrementalComputePlanWithDeletedRows>
{
    ...
    {
        super(operand(Aggregate.class, operand(Union.class, operand(Aggregate.class, any()))), HiveRelFactories.HIVE_BUILDER, "HiveAggregateInsertDeleteIncrementalRewritingRule", 2);
    }
    ...
    {
        RexBuilder rexBuilder = relBuilder.getRexBuilder();
        RexNode countArg = rexBuilder.makeInputRef(aggInput.getRowType().getFieldList().get(argumentIdx).getType(), argumentIdx);
        RexNode isNull = rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, countArg);
        return rexBuilder.makeCall(SqlStdOperatorTable.CASE, isNull, relBuilder.literal(0), relBuilder.literal(1));
    }
     ...
     */
    static class IncrementalComputePlanWithDeletedRows extends HiveAggregateIncrementalRewritingRuleBase.IncrementalComputePlan
    {
    ...
    protected RexNode createFilterCondition(IncrementalComputePlanWithDeletedRows incrementalComputePlan, RexNode flagNode, List<RexNode> projExprs, RelBuilder relBuilder)
    {
        ...
        RexNode insert = rexBuilder.makeCall(SqlStdOperatorTable.AND, rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, flagNode), countStarGT0);
