...
public class GenericUDAFBloomFilter implements GenericUDAFResolver2
{
     ...
     */
    @VectorizedUDAFs({VectorUDAFBloomFilter.class, VectorUDAFBloomFilterMerge.class})
        ...
        public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException
        {
            ...
            switch (inputOI.getPrimitiveCategory())
            {
                ...
                case DOUBLE:
                    ...
                    break;
                case DECIMAL:
                    HiveDecimalWritable vDecimal = ((HiveDecimalObjectInspector) inputOI).getPrimitiveWritableObject(parameters[0]);
                    int startIdx = vDecimal.toBytes(scratchBuffer);
                    bf.addBytes(scratchBuffer, startIdx, scratchBuffer.length - startIdx);
                    break;
                case DATE:
                    DateWritableV2 vDate = ((DateObjectInspector) inputOI).getPrimitiveWritableObject(parameters[0]);
                    bf.addLong(vDate.getDays());
                    break;
                case TIMESTAMP:
                    Timestamp vTimeStamp = ((TimestampObjectInspector) inputOI).getPrimitiveJavaObject(parameters[0]);
                    bf.addLong(vTimeStamp.toEpochMilli());
                    break;
                case CHAR:
                    Text vChar = ((HiveCharObjectInspector) inputOI).getPrimitiveWritableObject(parameters[0]).getStrippedValue();
                    bf.addBytes(vChar.getBytes(), 0, vChar.getLength());
                    break;
                case VARCHAR:
                    Text vVarChar = ((HiveVarcharObjectInspector) inputOI).getPrimitiveWritableObject(parameters[0]).getTextValue();
                    bf.addBytes(vVarChar.getBytes(), 0, vVarChar.getLength());
                    break;
                case STRING:
                    Text vString = ((StringObjectInspector) inputOI).getPrimitiveWritableObject(parameters[0]);
                    bf.addBytes(vString.getBytes(), 0, vString.getLength());
                    break;
                case BINARY:
                    BytesWritable vBytes = ((BinaryObjectInspector) inputOI).getPrimitiveWritableObject(parameters[0]);
                    bf.addBytes(vBytes.getBytes(), 0, vBytes.getLength());
                    break;
                default:
                    throw new UDFArgumentTypeException(0, "Bad primitive category " + inputOI.getPrimitiveCategory());
            }
        }
            ...
            if (sourceOperator != null && sourceOperator.getStatistics() != null)
            {
                ...
                switch (stats.getColumnStatsState())
                {
                    ...
                    case PARTIAL:
                        ...
                        ExprNodeColumnDesc colExpr = ExprNodeDescUtils.getColumnExpr(sourceOperator.getConf().getColList().get(0));
                        if (colExpr != null && stats.getColumnStatisticsFromColName(colExpr.getColumn()) != null)
                        {
