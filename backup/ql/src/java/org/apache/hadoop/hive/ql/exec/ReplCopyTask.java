...
public class ReplCopyTask extends Task<ReplCopyWork> implements Serializable
{
    ...
    public int execute()
    {
        ...
        try
        {
            ...
            if (ReplChangeManager.isCMFileUri(fromPath))
            {
                ...
                ReplChangeManager.FileInfo sourceInfo = ReplChangeManager.getFileInfo(new Path(result[0]), result[1], result[2], result[3], conf);
                DataCopyStatistics copyStatistics = new DataCopyStatistics();
                if (FileUtils.copy(sourceInfo.getSrcFs(), sourceInfo.getSourcePath(), dstFs, toPath, false, false, conf, copyStatistics))
                {
            ...
            if (work.getDeleteDestIfExist() && dstFs.exists(toPath))
            {
                ...
                getHive().cleanUpOneDirectoryForReplace(toPath, dstFs, HIDDEN_FILES_PATH_FILTER, conf, work.getNeedRecycle(), work.getIsAutoPurge());
            }
    ...
    private List<ReplChangeManager.FileInfo> filesInFileListing(FileSystem fs, Path dataPath) throws IOException
    {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(fs.open(fileListing))))
        {
            ...
            while ((line = br.readLine()) != null)
            {
                ...
                try
                {
                    ReplChangeManager.FileInfo f = ReplChangeManager.getFileInfo(new Path(fragments[0]), fragments[1], fragments[2], fragments[3], conf);
                    filePaths.add(f);
    ...
    {
        return getLoadCopyTask(replicationSpec, srcPath, dstPath, conf, isAutoPurge, needRecycle, readSourceAsFileList, false, true, dumpDirectory, metricCollector);
    }
    ...
    {
        return getLoadCopyTask(replicationSpec, srcPath, dstPath, conf, false, false, true, false, true, dumpDirectory, metricCollector);
    }
    ...
    {
        return getLoadCopyTask(replicationSpec, srcPath, dstPath, conf, false, false, readSourceAsFileList, overWrite, true, dumpDirectory, metricCollector);
    }
    ...
    {
        return getLoadCopyTask(replicationSpec, srcPath, dstPath, conf, false, false, readSourceAsFileList, overWrite, deleteDestination, dumpDirectory, metricCollector);
    }
