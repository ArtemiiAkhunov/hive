...
final class CommandAuthorizerV2
{
    ...
    private static List<HivePrivilegeObject> getHivePrivObjects(List<? extends Entity> privObjects, Map<String, List<String>> tableName2Cols, HiveOperationType hiveOpType, BaseSemanticAnalyzer sem) throws HiveException
    {
        ...
        for (Entity privObject : privObjects)
        {
            ...
            if (privObject.getTyp() == Type.FUNCTION && !HiveConf.getBoolVar(SessionState.get().getConf(), HiveConf.ConfVars.HIVE_AUTHORIZATION_FUNCTIONS_IN_VIEW) && hiveOpType == HiveOperationType.QUERY)
            {
                String[] qualifiedFunctionName = new String[]{privObject.getDatabase() != null ? privObject.getDatabase().getName() : null, privObject.getFunctionName()};
    ...
    private static void addHivePrivObject(Entity privObject, Map<String, List<String>> tableName2Cols, List<HivePrivilegeObject> hivePrivObjs, HiveOperationType hiveOpType) throws HiveException
    {
        ...
        switch (privObject.getType())
        {
            ...
            case TABLE:
                ...
                if (table.getStorageHandler() != null && HiveConf.getBoolVar(SessionState.getSessionConf(), HiveConf.ConfVars.HIVE_AUTHORIZATION_TABLES_ON_STORAGEHANDLERS))
                {
                    //TODO: add hive privilege object for storage based handlers for create and alter table commands.
                    if (privObject instanceof WriteEntity && (hiveOpType == HiveOperationType.CREATETABLE || hiveOpType == HiveOperationType.ALTERTABLE_PROPERTIES || hiveOpType == HiveOperationType.CREATETABLE_AS_SELECT))
                    {
                        try
                        {
            ...
            case PARTITION:
                ...
                List<String> col = tableName2Cols == null ? null : tableName2Cols.get(Table.getCompleteName(tbl.getDbName(), tbl.getTableName()));
                hivePrivObject = new HivePrivilegeObject(privObjType, tbl.getDbName(), tbl.getTableName(), null, col, actionType, null, null, tbl.getOwner(), tbl.getOwnerType());
                break;
            case SERVICE_NAME:
