...
public class AlterTableExchangePartitionsOperation extends DDLOperation<AlterTableExchangePartitionsDesc>
{
    ...
    public int execute() throws HiveException
    {
        ...
        List<Partition> partitions = context.getDb().exchangeTablePartitions(partitionSpecs, sourceTable.getDbName(), sourceTable.getTableName(), destTable.getDbName(), destTable.getTableName());
        for (Partition partition : partitions)
        {
            ...
            DDLUtils.addIfAbsentByName(new WriteEntity(new Partition(sourceTable, partition.getSpec(), null), WriteEntity.WriteType.DELETE), context);

            DDLUtils.addIfAbsentByName(new WriteEntity(new Partition(destTable, partition.getSpec(), null), WriteEntity.WriteType.INSERT), context);
        }
