...
public class HiveStructConverter extends HiveGroupConverter
{
    ...
    public HiveStructConverter(final GroupType requestedSchema, final GroupType tableSchema, Map<String, String> metadata, TypeInfo hiveTypeInfo)
    {
    ...
    public HiveStructConverter(final GroupType groupType, final ConverterParent parent, final int index, TypeInfo hiveTypeInfo)
    {
        this(groupType, parent, index, groupType, hiveTypeInfo);
    }

    public HiveStructConverter(final GroupType selectedGroupType, final ConverterParent parent, final int index, final GroupType containingGroupType, TypeInfo hiveTypeInfo)
    {
        this.parent = parent;
        this.index = index;
    ...
    private void init(final GroupType selectedGroupType, final ConverterParent parent, final int index, final GroupType containingGroupType, TypeInfo hiveTypeInfo)
    {
        if (parent != null)
        {
    ...
    private boolean isSubType(final GroupType groupType, final Type subtype)
    {
    ...
    private TypeInfo getStructFieldTypeInfo(String field, int fieldIndex)
    {
        ...
        if (Boolean.parseBoolean(getMetadata().get(DataWritableReadSupport.PARQUET_COLUMN_INDEX_ACCESS)) && fieldIndex < hiveFieldNames.size())
        {
    ...
    private Converter getFieldConverter(Type type, int fieldIndex, TypeInfo hiveTypeInfo)
    {
        ...
        if (type.isRepetition(Type.Repetition.REPEATED))
        {
            if (type.isPrimitive())
            {
                converter = new Repeated.RepeatedPrimitiveConverter(type.asPrimitiveType(), this, fieldIndex, hiveTypeInfo);
            } else
            {
                converter = new Repeated.RepeatedGroupConverter(type.asGroupType(), this, fieldIndex, hiveTypeInfo == null ? null : ((ListTypeInfo) hiveTypeInfo).getListElementTypeInfo());
            }
