...
public final class HiveFileFormatUtils
{
    ...
    public static class FileChecker
    {
        ...
        private FileChecker()
        {
            ...
            inputFormatCheckerMap = ImmutableMap.<Class<? extends InputFormat>, Class<? extends InputFormatChecker>>builder().put(SequenceFileInputFormat.class, SequenceFileInputFormatChecker.class).put(RCFileInputFormat.class, RCFileInputFormat.class).put(OrcInputFormat.class, OrcInputFormat.class).put(MapredParquetInputFormat.class, MapredParquetInputFormat.class).build();
            textInputFormatCheckerMap = ImmutableMap.<Class<? extends InputFormat>, Class<? extends InputFormatChecker>>builder().put(SequenceFileInputFormat.class, SequenceFileInputFormatChecker.class).build();
            outputFormatSubstituteMap = ImmutableMap.<Class<?>, Class<? extends OutputFormat>>builder().put(IgnoreKeyTextOutputFormat.class, HiveIgnoreKeyTextOutputFormat.class).put(SequenceFileOutputFormat.class, HiveSequenceFileOutputFormat.class).build();

            // updatable map that holds instances of the class
            inputFormatCheckerInstanceCache = CacheBuilder.newBuilder().maximumSize(MAX_CACHE_SIZE).build();
        }
    ...
    public static RecordUpdater getAcidRecordUpdater(JobConf jc, TableDesc tableInfo, int bucket, FileSinkDesc conf, Path outPath, ObjectInspector inspector, Reporter reporter, int rowIdColNum, Integer attemptId) throws HiveException, IOException
    {
        HiveOutputFormat<?, ?> hiveOutputFormat = getHiveOutputFormat(jc, tableInfo);
        ...
        return getRecordUpdater(jc, acidOutputFormat, bucket, inspector, tableInfo.getProperties(), outPath, reporter, rowIdColNum, conf, attemptId);
    }

    private static RecordUpdater getRecordUpdater(JobConf jc, AcidOutputFormat<?, ?> acidOutputFormat, int bucket, ObjectInspector inspector, Properties tableProp, Path outPath, Reporter reporter, int rowIdColNum, FileSinkDesc conf, Integer attemptId) throws IOException
    {
        return acidOutputFormat.getRecordUpdater(outPath, new AcidOutputFormat.Options(jc).isCompressed(conf.getCompressed()).tableProperties(tableProp).reporter(reporter).writingBase(conf.getInsertOverwrite()).minimumWriteId(conf.getTableWriteId()).maximumWriteId(conf.getTableWriteId()).bucket(bucket).inspector(inspector).recordIdColumn(rowIdColNum).statementId(conf.getStatementId()).maxStmtId(conf.getMaxStmtId()).finalDestination(conf.getDestPath()).attemptId(attemptId).temporary(conf.isTemporary()));
    }

    public static <T> T getFromPathRecursively(Map<Path, T> pathToPartitionInfo, Path dir, Map<Map<Path, T>, Map<Path, T>> cacheMap) throws IOException
    {
        return getFromPathRecursively(pathToPartitionInfo, dir, cacheMap, false);
    }

    public static <T> T getFromPathRecursively(Map<Path, T> pathToPartitionInfo, Path dir, Map<Map<Path, T>, Map<Path, T>> cacheMap, boolean ignoreSchema) throws IOException
    {
        return getFromPathRecursively(pathToPartitionInfo, dir, cacheMap, ignoreSchema, false);
    }

    public static <T> T getFromPathRecursively(Map<Path, T> pathToPartitionInfo, Path dir, Map<Map<Path, T>, Map<Path, T>> cacheMap, boolean ignoreSchema, boolean ifPresent) throws IOException
    {
     ...
     **/
    public static List<Operator<? extends OperatorDesc>> doGetWorksFromPath(Map<Path, List<String>> pathToAliases, Map<String, Operator<? extends OperatorDesc>> aliasToWork, Path dir)
    {
