...
public class HiveAugmentSnapshotMaterializationRule extends RelRule<HiveAugmentSnapshotMaterializationRule.Config>
{
    ...
    {
        return RelRule.Config.EMPTY.as(HiveAugmentSnapshotMaterializationRule.Config.class).withMvMetaStoredSnapshot(mvMetaStoredSnapshot).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).withOperandSupplier(operandBuilder -> operandBuilder.operand(TableScan.class).anyInputs()).withDescription("HiveAugmentSnapshotMaterializationRule").toRule();
    }
    ...
    public void onMatch(RelOptRuleCall call)
    {
        ...
        RexNode snapshotIdInputRef = rexBuilder.makeInputRef(tableScan.getTable().getRowType().getFieldList().get(snapshotIdIndex).getType(), snapshotIdIndex);

        final RelBuilder relBuilder = call.builder();
        relBuilder.push(tableScan);
        final RexNode snapshotIdLiteral = rexBuilder.makeLiteral(snapshotId, snapshotIdType(relBuilder.getTypeFactory()), false);
        final RexNode predicateWithSnapShotId = rexBuilder.makeCall(SqlStdOperatorTable.LESS_THAN_OR_EQUAL, snapshotIdInputRef, snapshotIdLiteral);
