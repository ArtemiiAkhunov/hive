...
public final class OrcFile extends org.apache.orc.OrcFile
{
    ...
    static class LlapAwareMemoryManager extends MemoryManagerImpl
    {
        ...
        public LlapAwareMemoryManager(Configuration conf)
        {
            ...
            {
                LOG.debug("Using LLAP memory manager for orc writer. memPerExecutor: {} maxLoad: {} totalMemPool: {}", LlapUtil.humanReadableByteCount(memPerExecutor), maxLoad, LlapUtil.humanReadableByteCount(totalMemoryPool));
            }
        }
     ...
     */
    public static class WriterOptions extends org.apache.orc.OrcFile.WriterOptions implements Cloneable
    {
    ...
    {
        return createWriter(path, writerOptions(conf).inspector(inspector).fileSystem(fs).stripeSize(stripeSize).compress(compress).bufferSize(bufferSize).rowIndexStride(rowIndexStride));
    }
}
