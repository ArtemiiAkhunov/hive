...
public class HiveMaterializedViewUtils
{
    ...
    public static Boolean isOutdatedMaterializedView(String validTxnsList, HiveTxnManager txnMgr, Hive db, Set<TableName> tablesUsed, Table materializedViewTable) throws HiveException
    {
        ...
        return isOutdatedMaterializedView(materializationTxnList, validTxnsList, txnMgr, tablesUsed, materializedViewTable);
    }
    ...
    private static Boolean isOutdatedMaterializedView(String materializationTxnList, String validTxnsList, HiveTxnManager txnMgr, Set<TableName> tablesUsed, Table materializedViewTable) throws LockException
    {
        ...
        for (String fullyQualifiedTableName : tablesUsedNames)
        {
            ...
            if (tableWriteIds == null)
            {
                ...
                LOG.warn("Materialized view " + materializedViewTable.getFullyQualifiedName() + " ignored for rewriting as details about txn ids for table " + fullyQualifiedTableName + " could not be found in " + materializationTxnWriteIds);
                ignore = true;
                break;
            }
    ...
    private static Boolean isOutdatedMaterializedView(MaterializationSnapshot snapshot, Hive db, Set<TableName> tablesUsed, Table materializedViewTable) throws HiveException
    {
        ...
        {
            LOG.debug("Materialized view {} ignored for rewriting as we could not obtain current snapshot ids", materializedViewTable.getFullyQualifiedName());
            return null;
        }
        ...
        for (String fullyQualifiedTableName : tablesUsedNames)
        {
            ...
            {
                LOG.debug("Materialized view {} ignored for rewriting as we could not get storage handler of table {}", materializedViewTable.getFullyQualifiedName(), fullyQualifiedTableName);
                return null;
            }
            if (!table.getStorageHandler().areSnapshotsSupported())
            {
                LOG.debug("Materialized view {} ignored for rewriting as storage handler of table {} " + "does not support snapshots.", materializedViewTable.getFullyQualifiedName(), fullyQualifiedTableName);
                return null;
            }
     ...
     * its invalidation.
     */
    public static HiveRelOptMaterialization augmentMaterializationWithTimeInformation(HiveRelOptMaterialization materialization, String validTxnsList, MaterializationSnapshot snapshot) throws LockException
    {

        RelNode modifiedQueryRel;
        if (snapshot != null && snapshot.getTableSnapshots() != null && !snapshot.getTableSnapshots().isEmpty())
        {
            modifiedQueryRel = applyRule(materialization.queryRel, HiveAugmentSnapshotMaterializationRule.with(snapshot.getTableSnapshots()));
        } else
        {
            String materializationTxnList = snapshot != null ? snapshot.getValidTxnList() : null;
            modifiedQueryRel = augmentMaterializationWithTimeInformation(materialization, validTxnsList, new ValidTxnWriteIdList(materializationTxnList));
        }
     ...
     */
    private static RelNode augmentMaterializationWithTimeInformation(HiveRelOptMaterialization materialization, String validTxnsList, ValidTxnWriteIdList materializationTxnList) throws LockException
    {
        ...
        return applyRule(materialization.queryRel, new HiveAugmentMaterializationRule(rexBuilder, currentTxnList, materializationTxnList));
    }
    ...
    public static List<HiveRelOptMaterialization> deriveGroupingSetsMaterializedViews(HiveRelOptMaterialization materialization)
    {
        ...
        for (ImmutableBitSet targetGroupSet : aggregate.getGroupSets())
        {
            ...
            materializationList.add(new HiveRelOptMaterialization(newTableRel, newQueryRel, null, ImmutableList.of(scanTable.getDbName(), scanTable.getTableName(), "#" + materializationList.size()), materialization.getScope(), materialization.getRebuildMode(), materialization.getAst()));
        }
    ...
    public static RelNode copyNodeNewCluster(RelOptCluster optCluster, RelNode node)
    {
        if (node instanceof Filter)
        {
            ...
            return new HiveFilter(optCluster, f.getTraitSet(), copyNodeNewCluster(optCluster, f.getInput()), f.getCondition());
        } else if (node instanceof Project)
        {
            final Project p = (Project) node;
            return HiveProject.create(optCluster, copyNodeNewCluster(optCluster, p.getInput()), p.getProjects(), p.getRowType(), Collections.emptyList());
        } else
        {
    ...
    public static boolean checkPrivilegeForMaterializedViews(List<Table> cachedMVTableList) throws HiveException
    {
        ...
        for (Table cachedMVTable : cachedMVTableList)
        {
            ...
            HivePrivilegeObject privObject = new HivePrivilegeObject(cachedMVTable.getDbName(), cachedMVTable.getTableName(), colNames);
