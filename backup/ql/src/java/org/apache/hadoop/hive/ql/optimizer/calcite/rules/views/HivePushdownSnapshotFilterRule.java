...
public class HivePushdownSnapshotFilterRule extends RelRule<HivePushdownSnapshotFilterRule.Config>
{
    ...
    public static final RelOptRule INSTANCE = RelRule.Config.EMPTY.as(HivePushdownSnapshotFilterRule.Config.class).withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER).withOperandSupplier(operandBuilder -> operandBuilder.operand(HiveFilter.class).anyInputs()).withDescription("HivePushdownSnapshotFilterRule").toRule();
    ...
    static class SnapshotIdShuttle extends RexShuttle
    {
        ...
        {
            if (call.operands.size() == 2 && (setSnapShotId(call.operands.get(0), call.operands.get(1)) || setSnapShotId(call.operands.get(1), call.operands.get(0))))
            {
