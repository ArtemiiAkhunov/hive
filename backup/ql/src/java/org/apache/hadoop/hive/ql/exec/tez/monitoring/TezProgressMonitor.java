...
public class TezProgressMonitor implements ProgressMonitor
{
     ...
     */
    TezProgressMonitor(DAGClient dagClient, DAGStatus status, List<BaseWork> topSortedWork, Map<String, Progress> progressMap, SessionState.LogHelper console, long executionStartTime) throws IOException, TezException
    {
        this.status = status;
    ...
    public static class VertexProgress
    {
        ...
        boolean isRunning()
        {
            return succeededTaskCount < totalTaskCount && (succeededTaskCount > 0 || runningTaskCount > 0 || failedTaskAttemptCount > 0);
        }
