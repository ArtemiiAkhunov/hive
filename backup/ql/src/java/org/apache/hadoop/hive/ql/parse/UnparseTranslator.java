...
public class UnparseTranslator
{
     ...
     *
     * @param tableName source node (which must be an tabName) to be replaced
     */
     ...
     *
     * @param identifier source node (which must be an identifier) to be replaced
     */
    ...
    {
        if (!(exprNode.getType() == HiveParser.TOK_TABLE_OR_COL && exprNode.getChild(0).getType() == HiveParser.TOK_DEFAULT_VALUE))
        {
            return;
        }
    ...
    public void applyTranslations(TokenRewriteStream tokenRewriteStream, String programName)
    {
        ...
        {
            if (entry.getKey() > 0)
            { // negative means the key didn't exist in the original
                // stream (i.e.: we changed the tree)
                tokenRewriteStream.replace(programName, entry.getKey(), entry.getValue().tokenStopIndex, entry.getValue().replacementText);
            }
        }
        for (CopyTranslation copyTranslation : copyTranslations)
        {
            ...
            tokenRewriteStream.replace(programName, copyTranslation.targetNode.getTokenStartIndex(), copyTranslation.targetNode.getTokenStopIndex(), replacementText);
        }
    }
