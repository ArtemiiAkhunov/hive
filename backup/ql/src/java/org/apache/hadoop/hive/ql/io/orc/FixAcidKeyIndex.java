...
public class FixAcidKeyIndex
{
    ...
    public static AcidKeyIndexValidationResult validate(Configuration conf, Path inputPath) throws IOException
    {
        ...
        FileSystem fs = inputPath.getFileSystem(conf);
        try (Reader reader = OrcFile.createReader(fs, inputPath); RecordReader rr = reader.rows())
        {
            ...
            for (int i = 0; i < stripes.size(); i++)
            {
                ...
                if (result.isValid && (stripes.size() != keyIndex.length || keyIndex[i] == null || recordIdentifier.compareTo(keyIndex[i]) != 0))
                {
                    result.isValid = false;
                }
            }
    ...
    static Options createOptions()
    {
        ...
        result.addOption(OptionBuilder.withLongOpt("check-only").withDescription("Check acid orc file for valid acid key index and exit without fixing").create('c'));

        result.addOption(OptionBuilder.withLongOpt("recover").withDescription("Fix the acid key index for acid orc file if it requires fixing").create('r'));

        result.addOption(OptionBuilder.withLongOpt("backup-path").withDescription("specify a backup path to store the corrupted files (default: /tmp)").hasArg().create());
    ...
    public static Collection<String> getAllFilesInPath(final Path path, final Configuration conf) throws IOException
    {
        List<String> filesInPath = new ArrayList<>();
