...
public class ReplDumpTask extends Task<ReplDumpWork> implements Serializable
{
    ...
    {
        COMMON("common", "c_"), FOREIGNKEY("fk", "f_");
        ...
        try
        {
            ...
            } else
            {
                ...
                if (shouldDump(previousValidHiveDumpPath, isFailoverMarkerPresent, isFailoverTarget))
                {
                    ...
                    if (isFailoverTarget)
                    {
                        ...
                        } else
                        {
                            ...
                            boolean isAbortTxnsListPresent = checkFileExists(previousValidHiveDumpPath.getParent(), conf, OptimisedBootstrapUtils.ABORT_TXNS_FILE);
                            ...
                            dumpDbMetadata(work.dbNameOrPattern, new Path(hiveDumpRoot, EximUtil.METADATA_PATH_NAME), fromEventId, getHive());
                ...
                } else
                {
                    ...
                    ReplUtils.reportStatusInReplicationMetrics(getName(), Status.SKIPPED, null, conf, work.dbNameOrPattern, work.isBootstrap() ? ReplicationType.BOOTSTRAP : ReplicationType.INCREMENTAL);
                }
            }
    ...
    private void initiateAuthorizationDumpTask() throws SemanticException
    {
        if (RANGER_AUTHORIZER.equalsIgnoreCase(conf.getVar(HiveConf.ConfVars.REPL_AUTHORIZATION_PROVIDER_SERVICE)))
        {
            ...
            RangerDumpWork rangerDumpWork = new RangerDumpWork(rangerDumpRoot, work.dbNameOrPattern, work.getMetricCollector());
        ...
        {
            throw new SemanticException(ErrorMsg.REPL_INVALID_CONFIG_FOR_SERVICE.format("Authorizer " + conf.getVar(HiveConf.ConfVars.REPL_AUTHORIZATION_PROVIDER_SERVICE) + " not supported for replication ", ReplUtils.REPL_RANGER_SERVICE));
        }
    }
    ...
    private void addAtlasDumpTask(boolean bootstrap, Path prevHiveDumpDir)
    {
        ...
        Path prevAtlasDumpDir = prevHiveDumpDir == null ? null : new Path(prevHiveDumpDir.getParent(), ReplUtils.REPL_ATLAS_BASE_DIR);
        ...
        AtlasDumpWork atlasDumpWork = new AtlasDumpWork(work.dbNameOrPattern, atlasDumpDir, bootstrap, prevAtlasDumpDir, tableListLoc, work.getMetricCollector());
    ...
    private void finishRemainingTasks() throws HiveException
    {
        ...
        {
            Utils.create(new Path(work.getCurrentDumpPath(), ReplUtils.REPL_HIVE_BASE_DIR + File.separator + ReplAck.FAILOVER_READY_MARKER), conf);
            LOG.info("Dump marked as failover ready.");
        }
        Path dumpAckFile = new Path(work.getCurrentDumpPath(), ReplUtils.REPL_HIVE_BASE_DIR + File.separator + ReplAck.DUMP_ACKNOWLEDGEMENT);
        ...
        if (work.isSecondDumpAfterFailover())
        {
            if (database != null)
            {
                ...
                LOG.info("Removing {} property from the database {} after successful optimised bootstrap dump", String.join(",", new String[]{TARGET_OF_REPLICATION, CURR_STATE_ID_TARGET.toString(), CURR_STATE_ID_SOURCE.toString(), REPL_TARGET_DB_PROPERTY}), work.dbNameOrPattern);
        ...
        {
            work.getMetricCollector().reportEnd(isFirstCycleOfResume(database) ? Status.RESUME_READY : Status.SUCCESS);
        }
    ...
    private void deleteAllPreviousDumpMeta(Path currentDumpPath)
    {
        try
        {
            ...
            {
                FileStatus[] statuses = fs.listStatus(dumpRoot, path -> !path.equals(currentDumpPath) && !path.toUri().getPath().equals(currentDumpPath.toString()));
                ...
                {
                    Arrays.sort(statuses, (Comparator.<FileStatus>comparingLong(fileStatus1 -> fileStatus1.getModificationTime()).thenComparingLong(fileStatus2 -> fileStatus2.getModificationTime())));
                }
    ...
    {
        if (ReplDumpWork.testDeletePreviousDumpMetaPath && (conf.getBoolVar(HiveConf.ConfVars.HIVE_IN_TEST) || conf.getBoolVar(HiveConf.ConfVars.HIVE_IN_TEST_REPL)))
        {
    ...
    private Path getPreviousValidDumpMetadataPath(Path dumpRoot) throws IOException
    {
        ...
        if (fs.exists(dumpRoot))
        {
            ...
            for (FileStatus status : statuses)
            {
                ...
                } else if (validDump(status.getPath()) && status.getModificationTime() > latestValidStatus.getModificationTime())
                {
                    latestValidStatus = status;
                }
            }
        }
        Path latestDumpDir = (latestValidStatus == null) ? null : new Path(latestValidStatus.getPath(), ReplUtils.REPL_HIVE_BASE_DIR);
    ...
    private boolean shouldDump(Path previousDumpPath, boolean isFailoverMarkerPresent, boolean isFailover) throws IOException, HiveException
    {
        ...
        } else if (isFailover)
        {
            ...
            if (!isEventAckFilePresent)
            {
                ...
                LOG.debug("EVENT_ACK file not found in {}. Proceeding with OptimisedBootstrap Failover", previousDumpPath.getParent());
                return true;
            }
            ...
            if (!dbEventId.equalsIgnoreCase(fileEventId))
            {
                ...
                LOG.error("The database eventID {} and the event id in the EVENT_ACK file {} both mismatch. FilePath {}", dbEventId, fileEventId, previousDumpPath.getParent());
                throw new RuntimeException("Database event id changed post table diff generation.");
            } else
            {
                // Check table_diff_complete and Load_ACK
                return checkFileExists(previousDumpPath.getParent(), conf, TABLE_DIFF_COMPLETE_DIRECTORY) && checkFileExists(previousDumpPath, conf, LOAD_ACKNOWLEDGEMENT.toString());
            }
        } else
        {
    ...
    {
        return (previousReplScopeModified()) || !tablesForBootstrap.isEmpty() || conf.getBoolVar(HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES) || conf.getBoolVar(HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES);
    }
    ...
    {
        return conf.getBoolVar(HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES) && (!conf.getBoolVar(REPL_DUMP_METADATA_ONLY) && !conf.getBoolVar(HiveConf.ConfVars.REPL_DUMP_METADATA_ONLY_FOR_EXTERNAL_TABLE));
    }
    ...
    private boolean shouldBootstrapDumpExternalTable(String tableName)
    {
        ...
        return conf.getBoolVar(HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES) && (conf.getBoolVar(HiveConf.ConfVars.REPL_BOOTSTRAP_EXTERNAL_TABLES) || !ReplUtils.tableIncludedInReplScope(work.oldReplScope, tableName));
    }
    ...
    private boolean shouldBootstrapDumpAcidTable(String tableName)
    {
        ...
        // for the first time in current dump. So, need to check if table is included in old policy.
        return ReplUtils.includeAcidTableInDump(conf) && (conf.getBoolVar(HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES) || !ReplUtils.tableIncludedInReplScope(work.oldReplScope, tableName));
    }
    ...
    private boolean shouldBootstrapDumpTable(Table table)
    {
        ...
        if (AcidUtils.isTransactionalTable(table) && shouldBootstrapDumpAcidTable(table.getTableName()))
        {
            return true;
        }
    ...
    private boolean doesTableSatisfyConfig(Table table)
    {
        ...
        if (TableType.EXTERNAL_TABLE.equals(table.getTableType()) && !conf.getBoolVar(HiveConf.ConfVars.REPL_INCLUDE_EXTERNAL_TABLES))
        {
            return false;
        }

        if (AcidUtils.isTransactionalTable(table) && !ReplUtils.includeAcidTableInDump(conf))
        {
            return false;
        }
    ...
    private Long incrementalDump(Path dumpRoot, DumpMetaData dmd, Path cmRoot, Hive hiveDb) throws Exception
    {
        ...
        EventsDumpMetadata eventsDumpMetadata = Utils.fileExists(ackFile, conf) ? EventsDumpMetadata.deserialize(ackFile, conf) : new EventsDumpMetadata(work.eventFrom, 0, shouldBatch);
        ...
        try
        {
            ...
            if (db != null && db.getParameters() != null && Boolean.parseBoolean(db.getParameters().get(REPL_RESUME_STARTED_AFTER_FAILOVER)))
            {
            ...
            {
                throw new SemanticException(String.format("batch size configured via %s cannot be set to zero since batching is enabled", HiveConf.ConfVars.REPL_APPROX_MAX_LOAD_TASKS.varname));
            }
            if (eventsDumpMetadata.isEventsBatched() != shouldBatch)
            {
                LOG.error("Failed to resume from previous dump. {} was set to {} in previous dump but currently it's" + " set to {}. Cannot dump events in {} manner because they were {} batched in " + "the previous incomplete run", HiveConf.ConfVars.REPL_BATCH_INCREMENTAL_EVENTS.varname, eventsDumpMetadata.isEventsBatched(), shouldBatch, shouldBatch ? "batched" : "sequential", shouldBatch ? "not" : "");

                throw new HiveException(String.format("Failed to resume from previous dump. %s must be set to %s, but currently it's set to %s", HiveConf.ConfVars.REPL_BATCH_INCREMENTAL_EVENTS, eventsDumpMetadata.isEventsBatched(), shouldBatch));
            }
        ...
        } finally
        {
            ...
            if (work.isSecondDumpAfterFailover())
            {
                dmd.setDump(DumpType.OPTIMIZED_BOOTSTRAP, work.eventFrom, lastReplId, cmRoot, executionId, previousReplScopeModified());
            } else
            {
        ...
        try (FileList managedTblList = createTableFileList(dumpRoot, EximUtil.FILE_LIST, conf); FileList extTableFileList = createTableFileList(dumpRoot, EximUtil.FILE_LIST_EXTERNAL, conf); FileList snapPathFileList = isSnapshotEnabled ? createTableFileList(SnapshotUtils.getSnapshotFileListPath(dumpRoot), EximUtil.FILE_LIST_EXTERNAL_SNAPSHOT_CURRENT, conf) : null)
        {
                    ...
                    for (String tableName : Utils.matchesTbl(hiveDb, matchedDbName, work.replScope))
                    {
                        try
                        {
                            ...
                            if (shouldBootstrapDumpTable(table))
                            {
                                ...
                                dumpTable(exportService, matchedDbName, tableName, validTxnList, dbRootMetadata, dbRootData, bootDumpBeginReplId, hiveDb, tableTuple, managedTblList, dataCopyAtLoad);
                            }
    ...
    private ReplicationMetricCollector initReplicationDumpMetricCollector(Path dumpRoot, boolean isBootstrap, boolean isPreOptimisedBootstrap, boolean isFailover) throws HiveException
    {
        ...
        } else if (isFailover)
        {
            ...
            String failoverType = MetaStoreUtils.isDbBeingPlannedFailedOver(getHive().getDatabase(work.dbNameOrPattern)) ? ReplConst.FailoverType.PLANNED.toString() : ReplConst.FailoverType.UNPLANNED.toString();
            if (isPreOptimisedBootstrap)
            {
                collector = new PreOptimizedBootstrapDumpMetricCollector(work.dbNameOrPattern, dumpRoot.toString(), conf, executorId, MetaStoreUtils.FailoverEndpoint.SOURCE.toString(), failoverType);
            } else
            {
                collector = new OptimizedBootstrapDumpMetricCollector(work.dbNameOrPattern, dumpRoot.toString(), conf, executorId, MetaStoreUtils.FailoverEndpoint.SOURCE.toString(), failoverType);
            }
        } else
        {
    ...
    private int getMaxEventAllowed(int currentEventMaxLimit)
    {
        ...
        if (maxDirItems > 0)
        {
            ...
            {
                LOG.warn("Changing the maxEventLimit from {} to {} as the '" + ReplUtils.DFS_MAX_DIR_ITEMS_CONFIG + "' limit encountered. Set this config appropriately to increase the maxEventLimit", currentEventMaxLimit, maxDirItems);
    ...
    private boolean needBootstrapAcidTablesDuringIncrementalDump()
    {
        ...
        return !work.replScope.includeAllTables() || previousReplScopeModified() || !tablesForBootstrap.isEmpty() || conf.getBoolVar(HiveConf.ConfVars.REPL_BOOTSTRAP_ACID_TABLES);
    }

    private void dumpEvent(NotificationEvent ev, Path evRoot, Path dumpRoot, Path cmRoot, Hive db, EventsDumpMetadata eventsDumpMetadata) throws Exception
    {
        EventHandler.Context context = new EventHandler.Context(evRoot, dumpRoot, cmRoot, db, conf, getNewEventOnlyReplicationSpec(ev.getEventId()), work.replScope, work.oldReplScope, tablesForBootstrap);
                ...
                try
                {
                    ...
                    for (String tblName : Utils.matchesTbl(hiveDb, dbName, work.replScope))
                    {
                        ...
                        try
                        {
                            ...
                            dumpTable(exportService, dbName, tblName, validTxnList, dbRoot, dbDataRoot, bootDumpBeginReplId, hiveDb, tableTuple, managedTblList, dataCopyAtLoad);
                        } catch (InvalidTableException te)
                        {
    ...
    private void setReplSourceFor(Hive hiveDb, String dbName, Database db) throws HiveException
    {
        if (!ReplChangeManager.isSourceOfReplication(db))
        {
            ...
            // as default.
            String value = conf.get(SCHEDULED_QUERY_SCHEDULENAME, "default_" + getQueryState().getQueryString());
        ...
        } else
        {
            ...
            if (!StringUtils.isEmpty(scheduleQuery))
            {
                ...
                {
                    updateReplSourceFor(hiveDb, dbName, db, getReplPolicyIdString(db) + ", " + scheduleQuery);
                }
            }
        }
    }
    ...
    void dumpTable(ExportService exportService, String dbName, String tblName, String validTxnList, Path dbRootMetadata, Path dbRootData, long lastReplId, Hive hiveDb, HiveWrapper.Tuple<Table> tuple, FileList managedTbleList, boolean dataCopyAtLoad) throws Exception
    {
        LOG.info("Bootstrap Dump for table " + tblName);
        TableSpec tableSpec = new TableSpec(tuple.object);
        TableExport.Paths exportPaths = new TableExport.Paths(work.astRepresentationForErrorMsg, dbRootMetadata, dbRootData, tblName, conf, true);
    ...
    private String getValidWriteIdList(String dbName, String tblName, String validTxnString) throws LockException
    {
        ...
        ValidWriteIdList validWriteIds = getTxnMgr().getValidWriteIds(Collections.singletonList(fullTableName), validTxnString).getTableValidWriteIdList(fullTableName);
    ...
    String getValidTxnListForReplDump(Hive hiveDb, long waitUntilTime) throws HiveException
    {
        ...
        if (conf.getBoolVar(REPL_BOOTSTRAP_DUMP_ABORT_WRITE_TXN_AFTER_TIMEOUT))
        {
            ...
            if (!openTxns.isEmpty())
            {
                ...
                if (!openTxns.isEmpty())
                {
                    LOG.warn("REPL DUMP unable to force abort all the open txns: {} after timeout due to unknown reasons. " + "However, this is rare case that shouldn't happen.", openTxns);
    ...
    {
        return (conf.getBoolVar(HiveConf.ConfVars.HIVE_IN_TEST) || conf.getBoolVar(HiveConf.ConfVars.HIVE_IN_TEST_REPL)) ? SLEEP_TIME_FOR_TESTS : SLEEP_TIME;
    }
    ...
    private HiveWrapper.Tuple<Function> functionTuple(String functionName, String dbName, Hive hiveDb)
    {
        try
        {
            ...
            {
                LOG.warn("Not replicating function: " + functionName + " as it seems to have been created " + "without USING clause");
                return null;
            }
            return tuple;
