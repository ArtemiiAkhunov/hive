...
public class HiveProjectFilterPullUpConstantsRule extends RelOptRule
{
    ...
    {
        super(operand(projectClass, operand(filterClass, any())), relBuilderFactory, null);
    }
        ...
        for (RexNode conjunction : conjunctions)
        {
            ...
            switch (conjCall.getOperator().getKind())
            {
                case EQUALS:
                    ...
                    break;
                case IS_NULL:
                    conditions.put(conjCall.operands.get(0).toString(), relBuilder.getRexBuilder().makeNullLiteral(conjCall.operands.get(0).getType()));
            }
        }
