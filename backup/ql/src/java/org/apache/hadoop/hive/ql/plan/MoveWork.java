...
public class MoveWork implements Serializable, BaseCopyWork
{
    ...
    public MoveWork(Set<ReadEntity> inputs, Set<WriteEntity> outputs, final LoadTableDesc loadTableWork, final LoadFileDesc loadFileWork, boolean checkFileFormat, boolean srcLocal)
    {
        this(inputs, outputs);
    ...
    public MoveWork(Set<ReadEntity> inputs, Set<WriteEntity> outputs, final LoadTableDesc loadTableWork, final LoadFileDesc loadFileWork, boolean checkFileFormat)
    {
        this(inputs, outputs, loadTableWork, loadFileWork, checkFileFormat, false);
    }

    public MoveWork(boolean isCTAS, Set<ReadEntity> inputs, Set<WriteEntity> outputs, final LoadTableDesc loadTableWork, final LoadFileDesc loadFileWork, boolean checkFileFormat)
    {
        this(inputs, outputs, loadTableWork, loadFileWork, checkFileFormat);
        this.isCTAS = isCTAS;
    }

    public MoveWork(Set<ReadEntity> inputs, Set<WriteEntity> outputs, final LoadTableDesc loadTableWork, final LoadFileDesc loadFileWork, boolean checkFileFormat, String dumpRoot, ReplicationMetricCollector metricCollector, boolean isReplication)
    {
        this(inputs, outputs, loadTableWork, loadFileWork, checkFileFormat, false);
    ...
    public void initializeFromDeferredContext(DeferredWorkContext deferredContext) throws HiveException
    {
        ...
        {
            LoadMultiFilesDesc loadFilesWork = new LoadMultiFilesDesc(Collections.singletonList(deferredContext.destPath), Collections.singletonList(deferredContext.tgtPath), true, null, null);
            setMultiFilesDesc(loadFilesWork);
            setNeedCleanTarget(deferredContext.replace);
        } else
        {
            LoadTableDesc loadTableWork = new LoadTableDesc(deferredContext.loadPath, Utilities.getTableDesc(deferredContext.table), new TreeMap<>(), deferredContext.loadFileType, deferredContext.writeId);
