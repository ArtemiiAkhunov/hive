...
public class GenTezUtils
{
    ...
    {
        return context.getOperation() == Context.Operation.DELETE && context.getLoadTableOutputMap().values().stream().map(WriteEntity::getTable).anyMatch(DDLUtils::isIcebergTable);
    }
    ...
    public static ReduceWork createReduceWork(GenTezProcContext context, Operator<?> root, TezWork tezWork)
    {
        ...
        float minSrcFraction = context.conf.getFloat(ShuffleVertexManager.TEZ_SHUFFLE_VERTEX_MANAGER_MIN_SRC_FRACTION, ShuffleVertexManager.TEZ_SHUFFLE_VERTEX_MANAGER_MIN_SRC_FRACTION_DEFAULT);
        reduceWork.setMinSrcFraction(minSrcFraction);
        float maxSrcFraction = context.conf.getFloat(ShuffleVertexManager.TEZ_SHUFFLE_VERTEX_MANAGER_MAX_SRC_FRACTION, ShuffleVertexManager.TEZ_SHUFFLE_VERTEX_MANAGER_MAX_SRC_FRACTION_DEFAULT);
    ...
    public static void removeSemiJoinOperator(ParseContext context, ReduceSinkOperator rs, TableScanOperator ts) throws SemanticException
    {
        ...
        {
            collectDynamicValuePredicates(ts.getConf().getFilterExpr(), filterDynamicValuePredicatesCollection);
        ...
        for (Operator<?> op : ts.getChildOperators())
        {
            ...
            collectDynamicValuePredicates(filterDesc.getPredicate(), filterDynamicValuePredicatesCollection);
            for (ExprNodeDesc nodeToRemove : filterDynamicValuePredicatesCollection.childParentMapping.keySet())
            {
                // Find out if this synthetic predicate belongs to the current cycle
                if (removeSemiJoinPredicate(context, rs, nodeToRemove))
    ...
    private static class DynamicValuePredicateProc implements SemanticNodeProcessor
    {
        ...
        public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx, Object... nodeOutputs) throws SemanticException
        {
            ...
            if (parent instanceof ExprNodeGenericFuncDesc)
            {
                ...
                if (parentFunc.getGenericUDF() instanceof GenericUDFBetween || parentFunc.getGenericUDF() instanceof GenericUDFInBloomFilter)
                {
    ...
    private static Integer obtainBufferSize(Operator<?> op, ReduceSinkOperator rsOp, int defaultTinyBufferSize)
    {
        if (op instanceof GroupByOperator)
        {
            ...
            if (groupByOperator.getConf().getKeys().isEmpty() && groupByOperator.getConf().getMode() == GroupByDesc.Mode.MERGEPARTIAL)
            {
                // Check configuration and value is -1, infer value
                int result = defaultTinyBufferSize == -1 ? (int) Math.ceil(groupByOperator.getStatistics().getDataSize() / 1E6) : defaultTinyBufferSize;
