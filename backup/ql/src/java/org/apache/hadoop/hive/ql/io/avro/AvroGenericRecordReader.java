...
public class AvroGenericRecordReader implements RecordReader<NullWritable, AvroGenericRecordWritable>, JobConfigurable
{
    ...
    private ZoneId extractWriterTimezoneFromMetadata(JobConf job, FileSplit split, GenericDatumReader<GenericRecord> gdr)
    {
        ...
        try (DataFileReader<GenericRecord> dataFileReader = new DataFileReader<GenericRecord>(new FsInput(split.getPath(), job), gdr))
        {
            if (dataFileReader.getMeta(AvroSerDe.WRITER_TIME_ZONE) != null)
            {
                try
                {
                    return ZoneId.of(new String(dataFileReader.getMeta(AvroSerDe.WRITER_TIME_ZONE), StandardCharsets.UTF_8));
    ...
    private Boolean extractWriterProlepticFromMetadata(JobConf job, FileSplit split, GenericDatumReader<GenericRecord> gdr)
    {
        if (job == null || gdr == null || split == null || split.getPath() == null)
        {
            return null;
        ...
        try (DataFileReader<GenericRecord> dataFileReader = new DataFileReader<GenericRecord>(new FsInput(split.getPath(), job), gdr))
        {
            if (dataFileReader.getMeta(AvroSerDe.WRITER_PROLEPTIC) != null)
            {
                try
                {
                    return Boolean.valueOf(new String(dataFileReader.getMeta(AvroSerDe.WRITER_PROLEPTIC), StandardCharsets.UTF_8));
                } catch (DateTimeException e)
                {
    ...
    private Boolean extractWriterZoneConversionLegacy(JobConf job, FileSplit split, GenericDatumReader<GenericRecord> gdr)
    {
        assert job != null;
        assert split != null;
        assert gdr != null;
