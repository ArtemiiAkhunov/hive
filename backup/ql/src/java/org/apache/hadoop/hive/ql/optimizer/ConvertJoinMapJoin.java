...
public class ConvertJoinMapJoin implements SemanticNodeProcessor
{
    ...
    {
        return computeOnlineDataSizeGeneric(statistics, -8, // the long key is stored in a slot
    ...
    {
        return computeOnlineDataSizeGeneric(statistics, 5 + 4, // list header ; value length stored as vint
    ...
    public long computeOnlineDataSizeOptimized(Statistics statistics)
    {
        ...
        return computeOnlineDataSizeGeneric(statistics, 2 * 6, // 2 offsets are stored using:  LazyBinaryUtils.writeVLongToByteArray
                8 // maintenance structure consists of 1 long
        );
    }
    ...
    @VisibleForTesting
    public MemoryMonitorInfo getMemoryMonitorInfo(final HiveConf conf, LlapClusterStateForCompile llapInfo)
    {
    ...
    private boolean checkConvertJoinSMBJoin(JoinOperator joinOp, OptimizeTezProcContext context, int bigTablePosition, TezBucketJoinProcCtx tezBucketJoinProcCtx) throws SemanticException
    {
        ...
        for (Operator<? extends OperatorDesc> parentOp : joinOp.getParentOperators())
        {
            ...
            if (op instanceof ReduceSinkOperator)
            {
                boolean hasUniformTrait = ((ReduceSinkOperator) op).getConf().getReducerTraits().contains(ReduceSinkDesc.ReducerTraits.UNIFORM);
    ...
    private boolean checkConvertJoinBucketMapJoin(JoinOperator joinOp, int bigTablePosition, TezBucketJoinProcCtx tezBucketJoinProcCtx) throws SemanticException
    {
        ...
        if (shouldCheckExternalTables)
        {
            ...
            for (Operator<?> parentOp : joinOp.getParentOperators())
            {
                ...
                {
                    LOG.debug("External table {} found in join and also could not provide statistics - " + "disabling bucket map join.", sb);
                    return false;
                }
            }
        }
    ...
    public MapJoinConversion getMapJoinConversion(JoinOperator joinOp, OptimizeTezProcContext context, int buckets, boolean skipJoinTypeChecks, long maxSize, boolean checkMapJoinThresholds) throws SemanticException
    {
        ...
        Set<Integer> bigTableCandidateSet = MapJoinProcessor.getBigTableCandidates(conds, /* isSupportFullOuter */ true);
        ...
        if (cartesianProductEdgeEnabled && !hasOuterJoin(joinOp) && isCrossProduct(joinOp))
        {
            for (int i = 0; i < joinOp.getParentOperators().size(); i++)
            {
                if (i != bigTablePosition)
                {
                    ...
                    if (parentStats.getNumRows() > HiveConf.getIntVar(context.conf, HiveConf.ConfVars.XPROD_SMALL_TABLE_ROWS_THRESHOLD))
                    {
    ...
    public MapJoinOperator convertJoinMapJoin(JoinOperator joinOp, OptimizeTezProcContext context, MapJoinConversion mapJoinConversion, boolean removeReduceSink) throws SemanticException
    {
        ...
        mapJoinDesc.setHybridHashJoin(HiveConf.getBoolVar(context.conf, HiveConf.ConfVars.HIVE_USE_HYBRIDGRACE_HASHJOIN));
    ...
    {
        if (context.conf.getBoolVar(HiveConf.ConfVars.HIVE_CONVERT_JOIN) && context.conf.getBoolVar(HiveConf.ConfVars.HIVE_DYNAMIC_PARTITION_HASHJOIN))
        {
    ...
    {
        MapJoinConversion mapJoinConversion = getMapJoinConversion(joinOp, context, estimateNumBuckets(joinOp, false), true, Long.MAX_VALUE, false);
        final int pos;
    ...
    {
        long max = HiveConf.getLongVar(context.parseContext.getConf(), HiveConf.ConfVars.HIVE_CONVERT_JOIN_MAX_ENTRIES_HASHTABLE);
        if (max < 1)
        {
    ...
    {
        long max = HiveConf.getLongVar(context.parseContext.getConf(), HiveConf.ConfVars.HIVE_CONVERT_JOIN_MAX_SHUFFLE_SIZE);
        if (max < 1)
        {
