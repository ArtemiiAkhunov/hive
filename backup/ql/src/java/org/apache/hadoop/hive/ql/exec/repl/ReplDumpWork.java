...
public class ReplDumpWork implements Serializable
{
    ...
    void overrideLastEventToDump(Hive fromDb, long bootstrapLastId, long failoverEventId) throws Exception
    {
        ...
        if (bootstrapLastId > 0)
        {
            ...
            LoggerFactory.getLogger(this.getClass()).debug("eventTo restricted to event id : {} because of bootstrap of ACID tables", eventTo);
            return;
        }
    ...
    {
        return externalTblCopyPathIterator != null || managedTblCopyPathIterator != null || functionCopyPathIterator != null;
    }
    ...
    public List<Task<?>> functionsBinariesCopyTasks(TaskTracker tracker, HiveConf conf)
    {
        ...
        if (functionCopyPathIterator != null)
        {
            while (functionCopyPathIterator.hasNext() && tracker.canAddMoreTasks())
            {
                ...
                Task<?> copyTask = ReplCopyTask.getDumpCopyTask(binaryCopyPath.getReplicationSpec(), binaryCopyPath.getSrcPath(), binaryCopyPath.getTargetPath(), conf, getCurrentDumpPath().toString(), getMetricCollector());
                tasks.add(copyTask);
                tracker.addTask(copyTask);
