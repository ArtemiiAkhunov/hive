...
public class FetchOperator implements Serializable
{
    ...
    private void ensureCorrectSchemaEvolutionConfigs(JobConf conf)
    {
        ...
        boolean isForcePositionalEvolution = HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_ORC_FORCE_POSITIONAL_SCHEMA_EVOLUTION);

        if (!isSchemaEvolution && isForcePositionalEvolution)
        {
            LOG.warn("invalid schema evolution ({}) and force positional evolution ({}) pair, falling back to disabling both", isSchemaEvolution, isForcePositionalEvolution);
    ...
    private void processCurrPathForMmWriteIds(InputFormat inputFormat, List<Path> dirs, List<Path> dirsWithOriginals) throws IOException
    {
        ...
        HiveInputFormat.processPathsForMmRead(Lists.newArrayList(currPath), job, validWriteIdList, dirs, dirsWithOriginals);
    }
    ...
    private String makeInputString(List<Path> dirs)
    {
        ...
        // remove duplicates
        return dirs.stream().map(dir -> StringUtils.escapeString(dir.toString())).distinct().collect(Collectors.joining(","));
    ...
    {
        return hasVC ? ObjectInspectorFactory.getUnionStructObjectInspector(Arrays.asList(valueOI, vcsOI)) : valueOI;
    }

    private StructObjectInspector getPartitionedRowOI(StructObjectInspector valueOI)
    {
        return ObjectInspectorFactory.getUnionStructObjectInspector(hasVC ? Arrays.asList(valueOI, partKeyOI, vcsOI) : Arrays.asList(valueOI, partKeyOI));
    }
