...
public class ScheduledQueryExecutionService implements Closeable
{
    ...
    public static ScheduledQueryExecutionService startScheduledQueryExecutorService(ScheduledQueryExecutionContext ctx)
    {
        synchronized (ScheduledQueryExecutionService.class)
        {
            if (INSTANCE != null)
            {
                throw new IllegalStateException("There is already a ScheduledQueryExecutionService in service; check it and close it explicitly if necessary");
            }
    ...
    class ScheduledQueryExecutor implements Runnable
    {
        ...
        {
            return String.format("Scheduled Query Executor(schedule:%s, execution_id:%d)", pollResponse.getScheduleKey().getScheduleName(), pollResponse.getExecutionId());
        }
        ...
        public synchronized void reportQueryProgress()
        {
            if (info != null)
            {
                LOG.info("Reporting query progress of {} as {} err:{}", info.getScheduledExecutionId(), info.getState(), info.getErrorMessage());
