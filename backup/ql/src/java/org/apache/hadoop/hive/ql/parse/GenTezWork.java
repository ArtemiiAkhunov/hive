...
public class GenTezWork implements SemanticNodeProcessor
{
        ...
        if (context.leafOperatorToFollowingWork.containsKey(operator))
        {
            ...
            } else
            {
                ...
                if (!context.connectedReduceSinks.contains(rs))
                {
                    ...
                    {
                        edgeProp = new TezEdgeProperty(context.conf, edgeType, true, rWork.isSlowStart(), rWork.getMinReduceTasks(), rWork.getMaxReduceTasks(), bytesPerReducer, rWork.getMinSrcFraction(), rWork.getMaxSrcFraction());
                    } else
                    {
    ...
    {
        return context.opMergeJoinWorkMap.values().stream().filter(mergeJoinWork -> mergeJoinWork.getBaseWorkList().contains(reduceWork)).map(MergeJoinWork::getMainWork)
                ...
                .filter(Objects::nonNull).findAny().orElse(reduceWork).getName();
    }
}
