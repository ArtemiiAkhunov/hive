...
public class BootstrapLoadLogger extends ReplLogger<String>
{
    ...
    {
        tableSeqNo++;
        (new BootstrapLoadTable(dbName, tableName, tableType, tableSeqNo, numTables)).log(LogTag.TABLE_LOAD);
    }
    ...
    {
        functionSeqNo++;
        (new BootstrapLoadFunction(dbName, funcName, functionSeqNo, numFunctions)).log(LogTag.FUNCTION_LOAD);
    }

    @Override
    public void endLog(String lastReplId)
    {
        (new BootstrapLoadEnd(dbName, numTables, numFunctions, dumpDir, lastReplId)).log(LogTag.END);
    }
