...
public class OptimisedBootstrapUtils
{
    ...
    public static Long createAndGetEventAckFile(Path currentDumpPath, DumpMetaData dmd, Path cmRoot, String dbEventId, String targetDbEventId, HiveConf conf, ReplDumpWork work) throws Exception
    {
        ...
        LOG.info("Created event_ack file at {} with source eventId {} and target eventId {}", filePath, dbEventId, targetDbEventId);
    ...
    {
        List<NotificationEvent> notificationEvents = hiveDb.getMSC().getNextNotification(eventId - 1, -1, new DatabaseAndTableFilter(work.dbNameToLoadIn, null)).getEvents();
    ...
    public static void prepareTableDiffFile(List<NotificationEvent> notificationEvents, Hive hiveDb, ReplLoadWork work, HiveConf conf) throws Exception
    {
        ...
        for (NotificationEvent event : notificationEvents)
        {
            ...
            if (tableName != null)
            {
                LOG.debug("Added table {} because of eventId {} and eventType {}", event.getTableName(), event.getEventId(), event.getEventType());
    ...
    public static String getTargetEventId(String dbName, Hive hiveDb) throws Exception
    {
        ...
        List<NotificationEvent> events = hiveDb.getMSC().getNextNotification(Long.parseLong(targetLastEventId) - 1, 1, null).getEvents();
    ...
    private static void buildListingForDirectory(ArrayList<String> listing, Path tableLocation, FileSystem tableFs) throws IOException
    {
        ...
        while (itr.hasNext())
        {
            ...
            {
                listing.add(fstatus.getPath() + FILE_ENTRY_SEPARATOR + fstatus.getLen() + FILE_ENTRY_SEPARATOR + tableFs.getFileChecksum(fstatus.getPath()));
            }
        }
    }
}
