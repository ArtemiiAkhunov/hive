...
public class UpgradeTool
{
    ...
    private void init()
    {
        try
        {
            cmdLineOptions.addOption(new Option("help", "Generates a script to execute on 3.x " + "cluster.  This requires 3.x binaries on the classpath and hive-site.xml."));
    ...
    private static IMetaStoreClient getHMS(HiveConf conf)
    {
        ...
        {
            throw new RuntimeException("Error connecting to Hive Metastore URI: " + conf.getVar(HiveConf.ConfVars.METASTORE_URIS) + ". " + e.getMessage(), e);
        }
    }
    ...
    public static int guessNumBuckets(long partitionSizeInBytes)
    {
        ...
        if (partitionSizeInBytes <= 1000 * OneGB)
        {//TB
            return 16;
        }
        if (partitionSizeInBytes <= 10 * 1000 * OneGB)
        {//10 TB
            return 32;
        }
        if (partitionSizeInBytes <= 100 * 1000 * OneGB)
        {//100TB
            return 64;
        }
        if (partitionSizeInBytes <= 1000 * 1000 * OneGB)
        {//PT
            return 128;
        }
        if (partitionSizeInBytes <= 10 * 1000 * 1000 * OneGB)
        {//10 PT
            return 256;
        }
        if (partitionSizeInBytes <= 100 * 1000 * 1000 * OneGB)
        {//100 PT
            return 512;
        }
        if (partitionSizeInBytes <= 1000 * 1000 * 1000 * OneGB)
        {//1000 PT
            return 1024;
        }
        return 2048;
     ...
     */
    private static void processConversion(Table t, List<String> convertToAcid, List<String> convertToMM, IMetaStoreClient hms, Hive db, boolean execute, PrintWriter pw) throws TException, HiveException, IOException
    {
        ...
        if (t.getPartitionKeysSize() <= 0)
        {
            if (canBeMadeAcid)
            {
                ...
                handleRenameFiles(t, new Path(t.getSd().getLocation()), execute, db.getConf(), t.getSd().getBucketColsSize() > 0, pw);
                if (execute)
                {
        ...
        } else
        {
            ...
            for (int i = 0; i < numWholeBatches; i++)
            {
                ...
                {
                    handleRenameFiles(t, new Path(part.getSd().getLocation()), execute, db.getConf(), t.getSd().getBucketColsSize() > 0, pw);
                }
            }
