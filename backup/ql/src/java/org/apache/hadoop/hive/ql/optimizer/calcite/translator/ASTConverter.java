...
public class ASTConverter
{
    ...
    public static ASTNode convert(final RelNode relNode, List<FieldSchema> resultSchema, boolean alignColumns, PlanMapper planMapper) throws CalciteSemanticException
    {
    ...
    public static ASTNode emptyPlan(RelDataType dataType)
    {
        ...
        ASTNode insert = ASTBuilder.construct(HiveParser.TOK_INSERT, "TOK_INSERT").add(ASTBuilder.destNode()).add(select).add(ASTBuilder.limit(0, 0)).node();

        return ASTBuilder.construct(HiveParser.TOK_QUERY, "TOK_QUERY").add(insert).node();
    }
    ...
    private static ASTNode createNullField(RelDataType fieldType)
    {
        ...
        return ASTBuilder.construct(HiveParser.TOK_FUNCTION, "TOK_FUNCTION").add(astNode).add(HiveParser.TOK_NULL, "TOK_NULL").node();
    }
    ...
    static ASTNode convertType(RelDataType fieldType)
    {
        ...
        if (fieldType.getSqlTypeName() == SqlTypeName.ROW)
        {
            ...
            {
                ASTNode colNode = ASTBuilder.construct(HiveParser.TOK_TABCOL, "TOK_TABCOL").add(HiveParser.Identifier, structFieldType.getName()).add(convertType(structFieldType.getType())).node();
    ...
    {
        RexInputRef iRef = new RexInputRef(i, root.getCluster().getTypeFactory().createSqlType(SqlTypeName.ANY));
    ...
    private static ASTNode createASTLateralView(TableFunctionScan tfs, Schema s, QueryBlockInfo tableFunctionSource, String sqAlias)
    {
        ...
        {
            ASTNode expr = rn.accept(new RexVisitor(s, rn instanceof RexLiteral, tfs.getCluster().getRexBuilder()));
            children.add(expr);
        }
        ASTNode function = buildUDTFAST(call.getOperator().getName(), children);
    ...
    static class RexVisitor extends RexVisitorImpl<ASTNode>
    {
        ...
        protected RexVisitor(Schema schema, boolean useTypeQualInLiteral, RexBuilder rexBuilder)
        {
            ...
            this.nullLiteralMap = new TreeMap<>(new Comparator<RexLiteral>()
            {
        ...
        public ASTNode visitLiteral(RexLiteral literal)
        {
            ...
            if (RexUtil.isNull(literal) && literal.getType().getSqlTypeName() != SqlTypeName.NULL && rexBuilder != null)
            {
        ...
        public ASTNode visitCall(RexCall call)
        {
            ...
            switch (op.kind)
            {
                ...
                case GREATER_THAN_OR_EQUAL:
                    if (rexBuilder != null && RexUtil.isReferenceOrAccess(call.operands.get(1), true) && RexUtil.isLiteral(call.operands.get(0), true))
                    {
                ...
                case IS_DISTINCT_FROM:
                    ...
                    return SqlFunctionConverter.buildAST(SqlStdOperatorTable.NOT, Collections.singletonList(SqlFunctionConverter.buildAST(SqlStdOperatorTable.IS_NOT_DISTINCT_FROM, astNodeLst, call.getType())), call.getType());
                case CAST:
            ...
            for (AggregateCall agg : aggs)
            {
                ...
                for (int i : agg.getArgList())
                {
                    ...
                    if (collationIterator.hasNext())
                    {
                        ...
                        b.add(ASTBuilder.createAST(HiveParser.NumberLiteral, Integer.toString(DirectionUtils.directionToCode(fieldCollation.getDirection()))));
                        b.add(ASTBuilder.createAST(HiveParser.NumberLiteral, Integer.toString(NullOrdering.fromDirection(fieldCollation.nullDirection).getCode())));
                    }
                }
    ...
    static class HiveAST
    {
        ...
        {
            ASTBuilder b = ASTBuilder.construct(HiveParser.TOK_QUERY, "TOK_QUERY").add(from).add(ASTBuilder.construct(HiveParser.TOK_INSERT, "TOK_INSERT").add(ASTBuilder.destNode()).add(select).add(where).add(groupBy).add(having).add(order).add(limit));
            return b.node();
        }
    }
    ...
    public ASTNode getUnionAllAST(ASTNode leftAST, ASTNode rightAST)
    {
        ...
        ASTNode unionTokAST = ASTBuilder.construct(HiveParser.TOK_UNIONALL, "TOK_UNIONALL").add(leftAST).add(rightAST).node();
