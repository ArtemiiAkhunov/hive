...
public class VectorizationContext
{
    ...
    public enum HiveVectorAdaptorUsageMode
    {
        ...
        {
            String string = HiveConf.getVar(hiveConf, HiveConf.ConfVars.HIVE_VECTOR_ADAPTOR_USAGE_MODE);
    ...
    public enum HiveVectorIfStmtMode
    {
        ...
        {
            String string = HiveConf.getVar(hiveConf, HiveConf.ConfVars.HIVE_VECTORIZED_IF_EXPR_MODE);
            return valueOf(string.toUpperCase());
        }
    }
    ...
    private boolean reuseScratchColumns = HiveConf.ConfVars.HIVE_VECTORIZATION_TESTING_REUSE_SCRATCH_COLUMNS.defaultBoolVal;
    ...
    private void setHiveConfVars(HiveConf hiveConf)
    {
        ...
        testVectorAdaptorOverride = HiveConf.getBoolVar(hiveConf, ConfVars.HIVE_TEST_VECTOR_ADAPTOR_OVERRIDE);
        hiveVectorIfStmtMode = HiveVectorIfStmtMode.getHiveConfValue(hiveConf);
        this.reuseScratchColumns = HiveConf.getBoolVar(hiveConf, ConfVars.HIVE_VECTORIZATION_TESTING_REUSE_SCRATCH_COLUMNS);
        this.ocm.setReuseColumns(reuseScratchColumns);
        useCheckedVectorExpressions = HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVE_VECTORIZATION_USE_CHECKED_EXPRESSIONS);
        adaptorSuppressEvaluateExceptions = HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVE_VECTORIZED_ADAPTOR_SUPPRESS_EVALUATE_EXCEPTIONS);
    }
    ...
    public static final Pattern decimalTypePattern = Pattern.compile("decimal.*", Pattern.CASE_INSENSITIVE);

    public static final Pattern charTypePattern = Pattern.compile("char.*", Pattern.CASE_INSENSITIVE);

    public static final Pattern varcharTypePattern = Pattern.compile("varchar.*", Pattern.CASE_INSENSITIVE);

    public static final Pattern charVarcharTypePattern = Pattern.compile("char.*|varchar.*", Pattern.CASE_INSENSITIVE);

    public static final Pattern structTypePattern = Pattern.compile("struct.*", Pattern.CASE_INSENSITIVE);

    public static final Pattern listTypePattern = Pattern.compile("array.*", Pattern.CASE_INSENSITIVE);

    public static final Pattern mapTypePattern = Pattern.compile("map.*", Pattern.CASE_INSENSITIVE);
    ...
    private static final long[] POWEROFTENTABLE = {1L,                   // 0
            10L, 100L, 1_000L, 10_000L, 100_000L, 1_000_000L, 10_000_000L, 100_000_000L,           // 8
            1_000_000_000L, 10_000_000_000L, 100_000_000_000L, 1_000_000_000_000L, 10_000_000_000_000L, 100_000_000_000_000L, 1_000_000_000_000_000L, 10_000_000_000_000_000L,   // 16
            100_000_000_000_000_000L, 1_000_000_000_000_000_000L, // 18
    };
    ...
    private static class OutputColumnManager
    {
        ...
        private int allocateOutputColumnInternal(String columnType, DataTypePhysicalVariation dataTypePhysicalVariation)
        {
            for (int i = 0; i < outputColCount; i++)
            {
                ...
                if (usedOutputColumns.contains(i) || !(scratchVectorTypeNames[i].equalsIgnoreCase(columnType) && scratchDataTypePhysicalVariations[i] == dataTypePhysicalVariation))
                {
                    continue;
                }
        ...
        public String toString()
        {
            ...
            builder.append(String.format("OutputColumnManager: initialOutputCol: %d, outputColCount: %d, usedOutputColumns#: %d" + ", reuseScratchColumns: %s, output cols:", initialOutputCol, outputColCount, usedOutputColumns.size(), reuseScratchColumns));
            for (int i = 0; i < outputColCount; i++)
            {
                builder.append(String.format("\n%d (%d): used: %s, scratchVectorTypeName: %s" + ", physicalVariation: %s, trackWasUsed: %s", i, i + initialOutputCol, usedOutputColumns.contains(i), scratchVectorTypeNames[i], scratchDataTypePhysicalVariations[i], scratchColumnTrackWasUsed[i]));
            }
    ...
    public VectorExpression getVectorExpression(ExprNodeDesc exprDesc, VectorExpressionDescriptor.Mode mode) throws HiveException
    {
        ...
        } else if (exprDesc instanceof ExprNodeFieldDesc)
        {
            ...
            ve = getGenericUDFStructField((ExprNodeFieldDesc) exprDesc, mode, exprDesc.getTypeInfo());
        }
        if (ve == null)
        {
    ...
    {
        boolean ret = castExpressionUdfs.contains(genericUDF.getClass()) || (genericUDF instanceof GenericUDFRound) || (genericUDF instanceof GenericUDFBetween);
    ...
    public static boolean isNonVectorizedPathUDF(ExprNodeGenericFuncDesc expr, VectorExpressionDescriptor.Mode mode)
    {
        ...
        if (gudf instanceof GenericUDFBridge)
        {
            ...
            return udfClass.equals(UDFHex.class) || udfClass.equals(UDFRegExpExtract.class) || udfClass.equals(UDFRegExpReplace.class) || udfClass.equals(UDFConv.class) || isCastToIntFamily(udfClass) && isStringFamily(arg0Type(expr)) || isCastToFloatFamily(udfClass) && isStringFamily(arg0Type(expr));
        } else if (gudf instanceof GenericUDFFromUnixTime && isIntFamily(arg0Type(expr)) || (gudf instanceof GenericUDFTimestamp && isStringFamily(arg0Type(expr)))
                 ...
                 */ || gudf instanceof GenericUDFCase || gudf instanceof GenericUDFWhen)
        {
            return true;
        } else // between has 4 args here, but can be vectorized like this
            if ((gudf instanceof GenericUDFToString || gudf instanceof GenericUDFToChar || gudf instanceof GenericUDFToVarchar) && (arg0Type(expr).equals("timestamp") || arg0Type(expr).equals("double") || arg0Type(expr).equals("float")))
            {
                return true;
    ...
    private VectorExpression getDecimal64VectorExpressionForUdf(GenericUDF genericUdf, Class<?> udfClass, List<ExprNodeDesc> childExprs, int numChildren, VectorExpressionDescriptor.Mode mode, TypeInfo returnTypeInfo) throws HiveException
    {
        ...
        if (scaleMismatch && !hasConstants && (genericUdf instanceof GenericUDFBaseArithmetic || genericUdf instanceof GenericUDFBaseCompare))
        {
            ...
            newConstant = new ExprNodeConstantDesc(new DecimalTypeInfo(scaleDiff, 0), HiveDecimal.create(POWEROFTENTABLE[scaleDiff]));
            ...
            ExprNodeGenericFuncDesc newScaledExpr = new ExprNodeGenericFuncDesc(resultTypeInfo, new GenericUDFOPScaleUpDecimal64(), " ScaleUp ", children);
            childExprs.remove(childIndexToRewrite);
            childExprs.add(childIndexToRewrite, newScaledExpr);
        }

        return createDecimal64VectorExpression(vectorClass, childExprs, childrenMode, isDecimal64ScaleEstablished, decimal64ColumnScale, returnTypeInfo, returnDataTypePhysicalVariation, dontRescaleArguments, genericUdf);
    }
    ...
    public void wrapWithDecimal64ToDecimalConversions(VectorExpression[] vecExprs) throws HiveException
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                DataTypePhysicalVariation outputDataTypePhysicalVariation = vecExpr.getOutputDataTypePhysicalVariation();
    ...
    private VectorExpression getIdentityForDateToDate(List<ExprNodeDesc> childExprs, TypeInfo returnTypeInfo) throws HiveException
    {
        ...
        if (childTypeInfo.getCategory() != Category.PRIMITIVE || ((PrimitiveTypeInfo) childTypeInfo).getPrimitiveCategory() != PrimitiveCategory.DATE)
        {
            return null;
        }
        if (returnTypeInfo.getCategory() != Category.PRIMITIVE || ((PrimitiveTypeInfo) returnTypeInfo).getPrimitiveCategory() != PrimitiveCategory.DATE)
        {
            return null;
        }
    ...
    private VectorExpression getCastToTimestamp(GenericUDFTimestamp udf, List<ExprNodeDesc> childExpr, VectorExpressionDescriptor.Mode mode, TypeInfo returnType) throws HiveException
    {
        ...
        {
            ve = createVectorExpression(CastMillisecondsLongToTimestamp.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        }
    ...
    {
        INT_FAMILY, TIMESTAMP, DATE, FLOAT_FAMILY, STRING_FAMILY, DECIMAL
    }
    ...
    private VectorExpression getInExpression(List<ExprNodeDesc> childExpr, VectorExpressionDescriptor.Mode mode, TypeInfo returnType) throws HiveException
    {
        ...
        } else if (isDecimalFamily(colType))
        {
            ...
            if (tryDecimal64)
            {
                ...
                expr = createDecimal64VectorExpression(cl, childExpr.subList(0, 1), VectorExpressionDescriptor.Mode.PROJECTION,
                        /* isDecimal64ScaleEstablished */ true,
                        /* decimal64ColumnScale */ scale, returnType, DataTypePhysicalVariation.NONE,
    ...
    private VectorExpression getCastToDecimal(List<ExprNodeDesc> childExpr, VectorExpressionDescriptor.Mode mode, TypeInfo returnType) throws HiveException
    {
        ...
        if (isIntFamily(inputType))
        {
            if (tryDecimal64Cast)
            {
                if (((DecimalTypeInfo) returnType).precision() <= 18)
                {
                    return createVectorExpression(CastLongToDecimal64.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.DECIMAL_64);
                }
                return null;
            }
            return createVectorExpression(CastLongToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (inputType.equals("float"))
        {
            return createVectorExpression(CastFloatToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (inputType.equals("double"))
        {
            return createVectorExpression(CastDoubleToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
            ...
            if (child instanceof ExprNodeColumnDesc)
            {
                ...
                if (dataTypePhysicalVariation == DataTypePhysicalVariation.DECIMAL_64)
                {
                    ...
                    if (tryDecimal64Cast && ((DecimalTypeInfo) returnType).precision() <= 18)
                    {
                        ...
                        ExprNodeDesc newConstant = new ExprNodeConstantDesc(new DecimalTypeInfo(scaleDiff, 0), HiveDecimal.create(POWEROFTENTABLE[scaleDiff]));
                        children.add(child);
                        children.add(newConstant);
                        ExprNodeGenericFuncDesc newScaledExpr = new ExprNodeGenericFuncDesc(returnType, new GenericUDFOPScaleUpDecimal64(), " ScaleUp ", children);
                ...
                {
                    return createVectorExpression(CastDecimalToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
                }
            } else
            {
                return createVectorExpression(CastDecimalToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        ...
        {
            DataTypePhysicalVariation dataTypePhysicalVariation = tryDecimal64Cast && ((DecimalTypeInfo) returnType).precision() <= 18 ? DataTypePhysicalVariation.DECIMAL_64 : DataTypePhysicalVariation.NONE;
            return createVectorExpression(CastStringToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, dataTypePhysicalVariation);
        } else if (inputType.equals("timestamp"))
        {
            return createVectorExpression(CastTimestampToDecimal.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        }
        return null;
    }
    ...
    private VectorExpression getCastToChar(List<ExprNodeDesc> childExpr, TypeInfo returnType) throws HiveException
    {
        ...
        {
            // Boolean must come before the integer family. It's a special case.
            return createVectorExpression(CastBooleanToCharViaLongToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (isIntFamily(inputType))
        {
            return createVectorExpression(CastLongToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (inputType.equals("float"))
        {
            return createVectorExpression(CastFloatToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (inputType.equals("double"))
        {
            return createVectorExpression(CastDoubleToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (isDecimalFamily(inputType))
        {
            return createVectorExpression(CastDecimalToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (isDateFamily(inputType))
        {
            return createVectorExpression(CastDateToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (isTimestampFamily(inputType))
        {
            return createVectorExpression(CastTimestampToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        } else if (isStringFamily(inputType))
        {
            return createVectorExpression(CastStringGroupToChar.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        }
        return null;
    }
    ...
    private VectorExpression getCastToBinary(List<ExprNodeDesc> childExpr, TypeInfo returnType) throws HiveException
    {
        ...
        {
            return createVectorExpression(CastCharToBinary.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
        }
        return null;
    }
    ...
    private VectorExpression getCastToDoubleExpression(Class<?> udf, List<ExprNodeDesc> childExpr, TypeInfo returnType) throws HiveException
    {
        ...
        if (isIntFamily(inputType))
        {
            if (udf.equals(UDFToFloat.class))
            {
                ...
                return createVectorExpression(CastLongToFloatViaLongToDouble.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
            } else
            {
                return createVectorExpression(CastLongToDouble.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
            }
        } else if (inputType.equals("timestamp"))
        {
            return createVectorExpression(CastTimestampToDouble.class, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
    ...
    private VectorExpression getCastWithFormat(GenericUDF udf, List<ExprNodeDesc> childExpr, TypeInfo returnType) throws HiveException
    {
        ...
        return createVectorExpression(veClass, childExpr, VectorExpressionDescriptor.Mode.PROJECTION, returnType, DataTypePhysicalVariation.NONE);
    }
    ...
    private Class<?> getCastFormatVectorExpressionClass(List<ExprNodeDesc> childExpr, TypeInfo returnType, String inputType) throws HiveException
    {
        ...
        if (inputType.equals(serdeConstants.STRING_TYPE_NAME) || inputType.startsWith(serdeConstants.CHAR_TYPE_NAME) || inputType.startsWith(serdeConstants.VARCHAR_TYPE_NAME))
        {
    ...
    private VectorExpression tryDecimal64Between(VectorExpressionDescriptor.Mode mode, boolean isNot, ExprNodeDesc colExpr, List<ExprNodeDesc> childrenAfterNot, TypeInfo returnTypeInfo) throws HiveException
    {
        ...
        return createDecimal64VectorExpression(cl, childrenAfterNot, VectorExpressionDescriptor.Mode.PROJECTION,
                /* isDecimal64ScaleEstablished */ true,
                /* decimal64ColumnScale */ ((DecimalTypeInfo) colExpr.getTypeInfo()).getScale(), returnTypeInfo, DataTypePhysicalVariation.NONE,
                /* dontRescaleArguments */ false, new GenericUDFBetween());
    }
    ...
    private VectorExpression getBetweenExpression(List<ExprNodeDesc> childExpr, VectorExpressionDescriptor.Mode mode, TypeInfo returnType) throws HiveException
    {
        ...
        } else if (isDecimalFamily(colType) && !notKeywordPresent)
        {
            ...
            if (tryDecimal64)
            {
                VectorExpression decimal64VecExpr = tryDecimal64Between(mode, /* isNot */ false, colExpr, childrenAfterNot, returnType);
        ...
        } else if (isDecimalFamily(colType) && notKeywordPresent)
        {
            ...
            if (tryDecimal64)
            {
                VectorExpression decimal64VecExpr = tryDecimal64Between(mode, /* isNot */ true, colExpr, childrenAfterNot, returnType);
                if (decimal64VecExpr != null)
                {
                    return decimal64VecExpr;
                }
    ...
    static String getScratchName(TypeInfo typeInfo) throws HiveException
    {
        ...
        if (typeInfo.getCategory() == Category.PRIMITIVE && ((PrimitiveTypeInfo) typeInfo).getPrimitiveCategory() == PrimitiveCategory.DECIMAL)
        {
