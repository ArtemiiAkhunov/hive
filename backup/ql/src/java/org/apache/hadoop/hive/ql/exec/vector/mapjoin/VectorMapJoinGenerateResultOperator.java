...
public abstract class VectorMapJoinGenerateResultOperator extends VectorMapJoinCommonOperator
{
    ...
    private void setUpInterruptChecking()
    {
        for (Operator<? extends OperatorDesc> child : childOperatorsArray)
        {
            ...
            if (child instanceof VectorMapJoinGenerateResultOperator)
            {
                return;
            }
        }
    ...
    protected void performValueExpressions(VectorizedRowBatch batch, int[] allMatches, int allMatchCount) throws HiveException
    {
        /*
    ...
    protected void doSmallTableValueDeserializeRow(VectorizedRowBatch batch, int batchIndex, ByteSegmentRef byteSegmentRef, VectorMapJoinHashMapResult hashMapResult) throws HiveException
    {
     ...
     *
     * @param batch           The big table batch.
     * @param hashMapResult   The hash map results for the matching key.
     * @param allMatches      The selection array for all matches key.
     * @param allMatchesIndex Index into allMatches of the matching key we are generating for.
     * @param duplicateCount  Number of equal key rows.
     * @param numSel          Current number of rows that are remaining in the big table for forwarding.
     * @return The new count of selected rows.
     */
    protected int generateHashMapResultSingleValue(VectorizedRowBatch batch, VectorMapJoinHashMapResult hashMapResult, int[] allMatches, int allMatchesIndex, int duplicateCount, int numSel) throws HiveException, IOException
    {
        ...
        for (int i = 0; i < duplicateCount; i++)
        {
            ...
            {
                doSmallTableValueDeserializeRow(batch, batchIndex, byteSegmentRef, hashMapResult);
            }
     ...
     * @param batch           The big table batch.
     * @param hashMapResult   The hash map results for the matching key.
     * @param allMatches      The all match selected array that contains (physical) batch indices.
     * @param allMatchesIndex The index of the match key.
     * @param duplicateCount  Number of equal key rows.
     */
    protected void generateHashMapResultMultiValue(VectorizedRowBatch batch, VectorMapJoinHashMapResult hashMapResult, int[] allMatches, int allMatchesIndex, int duplicateCount) throws HiveException, IOException
    {
        ...
        if (useOverflowRepeatedThreshold && hashMapResult.isCappedCountAvailable() && hashMapResult.cappedCount() > overflowRepeatedThreshold)
        {
            ...
            generateHashMapResultLargeMultiValue(batch, hashMapResult, allMatches, allMatchesIndex, duplicateCount);
            return;
        }
        ...
        for (int i = 0; i < duplicateCount; i++)
        {
            ...
            while (byteSegmentRef != null)
            {
                ...
                if (nonOuterSmallTableKeyVectorCopy != null)
                {
                    ...
                    //
                    nonOuterSmallTableKeyVectorCopy.copyByValue(batch, batchIndex, overflowBatch, overflowBatch.size);
                }
                ...
                if (outerSmallTableKeyVectorCopy != null)
                {
                    ...
                    //
                    outerSmallTableKeyVectorCopy.copyByValue(batch, batchIndex, overflowBatch, overflowBatch.size);
                }

                if (smallTableValueVectorDeserializeRow != null)
                {

                    doSmallTableValueDeserializeRow(overflowBatch, overflowBatch.size, byteSegmentRef, hashMapResult);
                }
     ...
     * @param duplicateCount  Number of equal key rows.
     */
    private void generateHashMapResultLargeMultiValue(VectorizedRowBatch batch, VectorMapJoinHashMapResult hashMapResult, int[] allMatches, int allMatchesIndex, int duplicateCount) throws HiveException, IOException
    {
        ...
        while (byteSegmentRef != null)
        {
            ...
            for (int i = 0; i < duplicateCount; i++)
            {
                ...
                if (nonOuterSmallTableKeyVectorCopy != null)
                {
                    ...
                    // across to overflow batch by value so Small Key projection will see its keys...
                    //
                    nonOuterSmallTableKeyVectorCopy.copyByValue(batch, batchIndex, overflowBatch, 0);
                ...
                if (outerSmallTableKeyVectorCopy != null)
                {
                    ...
                    // to overflow batch by value.
                    //
                    outerSmallTableKeyVectorCopy.copyByValue(batch, batchIndex, overflowBatch, 0);
     ...
     *
     * @param batch         The big table batch.
     * @param hashMapResult The hash map results for the repeated key.
     */
    protected void generateHashMapResultRepeatedAll(VectorizedRowBatch batch, VectorMapJoinHashMapResult hashMapResult) throws IOException, HiveException
    {
        ...
        {
            numSel = generateHashMapResultSingleValue(batch, hashMapResult, batch.selected, 0, batch.size, numSel);

        } else
        {
            generateHashMapResultMultiValue(batch, hashMapResult, batch.selected, 0, batch.size);
        }
    ...
    private void setupSpillSerDe(VectorizedRowBatch batch) throws HiveException
    {
        ...
        TypeInfo[] inputObjInspectorsTypeInfos = VectorizedBatchUtil.typeInfosFromStructObjectInspector((StructObjectInspector) inputObjInspectors[posBigTable]);
        ...
        for (int i = 0; i < projectionSize; i++)
        {
            ...
            if (batch.cols[projectedColumn] != null && inputObjInspectorsTypeInfos[i].getCategory() == Category.PRIMITIVE)
            {
    ...
    private void spillSerializeRow(VectorizedRowBatch batch, int batchIndex, int partitionId) throws IOException
    {
    ...
    protected void spillRow(VectorizedRowBatch batch, int batchIndex, int partitionId) throws HiveException, IOException
    {
    ...
    {
        if (ownThread == null || --interruptCheckCounter > 0)
        {
            return;
        }
