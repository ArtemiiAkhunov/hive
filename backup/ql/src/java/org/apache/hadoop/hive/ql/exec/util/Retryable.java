...
public class Retryable
{
    ...
    public <T> T executeCallable(Callable<T> callable) throws Exception
    {
        ...
        while (true)
        {
            ...
            {
                if (this.failOnParentExceptions.stream().noneMatch(k -> k.isAssignableFrom(e.getClass())) && this.failOn.stream().noneMatch(k -> e.getClass().equals(k)) && this.retryOn.stream().anyMatch(k -> e.getClass().isAssignableFrom(k)))
                {
    ...
    private long getNextDelay(long currentDelay, final Exception previousException, final Exception currentException)
    {
        ...
        if (currentDelay <= 0)
        { // in case initial delay was set to 0.
    ...
    public static class Builder
    {
        ...
        public Builder withHiveConf(HiveConf conf)
        {
            ...
            runnable.maxRetryDelayInSeconds = conf.getTimeVar(HiveConf.ConfVars.REPL_RETRY_MAX_DELAY_BETWEEN_RETRIES, TimeUnit.SECONDS);
        ...
        public synchronized Builder withFailOnParentExceptionList(final List<Class<? extends Exception>> exceptionClassList)
        {
            for (final Class<? extends Exception> exceptionClass : exceptionClassList)
            {
                if (exceptionClass != null)
