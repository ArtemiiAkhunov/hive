...
public class AtlasDumpTask extends Task<AtlasDumpWork> implements Serializable
{
    ...
    public int execute()
    {
        try
        {
            ...
            LOG.info("Dumping Atlas metadata of srcDb: {}, for TgtDb: {} to staging location: {}", atlasReplInfo.getSrcDB(), atlasReplInfo.getTgtDB(), atlasReplInfo.getStagingDir());
            ...
            atlasRestClient = new AtlasRestClientBuilder(atlasReplInfo.getAtlasEndpoint()).getClient(atlasReplInfo.getConf());
            AtlasRequestBuilder atlasRequestBuilder = new AtlasRequestBuilder();
            String entityGuid = checkHiveEntityGuid(atlasRequestBuilder, atlasReplInfo.getSrcCluster(), atlasReplInfo.getSrcDB());
        ...
        } catch (RuntimeException e)
        {
            ...
            try
            {
                ReplUtils.handleException(true, e, work.getStagingDir().getParent().toString(), work.getMetricCollector(), getName(), conf);
            } catch (Exception ex)
            {
        ...
        } catch (Exception e)
        {
            ...
            try
            {
                return ReplUtils.handleException(true, e, work.getStagingDir().getParent().toString(), work.getMetricCollector(), getName(), conf);
            } catch (Exception ex)
            {
                LOG.error("Failed to collect replication metrics: ", ex);
                return errorCode;
    ...
    private AtlasReplInfo createAtlasReplInfo() throws SemanticException, MalformedURLException
    {
        ...
        String endpoint = new URL(ReplUtils.getNonEmpty(HiveConf.ConfVars.REPL_ATLAS_ENDPOINT.varname, conf, errorFormat)).toString();
        ...
        AtlasReplInfo atlasReplInfo = new AtlasReplInfo(endpoint, work.getSrcDB(), tgtDB, srcCluster, tgtCluster, work.getStagingDir(), work.getTableListPath(), conf);
    ...
    private String checkHiveEntityGuid(AtlasRequestBuilder atlasRequestBuilder, String clusterName, String srcDb) throws SemanticException
    {
        ...
        {
            throw new SemanticException(ErrorMsg.REPL_INVALID_INTERNAL_CONFIG_FOR_SERVICE.format("Could find " + "entries in objectId for:" + clusterName, ReplUtils.REPL_ATLAS_SERVICE));
        }
        ...
        {
            throw new SemanticException(ErrorMsg.REPL_INVALID_INTERNAL_CONFIG_FOR_SERVICE.format("Entity not found:" + objectId, ReplUtils.REPL_ATLAS_SERVICE));
        }
        return guid;
    }
