...
public class CopyUtils
{
    ...
    {
        Retryable retryable = Retryable.builder().withHiveConf(hiveConf).withRetryOnException(IOException.class).withFailOnParentExceptionList(failOnParentExceptionList).build();
        try
        {
    ...
    void copyFilesBetweenFS(FileSystem srcFS, Path[] paths, FileSystem dstFS, Path dst, boolean deleteSource, boolean overwrite, DataCopyStatistics copyStatistics) throws IOException
    {
        retryableFxn(() ->
        {
            ...
            FileUtils.copy(srcFS, paths, dstFS, dst, deleteSource, overwrite, preserveXAttrs, hiveConf, copyStatistics);
            return null;
        });
    }
    ...
    public void copyAndVerify(Path destRoot, List<ReplChangeManager.FileInfo> srcFiles, Path origSrcPath, boolean readSrcAsFilesList, boolean overwrite) throws IOException, LoginException, HiveFatalException
    {
    ...
    @VisibleForTesting
    void doCopy(Map.Entry<Path, List<ReplChangeManager.FileInfo>> destMapEntry, UserGroupInformation proxyUser, boolean useRegularCopy, boolean overwrite, DataCopyStatistics copyStatistics) throws IOException, LoginException, HiveFatalException
    {
    ...
    private void doCopyRetry(FileSystem sourceFs, List<ReplChangeManager.FileInfo> srcFileList, Path destination, UserGroupInformation proxyUser, boolean useRegularCopy, boolean overwrite, DataCopyStatistics copyStatistics) throws IOException, LoginException, HiveFatalException
    {
        int repeat = 0;
        ...
        while (!pathList.isEmpty() && (repeat < MAX_IO_RETRY))
        {
            ...
            } catch (IOException e)
            {
                ...
                if (repeat >= (MAX_IO_RETRY - 1) || failOnParentExceptionList.stream().anyMatch(k -> k.isAssignableFrom(e.getClass())) || ErrorMsg.REPL_FILE_SYSTEM_OPERATION_RETRY.getMsg().equals(e.getMessage()))
                {
                    break;
                }
  ...
  where the first event refers to source path and  second event refers to CM path */
    ...
    private void doDistCpCopyOnce(FileSystem sourceFs, List<Path> srcList, Path destination, UserGroupInformation proxyUser, DataCopyStatistics copyStatistics) throws IOException
    {
        ...
        if (!FileUtils.distCp(sourceFs, // source file system
                srcList,  // list of source paths
                destination, false, proxyUser, hiveConf, ShimLoader.getHadoopShims()))
        {
   ...
   */
    ...
    public void doCopy(Path destination, List<Path> srcPaths) throws IOException, LoginException
    {
        ...
        for (Map.Entry<FileSystem, List<Path>> entry : map.entrySet())
        {
            ...
            List<ReplChangeManager.FileInfo> fileList = Lists.transform(entry.getValue(), path -> new ReplChangeManager.FileInfo(sourceFs, path, null));
            doCopyOnce(sourceFs, entry.getValue(), destination, regularCopy(sourceFs, fileList), proxyUser, false, new DataCopyStatistics());
        }
    }
