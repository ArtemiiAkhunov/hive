...
@Description(name = "first_value", value = "_FUNC_(x)")
@WindowFunctionDescription(supportsWindow = true, pivotResult = false, impliesOrder = true, supportsNullTreatment = true)
        ...
        public Object terminate(AggregationBuffer agg) throws HiveException
        {
            ...
            for (int i = Math.max(0, wFrameDef.getStart().getRelativeOffset()); i < wFrameDef.getEnd().getRelativeOffset(); i++)
            {
                ...
                if (r != null)
                {
                    ...
                    if (!wFrameDef.isStartUnbounded() && s.numRows >= fIdx + wFrameDef.getWindowSize() && !s.valueChain.isEmpty())
                    {
                        s.valueChain.removeFirst();
