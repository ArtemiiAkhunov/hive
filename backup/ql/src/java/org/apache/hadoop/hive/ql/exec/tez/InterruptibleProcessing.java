...
public class InterruptibleProcessing
{
    ...
    private static final int CHECK_INTERRUPTION_AFTER_ROWS_DEFAULT = 1000, CHECK_INTERRUPTION_AFTER_ROWS_MAX = 100000, CHECK_INTERRUPTION_AFTER_ROWS_MIN = 1;
    ...
    {
        if (nRows++ < checkInterruptionAfterRows)
        {
            return;
        }
        ...
        if (elapsedNs >= 0)
        {
            ...
            int newRows = Math.min(CHECK_INTERRUPTION_AFTER_ROWS_MAX, Math.max(CHECK_INTERRUPTION_AFTER_ROWS_MIN, (int) (diff * checkInterruptionAfterRows)));
            if (checkInterruptionAfterRows != newRows && LOG.isDebugEnabled())
            {
                LOG.debug("Adjusting abort check rows to " + newRows + " from " + checkInterruptionAfterRows);
            }
