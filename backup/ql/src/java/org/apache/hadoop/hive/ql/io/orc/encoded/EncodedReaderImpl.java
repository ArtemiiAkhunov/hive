...
class EncodedReaderImpl implements EncodedReader
{
    ...
    private static void addRgFilteredStreamToRanges(OrcProto.Stream stream, boolean[] includedRowGroups, boolean isCompressed, OrcProto.RowIndex index, OrcProto.ColumnEncoding encoding, TypeDescription.Category colType, int compressionSize, boolean hasNull, long offset, long length, DiskRangeList.CreateHelper list, boolean doMergeBuffers)
    {
        for (int group = 0; group < includedRowGroups.length; ++group)
        {
            ...
            int posn = RecordReaderUtils.getIndexPosition(encoding.getKind(), colType, stream.getKind(), isCompressed, hasNull);
            ...
            for (int rgIx = 0; rgIx < rgCount; ++rgIx)
            {
                ...
                try
                {
                    ...
                    for (int colIx = 0; colIx < colCtxs.length; ++colIx)
                    {
                        ...
                        if (ctx == null)
                        {
                            continue; // This column is not included
                        }
    ...
    private DiskRangeList.MutateHelper getDataFromCacheAndDisk(DiskRangeList listToRead, long stripeOffset, boolean hasFileId, IdentityHashMap<ByteBuffer, Boolean> toRelease) throws IOException
    {
        ...
        {
            LOG.debug("Resulting disk ranges to read (file " + fileKey + "): " + RecordReaderUtils.stringifyDiskRanges(toRead.next));
        }
        ...
        if (hasFileId)
        {
            ...
            if (LOG.isDebugEnabled())
            {
                LOG.debug("Disk ranges after cache (found everything " + isAllInCache.value + "; file " + fileKey + ", base offset " + stripeOffset + "): " + RecordReaderUtils.stringifyDiskRanges(toRead.next));
            }
    ...
    private ColumnStreamData createRgColumnStreamData(int rgIx, boolean isLastRg, int colIx, StreamContext sctx, long cOffset, long endCOffset, boolean isCompressed, long unlockUntilCOffset)
    {
    ...
    private static class ProcCacheChunk extends CacheChunk
    {
        ...
        public String toString()
        {
            return super.toString() + ", original is set " + (this.originalData != null) + ", buffer was replaced " + (originalCbIndex == -1);
        }
    ...
    public DiskRangeList readEncodedStream(long baseOffset, DiskRangeList start, long cOffset, long endCOffset, ColumnStreamData csd, long unlockUntilCOffset, long streamOffset, IdentityHashMap<ByteBuffer, Boolean> toRelease) throws IOException
    {
        ...
        {
            LOG.error("Failed " + (isCompressed ? "" : "un") + "compressed read; cOffset " + cOffset + ", endCOffset " + endCOffset + ", streamOffset " + streamOffset + ", unlockUntilCOffset " + unlockUntilCOffset + "; ranges passed in " + RecordReaderUtils.stringifyDiskRanges(start) + "; ranges passed to prepare " + RecordReaderUtils.stringifyDiskRanges(current)); // Don't log exception here.
            throw (ex instanceof IOException) ? (IOException) ex : new IOException(ex);
        }
    ...
    private void preReadUncompressedStreams(long baseOffset, MutateHelper toRead, IdentityHashMap<ByteBuffer, Boolean> toRelease) throws IOException
    {
        if (isCompressed)
        {
            return;
    ...
    private CacheChunk prepareRangesForCompressedRead(long cOffset, long endCOffset, long streamOffset, long unlockUntilCOffset, DiskRangeList current, ColumnStreamData columnStreamData, IdentityHashMap<ByteBuffer, Boolean> toRelease, List<ByteBuffer> toReleaseCopies, List<ProcCacheChunk> toDecompress, List<IncompleteCb> badEstimates) throws IOException
    {
        ...
        while (true)
        {
            ...
            } else
            {
                ...
                ProcCacheChunk newCached = addOneCompressionBuffer(bc, columnStreamData.getCacheBuffers(), toDecompress, toRelease, toReleaseCopies, badEstimates);
    ...
    private DiskRangeList preReadUncompressedStream(long baseOffset, DiskRangeList start, long streamOffset, long streamEnd, Kind kind) throws IOException
    {
        ...
        if (toCache == null)
        {
            return lastUncompressed; // Nothing to copy and cache.
        }
    ...
    static BufferChunk readLengthBytesFromSmallBuffers(BufferChunk first, long cbStartOffset, int[] result, List<IncompleteCb> badEstimates, boolean isTracingEnabled, IoTrace trace) throws IOException
    {
        ...
        while (true)
        {
            ...
            if (ix == 3)
            {
                return currentBc; // Done, we have 3 bytes. Continue reading this buffer.
            }
    ...
    private static IncompleteCb addIncompleteCompressionBuffer(long cbStartOffset, DiskRangeList target, int extraChunkCountToLog, boolean isTracingEnabled, IoTrace trace)
    {
        ...
        if (isTracingEnabled)
        {
            LOG.trace("Replacing " + target + " (and " + extraChunkCountToLog + " previous chunks) with " + icb + " in the buffers");
        }
    ...
    private static class IndexStream extends InputStream
    {
        ...
        public String toString()
        {
            return "position: " + currentOffset + " length: " + length + " range: " + rangeIx + " offset: " + (range == null ? 0 : range.position()) + " limit: " + (range == null ? 0 : range.limit());
        }
    }
    ...
    private DiskRangeList preReadUncompressedStreams(long stripeOffset, ReadContext[] colCtxs, MutateHelper toRead, IdentityHashMap<ByteBuffer, Boolean> toRelease) throws IOException
    {
        if (isCompressed)
        {
            return toRead.next;
        ...
        try
        {
            for (int colIx = 0; colIx < colCtxs.length; ++colIx)
            {
                ...
                {
                    StreamContext sctx = ctx.streams[streamIx];
                    DiskRangeList newIter = preReadUncompressedStream(stripeOffset, iter, sctx.offset, sctx.offset + sctx.length, sctx.kind);
                    if (newIter != null)
                    {
                        iter = newIter;
                    }
            ...
            if (LOG.isDebugEnabled())
            {
                LOG.debug("Disk ranges after pre-read (file " + fileKey + ", base offset " + stripeOffset + "): " + RecordReaderUtils.stringifyDiskRanges(toRead.next));
            }
    ...
    private static boolean hadBadBloomFilters(TypeDescription.Category category, WriterVersion version)
    {
        switch (category)
        {
            case STRING:
