...
public class ExprNodeConverter extends RexVisitorImpl<ExprNodeDesc>
{
    ...
    public ExprNodeConverter(String tabAlias, String columnAlias, RelDataType inputRowType, RelDataType outputRowType, Set<Integer> inputVCols, RexBuilder rexBuilder)
    {
    ...
    public ExprNodeDesc visitInputRef(RexInputRef inputRef)
    {
        ...
        return new ExprNodeColumnDesc(TypeConverter.convert(f.getType()), f.getName(), tabAlias, inputVCols.contains(inputRef.getIndex()));
    }
    ...
    public static ExprNodeConstantDesc toExprNodeConstantDesc(RexLiteral literal)
    {
        ...
        } else
        {
            ...
            {
                case BOOLEAN:
                    return new ExprNodeConstantDesc(TypeInfoFactory.booleanTypeInfo, RexLiteral.booleanValue(literal));
                case TINYINT:
                ...
                case FLOAT:
                case REAL:
                    return new ExprNodeConstantDesc(TypeInfoFactory.floatTypeInfo, ((Number) literal.getValue3()).floatValue());
                case DOUBLE:
                    return new ExprNodeConstantDesc(TypeInfoFactory.doubleTypeInfo, ((Number) literal.getValue3()).doubleValue());
                case DATE:
                    return new ExprNodeConstantDesc(TypeInfoFactory.dateTypeInfo, Date.valueOf(literal.getValueAs(DateString.class).toString()));
                case TIME:
                    return new ExprNodeConstantDesc(TypeInfoFactory.timestampTypeInfo, Timestamp.valueOf(literal.getValueAs(TimeString.class).toString()));
                case TIMESTAMP:
                    return new ExprNodeConstantDesc(TypeInfoFactory.timestampTypeInfo, Timestamp.valueOf(literal.getValueAs(TimestampString.class).toString()));
                case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                    HiveConf conf;
                    try
                    {
                    ...
                    return new ExprNodeConstantDesc(TypeInfoFactory.getTimestampTZTypeInfo(conf.getLocalTimeZone()), TimestampTZUtil.parse(literal.getValueAs(TimestampString.class).toString() + " UTC"));
                case BINARY:
                    return new ExprNodeConstantDesc(TypeInfoFactory.binaryTypeInfo, literal.getValueAs(ByteString.class).getBytes());
                case DECIMAL:
                ...
                case CHAR:
                {
                    Preconditions.checkState(literal.getValue() instanceof NlsString, "char values must use NlsString for correctness");
                ...
                case VARCHAR:
                {
                    Preconditions.checkState(literal.getValue() instanceof NlsString, "varchar/string values must use NlsString for correctness");
                    int precision = lType.getPrecision();
