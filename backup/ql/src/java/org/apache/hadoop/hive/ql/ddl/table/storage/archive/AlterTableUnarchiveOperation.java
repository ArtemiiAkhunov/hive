...
public class AlterTableUnarchiveOperation extends DDLOperation<AlterTableUnarchiveDesc>
{
    ...
    public int execute() throws HiveException, URISyntaxException
    {
        ...
        Path intermediateExtractedDir = AlterTableArchiveUtils.getInterMediateDir(originalDir, context.getConf(), ConfVars.METASTORE_INT_EXTRACTED);
        ...
        if (!HdfsUtils.pathExists(intermediateArchivedDir, context.getConf()) && !HdfsUtils.pathExists(archivePath, context.getConf()))
        {
    ...
    {
        if (HdfsUtils.pathExists(intermediateArchivedDir, context.getConf()) || HdfsUtils.pathExists(intermediateExtractedDir, context.getConf()))
        {
    ...
    private void moveOriginalDirToIntermediateDir(Path originalDir, Path intermediateArchivedDir, FileSystem fs) throws HiveException
    {
        ...
        {
            context.getConsole().printInfo(intermediateArchivedDir + " already exists. " + "Assuming it contains the archived version of the partition");
        }
    }
    ...
    private void moveIntermediateExtractedDirToOriginalParent(Path originalDir, Path intermediateExtractedDir, FileSystem fs) throws HiveException
    {
        ...
        {
            context.getConsole().printInfo(originalDir + " already exists. " + "Assuming it contains the extracted files in the partition");
        }
    }
