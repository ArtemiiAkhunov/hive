...
public class PTFOperator extends Operator<PTFDesc> implements Serializable
{
    ...
    class PTFInvocation
    {
        ...
        void initializeStreaming(Configuration cfg, boolean isMapSide) throws HiveException
        {
            ...
            ObjectInspector inputOI = conf.getStartOfChain() == tabDef ? inputObjInspectors[0] : inputDef.getOutputShape().getOI();
        ...
        private void createInputPartition() throws HiveException
        {
            ...
            AbstractSerDe serde = conf.isMapSide() ? tabDef.getInput().getOutputShape().getSerde() : tabDef.getRawInputShape().getSerde();
            StructObjectInspector outputOI = conf.isMapSide() ? tabDef.getInput().getOutputShape().getOI() : tabDef.getRawInputShape().getOI();
            inputPart = PTFPartition.create(conf.getCfg(), serde, (StructObjectInspector) inputOI, outputOI);
        }
