    ...
    protected static class RexReplacer extends RexShuttle
    {
        ...
        public RexNode visitCall(RexCall call)
        {
            ...
            if (node != call)
            {
                ...
                RexSimplify simplify = new RexSimplify(rexBuilder, RelOptPredicateList.EMPTY, filterOp.getCluster().getPlanner().getExecutor());
        ...
        private ColStatistics extractColStats(RexInputRef ref)
        {
            ...
            if (columnOrigin != null)
            {
                ...
                if (table != null)
                {
                    ColStatistics colStats = table.getColStat(Lists.newArrayList(columnOrigin.getOriginColumnOrdinal()), false).get(0);
                    if (colStats != null && StatsUtils.areColumnStatsUptoDateForQueryAnswering(table.getHiveTableMD(), table.getHiveTableMD().getParameters(), colStats.getColumnName()))
                    {
                        return colStats;
                    }
                }
        ...
        private Long extractRowCount(RexInputRef ref)
        {
            ...
            if (columnOrigin != null)
            {
                ...
                if (table != null)
                {
                    if (StatsUtils.areBasicStatsUptoDateForQueryAnswering(table.getHiveTableMD(), table.getHiveTableMD().getParameters()))
                    {
