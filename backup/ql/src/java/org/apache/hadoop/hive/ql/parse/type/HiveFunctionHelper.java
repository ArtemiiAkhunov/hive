...
public class HiveFunctionHelper implements FunctionHelper
{
    ...
    public RelDataType getReturnType(FunctionInfo fi, List<RexNode> inputs) throws SemanticException
    {
        ...
        return TypeConverter.convert(TypeInfoUtils.getTypeInfoFromObjectInspector(oi), rexBuilder.getTypeFactory());
    }
    ...
    private RexNode convert(TypeInfo targetType, RexNode input) throws SemanticException
    {
        ...
        {
            return RexNodeTypeCheck.getExprNodeDefaultExprProcessor(rexBuilder).createConversionCast(input, (PrimitiveTypeInfo) targetType);
        } else
        {
            ...
            {
                exprNodes.add(convert(structTypeInfo.getAllStructFieldTypeInfos().get(j), call.getOperands().get(j)));
            }
    ...
    private void checkForStatefulFunctions(List<RexNode> exprs) throws SemanticException
    {
        RexVisitor<Void> visitor = new RexVisitorImpl<Void>(true)
        {
            ...
            public Void visitCall(final RexCall call)
            {
                ...
                if (nodeUDF == null)
                {
                    throw new AssertionError("Cannot retrieve function " + call.getOperator().getName() + " within StatefulFunctionsChecker");
                }
                // Stateful?
     ...
     */
    @Override
    public AggregateInfo getAggregateFunctionInfo(boolean isDistinct, boolean isAllColumns, String aggregateName, List<RexNode> aggregateParameters, List<FieldCollation> fieldCollations) throws SemanticException
    {
        ...
        for (RexNode aggParameter : aggregateParameters)
        {
            ...
            if (obKeyIterator.hasNext())
            {
                ...
                aggParameterOIs.add(PrimitiveObjectInspectorFactory.getPrimitiveWritableConstantObjectInspector(TypeInfoFactory.intTypeInfo, new IntWritable(fieldCollation.getSortDirection())));
                aggParameterOIs.add(PrimitiveObjectInspectorFactory.getPrimitiveWritableConstantObjectInspector(TypeInfoFactory.intTypeInfo, new IntWritable(fieldCollation.getNullOrdering().getCode())));
            }
        }
    ...
    public AggregateInfo getWindowAggregateFunctionInfo(boolean isDistinct, boolean isAllColumns, String aggregateName, List<RexNode> aggregateParameters) throws SemanticException
    {
        ...
        } else
        {
            ...
            try
            {
                ...
                if (aggregateName.toLowerCase().equals(FunctionRegistry.LEAD_FUNC_NAME) || aggregateName.toLowerCase().equals(FunctionRegistry.LAG_FUNC_NAME))
                {
                    GenericUDAFEvaluator genericUDAFEvaluator = FunctionRegistry.getGenericWindowingEvaluator(aggregateName, aggParameterOIs, isDistinct, isAllColumns, true);
                    GenericUDAFInfo udaf = SemanticAnalyzer.getGenericUDAFInfo2(genericUDAFEvaluator, udafMode, aggParameterOIs);
        ...
        return returnType != null ? new AggregateInfo(aggregateParameters, returnType, aggregateName, isDistinct) : null;
    }
    ...
    public RexCall getUDTFFunction(String functionName, List<RexNode> operands) throws SemanticException
    {
        ...
        SqlOperator calciteOp = SqlFunctionConverter.getCalciteOperator(functionName, genericUDTF, ImmutableList.copyOf(operandTypes), udtfRetType);
    ...
    private ObjectInspector createObjectInspector(RexNode expr)
    {
        ...
        return TypeInfoUtils.getStandardWritableObjectInspectorFromTypeInfo(TypeConverter.convert(expr.getType()));
    }
