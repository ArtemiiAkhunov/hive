...
public class WindowingTableFunction extends TableFunctionEvaluator
{
    ...
    {
        BasePartitionEvaluator partitionEval = wFn.getWFnEval().getPartitionWindowingEvaluator(wFn.getWindowFrame(), partition, wFn.getArgs(), wFn.getOI(), nullsLast);
    ...
    private static boolean processWindow(WindowFrameDef frame)
    {
        ...
        if (frame.getStart().getAmt() == BoundarySpec.UNBOUNDED_AMOUNT && frame.getEnd().getAmt() == BoundarySpec.UNBOUNDED_AMOUNT)
        {
            return false;
        }
        return true;
