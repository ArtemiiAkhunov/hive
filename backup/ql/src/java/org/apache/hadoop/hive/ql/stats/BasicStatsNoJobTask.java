...
public class BasicStatsNoJobTask implements IStatsProcessor
{
    ...
    abstract static class StatCollector implements Runnable
    {
        ...
        {
            return StatsSetupConst.SUPPORTED_STATS.stream().map(st -> st + "=" + parameters.get(st)).collect(Collectors.joining(", "));
        }
    }
        ...
        public void run()
        {
            ...
            try
            {
                ...
                if (fileList.size() > 1 && numThreadsFactor > 0)
                {
                    ...
                    tpE = new ThreadPoolExecutor(numThreads, numThreads, 0, TimeUnit.SECONDS, new LinkedBlockingQueue<>(), threadFactory);
    ...
    private static class FileStatProcessor implements Callable<FileStats>
    {
        ...
        {
            try (org.apache.hadoop.mapred.RecordReader<?, ?> recordReader = inputFormat.getRecordReader(dummySplit, jc, Reporter.NULL))
            {
                if (recordReader instanceof StatsProvidingRecordReader)
                {
                    StatsProvidingRecordReader statsRR;
                    statsRR = (StatsProvidingRecordReader) recordReader;
                    final FileStats fileStats = new FileStats(statsRR.getStats().getRawDataSize(), statsRR.getStats().getRowCount(), file.getLen(), file.isErasureCoded());
                    return fileStats;
                } else
                {
