...
public class DescTableDesc implements DDLDesc, Serializable
{
    ...
    private static final List<String> PURE_STATISTICS_HEADERS = ImmutableList.of("min", "max", "num_nulls", "distinct_count", "avg_col_len", "max_col_len", "num_trues", "num_falses", "bit_vector");
    // keeping this around for backward compatibility, use "getColumnStatisticsHeaders" instead
    @SuppressWarnings("unused")
    public static final List<String> COLUMN_STATISTICS_HEADERS = ImmutableList.<String>builder().add("col_name").add("data_type").addAll(PURE_STATISTICS_HEADERS).add("comment").build();
    ...
    @Explain(displayName = "extended", displayOnlyOnTrue = true, explainLevels = {Level.USER, Level.DEFAULT, Level.EXTENDED})
    public boolean isExtended()
    {
        return isExtended;
    }

    @Explain(displayName = "formatted", displayOnlyOnTrue = true, explainLevels = {Level.USER, Level.DEFAULT, Level.EXTENDED})
    ...
    {
        ImmutableList.Builder<String> builder = ImmutableList.<String>builder().add("col_name").add("data_type").addAll(PURE_STATISTICS_HEADERS);
