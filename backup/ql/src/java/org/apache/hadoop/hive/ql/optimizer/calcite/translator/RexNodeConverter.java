...
public class RexNodeConverter
{
        ...
        if (gUDF instanceof GenericUDFBridge)
        {
            ...
            if (udfClassName != null)
            {
                ...
                if (sp >= 0 & (sp + 1) < udfClassName.length())
                {
                    ...
                    if (udfClassName.equals("UDFToBoolean") || udfClassName.equals("UDFToByte") || udfClassName.equals("UDFToDouble") || udfClassName.equals("UDFToInteger") || udfClassName.equals("UDFToLong") || udfClassName.equals("UDFToShort") || udfClassName.equals("UDFToFloat"))
                    {
                        castExpr = true;
                    }
                }
    ...
    public static RexNode handleExplicitCast(GenericUDF udf, RelDataType returnType, List<RexNode> childRexNodeLst, RexBuilder rexBuilder)
    {
        ...
        {
            if ((udf instanceof GenericUDFToChar) || (udf instanceof GenericUDFToVarchar) || (udf instanceof GenericUDFToString) || (udf instanceof GenericUDFToDecimal) || (udf instanceof GenericUDFToDate) || (udf instanceof GenericUDFTimestamp) || (udf instanceof GenericUDFToTimestampLocalTZ) || (udf instanceof GenericUDFToBinary) || castExprUsingUDFBridge(udf) || (udf instanceof GenericUDFToMap) || (udf instanceof GenericUDFToArray) || (udf instanceof GenericUDFToStruct))
            {
    ...
    public static List<RexNode> rewriteCaseChildren(String funcText, List<RexNode> childRexNodeLst, RexBuilder rexBuilder) throws SemanticException
    {
        ...
        {
            newChildRexNodeLst.add(rexBuilder.makeNullLiteral(newChildRexNodeLst.get(newChildRexNodeLst.size() - 1).getType()));
        }
    ...
    public static List<RexNode> transformInToOrOperands(List<RexNode> operands, RexBuilder rexBuilder)
    {
        ...
        if (operands.get(0).getKind() != SqlKind.ROW)
        {
            ...
            for (int i = 1; i < operands.size(); i++)
            {
                ...
                disjuncts.add(rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, columnExpression, valueExpression));
            }
        } else
        {
            ...
            for (int i = 1; i < operands.size(); i++)
            {
                ...
                {
                    conjuncts.add(rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, columnExpressions.getOperands().get(j), valueExpressions.getOperands().get(j)));
                }
                if (conjuncts.size() > 1)
                {
                    disjuncts.add(rexBuilder.makeCall(SqlStdOperatorTable.AND, conjuncts));
                } else
                {
    ...
    public static List<RexNode> rewriteCoalesceChildren(List<RexNode> childRexNodeLst, RexBuilder rexBuilder)
    {
        ...
        for (; i < childRexNodeLst.size() - 1; ++i)
        {
            ...
            RexNode childCond = rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, child);
