...
public class TezProcessor extends AbstractLogicalIOProcessor
{
    ...
    private static class ReflectiveProgressHelper
    {
        ...
        ReflectiveProgressHelper(Configuration conf, Map<String, LogicalInput> inputs, ProcessorContext processorContext, String processorName)
        {
            ...
            try
            {
                ...
                progressHelper = progressHelperClass.getDeclaredConstructor(Map.class, ProcessorContext.class, String.class).newInstance(inputs, processorContext, processorName);
        ...
        void scheduleProgressTaskService(long delay, long period)
        {
            ...
            try
            {
                progressHelperClass.getDeclaredMethod("scheduleProgressTaskService", long.class, long.class).invoke(progressHelper, delay, period);
    ...
    public void run(Map<String, LogicalInput> inputs, Map<String, LogicalOutput> outputs) throws Exception
    {
        ...
        synchronized (this)
        {
            ...
            if (limitReached)
            {
                LOG.info("TezProcessor exits early as query limit already reached, vertex: {}, task: {}, attempt: {}", jobConf.get(HIVE_TEZ_VERTEX_NAME), jobConf.get(HIVE_TEZ_TASK_INDEX), jobConf.get(HIVE_TEZ_TASK_ATTEMPT_NUMBER));
                aborted.set(true);
            }
