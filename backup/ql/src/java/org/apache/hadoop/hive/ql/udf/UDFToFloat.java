 ...
 * UDFToFloat.
 */
@VectorizedExpressions({CastTimestampToDouble.class, CastLongToFloatViaLongToDouble.class, CastDecimalToFloat.class, CastStringToFloat.class})
...
public class UDFToFloat extends UDF
{
    ...
    public FloatWritable evaluate(TimestampWritableV2 i)
    {
        ...
        } else
        {
            try
            {
                floatWritable.set((float) TimestampTZUtil.convertTimestampTZToDouble(UDFUtils.getTimestampTZFromTimestamp(i.getTimestamp())));
                return floatWritable;
            } catch (NumberFormatException e)
            {
                // MySQL returns 0 if the string is not a well-formed numeric value.
