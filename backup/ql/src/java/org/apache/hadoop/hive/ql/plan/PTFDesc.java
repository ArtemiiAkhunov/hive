...
public class PTFDesc extends AbstractOperatorDesc
{
    ...
    public class PTFOperatorExplainVectorization extends OperatorExplainVectorization
    {
        ...
        {
            return Arrays.asList(vectorPTFInfo.getEvaluatorInputExpressions()).stream().map(i -> i[0]).collect(Collectors.toList()).toString();
        }
        ...
        @Explain(vectorization = Vectorization.DETAIL, displayName = "allEvaluatorsAreStreaming", explainLevels = {Level.DEFAULT, Level.EXTENDED})
