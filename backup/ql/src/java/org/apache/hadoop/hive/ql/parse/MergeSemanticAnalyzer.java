...
public class MergeSemanticAnalyzer extends RewriteSemanticAnalyzer<MergeStatement>
{
    ...
    */
        ...
        MergeStatement.MergeStatementBuilder mergeStatementBuilder = MergeStatement.withTarget(targetTable, getFullTableNameForSQL(targetNameNode), targetAlias).sourceName(sourceName).sourceAlias(getSourceAlias(source, sourceName)).onClauseAsText(onClauseAsText);
        ...
        for (ASTNode whenClause : whenClauses)
        {
            switch (getWhenClauseOperation(whenClause).getType())
            {
                case HiveParser.TOK_INSERT:
                    ...
                    OnClauseAnalyzer oca = new OnClauseAnalyzer(onClause, targetTable, targetAlias, conf, onClauseAsText);
                    oca.analyze();

                    mergeStatementBuilder.addWhenClause(handleInsert(whenClause, oca.getPredicate(), targetTable)).onClausePredicate(oca.getPredicate());
                    break;
    ...
    private MergeStatement.UpdateClause handleUpdate(ASTNode whenMatchedUpdateClause, Table targetTable, String deleteExtraPredicate) throws SemanticException
    {
        ...
        for (FieldSchema fs : nonPartCols)
        {
            ...
            if (setColsExprs.containsKey(name))
            {
                ...
                if (setColExpr.getType() == HiveParser.TOK_TABLE_OR_COL && setColExpr.getChildCount() == 1 && setColExpr.getChild(0).getType() == HiveParser.TOK_DEFAULT_VALUE)
                {
    ...
    private List<ASTNode> findWhenClauses(ASTNode tree, int start) throws SemanticException
    {
        ...
        for (int idx = start; idx < tree.getChildCount(); idx++)
        {
            ...
            assert whenClause.getType() == HiveParser.TOK_MATCHED || whenClause.getType() == HiveParser.TOK_NOT_MATCHED : "Unexpected node type found: " + whenClause.getType() + addParseInfo(whenClause);
    ...
    private MergeStatement.InsertClause handleInsert(ASTNode whenNotMatchedClause, String onClausePredicate, Table targetTable) throws SemanticException
    {
        ...
        ASTNode valuesNode = (ASTNode) children.stream().filter(n -> ((ASTNode) n).getType() == HiveParser.TOK_FUNCTION).findFirst().get();
        ASTNode columnListNode = (ASTNode) children.stream().filter(n -> ((ASTNode) n).getType() == HiveParser.TOK_TABCOLNAME).findFirst().orElse(null);
        ...
        if (columnListNode != null)
        {
            ...
            for (int i = 0; i < columnListNode.getChildCount(); ++i)
            {
                ...
                String columnName = ctx.getTokenRewriteStream().toString(columnNameNode.getTokenStartIndex(), columnNameNode.getTokenStopIndex()).trim();
        ...
        for (int i = 1; i < valuesNode.getChildCount(); ++i)
        {
            ...
            String value;
            if (valueNode.getType() == HiveParser.TOK_TABLE_OR_COL && valueNode.getChild(0).getType() == HiveParser.TOK_DEFAULT_VALUE)
            {
                value = ObjectUtils.defaultIfNull(defaultConstraints.get(i - 1), "NULL");
            } else
            {
                value = ctx.getTokenRewriteStream().toString(MERGE_INSERT_VALUES_PROGRAM, valueNode.getTokenStartIndex(), valueNode.getTokenStopIndex()).trim();
            }
            values.add(value);
        }
    ...
    private static final class OnClauseAnalyzer
    {
        ...
        private void analyze()
        {
            ...
            {
                throw new IllegalArgumentException("Found > 2 table refs in ON clause.  Found " + tableNamesFound + " in " + onClauseAsString);
            }
            handleUnresolvedColumns();
            if (tableNamesFound.size() > 2)
            {
                throw new IllegalArgumentException("Found > 2 table refs in ON clause (incl unresolved).  " + "Found " + tableNamesFound + " in " + onClauseAsString);
            }
        }
        ...
        private String getPredicate()
        {
            ...
            if (targetCols == null)
            {
                ...
                throw new IllegalArgumentException(ErrorMsg.INVALID_TABLE_IN_ON_CLAUSE_OF_MERGE.format(targetTableNameInSourceQuery, onClauseAsString));
            }
