...
public class TestTablePermissionFilterAlgos
{
    ...
    // lookup some
    private final List<TestTable> filtered = tables.stream().filter(t -> t.getTableName().endsWith("0")).collect(Collectors.toList());
    ...
    public void testIndexedLookup()
    {
        ...
        for (int l = 0; l < NLOOPS; ++l)
        {
            ...
            List<TestTable> tts = l % 2 == 1 ? tables : filtered;
            cnt = 0;
            // lookup all
    ...
    public void testKeyPairLookup()
    {
        ...
        for (int l = 0; l < NLOOPS; ++l)
        {
            Set<ImmutablePair<String, String>> index = tables.stream().map(TestTablePermissionFilterAlgos::tableKey).collect(Collectors.toSet());
            List<TestTable> tts = l % 2 == 1 ? tables : filtered;
            int cnt = 0;
            // lookup all
            for (TestTable tt : tts)
    ...
    public void testFullNameLookup()
    {
        ...
        for (int l = 0; l < NLOOPS; ++l)
        {
            Set<String> index = tables.stream().map(TestTablePermissionFilterAlgos::tableFullName).collect(Collectors.toSet());
            List<TestTable> tts = l % 2 == 1 ? tables : filtered;
            int cnt = 0;
            // lookup all
            for (TestTable tt : tts)
