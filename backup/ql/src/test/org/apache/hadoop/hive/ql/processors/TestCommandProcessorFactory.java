...
public class TestCommandProcessorFactory
{
    ...
    public void testAvailableCommands() throws Exception
    {
        ...
        for (HiveCommand command : HiveCommand.values())
        {
            ...
            Assert.assertNotNull("Cmd " + cmd[0] + " not return null", CommandProcessorFactory.getForHiveCommandInternal(cmd, conf, command.isOnlyForTesting()));
            Assert.assertNotNull("Cmd " + cmd + " not return null", CommandProcessorFactory.getForHiveCommandInternal(cmd, conf, command.isOnlyForTesting()));
        }
        ...
        for (HiveCommand command : HiveCommand.values())
        {
            ...
            try
            {
                CommandProcessorFactory.getForHiveCommandInternal(cmd, conf, command.isOnlyForTesting());
