...
public class TestRangerLoadTask
{
    ...
    public void setup() throws Exception
    {
        ...
        when(mockClient.changeDataSet(Mockito.anyList(), Mockito.anyString(), Mockito.anyString())).thenCallRealMethod();
        when(mockClient.getDenyPolicyForReplicatedDb(Mockito.anyString(), Mockito.anyString(), Mockito.anyString())).thenCallRealMethod();
    ...
    public void testSuccessRangerDumpMetrics() throws Exception
    {
        ...
        Assert.assertTrue(logStr.contains("{\"sourceDbName\":\"srcdb\",\"targetDbName\":\"tgtdb\"" + ",\"estimatedNumPolicies\":1,\"loadStartTime\":"));
        Assert.assertTrue(logStr.contains("{\"sourceDbName\":\"srcdb\",\"targetDbName\"" + ":\"tgtdb\",\"actualNumPolicies\":1,\"loadEndTime\""));
    ...
    public void testRangerDenyTask() throws Exception
    {
        ...
        Mockito.verify(mockClient, Mockito.never()).deleteRangerPolicy(policyName.capture(), rangerEndpoint.capture(), serviceName.capture(), confCaptor.capture());
        Mockito.verify(mockClient, Mockito.times(1)).importRangerPolicies(rangerPolicyCapture.capture(), targetDb.capture(), rangerEndpoint.capture(), serviceName.capture(), confCaptor.capture());
        ...
        Assert.assertEquals(0, status);
        Mockito.verify(mockClient, Mockito.times(1)).deleteRangerPolicy(policyName.capture(), rangerEndpoint.capture(), serviceName.capture(), confCaptor.capture());
    ...
    public void testRangerEndpointCreation() throws Exception
    {
        ...
        Assert.assertTrue(rangerRestClient.getRangerExportUrl("http://ranger.apache.org:6080", "hive", "dbname").equals("http://ranger.apache.org:6080/service/plugins/" + "policies/exportJson?serviceName=hive&polResource=dbname&resource%3Adatabase=dbname&serviceType=hive" + "&resourceMatchScope=self_or_ancestor&resourceMatch=full"));

        Assert.assertTrue(rangerRestClient.getRangerExportUrl("http://ranger.apache.org:6080/", "hive", "dbname").equals("http://ranger.apache.org:6080/service/plugins/" + "policies/exportJson?serviceName=hive&polResource=dbname&resource%3Adatabase=dbname&serviceType=hive" + "&resourceMatchScope=self_or_ancestor&resourceMatch=full"));

        Assert.assertTrue(rangerRestClient.getRangerImportUrl("http://ranger.apache.org:6080/", "dbname").equals("http://ranger.apache.org:6080/service/plugins/policies/importPoliciesFromFile" + "?updateIfExists=true&polResource=dbname&policyMatchingAlgorithm=matchByName"));
    ...
    public void testChangeDataSet() throws Exception
    {
        ...
        assertEqualsRangerPolicies(rangerPolicies, rangerRestClient.changeDataSet(rangerPolicies, null, null), "aa");
        assertEqualsRangerPolicies(rangerPolicies, rangerRestClient.changeDataSet(rangerPolicies, "aa", null), "aa");
        assertEqualsRangerPolicies(rangerPolicies, rangerRestClient.changeDataSet(rangerPolicies, null, "aa"), "aa");
        assertEqualsRangerPolicies(rangerPolicies, rangerRestClient.changeDataSet(rangerPolicies, "aa", "aa"), "aa");
        assertNotEqualsRangerPolicies(rangerPolicies, rangerRestClient.changeDataSet(rangerPolicies, "aa", "tgt_aa"), "tgt_aa");
    }
    ...
    private void assertNotEqualsRangerPolicies(List<RangerPolicy> expectedRangerPolicies, List<RangerPolicy> actualRangerPolicies, String targetName)
    {
        ...
        for (int index = 0; index < expectedRangerPolicies.size(); index++)
        {
            ...
            Assert.assertEquals(expectedRangerPolicies.get(index).getDescription(), actualRangerPolicies.get(index).getDescription());
            Assert.assertEquals(expectedRangerPolicies.get(index).getPolicyType(), actualRangerPolicies.get(index).getPolicyType());
            Assert.assertEquals(expectedRangerPolicies.get(index).getResources().size(), actualRangerPolicies.get(index).getResources().size());
            Assert.assertEquals(expectedRangerPolicies.get(index).getResources().size(), actualRangerPolicies.get(index).getResources().size());
            RangerPolicy.RangerPolicyResource expectedRangerPolicyResource = expectedRangerPolicies.get(index).getResources().get("database");
            RangerPolicy.RangerPolicyResource actualRangerPolicyResource = actualRangerPolicies.get(index).getResources().get("database");
            Assert.assertEquals(expectedRangerPolicyResource.getValues().size(), actualRangerPolicyResource.getValues().size());
            for (int resourceIndex = 0; resourceIndex < expectedRangerPolicyResource.getValues().size(); resourceIndex++)
            {
                Assert.assertEquals(actualRangerPolicyResource.getValues().get(index), targetName);
            }
        }
    }
    ...
    private void assertEqualsRangerPolicies(List<RangerPolicy> expectedRangerPolicies, List<RangerPolicy> actualRangerPolicies, String sourceName)
    {
        ...
        for (int index = 0; index < expectedRangerPolicies.size(); index++)
        {
            ...
            for (int resourceIndex = 0; resourceIndex < expectedRangerPolicyResource.getValues().size(); resourceIndex++)
            {
                ...
                Assert.assertEquals(actualRangerPolicyResource.getValues().get(index), sourceName);
            }
        }
    }
}
