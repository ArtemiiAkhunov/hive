...
public class TestHiveCredentialProviders
{
    ...
    private static final Collection<String> REDACTED_PROPERTIES = Arrays.asList(JobConf.MAPRED_MAP_TASK_ENV, JobConf.MAPRED_REDUCE_TASK_ENV, MRJobConfig.MR_AM_ADMIN_USER_ENV, TezConfiguration.TEZ_AM_LAUNCH_ENV);
    ...
    public void testJobCredentialProvider() throws Exception
    {
        ...
        Assert.assertEquals(HIVE_JOB_CREDSTORE_PASSWORD_ENVVAR_VAL, getValueFromJobConf(jobConf.get(MRJobConfig.MR_AM_ADMIN_USER_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));

        // make sure TEZ AM environment points to HIVE_JOB_CREDSTORE_PASSWORD
        Assert.assertEquals(HIVE_JOB_CREDSTORE_PASSWORD_ENVVAR_VAL, getValueFromJobConf(jobConf.get(TezConfiguration.TEZ_AM_LAUNCH_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));

        Assert.assertTrue(jobConf.getStringCollection(MRJobConfig.MR_JOB_REDACTED_PROPERTIES).containsAll(REDACTED_PROPERTIES));
    }
    ...
    public void testHadoopCredentialProvider() throws Exception
    {
        ...
        Assert.assertEquals(HADOOP_CREDSTORE_PASSWORD_ENVVAR_VAL, getValueFromJobConf(jobConf.get(MRJobConfig.MR_AM_ADMIN_USER_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));

        // make sure TEZ AM environment points to HADOOP_CREDSTORE_PASSWORD
        Assert.assertEquals(HADOOP_CREDSTORE_PASSWORD_ENVVAR_VAL, getValueFromJobConf(jobConf.get(TezConfiguration.TEZ_AM_LAUNCH_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));
    ...
    public void testNoCredentialProviderWithPassword() throws Exception
    {
        ...
        Assert.assertNull(getValueFromJobConf(jobConf.get(MRJobConfig.MR_AM_ADMIN_USER_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));

        Assert.assertNull(getValueFromJobConf(jobConf.get(TezConfiguration.TEZ_AM_LAUNCH_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));

        REDACTED_PROPERTIES.forEach(property -> Assert.assertFalse(jobConf.getStringCollection(MRJobConfig.MR_JOB_REDACTED_PROPERTIES).contains(property)));
    }
    ...
    public void testNoCredentialProvider() throws Exception
    {
        ...
        assertNull(getValueFromJobConf(jobConf.get(MRJobConfig.MR_AM_ADMIN_USER_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));

        assertNull(getValueFromJobConf(jobConf.get(TezConfiguration.TEZ_AM_LAUNCH_ENV), HADOOP_CREDENTIAL_PASSWORD_ENVVAR));
