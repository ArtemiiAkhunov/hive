...
public class TestOrcSplit
{
    ...
    public void testDeleteDeltasFiltering() throws Exception
    {
        ...
        for (int i = 1; i <= 5; i = i + 2)
        {
            ...
            options.minimumWriteId(i).maximumWriteId(i);
            updater = new OrcRecordUpdater(root, options);
            updater.insert(options.getMinimumWriteId(), new DummyRow(i, 0, options.getMinimumWriteId(), bucket));
            updater.close(false);

            // Transaction that deletes previously inserted row
            options.minimumWriteId(i + 1).maximumWriteId(i + 1);
            updater = new OrcRecordUpdater(root, options);
            updater.delete(options.getMinimumWriteId(), new DummyRow(-1, 0, i, bucket));
            updater.close(false);
        }

        conf.set(ValidTxnList.VALID_TXNS_KEY, new ValidReadTxnList(new long[0], new BitSet(), 1000, Long.MAX_VALUE).writeToString());
        conf.set(ValidWriteIdList.VALID_WRITEIDS_KEY, "tbl:6:" + Long.MAX_VALUE + "::");
