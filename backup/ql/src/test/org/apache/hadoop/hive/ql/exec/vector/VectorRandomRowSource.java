...
public class VectorRandomRowSource
{
    ...
    public static class GenerationSpec
    {
        ...
        {
            SAME_TYPE, OMIT_GENERATION, STRING_FAMILY, STRING_FAMILY_OTHER_TYPE_VALUE, TIMESTAMP_MILLISECONDS, VALUE_LIST
        }
        ...
        private GenerationSpec(GenerationKind generationKind, TypeInfo typeInfo, boolean columnAllowNulls, TypeInfo sourceTypeInfo, StringGenerationOption stringGenerationOption, List<Object> valueList)
        {
        ...
        {
            return new GenerationSpec(GenerationKind.SAME_TYPE, typeInfo, allowNulls, null, null, null);
        }
        ...
        {
            return new GenerationSpec(GenerationKind.STRING_FAMILY_OTHER_TYPE_VALUE, typeInfo, true, otherTypeTypeInfo, null, null);
        }
        ...
        {
            return new GenerationSpec(GenerationKind.VALUE_LIST, typeInfo, true, null, null, valueList);
        }
    }
    ...
    public StructObjectInspector partialRowStructObjectInspector(int partialFieldCount)
    {
        ...
        return ObjectInspectorFactory.getStandardStructObjectInspector(columnNames, objectInspectorList);
    }
    ...
    public void init(Random r, SupportedTypes supportedTypes, int maxComplexDepth, boolean allowNull, boolean isUnicodeOk)
    {
        this.r = r;
    ...
    public void init(Random r, Set<String> allowedTypeNameSet, int maxComplexDepth, boolean allowNull, boolean isUnicodeOk)
    {
        this.r = r;
        this.allowNull = allowNull;
        this.isUnicodeOk = isUnicodeOk;
    ...
    public void initExplicitSchema(Random r, List<String> explicitTypeNameList, int maxComplexDepth, boolean allowNull, boolean isUnicodeOk, List<DataTypePhysicalVariation> explicitDataTypePhysicalVariationList)
    {
        this.r = r;
        this.allowNull = allowNull;
        this.isUnicodeOk = isUnicodeOk;
        ...
        {
            TypeInfo typeInfo = TypeInfoUtils.getTypeInfoFromTypeString(explicitTypeName);
            generationSpecList.add(GenerationSpec.createSameType(typeInfo));
        }

        chooseSchema(SupportedTypes.ALL, null, generationSpecList, explicitDataTypePhysicalVariationList, maxComplexDepth);
    }

    public void initGenerationSpecSchema(Random r, List<GenerationSpec> generationSpecList, int maxComplexDepth, boolean allowNull, boolean isUnicodeOk, List<DataTypePhysicalVariation> explicitDataTypePhysicalVariationList)
    {
        this.r = r;
        this.allowNull = allowNull;
        this.isUnicodeOk = isUnicodeOk;
     ...
     */
    private static String[] possibleHivePrimitiveTypeNames = {"boolean", "tinyint", "smallint", "int", "bigint", "date", "float", "double", "string", "char", "varchar", "binary", "date", "timestamp", "interval_year_month", "interval_day_time", "decimal", "decimal64"};

    private static String[] possibleHiveComplexTypeNames = {"array", "struct", "uniontype", "map"};

    public static String getRandomTypeName(Random random, SupportedTypes supportedTypes, Set<String> allowedTypeNameSet)
    {
    ...
    public static String getDecoratedTypeName(Random random, String typeName, SupportedTypes supportedTypes, Set<String> allowedTypeNameSet, int depth, int maxDepth)
    {
        ...
        } else if (typeName.equals("array"))
        {
            ...
            elementTypeName = getDecoratedTypeName(random, elementTypeName, supportedTypes, allowedTypeNameSet, depth, maxDepth);
            typeName = String.format("array<%s>", elementTypeName);
        } else if (typeName.equals("map"))
        {
            String keyTypeName = getRandomTypeName(random, SupportedTypes.PRIMITIVES, allowedTypeNameSet);
            keyTypeName = getDecoratedTypeName(random, keyTypeName, supportedTypes, allowedTypeNameSet, depth, maxDepth);
            String valueTypeName = getRandomTypeName(random, supportedTypes, allowedTypeNameSet);
            valueTypeName = getDecoratedTypeName(random, valueTypeName, supportedTypes, allowedTypeNameSet, depth, maxDepth);
        ...
        } else if (typeName.equals("struct"))
        {
            ...
            {
                String fieldTypeName = getRandomTypeName(random, supportedTypes, allowedTypeNameSet);
                fieldTypeName = getDecoratedTypeName(random, fieldTypeName, supportedTypes, allowedTypeNameSet, depth, maxDepth);
                if (i > 0)
                {
                    sb.append(",");
                }
    ...
    private void chooseSchema(SupportedTypes supportedTypes, Set<String> allowedTypeNameSet, List<GenerationSpec> generationSpecList, List<DataTypePhysicalVariation> explicitDataTypePhysicalVariationList, int maxComplexDepth)
    {
        ...
        for (int c = 0; c < columnCount; c++)
        {
            ...
            if (generationSpecList != null)
            {
                ...
                dataTypePhysicalVariation = explicitDataTypePhysicalVariationList != null ? explicitDataTypePhysicalVariationList.get(c) : DataTypePhysicalVariation.NONE;
            ...
            String decoratedTypeName = getDecoratedTypeName(typeName, supportedTypes, allowedTypeNameSet, 0, maxComplexDepth);
    ...
    {
        String string;
        string = VectorRandomRowSource.randomPrimitiveObject(random, (PrimitiveTypeInfo) specialValueTypeInfo).toString();
        ...
        } else
        {
            ...
            if (generateSentences)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    if (randomWord.length() > 0 && ((i == 0 && capitalizeFirstWord) || random.nextInt(20) == 0))
                    {
    ...
    public static Object[] randomPrimitiveRow(int columnCount, Random r, PrimitiveTypeInfo[] primitiveTypeInfos, DataTypePhysicalVariation[] dataTypePhysicalVariations)
    {
        ...
        return randomPrimitiveRow(columnCount, r, primitiveTypeInfos, dataTypePhysicalVariations, false);
    }

    public static Object[] randomPrimitiveRow(int columnCount, Random r, PrimitiveTypeInfo[] primitiveTypeInfos, DataTypePhysicalVariation[] dataTypePhysicalVariations, boolean isUnicodeOk)
    {

        final Object row[] = new Object[columnCount];
        for (int c = 0; c < columnCount; c++)
        {
            row[c] = randomPrimitiveObject(r, primitiveTypeInfos[c], dataTypePhysicalVariations[c], isUnicodeOk);
        }
        return row;
    }
    ...
    public static Object[] randomWritablePrimitiveRow(int columnCount, Random r, PrimitiveTypeInfo[] primitiveTypeInfos, DataTypePhysicalVariation[] dataTypePhysicalVariations)
    {
        ...
        for (int c = 0; c < columnCount; c++)
        {
            ...
            final DataTypePhysicalVariation dataTypePhysicalVariation = (dataTypePhysicalVariations != null ? dataTypePhysicalVariations[c] : DataTypePhysicalVariation.NONE);
            final ObjectInspector objectInspector;
            if (primitiveTypeInfo instanceof DecimalTypeInfo && dataTypePhysicalVariation == DataTypePhysicalVariation.DECIMAL_64)
            {
                objectInspector = PrimitiveObjectInspectorFactory.getPrimitiveWritableObjectInspector(TypeInfoFactory.longTypeInfo);
    ...
    {
        return getWritablePrimitiveObject(primitiveTypeInfo, objectInspector, DataTypePhysicalVariation.NONE, object);
    }
    ...
    public static Object getWritablePrimitiveObject(PrimitiveTypeInfo primitiveTypeInfo, ObjectInspector objectInspector, DataTypePhysicalVariation dataTypePhysicalVariation, Object object)
    {
        ...
        switch (primitiveTypeInfo.getPrimitiveCategory())
        {
            ...
            case DECIMAL:
            {
                // Do not represent DECIMAL_64 to make ROW mode tests easier --
                // make the VECTOR mode tests convert into the VectorizedRowBatch.
                WritableHiveDecimalObjectInspector writableOI = new WritableHiveDecimalObjectInspector((DecimalTypeInfo) primitiveTypeInfo);
    ...
    public static Object getWritableObject(TypeInfo typeInfo, ObjectInspector objectInspector, Object object)
    {
        ...
        {
            case PRIMITIVE:
                return getWritablePrimitiveObject((PrimitiveTypeInfo) typeInfo, objectInspector, DataTypePhysicalVariation.NONE, object);
            case STRUCT:
            {
    ...
    public static Object getNonWritablePrimitiveObject(Object object, TypeInfo typeInfo, ObjectInspector objectInspector)
    {
        ...
        switch (primitiveTypeInfo.getPrimitiveCategory())
        {
            ...
            case BINARY:
            default:
                throw new RuntimeException("Unexpected primitive category " + primitiveTypeInfo.getPrimitiveCategory());
        }
    }

    public static Object getNonWritableObject(Object object, TypeInfo typeInfo, ObjectInspector objectInspector)
    {
        final Category category = typeInfo.getCategory();
        switch (category)
        {
            ...
            case STRUCT:
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    final StructField structField = structFields.get(i);
                    final TypeInfo fieldTypeInfo = fieldTypeInfos.get(i);
                    result.add(getNonWritableObject(input.get(i), fieldTypeInfo, structField.getFieldObjectInspector()));
                }
                return result;
            }
            default:
    ...
    {
        return randomWritable(r, typeInfos[column], objectInspectorList.get(column), dataTypePhysicalVariations[column], columnAllowNull && allowNull);
    }

    public Object randomWritable(int column)
    {
        return randomWritable(r, typeInfos[column], objectInspectorList.get(column), dataTypePhysicalVariations[column], allowNull);
    }
    ...
    public static Object randomWritable(Random random, TypeInfo typeInfo, ObjectInspector objectInspector, boolean allowNull)
    {
        return randomWritable(random, typeInfo, objectInspector, DataTypePhysicalVariation.NONE, allowNull);
    }

    public static Object randomWritable(Random random, TypeInfo typeInfo, ObjectInspector objectInspector, DataTypePhysicalVariation dataTypePhysicalVariation, boolean allowNull)
    {
        ...
        switch (typeInfo.getCategory())
        {
            case PRIMITIVE:
            {
                ...
                return getWritablePrimitiveObject((PrimitiveTypeInfo) typeInfo, objectInspector, dataTypePhysicalVariation, object);
            }
            case LIST:
            {
                if (allowNull && random.nextInt(20) == 0)
                ...
                final StandardListObjectInspector listObjectInspector = (StandardListObjectInspector) objectInspector;
                final ObjectInspector elementObjectInspector = listObjectInspector.getListElementObjectInspector();
                final TypeInfo elementTypeInfo = TypeInfoUtils.getTypeInfoFromObjectInspector(elementObjectInspector);
                ...
                if (elementTypeInfo.getCategory() == Category.PRIMITIVE)
                {
                    ...
                    if (primitiveCategory == PrimitiveCategory.STRING || primitiveCategory == PrimitiveCategory.BINARY || primitiveCategory == PrimitiveCategory.CHAR || primitiveCategory == PrimitiveCategory.VARCHAR)
                    {
                        isStringFamily = true;
                    }
                }
                ...
                for (int i = 0; i < elementCount; i++)
                {
                    final Object ele = randomWritable(random, elementTypeInfo, elementObjectInspector, allowNull);
            ...
            case MAP:
            {
                ...
                final StandardMapObjectInspector mapObjectInspector = (StandardMapObjectInspector) objectInspector;
                final ObjectInspector keyObjectInspector = mapObjectInspector.getMapKeyObjectInspector();
                final TypeInfo keyTypeInfo = TypeInfoUtils.getTypeInfoFromObjectInspector(keyObjectInspector);
                final ObjectInspector valueObjectInspector = mapObjectInspector.getMapValueObjectInspector();
                final TypeInfo valueTypeInfo = TypeInfoUtils.getTypeInfoFromObjectInspector(valueObjectInspector);
            ...
            case STRUCT:
            {
                ...
                final StandardStructObjectInspector structObjectInspector = (StandardStructObjectInspector) objectInspector;
            ...
            case UNION:
            {
                final StandardUnionObjectInspector unionObjectInspector = (StandardUnionObjectInspector) objectInspector;
                ...
                final ObjectInspector fieldObjectInspector = objectInspectorList.get(tag);
                final TypeInfo fieldTypeInfo = TypeInfoUtils.getTypeInfoFromObjectInspector(fieldObjectInspector);
    ...
    public static Object randomPrimitiveObject(Random r, PrimitiveTypeInfo primitiveTypeInfo, DataTypePhysicalVariation dataTypePhysicalVariation, boolean isUnicodeOk)
    {
    ...
    {
        return !isUnicodeOk || r.nextBoolean() ? RandomTypeUtil.getRandString(r, "abcdefghijklmnopqrstuvwxyz", length) : RandomTypeUtil.getRandUnicodeString(r, length);
    }
    ...
    public static HiveVarchar getRandHiveVarchar(Random r, VarcharTypeInfo varcharTypeInfo, boolean isUnicodeOk)
    {
    ...
    public static HiveDecimal getRandHiveDecimal(Random r, DecimalTypeInfo decimalTypeInfo)
    {
        while (true)
        {
            ...
            dec = HiveDecimal.enforcePrecisionScale(dec, decimalTypeInfo.getPrecision(), decimalTypeInfo.getScale());
            if (dec != null)
            {
                return dec;
            }
    ...
    public static HiveIntervalYearMonth getRandIntervalYearMonth(Random r)
    {
        ...
        final String intervalYearMonthStr = String.format("%s%d-%d", yearMonthSignStr, Integer.valueOf(1800 + r.nextInt(500)),  // year
    ...
    public static HiveIntervalDayTime getRandIntervalDayTime(Random r)
    {
        ...
        final String yearMonthSignStr = r.nextInt(2) == 0 ? "" : "-";
        final String dayTimeStr = String.format("%s%d %02d:%02d:%02d%s", yearMonthSignStr, Integer.valueOf(1 + r.nextInt(28)),      // day
