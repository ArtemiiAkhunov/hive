...
public class TestReplStateLogTimeFormat
{
    ...
    private void verifyAnnotationAndSetEpoch(ReplState replState) throws Exception
    {
        ...
        assertTrue(String.format("Class %s has a time field which is not annotated with @JsonSerialize(using = ReplUtils.TimeSerializer.class) " + "Please annotate the time field with it or add it to appropriate exempted set above", replState.getClass().getName()), isAnnotationSet);
    }
    ...
    private void verifyUTCString(ReplState replState) throws Exception
    {
        ...
        assertTrue(String.format("Expected UTC string %s not found in serialized representation of %s", UTCString, replState.getClass().getName()), json.contains(UTCString));
    }


    private void verifyTimeFormat(String packagePath, Set<Class<? extends ReplState>> EXEMPTED_CLASS) throws Exception
    {
        Set<Class<? extends ReplState>> replStateLogClasses = new Reflections(packagePath).getSubTypesOf(ReplState.class);
