...
public class TestVectorDateExpressions
{
    ...
    {
        return new TimestampWritableV2(org.apache.hadoop.hive.common.type.Timestamp.ofEpochMilli(DateWritableV2.daysToMillis((int) daysSinceEpoch)));
    }

    private TimestampLocalTZWritable toTimestampLocalTZWritable(long daysSinceEpoch)
    {
        return new TimestampLocalTZWritable(TimestampTZUtil.convert(Timestamp.ofEpochMilli(DateWritableV2.daysToMillis((int) daysSinceEpoch)), ZoneId.systemDefault()));
    }
    ...
    {
        runner = Executors.newFixedThreadPool(3, new ThreadFactoryBuilder().setNameFormat("date-tester-thread-%d").build());
    }
