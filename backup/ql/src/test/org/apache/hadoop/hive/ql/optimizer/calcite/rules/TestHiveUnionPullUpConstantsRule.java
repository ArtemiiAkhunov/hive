...
public class TestHiveUnionPullUpConstantsRule
{
    ...
    public void testNonNullableFields()
    {
        ...
        final RelNode plan = relBuilder.scan("t").filter(eq(relBuilder, "f1", 1)).project(relBuilder.field("f1"), relBuilder.field("f2")).scan("t").filter(eq(relBuilder, "f1", 1)).project(relBuilder.field("f1"), relBuilder.field("f2")).union(true).build();
    ...
    public void testNullableFields()
    {
        ...
        final RelNode plan = relBuilder.scan("t").filter(eq(relBuilder, "f1", 1)).project(relBuilder.field("f1"), relBuilder.field("f2")).scan("t").filter(eq(relBuilder, "f1", 1)).project(relBuilder.field("f1"), relBuilder.field("f2")).union(false).build();
