...
public class TestVectorizer
{
    ...
    public void testAggregateOnUDF() throws HiveException, VectorizerCannotVectorizeException
    {
        ...
        vectorDesc.setVecAggrDescs(new VectorAggregationDesc[]{new VectorAggregationDesc.VectorAggregationDescBuilder().aggregationName(aggDesc.getGenericUDAFName()).evaluator(new GenericUDAFSum.GenericUDAFSumLong()).udafEvaluatorMode(aggDesc.getMode()).inputTypeInfo(TypeInfoFactory.longTypeInfo).inputColVectorType(ColumnVector.Type.LONG).outputTypeInfo(TypeInfoFactory.longTypeInfo).outputColVectorType(ColumnVector.Type.LONG).vectorAggregationClass(VectorUDAFCountStar.class).build()});
