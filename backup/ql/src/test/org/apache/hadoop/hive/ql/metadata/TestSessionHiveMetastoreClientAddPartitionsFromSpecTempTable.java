...
public class TestSessionHiveMetastoreClientAddPartitionsFromSpecTempTable extends TestAddPartitionsFromPartSpec
{
    ...
    protected void verifyPartition(Table table, String name, List<String> values, int index) throws Exception
    {
        ...
        Assert.assertEquals("The partition's parameter map should contain the partparamkey - partparamvalue pair.", DEFAULT_PARAM_VALUE + index, part.getParameters().get(DEFAULT_PARAM_KEY + index));
        ...
        Assert.assertEquals("The parameter map of the partition's storage descriptor should contain the partsdkey - partsdvalue pair.", "partsdvalue" + index, sd.getParameters().get("partsdkey" + index));
        Assert.assertEquals("The parameter's location is not correct.", getMetaStore().getWarehouseRoot() + "/" + TABLE_NAME + "/" + name, sd.getLocation());
