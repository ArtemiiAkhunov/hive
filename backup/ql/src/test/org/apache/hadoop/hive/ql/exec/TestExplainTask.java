...
public class TestExplainTask
{
    ...
    public void testGetJSONPlan() throws Exception
    {
        ...
        JsonNode result = objectMapper.readTree(uut.getJSONPlan(null, tasks, null, true, false, false, "Plan Optimized by CBO", null, null).toString());
        JsonNode expected = objectMapper.readTree("{\"cboInfo\":\"Plan Optimized by CBO\", \"STAGE DEPENDENCIES\":{\"mockTaskId\":" + "{\"ROOT STAGE\":\"TRUE\",\"BACKUP STAGE\":\"backup-id-mock\"}},\"STAGE PLANS\":" + "{\"mockTaskId\":{}}}");
    ...
    public void testOutputDependenciesJsonShouldMatch() throws Exception
    {
        ...
        JsonNode result = objectMapper.readTree(uut.outputDependencies(task, out, null, true, true, 0).toString());
        JsonNode expected = objectMapper.readTree("{\"ROOT STAGE\":\"TRUE\",\"BACKUP STAGE\":" + "\"" + BACKUP_ID + "\",\"TASK TYPE\":\"EXPLAIN\"}");
    ...
    {
        JsonNode result = objectMapper.readTree(uut.getJSONLogicalPlan(null, mockExplainWork()).toString());
    ...
    public void testOutputMapJsonShouldMatch() throws Exception
    {
        ...
        List<Object> tezList2 = new ArrayList<>(Arrays.asList(new Object[]{mockTezWorkDependency(), mockTezWorkDependency()}));
        ...
        JsonNode result = objectMapper.readTree(uut.outputMap(map, false, null, false, true, 0).toString());
        JsonNode expected = objectMapper.readTree("{\"key-1\":\"value-1\",\"tez-list-2\":" + "[{\"parent\":\"name\"}," + "{\"parent\":\"name\"}],\"tez-list-1\":" + "{\"parent\":\"name\"},\"empty-list\":\"[]\", \"map-1\":\"{inner-key-1=inner-value-1, " + "inner-key-2=[mock-tez-dependency]}\"}");
    ...
    public void testOutputPlanJsonShouldMatch() throws Exception
    {
        ...
        JsonNode result = objectMapper.readTree(uut.outputPlan(work, null, false, true, 0, null).toString());
        ...
        parentCollectOperator1.setChildOperators(new ArrayList<Operator<? extends OperatorDesc>>(Arrays.asList(new CollectOperator[]{child1})));
        parentCollectOperator1.setConf(new CollectDesc());

        result = objectMapper.readTree(uut.outputPlan(parentCollectOperator1, null, false, true, 0, null).toString());
        ...
        parentCollectOperator2.setChildOperators(new ArrayList<Operator<? extends OperatorDesc>>(Arrays.asList(new CollectOperator[]{child1, child2})));
        parentCollectOperator2.setConf(new CollectDesc());
        result = objectMapper.readTree(uut.outputPlan(parentCollectOperator2, null, false, true, 0, null).toString());
        expected = objectMapper.readTree("{\"Collect\":{\"children\":[{},{}]}}");
        assertEquals(expected, result);
    }
    ...
    public void testCollectAuthRelatedEntitiesJsonShouldMatch() throws Exception
    {
        ...
        JsonNode expected = objectMapper.readTree("{\"CURRENT_USER\":\"test-user\"," + "\"OPERATION\":\"EXPLAIN\",\"INPUTS\":[],\"OUTPUTS\":[]}");
        assertEquals(expected, result);
    }
