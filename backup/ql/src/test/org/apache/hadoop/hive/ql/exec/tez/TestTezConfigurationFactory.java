...
public class TestTezConfigurationFactory
{
    ...
    public void testAddProgrammaticallyAddedTezOptsToDagConf()
    {
        ...
        Assert.assertTrue("dagConfig should contain value for " + TezConfiguration.TEZ_GENERATE_DEBUG_ARTIFACTS, dagConf.containsKey(TezConfiguration.TEZ_GENERATE_DEBUG_ARTIFACTS));
        ...
        TezConfigurationFactory.addProgrammaticallyAddedTezOptsToDagConf(dagConf, conf);
        Assert.assertFalse("dagConfig should not contain value for xml sourced config " + TezConfiguration.TEZ_GENERATE_DEBUG_ARTIFACTS, dagConf.containsKey(TezConfiguration.TEZ_GENERATE_DEBUG_ARTIFACTS));

        conf.set(TezConfiguration.TEZ_AM_LAUNCH_CMD_OPTS, "asdf");
        TezConfigurationFactory.addProgrammaticallyAddedTezOptsToDagConf(dagConf, conf);
        // filtered out because it's AM scoped
        Assert.assertFalse("dagConfig should not contain value for " + TezConfiguration.TEZ_AM_LAUNCH_CMD_OPTS, dagConf.containsKey(TezConfiguration.TEZ_AM_LAUNCH_CMD_OPTS));
