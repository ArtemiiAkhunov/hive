...
public class TestHive
{
    ...
    public void testTable() throws Throwable
    {
        try
        {
            ...
            try
            {
                hm.dropTable(Warehouse.DEFAULT_DATABASE_NAME, tableName, true, false);
                Table ft2 = hm.getTable(Warehouse.DEFAULT_DATABASE_NAME, tableName, false);
    ...
    private Table createPartitionedTable(String dbName, String tableName) throws Exception
    {
        try
        {
            ...
            hm.dropTable(dbName, tableName);
            hm.createTable(tableName, Arrays.asList("key", "value"),   // Data columns.
        ...
        {
            fail("Unable to drop and create table " + StatsUtils.getFullyQualifiedTableName(dbName, tableName) + " because " + StringUtils.stringifyException(exception));
            throw exception;
        }
    }
    ...
    public void testDropMissingPartitionsByFilter() throws Throwable
    {
        ...
        for (int i = 10; i <= 12; i++)
        {
            Map<String, String> partitionSpec = new ImmutableMap.Builder<String, String>().put("ds", "20231129").put("hr", String.valueOf(i)).build();
            hm.createPartition(table, partitionSpec);
        }
        ...
        try
        {
            ...
            ExprNodeColumnDesc column = new ExprNodeColumnDesc(TypeInfoFactory.stringTypeInfo, "ds", null, true);
            List<String> values = Arrays.asList("20231130", "20231129");
            for (int i = 0; i < values.size(); i++)
            {
                ExprNodeGenericFuncDesc expr = PartitionUtils.makeBinaryPredicate("=", column, new ExprNodeConstantDesc(TypeInfoFactory.stringTypeInfo, values.get(i)));
    ...
    public void testGetPartitionsWithMaxLimit() throws Exception
    {
        ...
        try
        {
            ...
            // Default
            Assert.assertEquals(((List<Partition>) hm.getPartitions(table, new HashMap(), (short) 1)).get(0).getTPartition().getValues(), Arrays.asList("2023-04-15", "12"));
            ...
            Assert.assertEquals(((List<Partition>) hm.getPartitions(table, new HashMap(), (short) 1)).get(0).getTPartition().getValues(), Arrays.asList("2023-09-01", "10"));

            // Sort by "PART_NAME" desc
            hm.setMetaConf(MetastoreConf.ConfVars.PARTITION_ORDER_EXPR.getVarname(), "\"PART_NAME\" desc");
            Assert.assertEquals(((List<Partition>) hm.getPartitions(table, new HashMap(), (short) 1)).get(0).getTPartition().getValues(), Arrays.asList("2025-06-30", "11"));

            // Test MetaStoreClient
            Assert.assertEquals(hm.getMSC().listPartitions(table.getDbName(), table.getTableName(), (short) 1).get(0).getValues(), Arrays.asList("2025-06-30", "11"));
        } catch (Exception e)
        {
            fail("Unexpected exception: " + StringUtils.stringifyException(e));
        } finally
        {
            hm.setMetaConf(MetastoreConf.ConfVars.PARTITION_ORDER_EXPR.getVarname(), MetastoreConf.ConfVars.PARTITION_ORDER_EXPR.getDefaultVal().toString());
            cleanUpTableQuietly(dbName, tableName);
        }
    }
    ...
    public void testHiveRefreshOnConfChange() throws Throwable
    {
        ...
        newHconf.setIntVar(ConfVars.METASTORE_THRIFT_CONNECTION_RETRIES, newHconf.getIntVar(ConfVars.METASTORE_THRIFT_CONNECTION_RETRIES) + 1);
        newHiveObj = Hive.get(newHconf);
        assertTrue(prevHiveObj != newHiveObj);
    }
    ...
    public void testFireInsertEvent() throws Throwable
    {
        ...
        hiveDb.createTable(tableName, Lists.newArrayList("col1"), null, TextInputFormat.class, HiveIgnoreKeyTextOutputFormat.class);
        ...
        Path insert1 = new Path(tablePath, "insert1"), insert2 = new Path(tablePath, "insert2"), insert3 = new Path(tablePath, "insert3");

        try (OutputStream os1 = fileSystem.create(insert1); OutputStream os2 = fileSystem.create(insert2); OutputStream os3 = fileSystem.create(insert3))
        {
            os1.write(new StringBuilder("hello, ").append(System.lineSeparator()).append("world1").toString().getBytes());
            os2.write(new StringBuilder("hello, ").append(System.lineSeparator()).append("world2").toString().getBytes());
            os3.write(new StringBuilder("hello, ").append(System.lineSeparator()).append("world3").toString().getBytes());
        }

        // Fire the InsertData event
        hiveDb.fireInsertEvent(hiveDb.getDatabaseCurrent().getName(), tableName, null, true, Arrays.asList(insert1.toString(), insert2.toString(), insert3.toString()));
