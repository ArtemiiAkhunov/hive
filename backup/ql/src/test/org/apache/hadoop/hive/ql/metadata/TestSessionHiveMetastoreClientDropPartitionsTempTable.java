...
public class TestSessionHiveMetastoreClientDropPartitionsTempTable extends TestDropPartitions
{
    ...
    {
        TableBuilder builder = new TableBuilder().setDbName(DB_NAME).setTableName(tableName).addCol("test_id", "int", "test col id").addCol("test_value", "string", "test col value").setPartCols(partCols).setLocation(getMetaStore().getWarehouseRoot() + "/" + tableName).setTemporary(true);
        if (tableParams != null)
        {
    ...
    protected void checkPartitionsAfterDelete(String tableName, List<Partition> droppedPartitions, List<Partition> existingPartitions, boolean deleteData, boolean purge) throws Exception
    {
        ...
        Assert.assertEquals("The table " + tableName + " has " + partitions.size() + " partitions, but it should have " + existingPartitions.size(), existingPartitions.size(), partitions.size());
        for (Partition droppedPartition : droppedPartitions)
        {
            ...
            if (deleteData)
            {
                Assert.assertFalse("The location '" + partitionPath.toString() + "' should not exist.", getMetaStore().isPathExists(partitionPath));
            } else
            {
                Assert.assertTrue("The location '" + partitionPath.toString() + "' should exist.", getMetaStore().isPathExists(partitionPath));
            }
        }
