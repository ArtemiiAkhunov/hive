...
public class TestVectorBetweenIn
{
    ...
    {
        ROW_MODE, ADAPTOR, VECTOR_EXPRESSION;
    ...
    {
        FILTER_BETWEEN, FILTER_NOT_BETWEEN, PROJECTION_BETWEEN, PROJECTION_NOT_BETWEEN, FILTER_IN, PROJECTION_IN;
    ...
    private void doDecimalTests(Random random, boolean tryDecimal64) throws Exception
    {
        ...
        {
            doBetweenIn(random, typeInfo.getTypeName(), tryDecimal64);
        }
    }
    ...
    private void doStructTests(Random random) throws Exception
    {
        ...
        for (BetweenInVariation betweenInVariation : structInVarations)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                doBetweenStructInVariation(random, typeName, betweenInVariation);
            }
        }
    }
    ...
    private void doBetweenIn(Random random, String typeName, boolean tryDecimal64) throws Exception
    {
        ...
        for (BetweenInVariation betweenInVariation : BetweenInVariation.values())
        {
            ...
            while (true)
            {
                if (!doBetweenInVariation(random, typeName, tryDecimal64, betweenInVariation, subVariation))
                {
                    break;
                }
                subVariation++;
    ...
    private boolean needsValidDataTypeData(TypeInfo typeInfo)
    {
        ...
        if (primitiveCategory == PrimitiveCategory.STRING || primitiveCategory == PrimitiveCategory.CHAR || primitiveCategory == PrimitiveCategory.VARCHAR || primitiveCategory == PrimitiveCategory.BINARY)
        {
            return false;
        }
        return true;
    ...
    private boolean doBetweenInVariation(Random random, String typeName, boolean tryDecimal64, BetweenInVariation betweenInVariation, int subVariation) throws Exception
    {
        ...
        {
            valueList.add(VectorRandomRowSource.randomWritable(random, typeInfo, objectInspector, dataTypePhysicalVariation, /* allowNull */ false));
        }
        ...
        rowSource.initGenerationSpecSchema(random, generationSpecList, /* maxComplexDepth */ 0,
                /* allowNull */ true, /* isUnicodeOk */ true, explicitDataTypePhysicalVariationList);
        ...
        return executeTestModesAndVerify(typeInfo, betweenInVariation, compareList, columns, columnNames, children, udf, exprDesc, randomRows, rowSource, batchSource, outputTypeInfo,
                /* skipAdaptor */ false);
    }
    ...
    private boolean doBetweenStructInVariation(Random random, String structTypeName, BetweenInVariation betweenInVariation) throws Exception
    {
        ...
        for (int i = 0; i < valueCount; i++)
        {
            valueList.add(VectorRandomRowSource.randomWritable(random, structTypeInfo, structObjectInspector, DataTypePhysicalVariation.NONE,
                    /* allowNull */ false));
        }
        ...
        structRowSource.initGenerationSpecSchema(random, structGenerationSpecList, /* maxComplexDepth */ 0,
                /* allowNull */ true, /* isUnicodeOk */ true, structExplicitDataTypePhysicalVariationList);
        ...
        return executeTestModesAndVerify(structUdfTypeInfo, betweenInVariation, compareList, columns, columnNames, children, udf, exprDesc, randomRows, rowSource, batchSource, outputTypeInfo,
                /* skipAdaptor */ true);
    }
    ...
    private boolean executeTestModesAndVerify(TypeInfo typeInfo, BetweenInVariation betweenInVariation, List<Object> compareList, List<String> columns, String[] columnNames, List<ExprNodeDesc> children, GenericUDF udf, ExprNodeGenericFuncDesc exprDesc, Object[][] randomRows, VectorRandomRowSource rowSource, VectorRandomBatchSource batchSource, TypeInfo outputTypeInfo, boolean skipAdaptor) throws Exception
    {
        ...
        for (int i = 0; i < BetweenInTestMode.count; i++)
        {
            ...
            {
                case ROW_MODE:
                    if (!doRowCastTest(typeInfo, betweenInVariation, compareList, columns, children, udf, exprDesc, randomRows, rowSource.rowStructObjectInspector(), resultObjects))
                    {
                        return false;
                    }
                    break;
                ...
                case VECTOR_EXPRESSION:
                    if (!doVectorBetweenInTest(typeInfo, betweenInVariation, compareList, columns, columnNames, rowSource.typeInfos(), rowSource.dataTypePhysicalVariations(), children, udf, exprDesc, betweenInTestMode, batchSource, exprDesc.getWritableObjectInspector(), outputTypeInfo, resultObjects))
                    {
                        return false;
                    }
                    break;
        ...
        for (int i = 0; i < rowCount; i++)
        {
            ...
            for (int v = 1; v < BetweenInTestMode.count; v++)
            {
                ...
                if (betweenInVariation.isFilter && expectedResult == null && vectorResult != null)
                {
                    ...
                    if (vectorBoolean)
                    {
                        Assert.fail("Row " + i + " typeName " + typeInfo.getTypeName() + " outputTypeName " + outputTypeInfo.getTypeName() + " " + betweenInVariation + " " + betweenInTestMode + " result is NOT NULL and true" + " does not match row-mode expected result is NULL which means false here" + " row values " + Arrays.toString(randomRows[i]) + " exprDesc " + exprDesc.toString());
                    }
                } else if (expectedResult == null || vectorResult == null)
                {
                    if (expectedResult != null || vectorResult != null)
                    {
                        Assert.fail("Row " + i + " sourceTypeName " + typeInfo.getTypeName() + " " + betweenInVariation + " " + betweenInTestMode + " result is NULL " + (vectorResult == null ? "YES" : "NO result " + vectorResult.toString()) + " does not match row-mode expected result is NULL " + (expectedResult == null ? "YES" : "NO result " + expectedResult.toString()) + " row values " + Arrays.toString(randomRows[i]) + " exprDesc " + exprDesc.toString());
                    }
                } else
                {
    ...
    */
        ...
        for (int i = 0; i < rowCount; i++)
        {
            ...
            Object copyResult = ObjectInspectorUtils.copyToStandardObject(result, PrimitiveObjectInspectorFactory.writableBooleanObjectInspector, ObjectInspectorCopyOption.WRITABLE);
    ...
    */
        ...
        VectorizedRowBatchCtx batchContext = new VectorizedRowBatchCtx(columnNames, rowSource.typeInfos(), rowSource.dataTypePhysicalVariations(),
                /* dataColumnNums */ null,
                /* partitionColumnCount */ 0,
                /* virtualColumnCount */ 0,
                /* neededVirtualColumns */ null, vectorizationContext.getScratchColumnTypeNames(), vectorizationContext.getScratchDataTypePhysicalVariations());
