...
public class TestTezTotalOrderPartitioner
{
    ...
    private static <T> Path writePartitionFile(String testname, Configuration conf, T[] splits) throws IOException
    {
        ...
        try
        {
            w = SequenceFile.createWriter(conf, SequenceFile.Writer.file(p), SequenceFile.Writer.keyClass(HiveKey.class), SequenceFile.Writer.valueClass(NullWritable.class), SequenceFile.Writer.compression(CompressionType.NONE));
    ...
    public void testTotalOrderCustomComparator() throws Exception
    {
        ...
        Path p = TestTezTotalOrderPartitioner.<HiveKey>writePartitionFile("totalordercustomcomparator", conf, revSplitStrings);
        conf.setBoolean(TotalOrderPartitioner.NATURAL_ORDER, false);
