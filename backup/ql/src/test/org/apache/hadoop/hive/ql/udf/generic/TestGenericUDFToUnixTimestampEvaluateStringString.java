...
public class TestGenericUDFToUnixTimestampEvaluateStringString
{
    ...
    public static Collection<String[]> readInputs() throws IOException, CsvException
    {
        ...
        try (CSVReader reader = new CSVReaderBuilder(new InputStreamReader(TestGenericUDFToUnixTimestampEvaluateStringString.class.getResourceAsStream("TestGenericUDFToUnixTimestampEvaluateStringString.csv"))).withCSVParser(parser).build())
        {
    ...
    private void testEvaluateWithUDF(GenericUDF udfToTest) throws HiveException, InterruptedException
    {
        ...
        LongWritable result = (LongWritable) udfToTest.evaluate(new DeferredObject[]{new DeferredJavaObject(new Text(value)), new DeferredJavaObject(new Text(pattern))});
    ...
    {
        return udf.getDisplayString(new String[]{value, pattern}) + " sessionZone=" + zone + ", formatter=" + formatter + ", resolver Style=" + resolverStyle;
    }
}
