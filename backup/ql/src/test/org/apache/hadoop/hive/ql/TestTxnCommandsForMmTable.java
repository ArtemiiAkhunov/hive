...
public class TestTxnCommandsForMmTable extends TxnCommandsBaseForTests
{
    ...
    enum TableExtended
    {
        NONACIDPART("nonAcidPart", "p"), MMTBL("mmTbl"), MMTBL2("mmTbl2"), MMTBLPART("mmTblPart", "p");
    ...
    public void testInsertOverwriteForMmTable() throws Exception
    {
        ...
        status = fs.listStatus(new Path(getWarehouseDir() + "/" + (TableExtended.MMTBL).toString().toLowerCase()), FileUtils.STAGING_DIR_PATH_FILTER);
    ...
    public void testInsertOverwriteForPartitionedMmTable() throws Exception
    {
        ...
        {
            status = fs.listStatus(new Path(getWarehouseDir() + "/" + (TableExtended.MMTBLPART).toString().toLowerCase() + pStrings[i]), FileUtils.STAGING_DIR_PATH_FILTER);
        ...
        {
            status = fs.listStatus(new Path(getWarehouseDir() + "/" + (TableExtended.MMTBLPART).toString().toLowerCase() + pStrings[h]), FileUtils.STAGING_DIR_PATH_FILTER);
    ...
    public void testOperationsOnCompletedTxnComponentsForMmTable() throws Exception
    {
        ...
        Assert.assertEquals(TestTxnDbUtil.queryToString(hiveConf, "select * from COMPLETED_TXN_COMPONENTS"), 2, TestTxnDbUtil.countQueryAgent(hiveConf, "select count(*) from COMPLETED_TXN_COMPONENTS"));
        ...
        // Run Cleaner.
        runCleaner(hiveConf);
        Assert.assertEquals(TestTxnDbUtil.queryToString(hiveConf, "select * from COMPLETED_TXN_COMPONENTS"), 0, TestTxnDbUtil.countQueryAgent(hiveConf, "select count(*) from COMPLETED_TXN_COMPONENTS"));
    ...
    private void testImpalaTruncatedMmTable(boolean vectorized) throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] status;
        Path tblLocation = new Path(getWarehouseDir() + "/" + (TableExtended.MMTBL).toString().toLowerCase());
    ...
    private void verifyDir(int expectedDeltas, boolean expectBaseDir) throws Exception
    {
        ...
        FileStatus[] status = fs.listStatus(new Path(getWarehouseDir() + "/" + (TableExtended.MMTBL).toString().toLowerCase()), FileUtils.HIDDEN_FILES_PATH_FILTER);
        int sawDeltaTimes = 0;
        int sawBaseTimes = 0;
        for (int i = 0; i < status.length; i++)
        {
    ...
    public void testTruncateWithBase() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), TableExtended.MMTBL.toString().toLowerCase()), AcidUtils.baseFileFilter);
        if (1 != stat.length)
        {
    ...
    public void testTruncateWithBaseAllPartition() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), TableExtended.MMTBLPART.toString().toLowerCase() + "/p=a"), AcidUtils.baseFileFilter);
        if (1 != stat.length)
        {
            Assert.fail("Expecting 1 base and found " + stat.length + " files " + Arrays.toString(stat));
        }
    ...
    public void testTruncateWithBaseOnePartition() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), TableExtended.MMTBLPART.toString().toLowerCase() + "/p=b"), AcidUtils.baseFileFilter);
        if (1 != stat.length)
        {
            Assert.fail("Expecting 1 base and found " + stat.length + " files " + Arrays.toString(stat));
        }
        String name = stat[0].getPath().getName();
        Assert.assertEquals("base_0000003", name);
        stat = fs.listStatus(new Path(getWarehouseDir(), TableExtended.MMTBLPART.toString().toLowerCase() + "/p=a"), AcidUtils.deltaFileFilter);
        if (1 != stat.length)
        {
