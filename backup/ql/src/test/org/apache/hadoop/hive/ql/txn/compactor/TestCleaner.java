...
public class TestCleaner extends CompactorTest
{
    ...
    public void testRetryAfterFailedCleanup(boolean delayEnabled) throws Exception
    {
        ...
        List<TaskHandler> taskHandlers = TaskHandlerFactory.getInstance().getHandlers(conf, mockedHandler, metadataCache, false, fsRemover);
        ...
        for (int i = 1; i < 4; i++)
        {
            ...
            Assert.assertEquals(String.format("Expected error message: '%s', but got '%s'", errorMessage, ci.errorMessage), errorMessage, ci.errorMessage);
        ...
        Assert.assertEquals(String.format("Expected %d CompactionInfo, but got %d", 1, scr.getCompactsSize()), 1, scr.getCompactsSize());
        ShowCompactResponseElement scre = scr.getCompacts().get(0);
        //The state finally should be set to failed.
        Assert.assertEquals(String.format("Expected '%s' state, but got '%s'", "failed", scre.getState()), "failed", scre.getState());
        Assert.assertEquals(String.format("Expected error message: '%s', but got '%s'", errorMessage, scre.getErrorMessage()), errorMessage, scre.getErrorMessage());
    }
    ...
    public void testReady() throws Exception
    {
        ...
        // make sure cleaner didn't remove anything, and cleaning is still queued
        List<Path> paths = getDirectories(conf, t, p);
        Assert.assertEquals("Expected 5 files after minor compaction, instead these files were present " + paths, 5, paths.size());
        ShowCompactResponse rsp = txnHandler.showCompact(new ShowCompactRequest());
        Assert.assertEquals("Expected 1 compaction in queue, got: " + rsp.getCompacts(), 1, rsp.getCompactsSize());
        Assert.assertEquals(TxnStore.CLEANING_RESPONSE, rsp.getCompacts().get(0).getState());
    }
    ...
    public void testCompactionHighWatermarkIsHonored() throws Exception
    {
        ...
        List<String> actualDirs = getDirectories(conf, t, p).stream().map(Path::getName).sorted().collect(Collectors.toList());

        List<String> expectedDirs = Arrays.asList("base_19", addVisibilitySuffix(makeDeltaDirName(20, 22), 23), addVisibilitySuffix(makeDeltaDirName(20, 24), 27), makeDeltaDirName(23, 23), makeDeltaDirName(24, 24));
    ...
    private void allocateTableWriteId(String dbName, String tblName, long txnId) throws Exception
    {
        ...
        txnHandler.addWriteIdsToMinHistory(txnId, Collections.singletonMap(dbName + "." + tblName, awiResp.getTxnToWriteIds().get(0).getWriteId()));
    }
}
