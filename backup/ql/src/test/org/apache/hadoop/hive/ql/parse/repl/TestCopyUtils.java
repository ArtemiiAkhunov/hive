...
public class TestCopyUtils
{
    ...
    public void shouldThrowExceptionOnDistcpFailure() throws Exception
    {
        ...
        try (MockedStatic<FileUtils> fileUtilsMockedStatic = mockStatic(FileUtils.class); MockedStatic<Utils> utilsMockedStatic = mockStatic(Utils.class))
        {
            fileUtilsMockedStatic.when(() -> FileUtils.distCp(same(fs), anyList(), same(destination), anyBoolean(), eq(null), same(conf), same(ShimLoader.getHadoopShims()))).thenReturn(false);
    ...
    {
        try (MockedStatic<UserGroupInformation> userGroupInformationMockedStatic = mockStatic(UserGroupInformation.class); MockedStatic<ReplChangeManager> replChangeManagerMockedStatic = mockStatic(ReplChangeManager.class))
        {
            userGroupInformationMockedStatic.when(UserGroupInformation::getCurrentUser).thenReturn(mock(UserGroupInformation.class));
            HiveConf conf = mock(HiveConf.class);
    ...
    public void testParallelCopySuccess() throws Exception
    {
        try (MockedStatic<UserGroupInformation> userGroupInformationMockedStatic = mockStatic(UserGroupInformation.class))
        {
            ...
            doNothing().when(copyUtilsSpy).doCopy(Mockito.any(), Mockito.any(), anyBoolean(), anyBoolean(), Mockito.any());
            copyUtilsSpy.copyAndVerify(destination, srcFiles, source, true, true);
            Class<Collection<? extends Callable<Void>>> listClass = (Class<Collection<? extends Callable<Void>>>) (Class) List.class;
            ...
            Mockito.verify(mockExecutorService, Mockito.times(1)).invokeAll(callableCapture.capture());
        }
    }
}
