...
public class MapJoinTestConfig
{
    ...
    public static MapJoinDesc createMapJoinDesc(MapJoinTestDescription testDesc, boolean isFullOuterIntersect)
    {
        ...
        mapJoinDesc.setNoOuterJoin(testDesc.vectorMapJoinVariation != VectorMapJoinVariation.OUTER && testDesc.vectorMapJoinVariation != VectorMapJoinVariation.FULL_OUTER);
        ...
        if (testDesc.smallTableRetainKeyColumnNums.length == 0)
        {
            ...
            List<Integer> smallTableValueRetainColumnNumsList = intArrayToList(testDesc.smallTableRetainValueColumnNums);
        ...
        } else
        {
            ...
            int[] smallTableValueIndicesNums = ArrayUtils.toPrimitive(smallTableValueIndicesNumsList.toArray(new Integer[0]));
        ...
        TableDesc keyTableDesc = PlanUtils.getMapJoinKeyTableDesc(testDesc.hiveConf, PlanUtils.getFieldSchemasFromColumnList(smallTableKeyExpr, ""));
        ...
        TableDesc valueTableDesc = PlanUtils.getMapJoinValueTableDesc(PlanUtils.getFieldSchemasFromColumnList(smallTableValueExpr, ""));
    ...
    public static VectorMapJoinCommonOperator createNativeVectorMapJoinOperator(VectorMapJoinVariation VectorMapJoinVariation, MapJoinDesc mapJoinDesc, VectorMapJoinDesc vectorDesc, VectorizationContext vContext) throws HiveException
    {
        ...
        switch (vectorDesc.getHashTableKeyType())
        {
            ...
            case LONG:
                ...
                {
                    case INNER:
                        operator = new VectorMapJoinInnerLongOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case INNER_BIG_ONLY:
                        operator = new VectorMapJoinInnerBigOnlyLongOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case LEFT_SEMI:
                        operator = new VectorMapJoinLeftSemiLongOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case OUTER:
                        operator = new VectorMapJoinOuterLongOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case FULL_OUTER:
                        operator = new VectorMapJoinFullOuterLongOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    default:
            ...
            case STRING:
                ...
                {
                    case INNER:
                        operator = new VectorMapJoinInnerStringOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case INNER_BIG_ONLY:
                        operator = new VectorMapJoinInnerBigOnlyStringOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case LEFT_SEMI:
                        operator = new VectorMapJoinLeftSemiStringOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case OUTER:
                        operator = new VectorMapJoinOuterStringOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                    case FULL_OUTER:
                        operator = new VectorMapJoinFullOuterStringOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    default:
                        throw new RuntimeException("unknown operator variation " + VectorMapJoinVariation);
                }
            ...
            case MULTI_KEY:
                ...
                {
                    case INNER:
                        operator = new VectorMapJoinInnerMultiKeyOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case INNER_BIG_ONLY:
                        operator = new VectorMapJoinInnerBigOnlyMultiKeyOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case LEFT_SEMI:
                        operator = new VectorMapJoinLeftSemiMultiKeyOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case OUTER:
                        operator = new VectorMapJoinOuterMultiKeyOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    case FULL_OUTER:
                        operator = new VectorMapJoinFullOuterMultiKeyOperator(new CompilationOpContext(), mapJoinDesc, vContext, vectorDesc);
                        break;
                    default:
                        throw new RuntimeException("unknown operator variation " + VectorMapJoinVariation);
                }
    ...
    {
        return createMapJoinImplementation(mapJoinImplementation, testDesc, testData, mapJoinDesc, null);
    }
    ...
    public static CreateMapJoinResult createMapJoinImplementation(MapJoinTestImplementation mapJoinImplementation, MapJoinTestDescription testDesc, MapJoinTestData testData, MapJoinDesc mapJoinDesc, MapJoinTableContainer shareMapJoinTableContainer) throws SerDeException, IOException, HiveException
    {
        ...
        switch (mapJoinImplementation)
        {
            case ROW_MODE_HASH_MAP:
                ...
                // MapJoinOperator
                result = MapJoinTestConfig.createMapJoin(testDesc, testData, mapJoinDesc, /* isVectorMapJoin */ false,
                        /* isOriginalMapJoin */ true, shareMapJoinTableContainer);
                break;
            ...
            case ROW_MODE_OPTIMIZED:
                ...
                // MapJoinOperator
                result = MapJoinTestConfig.createMapJoin(testDesc, testData, mapJoinDesc, /* isVectorMapJoin */ false,
                        /* isOriginalMapJoin */ false, shareMapJoinTableContainer);
                break;

            case VECTOR_PASS_THROUGH:

                // VectorMapJoinOperator
                result = MapJoinTestConfig.createMapJoin(testDesc, testData, mapJoinDesc, /* isVectorMapJoin */ true,
                        /* n/a */ false, shareMapJoinTableContainer);
                break;

            case NATIVE_VECTOR_OPTIMIZED:
                result = MapJoinTestConfig.createNativeVectorMapJoin(testDesc, testData, mapJoinDesc, HashTableImplementationType.OPTIMIZED, shareMapJoinTableContainer);
                break;

            case NATIVE_VECTOR_FAST:
                result = MapJoinTestConfig.createNativeVectorMapJoin(testDesc, testData, mapJoinDesc, HashTableImplementationType.FAST, shareMapJoinTableContainer);
                break;
            default:
