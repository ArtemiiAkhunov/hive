...
public class TestTxnCommands extends TxnCommandsBaseForTests
{
    ...
    void initHiveConf()
    {
        ...
        MetastoreConf.setClass(hiveConf, MetastoreConf.ConfVars.FILTER_HOOK, DummyMetaStoreFilterHookImpl.class, MetaStoreFilterHook.class);

        HiveConf.setVar(hiveConf, HiveConf.ConfVars.HIVE_METASTORE_WAREHOUSE_EXTERNAL, new Path(getWarehouseDir(), "ext").toUri().getPath());
    }
    ...
    public void testParallelInsertAnalyzeStats() throws Exception
    {
        ...
        String[] queries = {String.format("insert into %s (a) values (999)", tableName), String.format("analyze table %s compute statistics for columns", tableName)};
        runParallelQueries(queries);
    ...
    public void testParallelTruncateAnalyzeStats() throws Exception
    {
        ...
        String[] queries = {String.format("truncate table %s", tableName), String.format("analyze table %s compute statistics for columns", tableName)};
        runParallelQueries(queries);
    ...
    public List<ColumnStatisticsObj> getTxnTableStats(IMetaStoreClient msClient, String tableName) throws TException, NoSuchObjectException, MetaException
    {
        ...
        List<ColumnStatisticsObj> stats;
        validWriteIds = msClient.getValidWriteIds("default." + tableName).toString();
        stats = msClient.getTableColumnStatistics("default", tableName, Lists.newArrayList("a"), Constants.HIVE_ENGINE, validWriteIds);
        return stats;
    }

    private void assertIsDelta(FileStatus stat)
    {
        Assert.assertTrue(stat.toString(), stat.getPath().getName().startsWith(AcidUtils.DELTA_PREFIX));
    }
     ...
     */
        for (int i = 0; i < explain.size(); i++)
        {
            ...
            {
                Assert.assertTrue("At i+1=" + (i + 1) + explain.get(i + 1), explain.get(i + 1).contains("Reducer 2 <- Map 1 (SIMPLE_EDGE), Map 8 (SIMPLE_EDGE)"));
                ...
                Assert.assertTrue("At i+1=" + (i + 6) + explain.get(i + 6), explain.get(i + 6).contains("Reducer 7 <- Reducer 2 (SIMPLE_EDGE)"));
                break;
            }
        }
    }
    ...
    public void testMergeUpdateDelete() throws Exception
    {
        ...
        String query = "merge into " + Table.ACIDTBL + " as t using " + Table.NONACIDORCTBL + " s ON t.a = s.a " + "WHEN MATCHED AND s.a < 3 THEN update set b = 0 " + //updates (2,1) -> (2,0)
    ...
    public void testNonAcidToAcidConversion01() throws Exception
    {
        ...
        query = "select ROW__ID, a, b" + (isVectorized ? "" : ", INPUT__FILE__NAME") + " from " + Table.NONACIDORCTBL + " order by ROW__ID";
        String[][] expected2 = new String[][]{{"{\"writeid\":0,\"bucketid\":536936448,\"rowid\":0}\t1\t2", "nonacidorctbl/base_10000001_v0000021/bucket_00001"}, {"{\"writeid\":0,\"bucketid\":536936448,\"rowid\":1}\t0\t12", "nonacidorctbl/base_10000001_v0000021/bucket_00001"}, {"{\"writeid\":0,\"bucketid\":536936448,\"rowid\":2}\t1\t5", "nonacidorctbl/base_10000001_v0000021/bucket_00001"}, {"{\"writeid\":10000001,\"bucketid\":536936448,\"rowid\":0}\t1\t17", "nonacidorctbl/base_10000001_v0000021/bucket_00001"}};
    ...
    public void testTruncateWithBase() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBL.toString().toLowerCase()), AcidUtils.baseFileFilter);
        if (1 != stat.length)
        {
    ...
    public void testTruncateWithBaseAllPartition() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLPART.toString().toLowerCase() + "/p=a"), AcidUtils.baseFileFilter);
        if (1 != stat.length)
        {
            Assert.fail("Expecting 1 base and found " + stat.length + " files " + Arrays.toString(stat));
        }
    ...
    public void testTruncateWithBaseOnePartition() throws Exception
    {
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLPART.toString().toLowerCase() + "/p=b"), AcidUtils.baseFileFilter);
        if (1 != stat.length)
        {
            Assert.fail("Expecting 1 base and found " + stat.length + " files " + Arrays.toString(stat));
        }
        String name = stat[0].getPath().getName();
        Assert.assertEquals("base_0000003", name);
        stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLPART.toString().toLowerCase() + "/p=a"), AcidUtils.deltaFileFilter);
        if (1 != stat.length)
        {
    ...
    private void dropWithBaseOnePartition(boolean reCreate) throws Exception
    {
        ...
        Assert.assertEquals("Unexpected number of compactions in history", 1, resp.getCompactsSize());
        Assert.assertTrue(resp.getCompacts().stream().anyMatch(ci -> TxnStore.CLEANING_RESPONSE.equals(ci.getState()) && "p=b".equals(ci.getPartitionname())));
        if (reCreate)
        {
        ...
        stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLPART.toString().toLowerCase()), path -> path.getName().equals("p=b"));
    ...
    private void dropDatabaseCascadeNonBlocking() throws Exception
    {
        ...
        runStatementOnDriver("create table " + database + "." + tableName + "1 (a int, b int) " + "partitioned by (ds string) stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into " + database + "." + tableName + "1 partition(ds) values(1,2,'foo'),(3,4,'bar')");

        runStatementOnDriver("create materialized view " + database + ".mv_" + tableName + "1 " + "partitioned on (ds) stored as orc TBLPROPERTIES ('transactional'='true')" + "as select a, ds from " + database + "." + tableName + "1 where b > 1");
        ...
        runStatementOnDriver("create table " + database + "." + tableName + "2 (a int, b int) " + "partitioned by (ds string) stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into " + database + "." + tableName + "2 partition(ds) values(1,2,'foo'),(3,4,'bar')");

        runStatementOnDriver("create materialized view " + database + ".mv_" + tableName + "2 " + "partitioned on (ds) stored as orc TBLPROPERTIES ('transactional'='true')" + "as select a, ds from " + database + "." + tableName + "2 where b > 1");
        ...
        runStatementOnDriver("create external table " + database + ".tab_ext (a int, b int) " + "partitioned by (ds string) stored as parquet");
        ...
        runStatementOnDriver("create table " + database + ".tab_nonacid (a int, b int) " + "partitioned by (ds string) stored as parquet");
        ...
        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir(), database + ".db"), t -> t.getName().matches("(mv_)?" + tableName + "2" + SOFT_DELETE_TABLE_PATTERN));
        if (2 != stat.length)
        {
        ...
        stat = fs.listStatus(new Path(getWarehouseDir(), "1"), t -> t.getName().equals("data"));
        Assert.assertEquals(1, stat.length);
        // External partition for managed table should be removed
        stat = fs.listStatus(new Path(getWarehouseDir(), "2"), t -> t.getName().equals("data"));
        Assert.assertEquals(0, stat.length);

        runCleaner(hiveConf);

        stat = fs.listStatus(new Path(getWarehouseDir(), database + ".db"), t -> t.getName().matches("(mv_)?" + tableName + "2" + SOFT_DELETE_TABLE_PATTERN));
        if (stat.length != 0)
        {
    ...
    public void testDropMaterializedViewWithSuffix() throws Exception
    {
        ...
        runStatementOnDriver("create table " + tableName + "(a int, b int) stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into " + tableName + " values(1,2),(3,4)");
        runStatementOnDriver("create materialized view " + mviewName + " stored as orc TBLPROPERTIES ('transactional'='true') " + "as select a from tab_acid where b > 1");
        runStatementOnDriver("drop materialized view " + mviewName);

        int count = TestTxnDbUtil.countQueryAgent(hiveConf, "select count(*) from TXN_TO_WRITE_ID where T2W_TABLE = '" + mviewName + "'");
        Assert.assertEquals(1, count);

        FileSystem fs = FileSystem.get(hiveConf);
        FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir()), t -> t.getName().matches(mviewName + SOFT_DELETE_TABLE_PATTERN));
        if (1 != stat.length)
        {
        ...
        houseKeeperService.run();
        count = TestTxnDbUtil.countQueryAgent(hiveConf, "select count(*) from TXN_TO_WRITE_ID where T2W_TABLE = '" + mviewName + "'");
        Assert.assertEquals(0, count);
        ...
        {
            Assert.assertTrue(ex.getMessage().contains(ErrorMsg.INVALID_TABLE.getMsg(StringUtils.wrap(mviewName, "'"))));
        }
        // Check status of compaction job
        TxnStore txnHandler = TxnUtils.getTxnStore(hiveConf);
        ShowCompactResponse resp = txnHandler.showCompact(new ShowCompactRequest());
        ...
        FileStatus[] status = fs.listStatus(new Path(getWarehouseDir()), t -> t.getName().matches(mviewName + SOFT_DELETE_TABLE_PATTERN));
        Assert.assertEquals(0, status.length);
    }
    ...
    public void testDropMaterializedViewWithoutSuffix() throws Exception
    {
        ...
        for (boolean enabled : asList(false, true))
        {
            ...
            FileSystem fs = FileSystem.get(hiveConf);
            FileStatus[] stat = fs.listStatus(new Path(getWarehouseDir()), t -> t.getName().equals(mviewName));
            Assert.assertEquals(0, stat.length);
    ...
    public void testRenameMakeCopyPartition() throws Exception
    {
        ...
        String name = stat[0].getPath().getName();
        Assert.assertEquals("base_0000003", name);
        stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLPART.toString().toLowerCase() + "/p=a"), AcidUtils.baseFileFilter);
        if (0 != stat.length)
        {
            Assert.fail("Expecting no base and found " + stat.length + " files " + Arrays.toString(stat));
        }
    ...
    public void testRenameMakeCopyNestedPartition() throws Exception
    {
        ...
        HiveConf.setBoolVar(hiveConf, HiveConf.ConfVars.HIVE_ACID_RENAME_PARTITION_MAKE_COPY, true);
        runStatementOnDriver("alter table " + Table.ACIDTBLNESTEDPART + " partition (p1='a', p2='b', p3='d')" + " rename to partition (p1='a', p2='c', p3='d')");
        ...
        stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLNESTEDPART.toString().toLowerCase() + "/p1=a/p2=c/p3=d"), AcidUtils.baseFileFilter);
        if (0 != stat.length)
        {
            Assert.fail("Expecting no base and found " + stat.length + " files " + Arrays.toString(stat));
        }
        ...
        stat = fs.listStatus(new Path(getWarehouseDir(), Table.ACIDTBLNESTEDPART.toString().toLowerCase() + "/p1=a/p2=b"), path -> path.getName().equals("p3=d"));
        if (0 != stat.length)
        {
            Assert.fail("Expecting partition data to be removed from FS");
        }
    ...
    public void testShowCompactions() throws Exception
    {
        ...
        runStatementOnDriver("create table mydb1.tbl0 " + "(a int, b int) partitioned by (p string) clustered by (a) into " + BUCKET_COUNT + " buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into mydb1.tbl0" + " PARTITION(p) " + " values(1,2,'p1'),(3,4,'p1'),(1,2,'p2'),(3,4,'p2'),(1,2,'p3'),(3,4,'p3')");
        ...
        TestTxnCommands2.runWorker(hiveConf);
        runStatementOnDriver("insert into mydb1.tbl0" + " PARTITION(p) " + " values(4,5,'p1'),(6,7,'p1'),(4,5,'p2'),(6,7,'p2'),(4,5,'p3'),(6,7,'p3')");
        runStatementOnDriver("alter table mydb1.tbl0" + " PARTITION (p='p1') compact 'MAJOR'");
        TestTxnCommands2.runWorker(hiveConf);
        runStatementOnDriver("alter table mydb1.tbl0" + " PARTITION (p='p2') compact 'MAJOR'");
        TestTxnCommands2.runWorker(hiveConf);
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getState().equals("ready for cleaning")).count() + 1, r.size());//includes Header row
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getId() == 1).count() + 1, r.size());//includes Header row
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        p = Pattern.compile("1\t.*");
        for (int i = 1; i < r.size(); i++)
        {
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb1")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());//includes Header row
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        //includes Header row
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb1")).filter(x -> x.getPoolName().equals("poolx")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());
        Assert.assertEquals(1, r.size());//only header row

        r = runStatementOnDriver("SHOW COMPACTIONS SCHEMA mydb1 POOL 'pool0' TYPE 'MAJOR'");
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb1")).filter(x -> x.getPoolName().equals("pool0")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());//includes Header row
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb1")).filter(x -> x.getPoolName().equals("pool0")).count() + 1, r.size());//includes Header row
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getTablename().equals("tbl0")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());//includes Header row
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        //includes Header row
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb1")).filter(x -> x.getTablename().equals("tbl0")).filter(x -> x.getPartitionname().equals("p=p3")).count() + 1, r.size());
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb1")).filter(x -> x.getTablename().equals("tbl0")).filter(x -> x.getPartitionname().equals("p=p3")).filter(x -> x.getPoolName().equals("pool0")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
    ...
    public void testShowCompactionFilterWithPartition() throws Exception
    {
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb")).filter(x -> x.getTablename().equals("tbl2")).filter(x -> x.getPartitionname().equals("ds=mon")).count() + 1, r.size());
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb")).filter(x -> x.getTablename().equals("tbl2")).filter(x -> x.getPartitionname().equals("ds=mon")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb")).filter(x -> x.getType().equals(CompactionType.MINOR)).count() + 1, r.size());
        Assert.assertEquals("CompactionId\tDatabase\tTable\tPartition\tType\tState\tWorker host\tWorker\tEnqueue Time\tStart Time" + "\tDuration(ms)\tHadoopJobId\tError message\tInitiator host\tInitiator\tPool name\tTxnId\tNext TxnId\tCommit Time\t" + "Highest WriteId", r.get(0));
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mydb")).filter(x -> x.getTablename().equals("tbl2")).filter(x -> x.getPartitionname().equals("ds=wed")).count() + 1, r.size());
        for (int i = 1; i < r.size(); i++)
        {
        ...
        Assert.assertEquals(rsp.getCompacts().stream().filter(x -> x.getDbname().equals("mymydbdb2")).filter(x -> x.getType().equals(CompactionType.MAJOR)).count() + 1, r.size());
        Assert.assertEquals(1, r.size());//only header row
    }
    ...
    public void testShowCompactionInputValidation() throws Exception
    {
        ...
        r = runStatementOnDriver("SHOW COMPACTIONS mydb2.tbl1 PARTITION (ds='mon') TYPE 'MINOR' " + "STATUS 'ready for clean'");// validates table
        r = runStatementOnDriver("SHOW COMPACTIONS mydb2.tbl2 PARTITION (p=101,day='Monday') POOL 'pool0' TYPE 'minor' " + "STATUS 'ready for clean'");// validates partspec
        r = runStatementOnDriver("SHOW COMPACTIONS mydb1.tbl0 PARTITION (p='p1') POOL 'pool0' TYPE 'minor' " + "STATUS 'ready for clean'");//validates compaction status
    }
    ...
    public void testShowCompactionFilterSortingAndLimit() throws Exception
    {
        ...
        runStatementOnDriver("create table mydb.tbl " + "(a int, b int) partitioned by (ds string) clustered by (a) into " + BUCKET_COUNT + " buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into mydb.tbl" + " PARTITION(ds) " + " values(1,2,'mon'),(3,4,'tue'),(1,2,'mon'),(3,4,'tue'),(1,2,'wed'),(3,4,'wed')");
        ...
        runStatementOnDriver("create table mydb.tbl2 " + "(a int, b int) partitioned by (dm string) clustered by (a) into " + BUCKET_COUNT + " buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into mydb.tbl2" + " PARTITION(dm) " + " values(1,2,'xxx'),(3,4,'xxx'),(1,2,'yyy'),(3,4,'yyy'),(1,2,'zzz'),(3,4,'zzz')");
        ...
        List<String> txnIdExpectedList = r.stream().skip(1).map(x -> x.split("\t")[15]).sorted(Collections.reverseOrder()).collect(Collectors.toList());
        ...
        txnIdExpectedList = r.stream().skip(1).map(x -> x.split("\t")[16]).sorted(Collections.reverseOrder()).collect(Collectors.toList());
    ...
    public void testAbortCompactions() throws Exception
    {
        ...
        runStatementOnDriver("create table mydb1.tbl2 " + "(a int, b int) partitioned by (p string) clustered by (a) into " + BUCKET_COUNT + " buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into mydb1.tbl2" + " PARTITION(p) " + " values(1,2,'p1'),(3,4,'p1'),(1,2,'p2'),(3,4,'p2'),(1,2,'p3'),(3,4,'p3')");
        ...
        TestTxnCommands2.runWorker(hiveConf);
        TestTxnCommands2.runCleaner(hiveConf);
        runStatementOnDriver("insert into mydb1.tbl2" + " PARTITION(p) " + " values(11,12,'p1'),(13,14,'p1'),(11,12,'p2'),(13,14,'p2'),(11,12,'p3'),(13,14,'p3')");
        runStatementOnDriver("alter table mydb1.tbl2" + " PARTITION (p='p1')  compact 'MINOR'");
        TestTxnCommands2.runWorker(hiveConf);

        runStatementOnDriver("create table mydb1.tbl3 " + "(a int, b int) partitioned by (ds string) clustered by (a) into " + BUCKET_COUNT + " buckets stored as orc TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into mydb1.tbl3" + " PARTITION(ds) " + " values(1,2,'today'),(3,4,'today'),(1,2,'tomorrow'),(3,4,'tomorrow'),(1,2,'yesterday'),(3,4,'yesterday')");
    ...
    private void setUpCompactionRequestsData(String dbName, String tbName) throws Exception
    {
        ...
        runStatementOnDriver("create table " + dbName + "." + tbName + " (a int, b int) partitioned by (ds String)  stored as orc " + "TBLPROPERTIES ('transactional'='true')");
        runStatementOnDriver("insert into " + dbName + "." + tbName + " PARTITION (ds) " + " values(1,2,'mon'),(3,4,'mon'),(1,2,'tue'),(3,4,'tue'),(1,2,'wed'),(3,4,'wed')");
    }
    ...
    public void testFetchTaskCachingWithConversion() throws Exception
    {
        ...
        runStatementOnDriver("create table fetch_task_table (a INT, b INT) stored as orc" + " tblproperties ('transactional'='true')");
