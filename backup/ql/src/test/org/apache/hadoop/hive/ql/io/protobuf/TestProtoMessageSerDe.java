...
public class TestProtoMessageSerDe
{
    ...
    private <T extends Message> ProtoMessageWritable<T> init(Class<T> clazz, String mapTypes) throws Exception
    {
        ...
        @SuppressWarnings("rawtypes") Constructor<ProtoMessageWritable> cons = ProtoMessageWritable.class.getDeclaredConstructor(Parser.class);
    ...
    {
        return Mesg1.newBuilder().addAnotherMap(makeMap(start + 1)).addAnotherMap(makeMap(start + 2)).setNoMap(makeMap(start + 3)).addIntList(start + 4).addIntList(start + 5).build();
    }

    @Test
    public void testSimpleMessage() throws Exception
    {
        ProtoMessageWritable<MapFieldEntry> writable = init(MapFieldEntry.class, " MapFieldEntry     , Invalid  ");
    ...
    public void testMapAndList() throws Exception
    {
        ...
        writable.setMessage(proto);
        assertArrayEquals(arr(map("key1", "val1", "key2", "val2"), arr("key3", "val3"), arr(4, 5)), (Object[]) serde.deserialize(writable));
    }
    ...
    public void testMapAndListNoMapConfigured() throws Exception
    {
        ...
        assertEquals(structoi(list("anotherMap", "noMap", "intList"), list(listoi(mfoi), mfoi, listoi(intoi))), serde.getObjectInspector());
        ...
        Mesg1 proto = makeMesg1(0);
        writable.setMessage(proto);
        assertArrayEquals(arr(arr(arr("key1", "val1"), arr("key2", "val2")), arr("key3", "val3"), arr(4, 5)), (Object[]) serde.deserialize(writable));
    }
