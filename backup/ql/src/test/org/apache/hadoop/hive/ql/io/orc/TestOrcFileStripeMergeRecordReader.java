...
public class TestOrcFileStripeMergeRecordReader
{
    ...
    public void testSplitStartsWithZeroOffset() throws IOException
    {
        ...
        Assert.assertEquals("NumberOfValues", TEST_STRIPE_SIZE, value.getStripeStatistics().getColStats(0).getNumberOfValues());
        reader.next(key, value);
        Assert.assertEquals("InputPath", tmpPath, key.getInputPath());
    ...
    {
        Path path = new Path(System.getProperty("test.tmp.dir", "target" + File.separator + "test" + File.separator + "tmp") + File.separator + "TestOrcFile." + testCaseName.getMethodName() + ".orc");
