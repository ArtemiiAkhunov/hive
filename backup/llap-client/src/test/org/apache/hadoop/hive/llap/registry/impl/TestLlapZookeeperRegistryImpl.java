...
public class TestLlapZookeeperRegistryImpl
{
    ...
    public void setUp() throws Exception
    {
        ...
        curatorFramework = CuratorFrameworkFactory.builder().connectString(server.getConnectString()).sessionTimeoutMs(1000).retryPolicy(new RetryOneTime(1000)).build();
    ...
    public void testRegister() throws Exception
    {
        ...
        // When
        registry.register();
        ServiceInstanceSet<LlapServiceInstance> serviceInstanceSet = registry.getInstances("LLAP", 1000);
        ...
        assertEquals(expectedQueueSize, parseInt(attributes.get(LlapRegistryService.LLAP_DAEMON_TASK_SCHEDULER_ENABLED_WAIT_QUEUE_SIZE)));
        assertEquals(expectedExecutorCount, parseInt(attributes.get(LlapRegistryService.LLAP_DAEMON_NUM_ENABLED_EXECUTORS)));
    }
    ...
    public void testUpdate() throws Exception
    {
        ...
        assertEquals(expectedQueueSize, attributes.get(LlapRegistryService.LLAP_DAEMON_TASK_SCHEDULER_ENABLED_WAIT_QUEUE_SIZE));
        assertEquals(expectedExecutorCount, attributes.get(LlapRegistryService.LLAP_DAEMON_NUM_ENABLED_EXECUTORS));
    }

    @Test
    public void testPersistentNodePath()
    {
        String llapRootNameSpace = "/" + LlapZookeeperRegistryImpl.getRootNamespace(hiveConf, HiveConf.getVar(hiveConf, HiveConf.ConfVars.LLAP_ZK_REGISTRY_NAMESPACE), NAMESPACE_PREFIX);
        String persistentNodeName = "/pnode0";

        Assert.assertEquals(llapRootNameSpace + "/user-" + System.getProperty("user.name") + persistentNodeName, registry.getPersistentNodePath());
    }
    ...
    static <T> void trySetMock(Object o, String field, T value)
    {
        try
        {
            Field fieldToChange = Arrays.stream(FieldUtils.getAllFields(o.getClass())).filter(f -> f.getName().equals(field)).findFirst().orElseThrow(NoSuchFieldException::new);
