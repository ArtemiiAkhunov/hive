...
public class LlapCoordinator
{
    ...
    private static final Cache<String, LlapTokenLocalClient> localClientCache = CacheBuilder.newBuilder().expireAfterAccess(10, TimeUnit.MINUTES).removalListener(new RemovalListener<String, LlapTokenLocalClient>()
    {
        @Override
    ...
    public LlapSigner getLlapSigner(final Configuration jobConf)
    {
        ...
        final String clusterId = DaemonId.createClusterString(clusterUser, LlapUtil.generateClusterName(jobConf));
        try
        {
    ...
    public LlapTokenLocalClient getLocalTokenClient(final Configuration conf, String clusterUser) throws IOException
    {
        // Note that we create the cluster name from user conf (hence, a user can target a cluster),
        // but then we create the signer using hiveConf (hence, we control the ZK config and stuff).
        assert UserGroupInformation.isSecurityEnabled();
