...
public class TezAmInstance extends ServiceInstanceBase
{
    ...
    TezAmInstance(ServiceRecord srv) throws IOException
    {
        ...
        if (plugin != null)
        {
            this.pluginPort = Integer.parseInt(RegistryTypeUtils.getAddressField(plugin.addresses.get(0), AddressTypes.ADDRESS_PORT_FIELD));
        } else
        {
            this.pluginPort = -1;
        }
    ...
    {
        return "TezAmInstance [" + getSessionId() + ", host=" + getHost() + ", rpcPort=" + getRpcPort() + ", pluginPort=" + pluginPort + ", token=" + token + "]";
    }
