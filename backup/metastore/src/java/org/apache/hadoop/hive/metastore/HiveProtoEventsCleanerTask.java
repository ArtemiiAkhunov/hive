...
public class HiveProtoEventsCleanerTask implements MetastoreTaskThread
{
    ...
    private static void computeExpiredDatePtn(long ttl)
    {
        ...
        LocalDate expiredDate = LocalDateTime.ofEpochSecond((clock.getTime() - ttl) / 1000, 0, ZoneOffset.UTC).toLocalDate();
    ...
    private void deleteDirByOwner(FileSystem fs, FileStatus eventsDir) throws IOException
    {
        ...
        } else
        {
            ...
            UserGroupInformation ugi = UserGroupInformation.createProxyUser(owner, UserGroupInformation.getLoginUser());
            try
            {
