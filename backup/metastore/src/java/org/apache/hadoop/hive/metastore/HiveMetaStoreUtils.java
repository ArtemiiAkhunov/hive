...
public class HiveMetaStoreUtils
{
     ...
     * @return Returns instantiated deserializer by looking up class name of deserializer stored in
     * storage descriptor of passed in table. Also, initializes the deserializer with schema
     * of table.
     * @throws MetaException if any problems instantiating the Deserializer
     *                       <p>
     *                                    todo - this should move somewhere into serde.jar
     */
    static public Deserializer getDeserializer(Configuration conf, org.apache.hadoop.hive.metastore.api.Table table, String metaTable, boolean skipConfError) throws MetaException
    {
    ...
    static public Deserializer getDeserializer(Configuration conf, org.apache.hadoop.hive.metastore.api.Partition part, org.apache.hadoop.hive.metastore.api.Table table) throws MetaException
    {
        ...
        try
        {
            AbstractSerDe deserializer = ReflectionUtil.newInstance(conf.getClassByName(lib).asSubclass(AbstractSerDe.class), conf);
            deserializer.initialize(conf, MetaStoreUtils.getTableMetadata(table), MetaStoreUtils.getPartitionMetadata(part, table));
            return deserializer;
    ...
    {
        Collection<String> noCommentSerdes = MetastoreConf.getStringCollection(conf, MetastoreConf.ConfVars.SERDES_WITHOUT_FROM_DESERIALIZER);
        ...
        {
            str_fields.add(new FieldSchema(last_name, oi.getTypeName(), determineFieldComment(null, noCommentFromDeserializer)));
        } else
        {
