...
public class TestHiveConf
{
    ...
    public void testHiddenConfig() throws Exception
    {
        ...
        ArrayList<String> hiddenList = Lists.newArrayList(HiveConf.ConfVars.METASTORE_PWD.varname, HiveConf.ConfVars.HIVE_SERVER2_SSL_KEYSTORE_PASSWORD.varname, HiveConf.ConfVars.HIVE_SERVER2_WEBUI_SSL_KEYSTORE_PASSWORD.varname, "fs.s3.awsSecretAccessKey", "fs.s3n.awsSecretAccessKey", "dfs.adls.oauth2.credential", "fs.adl.oauth2.credential", "fs.azure.account.oauth2.client.secret");

        for (String hiddenConfig : hiddenList)
        {
            // check configs are hidden
            Assert.assertTrue("config " + hiddenConfig + " should be hidden", conf.isHiddenConfig(hiddenConfig));
            ...
            Assert.assertTrue(conf.isHiddenConfig(hiddenConfig + "postfix"));
