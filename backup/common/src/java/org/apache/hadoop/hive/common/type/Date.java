...
public class Date implements Comparable<Date>
{
    ...
    private static final DateTimeFormatter PARSE_FORMATTER = new DateTimeFormatterBuilder().appendValue(YEAR, 1, 4, SignStyle.NORMAL).appendLiteral('-').appendValue(MONTH_OF_YEAR, 1, 2, SignStyle.NORMAL).appendLiteral('-').appendValue(DAY_OF_MONTH, 1, 2, SignStyle.NORMAL).toFormatter().withResolverStyle(ResolverStyle.STRICT);
    ...
    {
        localDate = LocalDateTime.ofInstant(Instant.ofEpochMilli(epochMilli), ZoneOffset.UTC).toLocalDate();
    }
    ...
    public static Date valueOf(final String text)
    {
        ...
        try
        {
            ...
            if (pos.getIndex() < trimedText.length())
            {
                ...
                {
                    throw new DateTimeParseException("Text '" + trimedText + "' could not be parsed, unparsed text found at index " + pos.getIndex(), trimedText, pos.getIndex());
                }
            }
