...
public class InPlaceUpdate
{
    ...
    public void render(ProgressMonitor monitor)
    {
        ...
        reprintLineWithColorAsBold(String.format(HEADER_FORMAT, monitor.headers().toArray()), Ansi.Color.CYAN);
        reprintLine(SEPARATOR);


        // Map 1 .......... container  SUCCEEDED      7          7        0        0       0       0
        List<String> printReady = monitor.rows().stream().map(row -> String.format(VERTEX_FORMAT, row.toArray())).collect(Collectors.toList());
        ...
        String footer = String.format(FOOTER_FORMAT, monitor.footerSummary(), getInPlaceProgressBar(monitor.progressedPercentage()), progressStr, elapsedTime);
