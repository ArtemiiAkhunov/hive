...
public class FixedSizedObjectPool<T> implements Pool<T>
{
    ...
    private static final long NO_MARKER = Marker.MARKER_MASK, NO_DELTA = Marker.DELTA_MASK, MAX_DELTA = NO_DELTA - 1, MAX_SIZE = NO_MARKER - 1;
    ...
    public FixedSizedObjectPool(int size, PoolObjectHelper<T> helper, boolean doTraceLog)
    {
        ...
        this.helper = helper;
        @SuppressWarnings("unchecked") T[] poolTmp = (T[]) new Object[size];
        pool = poolTmp;
    ...
    {
        if (t == null || pool.length == 0)
        {
            return false; // 0 size means no-pooling case - passthru.
        }
    ...
    private T takeImpl()
    {
        ...
        if (oldState == NO_INDEX)
        {
            return null; // For whatever reason, reserve failed.
        }
    ...
    private boolean offerImpl(T t)
    {
        ...
        if (oldState == NO_INDEX)
        {
            return false; // For whatever reason, reserve failed.
        }
    ...
    private long reserveArrayIndex(Marker from, Marker to)
    {
        while (true)
        {
            long oldVal = state.get(), marker = from.getMarker(oldVal), delta = from.getDelta(oldVal), rc = from.getRc(oldVal), toMarker = to.getMarker(oldVal), toDelta = to.getDelta(oldVal);
            if (marker == NO_MARKER)
            {
                return NO_INDEX; // The list is empty.
            }
            if (delta == MAX_DELTA)
            {
                return NO_INDEX; // Too many concurrent operations; spurious failure.
            }
            if (delta == NO_DELTA)
            {
                return NO_INDEX; // List is drained and recreated concurrently.
            }
            if (toDelta == NO_DELTA)
            { // Same for the OTHER list; spurious.
            ...
            if (newDelta == NO_DELTA)
            {
                return NO_INDEX; // Target list is being drained.
            }
            long newVal = from.setRc(from.setDelta(oldVal, newDelta), rc + 1); // Set delta and refcount.
            if (setState(oldVal, newVal))
            {
                return oldVal;
            }
        }
    ...
    private void commitArrayIndex(Marker from, Marker to, long originalMarker)
    {
        while (true)
        {
            ...
            if (rc == 1)
            {
                ...
                long marker = from.getMarker(oldVal), delta = from.getDelta(oldVal), otherMarker = to.getMarker(oldVal), otherDelta = to.getDelta(oldVal);
                assert rc <= delta;
                ...
                } else if (otherDelta > 0 && otherDelta != NO_DELTA && applyDeltaToMarker(otherMarker, marker, otherDelta) == NO_MARKER)
                {
    ...
    private long incDeltaValue(long markerFrom, long otherMarker, long delta)
    {
        ...
        {
            return NO_DELTA; // The (pool-sized) list is being fully drained.
        }
        long result = delta + 1;
    ...
    private long applyDeltaToMarker(long marker, long markerLimit, long delta)
    {
        if (delta == NO_DELTA)
        {
            return marker; // List was recreated while we were exhausting it.
        }
        if (delta == pool.length)
        {
        ...
        if (marker == markerLimit)
        {
            return NO_MARKER; // We hit the limit - the list was exhausted.
        }
        return marker;
    }
    ...
    private final static class CasLog
    {
        ...
        public synchronized void dumpLog(boolean doSleep)
        {
            ...
            {
                LOG.info("CAS history dump: " + FixedSizedObjectPool.toString(log[i << 1]) + " => " + FixedSizedObjectPool.toString(log[(i << 1) + 1]));
            }
            offset.set(0);
        }
    }
