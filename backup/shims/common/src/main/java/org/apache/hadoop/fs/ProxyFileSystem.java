...
public class ProxyFileSystem extends FilterFileSystem
{
    ...
    {
        return super.getFileBlockLocations(swizzleFileStatus(file, true), start, len);
    }
    ...
    {
        return super.create(swizzleParamPath(f), permission, overwrite, bufferSize, replication, blockSize, progress);
    }
    ...
    public RemoteIterator<FileStatus> listStatusIterator(Path f) throws IOException
    {
        ...
        {
            private final RemoteIterator<FileStatus> orig = ProxyFileSystem.super.listStatusIterator(swizzleParamPath(f));
