...
public class TestHadoop23Shims
{
    ...
    private Path getMockedPath(boolean supportXAttr) throws IOException
    {
        ...
        {
            when(fs.getXAttrs(any())).thenThrow(new UnsupportedOperationException("XAttr not supported for file system."));
        }
    ...
    public void testConstructDistCpParams() throws Exception
    {
        ...
        List<String> paramsWithCustomParamInjection = shims.constructDistCpParams(Collections.singletonList(copySrc), copyDst, conf);

        assertEquals(8, paramsWithCustomParamInjection.size());

        // check that the mandatory ones remain along with user passed ones.
        assertTrue("Distcp -update set even if not requested", paramsWithCustomParamInjection.contains("-update"));
        assertTrue("Distcp -delete set even if not requested", paramsWithCustomParamInjection.contains("-delete"));
        assertTrue("Distcp -foo is set as passes", paramsWithCustomParamInjection.contains("-foo"));
        assertTrue("Distcp -blah is set as passes", paramsWithCustomParamInjection.contains("-blah"));
        assertTrue("Distcp -pug is set as passes", paramsWithCustomParamInjection.contains("-pug"));
        assertTrue("Distcp -pbx not set as overridden", !paramsWithCustomParamInjection.contains("-pbx"));
        assertTrue("Distcp -skipcrccheck not set if not requested", !paramsWithCustomParamInjection.contains("-skipcrccheck"));
