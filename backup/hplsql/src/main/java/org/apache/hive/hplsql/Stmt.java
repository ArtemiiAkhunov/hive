...
public class Stmt
{
     ...
     */
    public Integer createTemporaryTable(HplsqlParser.QidentContext identCtx, HplsqlParser.Create_table_definitionContext defCtx, HplsqlParser.Create_table_preoptionsContext optCtx)
    {
        StringBuilder sql = new StringBuilder();
    ...
    {
        return range(0, count).mapToObj(i -> ctx.ident(i + 1).getText()).collect(Collectors.toList());
    }
    ...
    private String generateUpdateQuery(HplsqlParser.Update_stmtContext ctx)
    {
        ...
        StringBuilder sql = new StringBuilder(Exec.getFormattedText(ctx, ctx.start.getStartIndex(), (updateAssignmentContext.start.getStartIndex() - 1)));
        ...
        {
            exec.append(sql, Exec.getFormattedText(updateUpsertContext, updateUpsertContext.start.getStartIndex(), updateUpsertContext.stop.getStopIndex()), last, updateUpsertContext.getStart());
        }
        return sql.toString();
    }
    ...
    public Integer summaryStat(HplsqlParser.Summary_stmtContext ctx, String table, String select, Row row, String conn, Conn.Type connType)
    {
        ...
        try
        {
            ...
            if (query.next())
            {
                ...
                // Pad output
                String fmt = String.format("%%-%ds\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s" + "\t%%-11s\t%%-11s\t%%-11s\t%%-11s\t%%-11s\n", maxColName + 1);
                System.out.print(String.format(fmt, "Column", "Type", "Rows", "NonNull", "Unique", "Avg", "Min", "Max", "StdDev", "p05", "p25", "p50", "p75", "p95"));
                for (Column c : row.getColumns())
                {
                    ...
                    System.out.print(String.format(fmt, c.getName(), c.getType(), cntRows, query.column(1 + i * cc, String.class), query.column(2 + i * cc, String.class), avg, query.column(4 + i * cc, String.class), query.column(5 + i * cc, String.class), stddev, p05, p25, p50, p75, p95));
                    i++;
                }
            }
        } catch (QueryException e)
    ...
    public Integer summaryTop(HplsqlParser.Summary_stmtContext ctx, String table, String select, Row row, String conn, Conn.Type connType)
    {
        StringBuilder sql = new StringBuilder("SELECT id, col, cnt FROM (" + "SELECT id, col, cnt, ROW_NUMBER() OVER (PARTITION BY id ORDER BY cnt DESC) rn " + "FROM (SELECT CAST(GROUPING__ID AS DECIMAL) id, COALESCE(");   // CAST AS INT does not work as expected (ID is still considered as STRING in ORDER BY for some reason)
