...
public class HMSClientTest
{
    ...
    private static final ImmutableMap<String, String> TEST_DATABASE_PARAMS = new ImmutableMap.Builder<String, String>().put(PARAM_NAME, VALUE_NAME).build();
    private static boolean hasClient = false;

    private static final String TEST_TABLE_NAME = "test1";
    private static final Table TEST_TABLE = Util.TableBuilder.buildDefaultTable(TEST_DATABASE, TEST_TABLE_NAME);
    ...
    {
        Database db = new Util.DatabaseBuilder(TEST_DATABASE).withDescription(TEST_DATABASE_DESCRIPTION).withParams(TEST_DATABASE_PARAMS).build();
        // Create client and default test database
        try
        {
            client = new HMSClient(getServerUri(null, null), null);
    ...
    {
        Database db = new Util.DatabaseBuilder(TEST_DATABASE).build();
        db.setName(null);
        Throwable exception = Assertions.assertThrows(MetaException.class, () -> client.createDatabase(db));
    }
    ...
    public void createDatabaseEmptyName()
    {
        ...
        Database db = new Util.DatabaseBuilder(TEST_DATABASE).build();
        db.setName("");
        Throwable exception = Assertions.assertThrows(InvalidObjectException.class, () -> client.createDatabase(db));
    }
    ...
    {
        Throwable exception = Assertions.assertThrows(NoSuchObjectException.class, () -> client.getDatabase("WhatIsThisDatabase"));
    }
    ...
    {
        Throwable exception = Assertions.assertThrows(NoSuchObjectException.class, () -> client.dropDatabase("WhatIsThisDatabase"));
    }
