...
public class KafkaOutputFormat implements HiveOutputFormat<NullWritable, KafkaWritable>
{
    ...
    public FileSinkOperator.RecordWriter getHiveRecordWriter(JobConf jc, Path finalOutPath, Class<? extends Writable> valueClass, boolean isCompressed, Properties tableProperties, Progressable progress)
    {
        ...
        final WriteSemantic writeSemantic = WriteSemantic.valueOf(jc.get(KafkaTableProperties.WRITE_SEMANTIC_PROPERTY.getName()));
        ...
        switch (writeSemantic)
        {
            ...
            case EXACTLY_ONCE:
                ...
                recordWriter = new TransactionalKafkaWriter(topic, producerProperties, new Path(Preconditions.checkNotNull(finalOutPath), queryId), fs, optimisticCommit);
                break;
            default:
