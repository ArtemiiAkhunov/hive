...
class KafkaScanTrimmer
{
    ...
    Map<TopicPartition, KafkaInputSplit> computeOptimizedScan(ExprNodeGenericFuncDesc filterExpression)
    {
        ...
        if (LOG.isDebugEnabled())
        {
            if (optimizedScan != null)
            {
                ...
                optimizedScan.forEach((tp, input) -> LOG.debug("Topic-[{}] Partition-[{}] - Split startOffset [{}] :-> endOffset [{}]", tp.topic(), tp.partition(), input.getStartOffset(), input.getEndOffset()));
            } else
            {
                LOG.debug("No optimization thus using full scan ");
                fullHouse.forEach((tp, input) -> LOG.debug("Topic-[{}] Partition-[{}] - Split startOffset [{}] :-> endOffset [{}]", tp.topic(), tp.partition(), input.getStartOffset(), input.getEndOffset()));
            }
        }
    ...
    static Map<TopicPartition, KafkaInputSplit> buildScanFromPartitionPredicate(Map<TopicPartition, KafkaInputSplit> fullScan, PredicateLeaf.Operator operator, int partitionConst, boolean flip, boolean negation)
    {
        ...
        switch (operator)
        {
            case EQUALS:
                ...
                break;
            case LESS_THAN:
                intermediatePredicate = flip ? topicPartition -> topicPartition != null && partitionConst < topicPartition.partition() : topicPartition -> topicPartition != null && topicPartition.partition() < partitionConst;
        ...
        fullScan.entrySet().stream().filter(entry -> predicate.test(entry.getKey())).forEach(entry -> builder.put(entry.getKey(), KafkaInputSplit.copyOf(entry.getValue())));
        ...
        fullScan.forEach((tp, existingInputSplit) ->
        {
            ...
            if (startOffset != -1 && endOffset == -1)
            {
                newInputSplit = new KafkaInputSplit(tp.topic(), tp.partition(),
                        ...
                        Math.min(startOffset, existingInputSplit.getEndOffset()), existingInputSplit.getEndOffset(), existingInputSplit.getPath());
            ...
            } else if (endOffset == startOffset + 1)
            {
                ...
                {
                    newInputSplit = new KafkaInputSplit(tp.topic(), tp.partition(), startOffset, endOffset, existingInputSplit.getPath());
                }

            } else
            {
                newInputSplit = new KafkaInputSplit(tp.topic(), tp.partition(), existingInputSplit.getStartOffset(), existingInputSplit.getEndOffset(), existingInputSplit.getPath());
            }
    ...
    {
        long increment = (flip && operator == PredicateLeaf.Operator.LESS_THAN || negation && operator == PredicateLeaf.Operator.LESS_THAN_EQUALS) ? 1L : 0L;
