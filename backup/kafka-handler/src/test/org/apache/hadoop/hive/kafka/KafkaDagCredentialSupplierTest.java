...
{
    private static final java.nio.file.Path KEYSTORE_DIR = Paths.get(System.getProperty("test.tmp.dir"), "kdc_root_dir" + UUID.randomUUID());
    ...
    private static final KafkaBrokerResource KAFKA_BROKER_RESOURCE = new KafkaBrokerResource().enableSASL(KAFKA_USER_NAME, KAFKA_USER_KEYTAB.toString());
    ...
    @Test
    public void testObtainTokenFromSamlSslListenerNotNull() throws IOException, URISyntaxException, KeyStoreException, CertificateException, NoSuchAlgorithmException
    {
        Properties props = new Properties();
        props.setProperty(KafkaTableProperties.HIVE_KAFKA_BOOTSTRAP_SERVERS.getName(), KafkaBrokerResource.BROKER_SASL_SSL_PORT);
    ...
    private static MapWork createFileSinkWork(TableDesc tableDesc)
    {
        ...
        Operator<FileSinkDesc> op = OperatorFactory.get(new CompilationOpContext(), new FileSinkDesc(fakePath, tableDesc, true));
     ...
     */
    private static URI createCredentialStore(Map<String, String> keys) throws IOException, KeyStoreException, CertificateException, NoSuchAlgorithmException, URISyntaxException
    {
        ...
        try (OutputStream fos = Files.newOutputStream(storePath))
        {
            ...
            {
                ks.setKeyEntry(k.getKey(), new SecretKeySpec(k.getValue().getBytes(), "AES/CTR/NoPadding"), KEYSTORE_PASSWORD_DEFAULT, null);
            }
