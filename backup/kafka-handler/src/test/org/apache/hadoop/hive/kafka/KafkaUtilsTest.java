...
public class KafkaUtilsTest
{
    ...
    public void canNotSetForbiddenProp()
    {
        Configuration configuration = new Configuration();
        configuration.set("kafka.bootstrap.servers", "localhost:9090");
    ...
    @Test(expected = IllegalArgumentException.class)
    public void canNotSetForbiddenProp2()
    {
        Configuration configuration = new Configuration();
        configuration.set("kafka.bootstrap.servers", "localhost:9090");
    ...
    @Test
    public void testSetupKafkaSslPropertiesNoSslIsUnchanged()
    {
    ...
    public void testSetupKafkaSslPropertiesSslNotInHdfs()
    {
        Configuration config = new Configuration();
    ...
    @Test(expected = IllegalStateException.class)
    public void testSetupKafkaSslPropertiesCantRetrieveStore()
    {
        Configuration config = new Configuration();
        config.set(KafkaTableProperties.HIVE_KAFKA_SSL_CREDENTIAL_KEYSTORE.getName(), "nonexistentfile");
    ...
    @Test
    public void testMetadataEnumLookupMapper()
    {
        int partition = 1;
        long offset = 5L;
        long ts = 1000000L;
        ...
        // ORDER MATTERS here.
        List<Writable> expectedWritables = Arrays.asList(new BytesWritable(key), new IntWritable(partition), new LongWritable(offset), new LongWritable(ts));
        KafkaWritable kafkaWritable = new KafkaWritable(partition, offset, ts, value, key);

        List<Writable> actual = MetadataColumn.KAFKA_METADATA_COLUMN_NAMES.stream().map(MetadataColumn::forName).map(kafkaWritable::getHiveWritable).collect(Collectors.toList());
    ...
    public void testEnsureThatAllTheColumnAreListed()
    {
        ...
        Assert.assertFalse(Arrays.stream(MetadataColumn.values()).map(MetadataColumn::getName).anyMatch(name -> !MetadataColumn.KAFKA_METADATA_COLUMN_NAMES.contains(name)));
        Arrays.stream(MetadataColumn.values()).forEach(element -> Assert.assertNotNull(MetadataColumn.forName(element.getName())));
    }
