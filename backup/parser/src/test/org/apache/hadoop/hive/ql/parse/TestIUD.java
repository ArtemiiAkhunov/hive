...
public class TestIUD
{
    ...
    public void testDeleteWithWhere() throws ParseException
    {
        ...
        Assert.assertEquals("AST doesn't match", "(tok_delete_from " + "(tok_tabname src) " + "(tok_where " + "(and " + "(tok_function isnotnull (tok_table_or_col key)) " + "(< (. (tok_table_or_col src) value) 0))))", ast.toStringTree());
    }
    ...
    public void testUpdateWithWhereSingleSet() throws ParseException
    {
        ...
        Assert.assertEquals("AST doesn't match", "(tok_update_table " + "(tok_tabname src) " + "(tok_set_columns_clause " + "(= " + "(tok_table_or_col key) 3)) " + "(tok_where (tok_function isnull (tok_table_or_col value))))", ast.toStringTree());
    }
    ...
    public void testUpdateWithWhereSingleSetExpr() throws ParseException
    {
        ...
        Assert.assertEquals("AST doesn't match", "(tok_update_table (tok_tabname src) " + "(tok_set_columns_clause " + "(= (tok_table_or_col key) (+ (- 3) (% (* 5 9) 8))) " + "(= (tok_table_or_col val) (tok_function tok_int (+ 6.1 (tok_table_or_col c)))) " + "(= (tok_table_or_col d) (- (tok_table_or_col d) 1))) " + "(tok_where (tok_function isnull (tok_table_or_col value))))", ast.toStringTree());
    }
    ...
    public void testSelectStarFromAnonymousVirtTable1Row() throws ParseException
    {
        ...
        {
            Assert.assertEquals("Failure didn't match.", "line 1:24 cannot recognize input near 'values' '(' '3' in joinSource", ex.getMessage());
        }
    }
