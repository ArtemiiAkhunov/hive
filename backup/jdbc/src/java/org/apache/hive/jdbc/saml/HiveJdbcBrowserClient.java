...
public class HiveJdbcBrowserClient implements IJdbcBrowserClient
{
    ...
    private final BlockingQueue<HiveJdbcBrowserServerResponse> serverResponseQueue = new LinkedBlockingDeque<>();
    ...
    HiveJdbcBrowserClient(JdbcConnectionParams connectionParams) throws HiveJdbcBrowserException
    {
        portFromUrl = Integer.parseInt(connectionParams.getSessionVars().getOrDefault(JdbcConnectionParams.AUTH_BROWSER_RESPONSE_PORT, "0"));
        timeoutInMs = Integer.parseInt(connectionParams.getSessionVars().getOrDefault(JdbcConnectionParams.AUTH_BROWSER_RESPONSE_TIMEOUT_SECS, String.valueOf(DEFAULT_SOCKET_TIMEOUT_SECS))) * 1000L;
    }
    ...
    public void init(JdbcBrowserClientContext clientContext)
    {
        ...
        LOG.debug("Initialized the JDBCBrowser client with URL {}", clientContext.getSsoUri());
    }
    ...
    public void doBrowserSSO() throws HiveJdbcBrowserException
    {
        ...
        {
            throw new HiveJdbcBrowserException("Received invalid response from server. See driver logs for more details");
        }
    }
    ...
    private Map<String, String> getQueryParams(URI ssoUri) throws HiveJdbcBrowserException
    {
        String decodedUrl;
        try
        {
            ...
            } else
            {
                ...
                switch (os)
                {
                    ...
                    case LINUX:
                        ...
                        break;
                    case UNKNOWN:
                        throw new HiveJdbcBrowserException("Unknown operating system " + System.getProperty("os.name"));
                }
            }
        } catch (IOException e)
        {
    ...
    private void sendBrowserMsg(Socket socket, boolean success) throws IOException
    {
        ...
        if (success)
        {
            responseText = "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"/>" + "<title>SAML Response Received</title></head>" + "<body onload=\"waitAndClose()\">Successfully authenticated. You may close this window.</body>" + "<script>" + "  function waitAndClose() {" + "    setTimeout(function() {" + "      window.close()" + "    }, 100);" + "  }" + "</script>" + "</html>";
        } else
        {
            responseText = "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"/>" + "<title>SAML Response Received</title></head>" + "<body>Authentication failed. Please check server logs for details." + " You may close this window.</body></html>";
        }
