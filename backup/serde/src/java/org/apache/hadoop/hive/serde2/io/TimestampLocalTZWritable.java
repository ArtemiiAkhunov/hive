...
public class TimestampLocalTZWritable implements WritableComparable<TimestampLocalTZWritable>
{
    ...
    public void setTimeZone(ZoneId timeZone)
    {
        if (timestampTZ != null)
        {
            timestampTZ.setZonedDateTime(timestampTZ.getZonedDateTime().withZoneSameInstant(timeZone));
        }
        this.timeZone = timeZone;
    }
    ...
    private void populateBytes()
    {
        ...
        if (hasSecondVInt)
        {
            LazyBinaryUtils.writeVLongToByteArray(internalBytes, offset + 4 + WritableUtils.decodeVIntSize(internalBytes[offset + 4]), seconds >> 31);
        }
    }
    ...
    public void readFields(DataInput dataInput) throws IOException
    {
        ...
        if (hasDecimalOrSecondVInt(internalBytes[0]))
        {
            ...
            Preconditions.checkState(vlong >= -1000000000 && vlong <= 999999999, "Invalid nanos value for a TimestampTZWritable: " + vlong + ", expected to be between -1000000000 and 999999999.");
            if (vlong < 0)
            {
    ...
    public static long getSeconds(byte[] bytes, int offset)
    {
        ...
        return ((long) (firstVInt & ~DECIMAL_OR_SECOND_VINT_FLAG)) | (LazyBinaryUtils.readVLongFromByteArray(bytes, offset + 4 + WritableUtils.decodeVIntSize(bytes[offset + 4])) << 31);
    }
    ...
    private static long readSevenByteLong(byte[] bytes, int offset)
    {
        ...
        return (((0xFFL & bytes[offset]) << 56) | ((0xFFL & bytes[offset + 1]) << 48) | ((0xFFL & bytes[offset + 2]) << 40) | ((0xFFL & bytes[offset + 3]) << 32) | ((0xFFL & bytes[offset + 4]) << 24) | ((0xFFL & bytes[offset + 5]) << 16) | ((0xFFL & bytes[offset + 6]) << 8)) >> 8;
    }
}
