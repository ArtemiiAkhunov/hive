...
public final class TypeInfoUtils
{
    ...
    public static EnumMap<PrimitiveCategory, Integer> numericTypes = new EnumMap<PrimitiveCategory, Integer>(PrimitiveCategory.class);
    ...
    public static EnumMap<PrimitiveCategory, Integer> dateTypes = new EnumMap<PrimitiveCategory, Integer>(PrimitiveCategory.class);
    ...
    public static boolean doPrimitiveCategoriesMatch(TypeInfo ti1, TypeInfo ti2)
    {
        ...
        {
            if (((PrimitiveTypeInfo) ti1).getPrimitiveCategory() == ((PrimitiveTypeInfo) ti2).getPrimitiveCategory())
            {
                return true;
            }
        }
        ...
        public ArrayList<TypeInfo> parseTypeInfos()
        {
            ...
            while (iToken < typeInfoTokens.size())
            {
                ...
                if (iToken < typeInfoTokens.size())
                {
                    ...
                    {
                        throw new IllegalArgumentException("Error: ',', ':', or ';' expected at position " + separator.position + " from '" + typeInfoString + "' " + typeInfoTokens);
                    }
                }
            }
            return typeInfos;
    ...
    static ConcurrentHashMap<TypeInfo, ObjectInspector> cachedStandardObjectInspector = new ConcurrentHashMap<TypeInfo, ObjectInspector>();
        ...
        if (result == null)
        {
            switch (typeInfo.getCategory())
            {
                ...
                case UNION:
                {
                    ...
                    {
                        fieldObjectInspectors.add(getStandardWritableObjectInspectorFromTypeInfo(objectTypeInfos.get(i)));
                    }
            ...
            ObjectInspector prev = cachedStandardObjectInspector.putIfAbsent(typeInfo, result);
            if (prev != null)
            {
                result = prev;
            }
    ...
    static ConcurrentHashMap<TypeInfo, ObjectInspector> cachedStandardJavaObjectInspector = new ConcurrentHashMap<TypeInfo, ObjectInspector>();
    ...
    public static ObjectInspector getStandardJavaObjectInspectorFromTypeInfo(TypeInfo typeInfo)
    {
        ...
        if (result == null)
        {
            ...
            ObjectInspector prev = cachedStandardJavaObjectInspector.putIfAbsent(typeInfo, result);
            if (prev != null)
            {
                result = prev;
            }
    ...
    private static final Set<Set<PrimitiveCategory>> LOSSY_TYPE_CONVERSIONS = ImmutableSet.<Set<PrimitiveCategory>>builder().add(EnumSet.of(PrimitiveCategory.DECIMAL, PrimitiveCategory.CHAR)).add(EnumSet.of(PrimitiveCategory.DECIMAL, PrimitiveCategory.VARCHAR)).add(EnumSet.of(PrimitiveCategory.DECIMAL, PrimitiveCategory.STRING)).add(EnumSet.of(PrimitiveCategory.DOUBLE, PrimitiveCategory.LONG)).add(EnumSet.of(PrimitiveCategory.LONG, PrimitiveCategory.CHAR)).add(EnumSet.of(PrimitiveCategory.LONG, PrimitiveCategory.VARCHAR)).add(EnumSet.of(PrimitiveCategory.LONG, PrimitiveCategory.STRING)).build();
