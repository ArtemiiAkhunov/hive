...
public class TimestampWritableV2 implements WritableComparable<TimestampWritableV2>
{
    ...
    public int getNanos()
    {
        ...
        {
            return hasDecimalOrSecondVInt() ? TimestampWritableV2.getNanos(currentBytes, offset + 4) : 0;
        } else
        {
            throw new IllegalStateException("Both timestamp and bytes are empty");
        }
    ...
    public void readFields(DataInput in) throws IOException
    {
        ...
        if (TimestampWritableV2.hasDecimalOrSecondVInt(internalBytes[0]))
        {
            ...
            {
                throw new IOException("Invalid first vint value (encoded nanoseconds) of a TimestampWritableV2: " + vlong + ", expected to be between -1000000000 and 999999999.");
    ...
    public static long getSeconds(byte[] bytes, int offset)
    {
        ...
        return ((long) (lowest31BitsOfSecondsAndFlag & LOWEST_31_BITS_OF_SEC_MASK)) | (LazyBinaryUtils.readVLongFromByteArray(bytes, offset + 4 + WritableUtils.decodeVIntSize(bytes[offset + 4])) << 31);
    }
