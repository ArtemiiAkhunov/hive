...
public class LazyBinaryStruct extends LazyBinaryNonPrimitive<LazyBinaryStructObjectInspector> implements StructObject, SerDeStatsStruct
{
    ...
    private void parse()
    {
        ...
        for (int i = 0; i < fields.length; i++)
        {
            ...
            {
                fieldIsNull[i] = false;
                LazyBinaryUtils.checkObjectByteInfo(fieldRefs.get(i).getFieldObjectInspector(), bytes, lastFieldByteEnd, recordInfo, vInt);
        ...
        if (!missingFieldWarned && lastFieldByteEnd > structByteEnd)
        {
            ...
            LOG.warn("Missing fields! Expected " + fields.length + " fields but " + "only got " + fieldId + "! " + "Last field end " + lastFieldByteEnd + " and serialize buffer end " + structByteEnd + ". " + "Ignoring similar problems.");
        }
