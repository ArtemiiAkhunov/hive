...
public final class LazySimpleDeserializeRead extends DeserializeRead
{
    ...
    private int addComplexTypeHelper(Field complexField, int depth)
    {
        ...
        switch (complexField.complexCategory)
        {
            case LIST:
            {
                ...
                final ListComplexTypeHelper listHelper = new ListComplexTypeHelper(complexField, elementField);
            ...
            case MAP:
            {
                ...
                final MapComplexTypeHelper mapHelper = new MapComplexTypeHelper(complexField, keyField, valueField);
            ...
            case STRUCT:
            {
                ...
                final StructComplexTypeHelper structHelper = new StructComplexTypeHelper(complexField, fields);
            ...
            case UNION:
            {
                ...
                final Field[] fields = new Field[fieldTypeInfoList.size()];
                depth = addComplexFields(fieldTypeInfoList, fields, depth);
                final UnionComplexTypeHelper structHelper = new UnionComplexTypeHelper(complexField, fields);
                complexField.complexTypeHelper = structHelper;
            }
            break;
            default:
    ...
    public LazySimpleDeserializeRead(TypeInfo[] typeInfos, DataTypePhysicalVariation[] dataTypePhysicalVariations, boolean useExternalBuffer, LazySerDeParameters lazyParams)
    {
        ...
        timestampParser = (timestampFormats == null) ? new TimestampParser() : new TimestampParser(timestampFormats);
    ...
    public String getDetailedReadPositionString()
    {
        ...
        } else
        {
            ...
            int currentFieldLength = startPositions[currentTopLevelFieldIndex + 1] - startPositions[currentTopLevelFieldIndex] - 1;
            ...
            if (field.isPrimitive)
            {
                switch (field.primitiveCategory)
                {
                    case BOOLEAN:
                    {
                        ...
                        if (fieldLength == 4)
                        {
                            if ((bytes[i] == 'T' || bytes[i] == 't') && (bytes[i + 1] == 'R' || bytes[i + 1] == 'r') && (bytes[i + 2] == 'U' || bytes[i + 2] == 'u') && (bytes[i + 3] == 'E' || bytes[i + 3] == 'e'))
                            {
                                currentBoolean = true;
                            } else
                            {
            ...
            } else
            {
                switch (field.complexCategory)
                {
                    ...
                    case UNION:
                    {
                        ...
                        if (currentLevel > 0 && currentComplexTypeHelpers[currentLevel] == null && currentComplexTypeHelpers[currentLevel - 1] instanceof MapComplexTypeHelper)
                        {
                            currentLevel++;
                        }
    ...
    private void popComplexType()
    {
        ...
        if (currentLevel > 0)
        {
            ...
            if (currentComplexTypeHelpers[currentLevel - 1] == null)
            {
                ...
                Preconditions.checkState(currentComplexTypeHelpers[currentLevel - 2] instanceof MapComplexTypeHelper);
                currentLevel--;
            }
        }
    }
    ...
    {
        return currentLevel >= 2 && currentComplexTypeHelpers[currentLevel - 2] instanceof MapComplexTypeHelper;
    }
    ...
    public void logExceptionMessage(byte[] bytes, int bytesStart, int bytesLength, Category dataComplexCategory, PrimitiveCategory dataPrimitiveCategory)
    {
