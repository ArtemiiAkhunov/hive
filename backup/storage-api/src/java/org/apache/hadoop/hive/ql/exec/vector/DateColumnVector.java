...
public class DateColumnVector extends LongColumnVector
{
    ...
    private void updateDataAccordingProlepticSetting() throws Exception
    {
        ...
        {
            if (vector[i] >= CalendarUtils.SWITCHOVER_DAYS)
            { // no need for conversion
                continue;
            }
            vector[i] = usingProlepticCalendar ? CalendarUtils.convertDateToProleptic((int) vector[i]) : CalendarUtils.convertDateToHybrid((int) vector[i]);
        }
    }
