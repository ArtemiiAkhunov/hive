...
public class TestBytesColumnVector
{
    ...
    public void testConcatAndPadding()
    {
        ...
        for (int row = 0; row < col.vector.length; ++row)
        {
            StringExpr.padRight(col, row, col.vector[row], col.start[row], col.length[row], 6);
        }
        for (int row = 0; row < col.vector.length; ++row)
        {
            assertEquals("row " + row, "緑     ", col.toString(row));
        ...
        for (int row = 0; row < col.vector.length; ++row)
        {
            ...
            col.setConcat(row, col.vector[row], col.start[row], col.length[row], rowStr, 0, rowStr.length);
        }
        for (int row = 0; row < col.vector.length; ++row)
        {
