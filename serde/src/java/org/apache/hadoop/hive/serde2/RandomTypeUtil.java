...
public class RandomTypeUtil
{
    ...
    public static String getRandUnicodeString(Random r, int length)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            while (true)
            {
                ...
                if (!Character.isDefined(codePoint) || Character.getType(codePoint) == Character.PRIVATE_USE)
                {
                    continue;
                }
                ch = (char) codePoint;
    ...
    public static Timestamp getRandTimestamp(Random r, int minYear, int maxYear)
    {
        ...
        String timestampStr = String.format("%04d-%02d-%02d %02d:%02d:%02d%s", Integer.valueOf(minYear + r.nextInt(maxYear - minYear + 1)),  // year
                Integer.valueOf(1 + r.nextInt(12)),      // month
                Integer.valueOf(1 + r.nextInt(28)),      // day
