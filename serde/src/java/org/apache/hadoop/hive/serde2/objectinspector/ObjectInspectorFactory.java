...
public final class ObjectInspectorFactory
{
    ...
    private static CacheBuilder<Object, Object> boundedBuilder = CacheBuilder.newBuilder().initialCapacity(1024).maximumSize(10240) // 10x initial capacity
            .concurrencyLevel(Runtime.getRuntime().availableProcessors()).expireAfterAccess(5, TimeUnit.MINUTES).softValues();

    private static CacheBuilder<Object, Object> unboundedBuilder = CacheBuilder.newBuilder().initialCapacity(1024).concurrencyLevel(Runtime.getRuntime().availableProcessors()).expireAfterAccess(5, TimeUnit.MINUTES).softValues();
    ...
    {
        StandardListObjectInspector result = cachedStandardListObjectInspector.asMap().get(listElementObjectInspector);
        if (result == null)
        {
    ...
    public static StandardMapObjectInspector getStandardMapObjectInspector(ObjectInspector mapKeyObjectInspector, ObjectInspector mapValueObjectInspector)
    {
        ...
        StandardMapObjectInspector result = cachedStandardMapObjectInspector.asMap().get(signature);
        if (result == null)
        {
    ...
    {
        StandardUnionObjectInspector result = cachedStandardUnionObjectInspector.asMap().get(unionObjectInspectors);
        if (result == null)
        {
    ...
    public static ColumnarStructObjectInspector getColumnarStructObjectInspector(List<String> structFieldNames, List<ObjectInspector> structFieldObjectInspectors, List<String> structFieldComments)
    {
        ...
        ColumnarStructObjectInspector result = cachedColumnarStructObjectInspector.asMap().get(signature);
        if (result == null)
        {
