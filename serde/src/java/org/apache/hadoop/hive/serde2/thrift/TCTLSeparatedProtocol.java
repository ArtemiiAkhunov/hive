...
public class TCTLSeparatedProtocol extends TProtocol implements ConfigurableTProtocol, WriteNullsProtocol, SkippableTProtocol
{
     ...
     * @param secondarySeparator the separator within a field for things like sets and maps and
     *                           lists
     * @param mapSeparator       - the key/value separator
     * @param rowSeparator       - the record separator
     * @param returnNulls        - whether to return a null or an empty string for fields that seem
    ...
    public void initialize(Configuration conf, Properties tbl) throws TException
    {
        ...
        primarySeparator = getByteValue(tbl.getProperty(serdeConstants.FIELD_DELIM), primarySeparator);
        secondarySeparator = getByteValue(tbl.getProperty(serdeConstants.COLLECTION_DELIM), secondarySeparator);
        rowSeparator = getByteValue(tbl.getProperty(serdeConstants.LINE_DELIM), rowSeparator);
        mapSeparator = getByteValue(tbl.getProperty(serdeConstants.MAPKEY_DELIM), mapSeparator);
        returnNulls = Boolean.parseBoolean(tbl.getProperty(ReturnNullsKey, String.valueOf(returnNulls)));
        bufferSize = Integer.parseInt(tbl.getProperty(BufferSizeKey, String.valueOf(bufferSize)));
    ...
    {
        if (list.elemType == TType.STRUCT || list.elemType == TType.MAP || list.elemType == TType.LIST || list.elemType == TType.SET)
        {
            throw new TException("Not implemented: nested structures");
        }
        firstInnerField = true;
    ...
    {
        if (set.elemType == TType.STRUCT || set.elemType == TType.MAP || set.elemType == TType.LIST || set.elemType == TType.SET)
        {
            throw new TException("Not implemented: nested structures");
        }
        firstInnerField = true;
