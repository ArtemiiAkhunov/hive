...
public class LowLevelCacheMemoryManager implements MemoryManager
{
    ...
    public LowLevelCacheMemoryManager(long maxSize, LowLevelCachePolicy evictor, LlapDaemonCacheMetrics metrics)
    {
        ...
        {
            LlapIoImpl.LOG.info("Memory manager initialized with max size {} and" + " {} ability to evict blocks", maxSize, ((evictor == null) ? "no " : ""));
        }
    }
    ...
    public static class ReserveFailedException extends RuntimeException
    {
        ...
        {
            super("Cannot reserve memory" + (Thread.currentThread().isInterrupted() ? "; thread interrupted" : "") + ((isStopped != null && isStopped.get()) ? "; thread stopped" : ""));
        }
    }
    ...
    @VisibleForTesting
    public boolean reserveMemory(final long memoryToReserve, boolean waitForEviction, AtomicBoolean isStopped)
    {
