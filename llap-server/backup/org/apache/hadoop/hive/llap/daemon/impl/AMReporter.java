...
public class AMReporter extends AbstractService
{
   ...
   */
    ...
    public AMReporter(int numExecutors, int maxThreads, AtomicReference<InetSocketAddress> localAddress, QueryFailedHandler queryFailedHandler, Configuration conf, DaemonId daemonId, SocketFactory socketFactory)
    {
        ...
        {
            LOG.warn("maxThreads={} is less than numExecutors={}. Setting maxThreads=numExecutors", maxThreads, numExecutors);
            maxThreads = numExecutors;
        }
        ExecutorService rawExecutor = new ThreadPoolExecutor(numExecutors, maxThreads, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadFactoryBuilder().setDaemon(true).setNameFormat("AMReporter %d").build());
    ...
    public AMNodeInfo registerTask(boolean externalClientRequest, String amLocation, int port, String umbilicalUser, Token<JobTokenIdentifier> jobToken, QueryIdentifier queryIdentifier, TezTaskAttemptID attemptId, boolean isGuaranteed)
    {
                ...
                try
                {
                    ...
                    if (amNodeInfo.hasAmFailed() || amNodeInfo.isDone())
                    {
                        synchronized (knownAppMasters)
                        {
                            ...
                            {
                                LOG.debug("Removing am {} with last associated dag {} from heartbeat with taskCount={}, amFailed={}, isDone={}", amNodeInfo.amNodeId, amNodeInfo.getQueryIdentifier(), amNodeInfo.getTaskCount(), amNodeInfo.hasAmFailed(), amNodeInfo.isDone());
                            }
                        ...
                        if (amNodeInfo.getTaskCount() > 0)
                        {
                            ...
                            Futures.addCallback(future, new FutureCallback<Void>()
                            {
                                ...
                                public void onFailure(Throwable t)
                                {
                                    ...
                                    LOG.warn("Heartbeat failed to AM {}. Marking query as failed. query={}", amNodeInfo.amNodeId, currentQueryIdentifier, t);
        ...
        protected Void callInternal()
        {
            ...
            try
            {
                ...
                if (LlapUtil.isCloudDeployment(conf) && amNodeInfo.isExternalClientRequest())
                {
                    ...
                    amNodeInfo.getUmbilical().nodeHeartbeat(new Text(hostname), new Text(daemonId.getUniqueNodeIdInCluster()), externalClientCloudRpcPort, aw, guaranteed);
                } else
                {
                    amNodeInfo.getUmbilical().nodeHeartbeat(new Text(nodeId.getHostname()), new Text(daemonId.getUniqueNodeIdInCluster()), nodeId.getPort(), aw, guaranteed);
                }
            } catch (IOException e)
            {
                QueryIdentifier currentQueryIdentifier = amNodeInfo.getQueryIdentifier();
    ...
    {
        final InetSocketAddress address = NetUtils.createSocketAddrForHost(amNodeInfo.amNodeId.getHostname(), amNodeInfo.amNodeId.getPort());
        ...
        return ugi.doAs(new PrivilegedExceptionAction<LlapTaskUmbilicalProtocol>()
        {
            ...
            {
                return RPC.getProxy(LlapTaskUmbilicalProtocol.class, LlapTaskUmbilicalProtocol.versionID, address, UserGroupInformation.getCurrentUser(), amNodeInfo.conf, amNodeInfo.socketFactory, (int) (amNodeInfo.timeout));
            }
        });
    }
