 ...
 */
public class OrcEncodedDataReader extends CallableWithNdc<Void> implements ConsumerFeedback<OrcEncodedColumnBatch>, TezCounterSource
{
    ...
    public OrcEncodedDataReader(LowLevelCache lowLevelCache, BufferUsageManager bufferManager, MetadataCache metadataCache, Configuration daemonConf, Configuration jobConf, FileSplit split, Includes includes, SearchArgument sarg, OrcEncodedDataConsumer consumer, QueryFragmentCounters counters, SchemaEvolutionFactory sef, Pool<IoTrace> tracePool, Map<Path, PartitionDesc> parts, PathCache pathCache) throws IOException
    {
        ...
        consumer.setUseDecimal64ColumnVectors(HiveConf.getVar(jobConf, ConfVars.HIVE_VECTORIZED_INPUT_FORMAT_SUPPORTS_ENABLED).equalsIgnoreCase("decimal_64"));
    ...
    static Supplier<FileSystem> getFsSupplier(final Path path, final Configuration conf)
    {
        return () ->
        {
            try
    ...
    private void ensureDataReader() throws IOException
    {
        ...
        stripeReader = orcReader.encodedReader(fileKey, dw, dw, useObjectPools ? POOL_FACTORY : null, trace, useCodecPool, cacheTag, isReadCacheOnly);
    ...
    {
        if (fileMetadata.getCompressionKind() == CompressionKind.NONE)
        {
            return;
        }
    ...
    private static Object determineFileId(Supplier<FileSystem> fsSupplier, FileSplit split, Configuration daemonConf) throws IOException
    {
    ...
    private OrcProto.StripeFooter buildStripeFooter(BufferChunk bcs, int len, CompressionCodec codec, int bufferSize) throws IOException
    {
        ...
        if (codec != null)
        {
            options = InStream.options().withCodec(OrcCodecPool.getCodec(codec.getKind())).withBufferSize(bufferSize);
        }
        return OrcProto.StripeFooter.parseFrom(InStream.createCodedInputStream(InStream.create("footer", new BufferChunk(bcs.getData(), 0), 0, len, options)));
    }
    ...
    private void ensureRawDataReader(boolean isOpen) throws IOException
    {
        ...
        {
            options = InStream.options().withCodec(OrcCodecPool.getCodec(orcReader.getCompressionKind())).withBufferSize(orcReader.getCompressionSize());
        }

        rawDataReader = LlapRecordReaderUtils.createDefaultLlapDataReader(DataReaderProperties.builder().withFileSystemSupplier(fsSupplier).withPath(path).withCompression(options).withZeroCopy(useZeroCopy).build());
    ...
    private class DataWrapperForOrc implements LlapDataReader, DataCache, BufferObjectFactory
    {
        ...
        @Override
        public DiskRangeList getFileData(Object fileKey, DiskRangeList range, long baseOffset, DiskRangeListFactory factory, BooleanRef gotAllData)
        {
            DiskRangeList result = lowLevelCache.getFileData(fileKey, range, baseOffset, factory, counters, gotAllData);
            if (LlapIoImpl.ORC_LOGGER.isTraceEnabled())
            {
                LlapIoImpl.ORC_LOGGER.trace("Disk ranges after data cache (file " + fileKey + ", base offset " + baseOffset + "): " + RecordReaderUtils.stringifyDiskRanges(result));
            ...
            return (metadataCache == null) ? result : metadataCache.getIncompleteCbs(fileKey, result, baseOffset, gotAllData);
        }
        ...
        public long[] putFileData(Object fileKey, DiskRange[] ranges, MemoryBuffer[] data, long baseOffset, CacheTag tag)
        {
            if (data != null)
            {
                return lowLevelCache.putFileData(fileKey, ranges, data, baseOffset, Priority.NORMAL, counters, tag);
        ...
        public DiskRangeList readFileData(DiskRangeList range, long baseOffset, boolean doForceDirect) throws IOException
        {
            ...
            if (LlapIoImpl.ORC_LOGGER.isTraceEnabled())
            {
                LlapIoImpl.ORC_LOGGER.trace("Disk ranges after disk read (file {}, base offset {}): {}", fileKey, baseOffset, RecordReaderUtils.stringifyDiskRanges(result));
            }
        ...
        @Override
        public OrcIndex readRowIndex(StripeInformation stripe, TypeDescription fileSchema, OrcProto.StripeFooter footer, boolean ignoreNonUtf8BloomFilter, boolean[] included, OrcProto.RowIndex[] indexes, boolean[] sargColumns, org.apache.orc.OrcFile.WriterVersion version, OrcProto.Stream.Kind[] bloomFilterKinds, OrcProto.BloomFilterIndex[] bloomFilterIndices) throws IOException
        {
            return orcDataReaderRef.readRowIndex(stripe, fileSchema, footer, ignoreNonUtf8BloomFilter, included, indexes, sargColumns, version, bloomFilterKinds, bloomFilterIndices);
        }
