...
final class LlapCacheMetadataSerializer
{
        ...
        for (LlapCacheableBuffer b : buffers)
        {
            if (b instanceof LlapDataBuffer)
            {
                ...
                try
                {
                    ...
                    if (path != null)
                    {
                        LlapDaemonProtocolProtos.CacheEntry.Builder builder = lookupOrCreateCacheEntryFromDataBuffer(lookupMap, db, fileKey, path);
                        LlapDaemonProtocolProtos.CacheEntryRange.Builder range = LlapDaemonProtocolProtos.CacheEntryRange.newBuilder().setStart(db.getStart()).setEnd(db.getStart() + db.declaredCachedLength);
    ...
    {
        return ct.getPartitionDescCount() == 0 ? CacheTag.build(ct.getTableName()) : CacheTag.build(ct.getTableName(), ct.getPartitionDescList());
    }
