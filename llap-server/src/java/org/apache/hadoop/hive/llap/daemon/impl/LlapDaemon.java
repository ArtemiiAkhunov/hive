...
public class LlapDaemon extends CompositeService implements ContainerRunner, LlapDaemonMXBean
{
    ...
    private final AtomicReference<InetSocketAddress> srvAddress = new AtomicReference<>(), mngAddress = new AtomicReference<>();
    ...
    public LlapDaemon(Configuration daemonConf, int numExecutors, long executorMemoryBytes, boolean ioEnabled, boolean isDirectCache, long ioMemoryBytes, String[] localDirs, int srvPort, boolean externalClientCloudSetupEnabled, int externalClientsRpcPort, int mngPort, int shufflePort, int webPort, String appName)
    {
        ...
        {
            Preconditions.checkArgument(externalClientsRpcPort == 0 || (externalClientsRpcPort > 1024 && externalClientsRpcPort < 65536), "Server RPC port for external clients must be between 1025 and 65535, or 0 automatic selection");
        }
        ...
        Preconditions.checkArgument(shufflePort == 0 || (shufflePort > 1024 && shufflePort < 65536), "Shuffle Port must be between 1024 and 65535, or 0 for automatic selection");
        int outputFormatServicePort = HiveConf.getIntVar(daemonConf, HiveConf.ConfVars.LLAP_DAEMON_OUTPUT_SERVICE_PORT);
        Preconditions.checkArgument(outputFormatServicePort == 0 || (outputFormatServicePort > 1024 && outputFormatServicePort < 65536), "OutputFormatService Port must be between 1024 and 65535, or 0 for automatic selection");
        ...
        try
        {
            ...
            daemonId = new DaemonId(currentUser, LlapUtil.generateClusterName(daemonConf), hostName, appName, System.currentTimeMillis());
        ...
        int timedWindowAverageDataPoints = HiveConf.getIntVar(daemonConf, ConfVars.LLAP_DAEMON_METRICS_TIMED_WINDOW_AVERAGE_DATA_POINTS);
        long timedWindowAverageWindowLength = HiveConf.getTimeVar(daemonConf, ConfVars.LLAP_DAEMON_METRICS_TIMED_WINDOW_AVERAGE_WINDOW_LENGTH, TimeUnit.NANOSECONDS);
        int simpleAverageWindowDataSize = HiveConf.getIntVar(daemonConf, ConfVars.LLAP_DAEMON_METRICS_SIMPLE_AVERAGE_DATA_POINTS);

        Preconditions.checkArgument(timedWindowAverageDataPoints >= 0, "hive.llap.daemon.metrics.timed.window.average.data.points should be greater or equal to 0");
        Preconditions.checkArgument(timedWindowAverageDataPoints == 0 || timedWindowAverageWindowLength > 0, "hive.llap.daemon.metrics.timed.window.average.window.length should be greater than 0 if " + "hive.llap.daemon.metrics.average.timed.window.data.points is set fo greater than 0");
        Preconditions.checkArgument(simpleAverageWindowDataSize >= 0, "hive.llap.daemon.metrics.simple.average.data.points should be greater or equal to 0");

        if (ioEnabled)
        {
            int numThreads = HiveConf.getIntVar(daemonConf, HiveConf.ConfVars.LLAP_IO_THREADPOOL_SIZE);
            Preconditions.checkArgument(numThreads >= numExecutors, "hive.llap.io.threadpool.size (%s) should be greater or equal to hive.llap.daemon.num.executors (%s)", numThreads, numExecutors);
        }
        ...
        Preconditions.checkState(maxJvmMemory >= memRequired, "Invalid configuration. Xmx value too small. maxAvailable=" + LlapUtil.humanReadableByteCount(maxJvmMemory) + ", configured(exec + io if enabled)=" + LlapUtil.humanReadableByteCount(memRequired));
        ...
        if (strIntervals != null)
        {
            for (String strInterval : strIntervals)
            {
                ...
                {
                    LOG.warn("Ignoring task pre-emption metrics interval {} from {} as it is invalid", strInterval, Arrays.toString(strIntervals));
                }
            }
        }
        ...
        this.amReporter = new AMReporter(numExecutors, maxAmReporterThreads, srvAddress, new QueryFailedHandlerProxy(), daemonConf, daemonId, socketFactory);
        ...
        this.server = new LlapProtocolServerImpl(secretManager, numHandlers, this, srvAddress, mngAddress, srvPort, externalClientsRpcPort, mngPort, daemonId, metrics).withTokenManager(this.llapTokenManager);
        ...
        QueryTracker queryTracker = new QueryTracker(daemonConf, localDirs, daemonId.getClusterString());

        String waitQueueSchedulerClassName = HiveConf.getVar(daemonConf, ConfVars.LLAP_DAEMON_WAIT_QUEUE_COMPARATOR_CLASS_NAME);

        Scheduler<TaskRunnerCallable> executorService = new TaskExecutorService(numExecutors, waitQueueSize, waitQueueSchedulerClassName, enablePreemption, executorClassLoader, metrics, null);

        addIfService(queryTracker);
        addIfService(executorService);

        this.containerRunner = new ContainerRunnerImpl(daemonConf, numExecutors, this.shufflePort, srvAddress, executorMemoryPerInstance, metrics, amReporter, queryTracker, executorService, daemonId, fsUgiFactory, socketFactory);
        ...
        if (HiveConf.getBoolVar(daemonConf, HiveConf.ConfVars.HIVE_IN_TEST) && Integer.parseInt(ConfVars.LLAP_DAEMON_WEB_PORT.getDefaultValue()) == webPort)
        {
            LOG.info("Web UI was disabled in test mode because hive.llap.daemon.web.port was not " + "specified or has default value ({})", webPort);
    ...
    private static long determineXmxHeadroom(Configuration daemonConf, long executorMemoryBytes, long maxJvmMemory)
    {
        ...
        try
        {
            if (headroomStr.endsWith("%"))
            {
                ...
                Preconditions.checkState(percentage >= 0 && percentage < 100, "Headroom percentage should be in [0, 100) range; found " + headroomStr);
        ...
        Preconditions.checkArgument(xmxHeadRoomBytes < executorMemoryBytes, "LLAP daemon headroom size should be less than daemon max memory size. headRoomBytes: " + xmxHeadRoomBytes + " executorMemoryBytes: " + executorMemoryBytes + " (derived from " + headroomStr + " out of xmx of " + maxJvmMemory + ")");
    ...
    private static void initializeLogging(final Configuration conf)
    {
        ...
        if (llap_l4j2 != null)
        {
            ...
            LOG.debug("LLAP daemon logging initialized from {} in {} ms. Async: {}", llap_l4j2, (end - start), async);
        } else
        {
    ...
    public void serviceStart() throws Exception
    {
        ...
        if (LlapUtil.isCloudDeployment(getConfig()))
        {
            ...
            getConfig().setInt(ConfVars.LLAP_EXTERNAL_CLIENT_CLOUD_RPC_PORT.varname, server.getExternalClientsRpcServerBindAddress().getPort());
        }

        // Ensure this is set in the config so that the AM can read it.
        getConfig().setIfUnset(ConfVars.LLAP_DAEMON_TASK_SCHEDULER_WAIT_QUEUE_SIZE.varname, ConfVars.LLAP_DAEMON_TASK_SCHEDULER_WAIT_QUEUE_SIZE.getDefaultValue());
    ...
    @Override
    public QueryCompleteResponseProto queryComplete(QueryCompleteRequestProto request) throws IOException
    {
        return containerRunner.queryComplete(request);
    }

    @Override
    public TerminateFragmentResponseProto terminateFragment(TerminateFragmentRequestProto request) throws IOException
    {
        return containerRunner.terminateFragment(request);
    }

    @Override
    public UpdateFragmentResponseProto updateFragment(UpdateFragmentRequestProto request) throws IOException
    {
        return containerRunner.updateFragment(request);
    }

    @Override
    public SetCapacityResponseProto setCapacity(SetCapacityRequestProto request) throws IOException
    {

        Map<String, String> capacityValues = new HashMap<>(2);
        capacityValues.put(LlapRegistryService.LLAP_DAEMON_NUM_ENABLED_EXECUTORS, Integer.toString(request.getExecutorNum()));
        capacityValues.put(LlapRegistryService.LLAP_DAEMON_TASK_SCHEDULER_ENABLED_WAIT_QUEUE_SIZE, Integer.toString(request.getQueueSize()));
    ...
    private static final class LlapGlobalUdfChecker extends SerializationUtilities.Hook
    {
        ...
        public boolean preRead(Class<?> type)
        {
            ...
            {
                return true; // Run post-hook.
            }
            if (!(GenericUDF.class.isAssignableFrom(type) || UDF.class.isAssignableFrom(type)) || fnCheckerImpl.isUdfAllowed(type))
            {
                return false;
            }
