...
class LlapRecordReader implements RecordReader<NullWritable, VectorizedRowBatch>, Consumer<ColumnVectorBatch>
{
    ...
    public static LlapRecordReader create(JobConf job, FileSplit split, List<Integer> tableIncludedCols, String hostName, ColumnVectorProducer cvp, ExecutorService executor, InputFormat<?, ?> sourceInputFormat, Deserializer sourceSerDe, Reporter reporter, Configuration daemonConf) throws IOException, HiveException
    {
        ...
        if (mapWork == null)
        {
            return null; // No compatible MapWork.
        }
    ...
    private LlapRecordReader(MapWork mapWork, JobConf job, FileSplit split, List<Integer> tableIncludedCols, String hostName, ColumnVectorProducer cvp, ExecutorService executor, InputFormat<?, ?> sourceInputFormat, Deserializer sourceSerDe, Reporter reporter, Configuration daemonConf) throws IOException, HiveException
    {
        ...
        TypeDescription schema = orcSchemaOverrideString == null ? OrcInputFormat.getDesiredRowTypeDescr(job, isAcidScan, Integer.MAX_VALUE) : TypeDescription.fromString(orcSchemaOverrideString);
        ...
        final boolean decimal64Support = HiveConf.getVar(job, ConfVars.HIVE_VECTORIZED_INPUT_FORMAT_SUPPORTS_ENABLED).equalsIgnoreCase("decimal_64");
        int limit = determineQueueLimit(bestEffortSize, queueLimitBase, queueLimitMin, rbCtx.getRowColumnTypeInfos(), rbCtx.getDataColumnNums(), decimal64Support);
        ...
        if (isAcidScan)
        {
            ...
            this.acidReader = new VectorizedOrcAcidRowBatchReader(orcSplit, jobConf, Reporter.NULL, null, rbCtx, true, mapWork);
        ...
        this.includes = new IncludesImpl(tableIncludedCols, isAcidFormat, rbCtx, schema, job, isAcidScan && acidReader.includeAcidColumns());
        ...
        feedback = rp = cvp.createReadPipeline(this, split, includes, sarg, counters, includes, sourceInputFormat, sourceSerDe, reporter, job, mapWork.getPathToPartitionInfo());
    }
    ...
    private static final int COL_WEIGHT_COMPLEX = 16, COL_WEIGHT_HIVEDECIMAL = 10, COL_WEIGHT_STRING = 8;

    @VisibleForTesting
    static int determineQueueLimit(long maxBufferedSize, int queueLimitMax, int queueLimitMin, TypeInfo[] typeInfos, int[] projectedColumnNums, final boolean decimal64Support)
    {
    ...
    private static final class AcidWrapper implements RecordReader<NullWritable, VectorizedRowBatch>
    {
    ...
    public void close() throws IOException
    {
        if (LlapIoImpl.LOG.isTraceEnabled())
        {
            LlapIoImpl.LOG.trace("close called; closed {}, interrupted {}, err {}, pending {}", isClosed, isInterrupted, pendingError.get(), queue.size());
        }
    ...
    public void setDone() throws InterruptedException
    {
        ...
        {
            LlapIoImpl.LOG.debug("setDone called; closed {}, interrupted {}, err {}, pending {}", isClosed, isInterrupted, pendingError.get(), queue.size());
        }
    ...
    public void consumeData(ColumnVectorBatch data) throws InterruptedException
    {
        if (LlapIoImpl.LOG.isTraceEnabled())
        {
            LlapIoImpl.LOG.trace("consume called; closed {}, interrupted {}, err {}, pending {}", isClosed, isInterrupted, pendingError.get(), queue.size());
        }
    ...
    public void setError(Throwable t) throws InterruptedException
    {
        ...
        LlapIoImpl.LOG.debug("setError called; closed {}, interrupted {},  err {}, pending {}", isClosed, isInterrupted, pendingError.get(), queue.size());
    ...
    private void enqueueInternal(Object o) throws InterruptedException
    {
        ...
        do
        {
        } while (!isClosed && !isInterrupted && !queue.offer(o, 100, TimeUnit.MILLISECONDS));
    }
    ...
    private static class IncludesImpl implements SchemaEvolutionFactory, Includes
    {
        ...
        {
            return "logical columns " + readerLogicalColumnIds + ", physical columns " + filePhysicalColumnIds;
        }
        ...
        public SchemaEvolution createSchemaEvolution(TypeDescription fileSchema)
        {
            ...
            boolean[] readerIncludes = OrcInputFormat.genIncludedColumns(readerSchema, readerLogicalColumnIds);
            Reader.Options options = new Reader.Options(jobConf).include(readerIncludes).includeAcidColumns(includeAcidColumns);
            evolution = new SchemaEvolution(fileSchema, readerSchema, options);

            if (LOG.isDebugEnabled())
            {
                LOG.debug("Generated ORC schema evolution. Reader schema: {}, Reader included: {}, File schema: {}, File " + "included: {}", evolution.getReaderSchema(), evolution.getReaderIncluded(), evolution.getFileSchema(), evolution.getFileIncluded());
            }
        ...
        private void generateLogicalOrderedColumnIds()
        {
            ...
            LOG.debug("Logical ordered column IDs generated. Result: {}, fileSchemaMap: {}, readSchemaMap: {}", logicalOrderedColumnIds, fileSchemaMap, readSchemaMap);
        }
        ...
        {
            return OrcInputFormat.genIncludedTypes(fileSchema, filePhysicalColumnIds, acidStructColumnId);
        }

        @Override
        public String[] getOriginalColumnNames(TypeDescription fileSchema)
        {
            return OrcInputFormat.genIncludedColNames(fileSchema, filePhysicalColumnIds, acidStructColumnId);
        }
