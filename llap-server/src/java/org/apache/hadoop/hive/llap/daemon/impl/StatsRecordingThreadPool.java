...
public class StatsRecordingThreadPool extends ThreadPoolExecutor
{
    ...
    public StatsRecordingThreadPool(final int corePoolSize, final int maximumPoolSize, final long keepAliveTime, final TimeUnit unit, final BlockingQueue<Runnable> workQueue, final ThreadFactory threadFactory)
    {
        this(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue, threadFactory, null);
    }

    public StatsRecordingThreadPool(final int corePoolSize, final int maximumPoolSize, final long keepAliveTime, final TimeUnit unit, final BlockingQueue<Runnable> workQueue, final ThreadFactory threadFactory, Thread.UncaughtExceptionHandler handler)
    {
    ...
    private static class WrappedCallable<V> implements Callable<V>
    {
        ...
        WrappedCallable(final Callable<V> callable, final Thread.UncaughtExceptionHandler uncaughtExceptionHandler, ThreadMXBean mxBean)
        {
        ...
        public V call() throws Exception
        {
            ...
            long cpuTime = mxBean == null ? -1 : mxBean.getCurrentThreadCpuTime(), userTime = mxBean == null ? -1 : mxBean.getCurrentThreadUserTime();
        ...
        {
            EXECUTOR_CPU_NS, EXECUTOR_USER_NS;

        }

        private void updateCounters(final List<LlapUtil.StatisticsData> statsBefore, final Callable<V> actualCallable, long cpuTime, long userTime)
        {
            Thread thread = Thread.currentThread();
