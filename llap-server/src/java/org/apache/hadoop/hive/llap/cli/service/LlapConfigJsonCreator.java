...
class LlapConfigJsonCreator
{
    ...
    private JSONObject createConfigJson() throws JSONException
    {
        ...
        configs.put(ConfVars.LLAP_DAEMON_YARN_CONTAINER_MB.varname, conf.getLongVar(ConfVars.LLAP_DAEMON_YARN_CONTAINER_MB));
        ...
        configs.put(ConfVars.LLAP_DAEMON_MEMORY_PER_INSTANCE_MB.varname, conf.getIntVar(ConfVars.LLAP_DAEMON_MEMORY_PER_INSTANCE_MB));

        configs.put(ConfVars.LLAP_DAEMON_VCPUS_PER_INSTANCE.varname, conf.getIntVar(ConfVars.LLAP_DAEMON_VCPUS_PER_INSTANCE));
        ...
        configs.put(ConfVars.LLAP_HDFS_PACKAGE_DIR.varname, conf.getVar(ConfVars.LLAP_HDFS_PACKAGE_DIR));
    ...
    {
        try (FSDataOutputStream fsdos = fs.create(new Path(tmpDir, "config.json")); OutputStreamWriter w = new OutputStreamWriter(fsdos, Charset.defaultCharset()))
        {
            configs.write(w);
        }
    }
