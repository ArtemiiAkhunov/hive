...
public class FadvisedChunkedFile extends ChunkedFile
{
    ...
    public ByteBuf readChunk(ChannelHandlerContext ctx) throws Exception
    {
        ...
        {
            readaheadRequest = readaheadPool.readaheadStream(identifier, fd, currentOffset(), readaheadLength, endOffset(), readaheadRequest);
        }
    ...
    public void close() throws Exception
    {
        ...
        if (manageOsCache && endOffset() - startOffset() > 0)
        {
            try
            {
                NativeIO.POSIX.getCacheManipulator().posixFadviseIfPossible(identifier, fd, startOffset(), endOffset() - startOffset(), NativeIO.POSIX.POSIX_FADV_DONTNEED);
            } catch (Throwable t)
            {
