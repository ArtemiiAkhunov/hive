...
public class FadvisedFileRegion extends DefaultFileRegion
{
    ...
    public FadvisedFileRegion(RandomAccessFile file, long position, long count, boolean manageOsCache, int readaheadLength, ReadaheadPool readaheadPool, String identifier, int shuffleBufferSize, boolean shuffleTransferToAllowed, boolean canEvictAfterTransfer) throws IOException
    {
    ...
    public long transferTo(WritableByteChannel target, long position) throws IOException
    {
        ...
        {
            readaheadRequest = readaheadPool.readaheadStream(identifier, fd, position() + position, readaheadLength, position() + count(), readaheadRequest);
        }
        long written = 0;
    ...
    public void transferSuccessful()
    {
        if (manageOsCache && count() > 0)
        {
            try
            {
                ...
                {
                    NativeIO.POSIX.getCacheManipulator().posixFadviseIfPossible(identifier, fd, position(), count(), NativeIO.POSIX.POSIX_FADV_DONTNEED);
                }
            } catch (Throwable t)
            {
