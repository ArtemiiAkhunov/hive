...
public abstract class EncodedDataConsumer<BatchKey, BatchType extends EncodedColumnBatch<BatchKey>> implements Consumer<BatchType>, ReadPipeline
{
    ...
    private class CpuRecordingCallable implements Callable<Void>, TezCounterSource
    {
        ...
        public Void call() throws Exception
        {
            ...
            long cpuTime = mxBean.getCurrentThreadCpuTime(), userTime = mxBean.getCurrentThreadUserTime();
            try
            {
                return readCallable.call();
            } finally
            {
                counters.recordThreadTimes(mxBean.getCurrentThreadCpuTime() - cpuTime, mxBean.getCurrentThreadUserTime() - userTime);
            }
        }
        ...
        {
            return (readCallable instanceof TezCounterSource) ? ((TezCounterSource) readCallable).getTezCounters() : null;
        }
    ...
    protected abstract void decodeBatch(BatchType batch, Consumer<ColumnVectorBatch> downstreamConsumer) throws InterruptedException;
