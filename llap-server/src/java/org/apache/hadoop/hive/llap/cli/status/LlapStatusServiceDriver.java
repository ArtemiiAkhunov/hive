...
public class LlapStatusServiceDriver
{
    ...
    private static final EnumSet<State> NO_YARN_SERVICE_INFO_STATES = EnumSet.of(State.APP_NOT_FOUND, State.COMPLETE, State.LAUNCHING);
    private static final EnumSet<State> LAUNCHING_STATES = EnumSet.of(State.LAUNCHING, State.RUNNING_PARTIAL, State.RUNNING_ALL);
    ...
    private static final String CONFIG_IPC_CLIENT_CONNECT_RETRY_INTERVAL_MS = CONF_PREFIX + "ipc.client.connect.retry-interval-ms";
    ...
    private static final String CONFIG_TIMELINE_SERVICE_ENTITYGROUP_FS_STORE_RETRY_POLICY_SPEC = CONF_PREFIX + "timeline.service.fs-store.retry.policy.spec";
    ...
    private ApplicationReport getAppReport(String appName, long timeoutMs) throws LlapStatusCliException
    {
     ...
     * @throws LlapStatusCliException
     */
    private ExitCode populateAppStatusFromServiceStatus(String appName, ServiceClient serviceClient, AppStatusBuilder appStatusBuilder) throws LlapStatusCliException
    {
    ...
    public static void main(String[] args)
    {
        ...
        try (OutputStream os = cl.getOutputFile() == null ? System.out : new FileOutputStream(cl.getOutputFile()); Writer w = new OutputStreamWriter(os, Charset.defaultCharset()); PrintWriter pw = new PrintWriter(w))
        {

            LOG.info("Configured refresh interval: {}s. Watch timeout: {}s. Attempts remaining: {}." + " Watch mode: {}. Running nodes threshold: {}.", refreshInterval / 1000, watchTimeout / 1000, numAttempts, watchMode, new DecimalFormat("#.###").format(runningNodesThreshold));
            while (numAttempts > 0)
            {
                if (!firstAttempt)
                {
    ...
    private static long maybeLogSummary(Clock clock, long lastSummaryLogTime, LlapStatusServiceDriver statusServiceDriver, boolean watchMode, long watchTimeout, State launchingState)
    {
