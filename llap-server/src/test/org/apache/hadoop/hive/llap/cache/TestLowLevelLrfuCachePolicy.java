...
public class TestLowLevelLrfuCachePolicy
{
    ...
    public void testPurge()
    {
        ...
        LowLevelCacheMemoryManager mm = new LowLevelCacheMemoryManager(HEAP_SIZE, lrfu, m.metricsMock);
        lrfu.setEvictionListener(et);
        assertEquals(0, lrfu.purge());
        for (int testSize = 1; testSize <= HEAP_SIZE; ++testSize)
        {
            LOG.info("Starting with " + testSize);
            ArrayList<LlapDataBuffer> purge = new ArrayList<LlapDataBuffer>(testSize), dontPurge = new ArrayList<LlapDataBuffer>(testSize);
    ...
    public void testHotBuffers()
    {
        ...
        LlapDataBuffer[] buffs = IntStream.range(0, buffers).mapToObj(i -> LowLevelCacheImpl.allocateFake()).toArray(LlapDataBuffer[]::new);
    ...
    private void testProactiveEviction(float lambda, boolean isInstantDealloc) throws Exception
    {
        ...
        LowLevelCacheMemoryManager mm = new LowLevelCacheMemoryManager(lrfuMaxSize, lfu, LlapDaemonCacheMetrics.create("test", "1"));
        lfu.setEvictionListener(et);
        et.mm = mm;

        // 10 buffers, go into the cache policy
        LlapDataBuffer[] buffs = IntStream.range(0, lrfuMaxSize).mapToObj(i -> LowLevelCacheImpl.allocateFake()).toArray(LlapDataBuffer[]::new);
