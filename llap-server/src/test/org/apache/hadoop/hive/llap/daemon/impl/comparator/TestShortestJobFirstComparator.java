...
public class TestShortestJobFirstComparator
{
    ...
    public void testWaitQueueAging() throws InterruptedException
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            LlapDaemonProtocolProtos.SubmitWorkRequestProto proto = createSubmitWorkRequestProto(i, 1, 100 + i, 100 + i, "q" + i, true);
            r[i] = createTaskWrapper(proto, true, 100000);
        }

        // Make sure we dont have evictions triggered (maxSize = taskSize)
        EvictingPriorityBlockingQueue<TaskWrapper> queue = new EvictingPriorityBlockingQueue<>(new ShortestJobFirstComparator(), 50);
    ...
    {
        // Make sure we dont have evictions triggered (maxSize = taskSize)
        EvictingPriorityBlockingQueue<TaskWrapper> queue = new EvictingPriorityBlockingQueue<>(new ShortestJobFirstComparator(), 10);
