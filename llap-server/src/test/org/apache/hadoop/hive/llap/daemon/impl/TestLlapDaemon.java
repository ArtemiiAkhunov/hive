...
public class TestLlapDaemon
{
    ...
    private static final String[] METRICS_SOURCES = new String[]{"JvmMetrics", "LlapDaemonExecutorMetrics-" + MetricsUtils.getHostName(), "LlapDaemonJvmMetrics-" + MetricsUtils.getHostName(), MetricsUtils.METRICS_PROCESS_NAME};
    ...
    public void setUp()
    {
        ...
        daemon = new LlapDaemon(hiveConf, 1, LlapDaemon.getTotalHeapSize(), false, false, -1, localDirs, 0, false, 0, 0, 0, -1, "TestLlapDaemon");
    }
    ...
    public void testEnforceProperNumberOfIOThreads() throws IOException
    {
        ...
        LlapDaemon thisDaemon = new LlapDaemon(thisHiveConf, 1, LlapDaemon.getTotalHeapSize(), false, false, -1, new String[1], 0, false, 0, 0, 0, -1, "TestLlapDaemon");
        thisDaemon.close();
    }
    ...
    public void testUpdateRegistration() throws IOException
    {
        ...
        // When
        daemon.setCapacity(LlapDaemonProtocolProtos.SetCapacityRequestProto.newBuilder().setQueueSize(enabledQueue).setExecutorNum(enabledExecutors).build());
        verify(mockRegistry).updateRegistration(captor.capture());

        // Then
        Map<String, String> attributes = StreamSupport.stream(captor.getValue().spliterator(), false).collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
        ...
        assertEquals(enabledQueue, parseInt(attributes.get(LlapRegistryService.LLAP_DAEMON_TASK_SCHEDULER_ENABLED_WAIT_QUEUE_SIZE)));
        assertEquals(enabledExecutors, parseInt(attributes.get(LlapRegistryService.LLAP_DAEMON_NUM_ENABLED_EXECUTORS)));

    }

    static <T> void trySetMock(Object o, Class<T> clazz, T mock)
    {
        List<InstanceField> instanceFields = Fields.allDeclaredFieldsOf(o).filter(instanceField -> !clazz.isAssignableFrom(instanceField.jdkField().getType())).instanceFields();
