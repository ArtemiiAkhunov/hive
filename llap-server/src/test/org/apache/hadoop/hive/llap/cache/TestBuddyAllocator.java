...
public class TestBuddyAllocator
{
    ...
    public void testSameSizes() throws Exception
    {
        ...
        BuddyAllocator a = new BuddyAllocator(isDirect, isMapped, 1 << min, maxAlloc, maxAlloc, maxAlloc, 0, tmpDir, new DummyMemoryManager(), LlapDaemonCacheMetrics.create("test", "1"), null, true);
    ...
    public void testMultipleArenas() throws Exception
    {
        ...
        BuddyAllocator a = new BuddyAllocator(isDirect, isMapped, 1 << 3, maxAlloc, maxAlloc, maxAlloc * arenaCount, 0, tmpDir, new DummyMemoryManager(), LlapDaemonCacheMetrics.create("test", "1"), null, true);
    ...
    public void testMTT()
    {
        ...
        final BuddyAllocator a = new BuddyAllocator(isDirect, isMapped, 1 << min, maxAlloc, maxAlloc * 8, maxAlloc * 24, 0, tmpDir, new DummyMemoryManager(), LlapDaemonCacheMetrics.create("test", "1"), "both", true);
    ...
    public void testMTTArenas()
    {
        ...
        final BuddyAllocator a = new BuddyAllocator(isDirect, isMapped, 1 << min, maxAlloc, maxAlloc, (1 << min) * minAllocCount, 0, tmpDir, new DummyMemoryManager(), LlapDaemonCacheMetrics.create("test", "1"), null, true);
    ...
    {
        int min = 3, max = 8, maxAlloc = 1 << max;
        new BuddyAllocator(isDirect, isMapped, 1 << min, maxAlloc, maxAlloc, maxAlloc, 0, tmpDir + "/testifcreated", new DummyMemoryManager(), LlapDaemonCacheMetrics.create("test", "1"), null, false);
    }
    ...
    private void testVariableSizeInternal(int allocCount, int arenaSizeMult, int arenaCount) throws Exception
    {
        ...
        BuddyAllocator a = new BuddyAllocator(isDirect, isMapped, 1 << min, maxAlloc, arenaSize, arenaSize * arenaCount, 0, tmpDir, new DummyMemoryManager(), LlapDaemonCacheMetrics.create("test", "1"), null, true);
    ...
    {
        int pos = mem.getByteBufferRaw().position();
        assertEquals("Failed to match (" + pos + ") on " + str, testValue, mem.getByteBufferRaw().getLong(pos));
        int halfLength = mem.getByteBufferRaw().remaining() >> 1;
        if (halfLength + 8 <= mem.getByteBufferRaw().remaining())
        {
            assertEquals("Failed to match half (" + (pos + halfLength) + ") on " + str, testValue, mem.getByteBufferRaw().getLong(pos + halfLength));
        }
    }
