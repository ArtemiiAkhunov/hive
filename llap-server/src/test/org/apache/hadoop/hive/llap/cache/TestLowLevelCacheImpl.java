...
*/
    ...
    {
        LowLevelCacheImpl cache = new LowLevelCacheImpl(LlapDaemonCacheMetrics.create("test", "1"), new DummyCachePolicy(), new DummyAllocator(), true, -1); // no cleanup thread
        long fn1 = 1, fn2 = 2;
    ...
    public void testMultiMatch()
    {
        ...
        assertNull(cache.putFileData(fn, new DiskRange[]{dr(2, 4), dr(6, 8)}, fakes, 0, Priority.NORMAL, null, null));
    ...
    {
        LowLevelCacheImpl cache = new LowLevelCacheImpl(LlapDaemonCacheMetrics.create("test", "1"), new DummyCachePolicy(), new DummyAllocator(), false, -1); // no cleanup thread
        long fn = 1;
        MemoryBuffer[] fakes = new MemoryBuffer[]{fb(), fb()};
        assertNull(cache.putFileData(fn, new DiskRange[]{dr(2, 4), dr(6, 8)}, fakes, 0, Priority.NORMAL, null, null));
    ...
    public void testStaleValueReplace()
    {
        ...
        evict(cache, fakes[0]);
        evict(cache, fakes[3]);
        long[] mask = cache.putFileData(fn1, drs(1, 2, 3, 4), fbs(fakes, 4, 5, 6, 7), 0, Priority.NORMAL, null, null);
        assertEquals(1, mask.length);
    ...
    public void testCacheMetrics()
    {
        ...
        LowLevelCacheImpl cache = new LowLevelCacheImpl(metrics, new DummyCachePolicy(), new DummyAllocator(), true, -1); // no cleanup thread
        long fn = 1;
        MemoryBuffer[] fakes = new MemoryBuffer[]{fb(), fb(), fb()};
