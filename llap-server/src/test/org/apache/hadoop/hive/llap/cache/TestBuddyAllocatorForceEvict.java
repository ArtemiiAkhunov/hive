...
public class TestBuddyAllocatorForceEvict
{
    ...
    {
        runCustomDiscard(create(1024, 1, 1024, false, isBruteOnly), new int[]{256, 256, 128, 128, 128, 128}, new int[]{0, 2, 4}, 512);
        runCustomDiscard(create(1024, 1, 1024, false, isBruteOnly), new int[]{256, 256, 64, 64, 64, 64, 64, 64, 64, 64}, new int[]{0, 2, 4, 6, 8}, 512);

        runCustomDiscard(create(512, 2, 1024, false, isBruteOnly), new int[]{256, 256, 128, 128, 128, 128}, new int[]{0, 2, 4}, 512);
        runCustomDiscard(create(512, 2, 1024, false, isBruteOnly), new int[]{256, 256, 64, 64, 64, 64, 64, 64, 64, 64}, new int[]{0, 2, 4, 6, 8}, 512);
    }
    ...
    {
        runCustomDiscard(create(1024, 1, 1024, false, isBruteOnly), new int[]{256, 128, 64, 64, 256, 64, 64, 128}, new int[]{0, 3, 6, 7}, 512);
        runCustomDiscard(create(1024, 1, 1024, false, isBruteOnly), new int[]{256, 64, 64, 64, 64, 256, 64, 64, 128}, new int[]{0, 4, 7, 8}, 512);

        runCustomDiscard(create(512, 2, 1024, false, isBruteOnly), new int[]{256, 128, 64, 64, 256, 64, 64, 128}, new int[]{0, 3, 6, 7}, 512);
        runCustomDiscard(create(512, 2, 1024, false, isBruteOnly), new int[]{256, 64, 64, 64, 64, 256, 64, 64, 128}, new int[]{0, 4, 7, 8}, 512);
    }
    ...
    public void testMtt()
    {
        final int baseAllocSizeLog2 = 3, maxAllocSizeLog2 = 10, totalSize = 8192, baseAllocSize = 1 << baseAllocSizeLog2, maxAllocSize = 1 << maxAllocSizeLog2;
        ...
        @SuppressWarnings("unchecked") FutureTask<MttTestCallableResult>[] allocTasks = new FutureTask[threadCount];
        ...
        {
            allocTasks[i] = new FutureTask<>(new MttTestCallable(cdlIn, cdlOut, a, allocSize, totalSize / allocSize, iterCount));
    ...
    private static void checkInitialValues(LlapAllocatorBuffer[] bufs, int... indexes)
    {
        for (int index : indexes)
        {
            ...
            if (!incRefIfNotEvicted(buf, false))
            {
                continue;
            }
            try
    ...
    {
        if (!incRefIfNotEvicted(mem, mustExist))
        {
            return;
        }
        try
    ...
    {
        BuddyAllocator result = new BuddyAllocator(false, false, 8, max, arenas, total, 0, null, MM, METRICS, isBruteForceOnly ? "brute" : null, true);
        if (!isShortcut)
        {
