...
public final class RetryUtils
{
     ...
     */
    @SuppressWarnings("WeakerAccess")
    static <T> T retry(final Task<T> f, final Predicate<Throwable> shouldRetry, final int quietTries, final int maxTries, @Nullable final CleanupAfterFailure cleanupAfterFailure, @Nullable final String messageOnRetry) throws Exception
    {
    ...
    static <T> T retry(final Task<T> f, final Predicate<Throwable> shouldRetry, final int quietTries, final int maxTries) throws Exception
    {
        return retry(f, shouldRetry, quietTries, maxTries, null, null);
    }

    @SuppressWarnings("unused")
    public static <T> T retry(final Task<T> f, final Predicate<Throwable> shouldRetry, final CleanupAfterFailure onEachFailure, final int maxTries, final String messageOnRetry) throws Exception
    {
        return retry(f, shouldRetry, 0, maxTries, onEachFailure, messageOnRetry);
    }

    private static void awaitNextRetry(final Throwable e, @Nullable final String messageOnRetry, final int nTry, final int maxRetries, final boolean quiet) throws InterruptedException
    {
