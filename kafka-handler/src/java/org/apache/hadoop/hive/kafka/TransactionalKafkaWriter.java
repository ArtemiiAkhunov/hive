...
class TransactionalKafkaWriter implements FileSinkOperator.RecordWriter, RecordWriter<BytesWritable, KafkaWritable>
{
    ...
    TransactionalKafkaWriter(String topic, Properties producerProperties, Path queryWorkingPath, FileSystem fileSystem, @Nullable Boolean optimisticCommit)
    {
        ...
        Preconditions.checkState(producerProperties.getProperty(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG) != null, "set [" + ProducerConfig.BOOTSTRAP_SERVERS_CONFIG + "] property");
        ...
        } catch (Exception exception)
        {
            ...
            {
                LOG.error("Aborting Transaction [{}] cause by ERROR [{}]", producer.getTransactionalId(), exception.getMessage());
        ...
        openTxFileName = this.optimisticCommit ? null : new Path(new Path(new Path(queryWorkingPath, TRANSACTION_DIR), producer.getTransactionalId()), String.valueOf(producerEpoch));
    }
    ...
    public void close(boolean abort) throws IOException
    {
        ...
        String flushedOffsetMsg = offsets.entrySet().stream().map(topicPartitionLongEntry -> String.format(formattedMsg, topicPartitionLongEntry.getKey().topic(), topicPartitionLongEntry.getKey().partition(), topicPartitionLongEntry.getValue())).collect(Collectors.joining(","));
        ...
        checkExceptions();
        LOG.info("Closed writerId [{}], Sent [{}] records to Topic [{}]", producer.getTransactionalId(), sentRecords, topic);
    ...
    static Map<String, Pair<Long, Short>> getTransactionsState(FileSystem fs, Path queryWorkingDir) throws IOException
    {
        ...
        final Set<FileStatus> transactionSet = Arrays.stream(files).filter(FileStatus::isDirectory).collect(Collectors.toSet());
