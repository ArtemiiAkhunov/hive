...
public class KafkaInputFormat extends InputFormat<NullWritable, KafkaWritable> implements org.apache.hadoop.mapred.InputFormat<NullWritable, KafkaWritable>, VectorizedInputFormatInterface
{
    ...
    private static List<KafkaInputSplit> buildFullScanFromKafka(String topic, KafkaConsumer<byte[], byte[]> consumer, Path[] tablePaths, int maxTries)
    {
        ...
        {
            LOG.info("Found the following partitions [{}]", topicPartitions.stream().map(TopicPartition::toString).collect(Collectors.joining(",")));
        ...
        return topicPartitions.stream().map(topicPartition -> new KafkaInputSplit(topicPartition.topic(), topicPartition.partition(), starOffsetsMap.get(topicPartition), endOffsetsMap.get(topicPartition), tablePaths[0])).collect(Collectors.toList());
    }
    ...
    private List<KafkaInputSplit> computeSplits(Configuration configuration) throws IOException, InterruptedException
    {
        ...
        try (KafkaConsumer consumer = new KafkaConsumer(KafkaUtils.consumerProperties(configuration)))
        {
            ...
            if (filterExprSerialized != null && !filterExprSerialized.isEmpty())
            {
                ...
                try
                {
                    return futureTinyHouse.get(timeoutMs, TimeUnit.MILLISECONDS).stream()
                            // filter out empty splits
                            .filter(split -> split.getStartOffset() < split.getEndOffset()).collect(Collectors.toList());
    ...
    {
        return computeSplits(jobContext.getConfiguration()).stream().map(kafkaPullerInputSplit -> (org.apache.hadoop.mapreduce.InputSplit) kafkaPullerInputSplit).collect(Collectors.toList());
    }
