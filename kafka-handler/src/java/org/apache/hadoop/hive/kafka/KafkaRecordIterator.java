...
class KafkaRecordIterator implements Iterator<ConsumerRecord<byte[], byte[]>>
{
    ...
    KafkaRecordIterator(Consumer<byte[], byte[]> consumer, TopicPartition topicPartition, @Nullable Long requestedStartOffset, @Nullable Long requestedEndOffset, long pollTimeoutMs)
    {
        ...
        if (requestedStartOffset != null)
        {
            ...
            {
                LOG.warn("Current Start Offset [{}] is different form the requested start position [{}]", this.startOffset, requestedStartOffset);
            }
        } else
        {
            ...
            this.startOffset = consumer.position(topicPartition);
            LOG.info("Consumer at beginning of topic partition [{}], current start offset [{}]", topicPartition, this.startOffset);
        }

        consumerPosition = consumer.position(topicPartition);
        Preconditions.checkState(this.endOffset >= consumerPosition, "End offset [%s] need to be greater or equal than start offset [%s]", this.endOffset, consumerPosition);
        LOG.info("Kafka Iterator assigned to TopicPartition [{}]; start Offset [{}]; end Offset [{}]", topicPartition, consumerPosition, this.endOffset);
    ...
    */
    ...
    private void pollRecords()
    {
        ...
        {
            throw new PollTimeoutException(String.format(ERROR_POLL_TIMEOUT_FORMAT, pollTimeoutMs, topicPartition.toString(), startOffset, consumer.position(topicPartition), endOffset));
        }
