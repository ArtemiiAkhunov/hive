...
public class KafkaSerDe extends AbstractSerDe
{
    ...
    public void initialize(Configuration configuration, Properties tableProperties, Properties partitionProperties) throws SerDeException
    {
        ...
        final String className = properties.getProperty(KafkaTableProperties.SERDE_CLASS_NAME.getName(), KafkaTableProperties.SERDE_CLASS_NAME.getDefaultValue());
        ...
        {
            throw new SerDeException("Was expecting Struct Object Inspector but have " + delegateSerDe.getObjectInspector().getClass().getName());
        }
        delegateDeserializerOI = (StructObjectInspector) delegateSerDe.getObjectInspector();

        // Build column names Order matters here
        columnNames.addAll(delegateDeserializerOI.getAllStructFieldRefs().stream().map(StructField::getFieldName).collect(Collectors.toList()));
        ...
        inspectors.addAll(delegateDeserializerOI.getAllStructFieldRefs().stream().map(StructField::getFieldObjectInspector).collect(Collectors.toList()));
    ...
    public Writable serialize(Object obj, ObjectInspector objInspector) throws SerDeException
    {
        ...
        final byte[] keyBytes = key == null ? null : PrimitiveObjectInspectorFactory.writableBinaryObjectInspector.getPrimitiveJavaObject(key);
        final long recordTs = timestamp == null ? -1 : PrimitiveObjectInspectorUtils.getLong(timestamp, MetadataColumn.TIMESTAMP.getObjectInspector());
        final int recordPartition = partition == null ? -1 : PrimitiveObjectInspectorUtils.getInt(partition, MetadataColumn.PARTITION.getObjectInspector());

        //noinspection unchecked
        return new KafkaWritable(recordPartition, recordTs, bytesConverter.getBytes(delegateSerDe.serialize(row, delegateSerializerOI)), keyBytes);
    }
