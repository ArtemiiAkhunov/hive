...
final class KafkaUtils
{
    ...
    private static final String JAAS_TEMPLATE_SCRAM = "org.apache.kafka.common.security.scram.ScramLoginModule required " + "username=\"%s\" password=\"%s\" serviceName=\"%s\" tokenauth=true;";
    ...
    static Properties producerProperties(Configuration configuration)
    {
        ...
        properties.setProperty(CommonClientConfigs.CLIENT_ID_CONFIG, taskId == null ? "random_" + UUID.randomUUID().toString() : taskId);
    ...
    {
        return new ProducerRecord<>(topic, value.getPartition() != -1 ? value.getPartition() : null, value.getTimestamp() != -1L ? value.getTimestamp() : null, value.getRecordKey(), value.getValue());
    }
    ...
    {
        String[] securityProtocolConfigs = new String[]{CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, CONSUMER_CONFIGURATION_PREFIX + "." + CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, PRODUCER_CONFIGURATION_PREFIX + "." + CommonClientConfigs.SECURITY_PROTOCOL_CONFIG};
