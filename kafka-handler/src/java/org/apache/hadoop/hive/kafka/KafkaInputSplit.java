...
public class KafkaInputSplit extends FileSplit implements org.apache.hadoop.mapred.InputSplit
{
    ...
    public KafkaInputSplit(String topic, int partition, long startOffset, long endOffset, Path dummyPath)
    {
        ...
        Preconditions.checkArgument(startOffset >= 0 && startOffset <= endOffset, "start [%s] has to be positive and >= end [%]", startOffset, endOffset);
    }
    ...
    public boolean equals(Object o)
    {
        ...
        return Objects.equal(getTopic(), that.getTopic()) && Objects.equal(getStartOffset(), that.getStartOffset()) && Objects.equal(getPartition(), that.getPartition()) && Objects.equal(getEndOffset(), that.getEndOffset());
    }
    ...
    {
        return "KafkaInputSplit{" + "topic='" + topic + '\'' + ", startOffset=" + startOffset + ", partition=" + partition + ", endOffset=" + endOffset + ", path=" + super.getPath().toString() + '}';
    }
    ...
    public static List<KafkaInputSplit> slice(long sliceSize, final KafkaInputSplit split)
    {
        if (split.getEndOffset() - split.getStartOffset() > sliceSize)
        {
            ...
            {
                builder.add(new KafkaInputSplit(split.topic, split.partition, start, start + sliceSize + 1, split.getPath()));
                start += sliceSize + 1;
            }
            // last split
            if (start < split.getEndOffset())
            {
                builder.add(new KafkaInputSplit(split.topic, split.partition, start, split.getEndOffset(), split.getPath()));
            }
