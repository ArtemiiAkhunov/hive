...
public class KafkaStorageHandlerInfoTest
{
    ...
    private static final String RESULT = "Partition(topic = TEST1, partition = 0, leader = 0, replicas = [0], isr = [0]," + " offlineReplicas = []) [start offset = [0], end offset = [" + RECORD_NUMBER + "]]";
    ...
    private static void sendData()
    {
        ...
        RECORDS.stream().map(consumerRecord -> new ProducerRecord<>(consumerRecord.topic(), consumerRecord.partition(), consumerRecord.timestamp(), consumerRecord.key(), consumerRecord.value())).forEach(producer::send);
        producer.close();
    }
}
