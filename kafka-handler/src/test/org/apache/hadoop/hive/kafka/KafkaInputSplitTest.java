...
public class KafkaInputSplitTest
{
    ...
    public void andRangeOverLapping()
    {
        ...
        Assert.assertEquals(new KafkaInputSplit("test-topic", 2, 10, 200, new Path("/tmp")), KafkaInputSplit.intersectRange(kafkaInputSplit, kafkaInputSplit2));
    ...
    public void andRangeNonOverLapping()
    {
        ...
        KafkaInputSplit kafkaInputSplit2 = new KafkaInputSplit("test-topic", 2, 550, 700, new Path("/tmp"));
    ...
    public void orRange()
    {
        KafkaInputSplit kafkaInputSplit = new KafkaInputSplit("test-topic", 2, 300, 400, new Path("/tmp"));

        KafkaInputSplit kafkaInputSplit2 = new KafkaInputSplit("test-topic", 2, 3, 600, new Path("/tmp"));

        Assert.assertEquals(kafkaInputSplit2, KafkaInputSplit.unionRange(kafkaInputSplit, kafkaInputSplit2));

        KafkaInputSplit kafkaInputSplit3 = new KafkaInputSplit("test-topic", 2, 700, 6000, new Path("/tmp"));

        Assert.assertEquals(new KafkaInputSplit("test-topic", 2, 300, 6000, new Path("/tmp")), KafkaInputSplit.unionRange(kafkaInputSplit, kafkaInputSplit3));
    }
    ...
    public void testSlice()
    {
        ...
        Assert.assertEquals(kafkaInputSplitList.stream().mapToLong(kafkaPullerInputSplit1 -> kafkaPullerInputSplit1.getEndOffset() - kafkaPullerInputSplit1.getStartOffset()).sum(), kafkaInputSplit.getEndOffset() - kafkaInputSplit.getStartOffset());
        Assert.assertEquals(1, kafkaInputSplitList.stream().filter(kafkaPullerInputSplit1 -> kafkaInputSplit.getStartOffset() == kafkaPullerInputSplit1.getStartOffset()).count());
        Assert.assertEquals(1, kafkaInputSplitList.stream().filter(kafkaPullerInputSplit1 -> kafkaInputSplit.getEndOffset() == kafkaPullerInputSplit1.getEndOffset()).count());
