...
public class BlacklistingLlapMetricsListener implements LlapMetricsListener
{
    ...
    void init(Configuration conf, LlapRegistryService registry, LlapManagementProtocolClientImplFactory clientFactory)
    {
        ...
        this.minServedTasksNumber = HiveConf.getIntVar(conf, HiveConf.ConfVars.LLAP_NODEHEALTHCHECKS_MINTASKS);
        this.minConfigChangeDelayMs = HiveConf.getTimeVar(conf, HiveConf.ConfVars.LLAP_NODEHEALTHCHECKS_MININTERVALDURATION, TimeUnit.MILLISECONDS);
        this.timeThreshold = HiveConf.getFloatVar(conf, HiveConf.ConfVars.LLAP_NODEHEALTHCHECKS_TASKTIMERATIO);
        this.emptyExecutorsThreshold = HiveConf.getFloatVar(conf, HiveConf.ConfVars.LLAP_NODEHEALTHCHECKS_EXECUTORRATIO);
        this.maxBlacklistedNodes = HiveConf.getIntVar(conf, HiveConf.ConfVars.LLAP_NODEHEALTHCHECKS_MAXNODES);

        Preconditions.checkArgument(minServedTasksNumber > 0, "Minimum served tasks should be greater than 0");
        Preconditions.checkArgument(minConfigChangeDelayMs > 0, "Minimum config change delay should be greater than 0");
        Preconditions.checkArgument(timeThreshold > 1.0f, "The time threshold should be greater than 1");
        Preconditions.checkArgument(maxBlacklistedNodes > 0, "The maximum number of blacklisted node should be greater than 1");
        ...
        LOG.info("BlacklistingLlapMetricsListener initialized with " + "minServedTasksNumber={}, " + "minConfigChangeDelayMs={}, " + "timeThreshold={}, " + "emptyExecutorsThreshold={}, " + "maxBlacklistedNodes={}", minServedTasksNumber, minConfigChangeDelayMs, timeThreshold, emptyExecutorsThreshold, maxBlacklistedNodes);
    }
    ...
    public void newClusterMetrics(Map<String, LlapMetricsCollector.LlapMetrics> newMetrics)
    {
        ...
        for (String workerIdentity : newMetrics.keySet())
        {
            ...
            LOG.debug("Checking node {} with data: " + "requestHandled={}, " + "averageTime={}, " + "emptyExecutors={}, " + "maxExecutors={}", workerIdentity, requestHandled, averageTime, emptyExecutor, maxExecutors);
