...
public class SourceStateTracker
{
    ...
    private void maybeRegisterForVertexUpdates(String sourceName)
    {
        if (!notificationRegisteredVertices.contains(sourceName))
        {
            ...
            taskCommunicatorContext.registerForVertexStateUpdates(sourceName, EnumSet.of(VertexState.RUNNING, VertexState.SUCCEEDED));
        }
    }
    ...
    private int getVertexCompletedTaskCount(String vname)
    {
        ...
        try
        {
            completedTaskCount = taskCommunicatorContext.getVertexCompletedTaskCount(vname);
            return completedTaskCount;
        } catch (Exception e)
        {
            LOG.error("Failed to get vertex completed task count for sourceName={}", vname);
    ...
    private int getVertexTotalTaskCount(String vname)
    {
        ...
        try
        {
            totalCount = taskCommunicatorContext.getVertexTotalTaskCount(vname);
            return totalCount;
        } catch (Exception e)
        {
    ...
    {
        taskCommunicator.sendStateUpdate(nodeId, SourceStateUpdatedRequestProto.newBuilder().setQueryIdentifier(currentQueryIdentifier).setSrcName(sourceName).setState(Converters.fromVertexState(state)).build());
    }
