...
public class DruidSerDe extends AbstractSerDe
{
    ...
    private void initFromProperties(final Properties properties) throws SerDeException
    {
        ...
        {
            throw new SerDeException("Timestamp column (' " + DruidConstants.DEFAULT_TIMESTAMP_COLUMN + "') not specified in create table; list of columns is : " + properties.getProperty(serdeConstants.LIST_COLUMNS));
        }
        final List<PrimitiveTypeInfo> columnTypes = Utilities.getColumnTypes(properties).stream().map(TypeInfoFactory::getPrimitiveTypeInfo).collect(Collectors.toList()).stream().map(e -> e instanceof TimestampLocalTZTypeInfo ? tsTZTypeInfo : e).collect(Collectors.toList());
        final List<ObjectInspector> inspectors = columnTypes.stream().map(PrimitiveObjectInspectorFactory::getPrimitiveJavaObjectInspector).collect(Collectors.toList());
        columns = columnNames.toArray(new String[0]);
        types = columnTypes.toArray(new PrimitiveTypeInfo[0]);
        inspector = ObjectInspectorFactory.getStandardStructObjectInspector(columnNames, inspectors);
    }
    ...
    private void initFromDruidQueryPlan(Properties properties, String druidQuery)
    {
        ...
        final String fieldNamesProperty = Preconditions.checkNotNull(properties.getProperty(Constants.DRUID_QUERY_FIELD_NAMES, null));
        final String fieldTypesProperty = Preconditions.checkNotNull(properties.getProperty(Constants.DRUID_QUERY_FIELD_TYPES, null));
        ...
        } else
        {
            ...
            columnTypes = TypeInfoUtils.getTypeInfosFromTypeString(fieldTypesProperty).stream().map(e -> TypeInfoFactory.getPrimitiveTypeInfo(e.getTypeName())).map(primitiveTypeInfo ->
            {
    ...
    private static Object convertAsPrimitive(Object value, PrimitiveTypeInfo typeInfo) throws SerDeException
    {
        switch (typeInfo.getPrimitiveCategory())
        {
            ...
            case TIMESTAMPLOCALTZ:
                return new TimestampTZ(ZonedDateTime.ofInstant(Instant.ofEpochMilli(deserializeToMillis(value)), ((TimestampLocalTZTypeInfo) typeInfo).timeZone()));
            case DATE:
