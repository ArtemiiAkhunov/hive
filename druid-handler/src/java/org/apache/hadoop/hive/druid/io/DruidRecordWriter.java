...
public class DruidRecordWriter implements RecordWriter<NullWritable, DruidWritable>, FileSinkOperator.RecordWriter
{
    ...
    public DruidRecordWriter(DataSchema dataSchema, RealtimeTuningConfig realtimeTuningConfig, DataSegmentPusher dataSegmentPusher, int maxPartitionSize, final Path segmentsDescriptorsDir, final FileSystem fileSystem)
    {
        ...
        appenderator = Appenderators.createOffline("hive-offline-appenderator", this.dataSchema, tuningConfig, false, new FireDepartmentMetrics(), dataSegmentPusher, DruidStorageHandlerUtils.JSON_MAPPER, DruidStorageHandlerUtils.INDEX_IO, DruidStorageHandlerUtils.INDEX_MERGER_V9);
    ...
    private void pushSegments(List<SegmentIdWithShardSpec> segmentsToPush)
    {
        try
        {
            ...
            final Set<String> toPushSegmentsHashSet = segmentsToPush.stream().map(SegmentIdWithShardSpec::toString).collect(Collectors.toCollection(HashSet::new));
