...
public class TestDruidSerDe
{
    ...
    private static final Object[][] TIMESERIES_QUERY_RESULTS_RECORDS = new Object[][]{new Object[]{new TimestampTZ(Instant.ofEpochMilli(1325376000000L).atZone(ZoneOffset.UTC)), 0L, 1.0F, 2.2222F}, new Object[]{new TimestampTZ(Instant.ofEpochMilli(1325462400000L).atZone(ZoneOffset.UTC)), 2L, 3.32F, 4F}};
    ...
    private static final Object[][] GROUP_BY_QUERY_RESULTS_RECORDS = new Object[][]{new Object[]{(new TimestampTZ(Instant.ofEpochMilli(1325376000000L).atZone(ZoneOffset.UTC))), "India", "phone", 88L, 29.91233453, 60.32F}, new Object[]{(new TimestampTZ(Instant.ofEpochMilli(1325376012000L).atZone(ZoneOffset.UTC))), "Spain", "pc", 16L, 172.93494959, 6.333333F}};

    private static final Object[][] GB_MONTH_EXTRACTION_RESULTS_RECORDS = new Object[][]{new Object[]{(new TimestampTZ(Instant.ofEpochMilli(1325376000000L).atZone(ZoneOffset.UTC))), 1, 200L}, new Object[]{(new TimestampTZ(Instant.ofEpochMilli(1325376012000L).atZone(ZoneOffset.UTC))), 1, 400L}};
    ...
    public void setup() throws IOException
    {
        ...
        topNQueryResults = DruidStorageHandlerUtils.SMILE_MAPPER.writeValueAsBytes(DruidStorageHandlerUtils.JSON_MAPPER.readValue(TOPN_QUERY_RESULTS, new TypeReference<List<Result<TopNResultValue>>>()
        {
        }));
        groupByQueryResults = DruidStorageHandlerUtils.SMILE_MAPPER.writeValueAsBytes(DruidStorageHandlerUtils.JSON_MAPPER.readValue(GROUP_BY_QUERY_RESULTS, new TypeReference<List<Row>>()
        {
        }));
        groupByTimeExtractQueryResults = DruidStorageHandlerUtils.SMILE_MAPPER.writeValueAsBytes(DruidStorageHandlerUtils.JSON_MAPPER.readValue(GB_TIME_EXTRACTIONS_RESULTS, new TypeReference<List<Row>>()
        {
        }));
        groupByMonthExtractQueryResults = DruidStorageHandlerUtils.SMILE_MAPPER.writeValueAsBytes(DruidStorageHandlerUtils.JSON_MAPPER.readValue(GB_MONTH_EXTRACTIONS_RESULTS, new TypeReference<List<Row>>()
        {
        }));
        scanQueryResults = DruidStorageHandlerUtils.SMILE_MAPPER.writeValueAsBytes(DruidStorageHandlerUtils.JSON_MAPPER.readValue(SCAN_QUERY_RESULTS, new TypeReference<List<ScanResultValue>>()
        {
        }));
    }
    ...
    public void testDruidObjectSerializerwithNullTimestamp() throws Exception
    {
        ...
        tbl = createPropertiesSource(COLUMN_NAMES, COLUMN_TYPES);
        serDe.initialize(conf, tbl, null);
        Object[] row = new Object[]{null, new Text("dim1_val"), new HiveCharWritable(new HiveChar("dim2_v", 6)), new HiveVarcharWritable(new HiveVarchar("dim3_val", 8)), new DoubleWritable(10669.3D), new FloatWritable(10669.45F), new LongWritable(1113939), new IntWritable(1112123), new ShortWritable((short) 12), new ByteWritable((byte) 0), null
                // granularity
        };
    ...
    private static final Object[] ROW_OBJECT_2 = new Object[]{new TimestampTZ(Instant.ofEpochMilli(1377907200000L).atZone(ZoneOffset.UTC)), "dim1_val", new HiveChar("dim2_v", 6), new HiveVarchar("dim3_val", 8), 10669.3D, 10669.45F, 1113939L, 1112123, ((short) 12), ((byte) 0)};
    private static final DruidWritable DRUID_WRITABLE_2 = new DruidWritable(ImmutableMap.<String, Object>builder().put("__time", 1377907200000L).put("c0", "dim1_val").put("c1", "dim2_v").put("c2", "dim3_val").put("c3", 10669.3D).put("c4", 10669.45F).put("c5", 1113939L).put("c6", 1112123).put("c7", (short) 12).put("c8", (byte) 0).build());
