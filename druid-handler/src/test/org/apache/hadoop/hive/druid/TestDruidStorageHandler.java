...
public class TestDruidStorageHandler
{
    ...
    private DataSegment createSegment(String location, Interval interval, String version, ShardSpec shardSpec) throws IOException
    {
        ...
        return DataSegment.builder().dataSource(DATA_SOURCE_NAME).version(version).interval(interval).shardSpec(shardSpec).loadSpec(ImmutableMap.of("path", location)).size(1000L).build();
    }
    ...
    public void before() throws Throwable
    {
        ...
        config.set(String.valueOf(HiveConf.ConfVars.DRUID_SEGMENT_DIRECTORY), new Path(tableWorkingPath, "finalSegmentDir").toString());
        config.set("hive.druid.maxTries", "0");
        druidStorageHandler = new DruidStorageHandler(derbyConnectorRule.getConnector(), derbyConnectorRule.metadataTablesConfigSupplier().get());
    ...
    public void testPreCreateTableWhenDataSourceExists() throws MetaException, IOException
    {
        ...
        sqlMetadataStorageUpdaterJobHandler.publishSegments(segmentsTable, Collections.singletonList(dataSegment), DruidStorageHandlerUtils.JSON_MAPPER);
    ...
    {
        return connector.getDBI().withHandle(handle -> handle.createQuery(String.format("SELECT payload FROM %s WHERE used=true ORDER BY created_date ASC", metadataStorageTablesConfig.getSegmentsTable())).map((i, resultSet, statementContext) ->
        {
            try
            {
