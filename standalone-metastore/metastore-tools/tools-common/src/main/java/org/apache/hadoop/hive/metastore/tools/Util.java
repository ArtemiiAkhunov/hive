...
public final class Util
{
    ...
    public static class PartitionBuilder
    {
        ...
        Partition build()
        {
            ...
            List<String> partitionNames = table.getPartitionKeys().stream().map(FieldSchema::getName).collect(Collectors.toList());
            if (partitionNames.size() != values.size())
            {
                throw new RuntimeException("Partition values do not match table schema");
            }
            List<String> spec = IntStream.range(0, values.size()).mapToObj(i -> partitionNames.get(i) + "=" + values.get(i)).collect(Collectors.toList());
     ...
     */
    public static @Nullable URI getServerUri(@Nullable String host, @Nullable String portString) throws URISyntaxException
    {
        if (host == null)
        {
    ...
    {
        throwingSupplierWrapper(() -> addManyPartitions(client, dbName, tableName, parameters, arguments, npartitions));
    }
    ...
    public static List<String> filterMatches(@Nullable List<String> candidates, @Nullable Pattern[] positivePatterns, @Nullable Pattern[] negativePatterns)
    {
        ...
        final Pattern[] positive = (positivePatterns == null || positivePatterns.length == 0) ? MATCH_ALL_PATTERN : positivePatterns;
        final Pattern[] negative = negativePatterns == null ? EMPTY_PATTERN : negativePatterns;

        return candidates.stream().filter(c -> Arrays.stream(positive).anyMatch(p -> p.matcher(c).matches())).filter(c -> Arrays.stream(negative).noneMatch(p -> p.matcher(c).matches())).collect(Collectors.toList());
    }
}
