...
public class HiveStreamingConnection implements StreamingConnection
{
    ...
    {
        INACTIVE("I"), OPEN("O"), COMMITTED("C"), ABORTED("A"), PREPARED_FOR_COMMIT("P");
    ...
    public static class Builder
    {
        ...
        public Builder withTableObject(Table table)
        {
            ...
            this.isPartitioned = tableObject.getPartitionKeys() != null && !tableObject.getPartitionKeys().isEmpty();
            return this;
        }
        ...
        public HiveStreamingConnection connect() throws StreamingException
        {
            ...
            if ((writeId != -1 && tableObject == null) || (writeId == -1 && tableObject != null))
            {
    ...
    {
        return "{ metastore-uri: " + metastoreUri + ", " + "database: " + database + ", " + "table: " + table + ", " + "partitioned-table: " + isPartitionedTable() + ", " + "dynamic-partitioning: " + isDynamicPartitioning() + ", " + "username: " + username + ", " + "secure-mode: " + secureMode + ", " + "record-writer: " + recordWriter.getClass().getSimpleName() + ", " + "agent-info: " + agentInfo + ", " + "writeId: " + writeId + ", " + "statementId: " + statementId + " }";
    }
    ...
    public PartitionInfo createPartitionIfNotExists(final List<String> partitionValues) throws StreamingException
    {
        ...
        try
        {
            ...
            Partition partition = org.apache.hadoop.hive.ql.metadata.Partition.createMetaPartitionObject(tableObject, partSpec, location);
            ...
            {
                LOG.debug("Created partition {} for table {}", partName, tableObject.getFullyQualifiedName());
            }
        ...
        {
            throw new StreamingException("Unable to creation partition for values: " + partitionValues + " connection: " + toConnectionInfoString(), e);
        }
    ...
    {
        return recordWriter.getDeltaFileLocation(partitionValues, bucketId, minWriteId, maxWriteId, statementId, tableObject);
    }
            ...
            try (FileSystem fs = tableObject.getDataLocation().getFileSystem(conf))
            {
                if (BlobStorageUtils.isBlobStorageFileSystem(conf, fs))
                {
                    ...
                    try (FSDataOutputStream out = fs.create(path, false))
                    {
                        ...
                        {
                            throw new ConnectionError("The backing filesystem only supports transaction batch sizes of 1, but " + transactionBatchSize + " was requested.");
                        }
    ...
    private void beginNextTransaction() throws StreamingException
    {
        ...
        {
            LOG.info("Transaction batch {} is done. Rolling over to next transaction batch.", currentTransactionBatch);
    ...
    public void commitTransaction(Set<String> partitions, String key, String value) throws StreamingException
    {
        ...
        if (partitions != null)
        {
            for (String partition : partitions)
            {
                try
                {
                    PartitionInfo info = createPartitionIfNotExists(Warehouse.getPartValuesFromPartName(partition));
                    if (!info.isExists())
                    {
        ...
        this.partitions.addAll(currentTransactionBatch.getPartitions());
    ...
    private static IMetaStoreClient getMetaStoreClient(HiveConf conf, String metastoreUri, boolean secureMode, String owner) throws ConnectionError
    {
        ...
        {
            throw new ConnectionError("Error connecting to Hive Metastore URI: " + metastoreUri + ". " + e.getMessage(), e);
        }
    }
    ...
    {
        String key = (partitionValues == null) ? tableObject.getFullyQualifiedName() : partitionValues.toString();
    ...
    public void addWriteNotificationEvents() throws StreamingException
    {
        ...
        try
        {
            ...
            for (WriteDirInfo writeInfo : writePaths.values())
            {
                ...
                Hive.addWriteNotificationLog(conf, tableObject, writeInfo.getPartitionVals(), currentTxnId, currentWriteId, newFiles, null);
            }
