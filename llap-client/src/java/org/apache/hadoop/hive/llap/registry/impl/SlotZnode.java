...
public class SlotZnode implements Closeable
{
    ...
    private enum State
    {
        LATENT, INITIAL_SELECTION, AFTER_SELECTION, CLOSED
    }
    ...
    private void chooseSlotToTake() throws Exception
    {
        ...
        while (true)
        {
            ...
            if (slotToTake != currentSlot || !shouldFallBackOnCollision(approxWorkerCount))
            {
                break;
            }
            ++fallbackCount;
    ...
    private void startCreateCurrentNode()
    {
        ...
        try
        {
            ...
            client.create().withMode(CreateMode.EPHEMERAL).inBackground(backgroundCallback).forPath(createPath, data);
        } catch (Exception e)
        {
    ...
    private void handleCreatedNode(String path) throws Exception
    {
        ...
        {
            State localState = state.get();
            if (localState == State.CLOSED || localState == State.LATENT)
            {
                return;
            }
            if (state.compareAndSet(localState, State.AFTER_SELECTION))
            {
                break;
            }
        }
