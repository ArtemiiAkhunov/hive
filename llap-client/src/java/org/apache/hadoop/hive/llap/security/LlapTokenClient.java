...
public class LlapTokenClient
{
    ...
    public LlapTokenClient(Configuration conf)
    {
        ...
        retryPolicy = RetryPolicies.retryUpToMaximumTimeWithFixedSleep(16000, 2000l, TimeUnit.MILLISECONDS);
    }
    ...
    private ByteString getTokenWithUgi(final String appId, UserGroupInformation ugi) throws ServiceException
    {
        if (client == null)
        {
            client = new LlapManagementProtocolClientImpl(conf, clientInstance.getHost(), clientInstance.getManagementPort(), retryPolicy, socketFactory);
        }
        client.withUgi(ugi);
