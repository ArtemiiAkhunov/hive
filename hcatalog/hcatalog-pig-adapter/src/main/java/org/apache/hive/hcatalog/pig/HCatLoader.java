...
public class HCatLoader extends HCatBaseLoader
{
    ...
    {
        Table table = phutil.getTable(location, hcatServerUri != null ? hcatServerUri : PigHCatUtil.getHCatServerUri(job), PigHCatUtil.getHCatServerPrincipal(job), job);   // Pass job to initialize metastore conf overrides
    ...
    public ResourceSchema getSchema(String location, Job job) throws IOException
    {
        ...
        Table table = phutil.getTable(location, hcatServerUri != null ? hcatServerUri : PigHCatUtil.getHCatServerUri(job), PigHCatUtil.getHCatServerPrincipal(job),
    ...
    public ResourceStatistics getStatistics(String location, Job job) throws IOException
    {
        try
        {
            ...
            {
                throw new IOException("DB or table name unset. setLocation() must be invoked on this " + "loader to set them");
            }
            ResourceStatistics stats = new ResourceStatistics();
            long inputSize = -1;

            LinkedList<InputJobInfo> inputJobInfos = HCatUtil.getInputJobInfosFromConf(job.getConfiguration());
