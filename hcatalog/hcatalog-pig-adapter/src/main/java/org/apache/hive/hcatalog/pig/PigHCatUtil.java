...
class PigHCatUtil
{
    ...
    static
    {
        ...
        {
            LOG.info("This version of Pig does not support boolean fields. To enable " + "boolean-to-integer conversion, set the " + HCatConstants.HCAT_DATA_CONVERT_BOOLEAN_TO_INTEGER + "=true configuration parameter.");
        }
    }
    ...
    private static ResourceFieldSchema getResourceSchemaFromFieldSchema(HCatFieldSchema hfs) throws IOException
    {
        ...
        {
            rfSchema = new ResourceFieldSchema().setName(hfs.getName()).setDescription(hfs.getComment()).setType(getPigType(hfs)).setSchema(null); // no munging inner-schemas
        }
        return rfSchema;
    }
    ...
    protected static ResourceSchema getBagSubSchema(HCatFieldSchema hfs) throws IOException
    {
        ...
        } else
        {
            ...
            innerTupleFieldSchemas[0] = new ResourceFieldSchema().setName(innerFieldName).setDescription("The inner field in the tuple in the bag").setType(getPigType(arrayElementFieldSchema)).setSchema(null); // the element type is not a tuple - so no subschema
