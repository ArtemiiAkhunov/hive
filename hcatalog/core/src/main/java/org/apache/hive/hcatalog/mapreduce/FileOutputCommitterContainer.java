...
class FileOutputCommitterContainer extends OutputCommitterContainer
{
     ...
     * @throws IOException
     */
    public FileOutputCommitterContainer(JobContext context, org.apache.hadoop.mapred.OutputCommitter baseCommitter) throws IOException
    {
        ...
        if (dynamicPartitioningUsed && Boolean.parseBoolean((String) table.getProperty("EXTERNAL")) && jobInfo.getCustomDynamicPath() != null && jobInfo.getCustomDynamicPath().length() > 0)
        {
    ...
    private Partition constructPartition(JobContext context, OutputJobInfo jobInfo, String partLocnRoot, String dynPartPath, Map<String, String> partKVs, HCatSchema outputSchema, Map<String, String> params, Table table, FileSystem fs, HiveConf conf, HdfsUtils.HadoopFileStatus status) throws IOException
    {
     ...
     * @throws java.io.IOException
     */
    private void moveTaskOutputs(final Configuration conf, Path srcf, Path srcDir, Path destDir, boolean immutable) throws IOException
    {
        if (LOG.isDebugEnabled())
        {
            LOG.debug("moveTaskOutputs " + srcf + " from: " + srcDir + " to: " + destDir + " immutable: " + immutable);
        }
        ...
        if (customDynamicLocationUsed)
        {
            if (immutable && destFs.exists(destDir) && !org.apache.hadoop.hive.metastore.utils.FileUtils.isDirEmpty(destFs, destDir))
            {
                throw new HCatException(ErrorType.ERROR_DUPLICATE_PARTITION, "Data already exists in " + destDir + ", duplicate publish not possible.");
            }
        ...
        } else
        {
            ...
            while (!srcQ.isEmpty())
            {
                ...
                if (immutable && destFs.exists(finalOutputPath) && !org.apache.hadoop.hive.metastore.utils.FileUtils.isDirEmpty(destFs, finalOutputPath))
                {
     ...
     * @throws java.io.IOException
     */
    private void bulkMoveFiles(final Configuration conf, final FileSystem srcFs, final FileSystem destFs, final List<Pair<Path, Path>> pairs) throws IOException
    {
        if (pairs.isEmpty())
        {
            return;
        ...
        if (moveThreadsCount <= 0)
        {
            for (final Pair<Path, Path> pair : pairs)
            {
                ...
                {
                    throw new HCatException(ErrorType.ERROR_MOVE_FAILED, "Unable to move from " + srcP + " to " + dstP);
                }
            }
            return;
        }

        final ExecutorService pool = Executors.newFixedThreadPool(moveThreadsCount, new ThreadFactoryBuilder().setDaemon(true).setNameFormat("Move-Thread-%d").build());
    ...
    private boolean moveFile(final FileSystem srcFs, final Path srcf, final FileSystem destFs, final Path destf, final Configuration conf, final boolean canRename) throws IOException
    {
        ...
        if (!partitionsDiscovered)
        {
            ...
            } else
            {
                if ((maxDynamicPartitions != -1) && (status.length > maxDynamicPartitions))
                {
                    ...
                    throw new HCatException(ErrorType.ERROR_TOO_MANY_DYNAMIC_PTNS, "Number of dynamic partitions being created " + "exceeds configured max allowable partitions[" + maxDynamicPartitions + "], increase parameter [" + HiveConf.ConfVars.DYNAMIC_PARTITION_MAX_PARTS.varname + "] if needed.");
                }
    ...
    private void registerPartitions(JobContext context) throws IOException
    {
        ...
        try
        {
            ...
            if (!dynamicPartitioningUsed)
            {
                partitionsToAdd.add(constructPartition(context, jobInfo, tblPath.toString(), null, jobInfo.getPartitionValues(), jobInfo.getOutputSchema(), getStorerParameterMap(storer), table, fs, hiveConf, status));
            } else
            {
                for (Entry<String, Map<String, String>> entry : partitionsDiscoveredByPath.entrySet())
                {
                    partitionsToAdd.add(constructPartition(context, jobInfo, getPartitionRootLocation(entry.getKey(), entry.getValue().size()), entry.getKey(), entry.getValue(), jobInfo.getOutputSchema(), getStorerParameterMap(storer), table, fs, hiveConf, status));
                }
            }
            ...
            } else
            {
                ...
                {
                    HdfsUtils.setFullFileStatus(conf, status, status.getFileStatus().getGroup(), fs, new Path(p.getSd().getLocation()), true);
                }
            }
        } catch (Exception e)
        {
