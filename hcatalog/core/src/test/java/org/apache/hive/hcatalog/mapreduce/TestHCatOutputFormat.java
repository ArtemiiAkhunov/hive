...
public class TestHCatOutputFormat
{
    ...
    public void testGetTableSchema() throws Exception
    {
        ...
        HCatOutputFormat.setOutput(job, OutputJobInfo.create(dbName, tblName, new HashMap<String, String>()
        {{
            put("colname", "col_value");
        }}));

        HCatSchema rowSchema = HCatOutputFormat.getTableSchema(job.getConfiguration());
        assertEquals("Row-schema should have exactly one column.", 1, rowSchema.getFields().size());
        assertEquals("Row-schema must contain the data column.", "data_column", rowSchema.getFields().get(0).getName());
        assertEquals("Data column should have been STRING type.", serdeConstants.STRING_TYPE_NAME, rowSchema.getFields().get(0).getTypeString());
        ...
        assertEquals("Table-schema must contain the data column.", "data_column", tableSchema.getFields().get(0).getName());
        assertEquals("Data column should have been STRING type.", serdeConstants.STRING_TYPE_NAME, tableSchema.getFields().get(0).getTypeString());
        assertEquals("Table-schema must contain the partition column.", "colname", tableSchema.getFields().get(1).getName());
        assertEquals("Partition column should have been STRING type.", serdeConstants.STRING_TYPE_NAME, tableSchema.getFields().get(1).getTypeString());
