...
public class StatusDelegator extends TempletonDelegator
{
     ...
     */
    private static JobRequestExecutor<QueueStatusBean> jobRequest = new JobRequestExecutor<QueueStatusBean>(JobRequestExecutor.JobRequestType.Status, AppConfig.JOB_STATUS_MAX_THREADS, AppConfig.JOB_STATUS_TIMEOUT);
     ...
     */
    public QueueStatusBean run(final String user, final String id, boolean enableThreadPool) throws NotAuthorizedException, BadParam, IOException, InterruptedException, BusyException, TimeoutException, ExecutionException, TooManyRequestsException
    {
    ...
    private JobCallable<QueueStatusBean> getJobStatusCallableTask(final String user, final String id)
    {
        return new JobCallable<QueueStatusBean>()
        {
            ...
            public QueueStatusBean execute() throws NotAuthorizedException, BadParam, IOException, InterruptedException, BusyException
            {
                 ...
                 * to debug job issues.
                 */
                Thread.currentThread().setName(String.format("%s-%s-%s", JOB_STATUS_EXECUTE_THREAD_PREFIX, statusThreadId, Thread.currentThread().getId()));
    ...
    public QueueStatusBean run(final String user, final String id) throws NotAuthorizedException, BadParam, IOException, InterruptedException, BusyException, TimeoutException, ExecutionException, TooManyRequestsException
    {
