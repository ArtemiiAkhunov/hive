...
public class ZooKeeperStorage implements TempletonStorage
{
    ...
    {
        ZooKeeperHiveHelper xkHelper = ZooKeeperHiveHelper.builder().quorum(conf.get(ZK_HOSTS)).sessionTimeout(conf.getInt(ZK_SESSION_TIMEOUT, CuratorFrameworkFactory.builder().getSessionTimeoutMs())).baseSleepTime(1000).maxRetries(3).sslEnabled(conf.getBoolean(ZK_SSL_ENABLE, false)).keyStoreLocation(conf.get(ZK_KEYSTORE_LOCATION, "")).keyStorePassword(conf.get(ZK_KEYSTORE_PASSWORD, "")).keyStoreType(conf.get(ZK_KEYSTORE_TYPE, "")).trustStoreLocation(conf.get(ZK_TRUSTSTORE_LOCATION, "")).trustStorePassword(conf.get(ZK_TRUSTSTORE_PASSWORD, "")).trustStoreType(conf.get(ZK_TRUSTSTORE_TYPE, "")).build();
    ...
    private void setFieldData(Type type, String id, String name, String val) throws Exception
    {
        try
        {
            zk.create().withMode(CreateMode.PERSISTENT).withACL(Ids.OPEN_ACL_UNSAFE).forPath(makeFieldZnode(type, id, name), val.getBytes(ENCODING));
    ...
    @Override
    public void saveField(Type type, String id, String key, String val) throws NotFoundException
    {
        try
        {
            if (val != null)
