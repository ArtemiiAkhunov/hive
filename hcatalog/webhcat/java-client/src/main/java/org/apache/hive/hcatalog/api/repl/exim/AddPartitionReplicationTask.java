...
public class AddPartitionReplicationTask extends ReplicationTask
{
    ...
    public Iterable<? extends Command> getDstWhCommands()
    {
        ...
        return Iterables.transform(addPartitionMessage.getPartitions(), new Function<Map<String, String>, Command>()
        {
            ...
            public Command apply(@Nullable Map<String, String> ptnDesc)
            {
                return new ImportCommand(dstDbName, dstTableName, ptnDesc, dstStagingDirProvider.getStagingDirectory(ReplicationUtils.getUniqueKey(getEvent().getEventId(), addPartitionMessage.getDB(), // Note - important to retain the same key as the export
                        addPartitionMessage.getTable(), ptnDesc)), false, event.getEventId());
            }
        });
    }
}
